...
import { C as parseStrictNonNegativeInteger, P as resolvePositiveTimerTimeoutMs, y as parseDateStringTimestampMs } from "./number-coercion-CLj0HTDM.js";
...
import { d as resolveClaudeSonnet5ModelIdentity, o as resolveClaudeFable5ModelIdentity, u as resolveClaudeOpus5ModelIdentity } from "./src-5i09w5fd.js";
...
import { i as streamSimple, n as completeSimple } from "./stream-CXbsApnu.js";
...
import { createSseByteGuard, createToolArgumentPreviewSchedule, parseStreamingJson, parseTerminalToolCallArguments } from "@openclaw/ai/internal/runtime";
...
async function runLoop(initialContext, newMessages, initialConfig, signal, emit, streamFn, runtime) {
	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			turnOpen = false;
			if (fatalToolBatch) throw fatalToolBatch.error;
			if (await stopIfAborted()) return;
			if (terminateRun) {
...
async function executeToolCalls(currentContext, assistantMessage, config, signal, emit, criticalToolLoopSeen) {
	...
	if (config.toolExecution === "sequential" || hasSequentialToolCall) return executeToolCallsSequential(batch, toolCalls);
...
function validatedToolCallIds(batch, calls) {
	return calls.filter((call) => batch.validated.get(call)?.kind === "validated").map((call) => call.id);
}
async function executeToolCallsSequential(batch, toolCalls) {
	...
	for (let callIndex = 0; callIndex < toolCalls.length; callIndex++) {
		...
		let finalized;
		if (!("kind" in entry)) finalized = entry;
		else try {
			if (!batch.signal?.aborted) {
				const steering = getSteeringAtCheckpoint(batch.config);
				steeringMessages = Array.isArray(steering) ? steering : await steering;
			...
			if (launches.rejected) {
				...
				batch.lifecycle?.releaseSkippedCalls(validatedToolCallIds(batch, toolCalls.slice(callIndex)));
			...
			if (!executed) {
				...
				if (!startedOutcome) throw new Error("Tool launch finished without an outcome");
	...
	if (!fatal && !batch.signal?.aborted && steeringMessages.length === 0 && skippedStartIndex === toolCalls.length) {
		const steering = getSteeringAtCheckpoint(batch.config);
		steeringMessages = Array.isArray(steering) ? steering : await steering;
	}
	if (steeringMessages.length > 0) batch.lifecycle?.releaseSkippedCalls(validatedToolCallIds(batch, toolCalls.slice(skippedStartIndex)));
...
async function executeToolCallsParallel(batch, toolCalls) {
	const entries = [];
	try {
		for (const toolCall of toolCalls) {
			const entry = await prepareToolCallEntry(batch, toolCall);
		...
		const skippedToolCallIds = [...steeringMessages.length > 0 ? entries.flatMap((entry) => "kind" in entry ? [entry.toolCall.id] : []) : [], ...steeringMessages.length > 0 ? toolCalls.slice(entries.length).map((toolCall) => toolCall.id) : []];
		if (skippedToolCallIds.length > 0) batch.lifecycle?.releaseSkippedCalls(skippedToolCallIds);
		const orderedFinalizedCalls = entries.map((entry) => "kind" in entry ? void 0 : entry);
		let fatal;
		if (steeringMessages.length > 0) for (const [index, entry] of entries.entries()) {
			if (!("kind" in entry)) continue;
			entry.execution.dispose();
			orderedFinalizedCalls[index] = (await completeUnstartedToolCall(batch, entry.toolCall, {
				args: entry.execution.args,
				details: {
					status: "skipped",
					deniedReason: "steering"
			...
			if (fatal) {
				const unstartedIds = entries.slice(rejectedIndex).flatMap((entry) => "kind" in entry ? [entry.toolCall.id] : []);
				batch.lifecycle?.releaseSkippedCalls([...unstartedIds, ...validatedToolCallIds(batch, toolCalls.slice(entries.length))]);
			...
			await Promise.all(launched.started.map(async ({ index, entry, outcome }) => {
				try {
					const finalized = await finalizeExecutedToolCall(batch, entry, await outcome, entry.execution.args);
					await emitToolExecutionEnd(finalized, batch.emit);
					orderedFinalizedCalls[index] = finalized;
				} finally {
					entry.execution.dispose();
		...
		if ((batch.signal?.aborted || fatal) && entries.length < toolCalls.length) for (let i = entries.length; i < toolCalls.length; i++) {
			const skippedToolCall = toolCalls[i];
			if (!skippedToolCall) continue;
			const completed = await completeUnstartedToolCall(batch, skippedToolCall, fatal ? {
				details: TOOL_ADMISSION_FAILURE_DETAILS,
				message: TOOL_ADMISSION_FAILURE_MESSAGE
			} : void 0);
			await emitToolResultMessage(completed.message, batch.emit);
		...
		return {
			messages,
			steeringMessages,
			terminate: shouldTerminateToolBatch(orderedFinalizedCalls.filter((entry) => entry !== void 0)),
			terminateRun: false,
			...fatal ? { fatal } : {}
		};
	} finally {
...
function readTurnTaintMetadata(message) {
	const record = asOptionalRecord(Reflect.get(message, "__openclaw"));
	if (!record) return;
	return {
...
function capCompactionSummary(summary, maxChars = MAX_COMPACTION_SUMMARY_CHARS, preservedSuffix = "") {
	...
	return `${truncateUtf16Safe(suffix ? summary.slice(0, -suffix.length) : summary, budget)}${SUMMARY_TRUNCATED_MARKER}${suffix}`;
}
...
function latestUserTurnCompleted(messages) {
	...
	for (const message of messages.toReversed()) {
		...
		if (!sawTurnTail && (message.role === "assistant" || message.role === "toolResult")) {
			sawTurnTail = true;
			completed = message.role === "assistant" && message.stopReason === "stop" && message.content.some((block) => block.type === "text" && block.text.trim().length > 0);
		}
	}
	return false;
}
...
function prepareCompaction(pathEntries, settings) {
	...
	if (prevBoundaryIndex >= 0) {
		...
		if (prevBoundary?.type === "compaction" && !prevBoundary.fromHook) previousSummaryDetails = parseCompactionDetails(prevBoundary.details);
	...
	const retainedTurnSuffixMessages = [];
	if (cutPoint.isSplitTurn) {
		...
		for (let i = cutPoint.firstKeptEntryIndex; i < boundaryEnd; i++) {
			const entry = effectiveEntries.at(i);
			if (!entry || i > cutPoint.firstKeptEntryIndex && isTurnStartEntry(entry)) break;
			const msg = getMessageFromEntryForCompaction(entry);
			if (msg) retainedTurnSuffixMessages.push(msg);
		}
	}
	...
	return ok({
		...
		...cutPoint.isSplitTurn ? { splitTurnCompleted: latestUserTurnCompleted([...turnPrefixMessages, ...retainedTurnSuffixMessages]) } : {},
		tokensBefore,
		previousSummary,
...
async function compact(preparation, model, apiKey, headers, customInstructions, signal, thinkingLevel, streamFn, runtime) {
	...
	const latestContextBudget = MAX_COMPACTION_SUMMARY_CHARS - 46 - fileOperations.length - preservedHistoryChars;
	latestContext = `${capCompactionSummary(latestContext, latestContextBudget)}${fileOperations}`;
	return ok({
		summary: capCompactionSummary(`${historyResult.value}${latestContext}`, MAX_COMPACTION_SUMMARY_CHARS, latestContext),
		firstKeptEntryId,
		tokensBefore,
		details: {
			readFiles,
...
async function generateBranchSummary(entries, options) {
	...
	const response = options.streamFn ? await consumeAgentCoreStream(options.streamFn(model, context, streamOptions)) : await resolveAgentCoreCompleteFn(options.runtime)(model, context, streamOptions);
...
//#endregion
export { formatFileOperations as A, COMPACTION_SUMMARY_SUFFIX as B, findTurnStartIndex as C, shouldCompact as D, prepareCompaction as E, parseCommandArgs as F, uuidv7 as G, convertToLlm as H, substituteArgs as I, BRANCH_SUMMARY_PREFIX as L, CompactionError as M, InvalidSummaryOutputError as N, MAX_FILE_OPS_SECTION_CHARS as O, buildSessionContext as P, BRANCH_SUMMARY_SUFFIX as R, findCutPoint as S, getLastAssistantUsage as T, runAgentLoop as U, bashExecutionToText as V, getAgentToolExecutionContext as W, calculateContextTokens as _, DEFAULT_MAX_LINES as a, estimateContextTokens as b, truncateLine as c, generateBranchSummary as d, prepareBranchEntries as f, SUMMARY_TRUNCATED_MARKER as g, MAX_COMPACTION_SUMMARY_CHARS as h, DEFAULT_MAX_BYTES as i, serializeConversation as j, computeFileLists as k, truncateTail as l, IMAGE_BLOCK_TOKENS as m, openClawAgentCoreRuntime as n, formatSize as o, DEFAULT_COMPACTION_SETTINGS as p, streamProxy as r, truncateHead as s, Agent as t, collectEntriesForBranchSummaryFromBranches as u, capCompactionSummary as v, generateSummary as w, estimateTokens as x, compact as y, COMPACTION_SUMMARY_PREFIX as z };
