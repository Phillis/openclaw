...
import { w as isNodesMachineOutput } from "./argv-CCdO9MSu.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-DXuFeGZ6.js";
import { a as withConsoleLogsRoutedToStderrForJson } from "./json-output-mode-XMIkPNjr.js";
...
import { g as randomIdempotencyKey } from "./call-Bwn2P4nz.js";
import { t as quoteCliArg } from "./quote-cli-arg-BriMa9wW.js";
import { c as parseCameraSnapPayload, d as writeBase64ToFile, f as writeCameraClipPayloadToFile, l as resolveCameraClipTarget, o as cameraTempPath, p as writeCameraPayloadToFile, r as screenRecordTempPath, s as parseCameraClipPayload, t as parseScreenRecordPayload, u as resolveCameraSnapTargets } from "./nodes-screen-qR9sh2i2.js";
...
import { n as getNodesTheme, r as runNodesCommand, t as formatConnectionFlagReminder } from "./cli-utils-BTXACHTk.js";
...
function registerNodesNotifyCommand(nodes) {
	nodesCallOpts(nodes.command("notify").description("Send a local notification on a node").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--title <text>", "Notification title").option("--body <text>", "Notification body").option("--sound <name>", "Notification sound").addOption(new Option("--priority <passive|active|timeSensitive>", "Notification priority").choices([
		...
		await runNodesCommand("notify", async () => {
			...
			const invokeParams = {
				nodeId: await resolveCliNodeId(opts, normalizeOptionalString(opts.node) ?? ""),
...
function registerNodesPairingCommands(nodes) {
	...
	nodesCallOpts(nodes.command("rename").description("Rename a paired node (display name override)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").requiredOption("--name <displayName>", "New display name").action(async (opts) => {
		await runNodesCommand("rename", async () => {
			...
			if (!name) throw new Error(`--name must not be empty. Run ${formatCliCommand("openclaw nodes list")} to see paired nodes, then rerun with --name <displayName>.`);
			const nodeId = await resolveCliNodeId(opts, normalizeOptionalString(opts.node) ?? "");
...
function registerNodesPushCommand(nodes) {
	nodesCallOpts(nodes.command("push").description("Send an APNs test push to an iOS node").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--title <text>", "Push title", "OpenClaw").option("--body <text>", "Push body").option("--environment <sandbox|production>", "Override APNs environment").action(async (opts) => {
		await runNodesCommand("push", async () => {
			...
			if (opts.environment && environment !== "sandbox" && environment !== "production") throw new Error("invalid --environment (use sandbox|production)");
			const nodeId = await resolveCliNodeId(opts, normalizeOptionalString(opts.node) ?? "");
			const params = {
				nodeId,
				title: normalizeOptionalString(opts.title) || "OpenClaw",
...
function registerNodesScreenCommands(nodes) {
	nodesCallOpts(nodes.command("screen").description("Capture screen recordings from a paired node").command("record").description("Capture a short screen recording from a node (prints the saved path)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--screen <index>", "Screen index (0 = primary)", "0").option("--duration <ms|10s>", "Clip duration (ms or 10s)", "10000").option("--fps <fps>", "Frames per second", "10").option("--no-audio", "Disable microphone audio capture").option("--out <path>", "Output path").option("--invoke-timeout <ms>", "Node invoke timeout in ms (default 120000)", "120000").action(async (opts) => {
		await runNodesCommand("screen record", async () => {
			...
			const written = await writeBase64ToFile(opts.out ?? screenRecordTempPath({ ext: parsed.format || "mp4" }), parsed.base64);
			if (opts.json) {
				defaultRuntime.writeJson({ file: {
					path: written.path,
