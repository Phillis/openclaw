...
import { t as BROWSER_NATIVE_HOST_NAME } from "./extension-native-host-CyFQarIX.js";
...
const FOUNDATION_CHROME_WEB_STORE_URL = `https://chromewebstore.google.com/detail/openclaw/${FOUNDATION_CHROME_WEB_STORE_EXTENSION_ID}`;
...
function launcherMatchesOrigins(params) {
	...
	return new RegExp(`^#!/bin/sh\\n${escapeRegExp(OWNED_LAUNCHER_MARKER)}\\nexport OPENCLAW_STATE_DIR=${quotedValue}\\n(?:export OPENCLAW_CONFIG_PATH=${quotedValue}\\n)?exec ${command} "\\$@"\\n$`, "u").test(params.content);
}
async function assertPrivateNativeHostFile(target, executable, platform) {
	await assertOwnedPath(target, "file");
	if (platform === "win32") return;
	const mode = (await fs$1.lstat(target)).mode & 511;
	if ((mode & 63) !== 0 || executable && (mode & 64) === 0) throw new Error("native host file has unsafe mode");
}
...
async function inspectRegistration(root, deps, expectedPathExtensionIds) {
	...
	try {
		...
		if (Object.keys(manifest).length !== exactKeys.length || !exactKeys.every((key) => Object.hasOwn(manifest, key)) || manifest.description !== NATIVE_HOST_DESCRIPTION || manifest.type !== "stdio" || !validOrigins || JSON.stringify(origins) !== JSON.stringify(canonicalOrigins) || expectedOrigins !== null && JSON.stringify(origins) !== JSON.stringify(expectedOrigins)) throw new Error("native host manifest does not contain exact allowed origins");
		await assertPrivateNativeHostFile(expectedLauncher, true, deps.platform ?? process.platform);
		if (!launcherMatchesOrigins({
			content: await fs$1.readFile(expectedLauncher, "utf8"),
			manifestPath,
...
async function installChromeExtensionBootstrap(params) {
	...
	if (preRegisteredRoots > 0) params.onProgress?.(`Native bootstrap is ready. Add OpenClaw from the Chrome Web Store: ${FOUNDATION_CHROME_WEB_STORE_URL}. For development, load unpacked from ${installed}.`);
...
async function browserExtensionStatus(params) {
	...
	const missingRegistration = chromeProductRoots(deps).some((root) => {
		if (!(discovery.discovered.some((entry) => entry.product === root.product) || discovery.storeDiscovered.some((entry) => entry.product === root.product))) return false;
		const manifestPath = path.join(root.nativeManifestDir, `${BROWSER_NATIVE_HOST_NAME}.json`);
		const registration = registrations.find((entry) => entry.manifestPath === manifestPath);
		return registration?.state !== "owned" || JSON.stringify(registration.extensionIds) !== JSON.stringify(expectedExtensionIds(predictedIds));
	});
	return {
		platform,
		...
		registrations,
		manualSetupRequired: platform === "win32" || installedCopy.present && !installedCopy.owned || discovery.discovered.length === 0 && discovery.storeDiscovered.length === 0 || discovery.identityMismatches.length > 0 || missingRegistration,
		issues: [
...
async function repairOwnedChromeExtensionNativeHosts(params) {
	...
	if (!before.installedCopy.owned || before.discovered.length === 0 && before.storeDiscovered.length === 0) return {
		changes: [],
		warnings: []
	};
	const changes = [];
	...
	for (const root of chromeProductRoots(deps)) {
		...
		if (!(before.discovered.some((entry) => entry.product === root.product) || before.storeDiscovered.some((entry) => entry.product === root.product))) continue;
		...
		try {
			const idsAreCurrent = JSON.stringify(registration.extensionIds) === JSON.stringify(expectedExtensionIds(predictedIds));
...
//#endregion
export { repairOwnedChromeExtensionNativeHosts as a, normalizeExtensionInstallWaitMs as i, browserExtensionStatus as n, resolveChromeExtensionLoadPath as o, installChromeExtensionBootstrap as r, uninstallChromeExtensionNativeHosts as s, FOUNDATION_CHROME_WEB_STORE_URL as t };
