...
import { t as buildChannelOutboundSessionRoute } from "./core-wiAGUTYa.js";
import "./channel-setup-o7ff3LvZ.js";
import { h as dispatchInboundDirectDm } from "./channel-inbound-BllqRtTK.js";
...
import { D as loadKeys, Dt as randomBytes, E as generateAndStoreKeys, H as reserveReefIdentityBinding, I as finalizeReefIdentityBinding, L as loadReefIdentityBinding, O as openStores, P as assertReefIdentityBinding, T as ReefInboxCursorStore, V as releaseReefIdentityReservation, _t as appendInboxRead, a as REEF_OUTBOUND_DELIVERY_TTL_MS, at as REEF_MAX_PLAINTEXT_BYTES, bt as canonicalBytes, c as isReefPairingApprovalToken, ft as fingerprint, gt as appendAudit, l as openReefTrustStore, mt as parseHandleEpoch, pt as formatHandleEpoch, st as bodyHash } from "./doctor-state-paths-Cb0p5p6T.js";
import { _ as verifyReceipt, c as createReefWebSocket, d as isRetryableReefRelayFailure, f as PipelineError, g as confirmDelivery, h as InvalidDeliveryReceiptError, i as ReefInboxConnection, l as isDefinitiveReefRegistrationFailure, m as composeOutbound, n as assertLegacyReefKeysMigrated, o as ReefTransportClient, p as composeInbound, r as ReefFriendManager, s as abortableSleep, u as isReefOwnershipRejection, v as createAnthropicGuard, y as createOpenAiGuard } from "./legacy-key-guard-JdFdABMf.js";
...
import { i as getReefRuntime, n as getActiveReef, r as getOptionalReefRuntime, t as createReefRuntimeAuthority } from "./runtime-DYnXNGON2.js";
...
function resolveReefInboundDispatchContent(message) {
	const threadId = message.thread ?? (message.replyTo ? void 0 : message.id);
	return {
...
function matchesReefToolTarget(target, toolContext) {
	const normalizedCurrent = normalizeReefTarget(toolContext?.currentMessagingTarget ?? toolContext?.currentChannelId ?? "");
	return normalizedCurrent !== void 0 && normalizeReefTarget(target) === normalizedCurrent;
}
const reefPlugin = {
	id: "reef",
	meta: {
	...
	threading: {
		...
		matchesToolContextTarget: ({ target, toolContext }) => matchesReefToolTarget(target, toolContext),
		resolveReplyToMode: () => "all",
		buildToolContext: ({ context, hasRepliedRef }) => {
			const currentTarget = context.To?.trim() || void 0;
			return {
		...
		resolveAutoThreadId: ({ to, toolContext }) => matchesReefToolTarget(to, toolContext) ? toolContext?.currentThreadTs : void 0
	},
	...
	gateway: { startAccount: async (ctx) => {
		...
		const inbox = new ReefInboxConnection(transport, (entries) => processReefInboxEntriesInOrder({
			...
			onState: (state) => {
				if (ctx.abortSignal.aborted || state !== "connected") return;
