...
import "./realtime-bootstrap-context-AjkqAJA4.js";
import "./text-utility-runtime-BNhX-3os.js";
import { s as GatewayDispatchEvents } from "./v10-BDbFcnZN.js";
import { c as ReadyListener, f as VoiceStateUpdateListener, kt as getGuildVoiceState, l as ResumedListener, v as isUnknownDiscordVoiceStateError } from "./discord-Cr3IyWY2.js";
import { t as getDiscordRuntime } from "./runtime-Dg4d9hPu.js";
import { n as formatDiscordUserTag } from "./format-21A1IG8x.js";
import { a as normalizeDiscordSlug, h as resolveDiscordOwnerAccess } from "./allow-list-CtAHaFe6.js";
...
import { n as resolveFetchedDiscordThreadLikeChannelContext } from "./thread-channel-context-bEb9-kxA.js";
import { t as buildDiscordGroupSystemPrompt } from "./inbound-context-9s_ff2u4.js";
import { _ as decodeOpusStreamChunks, a as VOICE_CONNECT_READY_TIMEOUT_MS, b as maybeControlDiscordVoiceAgentRun, c as isVoiceChannel, d as resolveVoiceTimeoutMs, g as decodeOpusStream, h as createDiscordOpusPlaybackStream, i as PLAYBACK_READY_TIMEOUT_MS, l as logVoiceVerbose, n as CAPTURE_FINALIZE_GRACE_MS, o as VOICE_RECONNECT_GRACE_MS, s as isDiscordRealtimeVoiceMode, t as loadDiscordVoiceSdk, u as resolveDiscordVoiceMode, v as writeVoiceWavFile, x as formatVoiceLogPreview, y as formatVoiceIngressPrompt } from "./sdk-runtime-qZdKg60I.js";
import { o as OpusError } from "./dist-BtXesAa1.js";
...
async function runDiscordVoiceAgentTurn(params) {
	...
	const payloads = (await getDiscordRuntime().agent.runCommandFromIngress({
...
async function processDiscordVoiceSegment(params) {
	...
	if (streamFailure && !entry.ttsStreamFallbackWarned) {
		...
		logger$4.warn(`discord voice: streaming TTS failed provider=${streamFailure.provider} reasonCode=${streamFailure.reasonCode}; using file fallback`);
	}
	...
	params.enqueuePlayback(entry, async () => {
		...
		const cancelStoppedPlayback = () => (!playbackStarted || entry.sessionLifecycle.status === "stopped") && playbackLifecycle.abort();
		try {
			...
			const resource = voiceSdk.createAudioResource(createDiscordOpusPlaybackStream(input), { inputType: voiceSdk.StreamType.Opus });
			entry.player.play(resource);
			await voiceSdk.entersState(entry.player, voiceSdk.AudioPlayerStatus.Playing, AbortSignal.any([AbortSignal.timeout(PLAYBACK_READY_TIMEOUT_MS), playbackLifecycle.signal]));
			playbackStarted = true;
			await voiceSdk.entersState(entry.player, voiceSdk.AudioPlayerStatus.Idle, playbackLifecycle.signal);
...
var DiscordVoiceSessions = class {
	...
	async joinUnlocked(params, options, authority) {
		...
		if (existingEntry) existingEntry.stop();
		...
		const player = isDiscordRealtimeVoiceMode(voiceMode) ? voiceSdk.createAudioPlayer({ behaviors: { maxMissedFrames: REALTIME_PLAYBACK_MAX_MISSED_FRAMES } }) : voiceSdk.createAudioPlayer();
		...
		if (isVoiceSessionStopped(entry) || this.params.destroyed() || authority && !authority.isCurrent()) {
			entry.stop(`${this.params.destroyed() ? "manager stopped" : "join cancelled"} during setup guild ${guildId} channel ${channelId}`);
			return {
				ok: false,
...
var DiscordVoiceManager = class {
	...
	async resolveAccessTarget(params) {
		const [guild, channel] = await Promise.all([this.client.fetchGuild(params.guildId).catch(() => null), this.client.fetchChannel(params.channelId).catch(() => null)]);
	...
	enqueueAutoJoin(entry) {
		const task = (this.autoJoinTasks.get(entry.guildId) ?? Promise.resolve()).catch(() => void 0).then(async () => await this.reconcileAutoJoinEntry(entry)).finally(() => {
	...
	async reconcileAutoJoinEntry(entry) {
		if (this.destroyed) return;
