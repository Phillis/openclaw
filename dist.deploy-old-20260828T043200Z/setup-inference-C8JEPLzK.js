...
import { n as createQuickstartNotePrompter } from "./setup-apply-BuUBt11i.js";
...
async function runSetupInferenceTest(params) {
	...
	try {
		...
		let result;
		if (plan.runner === "cli") result = await (deps.runCliAgent ?? (await import("./cli-runner-M-g8dvU1.js")).runCliAgent)({
		...
		else result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-DjHmTlnD.js")).runEmbeddedAgent)({
			preparedRunAdmission,
			sessionId,
			sessionKey,
			sessionManager,
