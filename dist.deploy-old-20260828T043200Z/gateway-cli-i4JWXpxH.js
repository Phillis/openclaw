...
import { o as resolveGatewayRpcOptions, r as callGatewayFromCliWithTransport, s as resolveGatewayRpcOptionsWithLocalPort, t as addGatewayClientOptions } from "./gateway-rpc-4LDXqcsd.js";
import { t as addGatewayServiceCommands } from "./register-service-commands-DICM0n7v.js";
...
function addGatewayRestartHandoffCommands(gateway) {
	...
	restartHandoff.command("consume").description("Atomically consume a gateway restart handoff").helpOption(false).allowUnknownOption().allowExcessArguments().option("--expected-pid [pid]", "PID of the exited gateway process", collectExpectedPid, []).option("--json", MACHINE_OUTPUT_JSON_OPTION_DESCRIPTION).action(async (opts, command) => {
		...
		try {
			const { consumeGatewayRestartHandoffSync } = await import("./restart-handoff-B4T3_jQQ.js");
...
const gatewayStatusModuleLoader = createLazyImportLoader(() => import("./gateway-status-BfWTgwJL.js"));
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-CeqxuFQG.js"));
...
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-DTREDmKw.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(() => import("./diagnostic-stability-bundle-BOkinCX5.js"));
const supportExportModuleLoader = createLazyImportLoader(() => import("./diagnostic-support-export-BOHYkFBD.js"));
const daemonStatusGatherModuleLoader = createLazyImportLoader(() => import("./status.gather-QTbimqHu.js"));
...
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayAuthErrorJson, formatGatewayClientRequestErrorJson, formatGatewayTransportErrorJson } = await import("./call-Dplee5Oc.js");
...
function registerGatewayCli(program, deps = {}) {
	...
	gateway.command("auth-token").description("Reveal the configured shared Gateway token").option("--show", "Print the token to an interactive terminal", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			...
			const { gatewayAuthTokenCommand } = await import("./gateway-auth-token-DYnbh4nr.js");
