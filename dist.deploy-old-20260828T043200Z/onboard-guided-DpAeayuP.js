...
import { a as requestTelemetryConsent, o as requireRiskAcknowledgement } from "./setup.shared-Bl7UavWq.js";
...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk, agentName) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted, setupAgentName) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-DTJUIQlO.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	const localOnboarding = custodianMode ? await import("./local-onboarding-state-BKaH097l.js") : void 0;
	...
	if (!custodianMode) {
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-BjP8hruo.js")).runSetupMemoryImportStep)({
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-CZejj76D.js")).applySystemAgentSetup;
			...
			const appliedSnapshot = localSetup?.status === "pending" ? await (await import("./setup-recovery-CKEb_aBP.js")).completeLocalSetupRecovery({
				owner: localSetup,
	...
	if (wantsDiscovery) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-BjP8hruo.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-D7DtHZeE.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
	...
	if (opts.tui !== true) {
		if ((await (deps.runBrowserHandoff ?? (await import("./onboard-browser-handoff-BS5rCDS3.js")).runBrowserHatchHandoff)({
			config: persistedConfig,
			prompter,
