...
import { a as getRuntimeConfigSnapshot, c as getRuntimeConfigSourceSnapshot, u as hashRuntimeConfigValue } from "./runtime-snapshot-Cv5MaU8U.js";
...
import { c as resolveProfileUnusableUntil, i as isAuthCooldownBypassedForProvider } from "./usage-state-C0QBjJnZ.js";
import { n as isStoredCredentialCompatibleWithAuthProvider, t as isConfiguredAwsSdkAuthProfileForProvider } from "./order-C7dw_-HZ.js";
import { a as NON_ENV_SECRETREF_MARKER, c as isNonSecretApiKeyMarker, n as CUSTOM_LOCAL_AUTH_MARKER, o as SECRETREF_ENV_HEADER_MARKER_PREFIX, s as isKnownEnvApiKeyMarker } from "./model-auth-markers-CYmICvL9.js";
...
function resolveUsableCustomProviderApiKey(params) {
	const { providerConfig: customProviderConfig, ref: apiKeyRef } = resolveProviderConfigSecretInput(params.cfg, params.provider);
	if (apiKeyRef) {
...
async function resolveProviderEntryApiKeyBinding(params) {
	...
	try {
		const { resolveApiKeyForProfile } = await import("./oauth-BMUka2Tk.js");
...
function resolveLiteralProviderConfigApiKeyAuth(params) {
	const { providerConfig, ref } = resolveProviderConfigSecretInput(params.cfg, params.provider);
	const apiKey = ref ? void 0 : normalizeOptionalSecretInput(providerConfig?.apiKey);
