...
import { o as resolveBundledStaticCatalogModel } from "./model.static-catalog-BYYtvCmS.js";
import { n as acquireReadOnlyPreparedModelRuntime } from "./prepared-model-runtime-afzKiwqZ.js";
...
import { n as buildRuntimeCompatibleToolInventory } from "./tools-effective-inventory-build-BDxL_BvT.js";
...
async function resolveEffectiveToolInventoryRuntimeModelContextAsync(params) {
	...
	if (staticContext.runtimeModel) return staticContext;
	const provider = normalizeProviderId(params.modelProvider ?? "");
	const modelId = params.modelId?.trim() ?? "";
	if (!provider || !modelId) return {};
	const agentId = params.agentId?.trim() || resolveSessionAgentId({ config: params.cfg });
