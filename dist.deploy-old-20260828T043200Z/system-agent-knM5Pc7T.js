...
import { n as describeSystemAgentPersistentOperation } from "./operations-CCkoW4hg.js";
...
import { t as getUpdateAvailable } from "./update-startup-Ddnh09d7.js";
import { n as getHealthCache } from "./health-state-DU1bpQDq.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-DoHto_Pc.js";
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-DTCJs8zH.js");
