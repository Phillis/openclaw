...
import { u as toSafeImportPath } from "./plugin-module-loader-cache-DNYw5tMM.js";
import { at as normalizeAgentRunTerminalReplySnapshot, it as mergeAgentRunTerminalReplySnapshot, nt as selectDeliverableSessionsReply } from "./openclaw-state-db-CeAO_dqo.js";
import { n as SILENT_REPLY_TOKEN } from "./tokens-DbQz-n_m.js";
import { c as isAgentEventLifecycleGenerationCurrent, s as getAgentEventLifecycleGeneration, u as onAgentEvent } from "./agent-events-CcZImb5w.js";
import { s as getAgentRunContext } from "./agent-run-registry-t4kvUyNQ.js";
import { n as clearGatewayContextResolver, r as getGatewayContextResolver, t as bindGatewayContextResolver, u as withPluginRuntimeRegistryScope } from "./gateway-request-scope-B19X7f09.js";
...
import { _ as runWithGatewayIndependentRootWorkAdmission, c as isGatewayRestartDraining, t as GatewayDrainingError, v as runWithGatewayIndependentRootWorkContinuation } from "./gateway-work-admission-CTDt7IQ1.js";
import { l as emitSessionLifecycleEvent } from "./session-history-eviction-6hHpt56d.js";
import { m as registerSessionMaintenancePreserveKeysProvider } from "./types-BEJRKmOU.js";
import { a as runWithoutOwnedSessionTranscriptWrites } from "./transcript-write-context-LK0MNWC3.js";
import { h as recordSubagentTerminalState } from "./session-state-events-BkuyPMaw.js";
import "./sessions-CdrF1uzY.js";
import { n as ToolInputError } from "./common-CI1GnPjt.js";
import { t as jsonResult } from "./tool-results-BCM3fdVS.js";
import { P as isProvisionalSubagentKillTask, V as createSubagentTaskBackingDetail, Z as getTaskFlowById, y as updateTask } from "./task-registry-LJQ782u-.js";
import { a as buildAgentRunTerminalOutcomeFromWaitResult, i as buildAgentRunTerminalOutcomeFromLifecycleEvent, t as AGENT_RUN_TERMINAL_RETRY_GRACE_MS } from "./agent-run-terminal-outcome-DafVNgmX.js";
import { N as getTasksByRunScope, p as SUBAGENT_KILL_TASK_ERROR } from "./task-registry-activity-DGPs-dN3.js";
import { a as finalizeTaskRunByRunId, n as createQueuedTaskRun, o as findDetachedTaskRun, r as createRunningTaskRun, u as startTaskRunByRunId } from "./detached-task-runtime-BAMhKtnX.js";
import { n as resolveAgentTimeoutMs } from "./timeout-DlFI6Ssz.js";
import { i as shouldSuppressSubagentRecoverySessionEffects } from "./subagent-recovery-state-1AUqfKvI.js";
import { u as retireSessionMcpRuntimeForSessionKey } from "./agent-bundle-mcp-manager-api-DUhEi3qH.js";
...
const subagentAnnounceLoader = createLazyImportLoader(() => import("./subagent-announce-DLKn1qqs.js"));
const browserCleanupLoader$1 = createLazyImportLoader(() => import("./browser-lifecycle-cleanup-B0i2Gk0B.js"));
...
const defaultSubagentRegistryDeps = {
	...
	maybeWakeRequesterAfterAllChildrenSettled: async (params) => (await import("./subagent-announce.requester-settle-wake-CSwyigPS.js")).maybeWakeRequesterAfterAllChildrenSettled(params)
};
...
const subagentRegistryPluginRuntimeLoader = createLazyPromiseLoader(() => import("./runtime-plugins-DWlWmnDo.js"));
...
function runWithSubagentCleanupWorkAdmission(run) {
	return isGatewayRestartDraining() ? runWithGatewayIndependentRootWorkAdmission(run) : runWithGatewayIndependentRootWorkContinuation(run);
}
...
function suspendPendingFinalDelivery(context, args) {
	...
	try {
		...
		for (const key of Object.keys(args.entry)) Reflect.deleteProperty(args.entry, key);
...
const startSubagentAnnounceCleanupFlow$1 = (context, runId, entry) => {
	...
	const announceParams = {
		...
		isCompletionDeliveryAllowed: () => entry.suppressCompletionDelivery !== true && context.isCleanupAttemptCurrent(runId, entry, cleanupGeneration),
...
const browserCleanupLoader = createLazyImportLoader(() => import("./browser-lifecycle-cleanup-B0i2Gk0B.js"));
...
function setCanonicalTaskBackingDetail(params) {
	try {
...
var SubagentRecoveryManager = class extends SubagentWaitManager {
	constructor(..._args) {
		...
		this.replaceSubagentRunAfterSteer = (replaceParams) => {
			...
			if ((source.expectsCompletionMessage === false ? "missing" : setCanonicalTaskBackingDetail({
				runtime: "subagent",
				...
				log$3.warn("failed to bind replacement subagent task generation; restored source lease", { runId: next.runId });
				if (replaceParams.persistenceFailure === "throw") throw new Error(`failed to bind replacement subagent task generation for ${next.runId}`);
				return false;
			}
			try {
...
function createSubagentRegistryRestorer(config) {
	const { runs, resumedRuns, deps, getGatewayContextResolver, persist, persistOrThrow, settleRequesterTurn, ensureListener, startSweeper, resumeRun, listSwarmRunsForGroup, startQueuedSubagentRun, terminateAcceptedRestoredCollectorRun, cleanupCollectorLaunchResources, settleFailedQueuedSubagentLaunch, completeCollectorLaunchCleanup, warn } = config;
	...
	function activateRestoredRuns() {
		...
		if (restoreState !== "succeeded" || activated) return;
		const cfg = deps().getRuntimeConfig();
				...
				enqueueSwarmRun({
					...
					start: async () => {
						await runWithGatewayIndependentRootWorkAdmission(async () => {
							...
							const gatewayRunId = readGatewayRunId(await gatewayRuntime.dispatchAgent(request.params, request.timeoutMs, launch.authorization ? {
...
const restartRecoveryLoader = createLazyImportLoader(() => import("./subagent-registry-restart-recovery-BMrV_ZVV.js"));
...
function resumeSubagentRun(runId) {
	...
	if (typeof entry.execution.endedAt === "number" && entry.execution.endedAt > 0) {
		...
		const orphanReason = resolveSubagentRunOrphanReason({ entry });
		if (orphanReason && reconcileOrphanedRun({
			runId,
			entry,
			reason: orphanReason,
			source: "resume",
...
const subagentRestorer = createSubagentRegistryRestorer({
	...
	resumedRuns,
	deps: () => subagentRegistryDeps,
	getGatewayContextResolver: () => fenceScheduledGatewayContextResolver(activeGatewayContextResolver),
	persist: persistSubagentRuns,
	persistOrThrow: persistSubagentRunsOrThrow,
...
function registerSubagentRun(params) {
	subagentRunManager.registerSubagentRun({
		...params,
		gatewayContextResolver: params.gatewayContextResolver ?? fenceScheduledGatewayContextResolver(activeGatewayContextResolver)
	});
}
...
function activateSubagentRegistry(resolveGatewayContext) {
	const lifecycleGatewayContextResolver = fenceScheduledGatewayContextResolver(resolveGatewayContext);
	activeGatewayContextResolver = resolveGatewayContext;
	for (const entry of subagentRuns.values()) if (!getGatewayContextResolver(entry)) bindGatewayContextResolver(entry, lifecycleGatewayContextResolver);
...
//#endregion
export { activateSwarmRun as A, replaceSubagentRunAfterSteerCore as C, settleRequesterAfterSessionSpawns as D, settleFailedQueuedSubagentLaunch as E, reserveSwarmRun as M, prependAgentSteeringPrompt as N, startQueuedSubagentRun as O, releaseSubagentRunKillClaim as S, scheduleSubagentRegistrySweep as T, markSubagentRunForSteerRestart as _, clearSubagentRunSteerRestart as a, registerSubagentRun as b, discardSubagentTerminalDelivery as c, getSubagentRunsByRunIds as d, getSwarmRunByLaunchReplayKey as f, markRequesterTurnYielded as g, listSwarmRunsForGroup as h, claimSubagentRunKill as i, removeQueuedSwarmRun as j, createStructuredOutputTool as k, getSubagentDeliveryBacklogPressure as l, leasePendingAgentSteeringItems as m, activateSubagentRegistry as n, completeCollectorLaunchCleanup as o, initSubagentRegistry as p, adoptPausedSubagentRunForFollowUp as r, countActiveRunsForSession as s, ackPendingAgentSteeringItems as t, getSubagentRunByRunId as u, markSubagentRunTerminated as v, resumeSubagentRun as w, releasePendingAgentSteeringItems as x, recordSwarmStructuredOutput as y };
