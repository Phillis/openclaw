...
import { D as resolveExpiresAtMsFromDurationMs, g as isFutureDateTimestampMs, t as MAX_DATE_TIMESTAMP_MS } from "./number-coercion-CLj0HTDM.js";
...
import { t as migrateSqliteSchemaToStrict } from "./sqlite-strict-EqLr_Ju4.js";
import { t as configureSqliteConnectionPragmas } from "./sqlite-wal-BHpwckP_.js";
import "./number-runtime-Cy4drVnh.js";
import "./string-coerce-runtime-C8jKEm3h.js";
import "./plugin-state-runtime-BlMBP7Yx.js";
...
const DEFAULT_WORKBOARD_DISPATCH_OWNER = "workboard-dispatcher";
...
function workboardCardConsumesOwnerSlot(card, now) {
	const claim = card.metadata?.claim;
	const activeClaim = claim && isFutureDateTimestampMs(claim.expiresAt, { nowMs: now });
	return !card.metadata?.archivedAt && !isWorkboardClaimReclaimable(claim, now) && (card.status === "running" || card.status !== "done" && activeClaim || card.execution?.status === "running");
}
function workboardCardSlotOwner(card, now) {
	const claim = card.metadata?.claim;
	return (claim && (now === void 0 || isFutureDateTimestampMs(claim.expiresAt, { nowMs: now })) ? claim.ownerId : void 0) || card.agentId || "workboard-dispatcher";
}
...
function groupByCardId(rows) {
	...
	for (const row of rows) {
		...
		else grouped.set(cardId, [row]);
	}
	return grouped;
}
function loadCardChildRows(db) {
	const byTable = /* @__PURE__ */ new Map();
	for (const table of CARD_CHILD_TABLES) byTable.set(table, groupByCardId(db.prepare(`SELECT * FROM ${table} ORDER BY card_id ASC, ordinal ASC`).all()));
	const workerProtocol = /* @__PURE__ */ new Map();
	for (const row of db.prepare("SELECT * FROM workboard_worker_protocol").all()) {
		const cardId = stringValue(row, "card_id");
		if (cardId) workerProtocol.set(cardId, row);
	}
	return {
		byTable,
		workerProtocol
...
function childRows(db, table, cardId, preloaded) {
...
function workerProtocolRow(db, cardId, preloaded) {
	if (preloaded) {
...
function readLabels(db, cardId, preloaded) {
	return childRows(db, "workboard_card_labels", cardId, preloaded).flatMap((row) => {
		const label = stringValue(row, "label");
		return label ? [label] : [];
	});
}
function readEvents(db, cardId, preloaded) {
...
function readMetadata(db, row, preloaded) {
	...
	const attachments = childRows(db, "workboard_card_attachments", cardId, preloaded).map((child) => {
		const entry = {
			id: requiredString(child, "id"),
	...
	const diagnostics = childRows(db, "workboard_card_diagnostics", cardId, preloaded).map((child) => ({
	...
	const notifications = childRows(db, "workboard_card_notifications", cardId, preloaded).map((child) => {
		const entry = {
			id: requiredString(child, "id"),
			kind: requiredString(child, "kind"),
			createdAt: requiredNumber(child, "created_at"),
...
function readCard(db, row, preloaded) {
	const card = {
		id: requiredString(row, "id"),
...
var WorkboardSqliteCardStore = class {
	...
	async registerIfAbsent(key, value) {
		...
		return runSqliteImmediateTransactionSync(this.db, () => {
			if (this.db.prepare("SELECT 1 FROM workboard_cards WHERE id = ?").get(key)) return false;
	...
	async registerIfUpdatedAt(key, value, expectedUpdatedAt) {
		this.validatePayload(key, value);
		return runSqliteImmediateTransactionSync(this.db, () => {
			const current = this.db.prepare("SELECT updated_at FROM workboard_cards WHERE id = ?").get(key);
			if (!isRecord(current) || numberValue(current, "updated_at") !== expectedUpdatedAt) return false;
			insertCard(this.db, value.card);
			return true;
		});
	}
	async claimIfOwnerAvailable(key, value, expectedUpdatedAt, ownerId, now) {
		...
		return runSqliteImmediateTransactionSync(this.db, () => {
			const current = this.db.prepare("SELECT updated_at FROM workboard_cards WHERE id = ?").get(key);
			if (!isRecord(current) || numberValue(current, "updated_at") !== expectedUpdatedAt) return "conflict";
			const rows = this.db.prepare("SELECT * FROM workboard_cards WHERE id <> ?").all(key);
			const preloaded = loadCardChildRows(this.db);
			for (const row of rows) {
				const card = readCard(this.db, row, preloaded);
				if (workboardCardConsumesOwnerSlot(card, now) && workboardCardSlotOwner(card) === ownerId) return "owner_busy";
			}
			insertCard(this.db, value.card);
			return "updated";
		});
	...
	async delete(key) {
		return runSqliteImmediateTransactionSync(this.db, () => this.deleteCard(key)).changes > 0;
	}
	deleteCard(key) {
		this.db.prepare(`
	...
	async entries() {
		const rows = this.db.prepare("SELECT * FROM workboard_cards ORDER BY created_at ASC, id ASC").all();
		const preloaded = loadCardChildRows(this.db);
		return rows.map((row) => ({
			key: requiredString(row, "id"),
			value: {
				version: 1,
				card: readCard(this.db, row, preloaded)
			}
		}));
	}
	...
	async register(key, value) {
		...
		this.db.prepare(`
          ...
          ON CONFLICT(id) DO UPDATE SET
            ...
            updated_at = excluded.updated_at,
            archived_at = excluded.archived_at
        `).run(board.id, bindNull(board.name), bindNull(board.description), bindNull(board.icon), bindNull(board.color), bindNull(board.automationJobId), jsonValue(board.defaultWorkspace), jsonValue(board.orchestration), board.createdAt, board.updatedAt, bindNull(board.archivedAt));
	}
	async lookup(key) {
		...
		return {
			...
			board: {
				...
				...stringValue(row, "automation_job_id") ? { automationJobId: stringValue(row, "automation_job_id") } : {},
...
//#endregion
export { DEFAULT_WORKBOARD_DISPATCH_OWNER as a, MAX_CARD_METADATA_BYTES as c, RUNNING_HEARTBEAT_STALE_MS as d, addWorkboardDurationMs as f, workboardCardSlotOwner as g, workboardCardConsumesOwnerSlot as h, DEFAULT_CLAIM_TTL_MS as i, POSITION_STEP as l, secondsToDurationMs as m, resolveWorkboardSqlitePath as n, MAX_ATTACHMENT_BYTES as o, isWorkboardClaimReclaimable as p, BLOCKED_TOO_LONG_MS as r, MAX_CARDS as s, createWorkboardSqliteStores as t, READY_STRANDED_MS as u };
