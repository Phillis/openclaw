...
import { i as resolveProviderPolicySurface } from "./provider-public-artifacts-qUJeTQDG.js";
...
function resolveThinkingCatalogEntry(params) {
	...
	return params.catalog?.find((entry) => selectedCatalogKey !== void 0 && buildCatalogModelKey(normalizeProviderId(entry.provider), entry.id) === selectedCatalogKey);
}
...
function appendCatalogAdvancedThinkingLevels(profile, compat, thinkingLevelMap, agentRuntime) {
	if (thinkingLevelMap) {
		...
		profile.levels = profile.levels.filter(({ id }) => id === "adaptive" || id === "ultra" || thinkingLevelMap[id] !== null);
	}
	...
	for (const effort of compat?.supportedReasoningEfforts ?? []) {
		...
		if ((level === "adaptive" || level === "xhigh" || level === "max") && (level === "adaptive" || thinkingLevelMap?.[level] !== null)) {
...
function resolveThinkingProfile(params) {
	...
	appendCatalogAdvancedThinkingLevels(profile, context.compat, context.thinkingLevelMap, params.agentRuntime);
	return profile;
}
function supportsThinkingLevel(provider, model, level, catalog, agentRuntime, configuredReasoning) {
...
function isThinkingLevelSupported(params) {
	return supportsThinkingLevel(params.provider, params.model, params.level, params.catalog, params.agentRuntime, params.configuredReasoning);
}
