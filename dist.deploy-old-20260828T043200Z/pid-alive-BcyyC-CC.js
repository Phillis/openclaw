...
function getPlatformProcessStartTime(pid) {
	try {
		...
		const startedAt = childProcess.execFileSync(windows ? "powershell.exe" : "/bin/ps", args, {
			encoding: "utf8",
			env: {
				...process.env,
				LC_ALL: "C",
		...
		return Number.isFinite(startedAtMs) ? Math.floor(startedAtMs / (windows ? 1 : 1e3)) : null;
	} catch {
		return null;
	}
}
...
function getFileLockProcessStartTime(pid) {
	...
	return process.platform === "darwin" || process.platform === "win32" ? getPlatformProcessStartTime(pid) : getProcessStartTime(pid);
}
//#endregion
