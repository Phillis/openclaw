...
import { a as OPENCLAW_DATABASE_SCHEMA_DOCS_URL, o as OPENCLAW_SQLITE_BUSY_TIMEOUT_MS } from "./openclaw-state-db-contract-C1w8kMxr.js";
import { An as executeSqliteQuerySync, Dt as assertOpenClawStateDatabaseForMaintenance, Fn as describeRunningOpenClawBuild, Ft as isOpenClawStateStartupRepairableSchemaIssue, It as OPENCLAW_STATE_SCHEMA_SQL, Kt as resolveOpenClawRegisteredAgentDatabasePath, Ln as readSqliteUserVersion, Mn as getNodeSqliteKysely, Mt as STATE_PERSISTENT_SCHEMA_COMPATIBILITY, Nt as getOpenClawStateRuntimeSchema, Ot as assertOpenClawStateDatabaseOwner, Pt as isOpenClawStateFirstUseSchemaIssue, Tn as collectSqliteSchemaIssues, Yt as resolveOpenClawStateSqlitePath, jt as OPENCLAW_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY, w as inspectOpenClawStateOwnershipFromDatabase } from "./openclaw-state-db-CeAO_dqo.js";
import { h as clearNodeSqliteKyselyCacheForDatabase, r as resolveImmutableSqliteFileUri, t as openNodeSqliteDatabase } from "./node-sqlite-_e3IvfT7.js";
import { t as assertSqliteIntegrity } from "./sqlite-integrity-D3VwDKmB.js";
import { t as assertOpenClawAgentDatabaseForMaintenance } from "./openclaw-agent-db-maintenance-DDqVWNe-.js";
...
async function preflightOpenClawStateDatabasePath(databasePath) {
	...
	try {
		...
		ownership = inspectOpenClawStateOwnershipFromDatabase(database, resolvedPath);
		if (foundVersion < 13) return result("migration-required", { requiresWrite: true });
...
function preflightOpenClawDatabaseSchemas(options) {
	...
	try {
		...
		if (options.verifyCurrentSchemaShape === true && stateVersion === 13) try {
