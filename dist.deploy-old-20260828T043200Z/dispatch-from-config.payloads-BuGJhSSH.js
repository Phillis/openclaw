...
import { a as getReplyPayloadMetadata, i as copyReplyPayloadMetadata, l as isReplyPayloadStatusNotice, u as isReplyPayloadTerminalContent } from "./reply-payload-BeeUJOmJ.js";
import { t as runAbortableTimeout } from "./with-timeout-DxxscSuL.js";
import { t as beginReplyOperationFinalizationWork } from "./reply-run-finalization-lease-Ds9-0UNB.js";
import { n as RUN_STALE_TAKEOVER_MS } from "./diagnostic-run-activity-CxbnPTtN.js";
import { o as prepareReplyPayloadForDispatcher } from "./reply-dispatcher-DRSctPVt.js";
...
function createBlockReplyPipeline(params) {
	...
	const sendPayload = (payload, bypassSeenCheck = false) => {
		...
		sendChain = sendChain.then(async () => {
			...
			if (!isStatusNotice) {
				didStream = true;
				if (isTerminalContent && hasOutboundReplyContent(payload, { trimText: true })) didStreamTerminalReply = true;
			}
		}).catch((err) => {
	...
	return {
		...
		hasSentPayload: (payload) => {
			...
			const assistantMessageIndex = getReplyPayloadMetadata(payload)?.assistantMessageIndex;
			const streamedFragments = assistantMessageIndex === void 0 ? streamedTextFragmentsByMessage.values() : [streamedTextFragmentsByMessage.get(assistantMessageIndex) ?? []];
			for (const fragments of streamedFragments) if (fragments.length > 0 && normalize(fragments.join("")) === target) return true;
			return false;
		},
