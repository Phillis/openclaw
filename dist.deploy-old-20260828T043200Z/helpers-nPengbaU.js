...
import { p as resolveChannelPreviewStreamMode } from "./streaming-B93dETXe.js";
...
import { r as renderMarkdownIRChunksWithinLimit } from "./text-chunking-CJz4kAsi.js";
...
import { i as normalizeAllowFrom, o as resolveTelegramEffectiveDmPolicy, r as isSenderAllowed, t as expandTelegramAllowFromWithAccessGroups } from "./access-groups-D9ipmCdc.js";
...
//#endregion
//#region extensions/telegram/src/text-width.ts
const TELEGRAM_GRAPHEME_SEGMENTER = new Intl.Segmenter(void 0, { granularity: "grapheme" });
const TELEGRAM_WIDE_CODE_POINT_PATTERN = /[\u1100-\u115F\u2E80-\u9FFF\uA000-\uA4FF\uAC00-\uD7AF\uF900-\uFAFF\uFE30-\uFE4F\uFF01-\uFF60\uFFE0-\uFFE6\u{20000}-\u{2FA1F}]/u;
const EMOJI_PRESENTATION_PATTERN = /\p{Emoji_Presentation}/u;
const EXTENDED_PICTOGRAPHIC_PATTERN = /\p{Extended_Pictographic}/u;
const NON_PRINTING_ONLY_PATTERN = /^(?:\p{Default_Ignorable_Code_Point}|\p{Control}|\p{Format}|\p{Mark})+$/u;
const EMOJI_PRESENTATION_SELECTOR = "️";
const KEYCAP_COMBINING_MARK = "⃣";
function telegramMonospaceGraphemeWidth(grapheme) {
	if (grapheme.includes(KEYCAP_COMBINING_MARK) || EMOJI_PRESENTATION_PATTERN.test(grapheme) || grapheme.includes(EMOJI_PRESENTATION_SELECTOR) && EXTENDED_PICTOGRAPHIC_PATTERN.test(grapheme)) return 2;
	if (TELEGRAM_WIDE_CODE_POINT_PATTERN.test(grapheme)) return 2;
	return NON_PRINTING_ONLY_PATTERN.test(grapheme) ? 0 : 1;
}
function telegramMonospaceWidth(text) {
	let width = 0;
	for (const { segment } of TELEGRAM_GRAPHEME_SEGMENTER.segment(text)) width += telegramMonospaceGraphemeWidth(segment);
	return width;
}
function padTelegramMonospaceCell(text, width) {
	return `${text}${" ".repeat(Math.max(0, width - telegramMonospaceWidth(text)))}`;
}
function renderTelegramMonospaceGrid(rows, options = {}) {
	const columnCount = Math.max(...rows.map((row) => row.length), 0);
	const widths = Array.from({ length: columnCount }, () => 3);
	for (const row of rows) for (let index = 0; index < columnCount; index += 1) widths[index] = Math.max(widths[index] ?? 3, telegramMonospaceWidth(row[index] ?? ""));
	const renderRow = (row) => `| ${widths.map((width, index) => padTelegramMonospaceCell(row[index] ?? "", width)).join(" | ")} |`;
	const lines = rows.map(renderRow);
	if (options.headerSeparator && lines.length > 0) lines.splice(1, 0, `| ${widths.map((width) => "-".repeat(width)).join(" | ")} |`);
	return lines.join("\n");
}
//#endregion
...
function renderTelegramRichHtmlRawTableFallback(tableHtml, rows) {
	return `<pre><code>${escapeHtml([rows.length > 0 ? telegramHtmlToPlainTextFallback(TELEGRAM_HTML_CAPTION_PATTERN.exec(tableHtml)?.[1] ?? "").trim() : "", rows.length > 0 ? renderTelegramMonospaceGrid(rows) : stripTelegramHtmlForPlainText(tableHtml).trim()].filter(Boolean).join("\n"))}</code></pre>\n\n`;
}
...
//#endregion
export { resolveTelegramPreviewStreamMode as $, hasBotMention as A, renderTelegramTextEntities as B, resolveTelegramThreadSpec as C, buildSenderName as D, buildSenderLabel as E, normalizeForwardedContext as F, markdownToTelegramHtmlChunks as G, escapeTelegramHtml as H, resolveTelegramPrimaryMedia as I, splitTelegramHtmlChunks as J, renderTelegramHtmlText as K, resolveTelegramRichMessageBody as L, hasLeadingBotCommandAddressedToOtherBot as M, isBinaryContent as N, extractTelegramLocation as O, joinTelegramTextParts as P, decodeTelegramHtmlEntities as Q, resolveTelegramRichMessagePlaceholder as R, resolveTelegramStreamMode as S, withResolvedTelegramForumFlag as T, markdownToTelegramChunks as U, countTelegramHtmlVisibleCharacters as V, markdownToTelegramHtml as W, wrapFileReferencesInHtml as X, telegramHtmlToPlainTextFallback as Y, renderTelegramMonospaceGrid as Z, resolveTelegramForumThreadId as _, buildTelegramInboundOriginTarget as a, resolveTelegramMessageThreadSpec as b, buildTelegramThreadParams as c, extractTelegramForumFlag as d, getCachedTelegramForumFlag as f, resolveTelegramForumFlag as g, resolveTelegramCommandAuthorization as h, buildTelegramGroupPeerId as i, hasBotMentionInText as j, getTelegramTextParts as k, buildTypingThreadParams as l, resolveTelegramBotHasTopicsEnabled as m, buildGroupLabel as n, buildTelegramParentPeer as o, isTelegramCommandsAllowFromConfigured as p, resolveTelegramHtmlVisibleText as q, buildTelegramGroupFrom as r, buildTelegramRoutingTarget as s, TelegramPairingStoreReadError as t, describeReplyTarget as u, resolveTelegramGroupAllowFromContext as v, shouldUseTelegramDmThreadSession as w, resolveTelegramReplyId as x, resolveTelegramMessageForumFlagHint as y, resolveTelegramRichMessageText as z };
