...
import { An as executeSqliteQuerySync, Mn as getNodeSqliteKysely, R as ensureWorkerEnvironmentNodeEnrollmentSchema, d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, jn as executeSqliteQueryTakeFirstSync } from "./openclaw-state-db-CeAO_dqo.js";
...
function createWorkerEnvironmentStore(options = {}) {
	...
	return {
		...
		hasPendingNodeEnrollmentSetup(setupIdInput, deviceIdInput) {
			const setupId = setupIdInput.trim();
			const deviceId = deviceIdInput.trim();
			if (!setupId || !deviceId) return false;
			const db = read();
			return executeSqliteQuerySync(db, query(db).selectFrom("worker_environments").select("environment_id").where("node_setup_id", "=", setupId).where("destroy_requested_at_ms", "is", null).where((eb) => eb.or([eb.and([eb("state", "=", "provisioning"), eb("node_device_id", "is", null)]), eb.and([eb("state", "in", [
				"provisioning",
				"bootstrapping",
				"ready",
				"idle",
				"attached"
			]), eb("node_device_id", "=", deviceId)])])).limit(2)).rows.length === 1;
		},
		ensureNodeEnrollment(environmentIdInput) {
			...
			return write((db) => {
				...
				const completedDeviceId = executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("device_pair_setup_completions").select("device_id").where("setup_id", "=", setupId))?.device_id ?? null;
		...
		transition(input) {
			...
			return write((db) => {
				...
				const nodeDeviceId = patch.nodeDeviceId === void 0 ? current.nodeDeviceId ?? null : patch.nodeDeviceId === null ? null : required(patch.nodeDeviceId, "node device id");
