...
function removeNativeCreatedFileIfStillPinned(params) {
	...
	try {
		...
		if (!parentPathStat.isSymbolicLink() && parentPathStat.dev === parentFdStat.dev && parentPathStat.ino === parentFdStat.ino && !target.isSymbolicLink() && sameFileIdentityForCleanup(target, params.created)) fs.rmSync(targetPath);
	} catch {}
}
			...
			try {
				...
				if (options.onCompromised && interval !== void 0 && interval > 0) {
					createdHeld.compromiseTimer = setInterval(() => {
						returnedHandle.verifyStillHeld().catch(() => false).then((stillHeld) => {
