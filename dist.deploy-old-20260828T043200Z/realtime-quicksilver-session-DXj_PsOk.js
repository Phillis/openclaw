import { l as readRequestBodyWithLimit } from "./http-body-DthsuKdw.js";
import { i as resolveAcceptedBrowserOrigin } from "./origin-check-Bai6m4aI.js";
import { m as resolveOpenAICodexAuthIdentity, s as resolveProviderAuthProfileApiKey } from "./provider-auth-DI4TAoBi.js";
import "./webhook-request-guards-BYzmIdMp.js";
import { n as isOpenAIGptLiveModel } from "./realtime-quicksilver-BdMyAyC5.js";
import { c as createOpenAIQuicksilverCall, f as resolveOpenAIQuicksilverVoice, l as hangupOpenAIRealtimeCall, r as buildOpenAIQuicksilverSession } from "./realtime-quicksilver-wire-CI6aAMtV.js";
import { t as OpenAIQuicksilverDelegationController } from "./realtime-quicksilver-delegation-controller-D7fe30Kp.js";
...
function createOpenAIQuicksilverBrowserSessionBroker(params) {
	...
	const broker = {
		...
		createBrowserSession: async (request, auth) => {
			...
			if (isGptLive && !request.runAgentConsult) throw new Error("OpenAI GPT-Live requires the Gateway agent-consult runtime");
			if (!isGptLive) {
				if (!request.gaSession) throw new Error("OpenAI GA realtime browser sessions require an initial session policy");
				if (request.gaSession.model !== model) throw new Error("OpenAI GA realtime session policy model must match the requested model");
			}
			prunePendingOffers();
	...
	const handleOffer = async (req, res) => {
		...
		try {
			...
			const sessionConfig = isOpenAIGptLiveModel(offer.request.model) ? buildOpenAIQuicksilverSession({
			...
			if (!sessionConfig) throw new Error("OpenAI GA realtime browser sessions require an initial session policy");
