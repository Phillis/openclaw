...
import { t as formatPluginBundleFormat } from "./plugins-list-format-B1-18TGM.js";
...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, buildPluginDiagnosticsReport, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-0EKylA5g.js");
	const { loadInstalledPluginIndexInstallRecords } = await import("./installed-plugin-index-records-Q_FQEIvw.js");
	...
	if (opts.all) {
		...
		const report = runtimeInspect ? tracePluginLifecyclePhase("runtime plugin registry load", () => buildPluginDiagnosticsReport(runtimeReportParams), {
			command: "inspect",
			all: true
	...
	if (!targetPlugin) {
		if (id === "skill-workshop") {
			const { detectSkillWorkshopToolPolicyDiagnostic } = await import("./tool-policy-diagnostic-Cj9XXPQM.js");
