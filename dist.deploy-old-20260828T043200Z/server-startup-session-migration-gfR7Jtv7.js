...
import { r as isSessionSqliteMigrationWarning } from "./doctor-session-sqlite-types-4f8oCqeR.js";
import { t as runSessionStartupMigration } from "./startup-migration-CENTdRHn.js";
...
async function runStartupSessionMigration(params) {
	if (await runSessionStartupMigration(params)) await runStartupSessionSqliteImport(params);
...
async function reconcileStartupSessionTranscriptIndexes(params) {
	...
	const reconcile = params.deps?.reconcileSessionTranscriptIndexes ?? (await import("./session-transcript-reconcile-IDlGYG83.js")).reconcileSessionTranscriptIndexes;
...
async function runStartupSessionSqliteImport(params) {
	...
	const runDoctorSessionSqlite = params.deps?.runDoctorSessionSqlite ?? (await import("./doctor-session-sqlite-DrcqMdmM.js")).runDoctorSessionSqlite;
	let report;
	try {
...
async function restoreFailedStartupSessionSqliteRun(params, report, blockingIssues) {
	...
	if (!restoreSessionSqliteMigrationRun || !writeSessionSqliteMigrationFailureReports) {
		const doctorModule = await import("./doctor-session-sqlite-DrcqMdmM.js");
