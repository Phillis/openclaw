...
import { n as getCurrentPluginMetadataSnapshot, s as createPluginIdScopeSet } from "./current-plugin-metadata-snapshot-CKAJM6x9.js";
...
import { i as passesManifestOwnerBasePolicy, n as isActivatedManifestOwner } from "./manifest-owner-policy-D8q1qLbp.js";
...
function resolveProviderOwnershipContext(params) {
	...
	return {
		manifestRegistry: params.manifestRegistry ?? metadataSnapshot?.manifestRegistry ?? resolveManifestRegistry({
			config,
			workspaceDir: params.workspaceDir,
			env,
...
function resolveOwningPluginIdsForCliBackendFromContext(normalizedBackend, context) {
	...
	const deduped = sortUniqueStrings(context.manifestRegistry.plugins.filter((plugin) => plugin.cliBackends.some((backendId) => normalizeProviderId(backendId) === normalizedBackend) || (plugin.setup?.cliBackends ?? []).some((backendId) => normalizeProviderId(backendId) === normalizedBackend)).map((plugin) => plugin.id));
...
//#endregion
export { resolveDiscoveredProviderPluginIds as a, resolveOwningPluginIdsForModelRef as c, resolveOwningPluginIdsForProviderRef as d, resolveProviderRefOwnership as f, resolveDiscoverableProviderOwnerPluginIds as i, resolveOwningPluginIdsForModelRefs as l, resolveBundledProviderCompatPluginIds as n, resolveEnabledProviderPluginIds as o, resolveUsageHookProviderPluginContracts as p, resolveCatalogHookProviderPluginIds as r, resolveExternalAuthProfileProviderPluginIds as s, resolveActivatableProviderOwnerPluginIds as t, resolveOwningPluginIdsForProvider as u };
