...
import { h as redactToolPayloadTextWithConfig, r as isSensitiveFieldKey, u as redactSensitiveFieldValueWithConfig } from "./redact-CWP17HFN.js";
import { o as resolveTerminalAssistantTranscriptRunId, t as attachSessionTranscriptRunId } from "./transcript-events-Ce7n2r8A.js";
...
import { H as isTranscriptOnlyOpenClawAssistantModel, h as redactTranscriptMessage } from "./session-accessor.sqlite-transcript-store-Bx_F0DmJ.js";
import { t as getGlobalHookRunner } from "./hook-runner-global-CWpWIBkz.js";
import { Ct as attachRuntimeUserTurnTranscriptRecorder, Tt as takeRuntimeUserTurnTranscriptRecorder, r as DefaultResourceLoader, wt as takeRuntimeUserTurnTranscriptContext } from "./sessions-PHTfe5gZ.js";
import { a as sanitizeToolCallInputs, r as makeMissingToolResult } from "./ai-transport-runtime-host-BOc97x0_.js";
import { n as extractToolResultId, t as extractToolCallsFromAssistant } from "./tool-call-id-DJVgVJu5.js";
import { i as jsonUtf8BytesOrInfinity, n as firstEnumerableOwnKeys, t as boundedJsonUtf8Bytes } from "./json-utf8-bytes-3IFmJZrr.js";
import { i as applyInputProvenanceToUserMessage } from "./input-provenance-CCQsDhUy.js";
import { b as formatContextLimitTruncationNotice, d as truncateToolResultMessage } from "./tool-result-truncation-BZr2-1Tz.js";
import { i as resolveLiveToolResultMaxChars, t as DEFAULT_MAX_LIVE_TOOL_RESULT_CHARS } from "./tool-result-limits-DISobJ_J.js";
import { i as setRawSessionAppendMessage, r as getRawSessionAppendMessage } from "./transcript-runtime-state-BKSN3FAG.js";
import { l as restorePreparedUserTurnOperationalMetaForRuntime, o as mergePreparedUserTurnMessageForRuntime } from "./user-turn-transcript-CyFoZBC6.js";
...
function installSessionToolResultGuard(sessionManager, opts) {
	...
	const appendMessageAndCacheTranscriptSeq = (message, options) => {
		...
		const { entryId, anchor } = originalAppendWithTranscriptAnchor(runOwnedMessage, options);
		if (sessionManager.getAppendParentId() === appendParentEntryId) return {
			entryId,
			message: runOwnedMessage,
			...anchor ? { anchor } : {}
		};
		opts?.onMessagePersisted?.(runOwnedMessage);
	...
	*/
	...
	const guardedAppend = (message, callerOptions) => {
		...
		const { anchor, entryId: result, message: persistedMessage, messageSeq, sessionTarget } = appendMessageAndCacheTranscriptSeq(finalMessage, { invalidateSerializedPrefixCache: callerInvalidatesCache || transformedMessage !== nextMessage || finalWrite.changed });
		if (sessionTarget) {
...
function guardSessionManager(sessionManager, opts) {
	...
	const guard = installSessionToolResultGuard(sessionManager, {
		...
		onUserMessagePersisted: async (message, persistence) => {
			...
			if (persistence.anchor) recorder?.markRuntimePersisted(message, persistence.anchor);
			else recorder?.markRuntimePersisted(message);
...
//#endregion
export { guardSessionManager as n, createEmbeddedAgentResourceLoader as t };
