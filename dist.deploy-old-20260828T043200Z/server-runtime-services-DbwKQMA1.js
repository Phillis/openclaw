...
import { p as loadSessionEntryReadOnly, w as resolveSessionStorePathForScope } from "./session-accessor.sqlite-entry-CNdoUuFZ.js";
...
import { f as recordSessionHumanDirectMessage, p as recordSessionStateEvent } from "./session-state-events-BkuyPMaw.js";
import { i as updateSessionUpstreamLinkMarker, n as listWatchedSessionUpstreamLinks, r as readSessionUpstreamLink, t as deleteSessionUpstreamLink } from "./session-upstream-links-CTGE6Gwv.js";
...
import { c as startSessionDeliveryRuntime, o as schedulePendingSessionDeliveries } from "./subagent-completion-delivery-2OFCl3SJ.js";
import "./embedded-agent-B-kid8Al.js";
import { n as runHeartbeatOnce, t as startHeartbeatRunner } from "./heartbeat-runner-B3Xtg6Hj.js";
import { t as resolveGatewayPluginConfig } from "./runtime-plugin-config-WSekhXHE.js";
import { r as assertQueuedConversationDeliveryAttemptAuthorized } from "./conversation-route-ownership-STTzyg-F.js";
...
function startPendingOutboundDeliveryRecovery(params) {
	...
	const recover = () => {
		...
		const settled = runWithGatewayIndependentRootWorkAdmission(async () => {
			if (stopped) return;
			const { drainPendingDeliveriesCore, recoverPendingDeliveries } = await import("./delivery-queue-recovery-CyL_zO5Z.js");
			const { deliverOutboundPayloadsInternal } = await import("./deliver-DyKjbTB1.js");
			if (stopped) return;
			...
			if (migrationPending) {
				...
				initialPass = false;
				const { migrateLegacyPendingOutboundDeliveries } = await import("./delivery-queue-migration-DVH2jhlr.js");
...
function startPendingSessionDeliveryRuntime(params) {
	...
	const timer = setTimeout(() => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { deliverQueuedSessionDelivery, recoverPendingRestartContinuationDeliveries, settleQueuedSessionDelivery } = await import("./server-restart-sentinel-gkoee3AA.js");
			if (stopped) return;
