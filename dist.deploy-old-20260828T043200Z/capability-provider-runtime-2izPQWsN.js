...
import { n as isManifestPluginAvailableForControlPlane, o as loadManifestContractSnapshot, r as isManifestPluginOwnerAllowedByControlPlanePolicy, t as hasManifestContractValue } from "./manifest-contract-eligibility-CZWL79I8.js";
import { a as resolveConfiguredTalkRealtimeProviderId, o as resolveConfiguredTalkSpeechProviderId } from "./talk-kxni9fig.js";
import { a as resolveRuntimePluginRegistry, c as loadOpenClawPluginsWithInternalOverrides, x as resolveVoiceModelRefs } from "./loader-D0AfkRZe.js";
...
import { a as registryContainsRuntimePluginIds, n as getLoadedRuntimePluginRegistry } from "./active-runtime-registry-BGBjj91t.js";
import { i as getPluginRuntimeLoadContext, t as buildPluginRuntimeLoadOptions } from "./load-context-Cj6rxf47.js";
...
function loadBundledCapabilityRuntimeRegistry(params) {
	...
	const discovery = params.manifestRegistry ? void 0 : params.discovery ?? discoverOpenClawPlugins({
		env,
		workspaceDir: params.workspaceDir,
		installRecords: params.installRecords
	});
	const pluginIds = new Set(params.pluginIds);
	const manifestRegistry = params.manifestRegistry ?? loadPluginManifestRegistryCore({
		config,
		env,
		workspaceDir: params.workspaceDir,
		installRecords: params.installRecords,
...
function findProviderById(entries, providerId) {
	...
	for (const entry of entries) {
		...
		if (typeof provider.id === "string" && normalizeCapabilityProviderId(provider.id) === normalizedProviderId) return entry.provider;
	}
	for (const entry of entries) {
		const provider = entry.provider;
		if (!isRecord(provider)) continue;
		if ((Array.isArray(provider.aliases) ? provider.aliases : []).some((alias) => typeof alias === "string" && normalizeCapabilityProviderId(alias) === normalizedProviderId)) return entry.provider;
	}
}
...
function loadCapabilityProviderEntries(params) {
	...
	const entries = filterAllowedEntries((scopedRegistry ? registryContainsRuntimePluginIds(scopedRegistry, params.loadOptions.onlyPluginIds) ? scopedRegistry : void 0 : getLoadedRuntimePluginRegistry({
...
function resolvePluginCapabilityProviders(params) {
	...
	return mergeCapabilityProviderEntries(activeProviders, activeProviders.length > 0 && missingRequestedProviders ? filterLoadedProvidersForRequestedConfig({
		key: params.key,
		requested: missingRequestedProviders,
		entries: requestedLoadedProviders
	}) : requestedLoadedProviders).map((entry) => entry.provider);
}
