...
import { C as createChildDiagnosticTraceContext, D as freezeDiagnosticTraceContext, o as emitTrustedDiagnosticEvent, t as areDiagnosticsEnabledForProcess, x as formatPropagatedDiagnosticTraceparent } from "./diagnostic-events-BGzDm6gu.js";
...
import { p as fireAndForgetBoundedHook, t as getGlobalHookRunner } from "./hook-runner-global-CWpWIBkz.js";
import { c as getStreamLlmRuntime } from "./stream-CXbsApnu.js";
import { f as createAnthropicVertexStreamFnForModel } from "./ai-transport-runtime-host-BOc97x0_.js";
import { _ as emitCoreSemanticRunProgressDiagnosticEvent, f as markDiagnosticRunProgress, v as emitCoreModelRequestEndedDiagnosticEvent, y as emitCoreModelRequestStartedDiagnosticEvent } from "./diagnostic-run-activity-CxbnPTtN.js";
import { a as diagnosticProviderRequestIdHash, i as diagnosticHttpStatusCode, n as diagnosticErrorFailureKind, t as diagnosticErrorCategory } from "./diagnostic-error-metadata-qnHBNuqn.js";
...
function emitProviderRequestTimelineEvent(eventBase, startedAt, durationMs, ok, responseStatus, providerAcceptanceKind) {
...
function emitModelCallCompleted(eventBase, startedAt, observer, ownerGeneration) {
	...
	emitProviderRequestTimelineEvent(eventBase, startedAt, durationMs, true, observer.state.responseStatus, observer.state.providerAcceptanceKind);
...
function emitModelCallError(eventBase, startedAt, observer, err, ownerGeneration) {
	if (observer.state.terminalEventEmitted) return;
	observer.state.terminalEventEmitted = true;
	const durationMs = Date.now() - startedAt;
	const sizeTimingFields = observer.sizeTimingFields();
	...
	emitProviderRequestTimelineEvent(eventBase, startedAt, durationMs, false, observer.state.responseStatus ?? (errorStatus === void 0 ? void 0 : Number(errorStatus)), observer.state.providerAcceptanceKind);
	emitCoreModelRequestEndedDiagnosticEvent({
...
function withDiagnosticRequestContext(options, trace, observer, callId) {
	...
	return withProviderAcceptanceObserver({
		...options,
		requestId: callId,
