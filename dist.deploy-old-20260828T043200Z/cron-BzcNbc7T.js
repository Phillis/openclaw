...
import { g as cronJobUsesToolRuntime } from "./store-pLPqGtqL.js";
import { t as cronStoreKey } from "./key-BBZ40bDq.js";
import { F as createAccountCronScheduledToolPolicy, I as createTrustedCronScheduledToolPolicy, O as isInvalidCronSessionTargetIdError, _ as normalizeCronJobCreate, j as resolveCronSessionTargetSessionKey, k as resolveCronDeliverySessionKey, y as normalizeCronJobPatch } from "./row-codec-LoN9q1nV.js";
...
import { ct as validateCronGetParams, dt as validateCronRunParams, ft as validateCronRunsParams, gt as validateCronUpdateParams, ht as validateCronStatusParams, io as validateWakeParams, lt as validateCronListParams, mt as validateCronScratchSetParams, pt as validateCronScratchGetParams, st as validateCronAddParams, ut as validateCronRemoveParams } from "./src-4dv5TpeQ.js";
...
import { a as isAgentHarnessSessionKey, d as resolveAgentHarnessSessionStoreEntryError, n as AGENT_HARNESS_SESSION_ID_LOCKED_MESSAGE, r as AGENT_HARNESS_SESSION_KEY_RESERVED_MESSAGE } from "./agent-harness-session-key-Bf-Q9dw5.js";
import { d as errorShape } from "./validation-errors-rELRlKfn.js";
import { a as validateTargetProviderPrefix, t as resolveTargetPrefixedChannel } from "./channel-target-prefix-Bxdln32l.js";
...
import { t as consumeCronCreatorAuthorityGrant } from "./cron-creator-authority-grant-C2m8myn9.js";
import { n as listConfiguredMessageChannels } from "./channel-selection-BRB1TsMb.js";
import { n as resolveCronDeliveryPlan, t as hasExplicitCronDeliveryTarget } from "./delivery-plan-DEniePks.js";
import { a as isInvalidCronTaskRunJobIdError, c as toPublicCronJob, h as resolveFailureAlert, r as applyJobPatch, s as readCronTaskRunHistoryPage, t as resolveCronListSnapshotRevision } from "./list-snapshot-revision-CKGufxNE.js";
...
import { i as operatorSessionCap, t as authorizeGatewaySessionCreation } from "./operator-role-policy-Bvt-UeJ1.js";
...
import { r as hasActiveAgentRuntimeAuthority, t as assertActiveAgentRuntimeAuthority } from "./agent-runtime-authority-DnVI97Gp.js";
...
function respondCronJobNotFound(respond, jobId, options = {}) {
	...
	respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, message, { details: {
...
const cronHandlers = {
	...
	"cron.list": async ({ params, respond, context, client }) => {
		...
		let page = callerScope ? await listCronPageForCallerScope({
			callerScope,
			context,
			options: listOptions
		...
		if (cronVisibility) {
			...
			for (;;) {
				...
				const sourcePage = callerScope ? await listCronPageForCallerScope({
					callerScope,
					context,
					options: sourceOptions
				}) : await context.cron.listPage(sourceOptions);
				visibleJobs.push(...sourcePage.jobs.filter((job) => cronJobIsVisible(job, cronVisibility, context.cron.getDefaultAgentId())));
				if (!sourcePage.hasMore || sourcePage.nextOffset === null) break;
	...
	"cron.add": async ({ params, respond, context, client }) => {
		...
		const callerScope = readCronCallerScope(client);
		const actor = (callerScope ? void 0 : resolveOperatorSessionCreation(client).actor) ?? (callerScope?.sessionKey ? loadGatewaySessionEntryReadOnly(callerScope.sessionKey, { agentId: callerScope.agentId }).entry?.createdActor : void 0);
	...
	"cron.runs": async ({ params, respond, context, client }) => {
		...
		if (scope === "all") {
			...
			if (cronVisibility) {
				...
				for (;;) {
					...
					visibleEntries.push(...sourcePage.entries.filter((entry) => visibleJobIds.has(entry.jobId) && (!entry.sessionKey || cronVisibility(entry.sessionKey, p.agentId))));
					if (!sourcePage.hasMore || sourcePage.nextOffset === null) break;
					sourceOffset = sourcePage.nextOffset;
				}
		...
		try {
			...
			if ((callerScope || p.agentId || cronVisibility) && !matchedJob || !job && readCronTaskRunHistoryPage({
				storeKey,
				jobId,
				limit: 1
			}).total === 0) {
			...
			if (cronVisibility) {
				const visibleEntries = page.entries.filter((entry) => !entry.sessionKey || cronVisibility(entry.sessionKey, matchedJob?.agentId));
				const total = visibleEntries.length;
				page = {
					...page,
