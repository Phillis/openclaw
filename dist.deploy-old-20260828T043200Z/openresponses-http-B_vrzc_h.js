...
import { c as toOpenAiResponsesUsage } from "./usage-DNKCVmJi.js";
...
import { t as renderFileContextBlock } from "./file-context-Lsbx4rxw.js";
import { a as extractImageContentFromSource, i as extractFileContentFromSource, n as DEFAULT_INPUT_IMAGE_MIMES, o as normalizeMimeList, s as resolveInputFileLimits } from "./input-files-BYrqCFUr.js";
...
import { a as authorizeOpenAiCompatibleHttpModelOverride, d as getHeader, g as resolveOpenAiCompatibleHttpSenderIsOwner, h as resolveOpenAiCompatibleHttpOperatorScopes, u as getBearerToken } from "./http-auth-utils-CrQlRW6b.js";
...
function resolveResponsePayloadText(result) {
	const payloads = result?.payloads;
	return Array.isArray(payloads) ? payloads.flatMap((payload) => typeof payload.text === "string" && payload.text ? [payload.text] : []).join("\n\n") : "";
}
...
async function runResponsesAgentCommand(params) {
	return agentCommandFromGatewayIngress({
		...
		...params.resolveGatewayContext ? { onAdmittedRunContext: (context) => bindGatewayContextResolver(context, params.resolveGatewayContext) } : {}
	...
	if (!stream) {
		...
		try {
			...
			if (isClientToolNameConflictError(err)) {
				sendJson(res, 400, createFailedResponse({
