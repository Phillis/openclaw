...
import { n as normalizeHealthCheck, t as defineSplitHealthCheckInput } from "./health-check-adapter-CWLXSjNJ.js";
...
async function runDoctorHealthRepairs(ctx, opts = {}) {
	const checks = (opts.checks ?? listHealthChecks().map(defineSplitHealthCheckInput)).map(normalizeHealthCheck);
	const findings = [];
