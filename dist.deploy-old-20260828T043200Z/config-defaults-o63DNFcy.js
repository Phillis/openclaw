...
import { i as CLAUDE_CLI_DEFAULT_ALLOWLIST_REFS, l as CLAUDE_CLI_PROFILE_ID, t as CLAUDE_CLI_BACKEND_ID } from "./cli-constants-Djv4WtLq.js";
import { n as resolveKnownAnthropicModelRef, t as resolveClaudeCliAnthropicModelRefs } from "./claude-model-refs-C-fcikup.js";
...
function resolveAnthropicDefaultAuthMode(config, env) {
	if (usesRetiredClaudeCliProviderEntry(config)) return "oauth";
...
function usesRetiredClaudeCliProviderEntry(config) {
	return Object.entries(config.models?.providers ?? {}).some(([provider, entry]) => normalizeProviderId(provider) === "anthropic" && entry.apiKey === CLAUDE_CLI_PROFILE_ID);
}
...
function usesSelectedClaudeCliAuthProfile(config) {
	if (usesRetiredClaudeCliProviderEntry(config)) return true;
	const profiles = config.auth?.profiles ?? {};
