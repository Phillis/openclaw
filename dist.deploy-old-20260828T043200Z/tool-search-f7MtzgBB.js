...
import { a as isPreExecutionBlockedToolResult, d as runWithToolExecutionValidation, l as rewrapToolWithBeforeToolCallHook, r as getBeforeToolCallFailureDisposition, u as wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call-DoS1-Lb6.js";
import { R as SESSION_TOOL_STDERR_TAIL_BYTES, z as appendBoundedTextTail } from "./sessions-PHTfe5gZ.js";
...
import { a as protectNetworkToolExecutionError, d as resolveToolResultFailureKind, i as isTrustedToolExecutionPreflightError } from "./tool-result-error-CnEQjVCq.js";
import { t as jsonResult } from "./tool-results-BCM3fdVS.js";
import { t as levenshteinDistance } from "./levenshtein-distance-CGoXGBU_.js";
import { T as isToolWrappedWithBeforeToolCallHook, h as getChannelAgentToolMeta } from "./gateway-caller-context-D1DYQtHE.js";
import { i as getPluginToolMeta } from "./tools-DL5ef4Om.js";
import { c as toToolSearchJsonSafe, s as transferMcpCodeModeGuestResult } from "./agent-bundle-mcp-materialize-VGxGQrwu.js";
import { t as isCoreCodingSurfaceToolName } from "./core-tool-factory-descriptors-C-BGWj2h.js";
...
function prepareToolSearchCatalogExecutionTool(entry, options) {
	const prepareInput = options.prepareInput && entry.source === "openclaw" && "prepareBeforeToolCallParams" in entry.tool && typeof entry.tool.prepareBeforeToolCallParams === "function";
	...
	return wrapperOptions ? rewrapToolWithBeforeToolCallHook(tool, void 0, wrapperOptions) : entry.tool;
}
...
function registerToolSearchCatalog(params) {
	...
	const fingerprint = params.fingerprint !== void 0 && next.entries.length === params.entries.length ? params.fingerprint : catalogEntriesFingerprint(next.entries);
...
function applyToolCatalogCompaction(params) {
	...
	const incomingFingerprint = existingCatalog || reusableKey ? catalogEntriesFingerprint(catalog) : void 0;
...
function readParameterText(parameters, depth = 0) {
	...
	if (isRecord(properties)) for (const [name, child] of Object.entries(properties)) parts.push(name, readParameterText(child, depth + 1));
...
function readMessageToolResultId(message) {
	...
	const direct = Reflect.get(message, "toolCallId") ?? Reflect.get(message, "toolUseId") ?? Reflect.get(message, "tool_use_id");
...
function snapshotToolSearchTargetTranscriptResult(result) {
	...
	return transferMcpCodeModeGuestResult(result, freezeJsonSnapshot(snapshot));
}
//#endregion
...
async function validateCatalogSchemaValue(entry, schemaName, value) {
	...
	try {
		schemaValidatorModulePromise ??= import("./schema-validator-BT7gq6o4.js");
...
function formatToolSearchControlResult(payload, runtime, parentToolCallId, terminalBatchStatus) {
	let result = jsonResult(payload);
	...
	return terminalBatchStatus !== "waiting" && runtime?.takeTerminalTargetBatch(parentToolCallId) === true ? {
		...result,
		terminate: true
	} : result;
}
...
function isToolSearchCodeModeSupported() {
	...
	return typeof process.versions.electron !== "string" && process.allowedNodeEnvironmentFlags.has("--permission");
}
