...
import { P as validateConfigObjectWithPlugins, s as readConfigFileSnapshot, u as readConfigFileSnapshotWithPluginMetadata } from "./io-DlN5njvP.js";
...
import { h as resolveConfigSnapshotHash } from "./io.read-helpers-YVBmmwxJ.js";
...
import { i as sameDefaultInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-B3bBax5n.js";
...
import { t as applySystemAgentModelSelection } from "./setup-model-selection-PA5KMEvN.js";
...
async function applySystemAgentSetup(params, hooks) {
	...
	const [{ readSetupConfigFileSnapshot, resolveQuickstartGatewayDefaults }, onboardHelpers, { applyLocalSetupWorkspaceConfig, resolveOnboardingWorkspaceConflict }, { transformConfigWithPendingPluginInstalls }] = await Promise.all([
		import("./setup.shared-ChWW005D.js"),
		import("./onboard-helpers-kk_BLYXI.js"),
		import("./onboard-config-BG5OGrQW.js"),
		import("./install-record-commit-B7gPOqSM.js")
	]);
	...
	let guardModules = expectedAgentId || expectedAgentDir || expectedModelRef ? await Promise.all([import("./agent-scope-WWPxWnDc.js"), import("./model-selection-JZ1qp18M.js")]) : void 0;
	...
	if (startedWithoutAuthoredRoster) {
		...
		guardModules ??= await Promise.all([import("./agent-scope-WWPxWnDc.js"), import("./model-selection-JZ1qp18M.js")]);
	...
	await runCommittedFollowUp(async () => {
		const { updateExecApprovals } = await import("./exec-approvals-rn9kqtcb.js");
	...
	if (refreshPluginRegistry && enablePluginId) await runCommittedFollowUp(async () => {
		const { refreshPluginRegistryAfterConfigMutation } = await import("./registry-refresh-DlEIC1K6.js");
	...
	if (surface === "cli") await runCommittedFollowUp(async () => {
		const { ensureGatewayServiceForOnboarding } = await import("./setup.finalize-DccivkAU.js");
		...
		if (gateway.status === "failed") lines.push(`Gateway service: ${gateway.error}`);
			...
			const probe = await onboardHelpers.waitForGatewayReachable({
				...
				password: settings.authMode === "password" ? await (await import("./setup.secret-input-DWOKsNgv.js")).resolveSetupSecretInputString({
					config: nextConfig,
