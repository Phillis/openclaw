...
import { M as resolveNonNegativeIntegerOption, b as parseFiniteNumber, j as resolveIntegerOption } from "./number-coercion-CLj0HTDM.js";
...
import { H as DEFAULT_BROWSER_DOWNLOAD_TIMEOUT_MS, y as getPlaywrightCore } from "./cdp.helpers-geYy1F8f.js";
import { l as resolveBrowserNavigationTimeoutMs, n as ACT_MAX_VIEWPORT_DIMENSION, o as resolveActInteractionTimeoutMs, r as ACT_MAX_WAIT_TIME_MS, s as resolveActWaitTimeoutMs, t as ACT_MAX_CLICK_DELAY_MS } from "./act-policy-Buz-hQnc.js";
import { a as resolveStrictExistingUploadPaths, n as DEFAULT_TRACE_DIR } from "./paths-oxXTelRQ.js";
import { u as BrowserTabNotFoundError } from "./tmp-openclaw-dir-DE5M5B-m.js";
import "./errors-Bg4swB3U.js";
import { C as buildRoleSnapshotFromAiSnapshot, D as parseRoleRef, L as withBrowserNavigationPolicy, M as assertBrowserNavigationAllowed, P as assertBrowserNavigationResultAllowed, T as finalizeRoleSnapshot, _ as formatAriaSnapshot, w as buildRoleSnapshotFromAriaSnapshot } from "./chrome-DjHNxomb.js";
import { i as planAnnotations, l as matchBrowserUrlPattern, n as buildOverlayClearScript, r as buildOverlayInjectionScript, t as appendSnapshotUrls, u as normalizeBrowserEvaluateFunctionSource } from "./snapshot-urls-DVqktkQ3.js";
import { A as getObservedBrowserStateForPage, C as retirePlaywrightBrowserConnection, D as armObservedDialogResponseOnPage, E as isDownloadStartingNavigationError, F as createDownloadCaptureForPage, I as isBrowserObservedDialogBlockedError, M as respondToObservedDialogOnPage, N as restoreRoleRefsForTarget, O as createObservedDialogAbortSignalForPage, P as storeRoleRefsForTarget, S as pageTargetInfo, T as beginActionDownloadCaptureOnPage, _ as wasBrowserNavigationSourcePreservedAfterPolicyDenial, a as forceDisconnectPlaywrightForTarget, b as ensureContextState, c as listPagesViaPlaywright, d as withPageScopedCdpClient, f as assertPageNavigationCompletedSafely, g as quarantineBlockedNavigationTarget, h as isPolicyDenyNavigationError, i as focusPageByTargetIdViaPlaywright, j as markObservedDialogsHandledRemotelyForPage, k as ensurePageState, l as refLocator, m as gotoPageWithNavigationGuard, n as closePlaywrightBrowserConnection, o as getMainFrameDocumentIdentityViaPlaywright, p as closeBlockedNavigationTarget, r as createPageViaPlaywright, s as getObservedBrowserStateViaPlaywright, t as closePageByTargetIdViaPlaywright, u as markBackendDomRefsOnPage, v as withPageNavigationRequestGuard, w as retirePlaywrightBrowserConnectionExact, x as getPageForTargetId, y as connectBrowser } from "./pw-session-B3UdriXd.js";
import { t as writeExternalFileWithinOutputRoot } from "./output-files-h6BhKnMo.js";
...
async function runGuardedPageInteraction(page, opts, action, errorLabel) {
	...
	try {
		...
		if (errorLabel !== void 0) throw toFriendlyInteractionError(error, errorLabel);
		throw error;
	} finally {
		cleanup();
	}
...
async function hoverViaPlaywright(opts) {
	...
	await runGuardedPageInteraction(page, opts, async () => await locator.hover({ timeout: resolveActInteractionTimeoutMs(opts.timeoutMs) }), label);
}
async function dragViaPlaywright(opts) {
	...
	await runGuardedPageInteraction(page, opts, async () => await startLocator.dragTo(endLocator, { timeout: resolveActInteractionTimeoutMs(opts.timeoutMs) }), `${startLabel} -> ${endLabel}`);
}
...
async function scrollIntoViewViaPlaywright(opts) {
	...
	const { label, locator } = resolveInteractionElement(page, resolved);
	await runGuardedPageInteraction(page, opts, async () => await locator.scrollIntoViewIfNeeded({ timeout }), label);
}
//#endregion
...
async function navigateViaPlaywright(opts) {
	...
	const navigate = async () => await gotoPageWithNavigationGuard({
		...
		...opts.resolveOperationTarget ? { assertPageCurrent: () => {
			if (opts.resolveOperationTarget?.() !== currentTargetId) throw new BrowserTabNotFoundError({ input: currentTargetId });
		} } : {}
	});
	...
	try {
		...
		if (opts.resolveOperationTarget) {
			...
			if (opts.resolveOperationTarget() !== replacementTargetId) throw new BrowserTabNotFoundError({ input: currentTargetId });
...
async function responseBodyViaPlaywright(opts) {
	...
	const promise = new Promise((resolve, reject) => {
		...
		const timer = setTimeout(() => {
			reject(/* @__PURE__ */ new Error(matched ? `Response body timed out after ${timeout}ms for url pattern "${pattern}".` : `Response not found for url pattern "${pattern}". Run 'openclaw browser requests' to inspect recent network activity.`));
		}, timeout);
		cleanup = () => {
			clearTimeout(timer);
			page.off("response", handler);
