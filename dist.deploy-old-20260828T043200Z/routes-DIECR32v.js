...
import { C as parseStrictNonNegativeInteger, S as parseStrictInteger, p as clampPositiveTimerTimeoutMs, s as asFiniteNumber, w as parseStrictPositiveInteger, x as parseStrictFiniteNumber } from "./number-coercion-CLj0HTDM.js";
...
import { a as ensureMediaDir, f as saveMediaBuffer } from "./store-B6ILpvye.js";
import "./number-runtime-Cy4drVnh.js";
import "./string-coerce-runtime-C8jKEm3h.js";
import { n as redactCdpUrl, t as parseBrowserHttpUrl } from "./browser-config-BDyn11gY.js";
import "./text-utility-runtime-BNhX-3os.js";
import { K as DEFAULT_BROWSER_SCREENSHOT_TIMEOUT_MS, R as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, _ as withCdpSocket, n as assertCdpEndpointAllowed, u as redactCdpErrorText, v as normalizeBrowserTimerDelayMs, z as DEFAULT_AI_SNAPSHOT_MAX_CHARS } from "./cdp.helpers-geYy1F8f.js";
import { a as normalizeActBoundedNonNegativeMs, l as resolveBrowserNavigationTimeoutMs, n as ACT_MAX_VIEWPORT_DIMENSION, r as ACT_MAX_WAIT_TIME_MS, t as ACT_MAX_CLICK_DELAY_MS } from "./act-policy-Buz-hQnc.js";
import { i as resolveExistingUploadPaths, n as DEFAULT_TRACE_DIR, t as DEFAULT_DOWNLOAD_DIR } from "./paths-oxXTelRQ.js";
import { a as resolveProfile, i as resolveManagedBrowserHeadlessMode, n as getOwnBrowserProfile, r as resolveBrowserConfig } from "./config-HQZ6_yJ9.js";
import { a as BrowserError, d as BrowserTargetAmbiguousError, f as BrowserValidationError, i as BrowserConflictError, m as toBrowserErrorResponse, o as BrowserProfileNotFoundError, s as BrowserProfileUnavailableError, t as BROWSER_ACT_ERROR_CODES, u as BrowserTabNotFoundError } from "./tmp-openclaw-dir-DE5M5B-m.js";
import { i as shouldUsePlaywrightForScreenshot, n as resolveDefaultSnapshotFormat, r as shouldUsePlaywrightForAriaSnapshot, t as getBrowserProfileCapabilities } from "./profile-capabilities-DWXzOQqP.js";
...
import { a as isValidProfileName, i as setDefaultBrowserProfile, n as deleteBrowserProfileConfig, t as createBrowserProfileConfig } from "./config-mutations-CjZjAUYG.js";
import { A as STRUCTURAL_ROLES, B as resolveBrowserNavigationProxyMode, E as getRoleSnapshotIdentityKeys, L as withBrowserNavigationPolicy, M as assertBrowserNavigationAllowed, O as CONTENT_ROLES, P as assertBrowserNavigationResultAllowed, S as appendRoleSnapshotDepthTruncationMarker, T as finalizeRoleSnapshot, U as resolveBrowserExecutableForPlatform, b as snapshotAria, h as captureScreenshot, k as INTERACTIVE_ROLES, n as getChromeWebSocketEndpoint, s as resolveOpenClawUserDataDir, u as ensureOutputDirectory, v as getMainFrameDocumentIdentityViaCdp, x as snapshotRoleViaCdp } from "./chrome-DjHNxomb.js";
import { _ as getPwAiModule$1, a as getOrCreateProfileRuntime, l as isProfileRestartRequiredError, o as getProfileLifecycle, r as beginProfileTransition } from "./server-context.lifecycle-CJEeE8hM.js";
import { f as ChromeMcpDocumentUnavailableError } from "./chrome-mcp-contracts-JXYE0tAu.js";
import { r as resolveCdpControlPolicy, t as movePathToTrash } from "./trash-CmvqpBj1.js";
import { i as resolveTargetIdFromTabs, r as withProfileContextOperation } from "./server-context-CWeEXVWZ.js";
import { n as importSystemProfileCookies, r as listSystemProfiles } from "./system-profiles-BAr5tVgs.js";
import "./sdk-setup-tools-CdZ70F9V.js";
import { S as withChromeMcpDocument, _ as resizeChromeMcpPage, b as takeChromeMcpSnapshot, d as fillChromeMcpElement, f as fillChromeMcpForm, g as pressChromeMcpKey, h as navigateChromeMcpPage, l as dragChromeMcpElement, m as hoverChromeMcpElement, o as clickChromeMcpCoords, r as getChromeMcpPid, s as clickChromeMcpElement, u as evaluateChromeMcpScript, x as uploadChromeMcpFile, y as takeChromeMcpScreenshot } from "./chrome-mcp-BC4D3LuQ.js";
import { a as scaleAnnotations, l as matchBrowserUrlPattern, s as normalizeBrowserFormField, t as appendSnapshotUrls, u as normalizeBrowserEvaluateFunctionSource } from "./snapshot-urls-DVqktkQ3.js";
...
function normalizeActRequest(body, options) {
	...
	switch (kind) {
		...
		case "press": {
			const key = toStringOrEmpty(body.key).split("+").map((part) => KEY_ALIASES.get(part.toLowerCase()) ?? part).join("+");
...
function captureBrowserOperationTarget(opts) {
	...
	return () => opts.ctx.state().extensionRelays?.get(opts.profileName) === relay ? resolveTarget?.() : void 0;
}
/** Accept only the acted-on Page or its exact relay-owned tab as a replacement target. */
function resolveOperationTargetOutcome(opts) {
	return opts.resolveRelayTarget ? opts.resolveRelayTarget() ?? opts.actedOnTargetId : opts.operationTargetId ?? opts.actedOnTargetId;
}
//#endregion
		...
		await withRouteTabContext({
			...
			run: async ({ profileCtx, cdpUrl, tab, signal, resolveTabUrl }) => {
				...
				const jsonOk = async (extra, options) => {
					const responseTargetId = options?.resolveCurrentTarget && (!isExistingSession || hasNavigationResultPolicy) ? resolveOperationTargetOutcome({
...
function registerBrowserAgentStorageRoutes(app, ctx) {
	const runMutation = async (req, res, targetId, feature, run) => {
		await withPlaywrightRouteContext({
			req,
			res,
			ctx,
	...
	app.post("/cookies/set-many", async (req, res) => {
		...
		await runMutation(req, res, targetId, "cookies set-many", async ({ cdpUrl, tab, pw, signal }) => {
	...
	app.post("/storage/:kind/set", async (req, res) => {
		...
		await runMutation(req, res, mutation.parsed.targetId, "storage set", async ({ cdpUrl, tab, pw }) => {
	...
	app.post("/storage/:kind/clear", async (req, res) => {
		const mutation = parseStorageMutationFromRequest(req, res);
		if (!mutation) return;
		await runMutation(req, res, mutation.parsed.targetId, "storage clear", async ({ cdpUrl, tab, pw }) => {
	...
	app.post("/set/device", async (req, res) => {
		...
		await runMutation(req, res, targetId, "device emulation", async ({ cdpUrl, tab, pw, signal }) => {
...
function normalizeChromeGraphicsInfo(value, observedAt = Date.now()) {
	const gpu = asNullableRecord(asNullableRecord(value)?.gpu);
	if (!gpu) return {
		status: "unavailable",
		observedAt,
...
function buildBrowserDoctorReport(params) {
	...
	if (transport === "chrome-mcp") checks.push({
		...
		checks.push({
			...
			...mismatch ? { fixHint: "Reload the OpenClaw extension from chrome://extensions. If the versions still differ, fully quit and reopen Chrome." } : {}
		});
	} else {
		checks.push({
			...
			const report = await runProfileRouteOperation({
				...
				run: async (signal) => {
					...
					const doctorReport = buildBrowserDoctorReport({
						status,
						extensionVersion: status.transport === "extension" ? ctx.state().extensionRelays?.get(profileCtx.profile.name)?.bridge.identity?.extensionVersion : void 0
					});
...
function registerBrowserTabRoutes(app, ctx) {
	...
	app.delete("/tabs/:targetId", async (req, res) => {
		...
		await runTabTargetMutation({
			...
			targetId,
			mutate: async (profileCtx, id) => {
				const canonicalTargetId = await profileCtx.closeTab(id, targetIdMode === "raw" ? { exactTargetId: true } : void 0);
