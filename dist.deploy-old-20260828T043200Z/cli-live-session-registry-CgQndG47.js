...
import { n as extractBalancedJsonFragments } from "./src-BntaCZM-.js";
...
import { r as scanReasoningTags, t as createReasoningTagTextPartitioner } from "./reasoning-tags-3YlFC272.js";
import { i as coerceToFailoverError, p as resolveFailoverStatus, t as FailoverError } from "./failover-error-DVBvcQuA.js";
...
function resolveCliTerminalErrorText(parsed, terminalFailure) {
	return collectExplicitCliErrorText(parsed) || (terminalFailure ? "Reached maximum number of turns." : "");
}
...
function buildCliLiveSessionFingerprint(params) {
	...
	return sha256Hex(JSON.stringify({
		...
		skillsFingerprint,
		env: Object.keys(params.env).toSorted().filter((key) => key !== "OPENCLAW_MCP_CLI_CAPTURE_KEY").map((key) => [key, key === "OPENCLAW_MCP_TOKEN" && normalizeGrantToken ? "<managed-mcp-grant>" : params.env[key] ? sha256Hex(params.env[key]) : ""])
	}));
}
//#endregion
...
function acceptsCliLiveSession(context) {
	return context.params.sessionEntry?.execHost !== "node" && Boolean(context.preparedBackend.execute) && context.preparedBackend.backend.liveSession !== void 0 && context.preparedBackend.backend.output === "jsonl" && context.preparedBackend.backend.input === "stdin";
}
...
function createCliLiveSessionCapability(params) {
	...
	const assertActive = () => {
		const assertion = resolveAdmittedRunActiveAssertion(params.context.params.admittedRunContext, params.abortSignal);
		if (!assertion) throw new Error("CLI live session turn is no longer active.");
		assertion();
	};
	...
	return Object.freeze({
		...
		register: (handle) => {
			...
			if (handle.fingerprint !== fingerprint || !handle.generation.trim() || liveSessions.has(ownerKey) || Array.from(liveSessions.values()).some((record) => record.handle === handle)) throw new Error("CLI live session registration does not match its admitted owner.");
...
//#endregion
export { getCliLiveSessionApprovalGrants as a, createCliExitFailoverError as c, parseCliOutput as d, createCliJsonlStreamingParser as f, createCliLiveSessionCapability as i, createCliFailoverError as l, buildCliLiveOwnerKey as n, getCliLiveSessionGeneration as o, closeCliLiveSession as r, hasCliLiveSession as s, acceptsCliLiveSession as t, formatCliOutputError as u };
