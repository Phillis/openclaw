...
import { h as resolveBrowserRateLimitMessage, r as closeTrackedCdpTarget } from "./cdp.helpers-geYy1F8f.js";
...
async function browserStatus(baseUrl, opts) {
	return await fetchBrowserJson(withProfilePath(baseUrl, "/", opts?.profile), {
...
async function browserDoctor(baseUrl, opts) {
	...
	return await fetchBrowserJson(withBaseUrl(baseUrl, `/doctor${params.size ? `?${params.toString()}` : ""}`), {
		timeoutMs: opts?.deep ? BROWSER_DEEP_DOCTOR_REQUEST_TIMEOUT_MS : BROWSER_DOCTOR_REQUEST_TIMEOUT_MS,
		signal: opts?.signal
	});
}
/** List configured browser profiles and their current status. */
async function browserProfiles(baseUrl, opts) {
	return (await fetchBrowserJson(withBaseUrl(baseUrl, `/profiles`), {
		timeoutMs: resolveBrowserClientTimeoutMs(opts, 3e3),
		signal: opts?.signal
	})).profiles ?? [];
}
/** List Chrome-family profiles available on the local macOS host. */
async function browserSystemProfiles(baseUrl, opts) {
	return (await fetchBrowserJson(withBaseUrl(baseUrl, `/system-profiles${opts?.browser ? `?browser=${encodeURIComponent(opts.browser)}` : ""}`), {
		timeoutMs: resolveBrowserClientTimeoutMs(opts, 3e3),
		signal: opts?.signal
	})).systemProfiles ?? [];
}
...
async function browserTabs(baseUrl, opts) {
	return normalizeBrowserTabsResult(await fetchBrowserJson(withProfilePath(baseUrl, "/tabs", opts?.profile), {
		timeoutMs: resolveBrowserClientTimeoutMs(opts, 3e3),
		signal: opts?.signal
	}));
}
...
function deleteVolatileTarget(tab) {
	...
	for (const [sessionKey, tabs] of state) {
		for (const [key, candidate] of tabs) if (volatileSessionTabTargetKey(candidate) === targetKey) {
			tabs.delete(key);
...
async function performVolatileCleanup(candidate, params, cleanupKind) {
	...
	if (!tab) return 0;
	if (cleanupKind === "sweep" && !sameVolatileSessionTab(tab, candidate)) return 0;
	...
	const cleanup = Promise.resolve().then(async () => {
		try {
			if (params.closeTab) await params.closeTab({
				...
				deleteVolatileTarget(tab);
				return outcome.status === "closed" ? 1 : 0;
			} else await browserCloseTabByRawTargetId(tab.route.baseUrl, tab.targetId, { profile: tab.profile });
		} catch (error) {
	...
	try {
		...
		if (inFlight.get(targetKey) === cleanup) inFlight.delete(targetKey);
	}
}
...
async function closeTrackedBrowserTabsForSessions(params) {
	return await closeTrackedTabs(selectTrackedTabsForSessions({
...
//#endregion
export { BrowserServiceError as C, buildProfileQuery as D, setBridgeAuthForPort as E, withBaseUrl as O, normalizeBrowserTabsResult as S, deleteBridgeAuthForPort as T, browserStatus as _, untrackSessionBrowserTab as a, browserTabAction as b, browserDeleteProfile as c, browserImportProfile as d, browserOpenTab as f, browserStart as g, browserSnapshot as h, trackSessionBrowserTab as i, browserDoctor as l, browserResetProfile as m, sweepTrackedBrowserTabs as n, browserCloseTab as o, browserProfiles as p, touchSessionBrowserTab as r, browserCreateProfile as s, closeTrackedBrowserTabsForSessions as t, browserFocusTab as u, browserStop as v, fetchBrowserJson as w, browserTabs as x, browserSystemProfiles as y };
