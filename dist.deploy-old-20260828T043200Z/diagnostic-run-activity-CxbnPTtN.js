import { F as markCoreSemanticRunProgressDiagnosticEvent, P as CORE_SEMANTIC_RUN_PROGRESS_METADATA_KEY, R as CORE_MODEL_REQUEST_LIFECYCLE_METADATA_KEY, h as onInternalDiagnosticEvent, o as emitTrustedDiagnosticEvent, s as emitTrustedDiagnosticEventWithPrivateData, u as getInternalDiagnosticEventSequence, z as markCoreModelRequestLifecycleDiagnosticEvent } from "./diagnostic-events-BGzDm6gu.js";
...
function mergeSessionActivity(target, source) {
	...
	for (const [generation, registration] of activeDiagnosticOwners) if (registration.activity === source) activeDiagnosticOwners.set(generation, {
		...registration,
		activity: target
	});
...
function recordModelStarted(event, provenance, coreRequestForTest = false) {
	...
	if (!activity) return;
	if (!provenance && !coreRequestForTest && [...activeDiagnosticOwners.values()].some(({ activity: ownerActivity }) => Object.is(ownerActivity, activity))) return;
...
function recordModelEnded(event, provenance) {
	...
	if (!activity) return;
	if (!provenance && [...activeDiagnosticOwners.values()].some(({ activity: ownerActivity }) => Object.is(ownerActivity, activity))) {
...
function recordRunCompleted(event) {
	...
	if ([...activeDiagnosticOwners.values()].some(({ activity: ownerActivity, owner }) => ownerActivity === activity && owner.runId === event.runId)) return;
...
function closeDiagnosticEmbeddedRunOwner(owner) {
	...
	if (activity.activeEmbeddedRuns.get(owner.workKey)?.generation === owner.generation) embeddedRunIndex.remove(activity, owner.workKey);
...
function startDiagnosticRunActivityTracking() {
	...
	unregisterDiagnosticRunActivityListener = onInternalDiagnosticEvent((event, metadata) => {
		...
		switch (event.type) {
			case "tool.execution.started": return recordToolStarted(event);
			...
			case "model.call.error":
				...
				return;
			case "run.progress": return applyRunProgress(event, isCoreSemanticRunProgressDiagnosticMetadata(metadata));
...
//#endregion
export { emitCoreSemanticRunProgressDiagnosticEvent as _, createDiagnosticEmbeddedRunOwner as a, isDiagnosticEmbeddedRunOwnerClosed as c, markDiagnosticEmbeddedRunStarted as d, markDiagnosticRunProgress as f, stopDiagnosticRunActivityTracking as g, startDiagnosticRunActivityTracking as h, closeDiagnosticEmbeddedRunOwner as i, markDiagnosticArgumentChurnObservation as l, resolveRunStaleThresholdMs as m, RUN_STALE_TAKEOVER_MS as n, getDiagnosticEmbeddedRunActivitySequence as o, resetDiagnosticRunActivityForTest as p, clearDiagnosticEmbeddedRunActivityForSession as r, getDiagnosticSessionActivitySnapshot as s, BLOCKED_TOOL_CALL_ABORT_FLOOR_MS as t, markDiagnosticEmbeddedRunEnded as u, emitCoreModelRequestEndedDiagnosticEvent as v, emitCoreModelRequestStartedDiagnosticEvent as y };
