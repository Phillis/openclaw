...
import { a as selectDefaultNodeFromList } from "../../nodes-utils-Cjfuq3J0.js";
...
import { n as isCanvasHostEnabled, t as canvasConfigSchema } from "../../config-BrlmZdHd.js";
import { t as A2UI_PATH } from "../../a2ui-shared-BVnsAcOM.js";
import { i as CanvasToolSchema, n as isEligibleCanvasNode, t as CANVAS_PRESENT_COMMAND } from "../../node-eligibility-D_OlD7mW.js";
import { randomUUID } from "node:crypto";
//#region node_modules/@a2ui/web_core/node_modules/zod/v3/helpers/util.js
var util;
(function(util) {
	util.assertEqual = (_) => {};
...
//#endregion
//#region node_modules/@a2ui/web_core/node_modules/zod/v3/ZodError.js
...
//#endregion
//#region node_modules/@a2ui/web_core/node_modules/zod/v3/locales/en.js
...
//#endregion
//#region node_modules/@a2ui/web_core/node_modules/zod/v3/errors.js
let overrideErrorMap = errorMap;
function getErrorMap() {
	return overrideErrorMap;
}
//#endregion
//#region node_modules/@a2ui/web_core/node_modules/zod/v3/helpers/parseUtil.js
...
//#endregion
//#region node_modules/@a2ui/web_core/node_modules/zod/v3/helpers/errorUtil.js
var errorUtil;
(function(errorUtil) {
...
//#endregion
//#region node_modules/@a2ui/web_core/node_modules/zod/v3/types.js
...
ZodPipeline.create;
//#endregion
//#region node_modules/@a2ui/web_core/src/v0_9/schema/common-types.js
...
const ComponentIdSchema = stringType().describe("REF:common_types.json#/$defs/ComponentId|The unique identifier for a component.");
unionType([arrayType(ComponentIdSchema).describe("A static list of child component IDs."), objectType({
...
unionType([objectType({ "event": objectType({
	"name": stringType(),
...
objectType({
	...
	}).describe("REF:common_types.json#/$defs/CheckRule|A check rule consisting of a condition and an error message.")).optional().describe("A list of checks to perform."),
...
const A2uiMessageSchema = unionType([
	objectType({
...
objectType({ messages: arrayType(A2uiMessageSchema).describe("A list of messages.") }).strict().describe("An object wrapping a list of messages.");
//#endregion
...
function validateA2UIJsonl(jsonl) {
	...
	lines.forEach((line, idx) => {
		...
		const actionKeys = (explicitVersion === "v0.9" ? A2UI_V09_ACTION_KEYS : A2UI_V08_ACTION_KEYS).filter((key) => key in record);
		if (actionKeys.length !== 1) {
			errors.push(`line ${idx + 1}: expected exactly one ${explicitVersion === "v0.9" ? "v0.9" : "v0.8"} action key`);
			return;
		}
		const allowedTopLevelKeys = new Set(explicitVersion === "v0.9" ? ["version", actionKeys[0]] : [actionKeys[0]]);
		...
		if (explicitVersion === "v0.9") {
			const result = A2uiMessageSchema.safeParse(record);
			if (!result.success) {
				errors.push(`line ${idx + 1}: ${result.error.issues[0]?.message ?? "invalid v0.9 message"}`);
				return;
			}
			sawV09 = true;
		} else sawV08 = true;
...
//#endregion
//#region extensions/canvas/src/board-widget.ts
...
function bootScript(messages, promptGranted) {
	return `<script>globalThis.openclawA2UIBoot=${JSON.stringify({
		messages,
		actionTier: promptGranted ? "prompt" : "state"
	}).replaceAll("<", "\\u003c").replaceAll("\u2028", "\\u2028").replaceAll("\u2029", "\\u2029")};<\/script>`;
}
function resourceScript(path, url) {
	if (url !== path) return `<script src="${escapeHtmlAttribute(url)}"><\/script>`;
	return `<script>(()=>{const match=location.pathname.match(/^\\/__openclaw__\\/cap\\/[^/]+/u);const script=document.createElement("script");script.src=(match?.[0]??"")+${JSON.stringify(path).replaceAll("<", "\\u003c")};document.head.appendChild(script);})();<\/script>`;
}
/** Canvas-owned A2UI source validation and document composition for boards. */
const canvasA2UIBoardWidgetKind = {
	kind: "a2ui",
	label: "A2UI",
	resources: {
		surface: "canvas",
	...
	composeDocument({ source, resourceUrls, promptGranted }) {
		...
		if (!bundleUrl) throw new Error(`A2UI renderer resource unavailable: ${bundlePath}`);
...
//#endregion
//#region extensions/canvas/src/widget-presenter.ts
const DEFAULT_CANVAS_NODE_INVOKE_TIMEOUT_MS = 3e4;
async function selectCanvasNode(nodesRuntime) {
...
function createCanvasWidgetPresenter(nodesRuntime) {
	return {
		target: "node_panel",
		description: "Show on a connected device panel",
		async availability() {
			try {
				return await selectCanvasNode(nodesRuntime) ? {
					ok: true,
...
function createLazyCanvasTool(agentSessionKey) {
	const loadTool = createLazyRuntimeModule(() => import("../../tool-CTGneOrj.js").then(({ createCanvasTool }) => createCanvasTool({ agentSessionKey })));
	return {
		label: "Canvas",
		name: "canvas",
...
var canvas_default = definePluginEntry({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			...
			const loadHttpRouteHandler = createLazyRuntimeModule(() => import("../../http-route-Clf-mMry.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler()));
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-CzwvsPZD.js");
