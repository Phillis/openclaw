...
import { X as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME } from "../../cdp.helpers-geYy1F8f.js";
import "../../paths-oxXTelRQ.js";
import { r as resolveBrowserConfig } from "../../config-HQZ6_yJ9.js";
import { H as readBrowserVersion, U as resolveBrowserExecutableForPlatform, V as parseBrowserMajorVersion, W as resolveGoogleChromeExecutableForPlatform } from "../../chrome-DjHNxomb.js";
import { a as repairOwnedChromeExtensionNativeHosts, n as browserExtensionStatus, t as FOUNDATION_CHROME_WEB_STORE_URL } from "../../extension-install-DGBxJr3z.js";
import { t as movePathToTrash } from "../../trash-CmvqpBj1.js";
import { r as listSystemProfiles } from "../../system-profiles-BAr5tVgs.js";
...
const BROWSER_PLUGIN_ROOT = fs.existsSync(path.join(BROWSER_MODULE_DIR, "package.json")) ? BROWSER_MODULE_DIR : path.dirname(BROWSER_MODULE_DIR);
...
function isLegacyClawdProfileConfigured(cfg, legacyProfileDir) {
	...
	for (const rawProfile of Object.values(configuredProfiles)) {
		...
		if (userDataDir && isPathInside(legacyProfileDir, resolveUserPath(userDataDir))) return true;
	}
	return false;
}
...
async function noteChromeMcpBrowserReadiness(cfg, deps) {
	...
	const resolved = resolveBrowserConfig(cfg.browser, cfg);
	if (resolved.enabled && resolved.extensionRelay.allowLegacyAuth) noteFn([
	...
	try {
		const extension = fs.existsSync(extensionCopyPath) ? await browserExtensionStatus({
		...
		if (extension && (extension.installedCopy.present || extension.discovered.length > 0)) {
			if (extension.manualSetupRequired) noteFn([
		...
		noteFn(`- Chrome extension bootstrap status could not be inspected: ${error instanceof Error ? error.message : String(error)}`, "Browser extension bootstrap");
	}
...
//#endregion
export { detectLegacyClawdBrowserProfileResidue, maybeArchiveLegacyClawdBrowserProfileResidue, maybeRepairOwnedChromeExtensionNativeHosts, noteChromeMcpBrowserReadiness };
