...
function normalizeVolcengineProviderConfig(rawConfig) {
	const raw = asOptionalRecord(asOptionalRecord(rawConfig.providers)?.volcengine) ?? asOptionalRecord(rawConfig.volcengine);
	return {
		...
		cluster: trimToUndefined(raw?.cluster) ?? trimToUndefined(process.env.VOLCENGINE_TTS_CLUSTER) ?? DEFAULT_CLUSTER,
