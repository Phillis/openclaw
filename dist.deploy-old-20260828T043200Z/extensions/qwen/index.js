import { QWEN_DEFAULT_MODEL_REF, QWEN_TOKEN_PLAN_DEFAULT_MODEL_REF, QWEN_TOKEN_PLAN_LEGACY_PROVIDER_ID, QWEN_TOKEN_PLAN_PROVIDER_ID, isQwenCodingPlanBaseUrl, isQwenStandardOnlyModelId, isQwenTokenPlanDeepSeekV4ModelId, isQwenTokenPlanGlmModelId, isQwenTokenPlanThinkingOnlyModelId, supportsQwenTokenPlanGlmMaxThinking } from "./models.js";
		...
		api.registerProvider({
			...
			catalog: {
				...
				run: async (ctx) => {
					...
					return { provider: await buildOpenAICompatibleLiveModelProviderConfig({
						providerId: QWEN_TOKEN_PLAN_PROVIDER_ID,
						providerConfig: buildQwenTokenPlanProvider({ baseUrl: resolveConfiguredQwenTokenPlanBaseUrl(ctx.config) }),
						apiKey: auth.apiKey,
						discoveryApiKey: auth.discoveryApiKey
					}) };
				}
