import { F as resolveTimerTimeoutMs, d as asPositiveSafeInteger } from "./number-coercion-CLj0HTDM.js";
...
import { c as isNonSecretApiKeyMarker, s as isKnownEnvApiKeyMarker } from "./model-auth-markers-CYmICvL9.js";
import { t as normalizeOptionalSecretInput } from "./normalize-secret-input-Df_qhWv_.js";
import { i as normalizeApiKeyConfig } from "./models-config.providers.secrets-C9iSVX4J.js";
import "./provider-auth-DI4TAoBi.js";
import "./response-limit-runtime-Dd4g9Wqb.js";
import "./number-runtime-Cy4drVnh.js";
import "./string-coerce-runtime-C8jKEm3h.js";
import { c as SELF_HOSTED_DEFAULT_MAX_TOKENS, s as SELF_HOSTED_DEFAULT_COST } from "./provider-setup-C98Jreue.js";
...
import { i as normalizeLmstudioTransportReasoningCompat, n as LMSTUDIO_OPENAI_COMPAT_REASONING_EFFORTS, r as buildLmstudioReasoningEffortMap, t as LMSTUDIO_OPENAI_COMPAT_ENABLED_REASONING_EFFORTS } from "./model-reasoning-BuI3S4Lz.js";
...
function normalizeLmstudioConfiguredCompat(value) {
	...
	if (record.codeMode === "preferred" || record.codeMode === "capable") compat.codeMode = record.codeMode;
...
function sanitizeLmstudioStringHeaders(headers) {
...
async function resolveLmstudioConfiguredApiKeyForProvider(params) {
	...
	const apiKeyInput = (config?.models?.providers?.[params.providerId])?.apiKey;
	if (apiKeyInput === void 0 || apiKeyInput === null || !config) return;
	...
	if (!trimmed) return;
	if (directApiKey !== void 0 && isKnownEnvApiKeyMarker(trimmed)) return normalizeOptionalSecretInput(env[trimmed]);
...
async function resolveLmstudioProviderHeaders(params) {
	...
	if (!params.config) return sanitizeLmstudioStringHeaders(headerInputs);
...
function redactLmstudioLoadError(value, headers) {
	return redactToolPayloadText(Object.entries(headers ?? {}).filter(([name]) => name.toLowerCase() !== "content-type").flatMap(([name, header]) => {
		const normalized = header.trim();
		if (!normalized) return [];
		return name.toLowerCase() === "authorization" ? [normalized, normalized.replace(/^\S+\s+/u, "")] : [normalized];
	}).toSorted((left, right) => right.length - left.length).reduce((redacted, credential) => redacted.replaceAll(credential, "***"), value));
}
...
async function ensureLmstudioModelLoaded(params) {
	...
	try {
		...
		try {
			if (!response.ok) {
				const bodyRead = await readResponseTextPrefix(response, LMSTUDIO_ERROR_BODY_LIMIT_BYTES, { chunkTimeoutMs: 1e4 });
				const detail = bodyRead.truncated ? "" : redactLmstudioLoadError(bodyRead.text, requestHeaders);
				throw new Error(`LM Studio model load failed (${response.status})${detail ? `: ${detail}` : ""}`);
			}
...
//#endregion
export { LMSTUDIO_DEFAULT_INFERENCE_BASE_URL as A, resolveLmstudioReasoningCapability as C, LMSTUDIO_DEFAULT_API_KEY_ENV_VAR as D, resolveLoadedContextWindow as E, LMSTUDIO_LOCAL_API_KEY_PLACEHOLDER as F, LMSTUDIO_MODEL_PLACEHOLDER as I, LMSTUDIO_PROVIDER_ID as L, LMSTUDIO_DEFAULT_MODEL_ID as M, LMSTUDIO_DOCKER_HOST_BASE_URL as N, LMSTUDIO_DEFAULT_BASE_URL as O, LMSTUDIO_DOCKER_HOST_INFERENCE_BASE_URL as P, LMSTUDIO_PROVIDER_LABEL as R, resolveLmstudioInferenceBase as S, resolveLmstudioServerBase as T, mapLmstudioWireModelsToConfig as _, resolveLmstudioConfiguredApiKey as a, normalizeLmstudioProviderConfig as b, resolveLmstudioRequestContext as c, hasLmstudioAuthorizationHeader as d, resolveLmstudioProviderAuthMode as f, mapLmstudioWireEntry as g, buildLmstudioModelName as h, buildLmstudioAuthHeaders as i, LMSTUDIO_DEFAULT_LOAD_CONTEXT_LENGTH as j, LMSTUDIO_DEFAULT_EMBEDDING_MODEL as k, resolveLmstudioRuntimeApiKey as l, shouldUseLmstudioSyntheticAuth as m, ensureLmstudioModelLoaded as n, resolveLmstudioConfiguredApiKeyForProvider as o, shouldUseLmstudioApiKeyPlaceholder as p, fetchLmstudioModels as r, resolveLmstudioProviderHeaders as s, discoverLmstudioModels as t, sanitizeLmstudioStringHeaders as u, normalizeLmstudioConfiguredCatalogEntries as v, resolveLmstudioReasoningCompat as w, resolveLmstudioCanonicalModelKey as x, normalizeLmstudioConfiguredCatalogEntry as y };
