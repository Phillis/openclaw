...
import { d as loadTaskRegistryStateFromSqliteReadOnly } from "./task-registry.store.sqlite-7NOoQ9mC.js";
import { a as loadTaskFlowRegistryStateFromSqliteReadOnly } from "./task-flow-registry.store.sqlite-BzofymYG.js";
import { t as note } from "./note-YH_0kY-3.js";
import { i as buildPluginCompatibilityWarnings } from "./status-DoE4_lNj.js";
...
function collectTaskFlowRecoveryFindings() {
	...
	const tasksByFlowId = /* @__PURE__ */ new Map();
	for (const task of loadTaskRegistryStateFromSqliteReadOnly().tasks.values()) {
		...
		if (flowId) {
			...
			else tasksByFlowId.set(flowId, [task]);
		}
	}
	return flows.flatMap((flow) => {
		const linkedTasks = tasksByFlowId.get(flow.flowId) ?? [];
		const findings = [];
		if (flow.syncMode === "managed" && flow.status === "running" && linkedTasks.length === 0 && flow.waitJson === void 0) findings.push({
			flowId: flow.flowId,
			message: `${flow.flowId}: running managed TaskFlow has no linked tasks or wait state; inspect or cancel it manually.`
		});
		if (flow.endedAt == null && flow.status === "blocked" && flow.blockedTaskId && !linkedTasks.some((task) => task.taskId === flow.blockedTaskId)) findings.push({
			flowId: flow.flowId,
...
function collectWorkspaceStatusHealthFindings(cfg, options = {}) {
	...
	for (const { agentId, workspaceDir } of scopes) {
		const collectForWorkspace = () => {
			...
			for (const diagnostic of pluginRegistry.diagnostics) findings.push(pluginDiagnosticToHealthFinding(diagnostic, `${prefix}${diagnostic.message}`));
			return findings;
		};
