...
import { a as isExpectedCliError, i as formatCliOperatorError } from "./failure-output-CdUzE2dC.js";
...
async function runCommandWithRuntime(runtime, action, onError) {
	try {
		...
		if (isJsonOutputModeActive(process.argv) || isExpectedCliError(err)) throw err;
		if (onError) {
			onError(err);
			return;
		}
