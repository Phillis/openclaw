...
function buildWidgetDocument(title, widgetCode, options = {}) {
	...
	const widgetBridge = "<script>(()=>{if(!window.parent||window.parent===window||Object.prototype.hasOwnProperty.call(window,\"openclaw\"))return;const parent=window.parent;const post=parent.postMessage.bind(parent);const P=Promise;const then=P.prototype.then;const ErrorCtor=Error;const stringify=String;const freeze=Object.freeze;const define=Object.defineProperty;const push=Array.prototype.push;const shift=Array.prototype.shift;const listen=window.addEventListener.bind(window);const path=Event.prototype.composedPath;const prevent=Event.prototype.preventDefault;const test=RegExp.prototype.test;const later=setTimeout.bind(window);const cancel=clearTimeout.bind(window);const c=new MessageChannel();const promptPost=c.port1.postMessage.bind(c.port1);const b=new MessageChannel();const bridgePost=b.port1.postMessage.bind(b.port1);const promptWaiting=[];let inlinePromptReady=false;c.port1.addEventListener(\"message\",event=>{if(event.data?.type!==\"openclaw:widget-prompt-host-ready\"||inlinePromptReady)return;inlinePromptReady=true;while(promptWaiting.length){const entry=shift.call(promptWaiting);if(entry)entry.inline();}});c.port1.start();let act=null;try{const ua=navigator.userActivation;const d=ua&&Object.getOwnPropertyDescriptor(Object.getPrototypeOf(ua),\"isActive\");if(d&&d.get)act=d.get.bind(ua);}catch{}post({type:\"openclaw:widget-prompt-offer\"},\"*\",[c.port2]);post({type:\"openclaw:widget-bridge-port-offer\"},\"*\",[b.port2]);let ticket=null;let controlUiBaseUrl=null;let sequence=0;let hostInitExpired=false;const pending=new Map();const waiting=[];const initTimer=later(()=>{hostInitExpired=true;while(waiting.length){const entry=shift.call(waiting);if(entry)entry.reject(new ErrorCtor(\"widget host capabilities unavailable\"));}while(promptWaiting.length){const entry=shift.call(promptWaiting);if(entry)entry.reject(new ErrorCtor(\"widget prompt host unavailable\"));}},5000);b.port1.addEventListener(\"message\",event=>{const data=event.data;if(data?.type===\"openclaw:widget-host-init\"&&typeof data.ticket===\"string\"){ticket=data.ticket;const base=data.controlUiBaseUrl;controlUiBaseUrl=typeof base===\"string\"&&/^https?:\\/\\//.test(base)?base:null;bridgePost({type:\"openclaw:widget-host-init-ack\",ticket});cancel(initTimer);while(waiting.length){const entry=shift.call(waiting);if(entry)entry.send();}while(promptWaiting.length){const entry=shift.call(promptWaiting);if(entry)entry.send();}return;}if(data?.type!==\"openclaw:widget-bridge-response\"||typeof data.id!==\"string\")return;const entry=pending.get(data.id);if(!entry)return;pending.delete(data.id);if(data.ok===true)entry.resolve(data.result);else entry.reject(new ErrorCtor(typeof data.error===\"string\"?data.error:\"widget host request failed\"));});b.port1.start();const request=(method,params)=>new P((resolve,reject)=>{const send=()=>{const id=\"widget-\"+(++sequence);pending.set(id,{resolve,reject});try{bridgePost({type:\"openclaw:widget-bridge-request\",id,method,params,ticket});}catch(error){pending.delete(id);reject(error);}};if(ticket)send();else if(hostInitExpired)reject(new ErrorCtor(\"widget host capabilities unavailable\"));else push.call(waiting,{send,reject});});const activate=event=>{if(event.isTrusted!==true||!ticket||event.defaultPrevented||event.shiftKey||event.altKey)return;const auxiliary=event.type===\"auxclick\"&&event.button===1;if(!auxiliary&&!(event.type===\"click\"&&event.button===0))return;for(let index=0,entries=path.call(event);index<entries.length;index++){const anchor=entries[index];if(anchor?.tagName!==\"A\")continue;const url=anchor.href;if(!url)continue;if(!auxiliary&&anchor.target!==\"_blank\")return;if(!test.call(/^https?:\\/\\//i,url))return;prevent.call(event);then.call(request(\"host.open\",{url}),()=>{},()=>{});return;}};listen(\"click\",activate);listen(\"auxclick\",activate);const sendPrompt=text=>{if(!act||act()!==true)return P.resolve(false);const value=stringify(text);if(ticket)return request(\"prompt.send\",{text:value});return new P((resolve,reject)=>{const send=()=>{const result=request(\"prompt.send\",{text:value});then.call(result,resolve,reject);};const inline=()=>{promptPost({type:\"openclaw:widget-prompt\",prompt:value});resolve(true);};if(inlinePromptReady)inline();else if(hostInitExpired)reject(new ErrorCtor(\"widget prompt host unavailable\"));else push.call(promptWaiting,{send,inline,reject});});};const host={};define(host,\"controlUiBaseUrl\",{enumerable:true,get:()=>controlUiBaseUrl});freeze(host);const api=freeze({host,prompt:freeze({send:sendPrompt}),state:freeze({emit:payload=>request(\"state.emit\",{payload})}),data:freeze({read:(bindingId,params)=>request(\"data.read\",{bindingId:stringify(bindingId),params})}),action:freeze({run:(action,params)=>request(\"action.run\",{action:stringify(action),params:params===undefined?{}:params})}),cron:freeze({trigger:jobId=>request(\"cron.trigger\",{jobId:stringify(jobId)})})});define(window,\"openclaw\",{value:api,writable:false,configurable:false});window.sendPrompt=text=>{void sendPrompt(text);};define(window,\"sendPrompt\",{value:window.sendPrompt,writable:false,configurable:false});post({type:\"openclaw:widget-bridge-ready\"},\"*\");})();<\/script>";
