...
import { i as listThinkingLevelOptions, r as listThinkingLevelLabels } from "./thinking-CNREPJ80.js";
import { n as shouldForwardModelCommandToServer } from "./commands-registry.data-DzbWwJN6.js";
...
import "./usage-format-DErPHjcA.js";
...
async function runTuiUnlocked(opts) {
	...
	if (opts.backend) client = opts.backend;
		...
		const { EmbeddedTuiBackend } = await import("./embedded-backend-BDpzzjOh.js");
		client = new EmbeddedTuiBackend();
	} else {
		const { GatewayChatClient } = await import("./gateway-chat-Bbk2ZgDK.js");
	...
	import("./tools-manager-B-VXu4XA.js").then(({ ensureTool }) => ensureTool("fd", true)).then((fdPath) => {
		if (fdPath) {
