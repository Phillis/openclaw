			...
			if (editSucceeded) {
				...
				if (supplementalPayload !== void 0) {
					...
					if (!params.deliverSupplemental || supplementalDelivered === false) {
						if (await params.deliverNormally(supplementalPayload) === false) throw new Error("Live preview supplemental payload was not delivered");
					}
				}
				return {
	...
	try {
		...
		if (delivered) {
			...
			await runBestEffortCleanup({
				cleanup: () => draft.clear(),
				onError: () => console.warn("Live preview cleanup failed after delivery; a stale preview may remain")
			});
		}
	}
	return {
