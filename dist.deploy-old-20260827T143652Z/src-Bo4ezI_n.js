...
import { a as NonEmptyString, i as InputProvenanceSchema, n as GatewayClientIdSchema, o as SecretInputSchema, r as GatewayClientModeSchema, s as SessionLabelString, t as ChatSendSessionKeyString } from "./primitives-BSb7UY-n.js";
import { E as WorkerLiveEventParamsSchema, F as WorkerSessionsSpawnParamsSchema, N as WorkerSessionsSendParamsSchema, at as withSince, b as WorkerConnectRequestFrameSchema, it as FailoverReasonSchema, v as WorkerAdmissionHandshakeSchema, x as WorkerHeartbeatParamsSchema, z as WorkerTranscriptCommitParamsSchema } from "./worker-admission-R0mXKdG7.js";
...
import { a as UsersListParamsSchema, d as UsersSelfParamsSchema, f as UsersSelfResultSchema, g as UsersSetDisplayNameResultSchema, h as UsersSetDisplayNameParamsSchema, i as UsersLinkEmailResultSchema, l as UsersPrefsSetParamsSchema, m as UsersSetAvatarResultSchema, p as UsersSetAvatarParamsSchema, r as UsersLinkEmailParamsSchema, s as UsersPrefsGetParamsSchema } from "./users-Bz4Tf7LO.js";
...
const SkillsSearchResultSchema = closedObject({ results: Type.Array(closedObject({
	score: Type.Number(),
	slug: NonEmptyString,
	installRef: Type.Optional(Type.String({
		minLength: 1,
		description: "Publisher-qualified reference for this result. Send it as `slug` to skills.detail and skills.install; several publishers can share one slug."
	})),
	displayName: NonEmptyString,
	summary: Type.Optional(Type.String()),
...
*/
const SessionsSendReconcileParamsSchema = closedObject({
	key: NonEmptyString,
	agentId: Type.Optional(NonEmptyString),
	runId: NonEmptyString
});
const SessionsSendReconcileResultSchema = closedObject({
	key: NonEmptyString,
	agentId: NonEmptyString,
	runId: NonEmptyString,
	status: Type.Union([
...
const ReleaseRequestIdSchema = Type.String({ pattern: "^handoff-v2-release:[a-f0-9]{32}$" });
...
const GATEWAY_SUSPEND_MODE_DURABLE = "handoff-durable-hold/v1";
const GatewaySuspendModeSchema = Type.Union([Type.Literal(GATEWAY_SUSPEND_MODE_LEGACY), Type.Literal(GATEWAY_SUSPEND_MODE_DURABLE)]);
...
const GatewaySuspendActiveStatusParamsSchema = closedObject({
	suspensionId: SuspensionTokenSchema,
	gatewayInstanceId: SuspensionTokenSchema,
	suspendMode: Type.Optional(GatewaySuspendModeSchema)
});
const GatewaySuspendReleaseStatusParamsSchema = closedObject({
...
const GatewaySuspendStatusParamsSchema = Type.Union([GatewaySuspendActiveStatusParamsSchema, GatewaySuspendReleaseStatusParamsSchema]);
const GatewaySuspendStatusRunningResultSchema = closedObject({
	status: Type.Literal("running"),
	gatewayInstanceId: SuspensionTokenSchema,
	suspendMode: GatewaySuspendModeSchema
});
...
const GatewaySuspendReleaseCommittedReceiptSchema = closedObject({
	...GatewaySuspendReleaseBindingSchema,
	status: Type.Literal("release_committed")
});
const GatewaySuspendReleaseCompletedReceiptSchema = closedObject({
	...GatewaySuspendReleaseBindingSchema,
...
const GatewaySuspendReleaseReceiptSchema = Type.Union([GatewaySuspendReleaseCommittedReceiptSchema, GatewaySuspendReleaseCompletedReceiptSchema]);
const GatewaySuspendReleaseRecoveryNeededResultSchema = closedObject({
...
const GatewaySuspendLegacyResumeParamsSchema = closedObject({
	suspensionId: SuspensionTokenSchema,
	gatewayInstanceId: SuspensionTokenSchema,
	resumeBeforeMs: CountSchema,
	suspendMode: Type.Optional(Type.Literal(GATEWAY_SUSPEND_MODE_LEGACY))
});
const GatewaySuspendDurableResumeParamsSchema = closedObject({
	suspensionId: SuspensionTokenSchema,
	gatewayInstanceId: SuspensionTokenSchema,
	resumeBeforeMs: CountSchema,
	suspendMode: Type.Literal(GATEWAY_SUSPEND_MODE_DURABLE),
...
const GatewaySuspendResumeParamsSchema = Type.Union([GatewaySuspendLegacyResumeParamsSchema, GatewaySuspendDurableResumeParamsSchema]);
const GatewaySuspendLegacyResumeResultSchema = closedObject({
	ok: Type.Literal(true),
	status: Type.Literal("running"),
...
const GatewaySuspendDurableResumeResultSchema = closedObject({
	ok: Type.Literal(true),
	status: Type.Literal("running"),
	resumed: Type.Boolean(),
	gatewayInstanceId: SuspensionTokenSchema,
...
const GatewaySuspendResumeResultSchema = Type.Union([GatewaySuspendLegacyResumeResultSchema, GatewaySuspendDurableResumeResultSchema]);
//#endregion
...
const SecretsStoreDeleteParamsSchema = closedObject({ name: SecretStoreNameSchema });
...
const validateSessionsSendReconcileParams = /* @__PURE__ */ lazyCompile(SessionsSendReconcileParamsSchema);
const validateSessionsSendReconcileResult = /* @__PURE__ */ lazyCompile(SessionsSendReconcileResultSchema);
...
const validateSessionsPatchParams = /* @__PURE__ */ lazyCompile(Type.Object({
	...SessionsPatchParamsSchema.properties,
...
//#endregion
export { validateConfigSchemaParams as $, TerminalUploadResultSchema as $_, validateWorkerHeartbeatParams as $a, WorkerDesktopLaunchParamsSchema as $c, ProjectsSearchRemoteParamsSchema as $d, SessionsCompactionBranchParamsSchema as $f, UiSidebarCommandSchema as $g, BoardChatDockSchema as $h, validateTalkSessionAcknowledgeMarkParams as $i, ChannelsPairingApproveResultSchema as $l, AgentsUpdateResultSchema as $m, validateSecretsStoreSetParams as $n, SessionDiscussionInfoParamsSchema as $o, ChatHistoryParamsSchema as $p, validateSessionsReclaimParams as $r, NodePendingAckParamsSchema as $s, validateModelsProbeParams as $t, AuditActivityListResultSchema as $u, ClawHubTrustErrorCodes as $v, validateBoardWidgetContent as A, SessionApprovalReplaySchema as A_, validateUsersSelfParams as Aa, FsDirEntrySchema as Ac, validateMigrationsMemoryApplyParams as Ad, SessionDiffCommitSchema as Af, BoardWidgetHeightModeSchema as Ag, SkillsProposalsListParamsSchema as Ah, validateSkillsUploadChunkParams as Ai, SystemAgentSetupAuthStartResultSchema as Al, UpdateAvailableSchema as Am, validatePortalCloseParams as An, PortalSummarySchema as Ao, SessionsObserverVisibilityParamsSchema as Ap, validateSessionsDeleteParams as Ar, QuestionRequestedEventSchema as As, validateEnvironmentsDestroyParams as At, TalkSessionSubmitToolResultParamsSchema as Au, PluginsInstallParamsSchema as Av, validateChatAbortParams as B, TerminalAttachParamsSchema as B_, validateWebPushTestParams as Ba, ExecApprovalsGetParamsSchema as Bc, ProjectRecentFolderSchema as Bd, SessionFileRelevanceSchema as Bf, BoardWidgetPutContentSchema as Bg, SkillsUploadBeginParamsSchema as Bh, validateSystemInfoResult as Bi, WizardCancelParamsSchema as Bl, AgentsCreateResultSchema as Bm, validateQuestionListParams as Bn, TaskSuggestionEventSchema as Bo, SessionsSendParamsSchema as Bp, validateSessionsGroupsListParams as Br, WebPushSubscribeParamsSchema as Bs, validateExecApprovalsSetParams as Bt, AuditRunIdentityAmbiguousV1Schema as Bu, PluginsSetEnabledParamsSchema as Bv, validateBoardActionParams as C, DeniedApprovalSnapshotSchema as C_, validateUpdateStatusParams as Ca, GatewaySuspendResumeParamsSchema as Cc, ConversationTurnParamsSchema as Cd, SessionSharingEventSchema as Cf, BoardViewTicketSchema as Cg, SkillsProposalInspectParamsSchema as Ch, validateSkillsProposalsListParams as Ci, SystemAgentChatHistoryTurnSchema as Cl, ConfigGetParamsSchema as Cm, validatePluginsSessionActionParams as Cn, PortalChangedEventSchema as Co, SessionsGroupsDeleteParamsSchema as Cp, validateSessionsCompactionBranchParams as Cr, QuestionListParamsSchema as Cs, validateDevicePairRemoveParams as Ct, TalkSessionAppendAudioParamsSchema as Cu, SessionVisibilitySchema as Cv, validateBoardPromptAuthorizeParams as D, PluginApprovalPresentationSchema as D_, validateUsersListParams as Da, GatewaySuspendStatusResultSchema as Dc, PollParamsSchema as Dd, SESSION_OBSERVER_HEALTH_VALUES as Df, BoardWidgetDeclaredSchema as Dg, SkillsProposalRequestRevisionResultSchema as Dh, validateSkillsStatusParams as Di, SystemAgentSetupActivateParamsSchema as Dl, ConfigSchemaParamsSchema as Dm, validatePluginsUiDescriptorsResult as Dn, PortalListResultSchema as Do, SessionsGroupsPutParamsSchema as Dp, validateSessionsCompanionResetParams as Dr, QuestionRequestParamsSchema as Ds, validateDeviceTokenRevokeParams as Dt, TalkSessionCreateResultSchema as Du, PluginCatalogInstallActionSchema as Dv, validateBoardGetParams as E, PendingApprovalSnapshotSchema as E_, validateUsersLinkEmailResult as Ea, GatewaySuspendStatusReadyResultSchema as Ec, MessageActionParamsSchema as Ed, SessionVisibilitySetResultSchema as Ef, BoardWidgetContentSchema as Eg, SkillsProposalRequestRevisionParamsSchema as Eh, validateSkillsSkillCardParams as Ei, SystemAgentChatResultSchema as El, ConfigSchemaLookupResultSchema as Em, validatePluginsUiDescriptorsParams as En, PortalListParamsSchema as Eo, SessionsGroupsMutationResultSchema as Ep, validateSessionsCompanionAskParams as Er, QuestionRecordSchema as Es, validateDevicePairSetupStatusParams as Et, TalkSessionCreateParamsSchema as Eu, PluginCatalogEntrySchema as Ev, validateChannelsPairingDismissParams as F, validateTerminalOpenParams as F_, validateUsersSetDisplayNameResult as Fa, DesktopObserveResultSchema as Fc, MigrationsMemoryPlanParamsSchema as Fd, SessionFileBrowserResultSchema as Ff, BoardWidgetNameSchema as Fg, SkillsSecurityVerdictsResultSchema as Fh, validateSystemAgentSetupAuthStartParams as Fi, SystemChangeEntrySchema as Fl, UpdateStatusParamsSchema as Fm, validateProjectsRegisterParams as Fn, TasksGetResultSchema as Fo, SessionsRewindParamsSchema as Fp, validateSessionsFilesListParams as Fr, QuestionStatusSchema as Fs, validateExecApprovalResolveParams as Ft, WebLoginStartParamsSchema as Fu, PluginsRefreshResultSchema as Fv, validateChatSendParams as G, TerminalExitEventSchema as G_, validateWizardStartParams as Ga, EnvironmentsCreateResultSchema as Gc, ProjectsAddParamsSchema as Gd, SessionWorktreeInfoSchema as Gf, BoardWidgetSchema as Gg, ToolsInvokeParamsSchema as Gh, validateTalkClientMutationResult as Gi, WizardStatusParamsSchema as Gl, AgentsFilesGetResultSchema as Gm, validateScopeUpgradeRequest as Gn, TaskSuggestionsCreateParamsSchema as Go, SessionsPatchManyTargetSchema as Gp, validateSessionsListParams as Gr, NodeInvokeInputEventSchema as Gs, validateGatewaySuspendResumeParams as Gt, AuditRunInspectParamsSchema as Gu, PluginsUninstallResultSchema as Gv, validateChatInjectParams as H, TerminalCloseParamsSchema as H_, validateWebPushVapidPublicKeyParams as Ha, EnvironmentStatusSchema as Hc, ProjectRecentSchema as Hd, SessionObserverDigestSchema as Hf, BoardWidgetPutResultSchema as Hg, SkillsUploadCommitParamsSchema as Hh, validateTalkClientCloseParams as Hi, WizardNextResultSchema as Hl, AgentsDeleteResultSchema as Hm, validateQuestionResolveParams as Hn, TaskSuggestionSchema as Ho, SESSIONS_PATCH_MANY_MAX_TARGETS as Hp, validateSessionsGroupsMutationResult as Hr, WebPushUnsubscribeParamsSchema as Hs, validateFsListDirParams as Ht, AuditRunIdentityUnknownV1Schema as Hu, PluginsUiDescriptorsParamsSchema as Hv, validateChannelsPairingListParams as I, validateTerminalResizeParams as I_, validateWakeParams as Ia, DesktopSourceSchema as Ic, PROJECTS_LIST_DEFAULT_LIMIT as Id, SessionFileContentEncodingSchema as If, BoardWidgetPluginContentSchema as Ig, SkillsSkillCardParamsSchema as Ih, validateSystemAgentSetupDetectParams as Ii, SystemChangeKindSchema as Il, UpdateStatusResultSchema as Im, validateProjectsRemoveParams as In, TasksListParamsSchema as Io, SessionsRewindResultSchema as Ip, validateSessionsFilesRevealParams as Ir, QuestionWaitAnswerParamsSchema as Is, validateExecApprovalsGetParams as It, WebLoginWaitParamsSchema as Iu, PluginsSearchParamsSchema as Iv, validateConfigApplyParams as J, TerminalOpenParamsSchema as J_, validateWorkerConnectRequestFrame as Ja, EnvironmentsListParamsSchema as Jc, ProjectsListResultSchema as Jd, SessionsBranchesListResultSchema as Jf, UiCommandResultSchema as Jg, BOARD_DATA_BINDING_ID_MAX_LENGTH as Jh, validateTalkClientToolCallResult as Ji, COMMAND_DESCRIPTION_MAX_LENGTH as Jl, AgentsFilesSetParamsSchema as Jm, validateSecretsResolveResult as Jn, TaskSuggestionsDismissResultSchema as Jo, SessionsResolveParamsSchema as Jp, validateSessionsObserverVisibilityParams as Jr, NodeListParamsSchema as Js, validateLogsTailParams as Jt, ExecutionIdentityContextV1Schema as Ju, SystemAgentErrorDetailCodes as Jv, validateChatToolTitlesParams as K, TerminalInputParamsSchema as K_, validateWizardStatusParams as Ka, EnvironmentsDestroyParamsSchema as Kc, ProjectsAddResultSchema as Kd, SessionsAbortParamsSchema as Kf, UiClosePaneCommandSchema as Kg, BOARD_CRON_JOB_ID_MAX_LENGTH as Kh, validateTalkClientSteerParams as Ki, WizardStatusResultSchema as Kl, AgentsFilesListParamsSchema as Km, validateScopeUpgradeWait as Kn, TaskSuggestionsCreateResultSchema as Ko, SessionsPatchMutationSchema as Kp, validateSessionsMessagesSubscribeParams as Kr, NodeInvokeParamsSchema as Ks, validateGatewaySuspendStatusParams as Kt, AuditRunInspectResultSchema as Ku, lazyCompile as Kv, validateChannelsStartParams as L, validateTerminalUploadParams as L_, validateWebLoginStartParams as La, ExecApprovalGetParamsSchema as Lc, PROJECTS_LIST_MAX_CHECKOUTS_PER_PROJECT as Ld, SessionFileEntrySchema as Lf, BoardWidgetPluginKindSchema as Lg, SkillsSkillCardResultSchema as Lh, validateSystemAgentSetupVerifyParams as Li, SystemChangeSourceSchema as Ll, AgentKindSchema as Lm, validateProjectsSearchRemoteParams as Ln, TasksListResultSchema as Lo, SessionsSearchHitSchema as Lp, validateSessionsFilesSetParams as Lr, QuestionWaitAnswerResultSchema as Ls, validateExecApprovalsNodeGetParams as Lt, AuditEventSchema as Lu, PluginsSearchResultSchema as Lv, validateBoardWidgetPutParams as M, validateTerminalAttachParams as M_, validateUsersSetAvatarParams as Ma, FsListDirResultSchema as Mc, MAX_MEMORY_MIGRATION_ITEMS as Md, SessionDiffFileStatusSchema as Mf, BoardWidgetMcpAppContentSchema as Mg, SkillsSearchParamsSchema as Mh, validateSystemAgentChatHistoryParams as Mi, SystemAgentSetupDetectResultSchema as Ml, UpdateHoldResultSchema as Mm, validatePortalOpenParams as Mn, TasksCancelParamsSchema as Mo, SessionsPluginPatchParamsSchema as Mp, validateSessionsDiffParams as Mr, QuestionResolveResultSchema as Ms, validateEnvironmentsStatusParams as Mt, TalkSpeakResultSchema as Mu, PluginsListParamsSchema as Mv, validateChannelsLogoutParams as N, validateTerminalCloseParams as N_, validateUsersSetAvatarResult as Na, DesktopLaunchParamsSchema as Nc, MigrationProtocolSchemas as Nd, SessionDiffScopeSchema as Nf, BoardWidgetMcpAppPutContentSchema as Ng, SkillsSearchResultSchema as Nh, validateSystemAgentChatParams as Ni, SystemAgentSetupVerifyParamsSchema as Nl, UpdateRunParamsSchema as Nm, validateProjectsAddParams as Nn, TasksCancelResultSchema as No, SessionsPreviewParamsSchema as Np, validateSessionsDispatchParams as Nr, QuestionResolvedEventSchema as Ns, validateExecApprovalGetParams as Nt, TtsSpeakParamsSchema as Nu, PluginsListResultSchema as Nv, validateBoardUpdateParams as O, PluginApprovalSeveritySchema as O_, validateUsersPrefsGetParams as Oa, GatewaySuspendStatusRunningResultSchema as Oc, SendParamsSchema as Od, SessionBranchSchema as Of, BoardWidgetGeneratedIdentitySchema as Og, SkillsProposalReviseParamsSchema as Oh, validateSkillsUpdateParams as Oi, SystemAgentSetupActivateResultSchema as Ol, ConfigSchemaResponseSchema as Om, validatePluginsUninstallParams as On, PortalOpenParamsSchema as Oo, SessionsGroupsRenameParamsSchema as Op, validateSessionsCompanionStateParams as Or, QuestionRequestQuestionSchema as Os, validateDeviceTokenRotateParams as Ot, TalkSessionOkResultSchema as Ou, PluginSearchPackageSchema as Ov, validateChannelsPairingApproveParams as P, validateTerminalInputParams as P_, validateUsersSetDisplayNameParams as Pa, DesktopObserveParamsSchema as Pc, MigrationsMemoryApplyParamsSchema as Pd, SessionFileBrowserEntrySchema as Pf, BoardWidgetMoveOpSchema as Pg, SkillsSecurityVerdictsParamsSchema as Ph, validateSystemAgentSetupActivateParams as Pi, SystemAgentSetupVerifyResultSchema as Pl, UpdateScheduleStateSchema as Pm, validateProjectsListParams as Pn, TasksGetParamsSchema as Po, SessionsResetParamsSchema as Pp, validateSessionsFilesGetParams as Pr, QuestionSchema as Ps, validateExecApprovalRequestParams as Pt, TtsSpeakResultSchema as Pu, PluginsRefreshParamsSchema as Pv, validateConfigSchemaLookupResult as Q, TerminalUploadParamsSchema as Q_, validateWorkerDesktopObserveResult as Qa, WorkerDesktopAppIdSchema as Qc, ProjectsRemoveResultSchema as Qd, SessionsCompactParamsSchema as Qf, UiPanelCommandSchema as Qg, BoardChangedEventSchema as Qh, validateTalkModeParams as Qi, ChannelsPairingApproveParamsSchema as Ql, AgentsUpdateParamsSchema as Qm, validateSecretsStoreMutationResult as Qn, SystemInfoResultSchema as Qo, ChatEventSchema as Qp, validateSessionsPreviewParams as Qr, NodePairRemoveParamsSchema as Qs, validateModelsListParams as Qt, AuditActivityListParamsSchema as Qu, readSystemAgentSessionInvalidatedErrorDetails as Qv, validateChannelsStatusParams as R, validateTerminalUploadResult as R_, validateWebLoginWaitParams as Ra, ExecApprovalRequestParamsSchema as Rc, PROJECTS_LIST_MAX_IDENTITY_PROBES as Rd, SessionFileKindSchema as Rf, BoardWidgetPluginPropsSchema as Rg, SkillsStatusParamsSchema as Rh, validateSystemChangesListParams as Ri, SystemChangesListParamsSchema as Rl, AgentSummarySchema as Rm, validatePushTestParams as Rn, TasksRecoveryParamsSchema as Ro, SessionsSearchParamsSchema as Rp, validateSessionsForkParams as Rr, PushTestParamsSchema as Rs, validateExecApprovalsNodeSetParams as Rt, AuditListParamsSchema as Ru, PluginsSessionActionParamsSchema as Rv, validateAuditRunInspectParams as S, CancelledApprovalSnapshotSchema as S_, validateUpdateRunParams as Sa, GatewaySuspendReleaseReceiptSchema as Sc, ConversationTurnCancelResultSchema as Sd, SessionSharingActionSchema as Sf, BoardUpdateParamsSchema as Sg, SkillsProposalEventsListResultSchema as Sh, validateSkillsProposalUpdateParams as Si, SystemAgentChatHistoryResultSchema as Sl, ConfigApplyParamsSchema as Sm, validatePluginsSearchParams as Sn, WorktreesRestoreParamsSchema as So, SessionsForkResultSchema as Sp, validateSessionsCompactParams as Sr, QuestionGetResultSchema as Ss, validateDevicePairRejectParams as St, TalkEventSchema as Su, SessionSharingRoleSchema as Sv, validateBoardEventParams as T, ExpiredApprovalSnapshotSchema as T_, validateUsersLinkEmailParams as Ta, GatewaySuspendStatusParamsSchema as Tc, ConversationTurnResultSchema as Td, SessionVisibilitySetParamsSchema as Tf, BoardWidgetAppViewResultSchema as Tg, SkillsProposalRecordResultSchema as Th, validateSkillsSecurityVerdictsParams as Ti, SystemAgentChatQuestionSchema as Tl, ConfigSchemaLookupParamsSchema as Tm, validatePluginsSetEnabledParams as Tn, PortalCloseResultSchema as To, SessionsGroupsListResultSchema as Tp, validateSessionsCompactionRestoreParams as Tr, QuestionOptionSchema as Ts, validateDevicePairSetupCodeParams as Tt, TalkSessionCloseParamsSchema as Tu, SessionPeerKindSchema as Tv, validateChatMessageGetParams as U, TerminalDataEventSchema as U_, validateWizardCancelParams as Ua, EnvironmentSummarySchema as Uc, ProjectRecordSchema as Ud, SessionObserverHealthSchema as Uf, BoardWidgetRemoveOpSchema as Ug, ToolsCatalogParamsSchema as Uh, validateTalkClientCreateParams as Ui, WizardStartParamsSchema as Ul, AgentsFileEntrySchema as Um, validateQuestionWaitAnswerParams as Un, TaskSuggestionsAcceptParamsSchema as Uo, SessionsPatchManyParamsSchema as Up, validateSessionsGroupsPutParams as Ur, WebPushVapidPublicKeyParamsSchema as Us, validateFsListDirResult as Ut, AuditRunIdentityUnsupportedV1Schema as Uu, PluginsUiDescriptorsResultSchema as Uv, validateChatHistoryParams as V, TerminalAttachResultSchema as V_, validateWebPushUnsubscribeParams as Va, ExecApprovalsSetParamsSchema as Vc, ProjectRecentProjectSchema as Vd, SessionGroupSchema as Vf, BoardWidgetPutParamsSchema as Vg, SkillsUploadChunkParamsSchema as Vh, validateTalkCatalogParams as Vi, WizardNextParamsSchema as Vl, AgentsDeleteParamsSchema as Vm, validateQuestionRequestParams as Vn, TaskSuggestionResolutionSchema as Vo, SessionsUsageParamsSchema as Vp, validateSessionsGroupsListResult as Vr, WebPushTestParamsSchema as Vs, validateExecutionIdentityContextV1 as Vt, AuditRunIdentityPresentV1Schema as Vu, PluginsSetEnabledResultSchema as Vv, validateChatMetadataParams as W, TerminalEventSchema as W_, validateWizardNextParams as Wa, EnvironmentsCreateParamsSchema as Wc, ProjectSummarySchema as Wd, SessionObserverPlanProgressSchema as Wf, BoardWidgetResizeOpSchema as Wg, ToolsEffectiveParamsSchema as Wh, validateTalkClientCreateResult as Wi, WizardStartResultSchema as Wl, AgentsFilesGetParamsSchema as Wm, validateRequestFrame as Wn, TaskSuggestionsAcceptResultSchema as Wo, SessionsPatchManyResultSchema as Wp, validateSessionsGroupsRenameParams as Wr, NodeEventResultSchema as Ws, validateGatewaySuspendPrepareParams as Wt, AuditRunIdentityV1Schema as Wu, PluginsUninstallParamsSchema as Wv, validateConfigPatchParams as X, TerminalResizeParamsSchema as X_, validateWorkerDesktopLaunchResult as Xa, EnvironmentsStatusParamsSchema as Xc, ProjectsRegisterResultSchema as Xd, SessionsBranchesSwitchResultSchema as Xf, UiFocusCommandSchema as Xg, BoardActionParamsSchema as Xh, validateTalkConfigParams as Xi, CommandsListResultSchema as Xl, AgentsListParamsSchema as Xm, validateSecretsStoreListParams as Xn, TaskSuggestionsListResultSchema as Xo, SessionsRecoverResultSchema as Xp, validateSessionsPatchParams as Xr, NodePairListParamsSchema as Xs, validateModelsAuthLogoutParams as Xt, AuditActivityEventV1Schema as Xu, buildSystemAgentSessionInvalidatedErrorDetails as Xv, validateConfigGetParams as Y, TerminalOpenResultSchema as Y_, validateWorkerDesktopLaunchParams as Ya, EnvironmentsListResultSchema as Yc, ProjectsRegisterParamsSchema as Yd, SessionsBranchesSwitchParamsSchema as Yf, UiCommandSchema as Yg, BOARD_WIDGET_TOOL_MAX_LENGTH as Yh, validateTalkClientTranscriptParams as Yi, CommandsListParamsSchema as Yl, AgentsFilesSetResultSchema as Ym, validateSecretsStoreDeleteParams as Yn, TaskSuggestionsListParamsSchema as Yo, SessionsRecoverParamsSchema as Yp, validateSessionsPatchManyParams as Yr, NodePairApproveParamsSchema as Ys, validateMessageActionParams as Yt, AuditActivityAgentRunV1Schema as Yu, buildSystemAgentInferenceUnavailableErrorDetails as Yv, validateConfigSchemaLookupParams as Z, TerminalSessionInfoSchema as Z_, validateWorkerDesktopObserveParams as Za, EnvironmentsStatusResultSchema as Zc, ProjectsRemoveParamsSchema as Zd, SessionsCleanupParamsSchema as Zf, UiNavigateCommandSchema as Zg, BoardCanvasDocumentSourceSchema as Zh, validateTalkConfigResult as Zi, TalkSessionAcknowledgeMarkParamsSchema as Zl, AgentsListResultSchema as Zm, validateSecretsStoreListResult as Zn, SystemInfoParamsSchema as Zo, SessionsCreateParamsSchema as Zp, validateSessionsPluginPatchParams as Zr, NodePairRejectParamsSchema as Zs, validateModelsAuthStatusParams as Zt, AuditActivityInboundMessageV1Schema as Zu, readSystemAgentInferenceUnavailableErrorDetails as Zv, validateArtifactsDownloadParams as _, ApprovalPresentationSchema as __, validateToolsInvokeParams as _a, GatewaySuspendPrepareParamsSchema as _c, ConversationListParamsSchema as _d, SessionMemberMutationResultSchema as _f, BoardTabIdSchema as _g, SkillsProposalApplyResultSchema as _h, validateSkillsProposalEvaluateParams as _i, CronScratchSetParamsSchema as _l, ShutdownEventSchema as _m, validatePluginApprovalRequestParams as _n, WorktreesGcResultSchema as _o, SessionsFilesRevealParamsSchema as _p, validateSessionsCatalogContinueParams as _r, SecretsStoreListResultSchema as _s, validateDesktopLaunchParams as _t, TalkClientToolCallParamsSchema as _u, SessionsCatalogStartTerminalParamsSchema as _v, validateAgentsDeleteParams as a, validateSkillsProposalHistoryStatusParams as a_, validateTalkSessionSubmitToolResultParams as aa, NodePluginToolsUpdateParamsSchema as ac, ArtifactsListParamsSchema as ad, SessionSuggestionSchema as af, BoardFocusTabCommandSchema as ag, ModelsProbeResultSchema as ah, validateSessionsSendParams as ai, WorkerTunnelStatusSchema as al, ChatToolTitlesParamsSchema as am, validateNodeListParams as an, validateWorktreesCreateParams as ao, SessionsCompanionResetResultSchema as ap, validateSessionMemberAddParams as ar, SessionPlacementSchema as as, validateConversationTurnParams as at, ChannelsStartParamsSchema as au, SessionCatalogSchema as av, validateAuditActivityListParams as b, ApprovalSnapshotSchema as b_, validateUpdateHoldParams as ba, GatewaySuspendReleaseCommittedReceiptSchema as bc, ConversationSendResultSchema as bd, SessionMembersListParamsSchema as bf, BoardTabsReorderOpSchema as bg, SkillsProposalEvaluateResultSchema as bh, validateSkillsProposalRequestRevisionParams as bi, CronUpdateParamsSchema as bl, SnapshotSchema as bm, validatePluginsListParams as bn, WorktreesRemoveParamsSchema as bo, SessionsFilesSetResultSchema as bp, validateSessionsCatalogStartTerminalParams as br, QuestionAnswersSchema as bs, validateDevicePairApproveParams as bt, TalkConfigParamsSchema as bu, SessionRowSchema as bv, validateAgentsFilesSetParams as c, validateApprovalResolveResult as c_, validateTaskSuggestionsCreateParams as ca, NodePresenceAliveReasonSchema as cc, AgentsWorkspaceGetParamsSchema as cd, SessionSuggestionsAddResultSchema as cf, BoardMcpAppDescriptorSchema as cg, SkillProposalLifecycleEventSchema as ch, validateSessionsUsageParams as ci, CronDeclarativeAddResultSchema as cl, LogsTailResultSchema as cm, validateNodePairRejectParams as cn, validateWorktreesRemoveParams as co, SessionsCreateResultSchema as cp, validateSessionSuggestionsAddParams as cr, SessionsDispatchResultSchema as cs, validateCronListParams as ct, ChannelsStopParamsSchema as cu, SessionsCatalogArchiveParamsSchema as cv, validateAgentsWorkspaceGetParams as d, ApprovalDecisionSchema as d_, validateTasksCancelParams as da, HooksStatusParamsSchema as dc, AgentsWorkspaceListResultSchema as dd, SessionSuggestionsResolveParamsSchema as df, BoardPromptAuthorizeParamsSchema as dg, SkillsCuratorStatusParamsSchema as dh, validateSkillsCuratorActionParams as di, CronListParamsSchema as dl, EventFrameSchema as dm, validateNodePendingDrainParams as dn, WorktreeRecordSchema as do, SessionsDiffParamsSchema as dp, validateSessionTypingParams as dr, isCloudWorkerPlacementState as ds, validateCronRunsParams as dt, TalkCatalogResultSchema as du, SessionsCatalogContinueResultSchema as dv, UiSplitCommandSchema as e_, validateTalkSessionAppendAudioParams as ea, NodePendingDrainParamsSchema as ec, AuditActivityOutboundMessageV1Schema as ed, ProjectsSearchRemoteResultSchema as ef, BoardCommandEventSchema as eg, AuthProbeStatusSchema as eh, validateSessionsRecoverParams as ei, WorkerDesktopLaunchResultSchema as el, ChatInjectParamsSchema as em, validateNodeDescribeParams as en, validateWorkerLiveEventParams as eo, SessionsCompactionListParamsSchema as ep, validateSendParams as er, SessionDiscussionInfoResultSchema as es, validateConfigSetParams as et, ChannelsPairingDismissParamsSchema as eu, SessionCatalogCapabilitiesSchema as ev, buildClawHubTrustErrorDetails as ey, validateAgentsWorkspaceListParams as f, ApprovalGetParamsSchema as f_, validateTasksGetParams as fa, GATEWAY_SUSPEND_MODE_DURABLE as fc, AgentEventSchema as fd, SessionSuggestionsResolveResultSchema as ff, BoardSetChatDockCommandSchema as fg, SkillsCuratorStatusResultSchema as fh, validateSkillsCuratorStatusParams as fi, CronRemoveParamsSchema as fl, GATEWAY_SERVER_CAPS as fm, validateNodePendingEnqueueParams as fn, WorktreeRepositoryStatusSchema as fo, SessionsDiffResultSchema as fp, validateSessionVisibilitySetParams as fr, SecretStoreEntrySchema as fs, validateCronScratchGetParams as ft, TalkClientCloseParamsSchema as fu, SessionsCatalogHostEventSchema as fv, validateApprovalResolveParams as g, ApprovalKindSchema as g_, validateToolsEffectiveParams as ga, GatewaySuspendPrepareBusyResultSchema as gc, ConversationListItemSchema as gd, SessionMemberAddParamsSchema as gf, BoardTabDeleteOpSchema as gg, SkillsProposalActionParamsSchema as gh, validateSkillsProposalCreateParams as gi, CronScratchGetResultSchema as gl, ResponseFrameSchema as gm, validateNodeSkillsUpdateParams as gn, WorktreesGcParamsSchema as go, SessionsFilesListResultSchema as gp, validateSessionsCatalogArchiveParams as gr, SecretsStoreListParamsSchema as gs, validateDecisionReceiptV1 as gt, TalkClientSteerParamsSchema as gu, SessionsCatalogReadResultSchema as gv, validateApprovalPresentation as h, ApprovalHistoryResultSchema as h_, validateToolsCatalogParams as ha, GatewaySuspendModeSchema as hc, AgentParamsSchema as hd, SessionTypingResultSchema as hf, BoardTabCreateOpSchema as hg, SkillsInstallParamsSchema as hh, validateSkillsProposalActionParams as hi, CronScratchGetParamsSchema as hl, RequestFrameSchema as hm, validateNodeRenameParams as hn, WorktreesCreateParamsSchema as ho, SessionsFilesListParamsSchema as hp, validateSessionsBranchesSwitchParams as hr, SecretsStoreDeleteParamsSchema as hs, validateCronUpdateParams as ht, TalkClientMutationResultSchema as hu, SessionsCatalogReadParamsSchema as hv, validateAgentsCreateParams as i, validateSkillsProposalHistoryScanParams as i_, validateTalkSessionSteerParams as ia, NodePluginToolDescriptorSchema as ic, ArtifactsGetParamsSchema as id, SessionSuggestionResolutionSchema as if, BoardEventParamsSchema as ig, ModelsProbeParamsSchema as ih, validateSessionsSearchParams as ii, WorkerEnvironmentStateSchema as il, ChatStatusEventSchema as im, validateNodeInvokeResultParams as in, validateWorktreesBranchesParams as io, SessionsCompanionResetParamsSchema as ip, validateSessionDiscussionOpenResult as ir, SessionDiscussionStateSchema as is, validateConversationTurnCancelParams as it, ChannelsLogoutParamsSchema as iu, SessionCatalogPullRequestSummarySchema as iv, validateBoardWidgetGrantParams as j, TerminalApprovalSnapshotSchema as j_, validateUsersSelfResult as ja, FsListDirParamsSchema as jc, validateMigrationsMemoryPlanParams as jd, SessionDiffFileSchema as jf, BoardWidgetHtmlContentSchema as jg, SkillsProposalsListResultSchema as jh, validateSkillsUploadCommitParams as ji, SystemAgentSetupDetectParamsSchema as jl, UpdateHoldParamsSchema as jm, validatePortalListParams as jn, TaskSummarySchema as jo, SessionsObserverVisibilityResultSchema as jp, validateSessionsDescribeParams as jr, QuestionResolveParamsSchema as js, validateEnvironmentsListParams as jt, TalkSpeakParamsSchema as ju, PluginsInstallResultSchema as jv, validateBoardWidgetAppViewParams as k, SessionApprovalEventSchema as k_, validateUsersPrefsSetParams as ka, GatewaySuspendTaskBlockerSchema as kc, WakeParamsSchema as kd, SessionCompanionExchangeSchema as kf, BoardWidgetGrantParamsSchema as kg, SkillsProposalUpdateParamsSchema as kh, validateSkillsUploadBeginParams as ki, SystemAgentSetupAuthStartParamsSchema as kl, ConfigSetParamsSchema as km, validatePollParams as kn, PortalOpenResultSchema as ko, SessionsListParamsSchema as kp, validateSessionsCreateParams as kr, QuestionRequestResultSchema as ks, validateEnvironmentsCreateParams as kt, TalkSessionSteerParamsSchema as ku, PluginSearchResultEntrySchema as kv, validateAgentsListParams as l, AllowedApprovalSnapshotSchema as l_, validateTaskSuggestionsDismissParams as la, NodeSkillDescriptorSchema as lc, AgentsWorkspaceGetResultSchema as ld, SessionSuggestionsListParamsSchema as lf, BoardOpSchema as lg, SkillsCuratorActionParamsSchema as lh, validateSessionsViewerPresenceSetParams as li, CronGetParamsSchema as ll, ConnectParamsSchema as lm, validateNodePairRemoveParams as ln, validateWorktreesRestoreParams as lo, SessionsDeleteParamsSchema as lp, validateSessionSuggestionsListParams as lr, SessionsReclaimParamsSchema as ls, validateCronRemoveParams as lt, TalkAgentControlResultSchema as lu, SessionsCatalogArchiveResultSchema as lv, validateApprovalHistoryParams as m, ApprovalHistoryParamsSchema as m_, validateTasksRecoveryParams as ma, GatewaySuspendBlockerSchema as mc, AgentIdentityResultSchema as md, SessionTypingParamsSchema as mf, BoardSnapshotSchema as mg, SkillsDetailResultSchema as mh, validateSkillsInstallParams as mi, CronRunsParamsSchema as ml, HelloOkSchema as mm, validateNodePresenceActivityPayload as mn, WorktreesBranchesResultSchema as mo, SessionsFilesGetResultSchema as mp, validateSessionsBranchesListParams as mr, SecretStoreSecretEntrySchema as ms, validateCronStatusParams as mt, TalkClientCreateResultSchema as mu, SessionsCatalogListResultSchema as mv, validateAgentParams as n, SkillsProposalHistoryScanResultSchema as n_, validateTalkSessionCloseParams as na, NodePendingEnqueueParamsSchema as nc, ArtifactSummarySchema as nd, SessionSuggestionActionSchema as nf, BoardCronActionParamsSchema as ng, ModelsAuthStatusParamsSchema as nh, validateSessionsResolveParams as ni, WorkerDesktopObserveResultSchema as nl, ChatRunStartupPhaseSchema as nm, validateNodeInvokeParams as nn, validateWorkerSessionsSpawnParams as no, SessionsCompanionAskParamsSchema as np, validateSessionDiscussionInfoResult as nr, SessionDiscussionOpenParamsSchema as ns, validateConversationListParams as nt, ChannelsPairingListParamsSchema as nu, SessionCatalogHostSchema as nv, readClawHubTrustErrorDetails as ny, validateAgentsFilesGetParams as o, validateApprovalGetResult as o_, validateTalkSpeakParams as oa, NodePresenceActivityPayloadSchema as oc, AgentsWorkspaceEntrySchema as od, SessionSuggestionStateSchema as of, BoardGetParamsSchema as og, ModelsProbeTargetResultSchema as oh, validateSessionsSendReconcileParams as oi, CronAddParamsSchema as ol, ChatToolTitlesResultSchema as om, validateNodePairApproveParams as on, validateWorktreesGcParams as oo, SessionsCompanionStateParamsSchema as op, validateSessionMemberRemoveParams as or, SessionPlacementStateSchema as os, validateCronAddParams as ot, ChannelsStatusParamsSchema as ou, SessionCatalogSessionSchema as ov, validateApprovalGetParams as p, ApprovalGetResultSchema as p_, validateTasksListParams as pa, GATEWAY_SUSPEND_MODE_LEGACY as pc, AgentIdentityParamsSchema as pd, SessionTypingEventSchema as pf, BoardSizeSchema as pg, SkillsDetailParamsSchema as ph, validateSkillsDetailParams as pi, CronRunParamsSchema as pl, GatewayFrameSchema as pm, validateNodePluginToolsUpdateParams as pn, WorktreesBranchesParamsSchema as po, SessionsFilesGetParamsSchema as pp, validateSessionsAbortParams as pr, SecretStoreEnvEntrySchema as ps, validateCronScratchSetParams as pt, TalkClientCreateParamsSchema as pu, SessionsCatalogListParamsSchema as pv, validateCommandsListParams as q, TerminalListResultSchema as q_, validateWorkerAdmissionHandshake as qa, EnvironmentsDestroyResultSchema as qc, ProjectsListParamsSchema as qd, SessionsBranchesListParamsSchema as qf, UiCommandParamsSchema as qg, BOARD_CRON_TRIGGER_PREFIX as qh, validateTalkClientToolCallParams as qi, WizardStepSchema as ql, AgentsFilesListResultSchema as qm, validateSecretsResolveParams as qn, TaskSuggestionsDismissParamsSchema as qo, SessionsPatchParamsSchema as qp, validateSessionsMessagesUnsubscribeParams as qr, NodeInvokeProgressParamsSchema as qs, validateHooksStatusParams as qt, DecisionReceiptV1Schema as qu, SESSION_AGENT_ATTENTION_ICON_IDS as qv, validateAgentWaitParams as r, SkillsProposalHistoryStatusParamsSchema as r_, validateTalkSessionCreateParams as ra, NodePendingEnqueueResultSchema as rc, ArtifactsDownloadParamsSchema as rd, SessionSuggestionEventSchema as rf, BoardDataReadParamsSchema as rg, ModelsListParamsSchema as rh, validateSessionsRewindParams as ri, WorkerEnvironmentMetadataSchema as rl, ChatSendParamsSchema as rm, validateNodeInvokeProgressParams as rn, validateWorkerTranscriptCommitParams as ro, SessionsCompanionAskResultSchema as rp, validateSessionDiscussionOpenParams as rr, SessionDiscussionOpenResultSchema as rs, validateConversationSendParams as rt, ChannelsPairingListResultSchema as ru, SessionCatalogLocatorSchema as rv, validateAgentsFilesListParams as s, validateApprovalHistoryResult as s_, validateTaskSuggestionsAcceptParams as sa, NodePresenceAlivePayloadSchema as sc, AgentsWorkspaceFileSchema as sd, SessionSuggestionsAddParamsSchema as sf, BoardLegacyEventParamsSchema as sg, SkillProposalEvaluationSchema as sh, validateSessionsSendReconcileResult as si, CronAddResultSchema as sl, LogsTailParamsSchema as sm, validateNodePairListParams as sn, validateWorktreesListParams as so, SessionsCompanionStateResultSchema as sp, validateSessionMembersListParams as sr, SessionsDispatchParamsSchema as ss, validateCronGetParams as st, ChannelsStatusResultSchema as su, SessionCatalogTranscriptItemSchema as sv, validateAgentIdentityParams as t, SkillsProposalHistoryScanParamsSchema as t_, validateTalkSessionCancelOutputParams as ta, NodePendingDrainResultSchema as tc, AuditActivityToolActionV1Schema as td, RemoteProjectSchema as tf, BoardCommandSchema as tg, ModelsAuthLogoutParamsSchema as th, validateSessionsResetParams as ti, WorkerDesktopObserveParamsSchema as tl, ChatMetadataParamsSchema as tm, validateNodeEventParams as tn, validateWorkerSessionsSendParams as to, SessionsCompactionRestoreParamsSchema as tp, validateSessionDiscussionInfoParams as tr, SessionDiscussionInfoSchema as ts, validateConnectParams as tt, ChannelsPairingDismissResultSchema as tu, SessionCatalogDescriptorSchema as tv, isClawHubTrustErrorCode as ty, validateAgentsUpdateParams as u, ApprovalAllowDecisionSchema as u_, validateTaskSuggestionsListParams as ua, NodeSkillsUpdateParamsSchema as uc, AgentsWorkspaceListParamsSchema as ud, SessionSuggestionsListResultSchema as uf, BoardPluginActionParamsSchema as ug, SkillsCuratorActionResultSchema as uh, validateSkillsBinsParams as ui, CronJobSchema as ul, ErrorShapeSchema as um, validateNodePendingAckParams as un, WorktreeBranchSchema as uo, SessionsDescribeParamsSchema as up, validateSessionSuggestionsResolveParams as ur, SessionsReclaimResultSchema as us, validateCronRunParams as ut, TalkCatalogParamsSchema as uu, SessionsCatalogContinueParamsSchema as uv, validateArtifactsGetParams as v, ApprovalResolveParamsSchema as v_, validateTtsSpeakParams as va, GatewaySuspendPrepareReadyResultSchema as vc, ConversationListResultSchema as vd, SessionMemberRemoveParamsSchema as vf, BoardTabSchema as vg, SkillsProposalCreateParamsSchema as vh, validateSkillsProposalEventsListParams as vi, CronScratchSetResultSchema as vl, TickEventSchema as vm, validatePluginApprovalResolveParams as vn, WorktreesListParamsSchema as vo, SessionsFilesRevealResultSchema as vp, validateSessionsCatalogListParams as vr, SecretsStoreMutationResultSchema as vs, validateDesktopObserveParams as vt, TalkClientToolCallResultSchema as vu, SessionsCatalogStartTerminalResultSchema as vv, validateBoardDataReadParams as w, ExecApprovalPresentationSchema as w_, validateUpdateStatusResult as wa, GatewaySuspendResumeResultSchema as wc, ConversationTurnReplySchema as wd, SessionSharingIdentitySchema as wf, BoardWidgetAppViewParamsSchema as wg, SkillsProposalInspectResultSchema as wh, validateSkillsSearchParams as wi, SystemAgentChatParamsSchema as wl, ConfigPatchParamsSchema as wm, validatePluginsSessionActionResult as wn, PortalCloseParamsSchema as wo, SessionsGroupsListParamsSchema as wp, validateSessionsCompactionListParams as wr, QuestionListResultSchema as ws, validateDevicePairRenameParams as wt, TalkSessionCancelOutputParamsSchema as wu, SessionClassificationSchema as wv, validateAuditListParams as x, ApprovalTerminalReasonSchema as x_, validateUpdateHoldResult as xa, GatewaySuspendReleaseCompletedReceiptSchema as xc, ConversationTurnCancelParamsSchema as xd, SessionMembersListResultSchema as xf, BoardTicketEventParamsSchema as xg, SkillsProposalEventsListParamsSchema as xh, validateSkillsProposalReviseParams as xi, SystemAgentChatHistoryParamsSchema as xl, StateVersionSchema as xm, validatePluginsRefreshParams as xn, WorktreesRemoveResultSchema as xo, SessionsForkParamsSchema as xp, validateSessionsCleanupParams as xr, QuestionGetParamsSchema as xs, validateDevicePairListParams as xt, TalkConfigResultSchema as xu, SessionToolOverridesSchema as xv, validateArtifactsListParams as y, ApprovalResolveResultSchema as y_, validateUiCommandParams as ya, GatewaySuspendPrepareResultSchema as yc, ConversationSendParamsSchema as yd, SessionMemberSchema as yf, BoardTabUpdateOpSchema as yg, SkillsProposalEvaluateParamsSchema as yh, validateSkillsProposalInspectParams as yi, CronStatusParamsSchema as yl, PresenceEntrySchema as ym, validatePluginsInstallParams as yn, WorktreesListResultSchema as yo, SessionsFilesSetParamsSchema as yp, validateSessionsCatalogReadParams as yr, SecretsStoreSetParamsSchema as ys, validateDesktopObserveResult as yt, TalkClientTranscriptParamsSchema as yu, SessionCreatedActorSchema as yv, validateChannelsStopParams as z, TerminalAckResultSchema as z_, validateWebPushSubscribeParams as za, ExecApprovalResolveParamsSchema as zc, ProjectCheckoutSchema as zd, SessionFilePreviewKindSchema as zf, BoardWidgetPresentationSchema as zg, SkillsUpdateParamsSchema as zh, validateSystemInfoParams as zi, SystemChangesListResultSchema as zl, AgentsCreateParamsSchema as zm, validateQuestionGetParams as zn, TasksRecoveryResultSchema as zo, SessionsSearchResultSchema as zp, validateSessionsGroupsDeleteParams as zr, PushTestResultSchema as zs, validateExecApprovalsNodeSnapshot as zt, AuditListResultSchema as zu, PluginsSessionActionResultSchema as zv };
