...
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-BdJBj7hj.js");
const loadManifestCommandAliasesRuntimeModule = async () => await import("../manifest-command-aliases.runtime-CSyXLyvd.js");
...
async function closeCliResources() {
	const finalizers = [
		async () => {
			const { listRegisteredAgentHarnesses, disposeRegisteredAgentHarnesses } = await import("../registry-Cn-t3-7P.js");
		...
		async () => {
			...
			if (hasManagedProviderLocalServices()) {
				const { stopManagedProviderLocalServices } = await import("../provider-local-service-CacgVExC.js");
		...
		async () => {
			...
			if (getActiveMcpLoopbackRuntime()) {
				const { closeMcpLoopbackServer } = await import("../mcp-http-C91LTc1w.js");
		...
		async () => {
			...
			if (hasMemoryRuntime()) {
				const { closeActiveMemorySearchManagersCore } = await import("../memory-runtime-BrdcX0O3.js");
...
async function shouldStartLocalOnboarding(snapshot) {
	...
	const { readLocalOnboardingStateForConfig } = await import("../local-onboarding-state-DL3bC6f_.js");
...
async function resolveReachableGateway(config, options) {
	...
	const { probeGatewayConfiguredModel } = await import("../onboard-helpers-B2IyzQ1N.js");
...
async function resolveGatewayProbePlan(config) {
	...
	if (normalizeOptionalString(config.gateway?.mode) === "remote" && remoteUrl) try {
		const { resolveGatewayClientBootstrap } = await import("../client-bootstrap-Cu-D86vV.js");
...
async function resolveLocalGatewayProbeTargets(config) {
	const [{ resolveGatewayPort }, { resolveControlUiLinks }, { resolveGatewayClientBootstrap }, { readActiveGatewayLockPort }] = await Promise.all([
		...
		import("../client-bootstrap-Cu-D86vV.js"),
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-HuqTIERt.js"), import("../coverage-BFQk1rJT.js")]));
...
async function runCliWithPreparedOutputMode(originalArgv, options) {
	...
	if (!isHelpOrVersionInvocation && !isDatabaseInvocation && !isAgentExecInvocation(normalizedInvocation.commandPath) && shouldLoadCliDotEnv(loadGlobalEnv)) await startupTrace.measure("dotenv", async () => {
		if (isRemoteAgentDispatchInvocation(normalizedArgv, normalizedInvocation.primary)) {
			const { loadGatewayDispatchCliDotEnv } = await import("../gateway-dispatch-dotenv-BEidkM6k.js");
	...
	if (!isHelpOrVersionInvocation && isGatewayRunInvocation) await startupTrace.measure("gateway-run-select-environment", async () => {
		const [{ selectGatewayRunEnvironment }, { defaultRuntime }] = await Promise.all([import("../pre-bootstrap-B1LKN_Nd.js"), import("../runtime-Dn74rUMz.js")]);
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-BapGs-eZ.js").then((configIo) => normalizedInvocation.primary === "update" ? configIo.readSourceConfigBestEffort() : configIo.readBestEffortConfig({
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-CEO8zKnH.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-BIozler_.js");
		...
		if (bareSessionInvocation) {
			...
			const { runTuiCliAction } = await import("../tui-cli-DXBZ9huE.js");
		...
		if (bareRootLaunchTarget) {
			if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
				...
				const { runRemoteGatewayInferenceOnboarding } = await import("../onboard-remote-gateway-C45u51l1.js");
			...
			if (bareRootLaunchTarget.kind === "onboarding") {
				...
				const { setupWizardCommand } = await import("../onboard-CuteSJUA.js");
			...
			if (bareRootLaunchTarget.kind === "tui") {
				...
				const { runTui } = await import("../tui-BO4Q9oYD.js");
				await runTui({
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-o04FNUJs.js"));
		...
		try {
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-D_NLluYk.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-C0rc5Hgp.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const [{ registerPluginCliCommandsFromValidatedConfig }, { resolveCliStartupPolicy }] = await Promise.all([import("../cli-B5pHiGhp.js"), import("../command-startup-policy-DuVCDd2T.js")]);
					const startupPolicy = resolveCliStartupPolicy({
						argv: parseArgv,
