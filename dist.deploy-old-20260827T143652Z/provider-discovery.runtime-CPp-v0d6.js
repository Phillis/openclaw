...
import { a as resolveDiscoveredProviderPluginIds } from "./providers-o7UIOzTf.js";
...
function resolvePluginDiscoveryProvidersRuntime(params) {
	...
	if (params.onlyPluginIds === void 0 && runtimeEntryProviders.length > 0) {
		...
		const fullProviders = fullPluginIds.length > 0 ? resolvePluginProvidersCore({
			...params,
			env,
	...
	if (runtimeManifestCatalogPluginIds.length > 0) return resolvePluginProvidersCore({
		...params,
		env,
	...
	if (entryProviders.length > 0) {
		...
		if (fullPluginIds.length > 0) return resolvePluginProvidersCore({
			...params,
			env,
			onlyPluginIds: fullPluginIds
		});
