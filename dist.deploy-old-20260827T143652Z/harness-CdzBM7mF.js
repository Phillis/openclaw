...
import "./simple-completion-runtime-DVPyUonm.js";
...
function createCodexAppServerAgentHarness(options) {
	...
	return {
		...
		...sessionCatalogControl && sessionRuntime ? { sessionFork: {
			...
			fork: async (params) => {
				const { forkCodexUpstreamSession } = await import("./upstream-session-fork-C2LzObUa.js");
		...
		authBinding: { fingerprint: async (params) => {
			const { fingerprintCodexAppServerAuthBinding } = await import("./auth-binding-De5NoieZ.js");
		...
		fetchUsageSnapshot: async (ctx) => {
			const { fetchCodexAppServerUsageSnapshot } = await import("./usage-CGx11bxu.js");
		...
		loadMcpToolCatalog: async (params) => {
			const { loadCodexEffectiveMcpCatalog } = await import("./effective-mcp-catalog-CkM6Q0iF.js");
		...
		runIsolatedCompletionV2: async (params) => {
			...
			const { runCodexIsolatedCompletion } = await import("./isolated-completion-9XDG-zvC.js");
		...
		finalizeSettledTurn: async (params) => {
			const { runCodexSettledTurnFinalization } = await import("./settled-turn-finalizer-CdiWRMi7.js");
			return runCodexSettledTurnFinalization(params, { pluginConfig: options?.resolvePluginConfig?.() ?? options?.pluginConfig });
		},
		runSideQuestion: async (params) => {
			const { runCodexAppServerSideQuestion } = await import("./side-question-M6jJwS6w.js");
		...
		compact: async (params) => {
			const { maybeCompactCodexAppServerSession } = await import("./compact-DZ2Dxy09.js");
		...
		reset: async (params) => {
			if (params.sessionId) {
				const [{ reclaimCurrentCodexSessionGeneration, sessionBindingIdentity }, { retireCodexAppServerSessionGeneration }] = await Promise.all([import("./session-binding-DlhdfFiB.js"), import("./session-retirement-DeFMYzv9.js")]);
