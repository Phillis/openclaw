...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	if (!shouldRepair) {
		const { repairStaleAgentModelRefs } = await import("./stale-agent-model-ref-repair-6X0QUL54.js");
	...
	if (shouldRepair) {
		const { runDoctorRepairSequence } = await import("./repair-sequencing-fRNPbs4M.js");
		...
		const { collectDoctorPreviewNotes } = await import("./preview-warnings-4Owqyuix.js");
