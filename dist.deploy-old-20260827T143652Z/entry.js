...
async function flushEntryStartupTraceForEarlyReturn(argv) {
	...
	const { loadCliDotEnvForEarlyDiagnostic } = await import("./dotenv-Dh6ZxbQY.js");
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-CEO8zKnH.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
	...
	await runCliWithExitFinalization({
		run: async () => {
			if (isNativeHookRelayArgv(argv) && !argv.includes("--help") && !argv.includes("-h")) {
				const { runNativeHookRelayCliFromArgv } = await import("./native-hook-relay-cli-CHC50m7p.js");
