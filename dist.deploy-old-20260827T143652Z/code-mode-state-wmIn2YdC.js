...
import { g as isFutureDateTimestampMs, k as resolveExpiresAtMsFromDurationSeconds } from "./number-coercion-oCkfUEEq.js";
...
import { n as ToolInputError, r as asToolParamsRecord } from "./common-BGOZLJ2_.js";
import { n as boundCodeModeValue, t as boundCodeModeResult } from "./code-mode-json-CWwCZ1yI.js";
import "./tool-search-6StV_VPD.js";
import { t as parseNodeList } from "./node-list-parse-B-QeHrg4.js";
import { i as resolveEligibleNodeFromList } from "./nodes-utils-Cg_2WHHU.js";
import { l as resolveInternalSessionKey, u as resolveMainSessionAlias } from "./sessions-helpers-DoiD7CEo.js";
import { d as getSwarmRunByLaunchReplayKey, f as initSubagentRegistry } from "./subagent-registry-BYIM_F-O.js";
import { t as resolveSwarmConfig } from "./swarm-config-DqpqlTRZ.js";
import { n as waitForCollectorCompletion } from "./agents-wait-tool-DQ0MvHfP.js";
...
/**
* Terminal outcomes of already-completed code-mode runs, retained so a later
* `wait(runId)` can collect the result instead of failing with the misleading
* "unavailable or expired". Models naturally poll a run whose nested exec
* outlives the code; without retention the first completed wait consumes the
* snapshot and every subsequent poll loses the work product. Bounded by the
* run's own snapshot TTL so retention cannot pin memory indefinitely.
*/
const terminalRunResults = /* @__PURE__ */ new Map();
function recordTerminalCodeModeRunResult(runId, result, ttlSeconds) {
	const expiresAt = Date.now() + Math.max(1, ttlSeconds) * 1e3;
	terminalRunResults.set(runId, {
		result,
		expiresAt
	});
	if (terminalRunResults.size > 256) {
		const oldest = terminalRunResults.keys().next();
		if (!oldest.done) terminalRunResults.delete(oldest.value);
	}
}
function takeTerminalCodeModeRunResult(runId) {
	const entry = terminalRunResults.get(runId);
	if (!entry) return void 0;
	if (Date.now() >= entry.expiresAt) {
		terminalRunResults.delete(runId);
		return;
	}
	return entry.result;
}
...
function removeExpiredRuns(now = Date.now()) {
	for (const [runId, entry] of terminalRunResults) if (now >= entry.expiresAt) terminalRunResults.delete(runId);
...
//#endregion
export { prepareSource as A, MAX_HEADLESS_WALL_CLOCK_MS as C, createCodeModeApiFilesForRun as D, codeModeFailureMessage as E, resolveCodeModeHeadlessConfig as F, toToolSearchConfig as I, codeModeReplayIdForToolCall as L, readPositiveInteger as M, readRunId as N, enforceSnapshotPayloadLimits as O, resolveCodeModeConfig as P, resolveCodeModeSkills as R, DEFAULT_HEADLESS_WALL_CLOCK_MS as S, codeModeFailureCode as T, takeTerminalCodeModeRunResult as _, disposeAllCodeModeRuns as a, waitForPendingBridgeSettlement as b, pendingBridgeStatesForSettlement as c, removeExpiredRuns as d, reserveActiveRunSlot as f, storeSnapshotState as g, snapshotState as h, createPendingBridgeStates as i, readCode as j, isCodeModeEngagedForModel as k, pendingToolCalls as l, settledBridgeRequestsInCompletionOrder as m, cancelPendingBridgeStates as n, disposeCodeModeRun as o, resumingRunIds as p, codeModeWaitingReason as r, pendingBridgeRequestsReplaySafe as s, activeRuns as t, recordTerminalCodeModeRunResult as u, takeUndeliveredCodeModeRunOutput as v, boundOutputToLimit as w, CODE_MODE_WORKER_WATCHDOG_GRACE_MS as x, telemetry as y };
