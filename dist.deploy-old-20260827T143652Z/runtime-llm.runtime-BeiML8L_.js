...
async function runIsolatedAgentRuntimeCompletion(params) {
	...
	try {
		const operation = (async () => {
			const { runIsolatedCompletion } = await import("./isolated-completion-DE8ycQoH.js");
...
async function resolveAgentId(params) {
	...
	const { resolveDefaultAgentId } = await import("./agent-scope-B7ocz6pI.js");
...
function createRuntimeLlm(options = {}) {
	...
	return { complete: async (params) => {
		...
		const [{ prepareSimpleCompletionModelForAgent, completeWithPreparedSimpleCompletionModel, resolveSimpleCompletionSelectionForAgent }, cfg] = await Promise.all([import("./simple-completion-runtime-DDn5DIze.js"), Promise.resolve(resolveRuntimeConfig(options))]);
