...
import { f as setGatewaySigusr1RestartPolicy, p as setPreRestartDeferralCheck } from "./restart-DHXBMqqF.js";
...
import "./server-plugins-gslZ7Mx7.js";
...
import { c as canReceiveSessionEvent } from "./session-sharing-B-QpnXEG.js";
...
import { a as incrementPresenceVersion, i as getPresenceVersion, n as getHealthCache, o as refreshGatewayHealthSnapshot, r as getHealthVersion } from "./health-state-BMJaisHV.js";
import { t as resolveGatewayPluginConfig } from "./runtime-plugin-config-CjkrzfO_.js";
import { t as createControlUiSessionPullRequestSubscriptions } from "./control-ui-session-pr-subscriptions-CAcMwhCu.js";
...
async function startGatewayCoreRuntime(input) {
	...
	const taskAndSkillsResident = residentRegistry.register({
		...
		stop: async () => {
			...
			const { stopTaskRegistryMaintenance } = await import("./task-registry.maintenance-CM1CScI3.js");
	...
	const [{ startGatewayEventSubscriptions }, { startGatewayRuntimeServices }] = await startupTrace.measure("runtime.post-early-imports", () => Promise.all([import("./server-runtime-subscriptions-CBnQdMG5.js"), import("./server-runtime-startup-services-C7rNX3Su.js")]));
	...
	const { createOperatorApprovalSessionEventRuntime } = await import("./operator-approval-session-events-B1rrQXzd.js");
	...
	const { execApprovalManager, cancelRunBoundApprovals, forwardPluginApprovalRequest, pluginApprovalIosPushDelivery, pluginApprovalManager, systemAgentApprovalManager, bindApprovalPublicationContext, unregisterApprovalAuthorityObserver, extraHandlers, coreGatewayHandlers } = await startupTrace.measure("gateway.handlers", async () => {
		const [{ createGatewayAuxHandlers }, { coreGatewayHandlers: coreGatewayHandlersLocal }] = await Promise.all([import("./server-aux-handlers-BNNBR2ZD.js"), import("./server-methods-DE4ExM2D.js")]);
		return {
...
async function createGatewayChatMetadataLifecycle(params) {
	...
	const { ChatMetadataSnapshotUnavailableError, createGatewayChatMetadataRuntime } = await import("./chat-metadata-runtime-DZBImiJf.js");
	const runtime = createGatewayChatMetadataRuntime({
		...
		...params.minimalTestGateway ? {
			beforeRefresh: async () => {
				const { refreshPreparedModelRuntimeSnapshots } = await import("./prepared-model-runtime-nZZnJ7RU.js");
	...
	const registerRefreshListeners = async () => {
		...
		const [{ registerRuntimeAuthProfileStoreMutationListener }, { registerPreparedModelRuntimePublicationListener }, { registerSkillsChangeListener }] = await Promise.all([
			...
			import("./prepared-model-runtime-nZZnJ7RU.js"),
...
async function attachInitialGatewayLifetimeSidecars(params) {
	...
	params.sidecars.push({ stop: async () => {
		const { flushPendingSessionsChangedEvents } = await import("./session-change-event-CpIzxKvM.js");
...
async function prepareGatewayKernelRequestRuntime(params) {
	...
	const { createGatewayInstanceRuntime } = await import("./server-instance-runtime-gh5bkhim.js");
...
function createLazyGatewayCronState(params) {
	...
	const cronStateLoader = createLazyPromiseLoader(() => import("./server-cron-CxM6ngOG.js").then(({ buildGatewayCronService }) => {
		loaded = {
...
async function prepareGatewayLifecycle(params) {
	...
	const { createGatewayNodeSessionRuntime } = await import("./server-node-session-runtime-C39QmF6m.js");
	...
	const { createWatchNodeHttpRuntime } = await import("./watch-node-http-C2Lqlx-8.js");
	...
	const createCloseHandler = () => async (optsValue) => {
		...
		await createGatewayCloseHandler({
			...
			markMainSessionsAbortedForRestart: async ({ sessionKeys, sessionIds, activeRuns, reason, isActiveRun }) => {
				...
				const { markRestartAbortedMainSessions } = await import("./main-session-restart-recovery-rE6FxaZP.js");
...
async function prepareGatewayKernelState(params) {
	...
	const { runDefaultChannelSetupWizard, runDefaultSetupWizard } = await import("./wizard-DBhZPmDs.js");
...
async function prepareGatewayServerBootstrap(input) {
	...
	const startupConfigModulePromise = import("./server-startup-config-Hp-Ut1pP.js");
	const loadStartupPluginsModule = createLazyPromise(() => import("./server-startup-plugins-B1Dg6Mhz.js"), { cacheRejections: true });
...
const loadGatewayModelCatalogModule = createLazyRuntimeModule(() => import("./server-model-catalog-p9dvDk8P.js"));
const loadWorkerEnvironmentStartupModule = createLazyRuntimeModule(() => import("./server-worker-environment-startup-Ch7W9ETJ.js"));
const loadWorkerPlacementStartupModule = createLazyRuntimeModule(() => import("./server-worker-placement-startup-MvpZ5LW8.js"));
const loadGatewayStartupEarlyModule = createLazyRuntimeModule(() => import("./server-startup-early-D7NJ4F2z.js"));
const loadGatewayPluginBootstrapModule = createLazyRuntimeModule(() => import("./server-plugin-bootstrap-D6rahfxN.js"));
...
const getChannelRuntime = createLazyRuntimeModule(() => import("./runtime-channel-dBLthMrT.js").then(({ createRuntimeChannel }) => createRuntimeChannel()));
...
const getControlUiModule = createLazyRuntimeModule(() => import("./control-ui-hRtK1xDb.js"));
const getCanvasServeModule = createLazyRuntimeModule(() => import("./serve.runtime.js"));
const getBoardHttpModule = createLazyRuntimeModule(() => import("./board-http-DXb9ZZiI.js"));
const getEmbeddingsHttpModule = createLazyRuntimeModule(() => import("./embeddings-http-Bpgmu8R0.js"));
const getManagedMediaAttachmentsModule = createLazyRuntimeModule(() => import("./managed-image-attachments-yEI3vxza.js"));
const getMcpAppStandaloneModule = createLazyRuntimeModule(() => import("./mcp-app-standalone-Dj0ut_0W.js"));
const getPluginIconHttpModule = createLazyRuntimeModule(() => import("./plugin-icon-http-C77o06CH.js"));
const getWorkspaceIconHttpModule = createLazyRuntimeModule(() => import("./workspace-icon-http-BGGbw8-j.js"));
const getModelsHttpModule = createLazyRuntimeModule(() => import("./models-http-B0CmSF-0.js"));
const getOpenAiHttpModule = createLazyRuntimeModule(() => import("./openai-http-y7OwCX_K.js"));
const getOpenResponsesHttpModule = createLazyRuntimeModule(() => import("./openresponses-http-CoA1_I-t.js"));
const getSessionHistoryHttpModule = createLazyRuntimeModule(() => import("./sessions-history-http-Dlx8AwQQ.js"));
const getSessionKillHttpModule = createLazyRuntimeModule(() => import("./session-kill-http-D1qgRDsm.js"));
const getToolsInvokeHttpModule = createLazyRuntimeModule(() => import("./tools-invoke-http-8T1tvjid.js"));
const getUserProfilesHttpModule = createLazyRuntimeModule(() => import("./user-profiles-http-B5eb8M9A.js"));
	...
	const handleHooksRequest = async (req, res) => {
		...
		return await runWithGatewayHttpWorkAdmission(res, async () => {
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-C26HV-iH.js");
...
async function finishGatewayStartup(params) {
	...
	const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] = await startupTrace.measure("gateway.ws-imports", () => Promise.all([import("./server-ws-runtime-BsG4Ynsu.js"), import("./route-capability-CCISCYmy.js")]));
	...
	const loadScheduledServicesModule = createLazyPromise(() => import("./server-runtime-services-BWxHSeh1.js"), { cacheRejections: true });
	...
	activateScheduledServicesWhenReady();
	const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-C6LLYSXq.js");
...
const loadGatewayStartupPostAttachModule = createLazyRuntimeModule(() => import("./server-startup-post-attach-BTrrVXF9.js"));
