...
import { r as formatSystemAgentStartupMessage } from "./overview-DHobpgIW.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-C3cbtid4.js";
import { g as setCommandLaneConcurrency, r as enqueueCommandInLane } from "./command-queue-CqN2qr5o.js";
import { n as describeSystemAgentPersistentOperation } from "./operations-nR44JMF_.js";
...
import { f as listVisiblePendingApprovalRequests, i as buildRequestedApprovalEvent, s as handlePendingApprovalRequest } from "./approval-shared-DPu2FPJR.js";
import { t as getUpdateAvailable } from "./update-startup-CQ3ipCAi.js";
import { n as getHealthCache } from "./health-state-BMJaisHV.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-R8WRBL1s.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = await createAdmittedWizardSession(() => new WizardSession(async (prompter, signal, runnerSession) => {
			await runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-EBrmViUs.js"), import("./setup.shared-BaXgZkEJ.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-C94u655m.js");
