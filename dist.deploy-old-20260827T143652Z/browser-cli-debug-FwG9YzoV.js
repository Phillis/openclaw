...
import { c as printBrowserJsonResult, l as resolveBrowserProfileQuery, n as callBrowserRequest, t as BROWSER_TAB_REFERENCE_HELP, u as runBrowserCliCommand } from "./browser-cli-shared-Q6ddQwVz.js";
...
function registerBrowserDebugCommands(browser, parentOpts) {
	browser.command("highlight").description("Highlight an element by ref").argument("<ref>", "Ref id from snapshot").option("--target-id <id>", BROWSER_TAB_REFERENCE_HELP).action(async (ref, opts, cmd) => {
		await withDebugContext(cmd, parentOpts, async ({ parent, profile }) => {
			if (printBrowserJsonResult(parent, await callDebugRequest(parent, {
				method: "POST",
				path: "/highlight",
