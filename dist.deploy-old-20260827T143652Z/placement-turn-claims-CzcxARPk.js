...
import { b as serializeWorkerWorkspaceReconciliationPlan, v as parseWorkerWorkspaceReconciliationPlan } from "./workspace-actual-manifest-B7ccel6H.js";
import "./workspace-reconcile-pxprMj1H.js";
import { a as find, c as query$2, d as advanceCursor, g as normalizeIdentity, h as normalizeEpoch, i as ensureLocal, l as transitionValues, n as clearWorkerTurnToolState, p as isCurrentPlacementTurnClaim, r as createPlacementSessionToolOperationOps, s as getRequired, t as assertNoRunningWorkerSessionToolOperations, v as required, y as resolvePlacementTurnEnvironment } from "./placement-session-tool-operations-Ba4gGaev.js";
...
function markWorkerWorkspacePendingResultAccepted(db, claim, nowMs) {
	const environment = resolvePlacementTurnEnvironment(getRequired(db, claim.sessionId), claim);
	if (!environment) throw new Error(`Cannot accept stale worker workspace result for ${claim.sessionId}`);
	if (executeSqliteQuerySync(db, query(db).updateTable("worker_workspace_pending_results").set({ workspace_accepted_at_ms: nowMs }).where("session_id", "=", claim.sessionId).where("environment_id", "=", environment.environmentId).where("owner_epoch", "=", environment.ownerEpoch).where("placement_generation", "=", claim.placementGeneration).where("claim_id", "=", claim.claimId).where("run_id", "=", claim.runId)).numAffectedRows !== 1n) throw new Error(`Cannot accept stale worker workspace result for ${claim.sessionId}`);
}
function createPlacementWorkspaceResultOps(runtime) {
	...
	const assertPendingClaim = (db, claim) => {
		...
		if (!environment || !row || row.environment_id !== environment.environmentId || row.owner_epoch !== environment.ownerEpoch || row.placement_generation !== claim.placementGeneration || row.claim_id !== claim.claimId || row.run_id !== claim.runId) throw new Error(`Cannot update stale worker workspace result for ${claim.sessionId}`);
		return row;
	};
	return {
...
function createPlacementTurnClaimOps(runtime) {
	...
	const claimTurnInDatabase = (db, input, updatedAtMs) => {
		...
		if (owner.kind === "local") {
			...
			const remotePlacement = current.executionMode === "remote-exec" && current.state === "active" && owner.environmentId === current.environmentId && owner.ownerEpoch === current.activeOwnerEpoch;
		...
		} else if (current.executionMode !== "worker-turn" || current.state !== "active" || current.environmentId !== owner.environmentId || current.activeOwnerEpoch !== owner.ownerEpoch) throw new Error(`Worker turn rejected for session ${identity.sessionId}: stale owner`);
	...
	return {
		...
		completeWorkspaceResultAndReleaseTurn(claim, options = {}) {
			...
			const released = write((db) => {
				...
				const current = getRequired(db, sessionId);
				if (!resolvePlacementTurnEnvironment(current, claim)) throw new Error(`Session ${sessionId} workspace result owner changed before release`);
				assertNoRunningWorkerSessionToolOperations(db, {
					sessionId,
					claimId
				});
		...
		updateWorkspaceBaseManifest(input) {
			...
			return write((db) => {
				...
				if (!environment) throw new Error(`Cannot advance stale worker workspace for session ${sessionId}`);
				const { environmentId, ownerEpoch } = environment;
				if (!isCurrentPlacementTurnClaim(current, input.claim)) throw new Error(`Cannot advance stale worker workspace for session ${sessionId}`);
				...
				if (executeSqliteQuerySync(db, query$2(db).updateTable("worker_session_placements").set({
					...
					updated_at_ms: now()
				}).where("session_id", "=", sessionId).where("state", "=", current.state).where("transition_generation", "=", current.generation).where("environment_id", "=", environmentId).where("active_owner_epoch", "=", ownerEpoch).where("turn_claim_owner", "=", input.claim.owner.kind).where("turn_claim_id", "=", claimId).where("turn_claim_run_id", "=", runId).where("turn_claim_generation", "=", placementGeneration).where("turn_claim_owner_epoch", input.claim.owner.kind === "worker" ? "=" : "is", input.claim.owner.kind === "worker" ? ownerEpoch : null)).numAffectedRows !== 1n) throw new Error(`Worker session workspace ${sessionId} changed during reconciliation`);
				if (reconciliation) {
...
//#endregion
export { clearWorkerWorkspaceReconciliation as a, signalWorkerTurnClaimClosed as c, hasWorkerWorkspacePendingResult as i, createPlacementTurnClaimOps as n, createPlacementWorkspaceJournalOps as o, createPlacementWorkspaceResultOps as r, registerWorkerTurnClaimClosedHandler as s, ActiveTurnClaimError as t };
