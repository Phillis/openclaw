...
import { N as validateConfigObjectWithPlugins, f as readConfigFileSnapshotWithPluginMetadata, l as readConfigFileSnapshot, nt as resolveConfigSnapshotHash } from "./io-CeQckj5v.js";
...
import { i as sameDefaultInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-CMXQWcs6.js";
...
async function applySystemAgentSetup(params, hooks) {
	...
	const [{ readSetupConfigFileSnapshot, resolveQuickstartGatewayDefaults }, onboardHelpers, { applyLocalSetupWorkspaceConfig, resolveOnboardingWorkspaceConflict }, { transformConfigWithPendingPluginInstalls }] = await Promise.all([
		import("./setup.shared-BaXgZkEJ.js"),
		import("./onboard-helpers-B2IyzQ1N.js"),
		import("./onboard-config-NB8UUxrg.js"),
		import("./install-record-commit-DmAxRN_w.js")
	]);
	...
	let guardModules = expectedAgentId || expectedAgentDir || expectedModelRef ? await Promise.all([import("./agent-scope-B7ocz6pI.js"), import("./model-selection-CZ8Mp5_N.js")]) : void 0;
	...
	if (startedWithoutAuthoredRoster) {
		...
		guardModules ??= await Promise.all([import("./agent-scope-B7ocz6pI.js"), import("./model-selection-CZ8Mp5_N.js")]);
	...
	await runCommittedFollowUp(async () => {
		const { updateExecApprovals } = await import("./exec-approvals-DjshK8Fh.js");
	...
	if (surface === "cli") await runCommittedFollowUp(async () => {
		const { ensureGatewayServiceForOnboarding } = await import("./setup.finalize-ig-AtkcQ.js");
		...
		if (gateway.status === "failed") lines.push(`Gateway service: ${gateway.error}`);
			...
			const probe = await onboardHelpers.waitForGatewayReachable({
				...
				password: settings.authMode === "password" ? await (await import("./setup.secret-input-CV1LuKYa.js")).resolveSetupSecretInputString({
					config: nextConfig,
