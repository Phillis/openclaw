...
import { L as collectChannelSchemaMetadataCore, R as collectPluginSchemaMetadataCore } from "./io-CeQckj5v.js";
...
import { p as findWildcardHintMatch } from "./zod-schema-CLzqhoa9.js";
...
import { i as sameDefaultInferenceRoute, n as projectInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-CMXQWcs6.js";
import { i as appendSystemAgentAuditEntry, r as SYSTEM_AGENT_AUDIT_STORE_LABEL } from "./audit-axLAZUro.js";
...
import { i as isKernelOwnedChannelConfigKey, n as classifyConfigSchemaPathSegment, t as buildConfigSchemaCore } from "./schema-DqKbLJu8.js";
import { r as isOpenClawTrustedPluginInstallSpec } from "./install-provenance-5w7YB8TZ.js";
...
const loadOverviewModule$1 = async () => await import("./overview-B4tUj790.js");
...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { scheduleSafeGatewayRestart } = await import("./restart-coordinator-C721tLpb.js");
		return scheduleSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-BrBGxgvf.js");
	if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
	const listPlugins = deps?.listChannelSetupPlugins ?? (await import("./setup-registry-DgcYKyl8.js")).listChannelSetupPlugins;
	const resolveEntries = deps?.resolveChannelSetupEntries ?? (await import("./discovery-DcIaJWX1.js")).resolveChannelSetupEntries;
	const isConfigured = deps?.isChannelConfigured ?? (await import("./channel-configured-shared-7eH8tQug.js")).isStaticallyChannelConfigured;
	const { shouldShowChannelInSetup } = await import("./discovery-DcIaJWX1.js");
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-BanVvawm.js");
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-BanVvawm.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-DGBbZXLz.js")).applySystemAgentSetup;
			const surface = ctx.deps?.setupSurface ?? "cli";
			const recovery = surface === "cli" ? await (await import("./setup-recovery-HS4FDMH3.js")).loadLocalSetupRecovery(operation.workspace) : void 0;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-DGBbZXLz.js");
...
async function isPluginBackingDefaultInferenceRoute(pluginId) {
	...
	const { resolveOwningPluginIdsForProviderRef } = await import("./providers-CjnxerDf.js");
...
async function executePluginInstall(operation, runtime, opts) {
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			await ctx.commit(async () => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-wAO8-DSg.js");
...
const loadOverviewModule = async () => await import("./overview-B4tUj790.js");
...
async function executeSystemAgentOperation(operation, runtime, opts = {}) {
	switch (operation.kind) {
		...
		case "config-schema": {
			const { buildConfigSchemaCore, lookupConfigSchema } = await import("./schema-D_sii9VL.js");
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-CFawPW3H.js");
		...
		case "create-agent":
			...
			return await applyPersistentOperation({
				...
				opts,
				run: async (ctx) => {
					const createAgentForOperation = ctx.deps?.createAgent ?? (await import("./agent-create-a6Vr3vlk.js")).createAgent;
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-DtILs9oJ.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-D95Ye7ml.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-BO4Q9oYD.js")).runTui)({
