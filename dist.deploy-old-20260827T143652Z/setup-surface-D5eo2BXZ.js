...
import { N as splitSetupEntries, S as promptAccountId, p as mergeAllowFromEntries } from "./setup-wizard-helpers-BI1PZFar.js";
...
import { n as updateMatrixAccountConfig } from "./config-update-C0yvXuIj.js";
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as createMatrixSetupDmPolicy } from "./setup-dm-policy-md4NVAIR.js";
import { n as validateMatrixHomeserverUrl, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-C2MpdjIb.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-BauYVUvQ.js");
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = updateMatrixAccountConfig(next, params.accountId, { groupPolicy: accessConfig.policy });
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-DyKrziTE.js");
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-CSOmXSOV.js");
