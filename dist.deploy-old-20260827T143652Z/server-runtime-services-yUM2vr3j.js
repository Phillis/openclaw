...
import { c as startSessionDeliveryRuntime, o as schedulePendingSessionDeliveries } from "./subagent-completion-delivery-s-G8kBsp.js";
import "./embedded-agent-BTAFEnCK.js";
import { f as resolveHeartbeatAgents } from "./heartbeat-runner-session-ofLIVTtI.js";
import { t as startHeartbeatRunner } from "./heartbeat-runner-BkSTLoZp.js";
...
function startPendingOutboundDeliveryRecovery(params) {
	...
	const recover = (startup) => {
		...
		const settled = runWithGatewayIndependentRootWorkAdmission(async () => {
			if (stopped) return;
			const { drainPendingDeliveriesCore, recoverPendingDeliveries } = await import("./delivery-queue-recovery-CSR7sN6L.js");
			const { deliverOutboundPayloadsInternal } = await import("./deliver-Dj2TRnwT.js");
			if (stopped) return;
...
function startPendingSessionDeliveryRuntime(params) {
	...
	const timer = setTimeout(() => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { deliverQueuedSessionDelivery, recoverPendingRestartContinuationDeliveries, settleQueuedSessionDelivery } = await import("./server-restart-sentinel-H2PHOTHL.js");
			if (stopped) return;
