...
import "./tool-mutation-names-DbOogX1N.js";
...
*/
const FILE_MUTATING_TOOL_NAMES = /* @__PURE__ */ new Set([
	"apply_patch",
	"edit",
	"write"
]);
...
function isSameToolMutationAction(existing, next) {
	if (existing.actionFingerprint != null || next.actionFingerprint != null) {
		...
		const includesPatch = [existing.toolName, next.toolName].some((toolName) => normalizeLowercaseStringOrEmpty(toolName) === "apply_patch");
		const sameFileTarget = isFileMutatingToolName(existing.toolName) && isFileMutatingToolName(next.toolName) && existing.fileTarget !== void 0 && next.fileTarget !== void 0 && fileTargetsEqual(existing.fileTarget, next.fileTarget);
		if (includesPatch) return sameFileTarget;
...
//#endregion
export { isSameToolMutationAction as i, isMutatingToolCall as n, isReplaySafeToolCall as r, buildToolMutationState as t };
