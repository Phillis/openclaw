import { i as listContextEngineQuarantines } from "./registry-BcgtD5p6.js";
import { r as listLoadedRuntimePluginIds, t as getActiveRuntimePluginRegistry } from "./active-runtime-registry-B1DSutir.js";
import { n as listPersistedRuntimeToolSchemaQuarantines } from "./tool-schema-quarantine-health-DgbMv9XK.js";
import { n as resolveReadOnlyChannelPluginsForConfig } from "./read-only-DGFVk-Cu.js";
...
async function collectInstalledPluginHealthSnapshot(params) {
	const { buildPluginCompatibilityNotices, buildPluginSnapshotReport } = await import("./status-DSmvDcwx.js");
...
async function resolveUnregisteredMemoryEmbeddingProviders(params) {
	...
	try {
		const { collectRegisteredEmbeddingProviderIds, collectUnregisteredConfiguredMemoryEmbeddingProviders } = await import("./gateway-startup-plugin-ids-BE-i7-c5.js");
...
async function resolveShouldRunPluginIds(params) {
	...
	try {
		const { loadGatewayStartupPluginPlan } = await import("./gateway-startup-plugin-ids-BE-i7-c5.js");
		const { resolvePluginActivationSourceConfig } = await import("./activation-source-config-B13P-0E6.js");
		const { resolveGatewayStartupPluginActivationConfig } = await import("./plugin-activation-runtime-config-BYe2uYxh.js");
