...
import { d as recordChannelIngressResolution } from "./admission-evidence-UgNy_kxM.js";
...
function fanInChannelIngressLifecycles(inputs) {
	...
	const cancelAll = async () => {
		await Promise.all(lifecycles.map(async (lifecycle) => lifecycle.onCancelled ? await lifecycle.onCancelled() : await lifecycle.onAbandoned()));
	};
	return {
		lifecycle: {
