...
import { r as matchRealtimeVoiceActivationName } from "./activation-name-DlfylsUE.js";
import { $ as REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ, A as buildRealtimeVoiceSessionInstructions, F as resolveRealtimeVoiceAgentConsultTools, d as resolveConfiguredRealtimeVoiceProvider, h as canonicalizeRealtimeVoiceProviderId, t as createRealtimeVoiceSessionHarness, tt as realtimeVoiceAudioDurationMs, y as createRealtimeVoiceAgentTalkbackQueue, z as matchRealtimeVoiceConsultQuestions } from "./realtime-session-harness-BIfCDu_t.js";
import { a as resolveRealtimeVoiceMinBargeInAudioEndMs, c as classifyRealtimeVoiceConsultToolCall, i as resolveRealtimeVoiceInterruptResponseOnInputAudio, l as createRealtimeVoiceTurnContextTracker, n as isRealtimeVoiceWakeNameRequired, o as resolveRealtimeVoiceSessionPolicy, r as resolveRealtimeVoiceBargeIn, s as buildRealtimeVoiceSpeakExactMessage, u as classifySkippableRealtimeVoiceConsultTranscript } from "./realtime-voice-DvokcLRl.js";
import { l as parseRealtimeVoiceAgentControlToolArgs, r as REALTIME_VOICE_AGENT_CONTROL_TOOL, t as controlRealtimeVoiceAgentRun } from "./agent-run-control-B4RdSbFH.js";
import "./text-utility-runtime-BNhX-3os.js";
import { n as discordRealtimeVoiceSecretOwnerId } from "./secret-config-contract-BvEwvKfL.js";
import { b as maybeControlDiscordVoiceAgentRun, f as convertDiscordPcm48kStereoToRealtimePcm24kMono, l as logVoiceVerbose, m as createDiscordOpusEncodeStream, p as convertRealtimePcm24kMonoToDiscordPcm48kStereo, t as loadDiscordVoiceSdk, x as formatVoiceLogPreview, y as formatVoiceIngressPrompt } from "./sdk-runtime-koFzZTY4.js";
...
const DISCORD_REALTIME_MAX_PENDING_OUTPUT_BYTES = DISCORD_RAW_PCM_FRAME_BYTES * 6e3;
...
var DiscordRealtimePlayback = class {
	...
	finishOutputAudioStream(reason, { playBuffered = true } = {}) {
		...
		if (!this.outputDrainHandler) stream.end();
	}
	handleResponseDone(outcome) {
		if (this.exactSpeechState.status === "active" && !this.exactSpeechState.audioStarted) this.completeExactSpeechResponse(outcome.status);
	...
	enqueueExactSpeechMessage(text) {
		...
		if (retainedMessages >= DISCORD_REALTIME_MAX_RETAINED_EXACT_SPEECH_MESSAGES || retainedBytes + incomingBytes > DISCORD_REALTIME_MAX_RETAINED_EXACT_SPEECH_BYTES) {
			this.stopAfterOverflow("exact-speech-overflow", /* @__PURE__ */ new Error(`Discord realtime exact speech overflow: retained=${retainedMessages} retainedBytes=${retainedBytes} incomingBytes=${incomingBytes}`));
			return;
		}
	...
	stopAfterOverflow(reason, error) {
	...
	ensureOutputStream() {
		...
		stream.once("close", () => {
			if (this.outputStream !== stream || this.params.harness.outputActivity.snapshot().playbackStarted) return;
	...
	queueOutputAudio(stream, discordPcm) {
		...
		if (discordPcm.length > DISCORD_REALTIME_MAX_PENDING_OUTPUT_BYTES - pendingBytes) {
			this.stopAfterOverflow("output-audio-overflow", /* @__PURE__ */ new Error(`Discord realtime audio playback overflow: pendingBytes=${pendingBytes} incomingBytes=${discordPcm.length}`));
			return;
		}
		if (this.params.harness.outputActivity.snapshot().playbackStarted && !this.outputDrainHandler) {
			if (!stream.write(discordPcm)) this.waitForOutputDrain(stream);
			return;
		}
		this.outputPacedBuffers.push(discordPcm);
		this.outputPacedBytes += discordPcm.length;
		if (!this.outputDrainHandler && this.outputPacedBytes >= DISCORD_RAW_PCM_FRAME_BYTES * DISCORD_REALTIME_OUTPUT_PREROLL_FRAMES) this.startOutputPlayback(stream);
	}
	waitForOutputDrain(stream) {
		if (this.outputDrainHandler || this.outputStream !== stream) return;
		logger$1.info(`discord voice: realtime audio playback buffering guild=${this.params.entry.guildId} channel=${this.params.entry.channelId} bufferedBytes=${stream.writableLength + stream.readableLength}`);
		this.outputDrainHandler = () => {
			if (this.params.stopped() || this.outputStream !== stream || stream.destroyed) return;
			this.outputDrainHandler = void 0;
			...
			if (this.params.harness.outputActivity.snapshot().streamEnding) stream.end();
		};
	...
	startOutputPlayback(stream) {
		...
		pipeline(stream, opusStream, (err) => {
			if (!err || this.outputStream !== stream) return;
		...
		this.outputPacedBuffers = [];
		this.outputPacedBytes = 0;
		if (buffered.length > 0 && !stream.write(buffered)) this.waitForOutputDrain(stream);
...
var DiscordRealtimeVoiceSession = class {
	...
	async connect() {
		...
		if (configuredProviderId) {
			...
			const canonicalProviderId = canonicalizeRealtimeVoiceProviderId(configuredProviderId, this.params.cfg);
			if (canonicalProviderId) ownerProviderIds.add(canonicalProviderId);
			for (const providerId of ownerProviderIds) assertSecretOwnerAvailable("capability", discordRealtimeVoiceSecretOwnerId(this.params.accountId, providerId));
		}
		const resolved = resolveConfiguredRealtimeVoiceProvider({
			...
			isProviderAvailable: (provider) => isSecretOwnerAvailable("capability", discordRealtimeVoiceSecretOwnerId(this.params.accountId, provider.id)),
			assertProviderAvailable: (provider) => assertSecretOwnerAvailable("capability", discordRealtimeVoiceSecretOwnerId(this.params.accountId, provider.id)),
			noRegisteredProviderMessage: "No configured realtime voice provider registered"
		});
		assertSecretOwnerAvailable("capability", discordRealtimeVoiceSecretOwnerId(this.params.accountId, resolved.provider.id));
