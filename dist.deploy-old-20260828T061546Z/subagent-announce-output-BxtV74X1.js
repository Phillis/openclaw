import { n as isGatewayRpcUnavailableError } from "./transport-error-D_LRKgla.js";
...
import { et as isAnnounceSkip } from "./openclaw-state-db-kmBThqu6.js";
...
import { s as resolveLeastPrivilegeOperatorScopesForMethod } from "./method-scopes-BQC2sTma.js";
import "./delivery-context.shared-azPdmUls.js";
import "./session-history-eviction-DX5U9ZnW.js";
...
import { f as resolveProviderRefOwnership } from "./providers-BY0gR-NY.js";
...
import { a as buildAgentRunTerminalOutcomeFromWaitResult, o as classifyAgentRunTerminalOutcome } from "./agent-run-terminal-outcome-DafVNgmX.js";
import { u as isAbortedAgentStopReason } from "./run-termination-hzmbXtwI.js";
...
import { c as withAgentRuntimeExecutionLineage } from "./agent-runtime-identity-token-Cal_zwyQ.js";
import { a as runWithGatewaySessionSpawnParentExecutionIdentity, o as runWithGatewaySessionSpawnContext, t as callGatewayTool } from "./gateway-D8V0DEy4.js";
import { r as wrapPromptDataBlock } from "./sanitize-for-prompt-C5q9LjmF.js";
import "./sessions-helpers-GgSp1hTb.js";
import { f as withInProcessAgentRuntimeIdentity, t as dispatchGatewayMethodInProcess } from "./server-plugin-in-process-dispatch-BdIxABXU.js";
import { a as hasInProcessGatewayContext } from "./server-plugins-dgcbeM8t.js";
import { l as compareSubagentRunGeneration } from "./subagent-registry.store.sqlite-B8QBtOEs.js";
import { y as resolveSubagentRunTimerDelayMs } from "./subagent-run-liveness-CpuKir5n.js";
import { t as resolveSubagentCompletionResultText } from "./subagent-completion-result-DjS7tm-S.js";
import { n as forkSessionEntryFromParent } from "./session-fork-QEJxwfxN.js";
...
import { t as extractStoredAssistantText } from "./chat-history-text-CuXqMZU_.js";
import { t as readSubagentSessionEntry } from "./subagent-announce.runtime.js";
...
async function callSubagentGatewayWithDispatchMode(params, authorization, options) {
	...
	const hasInProcessGateway = deps.hasInProcessGatewayContext() || Boolean(gatewayCaller?.gatewayContextResolver?.());
	...
	if (hasInProcessGateway && request.params != null && typeof request.params === "object" && !Array.isArray(request.params)) {
		...
		const dispatch = async (workerIdentity) => {
			...
			return await deps.dispatchGatewayMethodInProcess(request.method, request.params, withInProcessAgentRuntimeIdentity({
				expectFinal: request.expectFinal,
				...allowModelOverride ? { allowSyntheticModelOverride: true } : {},
				...options?.agentRunTracking ? { agentRunTracking: options.agentRunTracking } : {},
				...gatewayCaller?.gatewayContextResolver ? { resolveGatewayContext: gatewayCaller.gatewayContextResolver } : {},
		...
		return {
			response: workerCapability ? await workerCapability.run(async (identity) => {
				if (gatewayCaller?.agentId !== identity.agentId || gatewayCaller.sessionKey !== identity.sessionKey || gatewayCaller.operationalRunInstance !== identity.operationalRunInstance || gatewayCaller.executionIdentityToken !== identity.executionIdentityToken || gatewayCaller.workerTurnClaim !== identity.turnClaim || parentExecutionIdentityToken !== identity.executionIdentityToken) throw new Error("worker child admission identity changed");
				return await dispatch(identity);
			}) : await dispatch(),
			dispatchMode: "in_process"
		};
	}
	const dispatchAgentRequest = (timeoutMs) => sessionSpawnContext && gatewayCaller?.operationalRunInstance ? runWithGatewaySessionSpawnContext(sessionSpawnContext, () => runWithGatewaySessionSpawnParentExecutionIdentity(parentExecutionIdentityToken, () => callGatewayTool(request.method, typeof timeoutMs === "number" ? { timeoutMs } : {}, request.params, {
		expectFinal: request.expectFinal,
		scopes,
		requireAgentRuntimeIdentity: true
	}))) : deps.callGateway(typeof timeoutMs === "number" ? {
		...request,
		timeoutMs
	} : request);
	return {
...
async function buildCompactAnnounceStatsLine(params) {
	...
	const parts = [`runtime ${formatDurationCompact(typeof params.startedAt === "number" && typeof params.endedAt === "number" ? Math.max(0, params.endedAt - params.startedAt) : void 0) ?? "n/a"}`, hasDirectionalUsage ? `tokens ${formatTokenCount(ioTotal)} (in ${formatTokenCount(input)} / out ${formatTokenCount(output)})` : promptCache === void 0 ? "tokens unknown" : `tokens ${formatTokenCount(promptCache)} prompt/cache`];
...
//#endregion
export { getSubagentSpawnDeps as C, deleteSubagentSessionForCleanup as E, withSubagentGatewayExecutionIdentity as S, applySubagentLaunchAuthorization as T, callNativeSubagentGateway as _, dedupeLatestChildCompletionRows as a, resolveSubagentAgentGatewayTimeoutMs as b, readSubagentOutput as c, withSubagentOutcomeTiming as d, classifySubagentTerminalOutcome as f, terminateAcceptedCollectorRun as g, retrySubagentCleanup as h, captureSubagentCompletionReply as i, readSubagentTimeoutProgress as l, cleanupProvisionalSession as m, buildChildCompletionFindings as n, filterCurrentDirectChildCompletionRows as o, cleanupFailedSpawnBeforeAgentStart as p, buildCompactAnnounceStatsLine as r, readLatestSubagentOutputWithRetry as s, applySubagentWaitOutcome as t, waitForSubagentRunOutcome as u, callSubagentGateway as v, setSubagentSpawnDepsForTest as w, buildSubagentExecutionSessionSpawnContext as x, readGatewayRunId as y };
