...
async function scanBindings({ context }, remove) {
...
const codexOrphanedSessionBindingMigration = {
	...
	async detectLegacyState(params) {
		return (await scanBindings(params)).found ? { preview: ["- Codex app-server bindings: remove orphaned session ownership"] } : null;
	},
...
async function migrateSource(source, candidates, params, store) {
	...
	try {
		return await withFileLock(source.sidecarPath, LEGACY_BINDING_LOCK_OPTIONS, async () => {
			...
			const [{ bindingStoreKey, createStoredCodexAppServerBinding, normalizeStoredCodexAppServerBindingFingerprints, readStoredCodexAppServerBinding }, { legacyCodexConversationBindingId }] = await Promise.all([import("../../session-binding-iDiVlreO.js"), import("../../conversation-binding-data-DCM0M6gS.js")]);
...
const stateMigrations = [...stateMigrations$1, codexOrphanedSessionBindingMigration];
//#endregion
