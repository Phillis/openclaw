...
import { Mn as executeSqliteQueryTakeFirstSync, Nn as getNodeSqliteKysely, Pn as iterateSqliteQuerySync, d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, jn as executeSqliteQuerySync } from "./openclaw-state-db-kmBThqu6.js";
...
import { o as isLegacyCodexProviderId } from "./codex-route-model-ref-BJZ-8dtR.js";
...
import { g as openOpenClawAgentDatabase, i as closeOpenClawAgentDatabaseByPath, p as isOpenClawAgentDatabaseOpen, y as runOpenClawAgentWriteTransaction } from "./openclaw-agent-db-BEQsKM0c.js";
...
import { $ as publishSessionEntryCacheInvalidation, B as copySessionNodeArtifactsForRepair, F as setCanonicalSqliteSessionMainKey, H as deleteSessionMembersForRepair, z as collectSessionStateIdsForEntry } from "./session-accessor.sqlite-entry-store-BIW-GrsF.js";
import { g as stripRuntimeOnlySessionSkillsFields, o as resolveDeliveryProvenCanonicalSessionKey } from "./store-entry-BN3xGmHe.js";
import { u as resolveSqliteTranscriptArchiveDirectory } from "./session-accessor.sqlite-scope-C7NrJaPh.js";
import { s as writeTranscriptArchive } from "./session-accessor.sqlite-archive-Be3BKsyF.js";
import { C as mergeSessionTranscriptVisiblePathWithOpaqueAppendPath, O as selectSessionTranscriptTreePathNodes, S as mergeSessionTranscriptTreePaths, T as scanSessionTranscriptTree, b as isSessionTranscriptLeafControl } from "./session-transcript-index-_z9fjL8c.js";
import { G as rewriteDoctorSessionEntries, K as listCanonicalSessionRepairFacts, Y as scanDoctorSessionEntriesTolerant, kt as applySessionEntryLifecycleMutation, q as loadCanonicalSessionRepairEntries } from "./session-accessor-fcDZuc2H.js";
import { a as listSqliteSessionGenerationIdsForCanonicalRepair, c as rehomeSqliteSessionDeliveryReferencesForCanonicalRepairBatch, i as ensureSqliteTranscriptGenerationsForCanonicalRepair, n as replaceSessionOwnerInTransaction, r as copySqliteSessionOwnedStateForCanonicalRepair, s as rehomeSqliteSessionDeliveryReferencesForCanonicalRepair } from "./session-accessor.sqlite-owner-C4EZWikF.js";
...
import { n as normalizeLegacySessionEntryDelivery } from "./state-migrations.legacy-session-store-jLfPw61O.js";
import { i as withDoctorSqliteMaintenanceLock, t as DoctorSqliteMaintenanceLockUnavailableError } from "./doctor-sqlite-maintenance-lock-9ifiaibD.js";
import { d as resolveTargetSqlitePath, r as listExistingAgentDatabaseTargets } from "./doctor-session-sqlite-readers-BpdoXrJ0.js";
import { o as runPostSessionPluginDoctorStateRepairs } from "./state-migrations.doctor-CpIOhL6e.js";
		...
		const operation = runDoctorAgentDatabaseOperation({
			...
			run: () => {
				scanDoctorSessionEntriesTolerant({
					...
					if (!recoveredFromProjections && params.transform(entry) !== entry) sessionKeys.push(sessionKey);
				});
...
function listTuiLastSessionStateRows(database) {
	return executeSqliteQuerySync(database, getNodeSqliteKysely(database).selectFrom("config_machine_state").select(["state_key", "value_json"]).where("state_key", "like", "tui.lastSession.%")).rows;
}
...
function repairReservedIncognitoSessionKeys(params) {
	...
	for (const target of targets) {
		const operation = runDoctorAgentDatabaseOperation({
			agentId: target.agentId,
			path: target.sqlitePath,
			run: () => withOpenClawAgentDatabaseReadOnly((database) => listReservedIncognitoKeys(database.db), {
				agentId: target.agentId,
				env: params.env,
				path: target.sqlitePath
			})
	...
	for (const target of targets) {
		const operation = runDoctorAgentDatabaseOperation({
			agentId: target.agentId,
			path: target.sqlitePath,
			run: () => withOpenClawAgentDatabaseReadOnly((database) => collectOccupiedSessionKeys(database.db), {
				agentId: target.agentId,
				env: params.env,
				path: target.sqlitePath
			})
	...
	for (const target of targets) {
		...
		try {
			runOpenClawAgentWriteTransaction((database) => applyReservedIncognitoKeyRenameColumns(database, renames), options, { operationLabel: "doctor.rename-reserved-incognito-session-keys" });
...
async function noteSessionSqliteMigrationHealth(params) {
	const { runDoctorSessionSqlite } = await import("./doctor-session-sqlite-Dh03WVGl.js");
	...
	const runSessionSqlite = async (maintenanceAuthority) => {
		...
		const { migrateLegacyMainSessionKeys } = await import("./legacy-main-session-migration-C3AviZqp.js");
