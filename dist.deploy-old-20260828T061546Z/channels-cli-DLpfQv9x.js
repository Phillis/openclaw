...
import { l as formatUnsupportedChannelActionMessage } from "./error-format-HTpcnFye.js";
import { o as callGateway } from "./call-BFtOrd_w.js";
import { a as normalizeChannelId, i as listChannelPlugins } from "./registry-CZjiz1Jg.js";
...
import { t as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-B9_prEbP.js";
...
const channelSetupCliOptionsLoader = createLazyImportLoader(() => import("./cli-add-options-CoCG6Zpc.js"));
...
const channelsCommandsLoader = createLazyImportLoader(() => import("./channels-NCDhKeMi.js"));
...
async function registerChannelsCli(program, argv = process.argv, options = {}) {
	...
	channels.command("resolve").description("Resolve channel/user names to IDs").argument("<entries...>", "Entries to resolve (names or ids)").option("--channel <name>", `Channel (${channelNames})`).option("--account <id>", "Account id (accountId)").option("--agent <id>", "Agent owner for channel resolution").addOption(new Option("--kind <kind>", "Target kind (auto|user|group|channel)").choices([
		...
		await runChannelsCommand(async () => {
			...
			await channelsResolveCommand({
				agent: agentSource && agentSource !== "default" ? typeof opts.agent === "string" ? opts.agent : void 0 : inheritOptionFromParent(command, "agent"),
	...
	deadLetters.command("list").description("List failed inbound events for one channel account").requiredOption("--channel <name>", "Channel id").option("--account <id>", "Account id", "default").option("--limit <n>", "Maximum entries", "100").option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsDeadLettersListCommand } = await import("./dead-letters-DDa5BfNr.js");
	...
	deadLetters.command("resubmit").description("Re-enqueue one failed inbound event").argument("<event-id>", "Ingress event id").requiredOption("--channel <name>", "Channel id").option("--account <id>", "Account id", "default").option("--json", "Output JSON", false).action(async (eventId, opts) => {
		await runChannelsCommand(async () => {
			const { channelsDeadLettersResubmitCommand } = await import("./dead-letters-DDa5BfNr.js");
