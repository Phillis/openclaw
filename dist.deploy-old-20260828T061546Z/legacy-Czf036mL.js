...
import { n as DEFAULT_GATEWAY_PORT } from "./paths-BBSTUjD5.js";
...
import { a as defineLegacyConfigMigration, c as mapLegacyAudioTranscription, l as mergeMissing, o as ensureRecord, s as getRecord, t as LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS } from "./legacy-config-migrations.runtime.models-DrZij-WN.js";
...
import { l as normalizeStaticProviderModelId } from "./model-ref-shared-D4yx0hwT.js";
import { i as resolveOpenClawMcpTransportAlias, n as isKnownCliMcpTypeAlias, t as canonicalizeConfiguredMcpServer } from "./mcp-config-normalize-dw5fHLEW.js";
import { n as TASK_SUGGESTION_TOOL_NAME_MIGRATION, o as migrateLegacyToolNamePolicies, r as findLegacyToolNamePaths, t as IMAGE_INSPECTION_TOOL_NAME_MIGRATION } from "./legacy-tool-name-migration-DciaZaSW.js";
import "./legacy.roster-DTfT89zD.js";
...
function bindingMatchHasLegacyDmPeerKind(binding) {
	const peer = getRecord(getRecord(getRecord(binding)?.match)?.peer);
...
const LEGACY_CONFIG_MIGRATIONS_RUNTIME_AGENTS = [
	defineLegacyConfigMigration({
		...
		legacyRules: [{
			path: ["bindings"],
			message: "bindings[].match.peer.kind uses the retired \"dm\" alias; use \"direct\". Run \"openclaw doctor --fix\".",
...
const GATEWAY_TAILSCALE_RESET_ON_EXIT_RULE = {
	...
	message: "gateway.tailscale.resetOnExit is retired because managed routes now follow the Gateway lifecycle automatically. Run \"openclaw doctor --fix\".",
...
const GATEWAY_TAILSCALE_SERVICE_NAME_RULE = {
	...
	message: "gateway.tailscale.serviceName is retired because named Services require persistent background routes that cannot follow the Gateway lifecycle. Run \"openclaw doctor --fix\"."
};
const CONTROL_UI_DEVICE_AUTH_MIGRATION_RULE = {
	...
	message: "gateway.controlUi.dangerouslyDisableDeviceAuth is retired and ignored. Control UI browsers pair through the normal device flow; run \"openclaw doctor --fix\" to remove the legacy key.",
	match: (value) => typeof value === "boolean"
};
...
function* visitKnownTtsConfigLocations(raw) {
	...
	for (const [index, entry] of agentList.entries()) yield [getRecord(getRecord(entry)?.tts), `agents.list[${index}].tts`];
	const channels = getRecord(raw.channels);
...
//#endregion
export { modelEntryWithRuntimePolicy as _, resolveMemoryHostAgentContextLimits as a, resolveRememberAcrossConversations as c, migrateLegacyWebSearchConfig as d, migrateLegacyXSearchConfig as f, migrateLegacyRuntimeModelRef as g, listLegacyRuntimeModelProviderAliases as h, normalizeConfiguredMemoryExtraPaths as i, stripRetiredTuningKnobs as l, legacyRuntimeModelAliasRequiresRuntimePolicy as m, LEGACY_CONFIG_MIGRATIONS as n, resolveMemoryHostAgentWorkspaceDir as o, ensureControlUiAllowedOriginsForNonLoopbackBind as p, MEMORY_HOST_ROOT_FILENAME as r, resolveMemoryHostSearchPathConfig as s, findLegacyConfigIssues as t, migrateLegacyWebFetchConfig as u, selectedCanonicalModelRefsForRuntimePolicy as v, hasOwnKey as y };
