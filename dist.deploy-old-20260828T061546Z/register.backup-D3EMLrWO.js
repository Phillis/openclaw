...
import { r as root, t as ensureAbsoluteDirectory } from "./fs-safe-CmrQUApq.js";
import { d as pinDirectory, f as syncDirectory, u as ensureDurableDirectory } from "./pinned-write-powa_mtU.js";
...
import { a as listAgentIds, m as resolveConfiguredAgentId } from "./agent-scope-config-CUBiGmG3.js";
import { n as getRuntimeConfig, s as readConfigFileSnapshot } from "./io-ClLVsBMp.js";
...
import { Ot as assertOpenClawStateDatabaseForMaintenance, Pt as getOpenClawStateRuntimeSchema, Rn as readSqliteUserVersion, Xt as resolveOpenClawStateSqlitePath } from "./openclaw-state-db-kmBThqu6.js";
...
import { c as createPrivateSqliteTempDirectory, d as buildPowerShellFailureCause, l as WINDOWS_POWERSHELL_COLD_SPAWN_TIMEOUT_MS, s as createPrivateSqliteDirectory, u as buildEncodedPowerShellArgs } from "./sqlite-readonly-location-BUsr5nKz.js";
import { u as applyPrivateModeSync } from "./state-database-coordinator-DNHhmvRb.js";
import { t as assertSqliteIntegrity } from "./sqlite-integrity-D3VwDKmB.js";
import { r as theme } from "./theme-vjDs9tao.js";
import "./config-B_0xOnKq.js";
import { t as formatDocsLink } from "./links-ClIwBcy4.js";
import { a as isImplicitLocalGatewayTargetFromCli, n as callGatewayFromCli, t as addGatewayClientOptions } from "./gateway-rpc-DJvB3IVo.js";
import { n as runExec } from "./exec-D2kbpwdA.js";
import { i as syncDirectoryIfSupported, n as publishFileNoClobber, r as requireDirectorySync, s as sha256File, t as getPublishFileExclusiveFailureDetails } from "./directory-durability-y-xIUhxC.js";
import "./openclaw-agent-db-BEQsKM0c.js";
import { O as OPENCLAW_AGENT_SCHEMA_SQL, t as assertOpenClawAgentDatabaseForMaintenance } from "./openclaw-agent-db-maintenance-_0tYy-zT.js";
import { a as requireGitCommandBuffer, i as requireGitCommand, r as executeGitCommand } from "./git-exec-DxrjoMs4.js";
import { t as loadSqliteVecExtension } from "./sqlite-vec-yun6599L.js";
import "./engine-storage-BWafvbUP.js";
import { n as isPathWithin } from "./cleanup-utils-DBlaUZGP.js";
...
import { a as publishVerifiedSqliteFile, i as createVerifiedSqliteSnapshot, n as sanitizeOpenClawGlobalStateSnapshot, r as sanitizeOpenClawStateLeaseRows, t as backupCreateCommand } from "./backup-pREqtFII.js";
import { a as BACKUP_MAX_DECOMPRESSION_RATIO, d as resolveBackupAgentRoot, f as resolveBackupAgentRoots, l as canonicalizePathForContainment, m as resolveRequiredBackupPath, u as recordBackupOutcomeBestEffort } from "./backup-archive-path-policy-Cz8a2UOj.js";
import { r as verifyBackupArchive, t as backupVerifyCommand } from "./backup-verify-D1Ky9nNj.js";
...
function serializeTable(database, table, rowFilter) {
...
async function dumpGitBackupDatabase(params) {
	...
	try {
		...
		for (const table of dataTables) {
			const serialized = serializeTable(database, table, table === "config_machine_state" && excludedConfigStateKeyPrefixes.length > 0 ? (row) => {
...
var LocalSqliteSnapshotProvider = class {
	...
	async #resolveSnapshotDirectory(snapshot) {
		...
		const repositoryStat = await lstatIfExists(this.#repositoryPath);
		if (!repositoryStat) throw new Error(`SQLite snapshot repository does not exist: ${this.#repositoryPath}. Check the snapshot path or create a snapshot with \`openclaw backup sqlite create\`.`);
		assertDirectory(repositoryStat, this.#repositoryPath, "SQLite snapshot repository");
		const snapshotStat = await lstatIfExists(snapshotDir);
		if (!snapshotStat) throw new Error(`SQLite snapshot does not exist: ${snapshotDir}. Run \`openclaw backup sqlite list --repository ${this.#repositoryPath}\` to inspect available snapshots.`);
...
async function resolveCreateDatabases(runtime, options) {
	...
	for (const { agentId, databasePath } of agents) {
		let resolvedPath;
		try {
			resolvedPath = await fs$1.realpath(databasePath);
		} catch (error) {
...
async function assertTargetOutsideLiveState(targetPath) {
	...
	const discoverySnapshot = resolveUpgradeConfigSnapshot(await readConfigFileSnapshot({ observe: false }));
	if (!discoverySnapshot) return;
	const agentRoots = await resolveBackupAgentRoots(discoverySnapshot.config);
	for (const { sourcePath } of agentRoots) if (isPathWithin(canonicalTarget, sourcePath)) throw new Error(`Backup restore target must be outside the live OpenClaw agent directory: ${targetPath}`);
}
...
async function resolveSnapshotDatabase(options) {
	...
	return {
		path: await fs$1.realpath(agentRoot.databasePath),
		identity: {
			role: "agent",
			agentId
		}
...
function resolveSnapshot(snapshot, scratch) {
	...
	return {
		provider: createLocalSqliteSnapshotProvider({
			repositoryPath,
			validationRootPath: scratch ? resolveRequiredBackupPath(scratch, "--scratch") : path.dirname(repositoryPath),
			...OPENCLAW_SNAPSHOT_READ_OPTIONS
		}),
		ref: { path: snapshotPath }
	};
