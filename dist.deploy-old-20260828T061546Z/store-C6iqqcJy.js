...
import { c as isSqliteLockError } from "./node-sqlite-_e3IvfT7.js";
import { a as resolveSharedAuthStorePath, i as resolveSharedAuthStoreOwnership } from "./path-resolve-Bjd7UUgA.js";
import { D as shouldPersistRuntimeExternalOAuthProfile, R as authProfilesLog, _ as mergeRuntimeExternalProfileReferences, a as loadPersistedAuthProfileStore, d as loadPersistedAuthProfileState, h as getRuntimeExternalCliProfileIds, l as buildPersistedAuthProfileState, m as isLegacyOAuthRef, n as buildPersistedAuthProfileSecretsStore, o as loadPersistedSharedAuthProfileStore, s as mergeAuthProfileStores, w as isSafeToAdoptMainStoreOAuthIdentity, y as setRuntimeExternalCliProfileIds, z as cloneAuthProfileStore } from "./persisted-Bjx2XcL3.js";
import { n as overlayExternalAuthProfiles, r as syncPersistedExternalCliAuthProfiles, t as listRuntimeExternalAuthProfiles } from "./external-auth-CHgvfGCk.js";
import { _ as writePersistedAuthProfileStoreRaw, a as inspectPersistedAuthProfileStoreRaw, c as readPersistedAuthProfileStateRaw, g as writePersistedAuthProfileStateRaw, h as runAuthProfileWriteTransaction, l as readPersistedAuthProfileStoreRaw, m as resolveAuthProfileDatabasePath, n as deferAuthProfilePostCommitPublication, r as deletePersistedAuthProfileStoreRaw, s as inspectPersistedSharedAuthProfileStoreRaw, w as listLegacyAuthProfileSources } from "./sqlite-BsvahKHa.js";
import { a as clearAuthProfileMigrationRequired, c as markAuthProfileMigrationRequired, l as warnLegacyAuthProfileSourcesIgnored, n as AuthProfileStoreUnreadableError, r as assertAuthProfileMigrationReady, t as AuthProfileMigrationRequiredError } from "./legacy-source-diagnostic-SYdrQUmo.js";
import { _ as setRuntimeAuthProfileStoreSnapshot, g as replaceRuntimeAuthProfileStoreSnapshots, l as getRuntimeAuthProfileStoreSnapshotRevisionAtDatabasePath, m as noteRuntimeAuthProfileStorePersistedMutation, p as listRuntimeAuthProfileStoreSnapshots, r as getPreparedRuntimeAuthProfileStoreSnapshotCore, s as getRuntimeAuthProfileStoreSnapshotCore, t as clearRuntimeAuthProfileStoreSnapshotCore, v as setRuntimeAuthProfileStoreSnapshotAtDatabasePath } from "./runtime-snapshots-a_60jBeK.js";
import "./source-check-kmLFZYHw.js";
...
function withAuthProfileStoreAgentDir(agentDir, sharedStateDir, run) {
	const env = {
		...process.env,
	...
	if (resolveSharedAuthStoreOwnership(env).location === "state-db") {
		...
		if (!shared && inspectPersistedSharedAuthProfileStoreRaw(env).status !== "missing") throw new AuthProfileStoreUnreadableError(void 0, env);
...
function publishRuntimeSnapshotsAfterCommit(publication) {
	...
	try {
		...
		replaceRuntimeAuthProfileStoreSnapshot(void 0, publication.agentDir, publication.databasePath);
...
function shouldKeepProfileInLocalStore(params) {
	...
	if (inherited && !params.persistedStores.localStore?.profiles[params.profileId]) {
		if (isDeepStrictEqual(buildPersistedAuthProfileSecretsStore({
			version: 1,
			profiles: { [params.profileId]: params.credential }
		}).profiles[params.profileId], inherited)) return false;
	}
...
function mergeLocalAuthProfileStoreWithInheritedStore(localStore, inheritedStore) {
	const merged = mergeAuthProfileStores(inheritedStore, localStore, { preserveBaseRuntimeExternalProfiles: true });
...
function refreshDerivedRuntimeAuthProfileStoreSnapshot(derived, committedSharedStore) {
	try {
		rebuildRuntimeAuthProfileStoreSnapshot(derived.agentDir, derived.store, void 0, derived.databasePath, committedSharedStore, derived.store.runtimeLocalProfileIds);
		return true;
	} catch (err) {
		replaceRuntimeAuthProfileStoreSnapshot(void 0, derived.agentDir, derived.databasePath);
...
function loadAuthProfileStore() {
	...
	return overlayExternalAuthProfiles(applyScopedAuthReadThrough(markRuntimePersistedProfiles(loadPersistedAuthProfileStore(agentDir) ?? createEmptyAuthProfileStore())), { agentDir });
}
function loadAuthProfileStoreForAgent(agentDir, options) {
	...
	const store = loadPersistedAuthProfileStore(effectiveAgentDir, resolvePersistedLoadOptions(effectiveOptions));
	if (!store && inspectPersistedAuthProfileStoreRaw(effectiveAgentDir, effectiveOptions?.database).status !== "missing") throw new AuthProfileStoreUnreadableError(effectiveAgentDir);
	...
	if (credentialSources.length > 0 && (!store || Object.keys(store.profiles).length === 0)) {
		...
		if (store) markAuthProfileMigrationRequired(effectiveAgentDir, migrationError);
	...
	return applyScopedAuthReadThrough(markRuntimePersistedProfiles(maybeSyncPersistedExternalCliAuthProfiles({
...
function loadAuthProfileStoreWithoutExternalProfiles(agentDir, loadOptions) {
	...
	return mergeLocalAuthProfileStoreWithInheritedStore(store, loadAuthProfileStoreForAgent(options.inheritedAuthDir, options));
}
...
function saveAuthProfileStoreInTransaction(store, agentDir, options, database, publishFromSuppliedStore = false) {
	...
	const savedAuthPath = persistenceAgentDir ? resolveAuthProfileDatabasePath(persistenceAgentDir) : database.path;
	const mainAuthPath = persistenceAgentDir ? resolveSharedAuthStorePath() : database.path;
	...
	const stateChanged = !isDeepStrictEqual(readPersistedAuthProfileStateRaw(persistenceAgentDir, database), statePayload);
	...
	const publishRuntimeSnapshots = () => {
		...
		if (credentialsChanged || stateChanged) noteRuntimeAuthProfileStorePersistedMutation(persistenceAgentDir, {
			credentialsChanged,
			profileSetChanged,
			stateChanged,
		...
		if (suppliedRuntimeStore) {
			...
			if (existing) setRuntimeAuthProfileStoreSnapshot(materializeRuntimeAuthProfileStoreSnapshot(suppliedRuntimeStore, existing), persistenceAgentDir);
			if (savesMainStore && (credentialsChanged || stateChanged)) {
				let converged = true;
				for (const derived of derivedSnapshots) converged = refreshDerivedRuntimeAuthProfileStoreSnapshot(derived, committedSharedStore) && converged;
				return converged;
			}
			return true;
		}
		if (savesMainStore) {
			const existing = getRuntimeAuthProfileStoreSnapshot(persistenceAgentDir);
			if (existing) setRuntimeAuthProfileStoreSnapshot(materializeRuntimeAuthProfileStoreSnapshot(committedSharedStore, existing), persistenceAgentDir);
		} else refreshRuntimeAuthProfileStoreSnapshot(persistenceAgentDir);
		let converged = true;
		for (const derived of derivedSnapshots) converged = refreshDerivedRuntimeAuthProfileStoreSnapshot(derived, committedSharedStore) && converged;
		return converged;
	};
...
function rebuildRuntimeAuthProfileStoreSnapshot(agentDir, existing, predecessor, databasePath, inheritedStore, capturedLocalProfileIds) {
	let refreshed;
	try {
		refreshed = loadAuthProfileStoreWithoutExternalProfiles(agentDir);
	} catch (err) {
		if (!inheritedStore) throw err;
...
function restoreAuthProfileStorePersistenceSnapshot(snapshot, owned, agentDir, options = {}) {
	...
	runAuthProfileWriteTransaction(agentDir, (database) => {
		...
		publishRuntimeSnapshots = {
			...agentDir ? { agentDir } : {},
			databasePath: agentDir ? resolveAuthProfileDatabasePath(agentDir) : database.path,
			publish: () => {
...
//#endregion
export { buildPortableAuthProfileStoreForAgentCopy as C, isInheritedMainOAuthCredentialFromStores as E, withEnvOnlyAuthProfileStore as S, resolveAuthProfilePortability as T, restoreAuthProfileStorePersistenceSnapshot as _, ensureAuthProfileStoreWithoutExternalProfiles as a, updateAuthProfileStoreWithLock as b, getRuntimeAuthProfileStoreSnapshot as c, loadAuthProfileStoreForRuntime as d, loadAuthProfileStoreForSecretsRuntime as f, resolveRuntimeAuthProfileAgentDir as g, resolvePersistedAuthProfileOwnerAgentDir as h, ensureAuthProfileStoreForLocalUpdate as i, isInheritedMainOAuthCredential as l, preserveResolvedSecretBackedCredentials as m, clearRuntimeAuthProfileStoreSnapshot as n, findPersistedAuthProfileCredential as o, loadAuthProfileStoreWithoutExternalProfiles as p, ensureAuthProfileStore as r, getPreparedRuntimeAuthProfileStoreSnapshot as s, captureAuthProfileStorePersistenceSnapshot as t, loadAuthProfileStore as u, saveAuthProfileStore as v, isAuthProfileCredentialPortableForAgentCopy as w, withAuthProfileStoreAgentDir as x, saveAuthProfileStoreIfPersistenceSnapshotMatches as y };
