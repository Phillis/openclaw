...
import { l as getActivePluginHttpRouteRegistry, u as getActivePluginHttpRouteRegistryVersion } from "./runtime-B2KAtS3O.js";
...
function isNoopGatewayReloadPlan(plan) {
	return !plan.restartGateway && plan.hotReasons.length === 0 && !plan.reloadHooks && !plan.refreshHooksPolicy && !plan.restartGmailWatcher && !plan.restartCron && !plan.restartHeartbeat && !plan.reconcileSkillReviewJobs && !plan.restartHealthMonitor && !plan.reloadPlugins && !plan.disposeMcpRuntimes && plan.restartChannels.size === 0 && (plan.restartChannelAccounts?.size ?? 0) === 0;
}
...
const BASE_RELOAD_RULES = [
	...
	{
		prefix: "talk.provider",
		kind: "hot",
		actions: ["reload-plugins"]
	},
	{
		prefix: "talk.realtime.provider",
		kind: "hot",
		actions: ["reload-plugins"]
	}
];
