...
import { c as resolveNpmJsonEntries, r as isExactSemverVersion } from "./npm-registry-spec-BdgyvSs0.js";
import "./worker-admission-v0PuudgP.js";
import { t as PROCESS_NODE_VERSION_CHECK } from "./node-version-B6qdK28I.js";
...
import { i as normalizeScpRemotePath } from "./scp-host-3FoHUz1i.js";
import { a as hashWorkerBundleManifest, i as compareWorkerBundlePaths, n as WORKER_BUNDLE_MANIFEST_VERSION, r as WORKER_BUNDLE_RSYNC_RECEIVER_PATH, t as WORKER_BUNDLE_ENTRY_PATH } from "./worker-bundle-hash-mYTNaYdm.js";
import { t as DEFAULT_WORKER_BUNDLE_ARCHIVE_LIMITS } from "./worker-bundle-limits-BFwcdQuE.js";
import { n as readWorkerBundleArchiveManifest } from "./worker-bundle-archive-yl24WAFg.js";
...
const WORKER_DEPLOY_ARTIFACT_PATHS = [WORKER_BUNDLE_ENTRY_PATH, WORKER_BUNDLE_RSYNC_RECEIVER_PATH];
async function stageWorkerDeployArtifact(params) {
	...
	try {
		...
		throw new Error(`OpenClaw worker deploy artifact is missing; build the running package at ${params.sourceRoot}`, { cause: error });
	}
	...
	try {
		...
		if (!openedStats.isFile() || currentStats.isSymbolicLink() || !currentStats.isFile() || currentRealPath !== expectedRealPath || currentStats.dev !== openedStats.dev || currentStats.ino !== openedStats.ino) throw new Error(`Worker deploy artifact changed while packaging: ${relativeSourcePath}`);
		contents = await handle.readFile();
	} finally {
		await handle.close();
	}
	const stagedPath = path.join(params.stagingRoot, params.artifactPath);
	await fs$1.writeFile(stagedPath, contents, { mode: 448 });
	await fs$1.chmod(stagedPath, 448);
	return {
...
async function collectWorkerBundleManifest(sourceRoot, stagingRoot) {
	const manifest = [];
	for (const artifactPath of WORKER_DEPLOY_ARTIFACT_PATHS) manifest.push(await stageWorkerDeployArtifact({
		sourceRoot,
		stagingRoot,
		artifactPath
	}));
...
async function pruneWorkerBundleCache(params) {
	const retained = new Set([params.currentBundleHash, ...params.retainedBundleHashes].filter((hash) => /^[a-f0-9]{64}$/u.test(hash)));
	let entries;
	try {
...
const BUNDLE_TRANSFER_TIMEOUT_MAX_MS = 60 * 6e4;
const BOOTSTRAP_OPERATION_HEADROOM_MS = 5 * 6e4;
...
const WORKER_BUNDLE_ARTIFACT_PATHS = [WORKER_BUNDLE_ENTRY_PATH, WORKER_BUNDLE_RSYNC_RECEIVER_PATH];
