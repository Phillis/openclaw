...
import { a as requestTelemetryConsent, o as requireRiskAcknowledgement } from "./setup.shared-C1C_TtR8.js";
...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk, agentName) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted, setupAgentName) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-DZNgycUh.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	const prompter = await (deps.createPrompter?.() ?? import("./clack-prompter-BzttpXIv.js").then(({ createClackPrompter }) => createClackPrompter()));
	...
	const localOnboarding = custodianMode ? await import("./local-onboarding-state-DH18SU33.js") : void 0;
	...
	if (!custodianMode) {
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-D_t-FBGb.js")).runSetupMemoryImportStep)({
	...
	const { resolveSetupWorkspaceSelection } = await import("./setup.workspace-DDeCfzJt.js");
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-BzxYMfcc.js")).applySystemAgentSetup;
			...
			const appliedSnapshot = localSetup?.status === "pending" ? await (await import("./setup-recovery-DIAXiFHd.js")).completeLocalSetupRecovery({
				owner: localSetup,
	...
	if (wantsDiscovery) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-D_t-FBGb.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-DOcGxI61.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
	...
	if (opts.tui !== true) {
		if ((await (deps.runBrowserHandoff ?? (await import("./onboard-browser-handoff-DYI6Ou83.js")).runBrowserHatchHandoff)({
			config: persistedConfig,
			prompter,
...
async function launchHatchTui(workspace, local) {
	const [{ launchTuiCli }, { DEFAULT_BOOTSTRAP_FILENAME }, { restoreTerminalState }, fs, path] = await Promise.all([
		import("./tui-launch-DNhHjhS0.js"),
		import("./workspace-BqQ5tRLi.js"),
...
async function runGuidedOnboarding(opts, runtime, deps = {}) {
	...
	if (handoff.next === "hatch") {
		...
		else await launchHatchTui(handoff.workspace, handoff.local);
		return;
	}
