...
import { N as splitSetupEntries, S as promptAccountId, p as mergeAllowFromEntries } from "./setup-wizard-helpers-BGcFrkxT.js";
...
import { n as updateMatrixAccountConfig } from "./config-update-D1VNZx_n.js";
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as createMatrixSetupDmPolicy } from "./setup-dm-policy-DkAt4N3e.js";
import { n as validateMatrixHomeserverUrl, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-C3YRhING.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-wiHooqxw.js");
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = updateMatrixAccountConfig(next, params.accountId, { groupPolicy: accessConfig.policy });
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-BKJsTF-b.js");
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-Bu2nOie6.js");
