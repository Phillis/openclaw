...
import { n as installDistEsmResolveFastPath, t as tryHandleRootVersionFastPath } from "./entry.version-fast-path-4o76C6Un.js";
...
async function flushEntryStartupTraceForEarlyReturn(argv) {
	...
	const { loadCliDotEnvForEarlyDiagnostic } = await import("./dotenv-DLvHYU08.js");
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-O3MVKtrv.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
	...
	await runCliWithExitFinalization({
		run: async () => {
			if (isNativeHookRelayArgv(argv) && !argv.includes("--help") && !argv.includes("-h")) {
				const { runNativeHookRelayCliFromArgv } = await import("./native-hook-relay-cli-CQkyXgrC.js");
