...
import { a as resolveOsHomeRelativePath, c as resolveUserPath } from "./home-dir-BFvskzn8.js";
...
import { n as isImmutableGitCommitRef } from "./git-install-BPOOTAxx.js";
...
import { U as copyPluginInstallTransactionRequest } from "./install-managed-npm-state-BKmVpI9X.js";
import { n as installPluginFromPath } from "./install-DJ6ueg-H.js";
...
async function downloadUrlToTempFile(url, timeoutMs) {
	...
	try {
		...
		try {
			...
			if (!isPathInside(createdTmpDir, targetPath)) throw new Error("invalid download filename");
...
async function ensureInsideMarketplaceRoot(rootDir, candidate, options) {
	...
	if (!isPathInside(rootDir, resolved)) return {
		ok: false,
...
async function installPluginFromMarketplace(params) {
	...
	try {
		...
		const result = await installPluginFromPath(copyPluginInstallTransactionRequest(params, {
