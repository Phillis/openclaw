...
import { Dt as isJsonObject, Q as CodexAppServerRpcError, c as getLeasedSharedCodexAppServerClient, ct as retainCodexAppServerLiveThread, h as releaseLeasedSharedCodexAppServerClient, tt as consumeCodexAppServerLiveThread } from "./shared-client-DsH0bBjk.js";
import { _ as sessionBindingIdentity, t as CODEX_APP_SERVER_BINDING_GUARDED_REQUEST_TIMEOUT_MS } from "./session-binding-BlplpJ9G.js";
import { _ as readCodexNotificationTurnId, a as closeCodexStartupClientBestEffort, d as unsubscribeCodexThreadBestEffort, g as readCodexNotificationThreadId, r as CodexAppServerUnsafeSubscriptionError, s as isCodexAlreadyTerminalInterruptError } from "./attempt-client-cleanup-CBrsZNhS.js";
import { i as readCodexThreadContextSnapshot, x as readCodexNotificationItem } from "./event-projector-usage-CAfp3B0n.js";
...
import { t as isSameCodexAppServerThreadOwner } from "./thread-ownership-1EHrSuJl.js";
...
function readIgnoredCompactionOverridePaths(params) {
	...
	for (const entry of readCompactionOverrideEntries(params)) {
		...
		if (localProvider) ignored.add(`${entry.path}.compaction.provider`);
	}
	return [...ignored];
}
			...
			try {
				const guardedResult = await options.bindingStore.withLease(bindingIdentity, async () => {
					...
					if (!currentBinding || !isSameNativeCompactionBinding(currentBinding, binding)) {
						log.warn("codex app-server compaction could not use the thread binding because it changed", {
							sessionId: params.sessionId,
							sessionKey: params.sessionKey,
							expectedThreadId: binding.threadId,
							currentThreadId: currentBinding?.threadId
