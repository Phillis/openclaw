...
import { Et as findOpenClawStateDatabaseSchemaMigrationRequiredError } from "./openclaw-state-db-kmBThqu6.js";
...
import { r as CONFIG_AUDIT_STORE_LABEL } from "./io.audit-sMaKpbcb.js";
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-Beo1KDuk.js");
	return {
		enabled: true,
...
async function runGatewayCommandOnce(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-CDmFXqHn.js");
	...
	const serverImportMeasurement = startupTrace.startMeasure("cli.server-import", () => import("./server-BRa50JVH.js"));
	...
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-CDmFXqHn.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-B7bo7hUW.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-CDmFXqHn.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			const { maybeExplainGatewayServiceStop } = await import("./shared-Dt6-by5k.js");
		...
		if (findOpenClawAgentDatabaseMediaMigrationRequiredError(err)) try {
			const { parkCurrentLaunchAgentForMaintenance } = await import("./launchd-DsG1_OXl.js");
...
async function runGatewayCommand(opts, hooks = {}, recoveryDeps) {
	try {
		...
		const { offerInvalidConfigRecovery } = await import("./invalid-config-recovery-Cmuwy99K.js");
