...
import { a as routeLogsToStderr, t as enableConsoleCapture } from "./console-SZn871dT.js";
import { n as DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS } from "./timeouts-D2XMKe-X.js";
import { t as notifyListeners } from "./listeners-BogSNJ-R.js";
import { J as WorkerTranscriptCommitResponseFrameSchema, L as WorkerPortalResponseFrameSchema, N as WorkerLiveEventResponseFrameSchema, O as WorkerHeartbeatResponseFrameSchema, S as WorkerAdmissionResponseFrameSchema, T as WorkerGitHubPublishResponseFrameSchema, U as WorkerSessionsSpawnResponseFrameSchema, V as WorkerSessionsSendResponseFrameSchema } from "./worker-admission-v0PuudgP.js";
import { f as WorkerProtocolCloseReasonSchema, o as WORKER_PROTOCOL_MAX_PAYLOAD_BYTES } from "./worker-protocol-primitives-Ch87u2k0.js";
import { t as createDeferredCore } from "./deferred-D0La5CRk.js";
import "./version-CwNT1gaY.js";
import { t as rawDataToString } from "./websocket-data-2vBvd4uX.js";
import { C as parseWorkerLaunchDescriptor, D as resolveWorkerConnectionTarget, T as WorkerConnectionEndpointError, a as WorkerConnectionStoppedError, c as isFencedCloseReason, d as toWorkerConnectionError, f as NODE_WORKER_CONNECTION_FAILURE_MESSAGE_TYPE, i as WorkerConnectionInterruptedError, n as WorkerAdmissionDeadlineExceededError, o as WorkerFencedError, r as WorkerAdmissionError, s as formatWorkerConnectionFailure, t as WORKER_ADMISSION_DEADLINE_MS, u as resolvePositiveTimeout, x as buildWorkerConnectParams } from "./worker-connection-contract-CLo4JQpE.js";
import "./backoff-BkMI1WEL.js";
import { a as WORKER_PROTOCOL_MAX_INFERENCE_PAYLOAD_BYTES, d as WorkerInferenceStartResponseFrameSchema, h as validateWorkerInferenceTerminalFrame, p as validateWorkerInferenceEventFrame, s as WorkerInferenceCancelResponseFrameSchema } from "./worker-inference-BzU_LUo9.js";
import { c as getActiveBackgroundExecSessionCount, x as waitForExecScope } from "./bash-process-registry-08adq0zn.js";
...
import { t as parseWorkerProcessRequest } from "./worker-process-protocol-DxScVuGj.js";
...
function connectWorkerConnectionAttempt(options) {
	...
	const socket = connectionOptions.createSocket ? connectionOptions.createSocket(target.url, socketOptions) : new WebSocket(target.url, socketOptions);
	...
	return new Promise((resolve, reject) => {
		...
		socket.on("error", (error) => {
			if (!admitted) rejectAttempt(new WorkerConnectionInterruptedError(`${opened ? "admission interrupted" : "connect failed"}: ${toWorkerConnectionError(error).message}`));
		});
		socket.on("open", () => {
...
const WORKER_SESSION_SPAWN_TIMEOUT_MS = 15 * 6e4;
...
var WorkerConnection = class {
	...
	async reconnectAfterClose() {
		try {
			...
			if (!this.isTerminal()) this.finishFailed(toWorkerConnectionError(error));
		} finally {
...
async function runWorkerDescriptor(descriptor, options = {}) {
	...
	try {
		...
		const [{ runWorkerEmbeddedTurn }, { createWorkerInferenceStreamAdapter }] = await Promise.all([import("./embedded-agent.runtime-Jix-bWVA.js"), import("./inference-stream.runtime.js")]);
			...
			const onData = (raw) => {
				...
				try {
					...
					while (offset < chunk.length) {
						...
						if (byteLength > 26214400) throw new Error("managed worker request exceeds the protocol payload limit");
...
async function runWorkerCommand(options) {
	...
	try {
		if (options.managed) {
			if (!await (options.lifetime?.started ?? Promise.resolve(true))) return;
		...
		const [descriptor, started] = await Promise.all([readLaunchDescriptor(options.input), options.lifetime?.started ?? Promise.resolve(true)]);
		if (!started) return;
		options.lifetime?.signal.addEventListener("abort", stopForLifetime, { once: true });
		if (options.lifetime?.signal.aborted) stopForLifetime();
		process.once("SIGINT", stop);
