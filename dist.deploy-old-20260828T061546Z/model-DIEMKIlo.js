...
import { g as randomIdempotencyKey, o as callGateway } from "./call-BFtOrd_w.js";
...
import { t as canonicalizeCaseOnlyCatalogModelRef } from "./model-selection-Cp8EGD61.js";
import { t as buildExplicitSessionIdSessionKey } from "./session-_tB-9DuM.js";
...
async function runModelAuthStatus(agent) {
	const captured = [];
	const { modelsStatusCommand } = await import("./list.status-command-BvyP9HTw.js");
...
function registerModelCapabilityCommands(capability) {
	...
	modelAuth.command("login").description("Run provider auth login").requiredOption("--provider <id>", "Provider id").option("--method <id>", "Provider auth method id").option("--agent <id>", "Agent id (default: configured default agent)").action(async (opts, command) => {
		await runCommandWithRuntime(defaultRuntime, async () => {
			...
			const { modelsAuthLoginCommand } = await import("./auth-5mdxPuOh.js");
