...
import { a as resolvePluginRootPublicSurfacePath, r as resolveBundledPluginPublicSurfacePath } from "./public-surface-runtime-Cp0SB5E0.js";
...
import { a as resolveLoaderPackageRoot, s as resolvePluginLoaderTryNative } from "./sdk-alias-TKlN0lQ-.js";
import { t as shouldRejectHardlinkedPluginFiles } from "./hardlink-policy-jAYIsS4O.js";
import { t as MissingPublicSurfaceError } from "./facade-loader-BwQ2fefX.js";
...
function loadBundledPublicSurfaceAtLocation(params) {
	return loadValidatedPublicSurfaceModule({
		...
		boundaryLabel: params.location.boundaryRoot === OPENCLAW_PACKAGE_ROOT ? "OpenClaw package root" : "plugin root",
...
function loadPluginPublicArtifactModuleSync(params) {
	...
	if (!modulePath) throw new MissingPublicSurfaceError(`Unable to resolve plugin public surface ${params.pluginRoot}/${params.artifactBasename}`);
	return loadValidatedPublicSurfaceModule({
		modulePath,
...
function loadBundledPluginPublicArtifactModuleFromCandidatesSync(params) {
	for (const artifactBasename of params.artifactCandidates) {
		...
		if (location) return loadBundledPublicSurfaceAtLocation({
			dirName: params.dirName,
			artifactBasename,
			location
		});
