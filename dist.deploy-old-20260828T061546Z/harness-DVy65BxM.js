...
import "./simple-completion-runtime-Jot6pF-p.js";
...
function createCodexAppServerAgentHarness(options) {
	...
	return {
		...
		...sessionCatalogControlFactory && sessionRuntime ? { sessionFork: {
			...
			fork: async (params) => {
				const { forkCodexUpstreamSession } = await import("./upstream-session-fork-Bp8ah5t3.js");
		...
		authBinding: { fingerprint: async (params) => {
			const { fingerprintCodexAppServerAuthBinding } = await import("./auth-binding-CzoJ3dT1.js");
		...
		fetchUsageSnapshot: async (ctx) => {
			const { fetchCodexAppServerUsageSnapshot } = await import("./usage-sOaerozF.js");
		...
		loadModelCatalog: async (params) => {
			const { loadCodexAppServerModelCatalog } = await import("./model-catalog-BK3xawgP.js");
		...
		loadMcpToolCatalog: async (params) => {
			const { loadCodexEffectiveMcpCatalog } = await import("./effective-mcp-catalog-BxbwpOrY.js");
		...
		runAttempt: async (params) => {
			const { runCodexAppServerAttempt } = await import("./run-attempt-9TjeASp7.js");
		...
		runIsolatedCompletionV2: async (params) => {
			...
			const { runCodexIsolatedCompletion } = await import("./isolated-completion-C5jKVZqP.js");
		...
		finalizeSettledTurn: async (params) => {
			const { runCodexSettledTurnFinalization } = await import("./settled-turn-finalizer-CdUOfj10.js");
			return runCodexSettledTurnFinalization(params, { pluginConfig: options?.resolvePluginConfig?.() ?? options?.pluginConfig });
		},
		runSideQuestion: async (params) => {
			const { runCodexAppServerSideQuestion } = await import("./side-question-V0uNCalQ.js");
		...
		compact: async (params) => {
			const { maybeCompactCodexAppServerSession } = await import("./compact-CoPWFuJs.js");
		...
		withSessionDeletion: async (params, run) => {
			const { withCodexAppServerSessionDeletion } = await import("./session-retirement-DnAUSA5k.js");
			params.assertCurrent();
			return withCodexAppServerSessionDeletion(options.bindingStore, params, run);
		},
		reset: async (params) => {
			if (params.sessionId && params.reason !== "deleted") {
				const [{ reclaimCurrentCodexSessionGeneration, sessionBindingIdentity }, { retireCodexAppServerSessionGeneration }] = await Promise.all([import("./session-binding-iDiVlreO.js"), import("./session-retirement-DnAUSA5k.js")]);
