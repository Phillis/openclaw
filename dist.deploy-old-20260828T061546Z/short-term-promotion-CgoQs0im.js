...
import { a as DEFAULT_MEMORY_DEEP_DREAMING_MIN_SCORE, b as isSameMemoryDreamingDay, y as formatMemoryDreamingDay } from "./dreaming-14k0XOwK.js";
...
import { n as listMemoryArtifactProvenance } from "./memory-artifact-provenance-DJ7b-VmM.js";
...
import { t as appendMemoryHostEvent } from "./memory-host-events-t0Tjcrv3.js";
import "./state-paths-DQKtm04E.js";
import "./text-utility-runtime-BNhX-3os.js";
import { b as writeMemoryCoreWorkspaceEntries, c as SHORT_TERM_LOCK_NAMESPACE, d as SHORT_TERM_RECALL_NAMESPACE, g as memoryCoreWorkspaceStateKey, n as DREAMING_MEMORY_BACKUP_NAMESPACE, s as SHORT_TERM_LOCK_MAX_ENTRIES, v as openMemoryCoreStateStore, y as readMemoryCoreWorkspaceEntries } from "./dreaming-state-B0qd2W7q.js";
import { t as extractAssistantText } from "./dreaming-shared-BXYFnMQo.js";
import { n as resolveMemoryCoreTimestamp, t as resolveMemoryCoreNowMs } from "./time-BhFVUM0b.js";
import { a as resolveLockPath, i as parseLockOwnerPid, n as deleteShortTermLockEntryIfCurrent, o as withMemoryWorkspaceLock, r as isProcessLikelyAlive } from "./memory-workspace-lock-BGmos1BO.js";
import { i as updateDreamsFile } from "./dreaming-dreams-file-BDci2YVx.js";
import { A as parseEntryRangeFromKey, B as summarizeConceptTagScriptCoverage, C as mergeRecentDistinct, D as normalizeShortTermRecallStore, E as normalizeMemoryPathForWorkspace, F as toNonNegativeInt, I as totalSignalCountForEntry, L as truncateShortTermSnippet, M as toFiniteNonNegativeInt, N as toFinitePositive, O as normalizeSnippet, P as toFiniteScore, R as deriveConceptTags, S as mergeQueryHashes, T as normalizeMemoryPath, _ as hashQuery, a as resolvePhaseSignalPath, b as isShortTermSessionCorpusPath, c as writeStore, d as buildDailyClaimEntryKey, f as buildEntryKey, g as enforceShortTermRecallStoreRetention, h as compareStoreTimestampDesc, i as readStore, j as parseStoreTimestampMs, k as normalizeWeights, l as SHORT_TERM_BASENAME_RE, m as clampScore, o as resolveStorePath, p as calculateRecencyComponent, r as readPhaseSignalStore, s as writePhaseSignalStore, t as emptyPhaseSignalStore, u as buildClaimHash, v as isContaminatedDreamingSnippet, w as normalizeIsoDay, x as mergeProjectKeyLists, y as isShortTermMemoryPath } from "./short-term-promotion-store-BlKDyXAW.js";
import { a as reconcileMemoryEntryOrigins, i as listMemorySessionTombstones, o as recordMemoryEntryOrigins } from "./memory-entry-origins-DKs1Aq26.js";
...
async function recordShortTermRecalls(params) {
	...
	await withMemoryWorkspaceLock(workspaceDir, async () => {
		...
		for (const result of admitted) {
			...
			const key = nonDailyEntry?.key ?? (signalType !== "recall" || store.entries[claimKey] ? claimKey : buildEntryKey(result));
			const existing = store.entries[key];
			const score = clampScore(result.score);
			const queryHash = hashQuery(signalType === "grounded" ? normalizeSnippet(result.query ?? query) || query : query);
	...
	try {
		...
		try {
			...
			while (restored < original.length) {
				const { bytesWritten } = await handle.write(original, restored, original.length - restored, restored);
				if (bytesWritten <= 0) throw new Error("MEMORY.md restore write made no progress", { cause: error });
				restored += bytesWritten;
			}
			await handle.truncate(original.length);
			await handle.sync();
		} catch (restoreError) {
			throw new Error("MEMORY.md in-place write failed and restoring the original content also failed", { cause: restoreError });
		}
		throw error;
	} finally {
		await handle.close();
...
async function applyShortTermPromotions(options) {
	...
	const rejectionReasons = /* @__PURE__ */ new Map();
	const eligible = currentCandidates.filter((candidate) => {
		...
		const reason = isPromotionOriginBlocked(candidate) ? `origin filter (${candidate.provenance?.originClass})` : options.consolidation && (!latest || !isConsolidationCandidateEligible(candidate)) ? "consolidation origin/session filter" : isContaminatedDreamingSnippet(candidate.snippet) ? "contamination filter" : candidate.promotedAt || latest?.promotedAt ? "already promoted" : candidate.score < minScore ? `score threshold (${candidate.score.toFixed(3)} < ${minScore})` : candidate.signalCount < minRecallCount ? `signal threshold (${candidate.signalCount} < ${minRecallCount})` : queryCount < minUniqueQueries ? `query threshold (${queryCount} < ${minUniqueQueries})` : maxAgeDays >= 0 && candidate.ageDays > maxAgeDays ? `age threshold (${candidate.ageDays.toFixed(1)}d > ${maxAgeDays}d)` : void 0;
		if (reason) rejectionReasons.set(candidate.key, reason);
		return !reason;
	});
	const selected = eligible.slice(0, limit);
	for (const candidate of eligible.slice(limit)) rejectionReasons.set(candidate.key, `selection limit (${limit})`);
	...
	for (const candidate of selected) {
		...
		} else rejectionReasons.set(candidate.key, !rehydrated ? "source rehydration failed" : sourceFingerprintBefore !== sourceFingerprintAfter ? "source changed during apply" : "contamination filter after rehydration");
	}
	...
	await withFileLock(await resolveMemoryPromotionLockTarget(workspaceDir), MEMORY_WRITE_LOCK_OPTIONS, async () => {
		await withMemoryWorkspaceLock(workspaceDir, async () => {
			...
			if (options.agentId && committedMemoryContent) reconcileMemoryEntryOrigins({
				agentIds: [options.agentId, ...options.workspaceAgentIds ?? []],
				previousMemory: existingMemory,
				currentMemory: committedMemoryContent,
				operations: consolidationResult && consolidationPlan ? consolidationPlan.operations : toAppend.map((candidate) => ({
	...
	return {
		...
		rejectedCandidates: currentCandidates.filter((candidate) => !committedCandidates.some((applied) => applied.key === candidate.key)).map((candidate) => ({
			candidate,
			reason: rejectionReasons.get(candidate.key) ?? "candidate changed during apply"
		})),
...
async function repairShortTermPromotionArtifacts(params) {
	...
	if (lockEntry && Date.now() - lockEntry.acquiredAt > 6e4) {
		const ownerPid = parseLockOwnerPid(lockEntry.owner);
		if (ownerPid === null || !isProcessLikelyAlive(ownerPid)) removedStaleLock = await deleteShortTermLockEntryIfCurrent(lockStore, lockKey, lockEntry);
	}
	await withMemoryWorkspaceLock(workspaceDir, async () => {
		const rawEntries = await readMemoryCoreWorkspaceEntries({
			namespace: SHORT_TERM_RECALL_NAMESPACE,
