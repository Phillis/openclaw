...
import { $ as MACHINE_OUTPUT_JSON_OPTION_DESCRIPTION, L as isGatewayMachineOutput } from "./argv-CCdO9MSu.js";
...
import { o as resolveGatewayRpcOptions, r as callGatewayFromCliWithTransport, s as resolveGatewayRpcOptionsWithLocalPort, t as addGatewayClientOptions } from "./gateway-rpc-DJvB3IVo.js";
import { t as addGatewayServiceCommands } from "./register-service-commands-BmvQS-uQ.js";
...
import { t as addGatewayRunCommand } from "./run-command-JPfMnnOE.js";
...
function addGatewayRestartHandoffCommands(gateway) {
	...
	restartHandoff.command("consume").description("Atomically consume a gateway restart handoff").helpOption(false).allowUnknownOption().allowExcessArguments().option("--expected-pid [pid]", "PID of the exited gateway process", collectExpectedPid, []).option("--json", MACHINE_OUTPUT_JSON_OPTION_DESCRIPTION).action(async (opts, command) => {
		...
		try {
			const { consumeGatewayRestartHandoffSync } = await import("./restart-handoff-BK5gYItt.js");
...
const gatewayStatusModuleLoader = createLazyImportLoader(() => import("./gateway-status-R4COamHq.js"));
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-CK080YwI.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(() => import("./bonjour-discovery-BwhM-hxM.js"));
const wideAreaDnsModuleLoader = createLazyImportLoader(() => import("./widearea-dns-Bgid0lIV.js"));
const healthStyleModuleLoader = createLazyImportLoader(() => import("./terminal-core/health-style.js"));
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-BVrwdtXp.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(() => import("./diagnostic-stability-bundle-BOkinCX5.js"));
const supportExportModuleLoader = createLazyImportLoader(() => import("./diagnostic-support-export-bYl803sM.js"));
const daemonStatusGatherModuleLoader = createLazyImportLoader(() => import("./status.gather-caIMpbge.js"));
...
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayAuthErrorJson, formatGatewayClientRequestErrorJson, formatGatewayTransportErrorJson } = await import("./call-DPYKD0iw.js");
...
function registerGatewayCli(program, deps = {}) {
	...
	gateway.command("auth-token").description("Reveal the configured shared Gateway token").option("--show", "Print the token to an interactive terminal", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			...
			const { gatewayAuthTokenCommand } = await import("./gateway-auth-token-C54IRn0F.js");
	...
	gatewayCallOpts(gateway.command("call").description("Call a Gateway method").argument("<method>", "Method name (health/status/system-presence/cron.*)").option("--params <json>", "JSON object string for params", "{}").action(async (method, opts, command) => {
		await runGatewayCommand(async () => {
			const rpcOpts = resolveGatewayRpcOptionsWithLocalPort(method === "openclaw.setup.detect" && command.getOptionValueSource("timeout") === "default" ? {
				...opts,
	...
	gatewayCallOpts(gateway.command("stability").description("Fetch payload-free Gateway stability diagnostics").option("--limit <limit>", "Maximum number of recent events", "25").option("--type <type>", "Filter by diagnostic event type").option("--since-seq <seq>", "Only include events after this sequence").option("--bundle [path]", "Read a persisted stability bundle instead of calling Gateway; pass \"latest\" for newest").option("--export", "Write a shareable support diagnostics export", false).option("--output <path>", "Diagnostics export output .zip path").action(async (opts, command) => {
		await runGatewayCommand(async () => {
			const { normalizeDiagnosticStabilityQuery, selectDiagnosticStabilitySnapshot } = await import("./diagnostic-stability-LO_W050g.js");
	...
	gateway.command("discover").description("Discover gateways via Bonjour (local + wide-area if configured)").option("--timeout <ms>", "Per-command timeout in ms", "2000").option("--json", "Output JSON", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			const [{ readSourceConfigBestEffort }, { discoverGatewayBeacons }, { resolveWideAreaDiscoveryDomain }, { dedupeBeacons, parseDiscoverTimeoutMs, pickBeaconHost, pickGatewayPort, renderBeaconLines }, { withProgress }] = await Promise.all([
				...
				import("./discover-Bi0nKeEP.js"),
				import("./progress-fJsOMfFV.js")
			]);
