...
import { t as BUNDLED_OFFICIAL_EXTERNAL_PLUGIN_CATALOGS } from "./official-external-plugin-bundled-catalogs-CgQNgVtn.js";
...
async function parseHostedCatalogFeedBody(params) {
	...
	if (params.verification?.mode === "signed") {
		const { verifyOfficialExternalPluginCatalogSignedEnvelope } = await import("./official-external-plugin-catalog-envelope-BAk-GWkU.js");
...
async function resolveHostedCatalogSnapshotStore(params) {
	...
	const { createSqliteHostedOfficialExternalPluginCatalogSnapshotStore } = await import("./official-external-plugin-catalog-snapshot-store-De9uECwj.js");
...
async function loadHostedOfficialExternalPluginCatalogEntries(params) {
	...
	let release;
	try {
		const { fetchWithSsrFGuard } = await import("./fetch-guard-D96SHa2t.js");
