...
import { C as hasSessionActiveAutoModelFallback, T as hasUserPinnedModelSelection } from "./agent-scope-DigoIwHb.js";
...
import { d as loadExactSessionEntryReadOnly, o as listSessionEntriesReadOnly } from "./session-accessor.sqlite-entry-Ik-U-wpI.js";
import { n as deliveryContextFromSession } from "./delivery-context.shared-azPdmUls.js";
import { d as peekSystemEvents } from "./system-events-BVZAS_Ok.js";
import { r as resolveSqliteTargetFromSessionStorePath } from "./session-sqlite-target-10dvR_dO.js";
import { l as resolveSessionTotalTokens, o as resolveFreshSessionTotalTokens } from "./types-gVK8DqPC.js";
...
import { l as toPublicPluginVerificationDiagnostic, o as listActiveDegradedPlugins } from "./runtime-degraded-state-B165q11W.js";
import { h as redactSecretDegradationReason, p as listActiveDegradedSecretOwners } from "./runtime-degraded-state-D5EZZ925.js";
import { t as areRuntimeModelRefsEquivalent } from "./model-runtime-aliases-Rrd1VaX1.js";
import { i as summarizeRetainedLostTaskAuditFindings, r as summarizeActionableTaskAuditFindings } from "./task-registry.audit-BaLk4XcN.js";
...
import { t as hasResolvableHeartbeatOwnerRoute } from "./targets-E-6YuwPm.js";
import { n as resolveHeartbeatSessionKey } from "./heartbeat-runner-session-Cl7bh5EH.js";
//#region src/status/summary.ts
const RECENT_SESSION_LIMIT = 10;
const channelSummaryModuleLoader = createLazyImportLoader(() => import("./channel-summary-C_wo8IWn.js"));
const channelPluginIdsModuleLoader = createLazyImportLoader(() => import("./channel-plugin-ids-C07SuiaL.js"));
const linkChannelModuleLoader = createLazyImportLoader(() => import("./link-channel-VU9idisy.js"));
const taskRegistryMaintenanceModuleLoader = createLazyImportLoader(() => import("./task-registry.maintenance-C5QPxLol.js"));
const staticModelCatalogResolverLoader = createLazyImportLoader(async () => {
	const modelCatalog = await import("./model.static-catalog-BYoJ3n3f.js");
	return {
...
async function getStatusSummary(options = {}) {
	...
	const storeSources = agentList.agents.map((agent) => {
		...
		return {
			agentId: agent.id,
			databasePath: resolveSqliteTargetFromSessionStorePath(storePath, { agentId: agent.id }).path,
			storePath
		};
	});
	...
	const byAgent = await Promise.all(storeSources.map(async ({ agentId, databasePath, storePath }) => {
		const candidates = loadSessionCandidates(storePath, agentId);
		const sessions = await buildSessionRows(selectRecentSessionCandidates(candidates, RECENT_SESSION_LIMIT), { agentIdOverride: agentId });
		return {
			agentId,
			path: databasePath,
	...
	const allSessions = storeSources.filter((source, index, sources) => {
		return sources.findIndex((candidate) => candidate.databasePath === source.databasePath) === index;
	}).flatMap((source) => loadSessionCandidates(source.storePath, pathCounts.get(source.databasePath) === 1 ? source.agentId : void 0));
	const recent = await buildSessionRows(selectRecentSessionCandidates(allSessions, RECENT_SESSION_LIMIT));
	const totalSessions = allSessions.length;
	const hostDesktopStatus = options.hostDesktopStatus ?? (await (await import("./host-source-BRwvqaCU.js")).inspectHostDesktop({ config: cfg.desktop?.host })).status;
	const summary = {
