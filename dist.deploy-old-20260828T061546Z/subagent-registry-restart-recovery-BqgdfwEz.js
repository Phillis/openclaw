...
import { i as isStaleUnendedSubagentRun, s as getSubagentSessionStartedAt } from "./subagent-run-liveness-CpuKir5n.js";
import { o as readSessionMessagesAsync } from "./session-transcript-readers-fCOIrclF.js";
import { r as resolveInternalSessionEffectsTarget } from "./internal-session-effects-DXOpVUug.js";
...
function buildRestartRecoveryResumeMessage(task, lastHumanMessage) {
	const boundContext = (text) => text.length > 2e3 ? `${truncateUtf16Safe(text, 2e3)}...` : text;
...
//#endregion
//#region src/agents/subagents/registry/subagent-registry-restart-recovery-message.ts
function readSubagentRecoveryTranscriptMessage(message) {
	const record = asOptionalRecord(message);
	if (!record) return null;
	const role = typeof record.role === "string" ? record.role : void 0;
	const text = extractAssistantPhaseText(role === "assistant" ? record : { content: Array.isArray(record.content) ? record.content.map((block) => ({
		type: "text",
...
async function recoverInterruptedSubagentRow(params) {
	...
	try {
		...
		const lastHumanMessage = recoveryMessages.toReversed().find((message) => message.role === "user")?.text;
		const configChanged = recoveryMessages.some((message) => message.role === "assistant" && /openclaw\.json|openclaw gateway restart|config\.patch/i.test(message.text ?? ""));
