...
function applyAccountScopedAllowlistConfigEdit(params) {
	...
	for (const path of params.paths.readPaths) {
		...
		hasStoredList = true;
		for (const entry of existingRaw) storedEntries.push(entry);
	}
	const existing = normalizeUniqueStringEntries(hasStoredList ? storedEntries : params.resolveEffectiveEntries?.() ?? []);
