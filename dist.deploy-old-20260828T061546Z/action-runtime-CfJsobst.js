...
import { r as sendDurableMessageBatch } from "./channel-outbound-DO-F9-0m.js";
import { _ as readToolStringParam, g as readStringOrNumberParam, h as readStringArrayParam, m as readReactionParams, p as readPositiveIntegerParam } from "./common-CI1GnPjt.js";
import { t as jsonResult } from "./tool-results-BCM3fdVS.js";
import { f as normalizeMessagePresentation, v as renderMessagePresentationFallbackText } from "./payload-C7E4iMOo.js";
...
import { a as resolveDefaultTelegramAccountId, c as resolveTelegramPollActionGateState, t as createTelegramActionGate } from "./accounts-3yDZGxKI.js";
...
async function describeTelegramAllowedReactionSample(params) {
	const emojis = (await telegramActionRuntime.getTelegramAllowedReactions(params.chatId, {
		cfg: params.cfg,
		token: params.token,
		accountId: params.accountId
	}).catch(() => null))?.filter((reaction) => reaction.type === "emoji").slice(0, TELEGRAM_REACTION_HINT_LIMIT).map((reaction) => reaction.emoji);
...
async function handleTelegramAction(params, cfg, options) {
	...
	if (action === "emoji-list") {
		...
		return jsonResult({
			ok: true,
			emojis: (allowed ?? TELEGRAM_SUPPORTED_REACTION_EMOJI_LIST.map((emoji) => ({
				type: "emoji",
				emoji
			}))).slice(0, limit).map((reaction) => reaction.type === "emoji" ? {
				name: reaction.emoji,
