...
import { l as runSqliteDeferredTransactionSync, u as runSqliteImmediateTransactionSync } from "./node-sqlite-_e3IvfT7.js";
...
import { B as resolveOpenClawAgentSqlitePath, D as ensureOpenClawAgentBoardSchemaInTransaction, F as listOpenClawRegisteredAgentDatabases } from "./openclaw-agent-db-maintenance-_0tYy-zT.js";
...
import { a as BoardValidationError, c as normalizeBoardLayout, i as BOARD_SIZE_PRESETS, o as applyBoardOps, r as normalizeBoardWidgetDeclared, s as insertBoardWidget, t as boardDeclarationIsSubset } from "./board-capabilities-hTT3cLrc.js";
...
function serializeManifest(ownership, declared, grantState, frameAuthority, widgetOptions, nameIdentity) {
	return JSON.stringify({
		...
		...frameAuthority?.kind === "registered" ? { registeredInstanceId: frameAuthority.instanceId } : {}
	});
}
...
function rowToWidget(row, manifest = parseManifest(row.manifest)) {
	...
	const instanceId = row.content_kind === "mcp-app" ? manifest.mcpAppInstanceId : pluginContent && "source" in pluginContent ? manifest.registeredInstanceId : row.view_generation;
	return {
		name: row.name,
		tabId: row.tab_id,
		...row.title !== null ? { title: row.title } : {},
...
var SqliteBoardStore = class {
	...
	readWidgetHtml(sessionKey, name) {
		...
		return row ? rowToHtmlDocument(row) : void 0;
	}
	...
	readWidgetRegistered(sessionKey, name) {
		const row = this.readWidgetRow(sessionKey, name);
		return row ? rowToRegisteredDocument(row) : void 0;
	}
...
function resolveGatewaySessionDatabase(sessionKey) {
	...
	const databasePath = resolveSqliteTargetFromSessionStorePath(resolveSessionStorePathCore(cfg.session?.store, { agentId }), { agentId }).path;
	return {
		agentId,
...
const boardStore = new SqliteBoardStore({ resolveSession: resolveGatewaySessionDatabase });
//#endregion
