...
import { r as stripAnsi$1 } from "./ansi-DjDeieuH.js";
import { s as registerUncaughtExceptionHandler } from "./unhandled-rejections-BjziovQ7.js";
import { n as signalProcessTree } from "./kill-tree-CR2oLt9D.js";
import { t as modelKey } from "./model-key-CMdQNkZf.js";
import { h as resolveSessionAgentId, l as resolveAgentIdByWorkspacePath, x as resolvePersistedSessionStoreOwnerForKey } from "./agent-scope-DigoIwHb.js";
import { n as ok, t as err } from "./result-BQGgYouL.js";
import { n as normalizeAgentId } from "./agent-id-CeT3w4ap.js";
import { C as tryResolveLegacyCompatibilityAgentId, S as tryResolveDefaultAgentId, g as resolveDefaultAgentId } from "./agent-scope-config-CUBiGmG3.js";
import { c as parseAgentSessionKey } from "./session-key-utils-Di3FvABa.js";
import { b as toAgentStoreSessionKey, i as agentSessionKeysMatchByRequestKey, u as normalizeMainKey } from "./session-key-Dbce_H9p.js";
import { n as getRuntimeConfig } from "./io-ClLVsBMp.js";
import { o as setConsoleSubsystemFilter } from "./console-SZn871dT.js";
...
import { g as isAuthErrorMessage, t as classifyFailoverReason } from "./classify-DkuNrlYG.js";
import { i as listThinkingLevelOptions, r as listThinkingLevelLabels } from "./thinking-DLPyZXEW.js";
import { n as shouldForwardModelCommandToServer } from "./commands-registry.data-D1RtQjvy.js";
import { n as listChatCommands, r as listChatCommandsForConfig } from "./commands-registry-list-D8I57510.js";
import { i as resolveTextCommand } from "./commands-registry-normalize-DUo8QOyN.js";
import { t as getProcessSupervisor } from "./supervisor-DmhWHZD2.js";
import "./usage-format-CVbhwZGU.js";
import "./embedded-agent-helpers-NAvzsIRm.js";
import { o as resolveToolDisplay, t as formatToolDetail } from "./tool-display-Dsp8HXJK.js";
...
import { a as isChatStopCommandText } from "./chat-abort-CsMNzOPX.js";
	...
	for (let index = 0; index < url.length; index++) {
		...
		if (ch === "(") open++;
			...
			if (open === 0) {
				...
				return knownUrls?.includes(authoredUrl) ? authoredUrl : trimUrlTrailingPunctuation(authoredUrl, knownUrls);
			}
			open--;
		}
	}
...
function findUrlRanges(visibleText, knownUrls, pending, nextVisibleText) {
	...
	while ((match = urlRe.exec(visibleText)) !== null) {
		...
		if (!found && knownUrls.includes(fragment)) found = true;
		if (!found) {
			let bestLen = 0;
	...
	*/
	smartFilter(query) {
		...
		this.preparedItems ??= this.items.map((item) => {
			const label = stripAnsi$1(this.getItemLabel(item));
			const description = stripAnsi$1(item.description ?? "");
			const searchText = stripAnsi$1(item.searchText ?? "");
			return {
				item,
...
function createCommandHandlers(context) {
	...
	const openModelSelector = async () => {
		...
		try {
			...
			openSelector(createSearchableSelectList(items, 9), (value) => applySessionSetting({ model: value }, `model set to ${value}`, "model set failed"), request);
		} catch (err) {
			if (request !== pickerRequest || !isCurrent()) return;
...
function matchesSelectedTuiSession(state, event, options) {
	...
	if (parsedEvent && parsedSelection && parsedEvent.agentId !== parsedSelection.agentId) return false;
...
function matchesOwnedTuiSession(session, agentId, event) {
	const owner = normalizeLowercaseStringOrEmpty(event.agentId);
	const selected = normalizeLowercaseStringOrEmpty(agentId);
	return agentSessionKeysMatchByRequestKey(event.sessionKey, session) && (parseAgentSessionKey(event.sessionKey)?.agentId || owner) === selected && (!owner || owner === selected);
}
//#endregion
	...
	drainHistoryReloadQueue() {
		...
		const finishReload = (result) => {
			...
			for (const { runId, flags } of reloads) {
				...
				const restoredInFlight = result.loaded && result.runOutcome.state === "active" && result.runOutcome.runId === runId;
...
function createLocalShellRunner(deps) {
	...
	if (!waitForScope) throw new Error("process supervisor must support scope extinction before running local shells");
		...
		try {
			...
			for (const entry of projection.entries) {
				...
				if (message.role === "toolResult") {
					...
					chatLog.startTool(toolCallId, toolName, {}).setResult(state.sessionInfo.verboseLevel === "full" ? {
		...
		const resolve = async (action) => {
			...
			try {
				...
				if (action.kind === "accept") {
					...
					acceptedKey = (await deps.client.acceptTaskSuggestion(suggestion.id, action.mode, action.cloudProfileId)).key;
				} else {
				...
				if (acceptedKey && action.mode !== "session" && matchesSession(suggestion)) await deps.onAccepted(acceptedKey);
			} catch (error) {
				if (disposed) return;
...
function beginTuiShutdown(params) {
	const hardExit = params.setTimeoutFn ? {
		kind: "custom",
		timer: params.setTimeoutFn(params.forceExit, params.hardExitMs)
	} : {
		kind: "native",
		timer: setTimeout(params.forceExit, params.hardExitMs)
	};
	...
	Promise.resolve().then(async () => {
		...
		if (params.keepHardExitArmed !== true) {
			...
			else if (hardExit.kind === "native") clearTimeout(hardExit.timer);
		}
		params.disposeStatus();
...
function scheduleProcessExitAfterTuiReturn(params = {}) {
	...
	const timer = params.setTimeoutFn ? params.setTimeoutFn(onTimeout, delayMs) : setTimeout(onTimeout, delayMs);
	timer.unref?.();
	return timer;
}
...
async function runTuiUnlocked(opts) {
	...
	if (opts.backend) client = opts.backend;
		...
		const { EmbeddedTuiBackend } = await import("./embedded-backend-BIqSvyKy.js");
		client = new EmbeddedTuiBackend();
	} else {
		const { GatewayChatClient } = await import("./gateway-chat-JDAJO_lZ.js");
	...
	import("./tools-manager-DdclqQCN.js").then(({ ensureTool }) => ensureTool("fd", true)).then((fdPath) => {
		if (fdPath) {
	...
	const restoreRememberedSession = async (expectedConnectionGeneration) => {
		...
		if (!sessions || expectedConnectionGeneration !== connectionGeneration || exitRequested) return;
		rememberedSessionApplied = true;
		...
		if (!restored || restored === state.currentSessionKey) return;
	...
	const runAuthFlow = isLocalMode ? async (params) => await withTuiSuspended(async () => {
		...
		return await authChild.spawnAndWait(() => spawn(invocation.command, invocation.args, {
			cwd,
			env: process.env,
			stdio: "inherit",
			...invocation.options
...
//#endregion
export { runTui as C, createEditorSubmitHandler as D, withEmbeddedTuiStateLock as E, createSubmitBurstCoalescer as O, resolveTuiToolsToggleActivityStatus as S, stopTuiSafely as T, resolveTuiCtrlCAction as _, createTuiConnectionLineage as a, resolveTuiSessionSelection as b, installTuiTerminalLossExitHandler as c, isTuiTerminalLossError as d, resolveCodexCliBin as f, resolveLocalAuthSpawnInvocation as g, resolveInitialTuiAgentId as h, createDeferredTuiFinish as i, shouldEnableWindowsGitBashPasteFallback as k, isIgnorableTuiStopError as l, resolveGatewayDisconnectState as m, cancelProcessExitAfterTuiReturn as n, createTuiSignalHandlers as o, resolveCtrlCAction as p, createBackspaceDeduper as r, drainAndStopTuiSafely as s, beginTuiShutdown as t, isTuiBusyActivityStatus as u, resolveTuiLocalAuthCliInvocation as v, scheduleProcessExitAfterTuiReturn as w, resolveTuiShutdownHardExitMs as x, resolveTuiSessionKey as y };
