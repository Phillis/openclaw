...
async function resolveLoader(params) {
	...
	const { loadPublishedPreparedModelCatalogOwnerSnapshot } = await import("./prepared-model-catalog-CiP0KTjK.js");
	return loadPublishedPreparedModelCatalogOwnerSnapshot;
}
async function resetPreparedModelCatalogStateForTest() {
	const [{ resetPreparedModelRuntimeSnapshotsForTest }, { resetModelCatalogBuilderCacheForTest }] = await Promise.all([import("./prepared-model-runtime.test-support-BYyBntIt.js"), import("./model-catalog-B2DbureB.js")]);
...
async function readPreparedGatewayModelCatalog(params) {
	const { getAvailablePreparedModelCatalogSnapshot } = await import("./prepared-model-catalog-CiP0KTjK.js");
...
async function readPreparedGatewayModelCatalogOwnerSnapshot(params) {
	const { getPublishedPreparedModelCatalogOwnerSnapshot } = await import("./prepared-model-catalog-CiP0KTjK.js");
	const config = (params?.getConfig ?? getRuntimeConfig)();
