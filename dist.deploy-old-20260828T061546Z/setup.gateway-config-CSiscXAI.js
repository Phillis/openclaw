...
import { f as validateGatewayPasswordInput, s as normalizeGatewayTokenInput } from "./onboard-helpers-DsV_5p6H.js";
import { t as maskApiKey } from "./secret-mask-BEdLuCrN.js";
import { t as validateDottedDecimalIPv4Input } from "./ipv4-BQKRfag7.js";
import { i as maybeAddTailnetOriginToControlUiAllowedOrigins, n as TAILSCALE_EXPOSURE_OPTIONS } from "./gateway-config-prompts.shared-B3NjWPSs.js";
...
async function configureGatewayForSetup(opts) {
	...
	if (authMode === "token") {
		...
		if (tokenMode === "ref") if (quickstartTokenRef) {
	...
	if (authMode === "password") {
		...
		const quickstartNeedsPasswordRef = flow === "quickstart" && opts.secretInputMode === "ref" && !existingPasswordRef && quickstartGateway.password !== opts.baseConfig.gateway?.auth?.password;
		let password = flow === "quickstart" && !quickstartNeedsPasswordRef ? quickstartGateway.password : existingPasswordRef ?? void 0;
		if (!password) if (await resolveSecretInputModeForEnvSelection({
			...
			let passwordInput;
			if (existingPassword) passwordInput = await prompter.confirm({
				message: t("wizard.gateway.existingPasswordConfirm", { password: maskApiKey(existingPassword) }),
				initialValue: true
			}) ? existingPassword : void 0;
			password = passwordInput ?? normalizeWizardTextInput(await prompter.text({
