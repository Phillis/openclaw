...
import { a as formatDoctorNonInteractiveHint, c as markUpdateRestartSentinelFailure, m as writeRestartSentinel, t as buildRestartSuccessContinuation } from "./restart-sentinel-DedQJXFM.js";
import fs from "node:fs/promises";
//#region src/infra/update-restart-sentinel-payload.ts
function normalizeControlPlaneUpdateResult(result) {
	const beforeSha = result.before?.sha?.trim();
	const afterSha = result.after?.sha?.trim();
	return result.status === "ok" && result.mode === "git" && result.postUpdate?.plugins?.changed !== true && beforeSha && afterSha && beforeSha === afterSha ? {
		...result,
		status: "skipped",
