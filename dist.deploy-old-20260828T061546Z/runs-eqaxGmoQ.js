...
import { c as isAgentEventLifecycleGenerationCurrent, s as getAgentEventLifecycleGeneration } from "./agent-events-CcZImb5w.js";
...
import { O as resolveReplyBackendQueueMessageMismatch, T as forceClearReplyOperation, b as waitForReplyRunEndBySessionId, c as isReplyRunAbortableForCompaction, d as listActiveReplyRunSessionIds, g as resolveActiveReplyRunSessionId, h as resolveActiveReplyOperationForSessionId, i as expireStaleReplyRunBySessionId, k as hasReplyOperationExecutionStarted, l as isReplyRunActiveForSessionId, n as abortReplyRunBySessionId, t as abortActiveReplyRuns, u as isReplyRunEvidenceStaleBySessionId, v as supersedeReplyRunByRunId, y as waitForReplyOperationOwnerSettlement } from "./reply-run-registry-Ch9Ye6re.js";
import { c as isDiagnosticEmbeddedRunOwnerClosed, d as markDiagnosticEmbeddedRunStarted, m as resolveRunStaleThresholdMs, s as getDiagnosticSessionActivitySnapshot, u as markDiagnosticEmbeddedRunEnded } from "./diagnostic-run-activity-CxbnPTtN.js";
import { a as logMessageQueuedWithBacklogPolicy, t as diagnosticLogger } from "./diagnostic-runtime-IUeGlWCe.js";
import { o as logSessionStateChange } from "./diagnostic-DOfHG5L4.js";
import { r as resolveSessionPlacementForcedTerminalSettlement } from "./session-placement-admission-CIMNOxmd.js";
import { _ as setActiveEmbeddedRunLifecycleGeneration, a as ACTIVE_EMBEDDED_RUNS_BY_RUN_ID, c as ACTIVE_EMBEDDED_RUN_SESSION_IDS_BY_KEY, d as EMBEDDED_RUN_WAITERS, f as RETAINED_EMBEDDED_RUN_ABORTABILITY_RUN_IDS, i as ACTIVE_EMBEDDED_RUNS, l as ACTIVE_EMBEDDED_RUN_SNAPSHOTS, n as ABANDONED_EMBEDDED_RUN_SESSION_IDS_BY_FILE, o as ACTIVE_EMBEDDED_RUN_REGISTRATIONS, r as ABANDONED_EMBEDDED_RUN_SESSION_IDS_BY_KEY, s as ACTIVE_EMBEDDED_RUN_SESSION_IDS_BY_FILE, t as ABANDONED_EMBEDDED_RUNS_BY_SESSION_ID, u as EMBEDDED_RUN_FORCED_TERMINAL_SETTLEMENTS } from "./run-state-CmAt4u6E.js";
...
function queueEmbeddedAgentMessageWithOutcome(sessionId, text, options) {
	...
	prepared.queueMessage(text, options ?? { steeringMode: "all" }).catch((err) => {
...
function resolveEmbeddedQueueMessage(sessionId, handle) {
	try {
		const injection = handle.messageInjection;
		if (injection) return injection.isAvailable() ? (text, options) => injection.queueMessage(text, options) : void 0;
		return (handle.isStopped ? !handle.isStopped() : handle.isStreaming()) ? (text, options) => handle.queueMessage(text, options) : void 0;
	} catch (err) {
...
async function queueEmbeddedAgentMessageWithOutcomeAsync(sessionId, text, options) {
	...
	if (prepared.kind === "complete") {
		const activeToolAuthorityFingerprint = normalizeOptionalString(ACTIVE_EMBEDDED_RUNS.get(sessionId)?.toolAuthorityFingerprint);
		const pendingInputAuthorityProven = Boolean(activeToolAuthorityFingerprint && (normalizeOptionalString(options?.toolAuthorityFingerprint) === activeToolAuthorityFingerprint || normalizeOptionalString(options?.pendingInputAuthorityFingerprint) === activeToolAuthorityFingerprint));
		if (!prepared.outcome.queued && (prepared.outcome.reason === "tool_authority_mismatch" || prepared.outcome.reason === "image_input_unsupported") && options?.isInboundUserMessage === true && options.images?.length && pendingInputAuthorityProven) try {
		...
		if (!prepared.outcome.queued && prepared.outcome.reason === "tool_authority_mismatch" && options?.isInboundUserMessage === true && !options.images?.length && pendingInputAuthorityProven) {
			const claimPendingUserInputAnswer = ACTIVE_EMBEDDED_RUNS.get(sessionId)?.claimPendingUserInputAnswer;
	...
	try {
		...
		if (queueResult?.transcriptCommit === "unconfirmed") {
			diagnosticLogger.warn(`queue message accepted without transcript confirmation: sessionId=${sessionId} err=${queueResult.errorMessage}`);
			logActiveRunMessageAccepted(sessionId);
			return {
				queued: true,
				sessionId,
...
function prepareEmbeddedAgentQueueMessage(sessionId, options) {
	...
	const deliveryModeMismatch = resolveReplyBackendQueueMessageMismatch(handle, options, resolveActiveReplyOperationForSessionId(sessionId));
...
function resolveEmbeddedAgentRunProgressState(sessionId) {
	...
	if (isEmbeddedRunHandleInProgress(ACTIVE_EMBEDDED_RUNS.get(sessionId)) || replyInProgress && replyOperation && hasReplyOperationExecutionStarted(replyOperation)) return "running";
...
//#endregion
export { waitForEmbeddedAgentRunEnd as A, resolveActiveEmbeddedRunSessionIdBySessionFile as C, setActiveEmbeddedRun as D, retainEmbeddedAgentRunAbortabilityForRunId as E, supersedeEmbeddedAgentRunByRunId as O, resolveActiveEmbeddedRunOwnerByRunId as S, resolveEmbeddedReplyActivity as T, queueEmbeddedAgentMessageWithOutcome as _, formatEmbeddedAgentQueueFailureSummary as a, resolveActiveEmbeddedRunHandleSessionIdBySessionFile as b, isEmbeddedAgentRunAbortableForCompaction as c, isEmbeddedAgentRunHandleActive as d, isEmbeddedAgentRunInProgress as f, preemptAndDrainEmbeddedHeartbeatRun as g, markActiveEmbeddedRunAbandoned as h, clearEmbeddedAgentRunAbortabilityForRunId as i, updateActiveEmbeddedRunSnapshot as k, isEmbeddedAgentRunAbortableForRunId as l, isEmbeddedRunAbandoned as m, abortEmbeddedAgentRun as n, getActiveEmbeddedRunSnapshot as o, isEmbeddedAgentRunStreaming as p, clearActiveEmbeddedRun as r, isActiveEmbeddedRunId as s, abortAndDrainEmbeddedAgentRun as t, isEmbeddedAgentRunActive as u, queueEmbeddedAgentMessageWithOutcomeAsync as v, resolveEmbeddedAgentRunProgressState as w, resolveActiveEmbeddedRunOwner as x, resolveActiveEmbeddedRunHandleSessionId as y };
