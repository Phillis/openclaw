...
import { f as upsertAuthProfileWithLockOrThrow } from "./profiles-FGrQtdwI.js";
import { n as suggestOAuthProfileIdForLegacyDefault } from "./repair-B_GWNYPm.js";
...
import { t as applyAuthProfileConfig } from "./provider-auth-helpers-Ci8FjjB5.js";
...
import { a as buildProviderReplayFamilyHooks, c as modelCostsEqual, p as cloneFirstTemplateModel } from "./provider-model-shared-CF2CrQqB.js";
import "./cli-runtime-BSWt0Rlh.js";
import "./provider-auth-api-key-BtzdBBqf.js";
import { i as buildOpenAICompatibleProviderCatalog } from "./provider-catalog-live-runtime-BWjq50pi.js";
import { n as buildManifestModelProviderConfig } from "./provider-catalog-shared-__IiTHyX.js";
import { c as CLAUDE_CLI_OFF_THINKING_PROFILE, i as CLAUDE_CLI_DEFAULT_ALLOWLIST_REFS, l as CLAUDE_CLI_PROFILE_ID, n as CLAUDE_CLI_CANONICAL_DEFAULT_MODEL_REF, t as CLAUDE_CLI_BACKEND_ID } from "./cli-constants-Djv4WtLq.js";
import { t as probeClaudeCliAuthStatus } from "./cli-auth-seam-B8D2x_9U.js";
import { t as buildClaudeCliCatalogEntries } from "./cli-catalog-CDqq6b_1.js";
import { o as supportsClaudeDynamicSystemPromptSections } from "./cli-shared-B3PGSlcB.js";
...
import { t as acceptsAnthropicLiveModelContract } from "./live-model-contract-gate-D-4bBvNM.js";
import { t as anthropicMediaUnderstandingProvider } from "./media-understanding-provider-szAhASH3.js";
import { n as resolveClaudeCliSyntheticAuth } from "./provider-discovery-Ci3VWwkL.js";
import { n as registerClaudeSessionDiscovery, t as createClaudeSessionNodeInvokePolicies } from "./session-catalog-registration-DDCXb4V9.js";
import { c as wrapAnthropicProviderStream, i as isAnthropicOAuthApiKey } from "./stream-wrappers-CCu0qwyR.js";
import { n as resolveAnthropicUsageAuth, t as fetchAnthropicUsage } from "./usage-DTwlzqFg.js";
...
function buildAnthropicForwardCompatModel(ctx) {
	...
	return {
		...
		...supportsClaudeNativeXhighEffort({ id: trimmedModelId }) ? { thinkingLevelMap: {
			...isAnthropicMandatoryClaude5Model(trimmedModelId) ? { minimal: "low" } : {},
			xhigh: "xhigh",
			max: "max"
...
async function runAnthropicCliMigration(ctx) {
	if (probeClaudeCliAuthStatus(resolveAnthropicCliAuthProbe(ctx.env ?? process.env)).status !== "available") throw new Error(["Claude CLI is not authenticated on this host.", `Run ${formatCliCommand("claude auth login")} first, then re-run this setup.`].join("\n"));
	return buildAnthropicCliMigrationResult(ctx.config);
}
async function runAnthropicCliMigrationNonInteractive(ctx) {
	const authStatus = probeClaudeCliAuthStatus(resolveAnthropicCliAuthProbe(process.env));
	if (authStatus.status !== "available") {
		const error = authStatus.status === "unreadable" ? ["Auth choice \"anthropic-cli\" could not verify the installed Claude CLI login.", `Run ${formatCliCommand("claude auth status")}, then retry.`] : ["Auth choice \"anthropic-cli\" requires Claude CLI auth on this host.", `Run ${formatCliCommand("claude auth login")} first.`];
...
function resolveAnthropicCliAuthProbe(env) {
	const backend = buildAnthropicCliBackend().config;
	const probeEnv = {
		...env,
		...backend.env
	};
	for (const name of backend.clearEnv ?? []) delete probeEnv[name];
	return {
...
function buildAnthropicProvider() {
	...
	return {
		...
		resolveSyntheticAuth: ({ config, provider }) => normalizeLowercaseStringOrEmpty(provider) === "claude-cli" ? resolveClaudeCliSyntheticAuth(config) : void 0,
...
function registerAnthropicPlugin(api) {
	...
	const dynamicSystemPromptSectionsProbe = (async () => {
		try {
			...
			supportsDynamicSystemPromptSections = result?.code === 0 && supportsClaudeDynamicSystemPromptSections(result.stdout);
		} catch {
