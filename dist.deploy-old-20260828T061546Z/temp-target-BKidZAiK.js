...
import { a as sanitizeSafePathSegment, r as assertSafePathSegment } from "./safe-path-segment-UYsB1OSq.js";
...
async function cleanupTempDir(dir, identity, onCleanupError) {
	try {
		const current = await fs$1.lstat(dir, { bigint: true }).catch((error) => {
			if (isNodeErrorWithCode(error, "ENOENT")) return;
			throw error;
		});
		if (!current || !sameFileIdentityForCleanup(current, identity)) return;
		await fs$1.rm(dir, {
			recursive: true,
			force: true
		});
...
async function tempFile(params) {
	...
	const dir = await fs$1.mkdtemp(path.join(rootDir, prefix));
	const identity = await fs$1.lstat(dir, { bigint: true });
