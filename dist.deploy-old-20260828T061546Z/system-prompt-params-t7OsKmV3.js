...
import { d as normalizeStringEntriesLower, m as normalizeUniqueStringEntries, u as normalizeStringEntries } from "./string-normalization-e_fvmxMf.js";
import { t as pruneMapToMaxSize } from "./map-size-DAGm21RM.js";
import { a as isSubagentSessionKey, l as parseCronRunScopeSuffix, n as isAcpSessionKey } from "./session-key-utils-Di3FvABa.js";
import { dt as resolveOwnerDisplaySetting, ft as resolveOwnerPromptNumbers } from "./io-ClLVsBMp.js";
import { t as CHANNEL_IDS } from "./ids-Cgp0iV_A.js";
import { n as findGitRoot } from "./git-commit-Bbf_PCaF.js";
...
import { u as isOpenClawMainPromptSurface } from "./command-registration-C3uxM4em.js";
import { t as buildPromisedWorkPromptSection } from "./promised-work-prompt-B5kR4qTd.js";
import { n as resolveEffectiveToolFsWorkspaceOnly } from "./tool-fs-policy-DwrFWb3k.js";
import { i as buildTtsSystemPromptHint } from "./tts-settings-BPz5yC2d.js";
import { i as isKnownNativeApprovalPromptChannel, r as hasNativeApprovalPromptRuntimeCapability } from "./native-approval-prompt-DzwFZJrJ.js";
import { t as isAutomaticMemoryEntryEligible } from "./types-BumKP00u.js";
import { a as getMemoryRuntime, t as buildMemoryPromptSection } from "./memory-state-BzIc97qw.js";
...
import { a as buildDelegationGuidanceSection, i as buildSkillWorkshopPromptSection, o as resolveMainSessionDelegationMode, t as buildWatchedSessionsPromptLines } from "./watched-sessions-prompt-CQ4_7B2D.js";
import { h as splitCuratedMarkdownEntries } from "./internal-y_9W5i9a.js";
import { i as resolveUserTimezone, t as formatDateStamp } from "./date-time-Ch20W-k8.js";
import { i as stripMemoryAnnotationCarriers, n as extractProjectKeysFromCuratedEntry, r as normalizeProjectAnnotationKey } from "./curated-annotations-mTWgerpx.js";
import "./engine-storage-BWafvbUP.js";
...
function buildMessagingSection(params) {
	...
	if (params.isMinimal) return messageToolOnly ? [
		"## Messaging",
	...
	const subagentOrchestrationGuidance = params.delegationSectionRenders ? "" : hasSessionsSpawn ? hasSubagents ? `- Subagents: \`sessions_spawn\` with objective/output/write-scope/verification; stable handle needs \`taskName\`, UI title \`label\`; isolated omits \`context\`, transcript needs \`context:"fork"\`; ${hasSessionsYield ? "wait via `sessions_yield`; " : ""}\`subagents(action=list)\` only status/debug.` : `- Subagents: \`sessions_spawn\` with objective/output/write-scope/verification; stable handle needs \`taskName\`, UI title \`label\`; isolated omits \`context\`, transcript needs \`context:"fork"\`${hasSessionsYield ? "; wait via `sessions_yield`" : ""}.` : hasSubagents ? "- Subagents: `subagents(action=list)` only for status/debug visibility." : "";
	return [
		"## Messaging",
		visibleReplyInstruction,
...
function buildDocsSection(params) {
	...
	return [
		...
		params.hasGateway ? "Config field: `gateway(config.schema.lookup)` exact path. Broader: `docs/gateway/configuration.md`, `docs/gateway/configuration-reference.md`." : "Configuration docs: `docs/gateway/configuration.md`, `docs/gateway/configuration-reference.md`.",
...
function buildAgentSystemPrompt(params) {
	...
	const availableTools = /* @__PURE__ */ new Set([...normalizeStringEntriesLower(params.toolNames), ...normalizeStringEntriesLower(params.capabilityToolNames)]);
	const coreToolSummaries = {
		...
		ls: "List directories",
		exec: params.codeModeActive ? "Run JavaScript/TypeScript Code Mode; call exact catalog tools from code, never shell/Python/imports" : promptSurface === "cli_backend" ? "Run shell on connected node; sync; host=node" : "Run shell; pty for TTY CLIs",
		...
		sessions_spawn: acpSpawnRuntimeEnabled ? `Spawn isolated subagent/ACP. Transcript needed: context="fork". ACP needs agentId unless default; ids from acp.allowedAgents${availableTools.has("agents_list") ? ", not agents_list" : ""}.` : "Spawn isolated subagent; transcript needed: context=\"fork\"",
	...
	const lines = [cacheStablePromptPrefix(hashStablePromptInput({
		...
		const lines = [
			...
			...acpHarnessSpawnAllowed ? [
				...
				...threadBoundAcpSpawnEnabled ? ["ACP thread: only `sessions_spawn(runtime:\"acp\", thread:true)`; never create a messaging thread for it."] : []
			] : [],
			...renderOpenClawToolWorkflowHints && subagentStatusTools.length > 0 ? [`Never loop-poll ${subagentStatusTools.map((name) => name === "subagents" ? "`subagents list`" : `\`${name}\``).join("/")}.${availableTools.has("sessions_yield") ? " Wait with `sessions_yield`." : ""} Status only on-demand/intervention/debug/request.`] : [],
			...
			...hasOpenClaw ? ["Gateway restart, config, channels, plugins, agents, models/providers, updates: ask `openclaw`. Never restart the Gateway through shell commands or write your own config."] : hasGateway ? ["Config read: `gateway` (`config.get|config.schema.lookup`). Write/restart unavailable; ask human."] : ["System controls unavailable; ask human."],
			"",
			...skillsSection,
	...
	lines.push(...providerSectionOverrides.tool_call_style || !hasExec ? [] : [buildExecApprovalPromptGuidance({
	...
	lines.push("## Runtime", buildRuntimeLine(runtimeInfo, runtimeChannel, runtimeCapabilities, params.defaultThinkLevel), ...modelIdentityLine ? [modelIdentityLine] : [], ...hasProcess ? buildActiveProcessSessionReferenceLines(runtimeInfo?.activeProcessSessions) : [], `Reasoning=${reasoningLevel}; hidden unless on/stream. Toggle /reasoning; /status shows when enabled.`);
	return lines.filter(Boolean).join("\n");
}
...
function resolveAgentSystemPromptConfig(params) {
	...
	return {
		...
		ttsHint: config ? buildTtsSystemPromptHint(config, agentId, { messageToolOnly: sourceReplyDeliveryMode === "message_tool_only" }) : void 0,
...
//#endregion
export { buildModelIdentityPromptLine as a, prepareProjectMemoryBootstrap as c, appendModelIdentitySystemPrompt as i, buildFullBootstrapPromptLines as l, resolveSystemPromptRepoRoot as n, resolveAgentPromptSurfaceForSessionKey as o, buildConfiguredAgentSystemPrompt as r, buildProjectMemoryWriteInstruction as s, buildSystemPromptParams as t, buildLimitedBootstrapPromptLines as u };
