...
import { r as formatSystemAgentStartupMessage } from "./overview-CefSkUmi.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-BnR7bfUE.js";
import { g as setCommandLaneConcurrency, r as enqueueCommandInLane } from "./command-queue-CBS1Vl32.js";
import { n as describeSystemAgentPersistentOperation } from "./operations-Djud2UfI.js";
import { n as isSystemAgentInferenceUnavailableError } from "./inference-error-MxT_vZPs.js";
import { f as isGatewayClientProfilePending, l as authenticatedProfileUnavailableError } from "./operator-role-policy-il7s4lXY.js";
...
import { i as buildRequestedApprovalEvent, p as listVisiblePendingApprovalRequests, s as handlePendingApprovalRequest } from "./approval-shared-D87k2z1-.js";
import { t as getUpdateAvailable } from "./update-startup-Dv-J8vQJ.js";
import { n as getHealthCache } from "./health-state-BzKAU1CD.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-BdQmRC9i.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = await createAdmittedWizardSession(() => new WizardSession(async (prompter, signal, runnerSession) => {
			await runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-8j3Y-N3C.js"), import("./setup.shared-BmxuNf4x.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-juDOhPGx.js");
