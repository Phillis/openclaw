...
async function resolveRouteRpcOptions(args, deps) {
	...
	const config = await (deps.readNonObservingHealthConfig ?? (await import("./health-CK080YwI.js")).readNonObservingHealthConfig)();
	return {
		...args.rpc,
...
async function runGatewayHealthJsonRoute(args, runtime, deps = {}) {
	...
	try {
		...
		writeRuntimeJson(runtime, await (deps.callGateway ?? (await import("./gateway-rpc-DFNkEQLq.js")).callGatewayFromCliWithTransport)("health", rpc, void 0, {
			defaultTimeoutMs: 1e4,
			sharedStateMode: "read-only"
		}));
	} catch (error) {
		if (!rpc) throw error;
		const [healthModule, callModule] = await Promise.all([deps.emitReachableGatewayAuthDiagnostic && deps.readNonObservingHealthConfig ? void 0 : import("./health-CK080YwI.js"), deps.formatGatewayAuthErrorJson && deps.formatGatewayClientRequestErrorJson && deps.formatGatewayTransportErrorJson ? void 0 : import("./call-DPYKD0iw.js")]);
