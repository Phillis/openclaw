...
import { S as getTaskById, T as listTaskRecordPage } from "./task-registry-aynazQHF.js";
import "./runtime-internal-C7MuMy9Z.js";
import { n as resolveRequestedSessionAgentId } from "./session-request-agent-C9E8iDY4.js";
import { i as retrySubagentCompletionDelivery, n as dismissSubagentCompletionDelivery } from "./subagent-completion-delivery-esPjci0g.js";
import { t as assertValidParams } from "./validation-kYFXohur.js";
import { n as canAccessTaskRequesterSession, t as mapTaskSummary } from "./task-summary-wQfFTZ7S.js";
...
const tasksHandlers = {
	...
	"tasks.dismiss": async ({ params, respond, context, client }) => {
		...
		const { discardSubagentTerminalDelivery } = await import("./subagent-registry-36rPN0fA.js");
		const results = [];
		const cfg = context.getRuntimeConfig();
		for (const taskId of params.taskIds) {
			const task = getTaskById(taskId);
