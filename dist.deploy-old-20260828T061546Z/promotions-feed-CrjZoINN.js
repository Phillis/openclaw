...
import { Nn as getNodeSqliteKysely, d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, jn as executeSqliteQuerySync } from "./openclaw-state-db-kmBThqu6.js";
import { _ as requestClawHub, a as fetchClawHubJson, d as readClawHubStringArrayField, f as readClawHubStringField, g as readRequiredClawHubStringField, i as decodeClawHubResponseBody, l as readClawHubBooleanField, m as readRequiredClawHubNumberField, p as readRequiredClawHubBooleanField, r as createClawHubError, t as CLAWHUB_JSON_MAX_BYTES, u as readClawHubBytes } from "./clawhub-client-Cjweitq0.js";
...
function readPromotionsFeedStateWithMetadata() {
	try {
		const stored = readConfigMachineState(PROMOTIONS_FEED_STATE_KEY);
		if (!stored) return {
			state: {
				...EMPTY_STATE,
				notifiedSlugs: /* @__PURE__ */ new Set()
		...
		return {
			state: {
				...
				...!payloadInvalid && typeof stored.sequence === "number" ? { sequence: stored.sequence } : {},
				...!payloadInvalid && expiresAtMs !== void 0 ? { expiresAtMs } : {},
				entries,
				...typeof stored.lastCheckedAtMs === "number" ? { lastCheckedAtMs: stored.lastCheckedAtMs } : {},
...
function writePromotionsFeedState(params) {
	updateConfigMachineState(PROMOTIONS_FEED_STATE_KEY, (existing) => ({
		etag: params.etag === void 0 ? existing?.etag ?? null : params.etag,
		payloadJson: params.payloadJson === void 0 ? existing?.payloadJson ?? null : params.payloadJson,
		sequence: params.sequence === void 0 ? existing?.sequence ?? null : params.sequence,
		lastCheckedAtMs: params.lastCheckedAtMs ?? existing?.lastCheckedAtMs ?? null,
		notifiedSlugs: params.notifiedSlugs ? [.../* @__PURE__ */ new Set([...existing?.notifiedSlugs ?? [], ...params.notifiedSlugs])].toSorted() : existing?.notifiedSlugs ?? []
	}));
}
