		...
		for (const comp of payload.components) {
			...
			if (componentType) {
				...
				if (componentApi) {
					...
					if (!validationResult.success) throw new A2uiValidationError(`Validation failed for component '${componentType}' (${id}): ${validationResult.error.errors.map((err) => `${err.path.join(".") || "root"}: ${err.message}`).join(", ")}`);
				}
			}
		}
		for (const comp of payload.components) {
...
const CheckableSchema = objectType$1({
	"checks": arrayType$1(objectType$1({
	...
	"isValid": booleanType$1().optional().describe("Whether the checks currently pass."),
	"validationErrors": arrayType$1(stringType$1()).optional().describe("Current validation error messages.")
