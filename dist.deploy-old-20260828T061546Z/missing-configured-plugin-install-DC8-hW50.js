...
import { l as readLegacyNpmPluginDeclaration } from "./discovery-KmR2BWJK.js";
import { c as resolveUserPath } from "./home-dir-BFvskzn8.js";
import "./utils-Bw16L5tB.js";
import { r as stripAnsi } from "./ansi-DjDeieuH.js";
import { n as sanitizeTerminalText } from "./safe-text-DbwznzfG.js";
import { n as VERSION, s as resolveCompatibilityHostVersion } from "./version-CkBmshxX.js";
import { i as loadInstalledPluginIndex } from "./installed-plugin-index-Cr71VmpU.js";
import { i as isOpenClawOrgNpmSpec, s as parseRegistryNpmSpec, t as compareOpenClawReleaseVersions } from "./npm-registry-spec-BdgyvSs0.js";
import { a as resolveDefaultPluginExtensionsDir, c as resolvePluginInstallDir, d as resolvePluginNpmPackageDir, s as resolveDefaultPluginNpmDir } from "./install-paths-DllFtsSG.js";
import { t as parseClawHubPluginSpec } from "./clawhub-spec-Er3Np6VI.js";
import { C as resolveOfficialExternalProviderPluginIdsForEnv, S as resolveOfficialExternalProviderPluginIds, _ as resolveOfficialExternalPluginId, f as listOfficialExternalPluginCatalogEntries, v as resolveOfficialExternalPluginInstall, w as resolveOfficialExternalWebProviderContractPluginIdsForEnv, x as resolveOfficialExternalProviderContractPluginIds, y as resolveOfficialExternalPluginLabel } from "./official-external-plugin-catalog-C1KgYx9P.js";
...
import { h as collectConfiguredMemoryEmbeddingProviderIds } from "./gateway-startup-plugin-ids-Dy6KWM9Y.js";
import { u as resolveConfiguredChannelPresencePolicy } from "./channel-presence-policy-oJ8soWzX.js";
...
import { n as resolveWebSearchInstallCatalogEntriesForEnv, r as resolveWebSearchInstallCatalogEntry } from "./web-search-install-catalog-DjERh4Zq.js";
import { l as buildNpmResolutionInstallFields, p as resolveNpmInstallRecordSpec, s as writePersistedInstalledPluginIndexInstallRecords } from "./installed-plugin-index-records-CHK-Mu2-.js";
import { t as withPluginLifecycleLease } from "./plugin-lifecycle-lease-QVxAzcU7.js";
import "./channel-plugin-ids-CD1dMDba.js";
import { n as resolveProviderInstallCatalogEntries } from "./provider-install-catalog-BvC2DHvQ.js";
...
import { r as listRawChannelPluginCatalogEntries } from "./catalog-BxGnlKhy.js";
import { n as updateNpmInstalledPlugins, r as isClawHubTrustSkippedOutcome } from "./update-BxTnzDTG.js";
import { i as collectConfiguredRuntimePluginIds, n as VERSION_BOUND_RUNTIME_PLUGIN_IDS, t as CONFIGURED_RUNTIME_PLUGIN_INSTALL_CANDIDATES } from "./configured-runtime-plugin-installs-Dw0dwCZZ.js";
import { c as isLegacyPackageUpdateDoctorPass, f as shouldDeferConfiguredPluginInstallRepair, u as isPostCoreConvergencePass } from "./update-phase-CUQAfBmE.js";
import { n as collectConfiguredModelProviderSelectionIds, r as collectConfiguredProviderSelectionIds, t as collectConfiguredMediaProviderSelectionIds } from "./configured-provider-selection-ids-CTLewJx9.js";
...
function collectConfiguredPluginIds(cfg, env) {
	...
	for (const [pluginId, entry] of Object.entries(entries ?? {})) {
		if (asNullableRecord(entry)?.enabled === false) continue;
...
async function resolveConfiguredPluginInstallContext(params) {
	...
	const knownIds = /* @__PURE__ */ new Set([...snapshot.plugins.filter((plugin) => plugin.origin !== "bundled").map((plugin) => plugin.id), ...currentBundledPlugins.map((plugin) => plugin.pluginId)]);
	...
	const bundledPluginsById = new Map(currentBundledPlugins.map((plugin) => [plugin.pluginId, { packageName: plugin.packageName }]));
...
function collectDownloadableInstallCandidates(params) {
	...
	for (const entry of CONFIGURED_RUNTIME_PLUGIN_INSTALL_CANDIDATES) {
		...
		if (existing && entry.versionBoundToOpenClaw) candidates.set(entry.pluginId, {
			...existing,
...
async function installCandidatePackage(params) {
	...
	const prepareConsent = (source, spec) => prepareManagedPluginArtifactConsentHandler({
	...
	if (existingNpmPackagePath && existingNpmPackageVersion && npmInstallSpec && params.mode !== "update" && isPostCoreConvergencePass(params.env)) {
		...
		const { result: clawhubResult, capabilityConsent } = await installWithChannelFallback({
			...
			install: async (spec) => {
				...
				return {
					...
					capabilityConsent: attemptConsent
				};
			},
	...
	const runNpmInstall = async (spec, mode) => {
	...
	const installOnce = async (spec) => {
		const attempt = await runNpmInstall(spec, npmInstallMode);
		return !attempt.result.ok && npmInstallMode === "install" && isPluginAlreadyExistsError(attempt.result.error) ? await runNpmInstall(spec, "update") : attempt;
	};
...
async function adoptExistingNpmPackage(params) {
	...
	return {
		records: {
			...params.records,
			[params.candidate.pluginId]: params.capabilityConsent.applyAcceptedSurface(params.candidate.pluginId, {
				source: "npm",
				spec: resolveNpmInstallRecordSpec({
...
async function repairMissingPluginInstalls(params) {
	return await withPluginLifecycleLease({ env: params.env }, () => repairMissingPluginInstallsWithLease(params));
}
async function repairMissingPluginInstallsWithLease(params) {
	...
	for (const candidate of collectDownloadableInstallCandidates({
		...
		if (shouldReplaceBrokenOfficialInstall) {
			...
			if (installed.records !== previousRecords && removalPath && (!installedRecord?.installPath || !installPathsEqual(resolveUserPath(installedRecord.installPath, env), removalPath))) try {
