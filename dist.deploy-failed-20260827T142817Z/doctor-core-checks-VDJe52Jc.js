...
import { r as listAgentEntries, x as tryResolveSoleAgentId } from "./agent-scope-config-BdXMWufB.js";
...
import { i as noteChromeMcpBrowserReadiness, n as maybeArchiveLegacyClawdBrowserProfileResidue, r as maybeRepairOwnedChromeExtensionNativeHosts, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-CcmVQOOI.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-m3bdeqFH.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-C_1PLcc5.js";
import { n as disableUnavailableSkillsInConfig, r as formatMissingSkillSummary } from "./doctor-skills-core-DZQVCoNO.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-CsFRfPqY.js";
import { a as collectDisabledCodexPluginRouteIssues, n as collectCodexRuntimeCompatibilityWarnings } from "./codex-route-warnings-BRCw6THo.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-B6nnj48Q.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-B0_1fKrb.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-Be6BYsXQ.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-DlxY9PyP.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-BTnmJjzg.js");
...
const legacyStateCheck = {
	...
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-D_nuZZMd.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		if (!ctx.cwd) return [];
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget } = await import("./bootstrap-budget-DLcjrrTk.js");
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-CgtIfibT.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-wr3iDt3H.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./provider-openai-chatgpt-oauth-tls-Dth049vJ.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	defaultEnabled: false,
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-PS_5h5V_.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-PS_5h5V_.js");
...
const telegramGeneralTopicConversationsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectTelegramGeneralTopicConversationRepairs } = await import("./doctor-telegram-general-topic-conversations-eXBLdSsE.js");
	...
	async repair(ctx) {
		const { repairTelegramGeneralTopicConversations } = await import("./doctor-telegram-general-topic-conversations-eXBLdSsE.js");
		const effect = {
			kind: "state",
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-DSNdsH-9.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-DSNdsH-9.js");
...
const gatewayPlatformNotesCheck = {
	...
	async detect(ctx) {
		...
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-DzZOOSEG.js");
...
function createConvertedWorkflowChecks(deps) {
	return [
		...
		...isExperimentalClawsEnabled() ? [{
			...
			source: "doctor",
			async detect(ctx) {
				const [{ collectClawStateHealthFindings }, { listConfiguredMcpServers }] = await Promise.all([import("./doctor-wVmTns25.js"), import("./mcp-config-6MMh8wNY.js")]);
