...
import { g as openOpenClawAgentDatabase } from "./openclaw-agent-db-CyHApqW_.js";
import { v as ensureContextEngineTurnOutboxSchema } from "./openclaw-agent-db-maintenance-1xIPEKIN.js";
import { u as resolveLogicalTurnContextEngines } from "./registry-DOMV71ew.js";
import { E as resolveSessionTranscriptDatabasePath, S as readClosedTranscriptTurn } from "./session-accessor-CVnxp3UM.js";
...
async function drainContextEngineTurnOutbox(params) {
	...
	const db = outboxDb(params.database);
	let pendingSessionsQuery = db.selectFrom("context_engine_turn_outbox").select("session_id").select(oldestOutboxEnqueueSequence().as("oldest_enqueue_sequence")).where("engine_id", "=", params.engineId).where("owner_plugin_id", params.ownerPluginId ? "=" : "is", params.ownerPluginId ?? null);
	...
	while (remaining > 0 && activeSessionIds.length > 0) {
		...
		for (const sessionId of activeSessionIds) {
			...
			const row = executeSqliteQueryTakeFirstSync(params.database.db, db.selectFrom("context_engine_turn_outbox").select([
				...
				"payload_json",
				"session_id"
			]).where("engine_id", "=", params.engineId).where("owner_plugin_id", params.ownerPluginId ? "=" : "is", params.ownerPluginId ?? null).where("session_id", "=", sessionId).orderBy(outboxEnqueueSequence(), "asc").limit(1));
			if (!row) continue;
			remaining -= 1;
...
function hasPendingContextEngineTurn(params) {
	let query = outboxDb(params.database).selectFrom("context_engine_turn_outbox").select("advancement_key").where("engine_id", "=", params.engineId).where("owner_plugin_id", params.ownerPluginId ? "=" : "is", params.ownerPluginId ?? null);
...
function resolveDurableTurnRuntimeSettings(params) {
	if (params.facts.runtimeSettings) return params.facts.runtimeSettings;
	const contextEngineHost = params.facts.contextEngineHostSupport;
	if (!contextEngineHost) return;
