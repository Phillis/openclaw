...
import { r as flushLogger } from "./logger-BWBYvpHz.js";
import { o as setConsoleSubsystemFilter, s as setConsoleTimestampPrefix } from "./console-Dqa67THW.js";
import { t as createSubsystemLogger } from "./subsystem-CDLhGl2-.js";
import { l as hasConfiguredSecretInput } from "./types.secrets-BrIfhxSG.js";
import { j as findOpenClawStateDatabaseSchemaMigrationRequiredError } from "./openclaw-state-db-CXrhNigN.js";
import { p as ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV } from "./config-env-vars-BITg3hPR.js";
import { r as CONFIG_AUDIT_STORE_LABEL } from "./io.audit-CQ4YEkdI.js";
import { t as clearRuntimeConfigSnapshot } from "./runtime-snapshot-Dp7mvsA3.js";
import { i as isInvalidConfigError, r as isDoctorRecoverableInvalidConfigError } from "./io.invalid-config-Ghwrjj2j.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-EiOnxvh_.js";
...
import { n as formatGatewayPidList, t as findVerifiedGatewayListenerPidsOnPortSync } from "./gateway-processes-DdfQ_p-o.js";
...
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-DHrQ5Qk2.js";
import { r as withProgress } from "./progress-F0nzmXYd.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-86BGsxSJ.js";
import { n as isGatewayRunFutureConfigAllowed, t as enforceGatewayRunFutureConfigGuard } from "./future-config-guard-C_VfZGjc.js";
import { a as inspectGatewayCrashLoopBreaker, i as formatGatewayCrashLoopManualChannelStartHint, n as GATEWAY_CRASH_LOOP_RECOVERED_REASON, o as recordGatewayBootStart, r as completeGatewayBootLifecycle, s as recordGatewayCrashLoopRecovery, t as GATEWAY_CRASH_LOOP_BREAKER_REASON } from "./gateway-boot-lifecycle-DGPgfovT.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-hXrhJCo3.js");
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	const { createConfigRuntimeEnv } = await import("./env-vars-CHjaYLeo.js");
	const { resolveShellEnvFallbackTimeoutMs, shouldDeferShellEnvFallback, shouldEnableShellEnvFallback } = await import("./shell-env-BYmNGlIv.js");
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-D8yaWK-I.js");
	return {
		enabled: true,
...
async function loadGatewayRunShellEnvFallback(plan) {
	const { loadShellEnvFallback } = await import("./shell-env-BYmNGlIv.js");
...
async function clearGatewayRunShellEnvFallback(values) {
	...
	const { clearShellEnvAppliedKeys } = await import("./shell-env-BYmNGlIv.js");
...
function createConfiguredGatewayHealthProbe(cfg) {
	...
	return async (params) => {
		...
		if (!tlsFingerprint) tlsFingerprint = (await import("./gateway-9QVk7JoZ.js").then(({ loadGatewayTlsRuntime }) => loadGatewayTlsRuntime({
			...tlsConfig,
			autoGenerate: false
...
async function maybeWriteGatewayStartupFailureBundle(err, reason = "gateway.startup_failed") {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-Du29_UbK.js");
...
async function runGatewayCommandOnce(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-CQ6pudny.js");
	...
	const serverImportMeasurement = startupTrace.startMeasure("cli.server-import", () => import("./server-DDnrSHFR.js"));
	...
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-CQ6pudny.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-SMpVNzIY.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-CQ6pudny.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-B049LOTQ.js");
		...
		if (stale.length > 0) {
			...
			if (process.platform === "linux") {
				const { findSystemdGatewayInstallation, formatDuelingScopesWarning } = await import("./systemd-CW7e5Yxp.js");
	...
	if (opts.force) {
		...
		try {
			const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-Dqnfwrsd.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const [{ formatPortDiagnostics }, { inspectPortUsage }] = await Promise.all([import("./ports-format-jqsgnc08.js"), import("./ports-inspect-BhsRJRmC.js")]);
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-CGDl3stA.js");
		...
		if (findOpenClawAgentDatabaseMediaMigrationRequiredError(err)) try {
			const { parkCurrentLaunchAgentForMaintenance } = await import("./launchd-D4w6Xdoo.js");
...
async function runGatewayCommand(opts, hooks = {}, recoveryDeps) {
	try {
		...
		const { offerInvalidConfigRecovery } = await import("./invalid-config-recovery-BFlETJe8.js");
