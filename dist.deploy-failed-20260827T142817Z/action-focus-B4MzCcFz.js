...
import { n as readAcpSessionEntry } from "./session-meta-BrtiZ04k.js";
import { r as resolveConversationBindingContextFromAcpCommand } from "./conversation-binding-input-C8z3qpDj.js";
import { c as resolveThreadBindingMaxAgeMsForChannel, l as resolveThreadBindingPlacementForCurrentContext, n as formatThreadBindingSpawnDisabledError, o as resolveThreadBindingIdleTimeoutMsForChannel, t as formatThreadBindingDisabledError, u as resolveThreadBindingSpawnPolicy } from "./thread-bindings-policy-CY5sQygn.js";
...
import { i as resolveFocusTargetSession, r as resolveCommandSubagentController } from "./shared-DQRlann8.js";
...
async function handleSubagentsFocusAction(ctx) {
	...
	if (!token) return commandReply("Usage: /focus <subagent-label|session-key|session-id|session-label>");
	const controller = resolveCommandSubagentController(params, ctx.requesterKey);
	if (controller.controlScope !== "children") return commandReply("⚠️ Leaf subagents cannot control other sessions.");
	const bindingContext = resolveFocusBindingContext(params);
	if (!bindingContext) return commandReply("⚠️ /focus must be run inside a bindable conversation.");
	const bindingService = getSessionBindingService();
	const capabilities = bindingService.getCapabilities({
		channel: bindingContext.channel,
		accountId: bindingContext.accountId
	});
	if (!capabilities.adapterAvailable || !capabilities.bindSupported) return commandReply("⚠️ Conversation bindings are unavailable for this account.");
	const focusTarget = await resolveFocusTargetSession({
		runs,
		token,
		requesterKey: controller.controllerSessionKey
	});
	if (!focusTarget) return commandReply(`⚠️ Unable to resolve focus target: ${token}`);
	if (bindingContext.placement === "child") {
		...
		if (!spawnPolicy.enabled) return commandReply(`⚠️ ${formatThreadBindingDisabledError({
			channel: spawnPolicy.channel,
			accountId: spawnPolicy.accountId,
			kind: "subagent"
		})}`);
		if (bindingContext.placement === "child" && !spawnPolicy.spawnEnabled) return commandReply(`⚠️ ${formatThreadBindingSpawnDisabledError({
			channel: spawnPolicy.channel,
			accountId: spawnPolicy.accountId,
			kind: "subagent"
		})}`);
	...
	if (existingBinding && boundBy && boundBy !== "system" && senderId && senderId !== boundBy) return commandReply(`⚠️ Only ${boundBy} can refocus this conversation.`);
	...
	if (!capabilities.placements.includes(bindingContext.placement)) return commandReply("⚠️ Conversation bindings are unavailable for this account.");
	let binding;
	try {
	...
	return commandReply(`✅ ${bindingContext.placement === "child" ? `created child conversation ${binding.conversation.conversationId} and bound it to ${binding.targetSessionKey}` : `bound this conversation to ${binding.targetSessionKey}`} (${focusTarget.targetKind}).`);
}
//#endregion
