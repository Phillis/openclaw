import { n as normalizeArrayBackedTrimmedStringList } from "./string-normalization-e_fvmxMf.js";
...
import { w as updatePairedDeviceNodeSurfaceInTransaction } from "./device-bootstrap-DCaGQVSG.js";
import { O as withPairedDeviceRecords, b as resolveNodePairingGeneration, i as clearNodePairingGenerationBins, x as resolveNodePairingState } from "./device-pairing-CFVs8foo.js";
	...
	return await withPairedDeviceRecords(params.baseDir, () => {
		return {
			value: updatePairedDeviceNodeSurfaceInTransaction(params.nodeId, params.baseDir, (device) => {
				const currentPairingGeneration = resolveNodePairingGeneration(device);
				if (!device?.nodeSurface || params.expectedPairingGeneration.nodeId !== device.deviceId || currentPairingGeneration?.key !== params.expectedPairingGeneration.key || device.nodeSurface.lastConnectedAtMs !== params.connectedAtMs || params.disconnectedAtMs < params.connectedAtMs) return {
					value: { recorded: false },
					persist: false
				};
				return {
					...
					nodeSurface: {
						...device.nodeSurface,
						lastDisconnectedAtMs: Math.max(device.nodeSurface.lastDisconnectedAtMs ?? params.disconnectedAtMs, params.disconnectedAtMs)
					}
				};
			}),
			persist: false
...
//#endregion
export { sameNodePermissionSurface as _, listNodePairing as a, recordPairedNodeDisconnection as c, renamePairedNode as d, requestNodePairing as f, sameNodeApprovalSurfaceSet as g, normalizeNodeApprovalSurfaceList as h, getPendingNodePairing as i, rejectNodePairing as l, updatePairedNodeBins as m, beginNodePairingConnect as n, projectNodePairing as o, reusePendingNodePairingForReconnect as p, finalizeNodePairingCleanupClaim as r, recordPairedNodeConnection as s, approveNodePairing as t, releaseNodePairingCleanupClaim as u };
