import { i as listContextEngineQuarantines } from "./registry-DOMV71ew.js";
import { r as listLoadedRuntimePluginIds, t as getActiveRuntimePluginRegistry } from "./active-runtime-registry-DA-5LJYr.js";
import { n as listPersistedRuntimeToolSchemaQuarantines } from "./tool-schema-quarantine-health-99_bSv-2.js";
import { n as resolveReadOnlyChannelPluginsForConfig } from "./read-only-CrbyxaJh.js";
...
async function collectInstalledPluginHealthSnapshot(params) {
	const { buildPluginCompatibilityNotices, buildPluginSnapshotReport } = await import("./status-DjY69rYW.js");
...
async function resolveUnregisteredMemoryEmbeddingProviders(params) {
	...
	try {
		const { collectRegisteredEmbeddingProviderIds, collectUnregisteredConfiguredMemoryEmbeddingProviders } = await import("./gateway-startup-plugin-ids-BhDkaSEy.js");
...
async function resolveShouldRunPluginIds(params) {
	...
	try {
		const { loadGatewayStartupPluginPlan } = await import("./gateway-startup-plugin-ids-BhDkaSEy.js");
		const { resolvePluginActivationSourceConfig } = await import("./activation-source-config-B13P-0E6.js");
		const { resolveGatewayStartupPluginActivationConfig } = await import("./plugin-activation-runtime-config-Bhn6MhD6.js");
