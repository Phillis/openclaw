...
import { Fi as validateSystemAgentSetupAuthStartParams, Ii as validateSystemAgentSetupDetectParams, Li as validateSystemAgentSetupVerifyParams, Mi as validateSystemAgentChatHistoryParams, Ni as validateSystemAgentChatParams, Pi as validateSystemAgentSetupActivateParams, Xv as buildSystemAgentSessionInvalidatedErrorDetails, Yv as buildSystemAgentInferenceUnavailableErrorDetails } from "./src-Bo4ezI_n.js";
import { s as errorShape } from "./error-codes-CMSvT5-d.js";
import { r as formatSystemAgentStartupMessage } from "./overview-ClhxVBNB.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-YBnpO-hy.js";
import { g as setCommandLaneConcurrency, r as enqueueCommandInLane } from "./command-queue-CqN2qr5o.js";
import { n as describeSystemAgentPersistentOperation } from "./operations-CCgw0hU4.js";
import { n as isSystemAgentInferenceUnavailableError } from "./inference-error-CLaGK7fO.js";
...
import { f as listVisiblePendingApprovalRequests, i as buildRequestedApprovalEvent, s as handlePendingApprovalRequest } from "./approval-shared-BBcVwZrS.js";
import { t as getUpdateAvailable } from "./update-startup-CbuCo10X.js";
import { n as getHealthCache } from "./health-state-XZDQrS9y.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-B5NFAI_I.js";
import { t as WizardSession } from "./session-BhplepEX.js";
import { i as runExclusiveSystemAgentSetupActivation, n as SetupAdmissionBusyError, r as createAdmittedWizardSession, t as SETUP_ADMISSION_BUSY_MESSAGE } from "./setup-admission-BWx5aUeI.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.detect": async ({ params, respond }) => {
		...
		const { detectSetupInferenceIsolated } = await import("./setup-inference-detection-DkvlXX1c.js");
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = await createAdmittedWizardSession(() => new WizardSession(async (prompter, signal, runnerSession) => {
			await runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-ErkQgUmi.js"), import("./setup.shared-BmVj-Htu.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-BMB876YH.js");
