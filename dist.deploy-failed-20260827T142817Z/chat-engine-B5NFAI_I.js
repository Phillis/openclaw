...
import { i as loadSystemAgentOverview } from "./overview-ClhxVBNB.js";
import { a as resolveSystemAgentVerifiedInferenceRoute, i as resolveSystemAgentExpectedAgentHarnessRuntimeArtifact } from "./verified-inference-G4rRrE4u.js";
import { s as prepareSystemAgentRunAdmission } from "./admitted-run-context-BxSN0sUe.js";
import { a as parseSystemAgentOperation, c as isSystemAgentSensitiveConfigValue, i as isPersistentSystemAgentOperation, l as redactSystemAgentConfigPath, n as describeSystemAgentPersistentOperation, s as isInvalidConfigSetOperation, t as executeSystemAgentOperation } from "./operations-CCgw0hU4.js";
import { n as isSystemAgentInferenceUnavailableError, t as SystemAgentInferenceUnavailableError } from "./inference-error-CLaGK7fO.js";
import { t as approvalQuestion } from "./dialogue-DqAPDbkR.js";
import { n as resolveOperatorApprovalDecision, r as resolvePendingOperatorProposal, t as classifySystemAgentApprovalText } from "./operator-approval-DQUXzKQy.js";
import { a as normalizeCliModel } from "./helpers-He0AI0FX.js";
import { a as SYSTEM_AGENT_SYSTEM_PROMPT } from "./assistant-prompts-CvAfERCE.js";
import { n as sanitizeWizardStepForClient, r as wizardStepAwaitsInput, t as WizardSession } from "./session-BhplepEX.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		import("./agent-events-BbkQ3nOe.js"),
		import("./embedded-agent-tool-results-Dmltm-WW.js"),
		import("./system-agent-tool-DXSQgcHL.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-CWH3UUi_.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-FeUdPibX.js")).runEmbeddedAgent)({
				...shared,
				preparedRunAdmission,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
...
var ChatTurnRouter = class {
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.options.classifyApproval ?? (await import("./approval-intent-DufyyZ9i.js")).classifySystemAgentApprovalIntent)({
			message: text,
	...
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.options.planWithAssistant ?? (await import("./assistant-DtkopFMy.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
	...
	async runOperation(operation, provenance) {
		...
		if (operation.kind === "open-setup") {
			...
			if (![
				...
				"search",
				"gateway"
			].includes(operation.target)) return {
		...
		if (operation.kind === "channel-setup") return await this.startWizard(this.wizard.startChannel(operation.channel));
		if (operation.kind === "skills-setup") return await this.startWizard(this.wizard.startSkills());
		if (operation.kind === "search-setup") return await this.startWizard(this.wizard.startSearch());
		if (operation.kind === "gateway-config-setup") return await this.startWizard(this.wizard.startGateway());
		if (operation.kind === "memory-import") return await this.startWizard(this.wizard.startMemoryImport());
		if (operation.kind === "model-setup") return this.startModelSetup();
		const capture = createCaptureRuntime();
		...
		const result = await this.executeOperation(operation, capture, this.options.yes === true || !isPersistentSystemAgentOperation(operation));
		const verify = result?.applied ? await this.callbacks.verifyConfigAfterWrite() : null;
		const followUp = this.armFollowUp(result?.followUp);
		const reply = [
			provenance,
...
var SystemAgentChatEngine = class {
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-DtkopFMy.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
