...
import { F as resolveTimerTimeoutMs, p as clampPositiveTimerTimeoutMs } from "./number-coercion-oCkfUEEq.js";
import { r as formatErrorMessage } from "./errors-CSNUPl5U.js";
import { i as resolveGlobalSingleton } from "./global-singleton-Dc_stLtU.js";
import { t as createSubsystemLogger } from "./subsystem-CDLhGl2-.js";
import { t as isPromiseLike } from "./promise-like-D7-l5Fsp.js";
import { i as withPluginMetadataSnapshotScope } from "./current-plugin-metadata-snapshot-B1YRfQOc.js";
import { _ as readToolAllowlistIntersection, f as attachToolAllowlistIntersection, g as normalizeToolPolicyName, h as normalizeToolList, m as expandToolGroups } from "./tool-policy-CWmnHLY1.js";
...
function createHookRunner(registry, options = {}) {
	...
	const mergeBeforeModelResolve = (acc, next) => {
		const thinkingLevelOverride = firstDefined(acc?.thinkingLevelOverride, next.thinkingLevelOverride);
		...
		return {
			...
			...thinkingLevelOverride !== void 0 ? { thinkingLevelOverride } : {},
			...fastModeOverride !== void 0 ? { fastModeOverride } : {}
		};
	};
	...
	*/
	async function runAgentEnd(event, ctx, optionsLocal) {
		return runVoidHook("agent_end", withAgentRunId({
			...event,
			terminal: event.terminal ?? true
		}, ctx), ctx, optionsLocal);
	}
	/**
	...
	*/
	...
	return {
		...
		runBeforeAgentReply,
		runModelCallStarted: async (event, ctx) => runVoidHook("model_call_started", event, ctx),
		runModelCallEnded: async (event, ctx) => runVoidHook("model_call_ended", event, ctx),
		runLlmInput: async (event, ctx) => runVoidHook("llm_input", event, ctx),
		runLlmOutput: async (event, ctx) => runVoidHook("llm_output", event, ctx),
		...
		runChannelPairingRequested: async (event, ctx) => runVoidHook("channel_pairing_requested", event, ctx),
		runMessageReceived: async (event, ctx) => runVoidHook("message_received", event, ctx),
		runBeforeDispatch,
		runReplyDispatch,
		...
		runSessionEnd: async (event, ctx) => runVoidHook("session_end", event, ctx),
		runSubagentSpawning,
		...
		runGatewayStop: async (event, ctx) => runVoidHook("gateway_stop", event, ctx),
		runHeartbeatPromptContribution,
		runCronReconciled: async (event, ctx) => runVoidHook("cron_reconciled", event, ctx),
		runCronChanged: async (event, ctx) => runVoidHook("cron_changed", event, ctx),
...
//#endregion
export { fireAndForgetBoundedHook as _, resetGlobalHookRunner as a, getPluginRuntimeGenerationRegistry as b, createPluginToolMatcherScope as c, pluginToolMatcherCoversTool as d, createPluginSubagentRequesterContext as f, joinPresentTextSegments as g, resolveBlockMessage as h, initializeGlobalHookRunner as i, mergePluginToolMatcherScopes as l, cloneHookIsolationValue as m, getGlobalPluginRegistry as n, runGlobalGatewayStopSafely as o, resolvePluginSubagentCompletionRequester as p, hasGlobalHooks as r, getToolHookMatcherScope as s, getGlobalHookRunner as t, normalizePluginToolMatcher as u, fireAndForgetHook as v, withPluginRuntimeGenerationScope as x, getGlobalHookRunnerRegistry as y };
