...
import { a as requireRiskAcknowledgement } from "./setup.shared-81PCw8JC.js";
...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk, agentName) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted, setupAgentName) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-DJA33qOy.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	const localOnboarding = custodianMode ? await import("./local-onboarding-state-B_av7bvz.js") : void 0;
	...
	if (!custodianMode) {
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-DhTxkRt_.js")).runSetupMemoryImportStep)({
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-kyaMYyTu.js")).applySystemAgentSetup;
			...
			const appliedSnapshot = localSetup?.status === "pending" ? await (await import("./setup-recovery-Cc7H-b0F.js")).completeLocalSetupRecovery({
				owner: localSetup,
	...
	if (wantsDiscovery) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-DhTxkRt_.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-DoDompoZ.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
	...
	if (opts.tui !== true) {
		if ((await (deps.runBrowserHandoff ?? (await import("./onboard-browser-handoff-BM5GVwHI.js")).runBrowserHatchHandoff)({
			config: persistedConfig,
			prompter,
