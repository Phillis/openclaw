...
import { u as isRootHelpInvocation } from "./argv-CgA2urTO.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-CtNEO_uG.js";
import { n as parseCliContainerArgs, r as resolveCliContainerTarget } from "./container-target-CmkzL0zt.js";
import { t as tryHandleRootVersionFastPath } from "./entry.version-fast-path-yee12Lx0.js";
...
async function writeCapturedCliArgumentError(message) {
	...
	const { enableConsoleCapture } = await import("./logging-x4ZSaZk5.js");
...
async function writeCliDiagnosticBlock(message) {
	...
	loadCliDotEnv({ quiet: true });
	await configureGatewayStartupTraceConsoleFormatting(gatewayEntryStartupTrace);
	const { formatConsoleDiagnosticBlock } = await import("./json-console-line-C2I0NKji.js");
...
async function flushEntryStartupTraceForEarlyReturn(argv) {
	...
	const { loadCliDotEnvForEarlyDiagnostic } = await import("./dotenv-hlkEfhz_.js");
...
if (!isMainModule({
	...
	const { assertSupportedRuntime, isCurrentRuntimeSupported } = await import("./runtime-guard-DOTJ_aad.js");
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-BqJiy0UL.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
	...
	await runCliWithExitFinalization({
		run: async () => {
			if (isNativeHookRelayArgv(argv) && !argv.includes("--help") && !argv.includes("-h")) {
				const { runNativeHookRelayCliFromArgv } = await import("./native-hook-relay-cli-uVDyg0XI.js");
		...
		onError: async (error) => {
			...
			const { enableConsoleCapture } = await import("./logging-x4ZSaZk5.js");
			enableConsoleCapture();
			const { formatCliFailureLines } = await import("./failure-output-Ca786-bi.js");
