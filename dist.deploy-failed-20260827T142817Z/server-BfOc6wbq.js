...
import { R as timestampMsToIsoString, j as resolveIntegerOption } from "./number-coercion-oCkfUEEq.js";
import { i as readNonNegativeInteger, n as readMetadataNumber, r as readMetadataString, t as readBool } from "./meta-QuIT1Mnh.js";
import { t as toAcpSessionLineageMeta } from "./session-lineage-meta-C_v-FnZ-.js";
import { n as defaultAcpSessionStore } from "./session-C4D-VRmj.js";
...
import { d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, r as closeOpenClawStateDatabase } from "./openclaw-state-db-CXrhNigN.js";
import { t as BASE_THINKING_LEVELS } from "./thinking.shared-bHYuuc1L.js";
...
import { r as resolveGatewayClientBootstrap } from "./client-bootstrap-W2cQ-6mE.js";
...
import { t as readSecretFromFile } from "./secret-file-Cx9wtUx4.js";
import { t as ACP_AGENT_INFO } from "./types-B6ZbCN2o.js";
...
var AcpTranslatorDisconnects = class {
	constructor(gateway, pendingPrompts, getPendingPrompt, finishPrompt, rejectPendingPrompt, log) {
		this.gateway = gateway;
		this.pendingPrompts = pendingPrompts;
		this.getPendingPrompt = getPendingPrompt;
...
var AcpTranslatorPromptStream = class {
	constructor(connection, gateway, opts, sessionStore, sessionUpdates, sessionState, approvalRelays, log) {
		...
		this.disconnects = new AcpTranslatorDisconnects(gateway, this.pendingPrompts, (sessionId, runId) => this.getPendingPrompt(sessionId, runId), (sessionId, pending, stopReason) => this.finishPrompt(sessionId, pending, stopReason), (pending, error, options) => this.rejectPendingPrompt(pending, error, options), log);
	}
	shutdown() {
		...
		return new Promise((resolve, reject) => {
			...
			const sendWithProvenanceFallback = async () => {
				...
				const applyTerminalAck = async (ack) => {
					const status = normalizeLowercaseStringOrEmpty(ack?.status);
					const pending = () => this.getPendingPrompt(params.sessionId, runId);
					if (status === "timeout") {
	...
	async cancelSessionWork(session) {
		...
		if (pending) {
			this.agentEvents.clearApprovalRelaysForPrompt(session.sessionId, pending.idempotencyKey, { denyActive: true });
	...
	async finishPrompt(sessionId, pending, stopReason) {
		...
		try {
			this.agentEvents.clearApprovalRelaysForPrompt(sessionId, pending.idempotencyKey, { denyActive: true });
...
const loadAcpCommandsModule = createLazyRuntimeModule(() => import("./commands-XP3NIHbI.js"));
