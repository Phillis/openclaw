...
import { p as resolveInstalledPluginIndexPolicyHash } from "./installed-plugin-index-BC03OFwf.js";
...
function withPluginMetadataSnapshotScope(snapshot, run, options = {}) {
	...
	if (options.config) {
		const policyHash = resolveInstalledPluginIndexPolicyHash(options.config);
		if (policyHash === snapshot.policyHash || compatiblePolicyHashes?.includes(policyHash)) configIdentities.add(options.config);
	}
...
//#endregion
export { createPluginIdScopeSet as a, normalizePluginIdScope as c, withPluginMetadataSnapshotScope as i, serializePluginIdScope as l, installTemporaryCurrentPluginMetadataSnapshot as n, hasExplicitPluginIdScope as o, setCurrentPluginMetadataSnapshot as r, hasNonEmptyPluginIdScope as s, getCurrentPluginMetadataSnapshot as t };
