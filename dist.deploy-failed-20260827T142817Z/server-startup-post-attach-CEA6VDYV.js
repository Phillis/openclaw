...
import { v as sweepSessionStateWatchNotices } from "./session-state-events-BTZJfLLh.js";
import { s as hasRestartSentinel } from "./restart-sentinel-BMlog4T5.js";
...
import { t as hasConfiguredInternalHooks } from "./configured-tyRvG8Ks.js";
...
function scheduleContextCachePrewarm(params) {
	...
	const warm = async () => {
		if (stopped) return;
		const { prewarmContextWindowCacheAfterReady } = await import("./context-aRoKCY0G.js");
...
async function prewarmGatewaySessionListData(cfg, agentId) {
	const [{ loadCombinedSessionStoreForGatewayCore }, { listSessionsFromStoreAsync }] = await Promise.all([import("./combined-store-gateway-Cl0lerEQ.js"), import("./session-utils-list-BzK6YZNw.js")]);
...
function dashboardDataPrewarmItems(cfg, log) {
	...
	const shouldPrewarmSessionData = async () => {
		...
		const { canPrewarmCombinedSessionStoresForGateway } = await import("./combined-store-gateway-Cl0lerEQ.js");
	...
	return [...agentIds.map((agentId) => ({
		...
		name: "plugins",
		load: async () => {
			const { listManagedPlugins } = await import("./management-service-C0DdHUjL.js");
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-R1Z9NtJJ.js"));
const loadMainSessionRestartRecoveryMarkingModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-marking-BcbRPeit.js"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-RjT9WtG0.js"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-CACl5c-f.js"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-5DL1ADPk.js"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-DW1Gbl7l.js"));
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	runWithGatewayIndependentRootWorkAdmission(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-C-LWHVp_.js");
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-wo9oXQGA.js");
			if (isStopped()) return;
...
async function hydrateConfiguredExternalCliAuth(params) {
	const deps = params.deps ?? await Promise.all([
		...
		import("./prepared-model-runtime.configured-BX1UWp5Q.js"),
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([loadInternalHooksModule(), import("./loader-UUwjr8kb.js")]);
	...
	let pluginServices = params.shouldStartPluginServices?.() === false ? null : await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-CIfAXzEk.js");
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-DKokSwoY.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadPreparedModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				...
				import("./prepared-model-catalog-DmTX1q65.js"),
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-C2nGFc4N.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-CnhLoWCZ.js")).scheduleGatewayUpdateCheck(...args),
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	const startSidecars = () => params.minimalTestGateway ? Promise.resolve({
		...
		try {
			const { scheduleSubagentRegistrySweep } = await import("./subagent-registry-CCzLJbcn.js");
	...
	sidecarsPromise.then(async (sidecarsResult) => {
		...
		if (hookRunner?.hasHooks("gateway_start")) {
			const { withPluginHttpRouteRegistry } = await import("./http-registry-BqRRE8Zl.js");
			runWithGatewayIndependentRootWorkAdmission(async () => {
