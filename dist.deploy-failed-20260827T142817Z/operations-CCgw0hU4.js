...
import { L as collectChannelSchemaMetadataCore, R as collectPluginSchemaMetadataCore } from "./io-D1h6pxaD.js";
...
import { p as findWildcardHintMatch } from "./zod-schema-YA3pcCEJ.js";
import { n as ChannelsSchema } from "./zod-schema.channels-config-CZ6nezAF.js";
import { r as isReservedSystemAgentId } from "./agent-id-BYpRMvce.js";
import { i as sameDefaultInferenceRoute, n as projectInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-BdB9LAmv.js";
import { i as appendSystemAgentAuditEntry, r as SYSTEM_AGENT_AUDIT_STORE_LABEL } from "./audit-DPaGSfB4.js";
import { n as t } from "./i18n-BzsUVhtU.js";
import { n as redactConfigObject, t as REDACTED_SENTINEL } from "./redact-snapshot-DuN7qyYL.js";
import { a as parseConfigSetPath, o as parseConfigSetValue } from "./config-cli-path-DUkpyN_X.js";
import { i as isKernelOwnedChannelConfigKey, n as classifyConfigSchemaPathSegment, t as buildConfigSchemaCore } from "./schema-BT17pjzs.js";
import { r as isOpenClawTrustedPluginInstallSpec } from "./install-provenance-C3hHeUzH.js";
...
const loadOverviewModule$1 = async () => await import("./overview-DdEJYWXB.js");
...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { scheduleSafeGatewayRestart } = await import("./restart-coordinator-DAdYL6Vj.js");
		return scheduleSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-CxeQkf94.js");
	if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
	const listPlugins = deps?.listChannelSetupPlugins ?? (await import("./setup-registry-Ch7wtSyh.js")).listChannelSetupPlugins;
	const resolveEntries = deps?.resolveChannelSetupEntries ?? (await import("./discovery-shF9VpRF.js")).resolveChannelSetupEntries;
	const isConfigured = deps?.isChannelConfigured ?? (await import("./channel-configured-shared-BAbhk0rQ.js")).isStaticallyChannelConfigured;
	const { shouldShowChannelInSetup } = await import("./discovery-shF9VpRF.js");
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-aB0jzb0S.js");
...
async function isDefaultAgentListPath(segments) {
	...
	const defaultAgentId = config?.agents?.defaults?.systemAgent?.agentId?.trim() ?? (config ? tryResolveLegacyCompatibilityAgentId(config) : void 0);
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-aB0jzb0S.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-kyaMYyTu.js")).applySystemAgentSetup;
			const surface = ctx.deps?.setupSurface ?? "cli";
			const recovery = surface === "cli" ? await (await import("./setup-recovery-Cc7H-b0F.js")).loadLocalSetupRecovery(operation.workspace) : void 0;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-kyaMYyTu.js");
...
async function isPluginBackingDefaultInferenceRoute(pluginId) {
	...
	const { resolveModelRuntimePolicy } = await import("./model-runtime-policy-CYG72Do5.js");
	...
	const { resolveOwningPluginIdsForProviderRef } = await import("./providers-DM3eHb4p.js");
...
async function executePluginInstall(operation, runtime, opts) {
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			await ctx.commit(async () => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-iz_cnTva.js");
...
const loadOverviewModule = async () => await import("./overview-DdEJYWXB.js");
	...
	switch (operation.kind) {
		...
		case "plugin-list":
			await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
				const { runPluginsListCommand } = await import("./plugins-list-command-B86OhNUY.js");
				await runPluginsListCommand({}, pluginRuntime);
			}))(runtime);
			return { applied: false };
		case "plugin-search":
			await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
				const { runPluginsSearchCommand } = await import("./plugins-search-command-BtpDDG_T.js");
		...
		case "config-schema": {
			const { buildConfigSchemaCore, lookupConfigSchema } = await import("./schema-BqshZYrJ.js");
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-C4RrV2Eu.js");
		...
		case "create-agent":
			...
			return await applyPersistentOperation({
				...
				opts,
				run: async (ctx) => {
					const createAgentForOperation = ctx.deps?.createAgent ?? (await import("./agent-create-D5s9Or1J.js")).createAgent;
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-Cn3ulp8m.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-BM-Tn0gr.js");
			await statusCommand({ timeoutMs: 1e4 }, runtime);
			return { applied: false };
		}
		case "health": {
			const { healthCommand } = await import("./health-ucfCHFsw.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-CVVTa1xA.js")).runTui)({
