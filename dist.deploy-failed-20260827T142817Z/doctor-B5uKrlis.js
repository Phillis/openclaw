...
import { n as formatConsoleDiagnosticLine } from "./json-console-line-DT9WEeLV.js";
...
import { r as readPersistedInstalledPluginIndex } from "./installed-plugin-index-store-BIcBJeAh.js";
...
import { i as withDoctorSqliteMaintenanceLock, r as isDestructiveDoctorSessionSqliteMode } from "./doctor-sqlite-maintenance-lock-mAgCgc2I.js";
...
async function doctorCommand(runtime, options) {
	if (options?.stateSqlite) {
		...
		const { runDoctorStateSqliteCompact } = await import("./doctor-state-sqlite-compact-B6Eb_3oC.js");
	...
	await (await import("./doctor-health-8KHUg3R-.js")).runDoctorHealthFlow(runtime, options);
}
async function maybeCreateSessionSqliteGithubIssue(runtime, report, options) {
	...
	if (!approved && options.nonInteractive !== true && options.json !== true) {
		const { promptYesNo } = await import("./prompt-CJf0HRd8.js");
