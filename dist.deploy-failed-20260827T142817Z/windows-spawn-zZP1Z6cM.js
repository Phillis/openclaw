...
function resolveWindowsExecutablePath(command, env) {
	...
	const pathEntries = normalizeStringEntries((resolveEnvironmentValue(env, "PATH", "win32") ?? resolveEnvironmentValue(process.env, "PATH", "win32") ?? "").split(";"));
	const hasExtension = path.extname(command).length > 0;
	const pathExtRaw = resolveEnvironmentValue(env, "PATHEXT", "win32") ?? resolveEnvironmentValue(process.env, "PATHEXT", "win32") ?? ".EXE;.CMD;.BAT;.COM";
