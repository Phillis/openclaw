...
import { n as assertOkOrThrowHttpError, u as readProviderBinaryResponse } from "./provider-http-errors-BH2HGv8j.js";
import "./directives-DzjGFYEk.js";
import { c as postJsonRequest, p as resolveProviderHttpRequestConfig } from "./shared-CzcciRDF.js";
...
function resolveProviderConfigRecord(rawConfig, providerConfigKey) {
	return asOptionalRecord(asOptionalRecord(rawConfig.providers)?.[providerConfigKey]) ?? asOptionalRecord(rawConfig[providerConfigKey]);
}
function readModelProviderConfig(cfg, providerConfigKey) {
	return asOptionalRecord(asOptionalRecord(asOptionalRecord(asOptionalRecord(cfg)?.models)?.providers)?.[providerConfigKey]);
}
