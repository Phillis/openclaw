...
function loadSetupShared() {
	return setupSharedPromise ??= import("./setup.shared-BmVj-Htu.js");
}
...
async function runHostedChannelSetup(channel, prompter, beforePersistentApply, runtime) {
	const { createChannelOnboardingPostWriteHookCollector, runCollectedChannelOnboardingPostWriteHooks, setupChannels } = await import("./onboard-channels-BN2qE8ec.js");
...
async function runHostedSkillsSetup(prompter, beforePersistentApply, runtime) {
	const [{ setupSkills }, { resolveSystemAgentOnboardingTarget }] = await Promise.all([import("./onboard-skills-KdGdmS4d.js"), import("./onboard-agent-target-BBLzsIeD.js")]);
	return await runHostedSetup({
		label: "Skills setup",
		runtime,
		beforePersistentApply,
...
async function runHostedSearchSetup(prompter, beforePersistentApply, runtime) {
	const { runSearchSetupFlow } = await import("./search-setup-9ikdPtfr.js");
	return await runHostedSetup({
...
async function runHostedGatewaySetup(prompter, beforePersistentApply, runtime) {
	const [{ resolveGatewayPort }, { configureGatewayForSetup }, { resolveQuickstartGatewayDefaults }] = await Promise.all([
		...
		import("./setup.gateway-config-Bl5Dbu8x.js"),
		loadSetupShared()
	]);
	return await runHostedSetup({
		label: "Gateway setup",
...
async function runHostedMemoryImport(prompter, beforePersistentApply, onProviderOutcome) {
	const [{ readSetupConfigFileSnapshot }, { resolveSystemAgentOnboardingTarget }] = await Promise.all([loadSetupShared(), import("./onboard-agent-target-BBLzsIeD.js")]);
	const snapshot = await readSetupConfigFileSnapshot();
	...
	const { runSetupMemoryImportStep } = await import("./setup.memory-import-DhTxkRt_.js");
