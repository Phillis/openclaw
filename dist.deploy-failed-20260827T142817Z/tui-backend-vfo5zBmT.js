...
import { i as loadSystemAgentOverview, r as formatSystemAgentStartupMessage } from "./overview-ClhxVBNB.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-YBnpO-hy.js";
import { o as resolveSystemAgentVerifiedInferenceState } from "./verified-inference-G4rRrE4u.js";
import { t as executeSystemAgentOperation } from "./operations-CCgw0hU4.js";
...
import { t as SystemAgentChatEngine } from "./chat-engine-B5NFAI_I.js";
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	await (opts.runChannelsAdd ?? (await import("./add-D0pnUCdA.js")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
...
async function runSystemAgentTui(opts, runtime) {
	...
	for (;;) {
		...
		const runTui = boundOpts.runTui ?? (await import("./tui-CVVTa1xA.js")).runTui;
		try {
			await runTui({
				local: true,
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (verified) {
			...
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-DmTX1q65.js"), import("./model-thinking-default-CisFOmRW.js")]);
