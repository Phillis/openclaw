...
async function probeGateway(opts) {
	...
	const deviceIdentity = await (async () => {
		try {
			...
			const { loadDeviceIdentityIfPresent } = await import("./device-identity-6j-wIYle.js");
			const identity = loadDeviceIdentityIfPresent({ env: opts.env });
			if (!identity) return null;
			return (opts.suppressStoredDeviceAuth ? null : deviceAuthScope ? loadOriginDeviceToken({
...
//#endregion
export { probeGateway as n, clampProbeTimeoutMs as t };
