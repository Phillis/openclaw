...
import { T as resolveSubagentConfiguredModelSelection, w as resolveDefaultModelForAgent } from "./codex-route-model-ref-Bw2nFxxx.js";
...
import { Et as inheritSessionSelection, H as createSessionEntryWithTranscript, Lt as resolveSessionEntryAccessTarget, gn as buildSessionCreationStamp, qt as listSessionEntriesReadOnly } from "./session-accessor-CVnxp3UM.js";
import { X as runExclusiveSessionLifecycleMutation, Y as isSessionWorkAdmissionActive, a as isAgentHarnessSessionKey, o as isAgentHarnessSessionKeyOwnedBy, r as AGENT_HARNESS_SESSION_KEY_RESERVED_MESSAGE } from "./agent-harness-session-key-D5rklW6u.js";
...
import { g as normalizeInheritedToolDenylist, h as normalizeInheritedToolAllowlist } from "./subagent-capabilities-C_xcyqSF.js";
import { i as hasInternalHookListeners, m as triggerInternalHook, n as createInternalHookEvent } from "./internal-hooks--FUKcLjc.js";
import { a as isModelSelectionLocked } from "./model-overrides-D4SC_nUZ.js";
import { i as forkSessionFromParentWithDecision, t as MODEL_SELECTION_LOCKED_PARENT_FORK_MESSAGE } from "./session-fork-Bb60vAkd.js";
import { n as shouldPreserveSessionAuthProfileOverride } from "./auth-profile-preservation-CkLTF0wv.js";
...
import { d as isSessionVisibilityAllowed, h as resolveSessionVisibility } from "./session-sharing-QTh4cZeN.js";
import { n as resolveSessionPatchModelSelection, t as projectSessionsPatchEntry } from "./sessions-patch-vkcbbb-Z.js";
...
async function createGatewaySession(params) {
	...
	if (explicitAgentId && explicitKeyAgentId && normalizeAgentId(explicitKeyAgentId) !== normalizeAgentId(explicitAgentId)) return {
		ok: false,
		error: errorShape(ErrorCodes.INVALID_REQUEST, `sessions.create key agent (${explicitKeyAgentId}) does not match agentId (${normalizeAgentId(explicitAgentId)})`)
	};
	const requestedKeyAgent = requestedKey ? resolveRequestedSessionAgentId(params.cfg, requestedKey, explicitAgentId, { allowUnconfiguredExplicitAgent: true }) : void 0;
	if (requestedKeyAgent && !requestedKeyAgent.ok) return requestedKeyAgent;
	const implicitSelectionKey = explicitAgentId ? `agent:${normalizeAgentId(explicitAgentId)}:main` : "main";
	const implicitAgent = requestedKeyAgent ? void 0 : resolveRequestedSessionAgentId(params.cfg, implicitSelectionKey, explicitAgentId, { allowUnconfiguredExplicitAgent: true });
	if (implicitAgent && !implicitAgent.ok) return implicitAgent;
	const agentId = normalizeAgentId(explicitAgentId ?? requestedKeyAgent?.agentId ?? implicitAgent?.agentId);
