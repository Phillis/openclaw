...
import { i as normalizeEnv, n as isTruthyEnvValue } from "../env-y-_yRnBE.js";
...
import { A as consumeGatewayFastPathRootOptionToken, I as getCoreCliCommandDescriptors, L as getCoreCliCommandNamesCore, M as resolveGatewayCatalogCommandPath, N as resolveGatewayRunPreBootstrapOptions, U as isMachineOutputStdoutTTY, f as isSimpleCommandHelpInvocation, g as normalizeRootNoColorArgv, h as normalizeRootLogLevelArgv, j as consumeGatewayRunOptionToken, m as normalizeRootHelpTargetArgv, p as normalizeGeneratedHelpCommandArgv, v as getSubCliEntriesCore, y as getSubCliParentDefaultHelpCommands, z as getCoreCliParentDefaultHelpCommands } from "../argv-CgA2urTO.js";
import { t as resolveCliArgvInvocation } from "../argv-invocation-CtNEO_uG.js";
import { n as parseCliContainerArgs, t as maybeRunCliInContainer } from "../container-target-CmkzL0zt.js";
import { a as tryOutputPrecomputedCommandHelp, i as parseCliProfileArgs, n as createGatewayDispatchStartupTrace, r as applyCliProfileEnv, t as configureGatewayStartupTraceConsoleFormatting } from "../startup-trace-0yFrg0nI.js";
...
import { n as resolveManifestCommandAliasOwnerInRegistry, r as resolveManifestToolOwnerInRegistry } from "../manifest-command-aliases-CtGMc6F7.js";
import { t as assertSupportedRuntime } from "../runtime-guard-CBEVEZk3.js";
import { a as isLoopbackAddress, l as isSecureWebSocketUrl, x as normalizeWebSocketProtocol } from "../net-BRYQcUG8.js";
import { t as ensureOpenClawCliOnPath } from "../path-env-L5fXdI0M.js";
import { a as shouldSkipPluginCommandRegistration, r as shouldRegisterPrimaryCommandOnly, t as isReservedNonPluginCommandRoot } from "../command-registration-policy-vJ2VPNBY.js";
import { n as resolveCliNetworkProxyPolicy, t as resolveCliCommandPathPolicy } from "../command-path-policy-DJ20rW4U.js";
import { t as resolveCliStartupPolicy } from "../command-startup-policy-BColu-bU.js";
import { t as isUnconfiguredConfigSource } from "../fresh-install-config-DhP5LyQI.js";
import { i as withConsoleLogsRoutedToStderrForJson, n as hasJsonOutputFlag, r as withConsoleLogsRoutedToStderr } from "../json-output-mode-Bg5EcQwj.js";
import { t as formatCliCommandSuggestions } from "../command-suggestions-99rXycqr.js";
...
const loadLoggingModule = async () => await import("../logging-x4ZSaZk5.js");
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-C0dSsDIS.js");
const loadManifestCommandAliasesRuntimeModule = async () => await import("../manifest-command-aliases.runtime-h8gUMCqn.js");
const loadProxyLifecycleModule = async () => await import("../proxy-lifecycle-C0MNKBZZ.js");
const loadProgressModule = async () => await import("../progress-DFl0T8IE.js");
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }, { ensureCliExecutionBootstrap }, { defaultRuntime }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-command-T8S0Pm0u.js"),
		import("../version-qnM0RpgZ.js"),
		import("../banner-cpjE5W7u.js"),
		import("../command-startup-policy-DuVCDd2T.js"),
		import("../command-execution-startup-BkXpurJd.js"),
		import("../runtime-Dn74rUMz.js")
	]));
	...
	const beforeRun = async (opts) => {
		...
		if (!await startupTrace.measure("gateway-run-pre-bootstrap", async () => {
			const { prepareGatewayRunBootstrap, recheckGatewayRunBootstrap, wasPreparedGatewayRunCoreStatePristine, wasPreparedGatewayRunStatePristine } = await import("../pre-bootstrap-CQ6pudny.js");
		...
		await startupTrace.measure("gateway-run-bootstrap", async () => {
			...
			const { reloadTrustedGatewayRunEnvironment } = await import("../pre-bootstrap-CQ6pudny.js");
...
async function closeCliResources() {
	const finalizers = [
		async () => {
			const { listRegisteredAgentHarnesses, disposeRegisteredAgentHarnesses } = await import("../registry-BztSF0h1.js");
		...
		async () => {
			...
			if (hasManagedProviderLocalServices()) {
				const { stopManagedProviderLocalServices } = await import("../provider-local-service-AYJpUIxL.js");
		...
		async () => {
			...
			if (hasProviderTransportDispatcherPool()) {
				const { closeProviderTransportDispatcherPool } = await import("../provider-transport-dispatcher-pool-gVXJGqJm.js");
		...
		async () => {
			...
			if (hasMemoryRuntime()) {
				const { closeActiveMemorySearchManagersCore } = await import("../memory-runtime-BZGRqjYH.js");
...
async function shouldStartLocalOnboarding(snapshot) {
	...
	const { readLocalOnboardingStateForConfig } = await import("../local-onboarding-state-B_av7bvz.js");
...
async function resolveConfiguredTuiLaunchTarget(config, options) {
	...
	const { resolveAgentEffectiveModelPrimary, resolveDefaultAgentId } = await import("../agent-scope-B7ocz6pI.js");
...
async function resolveReachableGateway(config, options) {
	...
	const { probeGatewayConfiguredModel } = await import("../onboard-helpers-DG0mt_gD.js");
...
async function resolveGatewayProbePlan(config) {
	...
	if (normalizeOptionalString(config.gateway?.mode) === "remote" && remoteUrl) try {
		const { resolveGatewayClientBootstrap } = await import("../client-bootstrap-BrXl3pXT.js");
...
async function resolveLocalGatewayProbeTargets(config) {
	const [{ resolveGatewayPort }, { resolveControlUiLinks }, { resolveGatewayClientBootstrap }, { readActiveGatewayLockPort }] = await Promise.all([
		import("../paths-C39JGP_F.js"),
		import("../control-ui-links-BpiTkBod.js"),
		import("../client-bootstrap-BrXl3pXT.js"),
		import("../gateway-lock-Dnv0pbd7.js")
	]);
...
async function ensureCliEnvProxyDispatcher() {
	try {
		const { hasEnvHttpProxyAgentConfigured } = await import("../proxy-env-B2MegWOe.js");
		if (!hasEnvHttpProxyAgentConfigured()) return;
		const { ensureGlobalUndiciEnvProxyDispatcher } = await import("../undici-global-dispatcher-B60AonXy.js");
...
async function resolveUnownedCliPrimaryMessage(params) {
	...
	const suggestion = displayPrimary === params.primary ? formatCliCommandSuggestions(params.primary) : "";
	return [`Unknown command: openclaw ${displayPrimary}. No built-in command or plugin CLI metadata owns "${displayPrimary}".`, suggestion].filter(Boolean).join("\n");
}
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-BGkY_Vq-.js"), import("../coverage-BFQk1rJT.js")]));
...
async function runCliWithPreparedOutputMode(originalArgv, options) {
	...
	if (!isHelpOrVersionInvocation && !isDatabaseInvocation && !isAgentExecInvocation(normalizedInvocation.commandPath) && shouldLoadCliDotEnv(loadGlobalEnv)) await startupTrace.measure("dotenv", async () => {
		if (isRemoteAgentDispatchInvocation(normalizedArgv, normalizedInvocation.primary)) {
			...
			const { loadCliDotEnv } = await import("../dotenv-hlkEfhz_.js");
			loadCliDotEnv({
				loadGlobalEnv,
				quiet: true
			});
	...
	if (!isHelpOrVersionInvocation && isGatewayRunInvocation) await startupTrace.measure("gateway-run-select-environment", async () => {
		const [{ selectGatewayRunEnvironment }, { defaultRuntime }] = await Promise.all([import("../pre-bootstrap-CQ6pudny.js"), import("../runtime-Dn74rUMz.js")]);
	...
	const bareSessionInvocation = !isHelpOrVersionInvocation && mayContainBareSessionUrl ? (await import("../session-ref-HkAbIZI3.js")).parseBareSessionInvocation(normalizedArgv) : null;
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-cWSCQOJ1.js").then((configIo) => normalizedInvocation.primary === "update" ? configIo.readSourceConfigBestEffort() : configIo.readBestEffortConfig({
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-BqJiy0UL.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-B0Gxxev8.js");
		...
		if (bareSessionInvocation) {
			...
			const { runTuiCliAction } = await import("../tui-cli-Cqrcn4Pc.js");
		...
		if (bareRootLaunchTarget) {
			if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
				...
				const { runRemoteGatewayInferenceOnboarding } = await import("../onboard-remote-gateway-BrglWHH3.js");
			...
			if (bareRootLaunchTarget.kind === "onboarding") {
				...
				const { setupWizardCommand } = await import("../onboard-Ckk0bMI3.js");
			...
			if (bareRootLaunchTarget.kind === "tui") {
				...
				const { runTui } = await import("../tui-CVVTa1xA.js");
				await runTui({
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-DYfWFQQo.js"));
		...
		try {
			const [{ buildProgram }, { formatUncaughtError }, { formatCliFailureLines }, { runFatalErrorHooks }, { installUnhandledRejectionHandler, isBenignUncaughtExceptionError, isUncaughtExceptionHandled }, { restoreRuntimeTerminalState }] = await startupTrace.measure("core-imports", () => Promise.all([
				import("../program-C6PoIqLm.js"),
				import("../infra/errors.js"),
				import("../failure-output-Ca786-bi.js"),
				import("../fatal-error-hooks-BiXaGF_Q.js"),
				import("../unhandled-rejections-BLz0rxi2.js"),
				import("../runtime-Dn74rUMz.js")
			]));
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-CpgOYFik.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-CrJvWq-1.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const [{ registerPluginCliCommandsFromValidatedConfig }, { resolveCliStartupPolicy }] = await Promise.all([import("../cli-DEX6Te-r.js"), import("../command-startup-policy-DuVCDd2T.js")]);
					const startupPolicy = resolveCliStartupPolicy({
						argv: parseArgv,
