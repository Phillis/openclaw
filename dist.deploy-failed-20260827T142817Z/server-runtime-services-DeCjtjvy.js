...
import { $t as loadSessionEntryReadOnly, hn as resolveSessionStorePathForScope } from "./session-accessor-CVnxp3UM.js";
import { f as recordSessionHumanDirectMessage, p as recordSessionStateEvent } from "./session-state-events-BTZJfLLh.js";
import { i as updateSessionUpstreamLinkMarker, n as listWatchedSessionUpstreamLinks, r as readSessionUpstreamLink, t as deleteSessionUpstreamLink } from "./session-upstream-links-DH4UZcUV.js";
import { l as isEmbeddedAgentRunActive } from "./runs-DdjJNEQM.js";
import { i as readRecentUserAssistantTextForSession } from "./transcript-CzAixYBD.js";
...
import { c as startSessionDeliveryRuntime, o as schedulePendingSessionDeliveries } from "./subagent-completion-delivery-ClRf4tJs.js";
import "./embedded-agent--GrdaB8E.js";
import { f as resolveHeartbeatAgents } from "./heartbeat-runner-session-DG5Kq8Z-.js";
import { t as startHeartbeatRunner } from "./heartbeat-runner-DCrrevnn.js";
...
function startPendingOutboundDeliveryRecovery(params) {
	...
	const recover = (startup) => {
		...
		const settled = runWithGatewayIndependentRootWorkAdmission(async () => {
			if (stopped) return;
			const { drainPendingDeliveriesCore, recoverPendingDeliveries } = await import("./delivery-queue-recovery-CgbjsBxQ.js");
			const { deliverOutboundPayloadsInternal } = await import("./deliver-IssBKsv_.js");
			if (stopped) return;
...
function startPendingSessionDeliveryRuntime(params) {
	...
	const timer = setTimeout(() => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { deliverQueuedSessionDelivery, recoverPendingRestartContinuationDeliveries, settleQueuedSessionDelivery } = await import("./server-restart-sentinel-DW1Gbl7l.js");
			if (stopped) return;
