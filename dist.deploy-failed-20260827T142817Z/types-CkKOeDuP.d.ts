import { H as QueueMode, M as TtsAutoMode, it as ChatType, pt as FastMode } from "./types.openclaw-CpYrAZv3.js";
...
type MainRestartRecoveryState = {
  ...
  chargedAttempts: number; /** Private safe token for one recovered outer turn; raw identity refs never enter session state. */
  executionIdentity?: {
    tokenVersion: 1;
    contextId: string;
    executionId: string;
  ...
  tombstone?: {
    reason: string; /** Durable successor returned when an explicit rollover request is retried. */
...
type SessionCompactionCheckpoint = {
  ...
  postCompaction: SessionCompactionTranscriptReference; /** Harness ownership generation that produced this checkpoint. */
  agentHarnessEpoch?: string; /** Harness that owned canonical context when this checkpoint was created. */
  agentHarnessId?: string;
};
type SessionAgentHarnessMigration = {
  schemaVersion: 1;
  migrationId: string;
...
type SessionEntryCore = SessionRestartRecoveryState & SessionEntryProvenance & {
   ...
   */
  archivedBy?: SessionCreatedActor | SessionRotationArchiveActor; /** Timestamp (ms) when the session was pinned for quick access. */
  ...
  projectId?: string; /** Explicit parent session linkage for dashboard-created child sessions. */
  parentSessionKey?: string; /** Exact parent incarnation captured when this child was created. */
  parentSessionId?: string; /** How this session node came to exist; written once and retained across sessionId rotations. */
   ...
   */
  rotationEpoch?: number; /** Turns served within the current rotation epoch (admission-bumped). */
  rotationTurnCount?: number; /** Timestamp (ms) when the session rotated to the next epoch key. */
  lastRotationAt?: number; /** Stable first-run start time for subagent sessions, persisted after completion. */
  ...
  status?: SessionRunStatus; /** Compact user-facing reason for the latest failed or timed-out run. */
  lastRunError?: string;
  /**
   ...
   */
  totalTokensFresh?: boolean; /** Version 1 records totalTokens as the current prompt/context snapshot only. */
   ...
   */
  agentHarnessEpoch?: string; /** Stable per-harness context lanes; switching harnesses does not discard a warm lane. */
  agentHarnessLaneEpochs?: Record<string, string>; /** Durable admission fence for an explicit cross-harness migration saga. */
...
type InternalSessionEntryCore = SessionEntryCore & {
  /** Run that owns the current non-terminal Gateway lifecycle projection. */lifecycleRunId?: string; /** Run admitted by the session lane; overwritten at admission and checked by transcript writes. */
...
//#endregion
export { ChannelId as C, SessionRunStatus as S, ChannelRouteRef as _, SessionEntry as a, SessionsCompanionAskResult as b, SessionSystemPromptReport as c, SessionCreatedVia as d, HookExternalContentSource as f, Skill as g, DeliveryContext as h, SessionContextBudgetStatus as i, SessionToolOverrides as l, SourceReplyDeliveryMode as m, GroupKeyResolution as n, SessionPluginJsonValue as o, SessionRestartRecoveryState as p, InternalSessionEntry as r, SessionScope as s, CliSessionBinding as t, SessionCreatedActor as u, CronScheduledToolPolicy as v, ChatChannelId as w, SessionsCompanionStateResult as x, SessionObserverDigest as y };