...
import { t as CanvasToolSchema } from "../../tool-schema-gFKvw4Ip.js";
...
function createLazyCanvasTool(params) {
	const loadTool = createLazyRuntimeModule(() => import("../../tool-CxZXumeh.js").then(({ createCanvasTool }) => createCanvasTool({
...
var canvas_default = definePluginEntry({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			const httpRouteHandlerLoader = createLazyRuntimeModule(() => import("../../http-route-DmCMd-Xg.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler({
				config: api.config,
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-v0BaozOS.js");
