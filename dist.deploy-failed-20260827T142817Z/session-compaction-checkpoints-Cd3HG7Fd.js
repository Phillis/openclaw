...
import { r as isCompactionCheckpointTranscriptFileName } from "./artifacts-Cg2BoGvO.js";
...
import { E as readSessionEntryRow, N as writeSessionEntry, k as readSessionIdentitySnapshot } from "./targets-BzJLDErS.js";
import { t as collectSessionEntryLookupKeys } from "./store-entry-BgSA4iwU.js";
import { n as parseSqliteSessionFileMarker } from "./legacy-sqlite-marker-COPKCuIN.js";
import { a as normalizeSqliteSessionKey, f as runExclusiveSqliteSessionWrite, i as getSessionKysely, p as toDatabaseOptions, r as formatSqliteSessionReferenceForScope, s as resolveSqliteScope } from "./session-accessor.sqlite-scope-CyEaWvgy.js";
import { x as scanSessionTranscriptTree } from "./session-transcript-index-U6HbS8-N.js";
import { p as emitCommittedSessionIdentityDiff } from "./session-accessor.sqlite-lifecycle-DM-jpNkz.js";
import { n as appendTranscriptEventsInTransaction, o as readTranscriptIdentityByEventId, p as createSessionTranscriptHeader, x as loadTranscriptEventsSync } from "./session-accessor.sqlite-transcript-store-DmssQj1u.js";
import { C as readFileRangeAsync, x as streamSessionTranscriptLines } from "./session-accessor.sqlite-canonical-repair--Uu8TNYz.js";
...
function branchSqliteCompactionCheckpointSessionInTransaction(database, params) {
	...
	if (currentEntry.sessionId !== params.expectedState.sessionId || currentEntry.lifecycleRevision !== params.expectedState.lifecycleRevision) return { status: "conflict" };
...
async function persistSessionCompactionCheckpoint(params) {
	...
	if (!await updateSessionEntry({
		...
		if (existing.agentHarnessMigration) {
			log.warn("skipping compaction checkpoint persist: harness migration in progress", { sessionKey: params.sessionKey });
			return null;
		}
		const activeHarnessId = params.agentHarnessId?.trim() ?? existing.agentHarnessId?.trim();
		if (existing.modelSelectionLocked === true && activeHarnessId && existing.agentHarnessId?.trim() !== activeHarnessId) {
			log.warn("skipping compaction checkpoint persist: locked harness owner changed", { sessionKey: params.sessionKey });
			return null;
		}
		const activeHarnessEpoch = activeHarnessId ? existing.agentHarnessLaneEpochs?.[activeHarnessId] ?? (existing.agentHarnessId?.trim() === activeHarnessId ? existing.agentHarnessEpoch : void 0) : void 0;
		checkpoint.agentHarnessId = activeHarnessId;
		checkpoint.agentHarnessEpoch = activeHarnessEpoch;
		const nextLaneEpoch = activeHarnessId ? randomUUID() : void 0;
