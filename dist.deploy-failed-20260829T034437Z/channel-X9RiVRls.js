...
import { c as parseAgentSessionKey, f as parseThreadSessionSuffix } from "./session-key-utils-Di3FvABa.js";
...
import { _ as readToolStringParam, i as createActionGate, p as readPositiveIntegerParam, t as ToolAuthorizationError } from "./common-CI1GnPjt.js";
...
import { s as createScopedDmSecurityResolver, t as adaptScopedAccountAccessor } from "./channel-config-helpers-C7An4wuC.js";
...
import { n as resolveApprovalRequestSessionConversation } from "./exec-approval-session-target-D17t9qJL.js";
import { r as createChannelNativeOriginTargetResolver } from "./approval-native-helpers-WU9kyZrx.js";
import { a as splitChannelApprovalCapability, i as createChannelApprovalCapability, n as createApproverRestrictedNativeApprovalCapability } from "./approval-delivery-helpers-uMzeSIy7.js";
...
import { n as createRuntimeOutboundDelegates, t as createRuntimeDirectoryLiveAdapter } from "./runtime-forwarders-DFFxee2-.js";
...
import { c as collectStatusIssuesFromLastError, d as createDefaultChannelRuntimeState, i as buildProbeChannelStatusSummary, u as createComputedAccountStatusAdapter } from "./status-helpers-CopMHs_f.js";
...
import { M as createConditionalWarningCollector, h as resolveScopeToolsPolicy, k as createAllowlistProviderOpenWarningCollector, m as resolveScopeRequireMention } from "./channel-policy-BG3-cCKG.js";
...
import { n as requiresExplicitMatrixDefaultAccount } from "./account-selection-DysKVyrG.js";
import { a as resolveMatrixAccountConfig } from "./account-config-C8YbJ_S9.js";
import { i as resolveMatrixAccount, r as resolveDefaultMatrixAccountId, t as listMatrixAccountIds } from "./accounts-CePJXe0h.js";
import { n as normalizeMatrixUserId } from "./allowlist-Cbcizppf.js";
import { t as normalizeMatrixApproverId } from "./approval-ids-DPAIXCWD.js";
import { a as isMatrixExecApprovalAuthorizedSender, c as shouldHandleMatrixApprovalRequest, d as matrixApprovalAuth, i as isMatrixApprovalClientEnabled, l as shouldSuppressLocalMatrixExecApprovalPrompt, n as getMatrixExecApprovalApprovers, o as isMatrixExecApprovalClientEnabled, r as isMatrixAnyApprovalClientEnabled, s as resolveMatrixExecApprovalTarget, t as getMatrixApprovalApprovers, u as getMatrixApprovalAuthApprovers } from "./exec-approvals-j973h9lV.js";
import { a as resolveMatrixDirectUserId, i as normalizeMatrixResolvableTarget, n as isMatrixRoomId, o as resolveMatrixTargetIdentity, r as normalizeMatrixMessagingTarget, t as isMatrixQualifiedUserId } from "./target-ids-DEUqtLsd.js";
import { t as matrixPluginBase } from "./channel.setup-D3MbyosQ.js";
import { n as normalizeCompatibilityConfig, t as legacyConfigRules } from "./doctor-contract-BUipuL2s.js";
import { r as resolveMatrixRoomScopePath, t as buildMatrixRoomScopeTree } from "./rooms-BjNS8Akg.js";
import { t as resolveMatrixInboundRoute } from "./route-Dt3y7UQW.js";
import { f as setMatrixThreadBindingMaxAgeBySessionKey, u as setMatrixThreadBindingIdleTimeoutBySessionKey } from "./thread-bindings-shared-C60gODTa.js";
import { n as collectRuntimeConfigAssignments, r as secretTargetRegistryEntries } from "./secret-contract-Cp_fiStw.js";
import { t as resolveMatrixStoredSessionMeta } from "./session-store-metadata-WsS_5iju.js";
import { n as resolveMatrixInboundConversation, t as defaultTopLevelPlacement } from "./thread-binding-api-D5wcic3R.js";
...
const matrixMessageActions = {
	...
	handleAction: async (ctx) => {
		...
		const resolveRoomId = () => readToolStringParam(params, "roomId") ?? readToolStringParam(params, "channelId") ?? readToolStringParam(params, "to", { required: true });
		if (action === "send") {
			const to = readToolStringParam(params, "to", { required: true });
			const mediaUrl = readToolStringParam(params, "media", { trim: false }) ?? readToolStringParam(params, "mediaUrl", { trim: false }) ?? readToolStringParam(params, "filePath", { trim: false }) ?? readToolStringParam(params, "path", { trim: false });
		...
		if (action === "emoji-list") {
			const roomId = readToolStringParam(params, "roomId") ?? readToolStringParam(params, "channelId") ?? readToolStringParam(params, "to") ?? (ctx.toolContext?.currentChannelProvider === "matrix" ? ctx.toolContext.currentChannelId : void 0);
		...
		if (action === "read") {
			...
			return await dispatch({
				...
				threadId: readToolStringParam(params, "threadId")
			});
		}
		if (action === "edit") {
			const messageId = readToolStringParam(params, "messageId", { required: true });
		...
		if (action === "permissions") {
			...
			const operation = normalizeLowercaseStringOrEmpty(readToolStringParam(params, "operation") ?? readToolStringParam(params, "mode") ?? "verification-list");
...
const matrixNativeApprovalCapability = createApproverRestrictedNativeApprovalCapability({
	...
	nativeRuntime: createLazyChannelApprovalNativeRuntimeAdapter({
		...
		load: async () => (await import("./approval-handler.runtime-A8jIYDYV.js")).matrixApprovalNativeRuntime
	})
});
...
function resolveMatrixConversationRouteOwner(params) {
	...
	const result = resolveMatrixInboundRoute({
		...
		isDirectMessage,
		dmSessionScope: resolveMatrixAccount({
			cfg,
			accountId
		}).config.dm?.sessionScope,
		threadId: conversation.threadId,
		resolveAgentRoute
	});
	if (!result.bindingOwnerAvailable) return { kind: "unavailable" };
	if (result.runtimeBindingId && !parseAgentSessionKey(result.route.sessionKey)?.agentId) return null;
	return {
		kind: "agent",
...
const loadMatrixChannelRuntime = createLazyRuntimeNamedExport(() => import("./channel.runtime-BW76BbeX.js"), "matrixChannelRuntime");
const loadMatrixDoctorModule = createLazyRuntimeModule(() => import("./doctor-BkwHB2ox.js"));
...
function collectMatrixSecurityWarningsForAccount(params) {
	...
	if (params.account.accountId !== "default" || params.account.config.autoJoin !== "always") return findings;
...
const matrixPlugin = createChatChannelPlugin({
	base: {
		...
		gateway: { startAccount: async (ctx) => {
			...
			try {
				monitorMatrixProvider = (await import("./monitor-B6TCJQYf.js")).monitorMatrixProvider;
			} finally {
				releaseLock();
			}
