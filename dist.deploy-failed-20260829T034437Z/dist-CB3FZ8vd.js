...
import { t as require_src$1 } from "./src-D0deKpII.js";
import { i as require_axios, n as require_lib$2, r as require_dist$3, t as require_dist$2 } from "./dist-BaLl2ahf.js";
import { t as require_Reflect } from "./Reflect-CrNmiFQu.js";
import { t as require_jws } from "./jws-B3OzQhTV.js";
...
var require_activity$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var ActivityContext = class {
		...
		constructor(value) {
			...
			if (rest.activity.type === "message") rest.activity = teams_api_1.MessageActivity.from(rest.activity);
			if (rest.activity.type === "messageUpdate") rest.activity = teams_api_1.MessageUpdateActivity.from(rest.activity);
			if (rest.activity.type === "messageDelete") rest.activity = teams_api_1.MessageDeleteActivity.from(rest.activity);
			if (rest.activity.type === "typing") rest.activity = teams_api_1.TypingActivity.from(rest.activity);
		...
		async signin(options) {
			...
			const recipient = this.activity.conversation.isGroup === true ? {
				...this.activity.from,
				isTargeted: true
			} : this.activity.from;
			const tokenExchangeResource = isChannel ? void 0 : resource.tokenExchangeResource;
			await this.send(overrideSignInActivity?.(tokenExchangeResource, resource.tokenPostResource, resource.signInLink) ?? {
				type: "message",
				recipient,
	...
	var ActivityProcessor = class {
		...
		*/
		async process(event) {
			...
			let userToken;
			if (this.options.shouldFetchUserToken()) try {
	...
	var HttpServer = class {
		...
		*/
		...
		constructor(adapter, options) {
			...
			this.dangerouslyAllowUnauthenticatedRequests = options.dangerouslyAllowUnauthenticatedRequests ?? false;
		}
		/**
		...
		*/
		async initialize(deps) {
			...
			if (!this.credentials && !this.dangerouslyAllowUnauthenticatedRequests) this.logger.warn("No credentials configured and dangerouslyAllowUnauthenticatedRequests is not enabled. All incoming requests will be rejected. Configure client authentication to securely receive messages, or set dangerouslyAllowUnauthenticatedRequests: true for local development.");
			if (this.credentials && !this.dangerouslyAllowUnauthenticatedRequests) this.serviceTokenValidator = new service_token_validator_1.ServiceTokenValidator(this.credentials.clientId, this.credentials.tenantId, void 0, this.logger, deps.cloud);
			else if (!this.credentials && this.dangerouslyAllowUnauthenticatedRequests) this.logger.warn(`No credentials configured (CLIENT_ID / CLIENT_SECRET / TENANT_ID), but dangerouslyAllowUnauthenticatedRequests is enabled. Bot will accept unauthenticated requests on ${this._messagingEndpoint}.`);
			else if (this.credentials && this.dangerouslyAllowUnauthenticatedRequests) this.logger.warn(`Credentials are configured, but dangerouslyAllowUnauthenticatedRequests is enabled. Bot will bypass Teams service token validation on ${this._messagingEndpoint}.`);
		...
		*/
		async authorize(headers, body) {
			if (this.dangerouslyAllowUnauthenticatedRequests) return {
				success: true,
				token: {
					appId: "",
					from: "azure",
...
//#endregion
//#region node_modules/@microsoft/teams.apps/dist/routes/invoke/widget-calltool.js
var require_widget_calltool = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
}));
//#endregion
...
var require_invoke = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	__exportStar(require_widget_calltool(), exports);
}));
//#endregion
...
//#endregion
//#region node_modules/@microsoft/teams.apps/dist/utils/env.js
var require_env = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.getBooleanEnvValue = getBooleanEnvValue;
	...
	var App = class {
		...
		*/
		...
		constructor(options = {}) {
			...
			if (dangerouslyAllowUnauthenticatedRequests === void 0 && this.options.skipAuth !== void 0) {
				this.log.warn("[DEPRECATED] skipAuth is deprecated. Use dangerouslyAllowUnauthenticatedRequests instead.");
				dangerouslyAllowUnauthenticatedRequests = this.options.skipAuth;
			}
			if (dangerouslyAllowUnauthenticatedRequests === void 0) {
				const unauthenticatedRequestsEnvValue = (0, env_1.getBooleanEnvValue)("DANGEROUSLY_ALLOW_UNAUTHENTICATED_REQUESTS");
				if (unauthenticatedRequestsEnvValue !== void 0) {
					this.log.warn("DANGEROUSLY_ALLOW_UNAUTHENTICATED_REQUESTS is set. Unauthenticated request behavior is configured by the environment.");
		...
		*/
		shouldFetchUserToken() {
			const explicit = this.options.oauth?.fetchUserToken;
			if (explicit !== void 0) return explicit;
			return this.options.oauth?.defaultConnectionName !== void 0;
		}
	};
	exports.App = App;
}));
...
//#endregion
//#region node_modules/@microsoft/teams.apps/dist/utils/html-widget.js
var require_html_widget = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.injectWidgetProtocol = injectWidgetProtocol;
	exports.buildHtmlWidgetMarkdown = buildHtmlWidgetMarkdown;
	exports.buildHtmlWidgetMessage = buildHtmlWidgetMessage;
	exports.validateSecurityPolicy = validateSecurityPolicy;
	exports.tryGetWidgetModelContext = tryGetWidgetModelContext;
	/**
	...
	*/
	function validateHtmlWidgetPayload(payload) {
		if (!payload.name.trim()) throw new Error("HTML widget payload requires a non-empty \"name\" field.");
		if (!payload.html.trim()) throw new Error("HTML widget payload requires a non-empty \"html\" field.");
		...
		if (!domainUrl || domainUrl.protocol !== "https:" || !domainUrl.hostname) throw new Error("HTML widget payload requires \"domain\" to be a valid URL starting with \"https://\".");
	}
	/**
	...
	* Diagnostic: ExperimentalTeamsHtmlWidget
	*/
	function injectWidgetProtocol(html, options) {
		if (/["']?method["']?\s*:\s*["']ui\/initialize["']/.test(html)) return html;
	...
	* Diagnostic: ExperimentalTeamsHtmlWidget
	*/
	function buildHtmlWidgetMarkdown(payload, options) {
		const resolved = {
			type: "widget/mcp-ui",
			...payload
		};
		...
		const injectedPayload = {
			...resolved,
			html: injectWidgetProtocol(resolved.html, {
		...
		const parts = [];
		if (options?.before) parts.push(options.before, "");
		parts.push("```html-widget", json, "```");
		if (options?.after) parts.push("", options.after);
		return parts.join("\n");
	}
	/**
	...
	* Diagnostic: ExperimentalTeamsHtmlWidget
	*/
	function buildHtmlWidgetMessage(payload, options) {
		return {
			type: "message",
	...
	*/
	function extractOrigin(url) {
		...
		if (!trimmed || trimmed.startsWith("data:") || trimmed.startsWith("#") || trimmed.startsWith("blob:")) return null;
	...
	* Diagnostic: ExperimentalTeamsHtmlWidget
	*/
	function validateSecurityPolicy(html, policy) {
		const warnings = [];
		function findTags(tagName) {
			...
			while (pos < lower.length) {
				...
				if (afterTag < lower.length && lower[afterTag] !== " " && lower[afterTag] !== "	" && lower[afterTag] !== "\n" && lower[afterTag] !== "\r" && lower[afterTag] !== ">" && lower[afterTag] !== "/") {
					pos = afterTag;
					continue;
				}
		...
		for (const { tag, attrRegex, source } of [
			{
				tag: "script",
				attrRegex: /src=["']([^"']+)["']/i,
				source: "<script src>"
			...
			if (attrMatch) {
				...
				if (origin && !isOriginAllowed(origin, policy.resourceDomains ?? [])) warnings.push({
					url: attrMatch[1],
					source,
		...
		for (const { regex, source } of [
			...
			while ((match = regex.exec(html)) !== null) {
				const origin = extractOrigin(match[1]);
				if (origin && !isOriginAllowed(origin, policy.connectDomains ?? [])) warnings.push({
					url: match[1],
					source,
		...
		for (const tagStr of findTags("iframe")) {
			...
			if (attrMatch) {
				const origin = extractOrigin(attrMatch[1]);
				if (origin && !isOriginAllowed(origin, policy.frameDomains ?? [])) warnings.push({
					url: attrMatch[1],
					source: "<iframe src>",
		...
		for (const tagStr of findTags("base")) {
			...
			if (attrMatch) {
				const origin = extractOrigin(attrMatch[1]);
				if (origin && !isOriginAllowed(origin, policy.baseUriDomains ?? [])) warnings.push({
					url: attrMatch[1],
					source: "<base href>",
	...
	* Diagnostic: ExperimentalTeamsHtmlWidget
	*/
	function tryGetWidgetModelContext(activity) {
		const value = activity?.value;
		if (!value || typeof value !== "object") return;
		const candidate = value.type === "widgetModelContext" && value.data && typeof value.data === "object" ? value.data : value;
		if (candidate.method !== "ui/update-model-context") return;
...
var require_dist = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.tryGetWidgetModelContext = exports.validateSecurityPolicy = exports.injectWidgetProtocol = exports.buildHtmlWidgetMessage = exports.buildHtmlWidgetMarkdown = exports.toThreadedConversationId = void 0;
	...
	var html_widget_1 = require_html_widget();
	Object.defineProperty(exports, "buildHtmlWidgetMarkdown", {
		enumerable: true,
		get: function() {
			return html_widget_1.buildHtmlWidgetMarkdown;
		}
	});
	Object.defineProperty(exports, "buildHtmlWidgetMessage", {
		enumerable: true,
		get: function() {
			return html_widget_1.buildHtmlWidgetMessage;
		}
	});
	Object.defineProperty(exports, "injectWidgetProtocol", {
		enumerable: true,
		get: function() {
			return html_widget_1.injectWidgetProtocol;
		}
	});
	Object.defineProperty(exports, "validateSecurityPolicy", {
		enumerable: true,
		get: function() {
			return html_widget_1.validateSecurityPolicy;
		}
	});
	Object.defineProperty(exports, "tryGetWidgetModelContext", {
		enumerable: true,
		get: function() {
			return html_widget_1.tryGetWidgetModelContext;
		}
	});
}));
//#endregion
