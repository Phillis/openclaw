...
import { S as getTaskById, T as listTaskRecordPage } from "./task-registry-DzN8snH1.js";
import "./runtime-internal-KZAce0-2.js";
import { n as resolveRequestedSessionAgentId } from "./session-request-agent-C9E8iDY4.js";
import { i as retrySubagentCompletionDelivery, n as dismissSubagentCompletionDelivery } from "./subagent-completion-delivery-D0JLJvTT.js";
...
const tasksHandlers = {
	...
	"tasks.dismiss": async ({ params, respond, context, client }) => {
		...
		const { discardSubagentTerminalDelivery } = await import("./subagent-registry-XilIqabO.js");
		const results = [];
		const cfg = context.getRuntimeConfig();
		for (const taskId of params.taskIds) {
			const task = getTaskById(taskId);
