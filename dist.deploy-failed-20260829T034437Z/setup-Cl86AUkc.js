...
import { rt as ConfigMutationConflictError } from "./io-DlN5njvP.js";
import { f as hasResolvedRosterBeforeMigrations } from "./provider-model-route-D-FYx-DP.js";
import { n as createMergePatch, t as applyMergePatch } from "./merge-patch-BukSB2Pq.js";
import "./config-B2bSneS2.js";
import { r as resolveGatewayProbeAuthSafeWithSecretInputs } from "./probe-auth-26arg1n8.js";
import { n as t } from "./i18n-BQpjgFU-.js";
import { s as resolveOnboardingSetupTarget } from "./onboard-agent-target--5lBUg6J.js";
import { r as buildPluginCompatibilitySnapshotNotices } from "./status-DoE4_lNj.js";
import { t as formatPluginCompatibilityNotice } from "./status-compatibility-DdQ1VWdc.js";
import { t as runWizardWithPromptNavigation } from "./navigation-prompter-D98bFrMO.js";
...
import { n as showSessionMigrationWarnings, t as promptFirstOnboardingAgent } from "./onboard-first-agent-XfvDUQOA.js";
import { a as runSetupModelAuthStep, i as offerLiveModelVerification, n as listSetupMigrationOptions, r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-a1hU0daF.js";
import { n as SetupMigrationTargetChangedError, t as SetupMigrationFreshnessError } from "./setup.migration-snapshot-De038w5n.js";
...
const loadConfigLoggingModule = createLazyRuntimeModule(() => import("./logging-sgSFUUa4.js"));
const loadOnboardConfigModule = createLazyRuntimeModule(() => import("./onboard-config-BG5OGrQW.js"));
...
async function runSetupWizardOnce(initialOpts, runtimeInput, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-kk_BLYXI.js");
	...
	const remoteOnboard = seededRemoteUrl ? await import("./onboard-remote-BFWDAuu1.js") : null;
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = remoteOnboard ?? await import("./onboard-remote-BFWDAuu1.js");
		...
		let nextConfig = await promptRemoteGatewayConfig(remoteSeedConfig, prompter, {
			...
			...opts.remoteUrl !== void 0 && storedRemoteUrl ? { edgeAuthOriginUrl: storedRemoteUrl } : {}
		});
	...
	if (opts.authChoice === void 0) {
		const { inferAuthChoiceFromFlags } = await import("./auth-choice-inference-DJ9CNQFb.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CXQngokL.js");
	...
	if (opts.skipChannels) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-jJLrzpUe.js");
		const { createChannelSetupTransaction, setupChannels } = await import("./onboard-channels-BA395Ry6.js");
		...
		nextConfig = await channelSetup.commit(nextConfig, async (config) => await writeSetupConfigFile(config, { allowConfigSizeDrop: false }));
	}
	if (opts.skipChannels) nextConfig = await writeSetupConfigFile(nextConfig, { allowConfigSizeDrop: false });
	...
	if (!usedImportFlow) {
		const { runSetupMemoryImportStep } = await import("./setup.memory-import-BjP8hruo.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { runSearchSetupFlow } = await import("./search-setup-BC294Q6C.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-LAvKEyvz.js");
	...
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-w-oyUyiV.js");
		...
		const { setupAppRecommendations } = await import("./setup.app-recommendations-BXCQ5h-W.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-O_7h-DUp.js");
	...
	const { finalizeSetupWizard } = await import("./setup.finalize-BqsFdT3F.js");
