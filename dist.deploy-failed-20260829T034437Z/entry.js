...
import { r as getCommandPositionalsWithRootOptions } from "./cli-root-options-CpQG4BXe.js";
import { n as getCommandPathWithRootOptions, u as isRootHelpInvocation } from "./argv-CCdO9MSu.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-DXuFeGZ6.js";
import { r as defaultRuntime } from "./runtime-LRpY2Icg.js";
import { n as runCliWithExitFinalization, t as requestExitAfterOneShotOutput } from "./one-shot-exit-CvLNCpcm.js";
import { n as installDistEsmResolveFastPath, t as tryHandleRootVersionFastPath } from "./entry.version-fast-path-DY2loDV4.js";
import { n as parseCliContainerArgs, r as resolveCliContainerTarget } from "./container-target-DeeG-3q9.js";
...
function shouldSkipRespawnForArgv(argv, platform = process.platform) {
	const invocation = resolveCliArgvInvocation(argv);
	return invocation.hasHelpOrVersion || isInteractiveTtyCommandArgv(argv) || isForegroundGmailRunArgv(argv) || shouldKeepNativeHookRelayInProcess(argv, platform) || invocation.primary === "gateway" && isForegroundGatewayRunArgv(argv);
}
...
async function writeCapturedCliArgumentError(message) {
	...
	const { enableConsoleCapture } = await import("./logging-DomQW3r8.js");
	enableConsoleCapture();
	const [{ formatCliJsonFailure }, { isJsonOutputModeActive }] = await Promise.all([import("./failure-output-Dl5h2loz.js"), import("./json-output-mode-DyOm0Nzp.js")]);
	if (isJsonOutputModeActive(process$1.argv)) defaultRuntime.writeJson(formatCliJsonFailure(message));
...
async function writeCliDiagnosticBlock(message) {
	...
	loadCliDotEnv({ quiet: true });
	await configureGatewayStartupTraceConsoleFormatting(gatewayEntryStartupTrace);
	const { formatConsoleDiagnosticBlock } = await import("./json-console-line-D2mHj3RF.js");
...
async function flushEntryStartupTraceForEarlyReturn(argv) {
	...
	const { loadCliDotEnvForEarlyDiagnostic } = await import("./dotenv-Dz6_8AD0.js");
...
const gatewayEntryStartupTrace = createGatewayDispatchStartupTrace(process$1.argv, "entry");
if (!isMainModule({
	...
	const { assertSupportedRuntime, isCurrentRuntimeSupported } = await import("./runtime-guard-D-guPp5S.js");
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-D7wfJUUo.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
	...
	await runCliWithExitFinalization({
		run: async () => {
			if (isNativeHookRelayArgv(argv) && !argv.includes("--help") && !argv.includes("-h")) {
				const { runNativeHookRelayCliFromArgv } = await import("./native-hook-relay-cli-DwGKNPAu.js");
		...
		onError: async (error) => {
			...
			const { enableConsoleCapture } = await import("./logging-DomQW3r8.js");
			enableConsoleCapture();
			const [{ formatCliFailureLines, formatCliJsonFailure }, { isJsonOutputModeActive }] = await Promise.all([import("./failure-output-Dl5h2loz.js"), import("./json-output-mode-DyOm0Nzp.js")]);
