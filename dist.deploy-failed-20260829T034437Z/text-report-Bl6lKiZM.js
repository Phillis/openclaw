...
import { a as buildPairingConnectRecoveryTitle, s as describePairingConnectRequirement } from "./connect-error-details-Dxf1zdDX.js";
...
import "./update-control-plane-sentinel-BujVAevG.js";
import { n as formatTimeAgo } from "./format-relative-DerIyym2.js";
import { r as readBackupFreshness, t as buildBackupStatusValue } from "./backup-health-Dx0sUrEb.js";
import "./format-DzOpGxRr.js";
import { u as buildStatusOverviewRowsFromSurface } from "./status-runtime-shared-CRkpabcE.js";
...
function buildStatusHeartbeatValue(params) {
	const parts = params.summary.heartbeat.agents.map((agent) => {
		...
		if (agent.waitingForRoute) return `${agent.every} (${agent.agentId}; waiting for delivery route — set commands.ownerAllowFrom or channel allowFrom, or heartbeat.target)`;
...
function buildStatusHealthRows(params) {
	...
	for (const line of params.formatHealthChannelLines(params.health, { accountMode: "all" })) {
		...
		const status = normalized === "healthy" || normalized.startsWith("ok") || normalized.startsWith("configured") ? params.ok("OK") : normalized.startsWith("not configured") ? params.muted("OFF") : normalized.startsWith("linked") ? params.ok("LINKED") : normalized.startsWith("not linked") ? params.warn("UNLINKED") : params.warn("WARN");
		rows.push({
			Item: item,
			Status: status,
			Detail: detail
...
function buildStatusCommandOverviewRows(params) {
	...
	const updatesDisabled = params.surface.cfg.update?.checkOnStart === false || isTruthyEnvValue(params.env.OPENCLAW_NO_AUTO_UPDATE) || resolveIsNixMode(params.env);
