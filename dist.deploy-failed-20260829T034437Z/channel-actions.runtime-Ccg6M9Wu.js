...
import { _ as readToolStringParam, h as readStringArrayParam, p as readPositiveIntegerParam } from "./common-CI1GnPjt.js";
import { d as normalizeLegacyInteractiveReply, f as normalizeMessagePresentation, v as renderMessagePresentationFallbackText } from "./payload-C7E4iMOo.js";
...
import { n as buildDiscordPresentationComponents, t as buildDiscordInteractiveComponents } from "./shared-interactive-CYeMGjmB.js";
import { s as coerceDiscordComponentParam } from "./components-CN5PDDJ9.js";
import { n as resolveDiscordChannelId, t as parseDiscordTarget } from "./target-parsing-CEpBARoV.js";
import { a as readDiscordAutoArchiveDurationParam, t as handleDiscordAction } from "./runtime-CiER80Iq.js";
...
import { r as isDiscordComponentSpecWithinMessageLimit, t as DISCORD_PRESENTATION_CAPABILITIES } from "./outbound-components-jBE2ZO6j.js";
import { t as tryHandleDiscordMessageActionGuildAdmin } from "./handle-action.guild-admin-oOQFyS5l.js";
import { n as notifyDiscordActiveTurnThreadCreated, r as notifyDiscordActiveTurnThreadReplyDelivered } from "./active-turn-thread-route-CdYTbCX-.js";
...
async function handleDiscordMessageAction(ctx) {
	...
	if (action === "send") {
		...
		const generatedPresentationComponents = buildDiscordPresentationComponents(presentation ? adaptMessagePresentationForChannel({
			presentation,
	...
	if (action === "upload-file") {
		...
		if (!mediaUrl) {
			if (readToolStringParam(params, "buffer", { trim: false })) throw new Error("Use action: \"send\" for base64 buffer attachments; upload-file requires filePath, path, or media.");
			throw new Error("upload-file requires filePath, path, or media.");
		}
		const content = readToolStringParam(params, "message", { allowEmpty: true }) ?? readToolStringParam(params, "content", { allowEmpty: true }) ?? readToolStringParam(params, "caption", { allowEmpty: true });
		const filename = readToolStringParam(params, "filename");
		const replyTo = readToolStringParam(params, "replyTo");
		const silent = readBooleanParam(params, "silent") === true;
		const suppressEmbeds = readBooleanParam(params, "suppressEmbeds");
