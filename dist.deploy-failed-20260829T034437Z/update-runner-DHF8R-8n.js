...
import { _ as sortUniqueStrings, u as normalizeStringEntries, v as uniqueStrings } from "./string-normalization-e_fvmxMf.js";
...
import { o as parseSemver } from "./runtime-guard-xF0n8O8f.js";
...
import { n as applyPosixNpmScriptShellEnv, r as createNpmFreshnessBypassArgs, t as applyNpmFreshnessBypassEnv } from "./npm-install-env-DIasXTZP.js";
import { g as updateInstallRootsMatch, h as resolveUpdateInstallRoot, p as trimLogTail } from "./restart-sentinel-BabYlCrz.js";
import { a as resolveDevUpdateTargetRevision } from "./update-dev-target-BGSFRuDr.js";
import { t as applyPathPrepend } from "./path-prepend-Up5PqO7Z.js";
import { a as resolveControlUiAssetHealth, o as resolveControlUiDistIndexPathForRoot } from "./control-ui-assets-D92--EZN.js";
import { a as channelToNpmTag, c as isStableTag, n as DEV_BRANCH, r as EXTENDED_STABLE_TAG_UNSUPPORTED_REASON, s as isBetaTag, u as resolveDevUpstreamRefs } from "./update-channels-D2-WrHya.js";
import { n as readPackageName, r as readPackageVersion } from "./package-json-BoWJND-q.js";
import { a as resolveExtendedStablePackage, c as parsePackageOpenClawSchemaVersions, l as detectPackageManager, n as compareSemverStrings, u as resolveBunGlobalInstallOwner } from "./update-check-iQLkkUmm.js";
import { t as PACKAGE_POST_INSTALL_DOCTOR_ADVISORY } from "./update-doctor-result-D3HkeqjI.js";
...
async function collectRelativeFiles(rootDir, baseDir, rules, fsLimit, onDirectory) {
	...
	try {
		const rootStats = await fsLimit(() => fs$1.lstat(rootDir));
		...
		const entries = await fsLimit(() => fs$1.readdir(rootDir, { withFileTypes: true }));
...
function resolveNpmLifecyclePolicy(version) {
	...
	return parsed.major >= 12 || parsed.major === 11 && parsed.minor >= 16 ? "allow-scripts" : "unflagged";
}
...
function resolveNpmLifecyclePolicyGate(installTarget) {
	...
	if (policy === "unflagged" || policy === "allow-scripts") return {
		policy,
		error: null
	};
	return {
...
function globalInstallArgs(managerOrCommand, spec, pkgRoot, installPrefix, installCwd, npmLifecyclePolicy = "allow-scripts") {
	...
	return [
		...
		"i",
		"-g",
		...npmLifecyclePolicy === "allow-scripts" ? [resolveNpmInstallScriptsAllowFlag(spec, installCwd)] : [],
...
function packageUpdateFailure(failedStep, verifiedPackageRoot, steps = [failedStep]) {
	return {
		steps,
		verifiedPackageRoot,
		afterVersion: null,
...
async function cleanupStagedNpmInstall(stage) {
	if (stage) await removePathBestEffort(stage.prefix);
}
...
async function replaceNpmBinShims(params) {
	...
	const backupDir = await fs$1.mkdtemp(path.join(params.targetLayout.globalRoot, ".openclaw-shim-backup-"));
	...
	try {
		...
		for (const entry of shimEntries) {
			...
			backedUpEntries.push({
				name: entry,
				hadExisting
			});
			if (hadExisting) await copyPathEntry(destination, path.join(backupDir, entry));
		}
		...
		for (const entry of backedUpEntries) {
			...
			await removePathBestEffort(destination);
			if (entry.hadExisting) await copyPathEntry(path.join(backupDir, entry.name), destination);
		}
		throw err;
	} finally {
...
async function activateStagedNpmPackageRoot(source, destination) {
	if (!(await fs$1.lstat(source)).isSymbolicLink()) {
	...
	const canonicalSource = await fs$1.realpath(source);
	await fs$1.symlink(canonicalSource, destination, process.platform === "win32" ? "junction" : void 0);
}
...
async function runGlobalPackageUpdateSteps(params) {
	...
	try {
		...
		if (npmPreflight.failedStep) return packageUpdateFailure(npmPreflight.failedStep, params.packageRoot ?? params.installTarget.packageRoot);
		...
		if (pnpmPreflight.failedStep) return packageUpdateFailure(pnpmPreflight.failedStep, params.packageRoot ?? params.installTarget.packageRoot);
		...
		const effectiveInstallEnv = params.installTarget.manager === "bun" && params.installTarget.globalRoot ? {
		...
		if (preparedInstall.failedStep) return packageUpdateFailure(preparedInstall.failedStep, params.packageRoot ?? null);
		const steps = [];
		...
		if (preparedSpec.failedStep) return packageUpdateFailure(preparedSpec.failedStep, params.packageRoot ?? null, steps);
		...
		const updateStep = await params.runStep({
			name: "global update",
			argv: globalInstallArgs(installCommandTarget, updateInstallSpec, void 0, installLocation, preparedSpec.installCwd, npmPreflight.policy ?? void 0),
		...
		if (updateStep.exitCode !== 0) {
			...
			if (preparedFallbackInstall.failedStep) {
				...
				return packageUpdateFailure(preparedFallbackInstall.failedStep, params.packageRoot ?? null, steps);
			}
			const fallbackArgv = globalInstallFallbackArgs(stagedInstall?.installTarget ?? params.installTarget, preparedSpec.installSpec, void 0, stagedInstall?.prefix, preparedSpec.installCwd, npmPreflight.policy ?? void 0);
			if (fallbackArgv) {
		...
		if (finalInstallStep.exitCode === 0 && !stagedInstall && params.installTarget.manager === "pnpm" && verificationPackageRoot) {
			...
			if (hasInstallGuard || hasPendingLifecycle) {
				...
				for (const [name, relativeScript] of lifecycleScripts) {
					...
					if (lifecycleStep.exitCode !== 0) return packageUpdateFailure(lifecycleStep, verifiedPackageRoot, steps);
				}
				try {
...
function isSupersededInstallFailure(step, steps) {
	return step.name === "deps install" && steps.some((candidate) => candidate.name === "deps install (ignore scripts)" && candidate.exitCode === 0);
}
function isPreflightCandidateFailure(step) {
	return /^preflight (?:checkout|package manager|deps install(?: \(ignore scripts\))?|build|config validate|lint) \(.+\)$/u.test(step.name);
}
...
async function testPreflightCandidates(params) {
	...
	for (const sha of params.candidates) {
		...
		try {
			...
			if ((await runStep(params.step(installName, installArgv, params.worktreeDir, installEnv))).exitCode !== 0) {
				sawOtherFailure = true;
				continue;
			}
			const runCandidateCheck = async (name, argv, env) => {
				return (await runStep(params.step(`preflight ${name} (${shortSha})`, argv, params.worktreeDir, env))).exitCode === 0;
			};
			const buildArgs = managerScriptArgs(manager.manager, "build");
			const buildEnv = resolveBuildEnv(manager.env, path.join(params.gitRoot, ".artifacts", "build-all-cache"));
			...
			if (!await runCandidateCheck("build", buildArgs, buildEnv) || !await runCandidateCheck("config validate", configArgs, manager.env) || shouldRunDevPreflightLint() && !await runCandidateCheck("lint", lintArgs, resolveDevPreflightLintEnv(manager.env))) {
				sawOtherFailure = true;
				continue;
			}
			selectedSha = sha;
...
async function runGitDevPreflight(params) {
	...
	if (devTargetRef) {
		...
		if (params.devTarget?.mode === "tracked") {
			if ((await runStep(params.step("tracked target ancestry", [
				"git",
				"-C",
				params.gitRoot,
				"merge-base",
			...
			], params.gitRoot))).exitCode !== 0) return {
				status: "error",
...
async function rebuildRolledBackGitRuntime(params) {
	...
	try {
		...
		const verified = commit === params.expectedSha && buildHead === params.expectedSha && runtimeHead === params.expectedSha && entryExists && uiHealth.kind === "ready";
		params.steps.push({
			name: "git rollback runtime verify",
			command: `verify rollback runtime ${params.expectedSha}`,
			cwd: params.gitRoot,
			...
			...verified ? {} : { stderrTail: `rollback runtime mismatch (build=${commit ?? "missing"}, buildStamp=${buildHead ?? "missing"}, runtimeStamp=${runtimeHead ?? "missing"}, entry=${entryExists}, ui=${uiHealth.kind})` }
		});
...
async function updateGitCheckout(params) {
	...
	if (manager.kind === "missing-required") return await rollbackError(manager.reason);
	try {
		const installEnv = resolveInstallEnv(manager.manager, manager.env);
		...
		if ((await resolveControlUiAssetHealth({ root: gitRoot })).kind !== "ready") {
			...
			if (repairedHealth.kind !== "ready") {
				...
				steps.push({
					...
					durationMs: 0,
					exitCode: 1,
					stderrTail: repairedHealth.kind === "incomplete" ? `missing startup asset ${repairedHealth.missingAsset} referenced by ${uiIndexPath}` : `missing ${uiIndexPath}`
				});
		...
		if (afterShaStep.exitCode !== 0) return await rollbackError("head-verification-failed");
		if (devTarget?.mode === "tracked" && devPreflight?.status === "ok" && afterShaStep.stdoutTail?.trim() !== devPreflight.selectedSha) return await rollbackError("target-sha-mismatch");
		return {
			...
			after: {
				...
				...!failedStep && devTarget?.mode === "tracked" ? { upstreamRef: devTarget.upstreamRef } : {}
			},
			steps,
			durationMs: Date.now() - startedAt
		};
...
async function resolveGitRoot(runCommand, candidates, timeoutMs, packageRoot) {
	for (const dir of candidates) {
		...
		if (root && (!packageRoot || updateInstallRootsMatch(root, packageRoot))) return root;
	}
	return null;
}
...
async function runGatewayUpdate(opts = {}) {
	...
	if (!gitRoot && pkgRoot) {
		...
		if (cwdRoot && updateInstallRootsMatch(cwdRoot, pkgRoot) && await looksLikeGitCheckout(cwdRoot)) gitRoot = resolveUpdateInstallRoot(cwdRoot);
	}
	...
	if (gitRoot && pkgRoot) return await updateGitCheckout({
		opts,
		gitRoot,
		runCommand,
		defaultCommandEnv,
...
function runGatewayUpdatePreflight(cwd, timeoutMs, devTarget) {
	const complete = /* @__PURE__ */ new Error("update-preflight-complete");
...
//#endregion
export { markPackagePostInstallDoctorAdvisory as a, createGlobalInstallEnv as c, resolveGlobalInstallSpec as d, resolveGlobalInstallTarget as f, resolveUpdateDoctorExecutionPolicy as i, detectGlobalInstallManagerByPresence as l, normalizePackageTagInput as m, runGatewayUpdatePreflight as n, runGlobalPackageUpdateSteps as o, resolveNpmLifecyclePolicyGate as p, resolveUpdateInstallSurface as r, canResolveRegistryVersionForPackageTarget as s, runGatewayUpdate as t, detectGlobalInstallManagerForRoot as u };
