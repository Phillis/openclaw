...
import { C as modelSupportsInput, c as hasConfiguredProviderModelRows, r as buildConfiguredModelCatalog } from "./model-selection-shared-I5TmV9jL.js";
...
import { o as resolveClaudeFable5ModelIdentity } from "./src-5i09w5fd.js";
...
const loadModelSuppression = createLazyPromise(() => import("./model-suppression-B5XhMLeE.js"));
const loadProviderApiKeyResolver = createLazyPromise(() => import("./models-config.providers.secrets-DLzt6FEm.js"));
...
function clearRouteBoundCatalogMetadata(entry) {
	const { contextWindow: _contextWindow, contextWindows: _contextWindows, contextWindowDefault: _contextWindowDefault, contextTokens: _contextTokens, reasoning: _reasoning, thinkingLevelMap: _thinkingLevelMap, input: _input, params: _params, compat: _compat, mediaInput: _mediaInput, ...routeNeutral } = entry;
	return routeNeutral;
}
...
function mergeCatalogEntries(models, entries, options) {
	...
	for (const entry of entries) {
		...
		if (existing) {
			const catalogRoute = options?.preserveBaseCompat ? options.catalogRoutes?.find((candidate) => catalogRouteVariantKey(candidate) === catalogRouteVariantKey(entry)) : void 0;
