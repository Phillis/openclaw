...
import { c as resolveUserPath, i as resolveOsHomeDir } from "./home-dir-BFvskzn8.js";
import "./path-guards-CQoZeoCG.js";
import { d as resolveConfigDir, t as CONFIG_DIR } from "./utils-Bw16L5tB.js";
...
import { a as normalizeSkillFilter, n as resolveEffectiveAgentSkillFilter, t as isSessionSkillEnabled } from "./agent-filter-BQrGxhsA.js";
...
import { o as createSyntheticSourceInfo } from "./skill-contract-6Z2EHE_Q.js";
import { i as resolveSkillManifestMetadata, n as resolveSkillInvocationPolicy, r as resolveSkillKey, t as parseSkillFrontmatter } from "./frontmatter-BUnBwW_N.js";
import { r as resolveSkillTelemetrySourceValue } from "./source-0ivX3dtK.js";
import { n as resolveAllowedSkillSymlinkTargetRealPaths, r as tryRealpath, t as findContainingAllowedSkillSymlinkTarget } from "./symlink-targets-BsV9JHNo.js";
import { r as getSkillsSnapshotVersion, t as bumpSkillsSnapshotVersion } from "./refresh-state-DHnXO3IV.js";
import { t as resolveBundledSkillsDir } from "./bundled-dir-BfHNzP7v.js";
import { n as readSkillFrontmatterSafe, t as loadSkillsFromDirSafe } from "./local-loader-DWyFL6N6.js";
import { l as shouldIncludeSkill, o as resolveBundledAllowlist } from "./config-axwakNc8.js";
import { r as resolveNodeIdFromNodeList } from "./node-resolve-Cxs-SER3.js";
import { n as redactConfigObject } from "./redact-snapshot-Cc1aNUFV.js";
import { n as resolvePluginSkillRootsFromMetadata, t as resolvePluginSkillRoots } from "./plugin-skills-C_zlbHBi.js";
...
function hasSkillFileCandidate(skillDir) {
	try {
...
function containsDiscoverableSkill(dir, opts) {
	...
	for (const candidate of queue) {
		...
		if (candidate.depth > 0 && hasSkillFileCandidate(candidate.dir)) return true;
...
function resolveNestedSkillsRoot(dir, opts) {
	...
	for (const candidate of queue) {
		if (!candidate) continue;
		if (hasSkillFileCandidate(candidate.dir)) return {
			baseDir: nested,
...
function resolveSkillFilePath(params) {
	...
	if (resolved || tryRealpath(params.candidatePath)) return resolved;
...
function discoverSkillCandidates(params) {
	...
	const baseDir = resolveNestedSkillsRoot(params.dir, { maxEntriesToScan: params.limits.maxCandidatesPerRoot }).baseDir;
	...
	if (path.resolve(baseDir) !== rootDir && hasSkillFileCandidate(rootDir)) {
		...
		if (configuredRootSkillRealPath) configuredRootCandidate = {
			skillDir: rootDir,
			skillDirRealPath: rootRealPath,
...
const reportedSkillCollisions = /* @__PURE__ */ new Map();
function normalizeWorkspaceSkillRoots(roots) {
	const agentWorkspaceDir = path.resolve(roots.agentWorkspaceDir);
	const executionSkillsDir = roots.executionSkillsDir ? path.resolve(roots.executionSkillsDir) : void 0;
...
function warnSkillPrecedenceCollision(winner, loser, workspaceDir) {
	...
	if (reportedSkillCollisions.has(collisionKey)) return;
	if (canonicalizePath(winner.filePath) === canonicalizePath(loser.filePath)) return;
...
function loadDiscoveredSkillRecords(params) {
	...
	const maxSkillsLoadedPerSource = Math.max(0, params.limits.maxSkillsLoadedPerSource);
	const loadCandidate = (candidate) => loadContainedSkillRecords({
		skillDir: candidate.skillDir,
		source: params.source,
...
function loadSkillEntries(workspaceDir, opts) {
	...
	const configuredCustodianAgentId = opts?.config ? tryResolveAmbientOwnerAgentId(opts.config) : void 0;
	const custodianAgentId = !workspaceOnly && opts?.agentId && configuredCustodianAgentId && normalizeAgentId(opts.agentId) === configuredCustodianAgentId ? configuredCustodianAgentId : void 0;
	...
	const loadSkills = (params) => loadDiscoveredSkillRecords({
		...
		rejectHardlinks: params.rejectHardlinks ?? shouldRejectHardlinkedPluginFiles({
	...
	const custodianSkillsDir = bundledSkillsDir && custodianAgentId ? path.join(path.dirname(bundledSkillsDir), CUSTODIAN_SKILLS_DIR_NAME) : void 0;
	const custodianSkills = custodianSkillsDir ? loadSkills({
		dir: custodianSkillsDir,
		source: "openclaw-custodian"
	}) : [];
	const extraSkills = [
		...extraDirs.flatMap((dir) => loadSkills({
			dir: resolveUserPath(dir),
			source: "openclaw-extra"
		})),
		...pluginSkillRoots.flatMap((root) => loadSkills({
			dir: root.dir,
			source: "openclaw-extra",
	...
	const mergeRecord = (record) => {
		...
		if (replaced) warnSkillPrecedenceCollision(record.skill, replaced.skill, workspaceDir);
		merged.set(record.skill.name, record);
	};
	for (const record of extraSkills) mergeRecord(record);
	const bundledTierSkills = [...bundledSkills, ...custodianSkills].toSorted((left, right) => left.skill.name.localeCompare(right.skill.name, "en") || left.skill.source.localeCompare(right.skill.source, "en"));
	for (const record of bundledTierSkills) mergeRecord(record);
	...
	const entries = Array.from(merged.values()).toSorted((a, b) => a.skill.name.localeCompare(b.skill.name, "en")).map((record) => {
...
function resolveWorkspaceSkillPromptEntries(workspaceDir, opts) {
	...
	return {
		eligible: filterSkillEntries(opts?.entries ?? mergeRemoteNodeSkillEntries(loadSkillEntries(workspaceDir, opts), {
...
function loadMergedWorkspaceSkills(params) {
	const { agentWorkspaceDir, executionSkillsDir } = normalizeWorkspaceSkillRoots(params);
	if (!executionSkillsDir) return loadWorkspaceSkills(agentWorkspaceDir, params);
	...
	return filterSkillEntries([...agentEntries, ...executionEntries], params.config, effectiveSkillFilter, params.skillOverrides, params.eligibility);
}
...
//#endregion
export { normalizeWorkspaceSkillRoots as a, fingerprintSkillSnapshotConfig as c, recordRemoteSkillNodeInfo as d, removeRemoteNodeSkills as f, loadWorkspaceSkills as i, resetSkillSnapshotConfigFingerprintCache as l, setRemoteSkillConnectionReconciler as m, loadMergedWorkspaceSkills as n, resolveWorkspaceSkillPromptEntries as o, replaceRemoteNodeSkills as p, loadVisibleSkills as r, compactPromptSkills as s, filterWorkspaceSkills as t, mergeRemoteNodeSkillEntries as u };
