...
import { t as decodeTextPrefix } from "./src-BntaCZM-.js";
import { t as parseBoolean } from "./boolean-coercion-1HZNNkFl.js";
import { a as asOptionalRecord } from "./record-coerce-DItp3I4t.js";
import { o as retryAsync } from "./src-BQ327IOM.js";
import { C as readEmbeddingVectors } from "./gateway-startup-plugin-ids-Dtzhwc1j.js";
import { c as resolveSafeTimeoutDelayMs } from "./timeouts-D2XMKe-X.js";
import { c as shouldUseEnvHttpProxyForUrl } from "./proxy-env-TVpcGZHW.js";
...
import { _ as hasNonTextEmbeddingParts, b as splitTextToUtf8ByteLimit, m as runMemoryHostTasksWithConcurrency, y as estimateUtf8Bytes } from "./internal-BFGgxRGi.js";
import { r as hashText } from "./read-retry-DV0CdWmZ.js";
import "./memory-search-Cyk11Xva.js";
import "./memory-embedding-provider-runtime-BVaHSmYL.js";
	...
	return await withRemoteHttpResponse({
		...
		onResponse: async (res) => {
			if (!res.ok) {
				...
				const err = /* @__PURE__ */ new Error(`${params.errorPrefix}: ${res.status} ${formatErrorMessage(text)}`);
...
const resolveApiKeyForProvider = async (...args) => {
	return (await import("./model-auth-BHj2hZ87.js")).resolveApiKeyForProviderCore(...args);
};
//#endregion
...
async function resolveRemoteEmbeddingBearerClient(params) {
	...
	if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) entries.push(...Object.entries(resolveOpenClawAttributionHeaders()));
	return {
		baseUrl,
...
function createRemoteEmbeddingProvider(params) {
	...
	return {
		...
		embedBatch: async (inputs, options) => {
			...
			if (options?.inputType === "query" && params.batchQueryInputs !== true) return await Promise.all(texts.map(async (text) => (await embedMany([text], options.signal))[0] ?? []));
...
function createLocalEmbeddingProvider(..._args) {
	return Promise.reject(/* @__PURE__ */ new Error("The legacy in-process llama.cpp embedding runtime is retired. Run `openclaw update repair` to install the managed llama-server provider, then restart OpenClaw."));
}
//#endregion
export { extractBatchErrorMessage as A, EMBEDDING_BATCH_ENDPOINT as C, buildRemoteBaseUrlPolicy as D, postJsonWithRetry as E, formatUnavailableBatchError as M, isEmbeddingBatchUnavailableError as N, withRemoteHttpResponse as O, normalizeBatchBaseUrl as S, readEmbeddingBatchJsonl as T, throwIfBatchCompletionError as _, normalizeEmbeddingModelWithPrefixes as a, runEmbeddingBatchGroups as b, mapBatchEmbeddingsByIndex as c, uploadBatchJsonlFile as d, embeddingProviderOwnsDestination as f, resolveCompletedBatchResult as g, resolveBatchCompletionFromStatus as h, fetchRemoteEmbeddingVectors as i, formatBatchErrorDetail as j, EmbeddingBatchUnavailableError as k, sanitizeEmbeddingCacheHeaders as l, resolveRemoteEmbeddingBearerClient as m, createRemoteEmbeddingProvider as n, debugEmbeddingsLog as o, resolveEmbeddingEndpointUrl as p, resolveRemoteEmbeddingClient as r, isMissingEmbeddingApiKeyError as s, createLocalEmbeddingProvider as t, enforceEmbeddingMaxInputTokens as u, throwIfBatchTerminalFailure as v, applyEmbeddingBatchOutputLine as w, buildBatchHeaders as x, buildEmbeddingBatchGroupOptions as y };
