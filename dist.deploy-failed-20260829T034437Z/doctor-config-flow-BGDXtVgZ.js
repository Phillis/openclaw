...
import { _ as parseLegacySecretRefEnvMarker, u as isLegacySecretRefEnvMarker } from "./types.secrets-Bre8L6Ts.js";
import { t as CONFIG_PATH } from "./paths-BBSTUjD5.js";
...
import { f as resolveAgentWorkspaceDir, o as readAgentRosterProperty, w as tryResolveSoleAgentId } from "./agent-scope-config-CUBiGmG3.js";
import { t as DEFAULT_ACCOUNT_ID } from "./account-id-BH0zJUew.js";
import "./session-key-Dbce_H9p.js";
import { U as resolveNormalizedProviderModelMaxTokens, ct as classifyOtelGrpcMigrationOwnership, lt as containsAuthoredInclude, nt as migrateLegacyContextBudgetConfig, st as applyChannelDoctorCompatibilityMigrations, ut as isSingleTopLevelIncludeMigration } from "./io-DlN5njvP.js";
import { t as createSubsystemLogger } from "./subsystem-a4KzJVZG.js";
import { s as HeartbeatSchema } from "./zod-schema.agent-runtime-WXtAE1HE.js";
import { i as getBundledChannelSetupPlugin, o as hasBundledChannelPackageSetupFeature } from "./bundled-BTOeOZSs.js";
import { r as collectConfiguredModelRefs } from "./configured-model-refs-0XUAFjEF.js";
import { _ as modelEntryWithRuntimePolicy, d as migrateLegacyWebSearchConfig, f as migrateLegacyXSearchConfig, g as migrateLegacyRuntimeModelRef, h as listLegacyRuntimeModelProviderAliases, l as stripRetiredTuningKnobs, m as legacyRuntimeModelAliasRequiresRuntimePolicy, u as migrateLegacyWebFetchConfig, v as selectedCanonicalModelRefsForRuntimePolicy, y as hasOwnKey } from "./legacy-C3aoLO5V.js";
import { r as isLegacyModelsAddCodexMetadataModel } from "./legacy-config-migrations.runtime.models-B0IK-Bzp.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-BIKs-96s.js";
import { r as retainLegacyDefaultAgentId } from "./legacy.default-agent-owner-CL_-T11Y.js";
import { d as configIncludeOwnsAgentRoster } from "./provider-model-route-D-FYx-DP.js";
import { a as isBlockedLegacyCodexModelRef } from "./codex-route-model-ref-Du1KAbLA.js";
import { r as normalizeTalkSection } from "./talk-kxni9fig.js";
...
import { a as runPluginSetupConfigMigrations } from "./setup-registry-CHJgnlrg.js";
...
import { n as resolveSingleAccountPromotion } from "./setup-promotion-helpers-BrVI5z6l.js";
import { a as noteOpencodeProviderOverrides, c as stripUnknownConfigKeys, i as noteMcpOriginWarning, n as noteImplicitFallbackClobberWarnings, o as noteSandboxOriginProxyWarning } from "./doctor-config-analysis-BqbMX9ZP.js";
import { t as migrateLegacyConfig } from "./legacy-config-migrate-D9RGhVd6.js";
import { t as runDoctorConfigPreflight } from "./doctor-config-preflight-CPfLsNSR.js";
import { n as cronCodexRuntimePolicyTargetKey } from "./store-migration-D5TjYSow.js";
import { t as applyDoctorConfigMutation } from "./config-mutation-state-CqSBOMKz.js";
...
function normalizeLegacyRuntimeAllowlistModels(rawModels, selectedRuntime, selectedRuntimeRequiresPolicy, policyRuntimes, blockedModelIdentities) {
	...
	for (const [rawKey, entry] of Object.entries(rawModels)) {
		...
		if (migrated && (migrated.runtime === selectedRuntime || migrated.legacyProvider === LEGACY_CODEX_CLI_RUNTIME_ID || policyRuntimes.has(migrated.runtime))) {
			changed = true;
			if (!policyRuntimes.has(migrated.runtime)) next[rawKey] = mergeModelEntry(entry, next[rawKey]);
			legacyEntries.push({
...
function normalizeLegacyRuntimeAgentContainer(raw, path, changes, blockedModelIdentities) {
	...
	const models = normalizeLegacyRuntimeAllowlistModels(raw.models, model.selectedRuntime, model.selectedRuntimeRequiresPolicy, modelPolicy.runtimes, blockedModelIdentities);
	if (models.changed) {
...
function collectInvalidHookTransformsDirWarnings(cfg, configPath) {
	...
	if (isPathInside(transformsRoot, path.isAbsolute(transformsDir) ? path.resolve(transformsDir) : path.resolve(transformsRoot, transformsDir))) return [];
...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	const { createDoctorPluginMetadataSnapshotScope } = await import("./plugin-metadata-snapshot-scope-BzANNCHE.js");
	...
	const { collectBlockedLegacyOpenAICodexProviderPlan } = await import("./legacy-config-migrations.runtime.models-CnYG5kEb.js");
	...
	if (preflight.cronCodexRuntimePolicyTargets?.length) {
		const { repairCronCodexRuntimePolicies } = await import("./runtime-policy-migration-C5dE0VSF.js");
	...
	const { prepareTailscaleConfigMigration } = await import("./doctor-tailscale-DDQNOr4w.js");
	...
	const { prepareRetiredPhoneControlCleanup } = await import("./doctor-retired-phone-control-MXJv54ft.js");
	...
	const { applyPluginAutoEnable } = await import("./plugin-auto-enable-B6KBJi0e.js");
	applyConfigMutation(runWithCurrentPluginMetadata(state.candidate, () => applyPluginAutoEnable({
		config: state.candidate,
		env: process.env
	})), { fixHint: `Run "${doctorFixCommand}" to apply these changes.` });
	if (!shouldRepair) {
		const { repairStaleAgentModelRefs } = await import("./stale-agent-model-ref-repair-DTwef9I7.js");
	...
	const { collectPluginToolAllowlistWarnings } = await import("./plugin-tool-allowlist-warnings-DimTOo8m.js");
	...
	if (hasConfiguredChannels) {
		const channelDoctor = await import("./channel-doctor-Bc5c2KyR.js");
	...
	const { repairHooksTokenReuseGatewayAuth } = await import("./hooks-token-reuse-repair-CPj2w3oj.js");
	...
	if (shouldRepair) {
		const { runDoctorRepairSequence } = await import("./repair-sequencing-BaR80rjo.js");
		...
		const { collectDoctorPreviewNotes } = await import("./preview-warnings-CBrypkRz.js");
	...
	if (configuredOpencodePluginIds.length > 0) {
		const { resolveEnabledProviderPluginIds } = await import("./providers-DzOYSf0w.js");
