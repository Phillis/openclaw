...
import { T as isDerivedWorkspacePath, a as activeWorkspaceHashContext, d as withWorkspaceHashMemo, h as MAX_RECONCILIATION_TOTAL_BYTES, m as MAX_RECONCILIATION_FILE_BYTES, n as readActualWorkspaceManifestImpl, p as MAX_RECONCILIATION_ENTRIES, r as readWorkspaceFileSnapshotWithLimit, u as withWorkspaceHashContext } from "./workspace-actual-manifest-DIThIqhg.js";
...
async function readWorkspaceFileSnapshot(root, entryPath) {
	return await readWorkspaceFileSnapshotWithLimit(localPath$1(root, entryPath), MAX_RECONCILIATION_FILE_BYTES, root);
}
...
async function entryMatches(root, entry) {
	if (entry.type === "symlink") return await absoluteEntryMatches(localPath$1(root, entry.path), entry);
...
async function directoryContainsOnlyJournalPaths(root, directory, paths, directories) {
	for (const name of await fs.readdir(localPath$1(root, directory))) {
		const child = `${directory}/${name}`;
		if (isDerivedWorkspacePath(child)) continue;
		const stats = await fs.lstat(localPath$1(root, child));
...
async function directoryContainsOnlyDerivedWorkspaceEntries(root, directory) {
	const names = await fs.readdir(localPath$1(root, directory));
...
async function localWorkspaceNode(root, entryPath) {
	const absolute = localPath$1(root, entryPath);
...
async function localWorkspaceDescendantPaths(root, entryPaths) {
	...
	while (pending.length > 0) {
		...
		const names = [];
		for await (const entry of await fs.opendir(localPath$1(root, directory))) {
		...
		for (const name of names.toSorted()) {
			...
			const stats = await fs.lstat(localPath$1(root, childPath));
...
async function preflightWorkspaceApply(params) {
	...
	for (const entryPath of structuralRoots) {
		const stats = await fs.lstat(localPath$1(params.root, entryPath)).catch(() => void 0);
	...
	for (const entryPath of paths) {
		...
		if (currentNodes.get(entryPath) === void 0 && !await fs.lstat(localPath$1(params.root, entryPath)).catch((error) => {
			if (hasNodeErrorCode(error, "ENOENT") || hasNodeErrorCode(error, "ENOTDIR")) return;
			throw error;
		})) continue;
		const segments = entryPath.split("/");
...
async function materializeSnapshotEntry(params) {
	const target = localPath$1(params.root, params.entry.path);
	...
	else if (params.sourceRoot) await fs.copyFile(localPath$1(params.sourceRoot, params.entry.path), target);
...
async function writeRawWorkspaceTree(params) {
	...
	for (const entry of reconciliationEntries(params.entries).toSorted((left, right) => left.path.localeCompare(right.path))) {
		const content = entry.type === "symlink" ? Buffer.from(entry.target) : await fs.readFile(localPath$1(params.repositoryRoot, entry.path));
		blobs.push({
			entry,
			mark,
			content
	...
	try {
		...
		for (const entry of params.baseEntries) {
			...
			if (entry.type === "file") {
				...
				content = await fs.readFile(localPath$1(params.root, entry.path));
...
async function createWorkspaceRecoveryPatch(params) {
	...
	try {
		...
		for (const entryPath of [...paths].toSorted()) {
			const absolute = localPath$1(params.root, entryPath);
			const stats = await fs.lstat(absolute).catch(() => void 0);
			if (!stats) {
...
async function assertWorkspaceRecoveryBase(params) {
	...
	for (const entry of appliedEntries) {
		...
		const existing = await fs.lstat(localPath$1(params.root, entry.path)).catch(() => void 0);
...
//#endregion
export { recoverWorkerWorkspaceReconciliation as a, changedPaths as c, readActualWorkspaceManifest as d, reconciliationEntries as f, parseAcceptedWorkspaceSettlement as i, inspectAcceptedWorkerWorkspace as l, localPath$1 as m, AcceptedWorkspacePublicationIndeterminateError as n, assertWorkspaceMatchesManifest as o, absoluteEntryMatches as p, isAcceptedWorkspacePublicationIndeterminateError as r, assertWorkspaceResultStable as s, applyStagedWorkerWorkspace as t, manifestNodes as u };
