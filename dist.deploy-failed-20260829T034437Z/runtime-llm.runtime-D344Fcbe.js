...
import { r as buildConfiguredModelCatalog } from "./model-selection-shared-I5TmV9jL.js";
...
async function runIsolatedAgentRuntimeCompletion(params) {
	...
	try {
		const operation = (async () => {
			const { runIsolatedCompletion } = await import("./isolated-completion-DCV7Cy2o.js");
...
function createRuntimeLlm(options = {}) {
	...
	return { complete: async (params) => {
		...
		const [{ prepareSimpleCompletionModelForAgent, completeWithPreparedSimpleCompletionModel, resolveSimpleCompletionSelectionForAgent }, cfg] = await Promise.all([import("./simple-completion-runtime-DmqSBwOq.js"), Promise.resolve(resolveRuntimeConfig(options))]);
