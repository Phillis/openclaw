...
import { A as usesFastLoopbackCdpProbeClass, O as PROFILE_ATTACH_RETRY_TIMEOUT_MS, W as DEFAULT_BROWSER_LOCAL_CDP_READY_TIMEOUT_MS, a as fetchJson, d as resolveCdpTabOwnership, k as resolveCdpReachabilityTimeouts, l as normalizeCdpHttpBaseForJsonEndpoints, n as assertCdpEndpointAllowed, o as fetchOk, t as appendCdpPath, w as CHROME_MCP_ATTACH_READY_WINDOW_MS } from "./cdp.helpers-CDKXkF5Z.js";
import { a as resolveProfile, n as getOwnBrowserProfile, r as resolveBrowserConfig } from "./config-KSi3HkFI.js";
import { c as BrowserResetUnsupportedError, d as BrowserTargetAmbiguousError, m as toBrowserErrorResponse, n as BROWSER_ERROR_REASONS, o as BrowserProfileNotFoundError, s as BrowserProfileUnavailableError, u as BrowserTabNotFoundError } from "./tmp-openclaw-dir-DE5M5B-m.js";
...
import { B as resolveBrowserNavigationProxyMode, I as requiresInspectableBrowserNavigationRedirectsForUrl, L as withBrowserNavigationPolicy, M as assertBrowserNavigationAllowed, P as assertBrowserNavigationResultAllowed, a as isChromeReachable, c as stopOpenClawChrome, f as diagnoseChromeCdp, g as createTargetViaCdp, i as isChromeCdpReady, j as InvalidBrowserNavigationUrlError, l as stopOwnedOpenClawChrome, o as launchOpenClawChrome, p as formatChromeCdpDiagnostic, r as isChromeCdpOwnedByPid, s as resolveOpenClawUserDataDir, t as ManagedChromeCleanupError, y as normalizeCdpWsUrl, z as waitForCdpCommittedNavigationUrl } from "./chrome-BeE6BX5Z.js";
import { _ as getPwAiModule, a as getOrCreateProfileRuntime, c as isProfileGenerationCurrent, f as registerProfileHandle, g as getChromeMcpModule, h as withProfileOperationLease, i as enqueueProfileStart, l as isProfileRestartRequiredError, n as assertProfileLifecycleContext, o as getProfileLifecycle, p as releaseProfileHandle, r as beginProfileTransition, s as isBrowserRuntimeRunning, t as ProfileRestartRequiredError, u as isWithinProfileOperationLease } from "./server-context.lifecycle-DI7diuDR.js";
import { i as resolveCdpReachabilityPolicy, n as assertChromeMcpCdpTransportAllowed, r as resolveCdpControlPolicy, t as movePathToTrash } from "./trash-CbyrL9VE.js";
...
function applyResolvedConfig(current, freshResolved) {
	...
	for (const [name, relay] of current.extensionRelays ?? []) {
		...
		if (runtime?.profile.driver === "extension" && !lifecycle?.terminal && !lifecycle?.transitionReason && !lifecycle?.cleanupRelays.has(relay) && profile?.driver === "extension" && profile.cdpPort === relay.port) extensionRelayInternalTokens[name] = relay.internalToken;
	}
	current.resolved = {
		...freshResolved,
...
function getExtensionRelayModule() {
	modPromise ??= import("./relay-lifecycle-DkUX6drI.js");
	return modPromise;
}
//#endregion
...
function createProfileAvailability({ opts, profile, state, runtime, configRevision }) {
	...
	const ensureExtensionRelay = async (signal) => {
		...
		await ensureExtensionRelayForProfile(state(), profile, signal);
		signal?.throwIfAborted();
	};
	...
	const ensureBrowserAvailableOnce = async (signal, generation, options) => {
		...
		if (capabilities.mode === "local-extension") {
			...
			const connected = await relay.bridge.waitForExtensionConnection(signal, CHROME_MCP_ATTACH_READY_WINDOW_MS);
			signal.throwIfAborted();
	...
	const stopRunningBrowser = async () => {
		...
		return { stopped: (await beginProfileTransition({
			...
			afterCleanup: async () => {
				if (profile.attachOnly || capabilities.mode !== "local-managed") return;
				stoppedOwnedProcess = await stopOwnedOpenClawChrome(current.resolved, profile);
			}
		})).stopped || stoppedOwnedProcess || profile.attachOnly || capabilities.isRemote };
	};
	return {
		isHttpReachable,
