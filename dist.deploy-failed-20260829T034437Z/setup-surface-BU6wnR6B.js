...
import { N as splitSetupEntries, S as promptAccountId, p as mergeAllowFromEntries } from "./setup-wizard-helpers-JxuPdtZE.js";
...
import { r as resolveMatrixEnvAuthReadiness } from "./env-auth-DsQllxGb.js";
import { i as resolveMatrixAccount, r as resolveDefaultMatrixAccountId, t as listMatrixAccountIds } from "./accounts-CePJXe0h.js";
import { n as isMatrixRoomId } from "./target-ids-DEUqtLsd.js";
import { n as updateMatrixAccountConfig } from "./config-update-CbMkRb2G.js";
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as createMatrixSetupDmPolicy } from "./setup-dm-policy-DzRsMtLY.js";
import { n as validateMatrixHomeserverUrl, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-BCECM942.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-CBjoQKfV.js");
...
async function configureMatrixInviteAutoJoin(params) {
	...
	while (true) {
		...
		if (allowlist.length === 0 || invalidEntries.length > 0) {
			await params.prompter.note(["Use only stable Matrix invite targets for auto-join: !roomId:server (or the suffixless !roomId form on room version 12+), #alias:server, or *.", invalidEntries.length > 0 ? `Invalid: ${invalidEntries.join(", ")}` : void 0].filter(Boolean).join("\n"), "Matrix invite auto-join");
			continue;
		}
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = updateMatrixAccountConfig(next, params.accountId, { groupPolicy: accessConfig.policy });
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-DVegj_it.js");
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-CcoZausS.js");
