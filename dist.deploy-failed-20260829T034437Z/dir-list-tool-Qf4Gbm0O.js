import { a as DIR_LIST_TOOL_DESCRIPTOR, i as DIR_LIST_HARD_MAX_ENTRIES } from "./descriptors-CxBbqcc6.js";
import { t as appendFileTransferAudit } from "./audit-DJn71F-n.js";
import { i as readClampedInt, n as readRequiredNodePath, t as invokeNodeToolPayload } from "./node-tool-invoke-BtR5AVyi.js";
...
function createDirListTool() {
	return {
		...
		execute: async (_toolCallId, args) => {
			...
			const summary = `Listed ${canonicalPath}: ${fileCount} file${fileCount !== 1 ? "s" : ""}, ${dirCount} subdir${dirCount !== 1 ? "s" : ""}${truncated ? " (more entries available)" : ""}${truncated && nextPageToken ? `. Call dir_list again with pageToken=${JSON.stringify(nextPageToken)}.` : ""}`;
