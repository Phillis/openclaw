...
import { i as consumeMcpCodeModeGuestResult } from "./agent-bundle-mcp-materialize-VGxGQrwu.js";
import "./tool-search-f7MtzgBB.js";
...
import { f as getSwarmRunByLaunchReplayKey, p as initSubagentRegistry } from "./subagent-registry-BNIH9OE4.js";
...
import { n as waitForCollectorCompletion } from "./agents-wait-tool-MiYhsFQn.js";
...
async function runBridgeRequest(params) {
	...
	try {
		...
		if (error?.mutationProvenance === "pre-mutation") settled.mutationProvenance = "pre-mutation";
...
*/
function codeModeMutationProvenance(state) {
	return state.preMutationFailures > 0 && state.potentiallyMutatingDispatches === 0 ? "pre-mutation" : void 0;
}
	...
	return params.pendingRequests.map((request) => {
		...
		const state = {
			...
			promise: completion.then((settled) => {
				...
				if (trustedNoStart && !recoverySafe) params.bridgeDispatch.potentiallyMutatingDispatches = Math.max(0, params.bridgeDispatch.potentiallyMutatingDispatches - 1);
				if (tracksDispatch && !recoverySafe && !trustedNoStart && settled.ok === false && settled.mutationProvenance === "pre-mutation") {
					params.bridgeDispatch.potentiallyMutatingDispatches = Math.max(0, params.bridgeDispatch.potentiallyMutatingDispatches - 1);
...
//#endregion
export { enforceSnapshotPayloadLimits as A, codeModeReplayIdForToolCall as B, CODE_MODE_WORKER_WATCHDOG_GRACE_MS as C, codeModeFailureCode as D, boundOutputToLimit as E, readRunId as F, resolveCodeModeSkills as H, resolveCodeModeConfig as I, resolveCodeModeHeadlessConfig as L, prepareSource as M, readCode as N, codeModeFailureMessage as O, readPositiveInteger as P, toToolSearchConfig as R, waitForPendingBridgeSettlement as S, MAX_HEADLESS_WALL_CLOCK_MS as T, createCodeModeCatalogProjection as V, settledBridgeRequestsInCompletionOrder as _, codeModeWaitingReason as a, takeUndeliveredCodeModeRunOutput as b, disposeAllCodeModeRuns as c, pendingBridgeRequestsReplaySafe as d, pendingBridgeStatesForSettlement as f, resumingRunIds as g, reserveActiveRunSlot as h, codeModeMutationProvenance as i, isCodeModeEngagedForModel as j, createCodeModeApiFilesForRun as k, disposeCodeModeRun as l, removeExpiredRuns as m, cancelPendingBridgeStates as n, createCodeModeBridgeDispatchState as o, pendingToolCalls as p, cancelPendingBridgeStatesById as r, createPendingBridgeStates as s, activeRuns as t, isCodeModeBridgeRepairEligible as u, snapshotState as v, DEFAULT_HEADLESS_WALL_CLOCK_MS as w, telemetry as x, storeSnapshotState as y, CODE_MODE_NODES_TOOL_ID as z };
