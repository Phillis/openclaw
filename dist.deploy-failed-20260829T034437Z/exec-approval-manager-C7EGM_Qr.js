...
import { p as truncatePluginApprovalDetail } from "./plugin-approvals-DzVTK3Ht.js";
import { t as resolveExecApprovalCommandDisplay } from "./exec-approval-command-display-XHb-Y_TM.js";
import { a as forceDenyOperatorApproval, c as insertOperatorApproval, p as resolveOperatorApproval, r as consumeOperatorApprovalAllowOnce } from "./operator-approval-store-BY5ymuaJ.js";
...
var ExecApprovalManager = class {
	...
	*/
	...
	resolveDetailed(recordId, decision, resolver, localResolvedBy = null, localResolutionSource = "operator") {
		...
		const standingGrant = decision === "allow-always" && localEntry ? this.options.resolveStandingGrantMint?.(localEntry.record.request) ?? void 0 : void 0;
		let result;
		try {
	...
	*/
	...
	runPendingContinuation(recordId, run) {
		const entry = this.pending.get(recordId);
		if (!entry?.admissionContinuation || entry.record.resolvedAtMs !== void 0 || entry.record.expiresAtMs <= Date.now()) return null;
	...
	consumeAllowOnce(recordId, consumerId = recordId) {
		if (this.forceDenyIfDelegatedAuthorityClosed(recordId)) return false;
		const entry = this.pending.get(recordId);
		if (!entry) return false;
		const nowMs = Date.now();
	...
	*/
	...
	projectDecisionIfActive(recordId, decision) {
		...
		if (!authority || this.options.validateAgentRuntimeDelegatedAuthority?.(authority) === true) return decision;
		this.forceDenyIfDelegatedAuthorityClosed(recordId);
		return null;
	}
	/** Atomically closes a live approval whose exact delegated owner is gone. */
	forceDenyIfDelegatedAuthorityClosed(recordId) {
		const authority = this.pending.get(recordId)?.record.agentRuntimeDelegatedAuthority;
		if (!authority || this.options.validateAgentRuntimeDelegatedAuthority?.(authority) === true) return null;
