...
import { o as createProviderHttpError } from "./provider-http-errors-BXG5plR9.js";
import { d as resolveProviderRequestHeaders } from "./provider-request-config-BAUXM8KH.js";
import { c as getEnvApiKey, h as resolveProviderContext, r as calculateCost } from "./llm-BkUeN9nv.js";
import { t as buildGuardedModelFetch } from "./provider-transport-fetch-BDjjVdoJ.js";
import { n as transformTransportMessages } from "./ai-transport-runtime-host-BOc97x0_.js";
import { A as resolveGoogleGemini3ThinkingLevel, D as isGoogleGemini3ProModel, E as isGoogleGemini3FlashModel, M as stripInvalidGoogleThinkingBudget, T as isGoogleGemini25ThinkingBudgetModel } from "./provider-stream-shared-DEARVxDz.js";
...
import "./provider-auth-runtime-C9IBkITf.js";
import "./provider-http-S5IuZe1q.js";
import { a as createWritableTransportEventStream, b as sortPromptCacheToolsByName, c as extractToolResultText, d as finalizeTransportStream, g as notifyProviderHttpResponse, i as createEmptyTransportUsage, l as failTransportStream, m as mergeTransportHeaders, n as coerceTransportToolCallArguments, o as describeToolResultMediaPlaceholder, x as stripSystemPromptCacheBoundary, y as sanitizeTransportPayloadText } from "./provider-transport-runtime-BcoSB6SZ.js";
import { r as stripGoogleProviderPrefix } from "./model-id-CAmKILzd.js";
import { d as isOfficialGoogleAiStudioBaseUrl, f as normalizeGoogleApiBaseUrl } from "./provider-policy-CAz-ImDw.js";
import "./thinking-api-CGXpFoLK.js";
import { t as isGoogleNativeVideoModelId } from "./provider-models-mmU8GWxC.js";
import { t as parseGeminiAuth } from "./gemini-auth-B2h6lAH-.js";
import { n as resolveGoogleVertexAuthorizedUserHeaders, t as isGoogleVertexCredentialsMarker } from "./vertex-adc-BhAYJCCH.js";
//#region extensions/google/transport-stream.ts
const GOOGLE_NATIVE_VIDEO_MIME = /* @__PURE__ */ new Set([
	"video/mp4",
	"video/mpeg",
	"video/quicktime",
	"video/avi",
...
const GOOGLE_REQUEST_BYTES_EXCLUSIVE = 2e7;
...
const GOOGLE_VERTEX_MODEL_RESOURCE_PREFIX = /^(?:projects\/[^/]+\/locations\/[^/]+\/)?publishers\/google\/models\//u;
...
function canonicalGoogleModel(model) {
	return {
		...model,
		input: model.input.filter((type) => type !== "video")
	};
}
function supportsGoogleNativeVideo(model) {
	return model.provider === "google" && normalizeGoogleTransportRouteApi(model.api) === "google-generative-ai" && isOfficialGoogleAiStudioBaseUrl(model.baseUrl) && isGoogleNativeVideoModelId(model.id) && model.input.includes("video");
}
...
function convertGoogleMessages(model, context, videoSlots) {
	const contents = [];
	...
	const transformedMessages = transformTransportMessages(normalizeGoogleTransportMessageRoutes(context.messages), canonicalGoogleModel(routeModel), (id) => requiresToolCallId(model.id) ? normalizeToolCallId(id) : id, { preserveCrossModelToolCallThoughtSignature: requiresToolCallThoughtSignature(model.id) });
	...
	for (const msg of transformedMessages) {
		...
		if (msg.role === "user") {
			...
			const parts = msg.content.map((item) => {
				if (item.type === "text") return { text: sanitizeTransportPayloadText(item.text) || " " };
				if (item.type === "image") return { inlineData: {
		...
		if (msg.role === "assistant") {
			...
			for (const block of msg.content) {
				...
				if (block.type === "toolCall") {
					if (isSameRoute) sameRouteToolCallIds.add(block.id);
		...
		if (msg.role === "toolResult") {
			...
			const functionResponse = { functionResponse: {
				...
				...sameRouteToolCallIds.has(msg.toolCallId) || requiresToolCallId(model.id) ? { id: msg.toolCallId } : {}
			} };
...
function buildGoogleGenerativeAiParams(model, context, options, videoSlots) {
	...
	const params = { contents: convertGoogleMessages(model, context, videoSlots) };
...
function replaceGooglePartWithText(part, text) {
	Object.keys(part).forEach((key) => Reflect.deleteProperty(part, key));
	part.text = text;
}
function materializeGoogleVideoSlots(request, slots) {
	const trusted = [];
	const visit = (value) => {
		...
		if (normalizeLowercaseStringOrEmpty((isRecord(value.inlineData) ? value.inlineData : void 0)?.mimeType).startsWith("video/")) {
...
function serializeGoogleRequest(request, videoSlots) {
	let body = JSON.stringify(request);
	for (const slot of videoSlots.toReversed()) {
		if (Buffer.byteLength(body, "utf8") < GOOGLE_REQUEST_BYTES_EXCLUSIVE) break;
		replaceGooglePartWithText(slot, GOOGLE_VIDEO_SLOT_OMISSION);
		body = JSON.stringify(request);
	}
	if (Buffer.byteLength(body, "utf8") >= GOOGLE_REQUEST_BYTES_EXCLUSIVE) throw new Error(`Google request body must be smaller than ${GOOGLE_REQUEST_BYTES_EXCLUSIVE} bytes`);
	return body;
}
...
function collectGoogleTransportApiKeys(params) {
	if (params.kind !== "google-generative-ai" || !isOfficialGoogleAiStudioBaseUrl(params.model.baseUrl) || isGoogleOauthApiKey(params.primaryApiKey) || hasGoogleAuthHeader(params.model.headers) || hasGoogleAuthHeader(params.options?.headers)) return [];
...
function shouldRetryGoogleGemini3FirstResponse(params) {
	...
	if (!isOfficialGoogleAiStudioBaseUrl(params.model.baseUrl)) return false;
...
async function notifyGoogleTransportHttpResponse(model, options, response, signal) {
...
async function openGoogleSseAttempt(params) {
	...
	const handleTimedOperationError = (error) => {
...
async function openGoogleSseChunks(params) {
	...
	if (!shouldRetryGoogleGemini3FirstResponse({
		...
		await notifyGoogleTransportHttpResponse(params.model, params.options, response, params.options?.signal);
	...
	try {
		while (true) {
			...
			if (done) {
				...
				if (!isRecord(trailingChunk) || !isRecord(trailingChunk.error)) throw new Error("Google SSE stream ended with an incomplete frame");
	...
	return (rawModel, context, rawOptions) => {
		...
		(async () => {
			...
			try {
				...
				const providerContext = supportsGoogleNativeVideo(model) ? await resolveProviderContext(context, options) : context;
				const videoSlots = /* @__PURE__ */ new Map();
				...
				for await (const chunk of chunks) {
					...
					if (responseModel && resolveGoogleModelPath(model.id.replace(GOOGLE_VERTEX_MODEL_RESOURCE_PREFIX, "")) !== resolveGoogleModelPath(responseModel.replace(GOOGLE_VERTEX_MODEL_RESOURCE_PREFIX, ""))) output.responseModel ||= responseModel;
					...
					if (typeof candidate?.finishReason === "string" && candidate.finishReason !== "FINISH_REASON_UNSPECIFIED") {
