import { h as workerWorkspaceCommandSucceeded, p as waitForQuiescenceRenewal, y as workspaceSyncError } from "./workspace-sync-helpers-Ba8iUX3R.js";
...
function createWorkerWorkspaceQuiescence(params) {
	return async (remoteWorkspaceDir) => {
		...
		if (!posixAbsolute && !windowsAbsolute) throw new Error("Worker workspace quiescence path must be absolute");
		if (!posixAbsolute && windowsAbsolute && !params.sharedHost) throw new Error("Windows worker workspace quiescence requires a shared host");
		...
		return {
			assertActive: async () => {
				if (resumed || renewalAbort.signal.aborted) throw new Error("Worker workspace quiescence was already released");
...
//#endregion
export { createWorkerWorkspaceQuiescence as t };
