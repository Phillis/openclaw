...
import { n as extractErrorCode, o as readErrorName, r as formatErrorMessage } from "./errors-Ccx0R-_Z.js";
...
import { i as updateDreamsFile, n as resolveDreamsPath, t as readDreamsFile } from "./dreaming-dreams-file-dsiKSa3Z.js";
import { n as DREAMING_SESSION_KEY_PREFIX, r as scrubDreamingNarrativeArtifacts } from "./dreaming-session-cleanup-B4Ndffm3.js";
import { i as readStore } from "./short-term-promotion-store-BlKDyXAW.js";
...
function classifyTerminalReplyNarrative(result) {
	const reply = result.terminalReply;
	if (!reply) return { kind: "absent" };
	const text = reply.disposition === "visible" && typeof reply.text === "string" ? reply.text.trim() : "";
	return text ? {
		kind: "visible",
		text
	} : { kind: "non-visible" };
...
async function appendNarrativeEntry(params) {
	...
	return await updateDreamsFile({
		...
		updater: async (existing, dreamsPath) => {
			...
			if (sourceKeys.some((key) => !currentSources?.[key]) || params.recentDiaryEntries?.some((block) => !currentDiary.has(clampDiaryContextEntry(block)))) return {
				content: existing,
				result: void 0,
				shouldWrite: false
			};
...
async function generateAndAppendDreamNarrative(params) {
	...
	await withNarrativeSessionLock(sessionKey, async () => {
		...
		try {
			...
			const narrative = terminalReply?.kind === "visible" ? terminalReply.text : terminalReply?.kind === "absent" ? await readSettledNarrativeText({
				subagent: params.subagent,
