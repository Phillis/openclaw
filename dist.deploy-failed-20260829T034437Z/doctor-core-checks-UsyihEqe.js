...
import { a as isDefaultInstallIdentity, w as resolveStateDir } from "./paths-BBSTUjD5.js";
import { n as isExperimentalClawsEnabled } from "./experimental-BMzbGmT5.js";
import "./agent-scope-DigoIwHb.js";
import { a as listAgentIds, r as listAgentEntries, w as tryResolveSoleAgentId } from "./agent-scope-config-CUBiGmG3.js";
import { n as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-DuqbQCOS.js";
...
import { i as noteChromeMcpBrowserReadiness, n as maybeArchiveLegacyClawdBrowserProfileResidue, r as maybeRepairOwnedChromeExtensionNativeHosts, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-Dd-QFUxv.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-DPkdMgE3.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-uniRtRmP.js";
import { n as disableUnavailableSkillsInConfig, r as formatMissingSkillSummary } from "./doctor-skills-core-DZQVCoNO.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-B_PrO5p9.js";
import { i as resolveKnownModelRefMigrationTarget, n as collectCodexRuntimeCompatibilityWarnings, o as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-zP2n26is.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-BVj2mVAT.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-Vwrvl_-h.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-CoSN_TIg.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-J0B8vV57.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-BdgdEGkX.js");
...
const legacyStateCheck = {
	...
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-DKl3Ssu4.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		if (!ctx.cwd) return [];
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget } = await import("./bootstrap-budget-DdTykizM.js");
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-tm1vBVwy.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-DJjwbXPM.js");
...
function createModelReferenceCheck() {
	return {
		...
		source: "doctor",
		async detect(ctx) {
			const { inspectConfiguredModelReferences } = await import("./model-reference-validation-D9ZFKCG8.js");
			return inspectConfiguredModelReferences({
				...
				if (inspection.status === "unknown-model" && inspection.active) return [{
					checkId: "core/doctor/model-references",
					severity: "info",
					source: "doctor",
					target: inspection.ref,
...
function createSecurityCheck(deps) {
	return {
		...
		source: "doctor",
		async detect(ctx) {
			return (await deps.collectSecurityWarnings(ctx.cfg)).map(securityAuditFindingToHealthFinding);
		}
	};
}
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./provider-openai-chatgpt-oauth-tls-Cm5q7LAY.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	defaultEnabled: false,
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-BnFzn7SE.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-BnFzn7SE.js");
...
const telegramGeneralTopicConversationsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectTelegramGeneralTopicConversationRepairs } = await import("./doctor-telegram-general-topic-conversations-ihjTiKkP.js");
	...
	async repair(ctx) {
		const { repairTelegramGeneralTopicConversations } = await import("./doctor-telegram-general-topic-conversations-ihjTiKkP.js");
		const effect = {
			kind: "state",
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-BvlBygLp.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-BvlBygLp.js");
...
const gatewayPlatformNotesCheck = {
	...
	async detect(ctx) {
		...
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-B9ZJAqQ0.js");
...
function createConvertedWorkflowChecks(deps) {
	return [
		...
		...isExperimentalClawsEnabled() ? [{
			...
			source: "doctor",
			async detect(ctx) {
				const [{ collectClawStateHealthFindings }, { listConfiguredMcpServers }] = await Promise.all([import("./doctor-CVMODPq8.js"), import("./mcp-config-Bp_HS8At.js")]);
