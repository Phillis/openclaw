...
import { c as getImageMetadata, d as readImageMetadataFromHeader, l as isImageProcessorUnavailableError, p as resizeToJpeg, r as buildImageResizeSideGrid, t as IMAGE_REDUCE_QUALITY_STEPS } from "./image-ops-CNJmjS8j.js";
	...
	for (const side of sideGrid) {
		for (const quality of IMAGE_REDUCE_QUALITY_STEPS) {
			...
			if (out.byteLength <= params.maxBytes) {
				...
				log.info(`Image resized to fit limits: ${sourceWithFile} ${formatBytesShort(buf.byteLength)} -> ${formatBytesShort(out.byteLength)} (${byteReductionPct < 0 ? "+" : ""}${-byteReductionPct}%)`, {
					label: params.label,
