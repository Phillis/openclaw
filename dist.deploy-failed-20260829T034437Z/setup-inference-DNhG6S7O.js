...
import { p as normalizeAuthProfileCredential } from "./profiles-B9i8Wh87.js";
...
import { i as appendSystemAgentAuditEntry } from "./audit-C7t95Ufg.js";
import { t as CliExecutionAuthProfileError } from "./cli-execution-auth-BeRuzQ77.js";
import { i as sameDefaultInferenceRoute, n as projectInferenceRoute, r as resolveSystemAgentConfiguredRouteFromConfig } from "./inference-route-B3bBax5n.js";
import { a as resolveSystemAgentVerifiedInferenceRoute, n as createSystemAgentVerifiedInferenceBinding, r as hasCurrentSystemAgentOwnerPluginArtifacts, t as captureSystemAgentOwnerPluginArtifacts } from "./verified-inference-D699BYSI.js";
...
import { n as createQuickstartNotePrompter } from "./setup-apply-Pl7dqviu.js";
import { n as createSystemAgentModelSelectionUpdater, t as applySystemAgentModelSelection } from "./setup-model-selection-PA5KMEvN.js";
import { n as extractAgentRunTerminalError, r as extractAgentRunText } from "./agent-run-result-DFovjOVm.js";
import { a as runProviderPluginAuthMethodUnpersisted, n as applyProviderPluginAuthMethodResultConfig } from "./provider-auth-choice-DmAuFIdl.js";
...
async function cleanupSetupInferenceTempDir(params) {
	try {
		(params.deps.disposeOpenClawAgentDatabaseByPath ?? (await import("./openclaw-agent-db-SfW2bbhN.js")).disposeOpenClawAgentDatabaseByPath)(path.join(params.tempDir, "agent", "openclaw-agent.sqlite"));
	} catch {
...
async function isCodexInstallRecordPersisted(record, deps) {
	try {
		const currentInstallRecords = await (deps.readPersistedInstalledPluginIndexInstallRecords ?? (await import("./installed-plugin-index-records-Q_FQEIvw.js")).readPersistedInstalledPluginIndexInstallRecords)();
...
async function clearUnownedCodexInstallCaches(deps) {
	try {
		(deps.clearLoadInstalledPluginIndexInstallRecordsCache ?? (await import("./installed-plugin-index-records-Q_FQEIvw.js")).clearLoadInstalledPluginIndexInstallRecordsCache)();
	} catch {
	...
	try {
		await (deps.invalidatePluginRuntimeDiscoveryAfterConfigMutation ?? (await import("./registry-refresh-DlEIC1K6.js")).invalidatePluginRuntimeDiscoveryAfterConfigMutation)({ logger: setupInferenceLog });
	} catch {
...
async function reloadCodexRegistryAfterActivation(params) {
	...
	try {
		await (params.deps.refreshPluginRegistryAfterConfigMutation ?? (await import("./registry-refresh-DlEIC1K6.js")).refreshPluginRegistryAfterConfigMutation)({
			config: sourceConfig,
	...
	try {
		(params.deps.ensurePluginRegistryLoaded ?? (await import("./runtime-registry-loader-B2rmHNCb.js")).ensurePluginRegistryLoaded)({
			scope: "all",
...
async function runSetupInferenceTest(params) {
	...
	try {
		...
		let result;
		if (plan.runner === "cli") result = await (deps.runCliAgent ?? (await import("./cli-runner-CCQXLSf9.js")).runCliAgent)({
		...
		else result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-ClAd_F7t.js")).runEmbeddedAgent)({
			preparedRunAdmission,
			sessionId,
			sessionKey,
			sessionManager,
...
async function persistActivatedSetupInference(input) {
	...
	const { stripPendingPluginInstallRecords } = await import("./install-record-commit-B7gPOqSM.js");
	...
	const transformConfig = deps.transformConfigWithPendingPluginInstalls ?? (await import("./install-record-commit-B7gPOqSM.js")).transformConfigWithPendingPluginInstalls;
	...
	try {
		...
		if (params.kind === "codex-cli") {
			const preparationFailure = await withPluginLifecycleLease({ signal: params.signal }, async () => {
				...
				const ensured = await (deps.ensureCodexRuntimePlugin ?? (await import("./codex-runtime-plugin-install-DyQHVcWv.js")).ensureCodexRuntimePluginForModelSelection)({
				...
				const refreshPluginRegistry = deps.refreshPluginRegistryAfterConfigMutation ?? (await import("./registry-refresh-DlEIC1K6.js")).refreshPluginRegistryAfterConfigMutation;
		...
		if (committedConfig && params.surface === "gateway" && params.kind === "codex-cli") try {
			...
			await (deps.refreshPreparedModelRuntimeSnapshots ?? (await import("./prepared-model-runtime-BH9aCf3S.js")).refreshPreparedModelRuntimeSnapshots)(codexReloadedRuntimeConfig);
		} catch (error) {
