...
import "./cdp.helpers-CDKXkF5Z.js";
import { a as resolveProfile } from "./config-KSi3HkFI.js";
import "./bounded-utf8-tail-C3tCSEoX.js";
import { a as getOrCreateProfileRuntime, h as withProfileOperationLease, m as waitForProfileOperation, o as getProfileLifecycle, s as isBrowserRuntimeRunning } from "./server-context.lifecycle-DI7diuDR.js";
import { n as readExtensionRelayToken } from "./relay-auth-CT67xN2t.js";
...
var ExtensionRelayBridge = class {
	...
	captureOperationTarget(targetId) {
		...
		return () => this.extension === extension && this.tabs.get(target.tabId) === target.tab ? target.tab.attached?.targetId : void 0;
	}
	/**
	...
	*/
	...
	handleExtensionGone() {
		...
		for (const [tabId, tab] of this.tabs) {
			tab.restoreAttachment ||= tab.attached !== void 0 || tab.attaching !== void 0;
	...
	syncTabs(tabs) {
		...
		for (const info of tabs) {
			...
			if (shouldAutoAttach && shouldAttach) this.ensureTabAttached(info.tabId).then(({ targetId, sessionId }) => {
	...
	enumerateTargetInfos() {
		if (!this.extensionConnected) return {
			status: "unavailable",
			reason: "extension-disconnected"
		};
		if ([...this.tabs.values()].some((tab) => !tab.attached)) return {
			status: "unavailable",
			reason: "target-identity-unresolved"
		};
		return {
			status: "available",
			targetInfos: [...this.tabs.values()].map((tab) => this.targetInfoForTab(tab, tab.attached?.targetId ?? ""))
		};
	}
