...
import { D as resolveExpiresAtMsFromDurationMs, b as parseFiniteNumber, g as isFutureDateTimestampMs } from "./number-coercion-CLj0HTDM.js";
...
import { D as PLAYWRIGHT_TARGET_INFO_TIMEOUT_MS, F as stripCdpUrlCredentials, M as withManagedProxyForCdpUrl, N as withNoProxyForCdpUrl, P as getHeadersWithAuth, _ as withCdpSocket, a as fetchJson, c as isWebSocketUrl, f as scopeCdpPolicyToConfiguredEndpoint, g as openCdpWebSocket, l as normalizeCdpHttpBaseForJsonEndpoints, n as assertCdpEndpointAllowed, t as appendCdpPath, u as redactCdpErrorText, y as getPlaywrightCore } from "./cdp.helpers-CDKXkF5Z.js";
...
import { L as withBrowserNavigationPolicy, M as assertBrowserNavigationAllowed, N as assertBrowserNavigationRedirectChainAllowed, P as assertBrowserNavigationResultAllowed, R as readCdpMainFrameDocumentIdentity, j as InvalidBrowserNavigationUrlError, m as AX_REF_PATTERN, n as getChromeWebSocketEndpoint, y as normalizeCdpWsUrl } from "./chrome-BeE6BX5Z.js";
...
function refLocator(page, ref) {
	...
	if (isRoleRef || AX_REF_PATTERN.test(normalized)) {
		...
		if (!isRoleRef && info.domMarker) return scope.locator(`[${BROWSER_REF_MARKER_ATTRIBUTE}="${normalized}"]`);
...
async function readPagesViaPlaywright(opts, attempt) {
	return await withPlaywrightSafeReadReconnect({
		...
		const resolvedPages = pageResults.flatMap((result) => result.status === "available" ? [result.page] : []);
		if (resolvedPages.length === 0 && pageResults.some((result) => result.status === "unresolved")) return {
			status: "unavailable",
...
async function createPageViaPlaywright(opts) {
	...
	if (targetUrl !== "about:blank") {
		...
		try {
			...
			if (!isPolicyDenyNavigationError(err) && !(err instanceof BlockedBrowserTargetError)) await page.close().catch(() => {});
			throw err;
		}
		try {
...
//#endregion
export { getObservedBrowserStateForPage as A, retirePlaywrightBrowserConnection as C, armObservedDialogResponseOnPage as D, isDownloadStartingNavigationError as E, createDownloadCaptureForPage as F, isBrowserObservedDialogBlockedError as I, respondToObservedDialogOnPage as M, restoreRoleRefsForTarget as N, createObservedDialogAbortSignalForPage as O, storeRoleRefsForTarget as P, pageTargetInfo as S, beginActionDownloadCaptureOnPage as T, wasBrowserNavigationSourcePreservedAfterPolicyDenial as _, forceDisconnectPlaywrightForTarget as a, ensureContextState as b, listPagesViaPlaywright as c, withPageScopedCdpClient as d, assertPageNavigationCompletedSafely as f, quarantineBlockedNavigationTarget as g, isPolicyDenyNavigationError as h, focusPageByTargetIdViaPlaywright as i, markObservedDialogsHandledRemotelyForPage as j, ensurePageState as k, refLocator as l, gotoPageWithNavigationGuard as m, closePlaywrightBrowserConnection as n, getMainFrameDocumentIdentityViaPlaywright as o, closeBlockedNavigationTarget as p, createPageViaPlaywright as r, getObservedBrowserStateViaPlaywright as s, closePageByTargetIdViaPlaywright as t, markBackendDomRefsOnPage as u, withPageNavigationRequestGuard as v, retirePlaywrightBrowserConnectionExact as w, getPageForTargetId as x, connectBrowser as y };
