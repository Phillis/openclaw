...
import { n as normalizeCompatibilityConfig } from "./doctor-contract-D52RneLG.js";
import { t as DISCORD_LEGACY_CONFIG_RULES } from "./doctor-shared-cypKrFwC.js";
import { t as isDiscordMutableAllowEntry } from "./security-doctor-BGDYFaWW.js";
...
function collectDiscordIdLists(prefix, account) {
	...
	const execApprovals = asNullableRecord(account.execApprovals);
	if (execApprovals) refs.push({
		pathLabel: `${prefix}.execApprovals.approvers`,
		holder: execApprovals,
		key: "approvers"
	});
	const guilds = asNullableRecord(account.guilds);
	if (!guilds) return refs;
	for (const guildId of Object.keys(guilds)) {
		const guild = asNullableRecord(guilds[guildId]);
		if (!guild) continue;
		refs.push({
		...
		for (const channelId of Object.keys(channels)) {
			const channel = asNullableRecord(channels[channelId]);
			if (!channel) continue;
			refs.push({
...
function collectDiscordTranscriptsAutoStartWarnings(cfg) {
	if (cfg.transcripts?.enabled === false || !Array.isArray(cfg.transcripts?.autoStart)) return [];
	...
	return cfg.transcripts.autoStart.flatMap((entry, index) => {
		...
		if (providerId !== discordVoiceTranscriptsSourceProvider.id && !discordVoiceTranscriptsSourceProvider.aliases?.includes(providerId ?? "") || normalizeOptionalString(entry.accountId)) return [];
		...
		return [`- ${path} cannot select a Discord voice account: ${resolution.error} Set ${path}.accountId to the intended enabled voice account, or set channels.discord.defaultAccount when one account should be the global default.`];
	});
}
function collectDiscordMutableAllowlistWarnings(cfg) {
	...
	for (const scope of collectProviderDangerousNameMatchingScopes(cfg, "discord")) {
		...
		const guilds = asNullableRecord(scope.account.guilds);
		if (!guilds) continue;
		for (const [guildId, guildRaw] of Object.entries(guilds)) {
			const guild = asNullableRecord(guildRaw);
			if (!guild) continue;
			addHits(`${scope.prefix}.guilds.${guildId}.users`, guild.users);
			const channels = asNullableRecord(guild.channels);
			if (!channels) continue;
			for (const [channelId, channelRaw] of Object.entries(channels)) {
				const channel = asNullableRecord(channelRaw);
