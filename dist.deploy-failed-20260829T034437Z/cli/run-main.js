...
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-jXZ6UAw0.js");
const loadManifestCommandAliasesRuntimeModule = async () => await import("../manifest-command-aliases.runtime-CzcUSgo0.js");
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { ensureCliExecutionBootstrap }, { defaultRuntime }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-command-BTK4uyqv.js"),
		import("../version-B4pBx2Bg.js"),
		import("../banner-somSRpoB.js"),
		import("../command-execution-startup-CjZ8pWf1.js"),
	...
	const beforeRun = async (opts) => {
		...
		if (!await startupTrace.measure("gateway-run-pre-bootstrap", async () => {
			const { prepareGatewayRunBootstrap, recheckGatewayRunBootstrap, wasPreparedGatewayRunCoreStatePristine, wasPreparedGatewayRunStatePristine } = await import("../pre-bootstrap-D3yU9m2v.js");
...
async function closeCliResources() {
	const finalizers = [
		async () => {
			const { listRegisteredAgentHarnesses, disposeRegisteredAgentHarnesses } = await import("../registry-55W1BeF6.js");
		...
		async () => {
			...
			if (hasManagedProviderLocalServices()) {
				const { stopManagedProviderLocalServices } = await import("../provider-local-service-mjATAvJB.js");
		...
		async () => {
			...
			if (getActiveMcpLoopbackRuntime()) {
				const { closeMcpLoopbackServer } = await import("../mcp-http-D6Wmh3N2.js");
		...
		async () => {
			...
			if (hasMemoryRuntime()) {
				const { closeActiveMemorySearchManagersCore } = await import("../memory-runtime-DIzDqg2E.js");
...
async function resolveReachableGateway(config, options) {
	...
	const { probeGatewayConfiguredModel } = await import("../onboard-helpers-kk_BLYXI.js");
...
async function resolveGatewayProbePlan(config) {
	...
	if (normalizeOptionalString(config.gateway?.mode) === "remote" && remoteUrl) try {
		const { resolveGatewayClientBootstrap } = await import("../client-bootstrap-DceMu-wI.js");
...
async function resolveLocalGatewayProbeTargets(config) {
	const [{ resolveControlUiLinks }, { resolveGatewayClientBootstrap }, { readActiveGatewayLockPort }] = await Promise.all([
		...
		import("../client-bootstrap-DceMu-wI.js"),
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-CDq4Tvk4.js"), import("../coverage-39SssipS.js")]));
...
async function runCliWithPreparedOutputMode(originalArgv, options) {
	...
	if (!isHelpOrVersionInvocation && isGatewayRunInvocation) await startupTrace.measure("gateway-run-select-environment", async () => {
		const [{ selectGatewayRunEnvironment }, { defaultRuntime }] = await Promise.all([import("../pre-bootstrap-D3yU9m2v.js"), import("../runtime-BO0y5md7.js")]);
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-DS-dUNoh.js").then((configIo) => useSourceOnlyBestEffortConfig ? configIo.readSourceConfigBestEffort() : configIo.readBestEffortConfig({
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-D7wfJUUo.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-DuskGXWg.js");
		...
		if (bareSessionInvocation) {
			...
			const { runTuiCliAction } = await import("../tui-cli-Dsl7s-j2.js");
		...
		if (bareRootLaunchTarget) {
			if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
				...
				const { runRemoteGatewayInferenceOnboarding } = await import("../onboard-remote-gateway--Tzhz50U.js");
			...
			if (bareRootLaunchTarget.kind === "onboarding") {
				...
				const { setupWizardCommand } = await import("../onboard-nuzyjRkG.js");
			...
			if (bareRootLaunchTarget.kind === "tui") {
				...
				const { runTui } = await import("../tui-CPl3a8pI.js");
				await runTui({
		...
		if (!isHelpOrVersionInvocation) {
			const route = await startupTrace.measure("route-import", () => import("../route-xrOpzbPM.js"));
		...
		try {
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-BMKjO46X.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-BFN1MBKh.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const { registerPluginCliCommandsFromValidatedConfig } = await import("../cli-A-W0aiU-.js");
					const startupPolicy = resolveCliStartupPolicy({
						argv: parseArgv,
