import { o as createProviderHttpError, p as readProviderJsonObjectResponse, r as assertOkOrThrowProviderError } from "./provider-http-errors-BXG5plR9.js";
import { g as waitProviderOperationPollInterval, h as resolveProviderOperationTimeoutMs, n as createProviderOperationDeadline } from "./shared-uZXUsfMB.js";
import "./provider-http-S5IuZe1q.js";
import { O as withRemoteHttpResponse, T as readEmbeddingBatchJsonl, b as runEmbeddingBatchGroups, j as formatBatchErrorDetail, k as EmbeddingBatchUnavailableError, o as debugEmbeddingsLog, p as resolveEmbeddingEndpointUrl, x as buildBatchHeaders, y as buildEmbeddingBatchGroupOptions } from "./memory-core-host-engine-embeddings-ByemCEFP.js";
import { t as parseGeminiAuth } from "./gemini-auth-B2h6lAH-.js";
import { a as sanitizeGeminiEmbedding } from "./embedding-provider-3Q0A0mCM.js";
...
function getGeminiBatchFileUrl(baseUrl, route, fileId) {
	...
	if (route === "upload" || base.origin === "https://generativelanguage.googleapis.com") {
		...
		base.pathname = version ? `${version[1]}/${route}/${version[2]}` : route === "upload" ? `${pathname}/upload` : pathname;
	}
	const endpoint = route === "upload" ? fileId : `${fileId.startsWith("files/") ? fileId : `files/${fileId}`}:download`;
	const url = new URL(resolveEmbeddingEndpointUrl(base.href, endpoint));
	url.searchParams.set(route === "upload" ? "uploadType" : "alt", route === "upload" ? "multipart" : "media");
	return url.href;
}
...
async function submitGeminiBatch(params) {
	...
	const batchEndpoint = resolveEmbeddingEndpointUrl(baseUrl, `${params.gemini.modelPath}:asyncBatchEmbedContent`);
...
function applyGeminiBatchOutputLine(params) {
	...
	const embedding = sanitizeGeminiEmbedding(params.line.embedding?.values ?? params.line.response?.embedding?.values ?? [], params.expectedDimensions);
