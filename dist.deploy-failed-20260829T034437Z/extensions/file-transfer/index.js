...
import { a as DIR_LIST_TOOL_DESCRIPTOR, c as FILE_FETCH_TOOL_DESCRIPTOR, d as FILE_WRITE_TOOL_DESCRIPTOR, r as DIR_FETCH_TOOL_DESCRIPTOR } from "../../descriptors-CxBbqcc6.js";
//#region extensions/file-transfer/src/shared/lazy-node-invoke-policy.ts
const loadFileTransferNodeInvokePolicy = async () => {
	const { createFileTransferNodeInvokePolicy } = await import("../../node-invoke-policy-zzM1WTj1.js");
...
var file_transfer_default = definePluginEntry({
	...
	nodeHostCommands: [
		{
			...
			handle: async (paramsJSON) => {
				const { handleFileFetch } = await import("../../file-fetch-DQVHDpIv.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirList } = await import("../../dir-list-h-t5H9H7.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirFetch } = await import("../../dir-fetch-DyikKuZ4.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleFileWrite } = await import("../../file-write-Cx_Wx6yP.js");
	...
	register(api) {
		api.registerCli(async ({ program }) => {
			const { registerFileTransferCli } = await import("../../cli-DyMQFjLO.js");
