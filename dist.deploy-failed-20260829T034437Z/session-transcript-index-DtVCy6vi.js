...
const SYNC_REBUILD_MAX_BYTES = 4 * 1024 * 1024;
...
function shouldRebuildSessionTranscriptIndexSynchronously(db, sessionId, events = []) {
	...
	const stored = executeSqliteQueryTakeFirstSync(db, getIndexKysely(db).selectFrom("transcript_events").select((eb) => [eb.fn.countAll().as("event_count"), eb.fn.sum(eb.fn("length", ["event_json"])).as("event_bytes")]).where("session_id", "=", sessionId));
...
//#endregion
export { mergeSessionTranscriptVisiblePathWithOpaqueAppendPath as C, selectSessionTranscriptLeafControlledPath as D, selectSessionTranscriptActiveEntries as E, selectSessionTranscriptTreePathNodes as O, mergeSessionTranscriptTreePaths as S, scanSessionTranscriptTree as T, selectVisibleTranscriptEventEntries as _, indexAppendedTranscriptEventInTransaction as a, isSessionTranscriptLeafControl as b, reconcileSessionTranscriptIndexInTransaction as c, appendPreparedSessionTranscriptProjectionChunkInTransaction as d, claimPreparedSessionTranscriptProjectionInTransaction as f, resolveVisibleTranscriptAppendParentId as g, prepareSessionTranscriptProjection as h, deleteSessionTranscriptIndexInTransaction as i, sessionTranscriptIndexNeedsReconcile as l, finalizePreparedSessionTranscriptProjectionInTransaction as m, SYNC_REBUILD_MAX_ROWS as n, listSessionsNeedingTranscriptIndexReconcile as o, deletePreparedSessionTranscriptProjectionChunkInTransaction as p, deleteOrphanedTranscriptIndexRowsInTransaction as r, markSessionTranscriptIndexDirtyInTransaction as s, SYNC_REBUILD_MAX_BYTES as t, shouldRebuildSessionTranscriptIndexSynchronously as u, selectVisibleTranscriptEvents as v, parseSessionTranscriptTreeEntry as w, isSessionTranscriptSideAppendEntry as x, isCanonicalSessionTranscriptEntry as y };
