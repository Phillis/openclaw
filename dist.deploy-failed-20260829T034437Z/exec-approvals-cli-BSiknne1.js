...
import { n as redactExecApprovals, t as normalizeExecApprovals } from "./exec-approvals-B5vSSaiI.js";
import { c as readExecApprovalsSnapshot, p as updateExecApprovals } from "./exec-approvals-generated-migration-DfpexxOF.js";
...
import { a as nodesCallOpts, u as resolveCliNodeId } from "./rpc-DSpNcV0f.js";
import { n as collectExecPolicyScopeSnapshots, t as SESSION_EXEC_OVERRIDES_NOTE } from "./exec-approvals-effective-C11Xml0k.js";
...
function failApprovalsCommand(err, opts) {
	rethrowExpectedCliError(err);
	const message = formatCliError(err);
	if (opts.json) throw new Error(message);
...
async function loadWritableAllowlistAgent(opts) {
	...
	if (agentKey !== "*") {
		const { config } = await loadConfigForApprovalsTarget({
			opts,
			source: !opts.gateway && !opts.node ? "local" : opts.gateway ? "gateway" : "node"
		});
		if (!config) exitWithError("Config unavailable; cannot validate --agent.");
