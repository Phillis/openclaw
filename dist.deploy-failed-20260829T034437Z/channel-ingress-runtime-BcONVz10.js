...
import { f as recordChannelIngressResolution } from "./admission-evidence-C8iAKYLB.js";
...
function fieldAuthentication(field, value) {
	return typeof field.authentication === "function" ? field.authentication(value) : field.authentication;
}
...
function adapterEntry(params) {
	...
	return {
		...
		authentication: fieldAuthentication(params.field, params.entry) ?? (dangerous ? "mutable" : "asserted"),
		dangerous,
...
function createIdentitySubject(identity, input) {
	return { identifiers: identityFields(identity).flatMap((field, index) => {
		...
		const authentication = input.authentication !== void 0 ? input.authentication[field.key] ?? "unverified" : fieldAuthentication(field, value) ?? (dangerous ? "mutable" : "asserted");
		return [{
			opaqueId: field.key,
			kind: field.kind,
			value,
...
function fanInChannelIngressLifecycles(inputs) {
	...
	const failAll = (error) => fanOut((lifecycle) => lifecycle.onFailed ? lifecycle.onFailed(error) : lifecycle.onAbandoned());
	const supportsCancellation = lifecycles.every((lifecycle) => lifecycle.onCancelled !== void 0);
	const cancelAll = () => fanOut((lifecycle) => lifecycle.onCancelled ? lifecycle.onCancelled() : lifecycle.onAbandoned());
	return {
		lifecycle: {
