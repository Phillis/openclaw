...
import { a as normalizeChannelId, n as getLoadedChannelPlugin, t as getChannelPlugin } from "./registry-CL5HFEAI.js";
...
import { n as resolveChannelSetupCliOptionMetadata } from "./cli-add-options-BKkMPg2a.js";
...
import { l as shouldUseWizard } from "./shared-CT2ea0M7.js";
...
const channelSetupPluginInstallLoader = createLazyImportLoader(() => import("./plugin-install-BbZ1w95z.js"));
const onboardChannelsLoader = createLazyImportLoader(() => import("./onboard-channels-BA395Ry6.js"));
...
async function resolveCatalogChannelEntry(raw, cfg) {
	...
	if (!trimmed) return;
	return (cfg ? await import("./trusted-catalog-CUdW7EN_.js").then(({ listTrustedChannelPluginCatalogEntries }) => listTrustedChannelPluginCatalogEntries({
		cfg,
		workspaceDir: resolveAgentWorkspaceDir(cfg, resolveAgentOperationAgentId(cfg))
	})) : await import("./catalog--hAJ5p-N.js").then(({ listRawChannelPluginCatalogEntries }) => listRawChannelPluginCatalogEntries({ excludeWorkspace: true }))).find((entry) => {
...
async function channelsAddCommandImpl(opts, runtime, params) {
	...
	if (shouldUseWizard(params)) {
		const { resolveInitialWizardChannelTarget, runChannelsAddWizardFlow } = await import("./add-wizard-Bq7ZyNvb.js");
	...
	if (catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-CUUejx00.js");
