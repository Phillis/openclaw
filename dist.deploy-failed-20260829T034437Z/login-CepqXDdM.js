import { D as resolveExpiresAtMsFromDurationMs, F as resolveTimerTimeoutMs, T as positiveSecondsToSafeMilliseconds, _ as nonNegativeSecondsToSafeMilliseconds } from "./number-coercion-CLj0HTDM.js";
...
import { _ as normalizeGithubCopilotDomain } from "./provider-auth-DI4TAoBi.js";
...
async function pollForAccessToken(params) {
	...
	while (Date.now() < params.expiresAt) {
		...
		if (err === "slow_down") {
			intervalMs = Math.max(Math.min(Number.MAX_SAFE_INTEGER, intervalMs + GITHUB_DEVICE_FLOW_SLOW_DOWN_INCREMENT_MS), positiveSecondsToSafeMilliseconds(json.interval) ?? 0);
			continue;
		}
...
//#endregion
export { runGitHubCopilotDeviceFlow as t };
