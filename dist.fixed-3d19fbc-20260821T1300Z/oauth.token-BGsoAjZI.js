import { k as resolveExpiresAtMsFromDurationSeconds } from "./number-coercion-oCkfUEEq.js";
import { c as isRecord } from "./record-coerce-DItp3I4t.js";
import { r as fetchWithSsrFGuard } from "./fetch-guard-IFayOKvf.js";
import { a as createProviderHttpError, p as readProviderJsonResponse } from "./provider-http-errors-DwYSuIHs.js";
...
function assertMSTeamsTokenResponseObject(data, failureLabel) {
	if (!isRecord(data)) throw new Error(`MSTeams ${failureLabel} failed: invalid token response fields`);
	return data;
}
...
async function fetchMSTeamsTokens(params) {
	...
	try {
		...
		return parseMSTeamsTokenResponse(assertMSTeamsTokenResponseObject(await readProviderJsonResponse(response, `MSTeams ${params.failureLabel} failed`), params.failureLabel), params.failureLabel);
	} finally {
		await release();
	}
}
