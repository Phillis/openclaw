...
import { m as shortenHomePath, t as CONFIG_DIR } from "./utils-D9gvQMP6.js";
...
import { S as isSkillsMachineOutput } from "./argv-ubyZhwcH.js";
import { r as stripAnsi, t as sanitizeForLog } from "./ansi-9qL8iF9E.js";
import { l as resolveAgentIdByWorkspacePath } from "./agent-scope-D9GLFAyB.js";
...
import { n as isImmutableGitCommitRef, r as parseGitPluginSpec } from "./git-install-CytRnPyN.js";
import { u as writeJson } from "./json-C_hP6p1e.js";
...
import { s as withInstallWorkspace } from "./install-source-utils-DyDPgWM7.js";
import { t as parseSkillFrontmatter } from "./frontmatter-BE0jYufM.js";
import { a as restoreCuratedSkill, i as pinCuratedSkill, o as unpinCuratedSkill, r as getSkillCuratorStatus } from "./curator-XlkqyZug.js";
import { n as decorativePrefix, t as decorativeEmoji } from "./decorative-emoji-DymOaaLu.js";
import { c as searchSkillsFromClawHub, g as installExtractedSkillRoot, h as untrackClawHubSkill, l as readVerifiedClawHubSkillSourceUrl, m as readTrackedClawHubSkillSlugs, o as resolveClawHubSkillVerificationTarget, r as updateSkillsFromClawHub, t as installSkillFromClawHub, y as validateRequestedSkillSlug } from "./clawhub-Br58P2LA.js";
import { i as CLAWHUB_SKILLS_SH_TRUST_LABEL, l as fetchClawHubSkillVerification, o as fetchClawHubSkillCard, t as CLAWHUB_TRUST_ERROR_CODE } from "./clawhub-install-trust-BsDhg4Ch.js";
import { n as resolveSkillStatusEntry } from "./status-BqwnYiuT.js";
import { i as readSkillProposalDraftFile, r as readSkillProposalDraftDirectory } from "./proposal-draft-zkJQJamp.js";
import { a as proposeUpdateSkill, c as reviseSkillProposal, f as inspectSkillProposal, i as proposeCreateSkill, n as applySkillProposal, o as quarantineSkillProposal, p as listSkillProposals, s as rejectSkillProposal } from "./service-D6R5IdXh.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-RsKHgCVk.js";
import { t as resolveOptionFromCommand } from "./cli-utils-Dg8R0Gwl.js";
import { n as parseStrictPositiveIntOption } from "./helpers-DMLW8VDh.js";
import { t as applyParentDefaultHelpAction } from "./parent-default-help-DQUF3qKA.js";
import { n as resolveClawHubRiskAcknowledgementCliOptions, t as resolveInstallPolicyWarningAcknowledgementCliOptions } from "./install-policy-warning-acknowledgement-HMAyC5S2.js";
...
async function loadSkillsStatusReport(options) {
	...
	const { buildWorkspaceSkillStatus } = await import("./status-DkNDzAGy.js");
...
async function runSkillProposalApply(resolved, proposalId) {
	const { callGateway, isGatewayCredentialsRequiredError, isGatewayTransportError } = await import("./call-DrR5WyUw.js");
	try {
		await callGateway({
			config: resolved.config,
			method: "health",
		...
		const { acquireGatewayLock } = await import("./gateway-lock-egrmX4RG.js");
		let lock;
		try {
...
function registerSkillsCli(program) {
	...
	skills.command("verify").description("Verify a ClawHub skill with ClawHub").argument("<skill-ref>", "ClawHub skill ref (@owner/slug)").option("--version <version>", "Verify a specific version").option("--tag <tag>", "Verify a dist tag").option("--card", "Print the generated Skill Card Markdown", false).option("--json", "Output as JSON", false).option("--global", "Resolve installed skill metadata from the shared managed skills directory", false).option("--agent <id>", "Target agent workspace (defaults to cwd-inferred, then default agent)").addHelpText("after", "\nExamples:\n  openclaw skills verify @owner/weather\n").action(async (slug, opts, command) => {
		let exitCode;
		const reportError = hasJsonOutput(opts) || opts.card !== true ? (message) => defaultRuntime.writeJson({ error: message }) : defaultRuntime.error;
		try {
	...
	const runWorkshopAction = async (opts, action, format) => {
		try {
	...
	const runWorkshopDraftAction = (opts, action, format = (proposal) => `${proposal.record.id}\n`) => runWorkshopAction(opts, async ({ config, workspaceDir, agentId }) => {
	...
	workshop.command("list").description("List pending and completed skill proposals").option("--json", "Output as JSON", false).action((opts) => runWorkshopAction(opts, ({ agentId, workspaceDir }) => listSkillProposals({
		agentId,
		workspaceDir
	...
	]]) workshop.command(name).description(description).argument("<proposal-id>", "Skill proposal id").option("--reason <text>", reasonDescription).option("--json", "Output as JSON", false).action((proposalId, opts) => runWorkshopAction(opts, ({ agentId, workspaceDir }) => action({
		agentId,
		eventActor: {
			type: "system",
			id: "cli"
