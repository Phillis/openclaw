...
function formatContextWindowWarningMessage(params) {
	...
	if (params.guard.source === "agentContextTokens") return `${base}; OpenClaw is capped by the agent's contextTokens setting (agents.list[].contextTokens or agents.entries.<id>.contextTokens, else agents.defaults.contextTokens), so raise that cap if you want to use more of the model context window`;
...
function formatContextWindowBlockMessage(params) {
	...
	if (!resolveContextWindowGuardHint({ runtimeBaseUrl: params.runtimeBaseUrl }).likelySelfHosted) return base;
	if (params.guard.source === "agentContextTokens") return `${base} OpenClaw is capped by the agent's contextTokens setting (agents.list[].contextTokens or agents.entries.<id>.contextTokens, else agents.defaults.contextTokens). Raise that cap.`;
