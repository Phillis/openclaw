...
async function runConfiguredSystemAgentText(params) {
	...
	try {
		...
		text = extractPlannerResultText(route.runner === "cli" ? await (params.deps?.runCliAgent ?? (await import("./cli-runner-t4mb-NuF.js")).runCliAgent)({
			...shared,
			preparedRunAdmission,
			executionMode: "side-question",
			cleanupCliLiveSessionOnRunEnd: true
		}) : await (params.deps?.runEmbeddedAgent ?? (await import("./embedded-agent-eyEW4bm0.js")).runEmbeddedAgent)({
			...shared,
			preparedRunAdmission,
			toolsAllow: [],
