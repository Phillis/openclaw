...
import { r as formatSystemAgentStartupMessage } from "./overview-Cs2G3rQv.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-D7oL5oyj.js";
import { g as setCommandLaneConcurrency, r as enqueueCommandInLane } from "./command-queue-B992TXUy.js";
import { n as describeSystemAgentPersistentOperation } from "./operations-CqBuFQx5.js";
import { n as isSystemAgentInferenceUnavailableError } from "./inference-error-4Boifouo.js";
...
import { f as listVisiblePendingApprovalRequests, i as buildRequestedApprovalEvent, s as handlePendingApprovalRequest } from "./approval-shared-BVHQZO7B.js";
import { t as getUpdateAvailable } from "./update-startup-DmGuUjWy.js";
import { n as getHealthCache } from "./health-state-Bbs5FPDZ.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-DSSUOWub.js";
import { t as WizardSession } from "./session-BhplepEX.js";
import { i as runExclusiveSystemAgentSetupActivation, n as SetupAdmissionBusyError, r as createAdmittedWizardSession, t as SETUP_ADMISSION_BUSY_MESSAGE } from "./setup-admission-COHvpMqL.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.detect": async ({ params, respond }) => {
		...
		const { detectSetupInferenceIsolated } = await import("./setup-inference-detection-BfCpwTka.js");
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = await createAdmittedWizardSession(() => new WizardSession(async (prompter, signal, runnerSession) => {
			await runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-TKBB_uEg.js"), import("./setup.shared-fjK1_Wf3.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-DFl7QGW0.js");
