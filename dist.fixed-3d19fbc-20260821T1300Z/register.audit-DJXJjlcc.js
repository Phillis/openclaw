...
import { R as timestampMsToIsoString, w as parseStrictPositiveInteger } from "./number-coercion-oCkfUEEq.js";
...
async function queryAuditRunInspection(params) {
	try {
		...
		if (!isUnsupportedRunInspectMethodError(error)) throw error;
...
function formatAuditRunInspection(result) {
	...
	if (result.identity.state === "present") {
		...
		lines.push(`  Context: ${safe(result.identity.context.contextId)}`, `  Created: ${timestampMsToIsoString(result.identity.context.createdAt) ?? String(result.identity.context.createdAt)}`, ...identityLines.slice(0, 8), "", "Authority", ...identityLines.slice(8), "", "Lineage", result.identity.context.lineage ? fieldLine("Parent", "present", result.identity.context.lineage.parentRunId ?? result.identity.context.lineage.parentContextId ?? `depth ${String(result.identity.context.lineage.depth)}`) : fieldLine("Parent", "absent"));
	...
	if (result.decisions.length === 0) lines.push("  none [absent]");
	else for (const receipt of result.decisions) lines.push(...decisionLines(receipt));
	lines.push("", "Missing evidence");
	lines.push(...result.coverage.missingEvidence.length > 0 ? result.coverage.missingEvidence.map((item) => `  - ${safe(item)}`) : ["  none"]);
	const remediation = [...result.identity.state === "present" ? [] : result.identity.remediation, ...result.decisions.flatMap((decision) => decision.remediation)];
