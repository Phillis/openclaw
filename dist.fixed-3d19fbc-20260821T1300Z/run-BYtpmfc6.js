...
import { _ as resolveGatewayPort, l as normalizeStateDirEnv, t as CONFIG_PATH } from "./paths-CqeDjSA4.js";
...
import { r as flushLogger } from "./logger-CufStxi-.js";
import { o as setConsoleSubsystemFilter, s as setConsoleTimestampPrefix } from "./console-BvkCzW3T.js";
...
import { j as findOpenClawStateDatabaseSchemaMigrationRequiredError } from "./openclaw-state-db-BciZ4rHE.js";
import { p as ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV } from "./config-env-vars-C1wvGC4M.js";
import { r as CONFIG_AUDIT_STORE_LABEL } from "./io.audit-BbTTac1R.js";
import { t as clearRuntimeConfigSnapshot } from "./runtime-snapshot-DIuCzlel.js";
import { i as isInvalidConfigError, r as isDoctorRecoverableInvalidConfigError } from "./io.invalid-config-D6XgN5OX.js";
...
import { h as resolveGatewayBindHost, o as isLoopbackHost, t as defaultGatewayBindMode } from "./net-BRYQcUG8.js";
import { n as formatGatewayPidList, t as findVerifiedGatewayListenerPidsOnPortSync } from "./gateway-processes-DUdgqg7t.js";
...
import { t as printClawBanner } from "./claw-banner-Dxxi8ctr.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-0F5NnA9C.js";
import { t as formatActiveTaskRestartBlocker } from "./task-restart-blocker-DNMfEXg3.js";
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-DSPmkLD0.js";
import { r as withProgress } from "./progress-F0nzmXYd.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-86BGsxSJ.js";
import { n as isGatewayRunFutureConfigAllowed, t as enforceGatewayRunFutureConfigGuard } from "./future-config-guard-ZP6V1VnA.js";
import { a as inspectGatewayCrashLoopBreaker, i as formatGatewayCrashLoopManualChannelStartHint, n as GATEWAY_CRASH_LOOP_RECOVERED_REASON, o as recordGatewayBootStart, r as completeGatewayBootLifecycle, s as recordGatewayCrashLoopRecovery, t as GATEWAY_CRASH_LOOP_BREAKER_REASON } from "./gateway-boot-lifecycle-suCc9PJc.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file--TUJDasJ.js");
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	const { createConfigRuntimeEnv } = await import("./env-vars-DRX8pnj2.js");
	const { resolveShellEnvFallbackTimeoutMs, shouldDeferShellEnvFallback, shouldEnableShellEnvFallback } = await import("./shell-env-CVsAMulx.js");
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-CXIBapua.js");
	return {
		enabled: true,
...
async function loadGatewayRunShellEnvFallback(plan) {
	const { loadShellEnvFallback } = await import("./shell-env-CVsAMulx.js");
...
async function clearGatewayRunShellEnvFallback(values) {
	...
	const { clearShellEnvAppliedKeys } = await import("./shell-env-CVsAMulx.js");
...
function createConfiguredGatewayHealthProbe(cfg) {
	...
	return async (params) => {
		...
		if (!tlsFingerprint) tlsFingerprint = (await import("./gateway-CTy7opcM.js").then(({ loadGatewayTlsRuntime }) => loadGatewayTlsRuntime({
			...tlsConfig,
			autoGenerate: false
...
async function maybeWriteGatewayStartupFailureBundle(err, reason = "gateway.startup_failed") {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-BIPU-l35.js");
...
async function runGatewayCommandOnce(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-DsEx-mIb.js");
	...
	const serverImportMeasurement = startupTrace.startMeasure("cli.server-import", () => import("./server-C4gDKlLT.js"));
	...
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-DsEx-mIb.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-BQHd74c0.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-DsEx-mIb.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-DDeTVZR8.js");
		...
		if (stale.length > 0) {
			...
			if (process.platform === "linux") {
				const { findSystemdGatewayInstallation, formatDuelingScopesWarning } = await import("./systemd-DRQ7KIjo.js");
	...
	if (opts.force) {
		...
		try {
			const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-B7PORd9c.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-RoECrtPs.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const [{ formatPortDiagnostics }, { inspectPortUsage }] = await Promise.all([import("./ports-format-jqsgnc08.js"), import("./ports-inspect-OgD2z9jJ.js")]);
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-DjGY-7J7.js");
		...
		if (findOpenClawAgentDatabaseMediaMigrationRequiredError(err)) try {
			const { parkCurrentLaunchAgentForMaintenance } = await import("./launchd-6NzpKCP8.js");
...
async function runGatewayCommand(opts, hooks = {}, recoveryDeps) {
	try {
		...
		const { offerInvalidConfigRecovery } = await import("./invalid-config-recovery-0OMg7VLK.js");
