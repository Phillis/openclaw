...
import { _ as toAgentEntriesRecord, r as listAgentEntries } from "./agent-scope-config-CsnnOL14.js";
import { N as validateConfigObjectWithPlugins, f as readConfigFileSnapshotWithPluginMetadata, l as readConfigFileSnapshot, nt as resolveConfigSnapshotHash } from "./io-BTBpQ7uO.js";
...
import { i as sameDefaultInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-DFbS6qbC.js";
...
async function createSystemAgentModelSelectionUpdater(params) {
	const [agentScope, modelConfig, runtimePolicy] = await Promise.all([
		...
		import("./shared-DPSYTxW_.js"),
		import("./model-runtime-policy-CmDglk1g.js")
	]);
	const modules = {
		agentScope,
		modelConfig,
...
async function applySystemAgentSetup(params, hooks) {
	...
	const [{ readSetupConfigFileSnapshot, resolveQuickstartGatewayDefaults }, onboardHelpers, { applyLocalSetupWorkspaceConfig, resolveOnboardingWorkspaceConflict }, { transformConfigWithPendingPluginInstalls }] = await Promise.all([
		import("./setup.shared-fjK1_Wf3.js"),
		import("./onboard-helpers-BTdKJ26T.js"),
		import("./onboard-config-CHVLAXRs.js"),
		import("./install-record-commit-CjuCS0pJ.js")
	]);
	...
	let guardModules = expectedAgentId || expectedAgentDir || expectedModelRef ? await Promise.all([import("./agent-scope-DHmCBvSP.js"), import("./model-selection-oGawnPze.js")]) : void 0;
	...
	if (startedWithoutAuthoredRoster) {
		...
		guardModules ??= await Promise.all([import("./agent-scope-DHmCBvSP.js"), import("./model-selection-oGawnPze.js")]);
	...
	await runCommittedFollowUp(async () => {
		const { updateExecApprovals } = await import("./exec-approvals-ZUkrvHzq.js");
	...
	if (refreshPluginRegistry && enablePluginId) await runCommittedFollowUp(async () => {
		const { refreshPluginRegistryAfterConfigMutation } = await import("./registry-refresh-BhS9qvzq.js");
	...
	if (surface === "cli") await runCommittedFollowUp(async () => {
		const { ensureGatewayServiceForOnboarding } = await import("./setup.finalize-RJ_UkS5n.js");
		...
		if (gateway.status === "failed") lines.push(`Gateway service: ${gateway.error}`);
			...
			const probe = await onboardHelpers.waitForGatewayReachable({
				...
				password: settings.authMode === "password" ? await (await import("./setup.secret-input-BDxDXnKW.js")).resolveSetupSecretInputString({
					config: nextConfig,
