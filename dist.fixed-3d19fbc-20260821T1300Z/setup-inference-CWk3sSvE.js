...
import { b as tryResolveLegacyCompatibilityAgentId, d as resolveAgentWorkspaceDir, g as resolveSystemAgentTargetAgentId, p as resolveDefaultAgentId, r as listAgentEntries, s as resolveAgentConfig } from "./agent-scope-config-CsnnOL14.js";
import { t as createSubsystemLogger } from "./subsystem-DNgaGOch.js";
import { o as normalizePluginTargetConfig } from "./config-state-DLiU5GYQ.js";
...
import { t as resolveModelRuntimePolicy } from "./model-runtime-policy-gZbN_P9A.js";
import { s as resolvePluginMetadataSnapshot } from "./plugin-metadata-snapshot-DciqS8td.js";
...
import { n as enablePluginInConfig } from "./enable-CD9vPpHS.js";
...
import { c as readCodexCliCredentialsCached, l as readGeminiCliCredentialsCached, o as readClaudeCliCredentialsCached, s as readCodexCliActiveApiKey } from "./external-cli-sync-DCM6idGB.js";
...
import { a as resolveCliBackendLiveSessionRequirement, i as resolveCliBackendConfig } from "./cli-backends-BqBcI5I1.js";
import { t as areRuntimeModelRefsEquivalent } from "./model-runtime-aliases-BoIMzL8U.js";
import { t as loadAgentRuntimePluginRegistryHandle } from "./runtime-plugins-7vwSc1WD.js";
import { d as normalizeAuthProfileCredential } from "./profiles-CaIWIvwD.js";
...
import { a as probeLocalCommand } from "./overview-Cs2G3rQv.js";
...
import { n as resolveManifestProviderAuthChoice, r as resolveManifestProviderAuthChoices } from "./provider-auth-choices-Cf_X4oEk.js";
import { n as loadAuthoredSetupConfig } from "./onboarding-welcome-D7oL5oyj.js";
import { a as resolveSystemAgentVerifiedInferenceRoute, n as createSystemAgentVerifiedInferenceBinding, r as hasCurrentSystemAgentOwnerPluginArtifacts, t as captureSystemAgentOwnerPluginArtifacts } from "./verified-inference-FlkjxyZP.js";
import { t as CliExecutionAuthProfileError } from "./cli-execution-auth-BZZXBeVT.js";
import { i as sameDefaultInferenceRoute, n as projectInferenceRoute, r as resolveSystemAgentConfiguredRouteFromConfig } from "./inference-route-DFbS6qbC.js";
import { t as buildAgentRuntimeAuthPlan } from "./auth-CV7_GVmX.js";
import { t as applyAutoLocalModelLean } from "./local-model-lean-auto-BPUF8q0b.js";
import { i as appendSystemAgentAuditEntry } from "./audit-BL7eqPcK.js";
import { i as createSystemAgentModelSelectionUpdater, r as createQuickstartNotePrompter, t as applySystemAgentModelSelection } from "./setup-apply-BaqfuRpy.js";
...
import { a as describeFailoverError } from "./failover-error-DKFCUqL9.js";
import { a as runProviderPluginAuthMethodUnpersisted, n as applyProviderPluginAuthMethodResultConfig } from "./provider-auth-choice-C-rlNdsa.js";
...
async function resolveSetupInferenceWorkspace(params) {
	...
	const { DEFAULT_WORKSPACE } = await import("./onboard-helpers-BTdKJ26T.js");
	return {
...
async function extractRunWinnerError(plan, result) {
	...
	if (winnerProvider === plan.provider) {
		...
		const { resolveDirectBundledProviderPolicySurface } = await import("./provider-policy-surface-DrEXw3ea.js");
...
async function cleanupSetupInferenceTempDir(params) {
	try {
		(params.deps.disposeOpenClawAgentDatabaseByPath ?? (await import("./openclaw-agent-db-B1pM2_Ig.js")).disposeOpenClawAgentDatabaseByPath)(path.join(params.tempDir, "agent", "openclaw-agent.sqlite"));
	} catch {
...
async function isCodexInstallRecordPersisted(record, deps) {
	try {
		const currentInstallRecords = await (deps.readPersistedInstalledPluginIndexInstallRecords ?? (await import("./installed-plugin-index-records-DiwFXIL8.js")).readPersistedInstalledPluginIndexInstallRecords)();
...
async function retainUnownedCodexInstall(params) {
	...
	try {
		const marked = await (params.deps.markRetainedManagedNpmInstall ?? (await import("./managed-npm-retention-DxWgaBAU.js")).markRetainedManagedNpmInstall)({
...
async function clearUnownedCodexInstallCaches(deps) {
	try {
		(deps.clearLoadInstalledPluginIndexInstallRecordsCache ?? (await import("./installed-plugin-index-records-DiwFXIL8.js")).clearLoadInstalledPluginIndexInstallRecordsCache)();
	} catch {
	...
	try {
		await (deps.invalidatePluginRuntimeDiscoveryAfterConfigMutation ?? (await import("./registry-refresh-BhS9qvzq.js")).invalidatePluginRuntimeDiscoveryAfterConfigMutation)({ logger: setupInferenceLog });
	} catch {
...
async function reloadCodexRegistryAfterActivation(params) {
	...
	try {
		await (params.deps.refreshPluginRegistryAfterConfigMutation ?? (await import("./registry-refresh-BhS9qvzq.js")).refreshPluginRegistryAfterConfigMutation)({
			config: sourceConfig,
	...
	try {
		(params.deps.ensurePluginRegistryLoaded ?? (await import("./runtime-registry-loader-8SWeZP3j.js")).ensurePluginRegistryLoaded)({
			scope: "all",
...
async function runSetupInferenceTest(params) {
	...
	try {
		...
		let result;
		if (plan.runner === "cli") result = await (deps.runCliAgent ?? (await import("./cli-runner-t4mb-NuF.js")).runCliAgent)({
		...
		else result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-eyEW4bm0.js")).runEmbeddedAgent)({
			preparedRunAdmission,
			sessionId,
			sessionKey,
			sessionManager,
...
async function persistActivatedSetupInference(input) {
	...
	const { stripPendingPluginInstallRecords } = await import("./install-record-commit-CjuCS0pJ.js");
	...
	const transformConfig = deps.transformConfigWithPendingPluginInstalls ?? (await import("./install-record-commit-CjuCS0pJ.js")).transformConfigWithPendingPluginInstalls;
...
async function activateSetupInferenceUnredacted(params, codexCliApiKey) {
	...
	try {
		...
		if (params.kind === "codex-cli") {
			...
			const ensured = await (deps.ensureCodexRuntimePlugin ?? (await import("./codex-runtime-plugin-install-DhqhPqCd.js")).ensureCodexRuntimePluginForModelSelection)({
			...
			const refreshPluginRegistry = deps.refreshPluginRegistryAfterConfigMutation ?? (await import("./registry-refresh-BhS9qvzq.js")).refreshPluginRegistryAfterConfigMutation;
...
async function redactSetupInferenceError(message, ...apiKeys) {
	...
	const { redactToolPayloadText } = await import("./redact-FRP7wna5.js");
