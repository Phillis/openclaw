...
import { h as validateApprovalPresentation } from "./src-BlUKtAtD.js";
...
function terminalApprovalReceiptMetadataRows(params) {
	const boundary = params.after ? executeSqliteQueryTakeFirstSync(params.db, params.stateDb.selectFrom("operator_approvals").select(["approval_id", "resolved_at_ms"]).where(operatorApprovalRowId(), "=", params.after.rowId).where("source_run_id", "=", params.runId).where("resolved_at_ms", "=", params.after.occurredAt)) : void 0;
	if (params.after && !boundary) throw new Error("operator approval decision cursor is no longer retained");
	const ordered = terminalApprovalsForRunQuery(params.stateDb, params.runId, params.nowMs).$if(boundary !== void 0 && boundary.resolved_at_ms !== null, (query) => query.where((eb) => eb.or([eb("operator_approvals.resolved_at_ms", ">", boundary.resolved_at_ms), eb.and([eb("operator_approvals.resolved_at_ms", "=", boundary.resolved_at_ms), eb("operator_approvals.approval_id", ">", boundary.approval_id)])]))).orderBy("operator_approvals.resolved_at_ms", "asc").orderBy("operator_approvals.approval_id", "asc").$if(params.offset !== void 0, (query) => query.offset(params.offset)).limit(params.limit);
	const metadata = (query) => query.select([
	...
	]).select([operatorApprovalRowId().as("receipt_rowid"), operatorApprovalPayloadBytes().as("payload_bytes")]);
	if (!tableExists(params.db, "operator_approval_execution_identities")) return executeSqliteQuerySync(params.db, metadata(ordered).select((eb) => [eb.val(null).as("binding_context_id"), eb.val(null).as("binding_execution_id")])).rows;
	return executeSqliteQuerySync(params.db, metadata(ordered).leftJoin("operator_approval_execution_identities", "operator_approval_execution_identities.approval_id", "operator_approvals.approval_id").select(["operator_approval_execution_identities.source_context_id as binding_context_id", "operator_approval_execution_identities.source_execution_id as binding_execution_id"])).rows;
}
function terminalApprovalReceiptRowsById(params) {
	if (params.ids.length === 0) return /* @__PURE__ */ new Map();
	const query = params.stateDb.selectFrom("operator_approvals").where("operator_approvals.approval_id", "in", [...params.ids]).where(operatorApprovalPayloadBytes(), "<=", OPERATOR_APPROVAL_RECEIPT_MAX_PAYLOAD_BYTES);
	if (!tableExists(params.db, "operator_approval_execution_identities")) {
		const rows = executeSqliteQuerySync(params.db, query.selectAll("operator_approvals").select((eb) => [eb.val(null).as("binding_context_id"), eb.val(null).as("binding_execution_id")])).rows;
		return new Map(rows.map((row) => [row.approval_id, row]));
	}
	const rows = executeSqliteQuerySync(params.db, query.leftJoin("operator_approval_execution_identities", "operator_approval_execution_identities.approval_id", "operator_approvals.approval_id").selectAll("operator_approvals").select(["operator_approval_execution_identities.source_context_id as binding_context_id", "operator_approval_execution_identities.source_execution_id as binding_execution_id"])).rows;
	return new Map(rows.map((row) => [row.approval_id, row]));
}
...
function summarizeOperatorApprovalReceiptsForRun(params) {
	return withExistingOpenClawStateDatabaseReadOnly(({ db }) => {
		...
		const hasOversizedRecord = metadataRows.some((row) => row.payload_bytes > OPERATOR_APPROVAL_RECEIPT_MAX_PAYLOAD_BYTES);
		const boundedMetadataRows = metadataRows.filter((row) => row.payload_bytes <= OPERATOR_APPROVAL_RECEIPT_MAX_PAYLOAD_BYTES);
		...
		const hasUnlinkedRecord = rows.some((row, index) => records[index] !== null && operatorApprovalExecutionLinkState(row, params.context) !== "exact");
		return {
			count,
...
function pageOperatorApprovalReceiptsForRun(params) {
	return withExistingOpenClawStateDatabaseReadOnly(({ db }) => {
		if (!tableExists(db, "operator_approvals")) return { receipts: [] };
		const stateDb = getNodeSqliteKysely(db);
		const metadataRows = terminalApprovalReceiptMetadataRows({
			db,
			stateDb,
		...
		const rowsById = terminalApprovalReceiptRowsById({
			db,
			stateDb,
			ids: pageMetadata.filter((row) => row.payload_bytes <= OPERATOR_APPROVAL_RECEIPT_MAX_PAYLOAD_BYTES).map((row) => row.approval_id)
		});
		const receipts = pageMetadata.map((metadata) => {
			if (metadata.payload_bytes > OPERATOR_APPROVAL_RECEIPT_MAX_PAYLOAD_BYTES) return projectOversizedOperatorApprovalReceipt(metadata, params.context);
			const row = rowsById.get(metadata.approval_id);
			if (!row) return projectCorruptOperatorApprovalReceipt(metadata, params.context);
			const record = decodeOperatorApprovalRow(row);
			if (!record) return projectCorruptOperatorApprovalReceipt(row, params.context);
			const linkState = operatorApprovalExecutionLinkState(row, params.context);
			return linkState === "exact" ? projectOperatorApprovalReceipt(record, params.context) : projectUnlinkedOperatorApprovalReceipt(record, params.context, linkState);
		});
		const last = pageMetadata.at(-1);
		return {
			receipts,
			...metadataRows.length > params.limit && last && last.resolved_at_ms !== null ? { nextCursor: {
