...
import { r as ModelSelectionLockedError } from "./model-overrides-D4SC_nUZ.js";
import { m as resolveStorePath, s as patchSessionEntry } from "./session-store-runtime-De3jWY_Z.js";
...
import { i as bindingStoreKey, l as isCodexAppServerNativeAuthProfile, u as normalizeCodexAppServerBindingModelProvider, z as isCodexFastServiceTier } from "./session-binding-BpH4J33R.js";
import { d as releaseLeasedSharedCodexAppServerClient, s as getLeasedSharedCodexAppServerClient } from "./shared-client-fWU6PNZb.js";
import { f as resolveCodexAppServerRequestModelSelection, m as resolveCodexBindingModelProviderFallback } from "./thread-lifecycle-C6hJgTs0.js";
...
async function setCodexConversationModel(params) {
	...
	const session = params.session ?? (params.identity.kind === "session" && params.identity.sessionKey ? {
	...
	if (session) {
		if (!await patchSessionEntry({
			...
			replaceEntry: true,
			update: (entry) => {
				if (entry.sessionId !== session.sessionId) throw new Error("Codex session changed while applying the model selection.");
		...
		if (params.identity.kind === "conversation") await patchThreadBinding(params.bindingStore, params.identity, binding.threadId, {
			model: nextModel,
		...
		else if (modelChanged && binding.contextEngine?.projection) await patchThreadBinding(params.bindingStore, params.identity, binding.threadId, { contextEngine: {
			...binding.contextEngine,
			projection: void 0
		} });
...
//#endregion
export { parseCodexPermissionsModeArg as a, setCodexConversationModel as c, stopCodexConversationTurn as d, trackCodexConversationActiveTurn as f, parseCodexFastModeArg as i, setCodexConversationPermissions as l, canMutateCodexHost as m, parseCodexPluginMarketplaceId as n, readCodexConversationActiveTurn as o, CODEX_NATIVE_EXECUTION_AUTH_ERROR as p, formatPermissionsMode as r, setCodexConversationFastMode as s, discoverCodexMarketplacePlugins as t, steerCodexConversationTurn as u };
