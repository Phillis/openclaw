...
import { n as CANVAS_HOST_PATH, o as canvasConfigSchema, r as CANVAS_WS_PATH, s as isCanvasHostEnabled, t as A2UI_PATH } from "../../a2ui-shared-DDz7uktk.js";
import { t as CanvasToolSchema } from "../../tool-schema-sn-TWqbG.js";
...
function createLazyCanvasTool(params) {
	const loadTool = createLazyRuntimeModule(() => import("../../tool-Duklf8Eg.js").then(({ createCanvasTool }) => createCanvasTool({
...
var canvas_default = definePluginEntry({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			const httpRouteHandlerLoader = createLazyRuntimeModule(() => import("../../http-route-CdVDor8f.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler({
				config: api.config,
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-DGGxOzAm.js");
