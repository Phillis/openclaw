...
import { h as finiteSecondsToTimerSafeMilliseconds, o as asDateTimestampMs } from "./number-coercion-oCkfUEEq.js";
...
import { h as runOpenClawStateWriteTransaction, kt as OPENCLAW_STATE_SCHEMA_SQL } from "./openclaw-state-db-BciZ4rHE.js";
import { t as KeyedAsyncQueue } from "./keyed-async-queue-CTreGrmR.js";
import { c as removeStaleCronJobFamilyRows, f as saveCronJobsStore, g as cronJobUsesToolRuntime, i as loadCronJobsStoreWithConfigJobs, s as noteCronJobsStoreCommit, t as getCronJobsStoreRevision } from "./store-DPYCi6M7.js";
import { B as cloneCronRuntimeAuthority, J as cronStoreKey, O as isInvalidCronSessionTargetIdError, S as cronStreamScheduleKey, a as loadedCronStoreFromRows, b as appendCronPayloadText, f as cronSchedulingInputsEqual, g as getInvalidPersistedCronJobReason, h as assertCronJobStateTimestamps, i as loadCronRows, m as resolvePacedNextRunAtMs, n as deleteCronJobRowInDatabase, q as normalizeCronJobIdentityFields, u as upsertCronJobRow, v as normalizeCronJobInput, x as createCronStreamSourceIdentity } from "./row-codec-RY4IJt5w.js";
...
import { h as runWithGatewayIndependentRootWorkContinuation, r as beginGatewayRootWorkAdmissionWhenOpen, t as GatewayDrainingError } from "./gateway-work-admission-BNrqZgKC.js";
import { n as deliveryContextFromSession } from "./delivery-context.shared-B3qeEQhR.js";
import { $t as loadSessionEntryReadOnly, Dt as applySessionEntryLifecycleMutation, Pt as listSessionEntriesCore, Zt as loadExactSessionEntryReadOnly } from "./session-accessor-CIiPoGwM.js";
import { x as resolveMaintenanceConfig } from "./agent-harness-session-key-BpWapmwX.js";
import { s as isRetryableHeartbeatSkipReason, t as HEARTBEAT_IDLE_RETRY_GRACE_MS } from "./heartbeat-wake-CAQb-fCA.js";
import { t as buildPendingGeneratedMediaSessionKeySet } from "./task-status-access-DsWf7lJY.js";
import { t as resolveCronAgentSessionKey } from "./session-key-D1ZCHwon.js";
import { c as markCronJobActive, d as noteActiveCronJobTriggerMutation, f as onCronJobInactive, l as noteActiveCronJobRemoval, n as clearCronJobActive, o as isCronActiveJobMarkerCurrent, p as requestActiveCronJobCancellation, s as isCronJobActive, u as noteActiveCronJobScheduleMutation } from "./active-jobs-D5QwO55Q.js";
import { i as registerActiveCronTaskRun, o as trackActiveCronTaskRunSettlement } from "./active-run-cancellation-DsAOvXZX.js";
import { r as enqueueCommandInLane } from "./command-queue-B992TXUy.js";
import { n as AgentDeletionCommitUncertainError, t as AgentDeletionAuthorityRollbackError } from "./agent-lifecycle-registry-Di5OPeL7.js";
import { a as preExecutionTimeoutErrorMessage, c as timeoutErrorMessage, i as normalizeCronRunErrorText, r as isSetupTimeoutErrorText, s as setupTimeoutErrorMessage, t as abortErrorMessage } from "./execution-errors-By7vAoOC.js";
import { t as materializeLegacyDefaultCronJobOwners } from "./legacy-default-agent-owner-migration-DyyWLh4s.js";
import { S as maybeAutoDisableCronJobAfterRunFailure, a as findJobOrThrow, b as resolveJobPayloadTextForMain, c as isJobDue, f as recomputeJobNextRunAtMs, g as recordScheduleComputeError, h as recomputeSingleJobForMaintenance, i as errorBackoffMs, l as isJobEnabled, m as recomputeNextRunsForMaintenance, n as computeJobNextRunAtMs, o as hasActiveCronRun, p as recomputeNextRuns, r as computeJobPreviousRunAtOrBeforeMs, s as hasScheduledNextRunAtMs, t as DEFAULT_ERROR_BACKOFF_SCHEDULE_MS, u as nextWakeAtMs, v as resolveJobErrorBackoffUntilMs, x as autoDisableCronJob, y as resolveJobLastRunStatus } from "./jobs-scheduling-BSkKV8yE.js";
import { c as assertSupportedJobSpec, d as failureNotificationDeliveryFromJobState, f as maybeEmitFailureAlert, g as wake, h as requestCronHeartbeat, i as createJob, l as assertTimeScheduleSatisfiable, m as enqueueCronSystemEvent, n as applyDeclarativeJobSpec, o as normalizeCronTaskRunJobId, p as resolveFailureAlert, r as applyJobPatch, t as resolveCronListSnapshotRevision, u as cronPatchTouchesDeliveryResolution } from "./list-snapshot-revision-CMqts_Py.js";
import { t as computeNextRunAtMs } from "./schedule-CIU3rGks.js";
import { t as resolveCronJobConfigRevision } from "./config-revision-CorYfjz-.js";
import { a as normalizeCronRunDiagnostics, n as createCronRunDiagnosticsFromError, o as summarizeCronRunDiagnostics } from "./run-diagnostics-CmYhGvLn.js";
import { n as resolveCronTriggerMinIntervalMs } from "./cron-limits-O2aLPbc9.js";
import { n as resolveCronDeliveryPlan, r as resolveFailureDestination } from "./delivery-plan-VeAjydWY.js";
import { n as resolveCronJobEffectiveAgentId } from "./agent-id-ewp3PcCW.js";
import { a as tryCreateCronTaskRun, c as tryUpdateCronTaskRunSession, i as resolveMainSessionCronRunSessionKey, l as withCronTaskRunId, o as tryFinishCronTaskRun, s as tryFinishCronTaskRunWithoutHistory, t as findCronTaskRunRecoveryInDatabase, u as resolveCronRunErrorReason } from "./task-runs-r-3XU7Zd.js";
import { a as writeCronJobScratch, r as readCronJobScratchState, t as deleteCronJobScratch } from "./scratch-store-pAN_cnHJ.js";
import { n as isHeartbeatTaskCronJob, r as isHeartbeatTaskDeclarationKey } from "./heartbeat-task-BM5cuDAj.js";
...
function arm(state) {
	...
	if (state.stopped || state.schedulingPaused || current.timer || current.byJobId.size === 0 || !reconcile) return;
...
function stopForeignReceiptMonitor(state, clear) {
	const current = monitor(state);
	if (current.timer) {
	...
	if (!ownsSchedule) {
		...
		if (retryDecision.retryable && retryDecision.backoffMs !== void 0) {
			...
			if (normalNext === void 0) {} else {
				...
				if (retryNextRunAtMs === void 0) return shouldDelete;
		...
		if (backoffNext === void 0) return shouldDelete;
...
async function ensureLoaded(state, opts) {
	...
	for (const [index, raw] of loadedJobs.entries()) {
		...
		const hydratedSchedule = hydratedRaw.schedule ?? {};
		if (!invalidReason && hydratedRaw.enabled !== false && hydratedSchedule.kind === "every") try {
...
async function deliverPrimaryWebhook(state, job, result, abortSignal, progress, deadlineAtMs, assertRunCurrent) {
...
async function executeJobCoreWithTimeout(state, job, opts) {
	...
	try {
		if (typeof jobTimeoutMs !== "number") {
			...
			const runPromise = withCronTaskRunId(opts?.runId, () => executeJobCore(state, job, runAbortController.signal, coreOptions)).then(async (result) => {
				progress.completedCoreResult = result;
				return await deliverPrimaryWebhook(state, job, result, runAbortController.signal, progress, void 0, assertRunCurrent);
			});
		...
		const corePromise = withCronTaskRunId(opts?.runId, () => executeJobCore(state, job, runAbortController.signal, coreOptions));
		watchdog.start();
		const runPromise = corePromise.then(async (result) => {
			progress.completedCoreResult = result;
			return await deliverPrimaryWebhook(state, job, result, runAbortController.signal, progress, watchdog.deadlineAtMs(), assertRunCurrent);
		});
		trackRunSettlement(runPromise);
		runPromise.catch((err) => {
			if (runAbortController.signal.aborted) state.deps.log.warn({
			...
			const committedReservations = commitCronRuntimeRows({
				...
				mutate: ({ database, jobs }) => {
					...
					for (const jobId of jobIds) {
						...
						if (!job || !planned || job.enabled !== planned.enabled || !params.forcedJobIds?.has(jobId) && job.state.nextRunAtMs !== planned.state.nextRunAtMs || job.state.lastRunAtMs !== planned.state.lastRunAtMs || job.state.lastRunStatus !== planned.state.lastRunStatus || job.state.queuedAtMs !== void 0 || job.state.runningAtMs !== void 0 || resolveCronJobConfigRevision(job) !== resolveCronJobConfigRevision(planned)) continue;
						committed.push(job);
					}
...
async function onAdmittedTimer(state) {
	...
	try {
		const dueJobs = await locked(state, async () => {
			...
			const now = state.deps.nowMs();
			return (await persistQueuedCronRunReservations({
				state,
				candidates: due,
				reservedAtMs: now
			})).map(({ job, runReceipt }) => ({
		...
		const releaseUnclaimedDueJobReservationsWithRetry = async () => {
			await cleanupQueuedCronRunReservations({
				state,
				reservations: dueJobs.filter((_, index) => !claimedIndexes.has(index)).map((due) => ({
					jobId: due.id,
				...
				try {
					const execution = await executeQueuedCronRun({
						...
						onCompleted: async (result) => {
							...
							if (hasSetupTimeoutRecoveryHandler && finalizedResults.length > 0 && !setupTimeoutNotified) {
					...
					if (execution.kind === "skipped") return pMapSkip;
...
async function prepareManualRun(state, id, mode, opts) {
	...
	return await locked(state, async () => {
		...
		const [reserved] = await persistQueuedCronRunReservations({
			state,
			candidates: [job],
			...mode === "force" ? { forcedJobIds: /* @__PURE__ */ new Set([job.id]) } : {},
...
async function releasePreparedManualReservation(state, prepared) {
	...
	const committedJob = commitCronRuntimeRows({
		...
		mutate: ({ database, jobs }) => {
			...
			if (!job || ownership?.identity !== prepared.reservationIdentity) return { value: void 0 };
			finishCronRunReceiptInDatabase({
				database,
				handle: ownership.runReceipt,
				status: "skipped",
...
async function add(state, input, opts) {
	...
	return await locked(state, async () => {
		...
		if (isHeartbeatTaskDeclarationKey(declarationKey) && opts?.systemOwned !== true) throw new Error("cron declarationKey namespace \"heartbeat-task:\" is system-owned; jobs cannot be created with it");
		await ensureLoaded(state, { skipRecompute: true });
...
function applyManualRunOutcome(params) {
	...
	const scheduleMode = scheduleOwnership === "stale" ? "stale-preserve" : params.mode === "force" ? "force-preserve" : "advance";
