...
import { i as loadPreparedModelRuntimeAuth, n as resolvePublishedModelCatalogOwner, r as getPreparedModelRuntimeAuthMaterializations } from "./prepared-model-catalog-owner-CUGU07tR.js";
...
async function resolveLoader(params) {
	...
	const { loadPublishedPreparedModelCatalogOwnerSnapshot } = await import("./prepared-model-catalog-BnhcKv7r.js");
	return loadPublishedPreparedModelCatalogOwnerSnapshot;
}
async function resetPreparedModelCatalogStateForTest() {
	const [{ resetPreparedModelRuntimeSnapshotsForTest }, { resetModelCatalogBuilderCacheForTest }] = await Promise.all([import("./prepared-model-runtime.test-support-BYyBntIt.js"), import("./model-catalog-CnYl0yzn.js")]);
...
async function readPreparedGatewayModelCatalog(params) {
	const { getPreparedModelCatalogSnapshot } = await import("./prepared-model-catalog-BnhcKv7r.js");
...
async function readPreparedGatewayModelCatalogOwnerSnapshot(params) {
	const { getPublishedPreparedModelCatalogOwnerSnapshot } = await import("./prepared-model-catalog-BnhcKv7r.js");
	const config = (params?.getConfig ?? getRuntimeConfig)();
