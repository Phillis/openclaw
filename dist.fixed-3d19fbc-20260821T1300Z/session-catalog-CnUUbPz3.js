...
import { a as listAgentIds, f as resolveDefaultAgentDir, p as resolveDefaultAgentId } from "./agent-scope-config-CsnnOL14.js";
...
import { _ as sessionBindingIdentity, m as reclaimCurrentCodexSessionGeneration, w as resolveCodexSupervisionAppServerRuntimeOptions } from "./session-binding-BpH4J33R.js";
import { B as resolveCodexAppServerClientInstanceId, V as CodexAppServerRpcError, d as releaseLeasedSharedCodexAppServerClient, ot as CODEX_INTERACTIVE_CUSTOM_THREAD_SOURCES, pt as resolveCodexAppServerLocalHomeDir, s as getLeasedSharedCodexAppServerClient, st as CODEX_INTERACTIVE_THREAD_SOURCE_KINDS, ut as isJsonObject, y as withTimeout } from "./shared-client-fWU6PNZb.js";
import "./text-utility-runtime-BSdEoze8.js";
import "./text-chunking-BrrQ2GHk.js";
import { n as assertCodexThreadForkParams } from "./protocol-validators-DQMpwHD0.js";
import { r as importCodexThreadHistoryToTranscript } from "./transcript-mirror-Ckkco68B.js";
import { t as CODEX_CONTROL_METHODS } from "./capabilities-CQCaXeG3.js";
import { t as buildCodexAppServerConnectionFingerprint } from "./plugin-app-cache-key-woCyhZVi.js";
import { n as requestCodexAppServerClientJson } from "./request-BQTgc8FH.js";
import { n as decodeNodePtyResumeParams, r as runNodePtyCommand, t as resolveNodeHostExecutable } from "./node-host-CSUwJ8BY.js";
import { s as listSessionCatalogEntries } from "./session-catalog-poyxEoXz.js";
import { t as createCodexCliNodeConversationBindingData } from "./conversation-binding-data-B5PuOlFt.js";
import { a as CODEX_CLI_SESSION_RESUME_COMMAND, t as codexControlRequest } from "./command-rpc-DpupuFui.js";
import { i as createImportedCodexSession, n as codexUpstreamBaseline, r as codexUpstreamContinueResult, t as codexLastTerminalTurnId } from "./session-upstream-marker-Bjs81h_a.js";
...
function resolveCodexCatalogTerminalHome(sources) {
	...
	const startOptions = resolveCodexSupervisionAppServerRuntimeOptions({ pluginConfig: sources.getPluginConfig() }).start;
...
function createCodexTerminalNodeHostCommand(control, configSources) {
	return {
...
async function listCodexSessionCatalog(params) {
	...
	const localHosts = params.includeLocal !== false && (!requestedHostIds || requestedHostIds.has("gateway:local")) ? [listGatewayHost({
		bindingStore: params.bindingStore,
		config: params.config,
...
function createCodexSessionCatalogNodeHostCommands(control, configSources) {
	return [
		{
			command: CODEX_APP_SERVER_THREADS_LIST_COMMAND,
			cap: CODEX_APP_SERVER_THREADS_CAPABILITY,
...
function registerCodexSessionCatalog(params) {
	const usesProcessHomeFallback = () => {
		const start = resolveCodexSupervisionAppServerRuntimeOptions({ pluginConfig: params.getPluginConfig() }).start;
		return start.transport === "stdio" && start.homeScope === "user" && !process.env.CODEX_HOME?.trim();
	};
	const assertLocalAccess = (hostId, allowProcessHomeFallback) => {
		if (hostId === "gateway:local" && allowProcessHomeFallback === false && usesProcessHomeFallback()) throw new CatalogParamsError("local Codex sessions are unavailable in isolated state");
	};
	...
	params.api.registerSessionCatalog({
		...
		list: async (query) => {
			...
			const { allowProcessHomeFallback, listNodes, onHost, sessionEntries, ...gatewayQuery } = query;
		...
		checkUpstreamActivity: (probes, policy) => checkUpstreamActivity(probes.filter((probe) => probe.hostId !== "gateway:local" || policy?.allowProcessHomeFallback !== false || !usesProcessHomeFallback())),
		...
		startTerminalSession: async (request) => {
			assertLocalAccess(request.nodeId ? `node:${request.nodeId}` : CODEX_LOCAL_SESSION_HOST_ID, request.allowProcessHomeFallback);
