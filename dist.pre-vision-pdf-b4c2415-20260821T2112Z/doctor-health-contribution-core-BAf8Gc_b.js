...
import { d as resolveAgentWorkspaceDir, x as tryResolveSoleAgentId } from "./agent-scope-config-BdXMWufB.js";
...
async function runStructuredHealthRepairs(ctx, resolveCoreChecks) {
	...
	const { registerBundledHealthChecks } = await import("./bundled-health-checks-Da1UPAEB.js");
...
async function runCoreContributionHealth(ctx, checkIds) {
	...
	const { CORE_HEALTH_CHECKS } = await import("./doctor-core-checks-CEZMNaYL.js");
	const { runDoctorHealthRepairs } = await import("./doctor-repair-flow-RX6im3Jm.js");
	const { note } = await import("./terminal-core/note.js");
