...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	if (!shouldRepair) {
		const { repairStaleAgentModelRefs } = await import("./stale-agent-model-ref-repair-DcNwmgm7.js");
	...
	if (shouldRepair) {
		const { runDoctorRepairSequence } = await import("./repair-sequencing-Deo8jvGm.js");
		...
		const { collectDoctorPreviewNotes } = await import("./preview-warnings-Dznfw0jU.js");
