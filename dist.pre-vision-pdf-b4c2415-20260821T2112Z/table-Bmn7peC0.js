...
import { l as splitAnsiSegments, n as splitGraphemes, o as truncateToVisibleWidth, s as visibleWidth } from "./ansi-DjDeieuH.js";
...
function indexOfWindowsPath(input, home, cursor) {
	...
	for (let index = cursor; index <= input.length - home.length; index += 1) if (lowercasePreservingWhitespace(input.slice(index, index + home.length)) === foldedHome) return index;
	return -1;
}
