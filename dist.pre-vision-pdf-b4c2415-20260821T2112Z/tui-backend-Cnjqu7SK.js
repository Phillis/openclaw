...
import { i as loadSystemAgentOverview, r as formatSystemAgentStartupMessage } from "./overview-DHobpgIW.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-C3cbtid4.js";
import { o as resolveSystemAgentVerifiedInferenceState } from "./verified-inference-D6Zm0v-p.js";
import { t as executeSystemAgentOperation } from "./operations-C5bGSw_S.js";
...
import { t as SystemAgentChatEngine } from "./chat-engine-DwXqLrYQ.js";
import { randomUUID } from "node:crypto";
//#region src/system-agent/tui-backend.ts
async function loadHostedSetupForTui() {
	const [{ createClackPrompter }, hostedSetup] = await Promise.all([import("./clack-prompter--aajjdty.js"), import("./hosted-setup.runtime.js")]);
	return {
		createClackPrompter,
		hostedSetup
	};
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	await (opts.runChannelsAdd ?? (await import("./add-knMbMmM_.js")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
...
async function runSystemAgentTui(opts, runtime) {
	...
	for (;;) {
		...
		const runTui = boundOpts.runTui ?? (await import("./tui-B5wdY4Dp.js")).runTui;
		try {
			await runTui({
				local: true,
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (verified) {
			...
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-BGLDTo2i.js"), import("./model-thinking-default-B2XhNHdp.js")]);
