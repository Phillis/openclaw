...
import { cr as validateSessionSuggestionsAddParams, dr as validateSessionTypingParams, lr as validateSessionSuggestionsListParams, ur as validateSessionSuggestionsResolveParams } from "./src-BlUKtAtD.js";
import { s as errorShape } from "./error-codes-CMSvT5-d.js";
import { a as listSessionSuggestions, i as finalizeSessionSuggestionClaim, n as addSessionSuggestion, o as releaseSessionSuggestionDispatch, r as claimSessionSuggestionDispatch, t as SESSION_SUGGESTION_DISPATCH_CLAIM_TTL_MS } from "./sessions-D-jhKYGW.js";
import { a as isSessionWorkStartInvalidatedError, s as resolveSessionWorkStartError } from "./lifecycle-BOW0O5mU.js";
...
import { i as updateTypingConnections, r as liveViewerIdentities, t as broadcastTypingThrottled } from "./session-typing-state-CGBbZuG8.js";
import { a as authorizeSessionSharingTarget, h as resolveSessionVisibility, m as resolveSessionSharingTarget, p as resolveSessionSharingRole, r as authorizeIncognitoSessionTarget, s as canManageSessionSharing, v as gatewayClientSessionCreator } from "./session-sharing-DOLHhSnW.js";
...
import { m as sessionObserverScopeKey } from "./session-observer-model-isCfLf_N.js";
...
async function dispatchSuggestion(params) {
	const compatibilityOwnerAgentId = tryResolveSessionCompatibilityOwnerAgentId(params.context.getRuntimeConfig(), params.target.storeKey);
	const activeRunState = params.resolution === "send" ? resolveVisibleActiveSessionRunState({
	...
	if (activeRunState?.active && activeRunState.runIds.length !== 1) {
		const message = activeRunState.runIds.length === 0 ? "active session run has no exact dispatch identity; refresh and retry" : "session has multiple active runs; choose the target run before sending the suggestion";
		return {
			ok: false,
	...
	const activeRunId = activeRunState?.active ? activeRunState.runIds[0] : void 0;
	let response;
	const chatParams = {
		sessionKey: params.target.canonicalKey,
		agentId: params.target.agentId,
		...
		...params.resolution === "queue" ? { queueMode: "followup" } : activeRunId ? {
			queueMode: "steer",
