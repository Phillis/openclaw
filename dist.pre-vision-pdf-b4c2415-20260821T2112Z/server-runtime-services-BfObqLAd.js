...
import { c as startSessionDeliveryRuntime, o as schedulePendingSessionDeliveries } from "./subagent-completion-delivery-BXP_3cYN.js";
import "./embedded-agent-DZF6Lvpn.js";
import { f as resolveHeartbeatAgents } from "./heartbeat-runner-session-DNByrh62.js";
import { t as startHeartbeatRunner } from "./heartbeat-runner-C-E7dvbk.js";
...
function startPendingOutboundDeliveryRecovery(params) {
	...
	const recover = (startup) => {
		...
		const settled = runWithGatewayIndependentRootWorkAdmission(async () => {
			if (stopped) return;
			const { drainPendingDeliveriesCore, recoverPendingDeliveries } = await import("./delivery-queue-recovery-C__y-Dbd.js");
			const { deliverOutboundPayloadsInternal } = await import("./deliver-SR3J2uWA.js");
			if (stopped) return;
...
function startPendingSessionDeliveryRuntime(params) {
	...
	const timer = setTimeout(() => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { deliverQueuedSessionDelivery, recoverPendingRestartContinuationDeliveries, settleQueuedSessionDelivery } = await import("./server-restart-sentinel-BNKYEqAa.js");
			if (stopped) return;
