...
import { c as isAgentEventLifecycleGenerationCurrent, f as registerAgentEventLifecycleRotationHandler, s as getAgentEventLifecycleGeneration } from "./agent-events-Cmj8toCy.js";
import { c as createAgentRunRestartAbortError, f as isAgentRunRestartAbortReason, l as createAgentRunSupersededAbortError, p as isAgentRunSupersededAbortReason } from "./run-termination-B0y7ra5H.js";
import { a as resetReplyRunSettleTimersForTesting, i as formatReplyOperationResult, n as createReplyRunFinalizationLease, r as createReplyRunSettleTimer } from "./reply-run-finalization-lease-CfBDn0LI.js";
import { t as createDeferredCore } from "./deferred-D0La5CRk.js";
import { a as getDiagnosticSessionActivitySnapshot, d as resolveRunStaleThresholdMs, l as markDiagnosticRunProgress } from "./diagnostic-run-activity-Bf46HUQp.js";
import { t as diagnosticLogger } from "./diagnostic-runtime-9hKw1z8y.js";
...
function resolveReplyMessageInjectionRejection(params) {
	...
	if (!expectedRunId && operation.originatingLeafEntryId !== params.originatingLeafEntryId) return { reason: "leaf_mismatch" };
	...
	if (expectedRunId && normalizeOptionalString(backend.runId) !== expectedRunId) return { reason: "run_mismatch" };
	try {
	...
	return mismatch ? { reason: mismatch } : {
		backend,
		injection
	};
}
function isLeafOwnershipRejection(reason) {
	return reason === "no_active_run" || reason === "not_running" || reason === "stale_run" || reason === "leaf_mismatch";
}
function beginReplyMessageInjectionTarget(target, text, options) {
	...
	const resolved = resolveReplyMessageInjectionRejection({
		...
		expectedRunId: target.identity === "run" ? target.runId : void 0,
	...
	if (!("injection" in resolved)) {
		...
		return {
			...
			...target.identity === "leaf" && isLeafOwnershipRejection(resolved.reason) ? { rejectBeforeAck: true } : {},
	...
	const outcome = queued.then((result) => {
...
function createReplyOperation(params) {
	...
	const operation = {
		...
		supersede() {
			if (result || stateCleared) return false;
...
const replyRunRegistry = {
	...
	resolveMessageInjectionTarget({ sessionKey, originatingLeafEntryId, expectedRunId }) {
		...
		if (!("injection" in resolved)) return;
		return {
			...
			originatingLeafEntryId,
			...operation?.toolAuthorityFingerprint ? { toolAuthorityFingerprint: operation.toolAuthorityFingerprint } : {}
		};
	},
	abort(sessionKey) {
		const operation = this.get(sessionKey);
...
//#endregion
export { isReplyRunAbortableForSignal as A, ReplyRunFollowupAdmissionBlockedError as B, createReplyOperation as C, beginReplyMessageInjectionTarget as D, abortReplyMessageInjectionTarget as E, runAfterReplyOperationClear as F, waitForReplyBarrierSettlement as I, REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS as L, isReplyRunSuccessorAdmissionBlocked as M, registerReplyOperationSuccessorBarrier as N, recordAcceptedReplyMessageInjectionTarget as O, retainReplyOperationUntilComplete as P, REPLY_RUN_TERMINAL_SETTLE_TIMEOUT_MS as R, waitForReplyRunSuccessorAdmission as S, forceClearReplyOperation as T, ReplyRunSuccessorAdmissionBlockedError as V, resolveReplyRunPhaseForSessionId as _, forceClearReplyRunBySessionId as a, waitForReplyRunEndBySessionId as b, isReplyRunActiveForSessionId as c, listActiveReplyRunSessionKeys as d, markReplyOperationGlobalLaneWaitProgress as f, resolveActiveReplyRunThreadId as g, resolveActiveReplyRunSessionId as h, expireStaleReplyRunBySessionId as i, isReplyRunEvidenceStale as j, resolveReplyBackendQueueMessageMismatch as k, isReplyRunEvidenceStaleBySessionId as l, resolveActiveReplyOperationForSessionId as m, abortReplyRunBySessionId as n, getActiveReplyRunCount as o, replyRunRegistry as p, clearReplyRunForResetBySessionId as r, isReplyRunAbortableForCompaction as s, abortActiveReplyRuns as t, listActiveReplyRunSessionIds as u, supersedeReplyRunByRunId as v, expireStaleReplyOperation as w, waitForReplyRunFollowupAdmission as x, waitForReplyOperationOwnerSettlement as y, ReplyRunAlreadyActiveError as z };
