...
import { i as isOpenClawOrgNpmSpec, s as parseRegistryNpmSpec } from "./npm-registry-spec-D3pNhy09.js";
...
import { l as clearLoadInstalledPluginIndexInstallRecordsCache } from "./installed-plugin-index-record-reader-PWJi_KhT.js";
...
import { a as resolveBundledPluginSources, n as findBundledPluginSourceInMap } from "./bundled-sources-C9V3iC71.js";
import { t as withPluginLifecycleLease } from "./plugin-lifecycle-lease-wbp6QsX_.js";
import { t as invalidatePluginRuntimeDiscoveryAfterConfigMutation } from "./registry-refresh-clDTOJoC.js";
import { i as installPluginFromNpmPackArchive, r as installPluginFromNpmSpec } from "./install-DeEAX-7s.js";
import { t as resolveBundledInstallPlanForCatalogEntry } from "./plugin-install-plan-BPxWVsBf.js";
...
async function finishOnboardingPluginInstall(params) {
	...
	return await markOnboardingPluginInstalled({
		cfg: params.install ? recordPluginInstall(enableResult.config, params.install) : params.prepareConfig?.(enableResult.config) ?? enableResult.config,
		pluginId: params.pluginId,
...
async function installLocalOnboardingPlugin(params) {
	return await finishOnboardingPluginInstall({
		...
		runtime: params.runtime,
		prepareConfig: (cfg) => pathsReferToSameDirectory(params.localPath, params.bundledLocalPath) ? cfg : recordLocalPluginInstall({
...
async function installPluginFromNpmSpecWithProgress(params) {
	return await runOnboardingPluginInstallWithProgress({
		...params,
		install: (logger, signal) => installPluginFromNpmSpec({
			spec: params.npmSpec,
			mode: "update",
			config: params.cfg,
...
async function installPluginFromNpmPackArchiveWithProgress(params) {
	return await runOnboardingPluginInstallWithProgress({
		...params,
		install: (logger, signal) => installPluginFromNpmPackArchive({
...
async function installPluginFromClawHubSpecWithProgress(params) {
	...
	try {
		const { installPluginFromClawHub } = await import("./clawhub-C28Kqo1w.js");
