...
import { Fi as validateSystemAgentSetupAuthStartParams, Ii as validateSystemAgentSetupDetectParams, Li as validateSystemAgentSetupVerifyParams, Mi as validateSystemAgentChatHistoryParams, Ni as validateSystemAgentChatParams, Pi as validateSystemAgentSetupActivateParams, Uv as buildSystemAgentInferenceUnavailableErrorDetails, Wv as buildSystemAgentSessionInvalidatedErrorDetails } from "./src-BlUKtAtD.js";
...
import { n as describeSystemAgentPersistentOperation } from "./operations-C5bGSw_S.js";
...
import { t as getUpdateAvailable } from "./update-startup-BDFGg4LQ.js";
import { n as getHealthCache } from "./health-state-Bkwq9Fq3.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-DwXqLrYQ.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = await createAdmittedWizardSession(() => new WizardSession(async (prompter, signal, runnerSession) => {
			await runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-B30ja5_a.js"), import("./setup.shared-BaXgZkEJ.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-CWBxHvDP.js");
