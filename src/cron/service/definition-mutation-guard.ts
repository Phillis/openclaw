...
function defaultEvidenceStatePath(fileName: string): string {
  return path.join(resolveStateDir(process.env), "state", fileName);
}
...
function inspectCronDefinitionMutationGuard({
  ...
  rolloutLockPath = testPathOverrides?.rolloutLockPath ??
    defaultEvidenceStatePath("model-router-rollout.lock"),
  nowMs = Date.now(),
}: {
  guardPath?: string;
