  ...
  agents: {
    defaults: {
      compaction: {
        ...
        contextUsageThreshold: 0.8, // compact once context usage reaches 80% of the window (0 < x < 1)
...
- `contextUsageThreshold`: optional fraction of the context window (`0 < x < 1`, for example `0.8` = 80%) at which automatic compaction triggers proactively once assessed context usage reaches that share of the active model window. Compaction is LLM-summarized. Applied consistently to embedded auto-compaction, auto-reply preflight compaction (channel sessions like Slack DMs), and the pre-compaction memory flush boundary. Leave unset for the default window-minus-reserve behavior.
