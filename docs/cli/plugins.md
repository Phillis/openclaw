...
<AccordionGroup>
  ...
  <Accordion title="Guarded archive replacement">
    `plugins replace-guarded` is a recovery-oriented command for an already-installed local archive plugin. It requires exact candidate archive, installed predecessor payload, and rollback archive SHA-256 identities plus a new receipt path whose parent directory already exists. Before any filesystem or database mutation, it creates and synchronizes a create-only receipt bound to the actual lifecycle lease owner. Receipt directory synchronization fails closed on supported filesystems; Windows receipts explicitly record the platform's weaker directory-durability status. The candidate is retained in checksum-addressed, read-only custody and canonical install metadata records that durable path plus SHA-256 SRI instead of the caller's mutable archive path.

    The command stores a monotonic trust anchor in the shared state database, then stages both archives and runs the canonical update policy with checksum-bound predecessor, candidate, rollback, target, and config facts. Its versioned payload identity includes supported file and directory modes as well as bytes and safe links. The rollback payload must match the installed predecessor. Captured predecessor and validated rollback fallback remain until the target and installed index are consistent, and the installed-index transition uses per-record compare-and-swap while preserving unrelated plugin records.

    If the process stops after publication, run `openclaw plugins replace-guarded reconcile --receipt <path>`. Reconciliation accepts only the current anchored receipt projection or its immediately previous revision, heals a safely lagging projection, then completes a committed candidate, restores the verified predecessor, or reports that operator authorization is required when neither state can be proven safe. Re-running reconciliation verifies the anchored state and otherwise makes no change.
