...
async function resolveExecBaseConfig(opts) {
	...
	const { createConfigIO, getRuntimeConfig } = await import("./io-CdPYG-9e.js");
...
async function agentExecCommand(positionalMessage, opts, runtime, deps = {}) {
	...
	try {
		...
		configIo = await import("./io-CdPYG-9e.js");
		...
		const pluginInstallContext = opts.isolated !== true && opts.authEnvOnly !== true ? await import("./install-root-context-Dr5nOBza.js") : void 0;
		...
		const { resolveDefaultAgentDir } = await import("./agent-scope-config-DT3SVabF.js");
		...
		if (isExecutionIdentityCollectionEnabled(runConfig)) try {
			stopLocalAuditWriter = (await import("./agent-local-audit-B9V68QLc.js")).startAgentLocalAuditWriter({ stateDir });
		} catch {}
		const [{ withAuthProfileStoreAgentDir, withEnvOnlyAuthProfileStore }, { withHostExecInheritedEnvOmitted }, { listKnownProviderAuthEnvVarNames }, runAgent] = await Promise.all([
			import("./auth-profiles-DkT9pQEE.js"),
			import("./host-env-security-B23eCvGK.js"),
			import("./provider-env-vars-C8lauJwP.js"),
			deps.runAgent ? Promise.resolve(deps.runAgent) : import("./agent-BSVY86V1.js").then((module) => module.agentCommand)
		]);
