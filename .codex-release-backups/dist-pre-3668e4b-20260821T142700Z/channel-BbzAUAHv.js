...
import { C as parseStrictNonNegativeInteger, h as finiteSecondsToTimerSafeMilliseconds } from "./number-coercion-oCkfUEEq.js";
...
import { At as boolean, Et as array, Rn as string, Tn as object, Zn as unknown, wn as number } from "./schemas-C7gqXY2T.js";
...
import { n as createHybridChannelConfigAdapter, s as createScopedDmSecurityResolver } from "./channel-config-helpers-C6dKYMZI.js";
...
import { n as createChannelIngressError } from "./ingress-monitor-D70rNE7-.js";
import "./channel-secret-basic-runtime-BUtqhYr9.js";
import { i as channelStoppedPatch, n as channelBlockedPatch, r as channelReadyPatch } from "./gateway-runtime-Bdl1Q2-8.js";
...
import { s as resolveStableChannelMessageIngress, t as createStandardRawEventIngressMonitor } from "./channel-ingress-runtime-CH0-nCCh.js";
import { D as composeWarningCollectors, I as projectAccountConfigWarningCollector, L as projectAccountWarningCollector, M as createConditionalWarningCollector } from "./channel-policy-Dn6aXK3G.js";
import { r as createEmptyChannelDirectoryAdapter } from "./directory-runtime-CBy-zfTh.js";
import { a as createFixedWindowRateLimiter } from "./webhook-ingress-Bfu_BdL5.js";
import { a as createWebhookInFlightLimiter, i as beginWebhookRequestPipelineOrReject } from "./webhook-request-guards-DNMZaVoi.js";
import { t as registerPluginHttpRoute } from "./http-registry-DBG4bPsQ.js";
import { a as resolveAccount, i as listAccountIds, n as synologyChatSetupContract, r as synologyChatSetupWizard } from "./setup-surface-7MtNlyql.js";
...
import { r as getSynologyRuntime, t as prepareSynologyHostedMedia } from "./outbound-media-BfnYHKjY.js";
...
async function sendMessageChunk(incomingUrl, text, userId, allowInsecureSsl = false) {
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			...
			if (await doPost(incomingUrl, body, allowInsecureSsl) === "accepted") return true;
			return false;
		} catch (error) {
			if (!isProvenPreConnectFailure(error)) return false;
		}
...
*/
async function sendHostedFileUrl(incomingUrl, fileUrl, userId, allowInsecureSsl = false) {
	let body;
	try {
...
function collectSynologyGatewayRoutingWarnings(params) {
	return collectSynologyGatewayStartupIssues({
		...
		account: params.account,
		accountId: params.account.accountId
	}).filter((issue) => issue.code === "inherited-shared-webhook-path" || issue.code === "duplicate-webhook-path" || issue.code === "duplicate-webhook-url").map((issue) => `- Synology Chat: ${issue.message}`);
}
	...
	try {
		unregister = registerPluginHttpRoute({
			...
			handler: async (req, res) => {
				const { tryHandleSynologyHostedMediaRequest } = await import("./outbound-media-BTn5opAa.js");
...
async function sendSynologyChatMedia(ctx) {
	...
	const sendResult = await sendHostedFileUrl(incomingUrl, prepared.url, ctx.to, account.allowInsecureSsl);
