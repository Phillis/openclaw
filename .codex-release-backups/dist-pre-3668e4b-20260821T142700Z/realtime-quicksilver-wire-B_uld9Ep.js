...
import { Et as array, Rn as string, Tn as object, dn as literal, wn as number, yt as _enum } from "./schemas-C7gqXY2T.js";
import { h as readResponseTextLimited, m as readProviderTextResponse } from "./provider-http-errors-DwYSuIHs.js";
...
async function createOpenAIQuicksilverCall(params) {
	...
	const multipart = isGptLive || params.gaSideband ? buildOpenAIQuicksilverMultipartBody({
		sdp: params.sdp,
		session: params.session
	}) : void 0;
	const callUrl = isGptLive ? OPENAI_QUICKSILVER_CALL_URL : params.gaSideband ? OPENAI_REALTIME_CALL_URL : `${OPENAI_REALTIME_CALL_URL}?model=${encodeURIComponent(params.session.model)}`;
	...
	if (!response.ok) {
		const detail = redactSensitiveText((await readResponseTextLimited(response, OPENAI_REALTIME_ERROR_BODY_MAX_BYTES).catch(() => "")).trim().slice(0, OPENAI_REALTIME_ERROR_DETAIL_MAX_CHARS), { mode: "tools" });
