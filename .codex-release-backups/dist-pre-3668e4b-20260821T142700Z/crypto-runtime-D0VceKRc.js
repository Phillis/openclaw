...
import { t as ensureMatrixCryptoRuntime } from "./deps-CzXytiGS.js";
import { n as LogService, r as noop } from "./logger-Dn0f-vbT.js";
import { _ as FDBKeyRange, a as fakeIndexedDB, c as FDBOpenDBRequest, d as FDBObjectStore, f as FDBRecord, g as FDBCursor, h as FDBCursorWithValue, i as restoreIdbFromDisk, l as FDBDatabase, m as FDBRequest, n as persistIdbToDisk, o as FDBFactory, p as FDBIndex, s as FDBVersionChangeEvent, u as FDBTransaction } from "./idb-persistence-DCqydG5o.js";
import { n as isRepairableSecretStorageAccessError } from "./recovery-key-store-Dq5VqOlT.js";
...
//#region ../../../../../../openclaw/node_modules/fake-indexeddb/auto/index.mjs
...
var MatrixCryptoBootstrapper = class {
	...
	async bootstrapCrossSigning(crypto, options) {
			...
			try {
				...
				if (options.allowSecretStorageRecreateWithoutRecoveryKey && isRepairableSecretStorageAccessError(err)) {
					...
					try {
						...
						if (options.strict) throw toStringifiedError(repairErr);
						return {
							ready: false,
							published: false
						};
		...
		try {
			...
			if (options.allowSecretStorageRecreateWithoutRecoveryKey && isRepairableSecretStorageAccessError(err)) {
				...
				try {
					...
					if (options.strict) throw toStringifiedError(resetErr);
					return {
						ready: false,
						published: false
					};
		...
		try {
			...
			if (options.strict) throw toStringifiedError(err);
			return {
				ready: false,
				published: false
			};
...
const MATRIX_DECRYPT_DRAIN_TIMEOUT_MS = 5e3;
...
var MatrixDecryptBridge = class {
	constructor(deps) {
		...
		this.pendingSdkDecryptions = /* @__PURE__ */ new Set();
	...
	attachEncryptedEvent(event, roomId) {
		if (this.quiescing || this.stopped) return;
		...
		if (sdkDecryption) {
			...
			sdkDecryption.then(forgetSdkDecryption, forgetSdkDecryption);
		}
	...
	async drainPendingDecryptions(_reason) {
		this.quiescing = true;
		...
		for (const [retryKey, state] of pending) {
			...
			if (!state.inFlight) this.runDecryptRetry(retryKey).catch(noop);
		}
		let timeout;
		try {
			await Promise.race([Promise.all([Promise.allSettled(pendingSdkDecryptions), this.waitForActiveRetryRunsToFinish()]), new Promise((_, reject) => {
				timeout = setTimeout(() => {
					reject(/* @__PURE__ */ new Error(`Matrix decryption drain did not finish within ${MATRIX_DECRYPT_DRAIN_TIMEOUT_MS}ms`));
