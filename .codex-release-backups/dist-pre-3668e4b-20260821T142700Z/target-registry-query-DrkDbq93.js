...
import { i as parseDotPath } from "./shared-D1NvanUW.js";
import { n as loadChannelSecretContractApiForRecord, r as listOfficialExternalChannelSecretTargetRegistryEntries, t as loadChannelSecretContractApi } from "./channel-contract-api-BVIuNt9D.js";
import { n as getPath } from "./path-utils-ED_buFn9.js";
...
function getConfiguredSecretTargetRegistryState(config, env) {
	return compileSecretTargetRegistryState(getSecretTargetRegistry({
		config,
		env
	}));
}
...
function discoverConfigSecretTargetsByIds(config, targetIds, options = {}) {
	...
	const configuredChannelEntries = !hasOnlyCoreTargetIds && !configHasPluginEntries(config) ? getConfiguredChannelOpenClawTargets(config, env) : null;
	...
	const registryState = configuredEntries !== null && (allowedTargetIds === null || Array.from(allowedTargetIds).every((targetId) => configuredEntriesById?.has(targetId))) ? null : getConfiguredSecretTargetRegistryState(config, env);
