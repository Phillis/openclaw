...
import { a as inspectPluginInstallRecordMap, l as setPluginInstallRecordMapEntry, n as copyPluginInstallRecordMap } from "./plugin-install-record-map-CWFLMnp7.js";
import { i as loadInstalledPluginIndex } from "./installed-plugin-index-uuE4SyLf.js";
import { a as inspectPersistedInstalledPluginIndexInstallRecordsSync, n as loadInstalledPluginIndexInstallRecords, s as resolveInstalledPluginIndexStorePath } from "./installed-plugin-index-record-reader-CDDyVBh4.js";
import { i as readPersistedInstalledPluginIndexSync, l as writePersistedInstalledPluginIndex, t as inspectPersistedInstalledPluginIndex } from "./installed-plugin-index-store-DCxz0axS.js";
import { t as loadPluginManifestRegistryForInstalledIndex } from "./manifest-registry-installed-DozKX1OI.js";
...
function invalidPersistedInstallRecordMessage(filePath) {
	return [`Persisted plugin install records are invalid at ${filePath}.`, "Stop the Gateway, back up this database, delete only the installed_plugin_index row with index_key='installed-plugin-index' using SQLite tooling, then rerun `openclaw doctor --fix` to rebuild it."].join(" ");
}
...
function preflightPluginRegistryInstallMigration(params = {}) {
	...
	if (inspectPersistedInstalledPluginIndexInstallRecordsSync(params).status === "invalid") throw new InvalidPluginInstallRecordStateError(invalidPersistedInstallRecordMessage(filePath));
	if (params.config && inspectShippedPluginInstallConfigRecords(params.config).status === "invalid") throw new InvalidPluginInstallRecordStateError(INVALID_CONFIG_INSTALL_RECORD_MESSAGE);
