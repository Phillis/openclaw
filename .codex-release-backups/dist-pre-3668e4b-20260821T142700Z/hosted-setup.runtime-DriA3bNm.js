...
//#region src/system-agent/hosted-setup.runtime.ts
let setupSharedPromise;
function loadSetupShared() {
	return setupSharedPromise ??= import("./setup.shared-fjK1_Wf3.js");
}
const GATEWAY_WRITE_POLICY = {
	mode: "none",
	reason: "Gateway setup defers runtime apply until explicit restart"
};
function requireLocalGateway(config) {
...
async function runHostedSetup(params) {
...
async function runHostedChannelSetup(channel, prompter, beforePersistentApply, runtime) {
	const { createChannelOnboardingPostWriteHookCollector, runCollectedChannelOnboardingPostWriteHooks, setupChannels } = await import("./onboard-channels-BVl-mh0D.js");
...
async function runHostedSkillsSetup(prompter, beforePersistentApply, runtime) {
	const [{ setupSkills }, { resolveSystemAgentOnboardingTarget }] = await Promise.all([import("./onboard-skills-DPtGLfPa.js"), import("./onboard-agent-target-DR1ExYhn.js")]);
	return await runHostedSetup({
		label: "Skills setup",
		runtime,
		beforePersistentApply,
		run: async ({ baseConfig, runtime: setupRuntime }) => ({ nextConfig: await setupSkills(baseConfig, resolveSystemAgentOnboardingTarget(baseConfig).workspaceDir, setupRuntime, prompter, { beforePersistentEffect: async () => await beforePersistentApply(setupRuntime) }) })
	});
}
async function runHostedSearchSetup(prompter, beforePersistentApply, runtime) {
...
async function runHostedGatewaySetup(prompter, beforePersistentApply, runtime) {
	const [{ resolveGatewayPort }, { configureGatewayForSetup }, { resolveQuickstartGatewayDefaults }] = await Promise.all([
		import("./config/config.js"),
		import("./setup.gateway-config-B8CoH10l.js"),
		loadSetupShared()
	]);
	return await runHostedSetup({
		label: "Gateway setup",
...
async function runHostedMemoryImport(prompter, beforePersistentApply, onProviderOutcome) {
	const [{ readSetupConfigFileSnapshot }, { resolveSystemAgentOnboardingTarget }] = await Promise.all([loadSetupShared(), import("./onboard-agent-target-DR1ExYhn.js")]);
	const snapshot = await readSetupConfigFileSnapshot();
...
async function auditMemoryImport(providers, appendAuditEntry = appendSystemAgentAuditEntry) {
	...
	await appendAuditEntry({
		...
		summary: indeterminateProviders.length > 0 ? `Memory import failed partway via chat: ${[providerSummary ? `confirmed ${providerSummary}` : "", indeterminateSummary].filter(Boolean).join("; ")}` : `Imported memory via chat: ${providerSummary}`,
		details: {
...
async function renderMemoryImport(outcome, appendAuditEntry = appendSystemAgentAuditEntry) {
...
//#endregion
export { GATEWAY_WRITE_POLICY, auditMemoryImport, renderMemoryImport, requireLocalGateway, runHostedChannelSetup, runHostedGatewaySetup, runHostedMemoryImport, runHostedSearchSetup, runHostedSetup, runHostedSkillsSetup };
