...
import { a as requireRiskAcknowledgement } from "./setup.shared-ntWcJh9N.js";
...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk, agentName) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted, setupAgentName) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-0nyT5D9e.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	const prompter = await (deps.createPrompter?.() ?? import("./clack-prompter-bWpU8ovy.js").then(({ createClackPrompter }) => createClackPrompter()));
	...
	const localOnboarding = custodianMode ? await import("./local-onboarding-state-MF0R7TfF.js") : void 0;
	...
	if (!custodianMode) {
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-B-T_rUtx.js")).runSetupMemoryImportStep)({
	...
	const { resolveSetupWorkspaceSelection } = await import("./setup.workspace-Cp_w-PKB.js");
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-Cm_ZGPqN.js")).applySystemAgentSetup;
			...
			const appliedSnapshot = localSetup?.status === "pending" ? await (await import("./setup-recovery-C6oOjT_5.js")).completeLocalSetupRecovery({
				owner: localSetup,
	...
	if (wantsDiscovery) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-B-T_rUtx.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-DM-RhRZb.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
	...
	if (opts.tui !== true) {
		if ((await (deps.runBrowserHandoff ?? (await import("./onboard-browser-handoff-CCOuQEkN.js")).runBrowserHatchHandoff)({
			config: persistedConfig,
			prompter,
...
async function launchHatchTui(workspace) {
	const [{ launchTuiCli }, { DEFAULT_BOOTSTRAP_FILENAME }, { restoreTerminalState }, fs, path] = await Promise.all([
		import("./tui-launch-CB1sevK3.js"),
		import("./workspace-CauMOCnK.js"),
