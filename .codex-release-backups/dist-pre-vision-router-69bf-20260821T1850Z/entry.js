...
import { n as runCliWithExitFinalization, t as requestExitAfterOneShotOutput } from "./one-shot-exit-exEwhVsw.js";
import { r as getCommandPositionalsWithRootOptions } from "./cli-root-options-CpQG4BXe.js";
import { u as isRootHelpInvocation } from "./argv-ubyZhwcH.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-DZhkFMuY.js";
import { n as parseCliContainerArgs, r as resolveCliContainerTarget } from "./container-target-BxmLKL0n.js";
import { t as tryHandleRootVersionFastPath } from "./entry.version-fast-path-DZPmyZjx.js";
...
async function writeCapturedCliArgumentError(message) {
	...
	const { enableConsoleCapture } = await import("./logging-BZAcYnCh.js");
...
async function writeCliDiagnosticBlock(message) {
	...
	loadCliDotEnv({ quiet: true });
	await configureGatewayStartupTraceConsoleFormatting(gatewayEntryStartupTrace);
	const { formatConsoleDiagnosticBlock } = await import("./json-console-line-BVGe0_he.js");
...
async function flushEntryStartupTraceForEarlyReturn(argv) {
	...
	const { loadCliDotEnvForEarlyDiagnostic } = await import("./dotenv-CbzVTcDw.js");
...
if (!isMainModule({
	...
	const { assertSupportedRuntime, isCurrentRuntimeSupported } = await import("./runtime-guard-BpZ8dL9f.js");
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-vfpcJXvO.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
	...
	await runCliWithExitFinalization({
		run: async () => {
			if (isNativeHookRelayArgv(argv) && !argv.includes("--help") && !argv.includes("-h")) {
				const { runNativeHookRelayCliFromArgv } = await import("./native-hook-relay-cli-DHaWmnbB.js");
		...
		onError: async (error) => {
			...
			const { enableConsoleCapture } = await import("./logging-BZAcYnCh.js");
			enableConsoleCapture();
			const { formatCliFailureLines } = await import("./failure-output-BdoII5gQ.js");
