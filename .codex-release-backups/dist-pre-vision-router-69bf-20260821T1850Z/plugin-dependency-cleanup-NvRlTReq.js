...
import { r as removeStalePluginRuntimeSymlinks } from "./plugin-runtime-symlinks-XrjqgLJh.js";
...
async function resolveSafeRemovalTarget(target, cleanupRoots) {
	...
	if (!cleanupRoots.some((root) => isPathInside(root.realPath, realPath))) return { warning: `Skipped legacy plugin dependency state ${targetPath}: resolved outside OpenClaw cleanup roots` };
	return { target: targetPath };
}
