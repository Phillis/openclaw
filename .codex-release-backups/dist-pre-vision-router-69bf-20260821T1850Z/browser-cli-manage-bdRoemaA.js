...
import { n as info, t as danger } from "./globals-DD_xHyf6.js";
import { n as redactCdpUrl } from "./browser-config-DZOFWPb4.js";
import { n as formatBrowserGraphicsSummary } from "./routes-ChelKskL.js";
import "./core-api-meznye3P.js";
import { c as printBrowserJsonResult, l as resolveBrowserProfileQuery, n as callBrowserRequest, s as parseBrowserPositiveIntegerValue, t as BROWSER_TAB_REFERENCE_HELP, u as runBrowserCliCommand } from "./browser-cli-shared-B1tWUNz1.js";
...
function registerBrowserManageCommands(browser, parentOpts) {
	...
	browser.command("open").description("Open a URL in a new tab").argument("<url>", "URL to open").option("--label <label>", "Assign a friendly tab label").action(async (url, opts, cmd) => {
		...
		await runBrowserCliCommand(async () => {
			...
			if (printBrowserJsonResult(parent, tabLocal)) return;
	...
	browser.command("profiles").description("List all browser profiles").action(async (_opts, cmd) => {
		...
		await runBrowserCliCommand(async () => {
			...
			if (printBrowserJsonResult(parent, { profiles })) return;
	...
	browser.command("system-profiles").description("List Chrome-family profiles available for cookie import").option("--browser <browser>", "System browser (chrome|brave|edge|chromium); omit to list all").action(async (opts, cmd) => {
		...
		await runBrowserCliCommand(async () => {
			...
			if (printBrowserJsonResult(parent, { systemProfiles })) return;
