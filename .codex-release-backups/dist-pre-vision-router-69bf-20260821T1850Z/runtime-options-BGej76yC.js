...
import { r as resolveAcpSessionIdentifierLinesFromIdentity } from "./session-identifiers-B5CDFQVW.js";
import { t as getAcpSessionManager } from "./manager-ROoWPv7n.js";
import { _ as validateRuntimeModelInput, g as validateRuntimeModeInput, h as validateRuntimeCwdInput, m as validateRuntimeConfigOptionInput, u as parseRuntimeTimeoutSecondsInput, y as validateRuntimePermissionProfileInput } from "./manager.turn-timeout-C3qUh0vl.js";
import { o as sanitizeTaskStatusText } from "./task-status-DMWIN7O1.js";
import { r as findLatestTaskForRelatedSessionKeyForOwner } from "./task-owner-access-CApi94oH.js";
import { n as commandReply } from "./command-gates-Cn1fPIAB.js";
import { C as withAcpCommandErrorBoundary, _ as parseSingleValueCommandInput, a as ACP_PERMISSIONS_USAGE, c as ACP_SET_MODE_USAGE, g as parseSetCommandInput, h as parseOptionalSingleTarget, i as ACP_MODEL_USAGE, l as ACP_STATUS_USAGE, m as formatRuntimeOptionsText, o as ACP_RESET_OPTIONS_USAGE, p as formatAcpCapabilitiesText, t as ACP_CWD_USAGE, u as ACP_TIMEOUT_USAGE } from "./shared-C4koublO.js";
	...
	return await withSingleTargetValue({
		...
		run: async ({ targetSessionKey, value }) => await withAcpCommandErrorBoundary({
			...
			onSuccess: ({ parsedValue, options }) => {
				...
				return commandReply(`✅ Updated ACP ${action.optionLabel} for ${targetSessionKey}: ${valueText}. Effective options: ${formatRuntimeOptionsText(options)}`);
			}
		})
	});
}
...
async function handleAcpSetModeAction(params, restTokens) {
	return await withSingleTargetValue({
		...
		run: async ({ targetSessionKey, value }) => await withAcpCommandErrorBoundary({
			...
			onSuccess: ({ runtimeMode, options }) => commandReply(`✅ Updated ACP runtime mode for ${targetSessionKey}: ${runtimeMode}. Effective options: ${formatRuntimeOptionsText(options)}`)
		})
	});
}
