...
import { n as describeSystemAgentPersistentOperation } from "./operations-XBlhHXtv.js";
...
import { t as getUpdateAvailable } from "./update-startup-BGPI5nIc.js";
import { n as getHealthCache } from "./health-state-CfGvdRC-.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-D-NZycL-.js";
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-4QE6cvgc.js");
