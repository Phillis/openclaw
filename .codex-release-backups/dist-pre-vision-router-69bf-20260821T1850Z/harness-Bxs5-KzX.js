...
function createCodexAppServerAgentHarness(options) {
	...
	return {
		...
		...sessionCatalogControl && sessionRuntime ? { sessionFork: {
			...
			fork: async (params) => {
				const { forkCodexUpstreamSession } = await import("./upstream-session-fork-DyuEAzCd.js");
		...
		authBinding: { fingerprint: async (params) => {
			const { fingerprintCodexAppServerAuthBinding } = await import("./auth-binding-OZdAbIZE.js");
		...
		fetchUsageSnapshot: async (ctx) => {
			const { fetchCodexAppServerUsageSnapshot } = await import("./usage-DnOVWJqQ.js");
		...
		loadMcpToolCatalog: async (params) => {
			const { loadCodexEffectiveMcpCatalog } = await import("./effective-mcp-catalog-r_lzV-ps.js");
		...
		runIsolatedCompletionV2: async (params) => {
			...
			const { runCodexIsolatedCompletion } = await import("./isolated-completion-BvdAVNnU.js");
		...
		finalizeSettledTurn: async (params) => {
			const { runCodexSettledTurnFinalization } = await import("./settled-turn-finalizer-BRLGbmNK.js");
			return runCodexSettledTurnFinalization(params, { pluginConfig: options?.resolvePluginConfig?.() ?? options?.pluginConfig });
		},
		runSideQuestion: async (params) => {
			const { runCodexAppServerSideQuestion } = await import("./side-question-DwB_Md-Q.js");
		...
		compact: async (params) => {
			const { maybeCompactCodexAppServerSession } = await import("./compact-Bgr6PH26.js");
		...
		reset: async (params) => {
			if (params.sessionId) {
				const [{ reclaimCurrentCodexSessionGeneration, sessionBindingIdentity }, { retireCodexAppServerSessionGeneration }] = await Promise.all([import("./session-binding-U_BTXSBG.js"), import("./session-retirement-CdX_6dqn.js")]);
