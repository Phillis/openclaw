...
import { C as resolveOAuthDir, b as resolveLegacyStateDirs, w as resolveStateDir, y as resolveIsNixMode } from "./paths-CqeDjSA4.js";
...
import { c as resolveExecApprovalsPath } from "./exec-approvals-config-moZwurok.js";
import { i as withSuppressedNotes } from "./note-C_xoKlB9.js";
...
async function ensureConfigReady(params, recoveryDeps) {
	...
	const shouldConsiderStateMigration = commandName !== "config" && commandName !== "health" && commandName !== "logs" && commandName !== "sessions" && !(commandName === "gateway" && subcommandName === "call") && !(commandName === "update" && subcommandName === "status");
	...
	const runStateMigrationPreflight = async () => {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-BD__blgj.js")).runDoctorConfigPreflight({
			migrateState: true,
	...
	const { formatConfigIssueLines } = await import("./issue-format-XF6PZp4t.js");
	...
	if (isPluginPackagingFailure || isNixManagedConfig || !shouldOfferRecovery) {
		const fixHint = isPluginPackagingFailure ? formatPluginPackagingRuntimeOutputRecoveryHint() : isNixManagedConfig ? new (await (import("./nix-mode-write-guard-DX3JKBBO.js"))).NixModeConfigMutationError({ configPath: snapshot.path }).message : commandText(formatCliCommand("openclaw doctor --fix"));
	...
	if (shouldOfferRecovery && !isPluginPackagingFailure) {
		...
		if ((await offerInvalidConfigRecovery({
			...
			retry: async () => {
				configSnapshotPromise = null;
				const { runDoctorConfigPreflight } = await import("./doctor-config-preflight-BD__blgj.js");
