...
import { E as isMissingPathError, c as redactSensitiveText } from "./redact-DP7p9QfH.js";
...
import { n as getDiagnosticStabilitySnapshot, t as MAX_DIAGNOSTIC_STABILITY_LIMIT } from "./diagnostic-stability-BfRs7P_B.js";
...
function readMemoryPressureEvidence(value) {
	...
	return {
		...
		...pressure.thresholdBytes !== void 0 ? { thresholdBytes: readRequiredNumber(pressure.thresholdBytes, "evidence.memoryPressure.thresholdBytes") } : {},
		...pressure.rssGrowthBytes !== void 0 ? { rssGrowthBytes: readRequiredNumber(pressure.rssGrowthBytes, "evidence.memoryPressure.rssGrowthBytes") } : {},
		...pressure.windowMs !== void 0 ? { windowMs: readRequiredNumber(pressure.windowMs, "evidence.memoryPressure.windowMs") } : {},
...
function readStabilityEventRecord(value, label) {
	...
	if (record.usage !== void 0) {
		...
		sanitized.usage = {
			...usage.input !== void 0 ? { input: readRequiredNumber(usage.input, `${label}.usage.input`) } : {},
			...usage.output !== void 0 ? { output: readRequiredNumber(usage.output, `${label}.usage.output`) } : {},
			...usage.cacheRead !== void 0 ? { cacheRead: readRequiredNumber(usage.cacheRead, `${label}.usage.cacheRead`) } : {},
			...usage.cacheWrite !== void 0 ? { cacheWrite: readRequiredNumber(usage.cacheWrite, `${label}.usage.cacheWrite`) } : {},
			...usage.promptTokens !== void 0 ? { promptTokens: readRequiredNumber(usage.promptTokens, `${label}.usage.promptTokens`) } : {},
			...usage.total !== void 0 ? { total: readRequiredNumber(usage.total, `${label}.usage.total`) } : {}
		};
	}
	if (record.context !== void 0) {
		const context = readObject(record.context, `${label}.context`);
		sanitized.context = {
			...context.limit !== void 0 ? { limit: readRequiredNumber(context.limit, `${label}.context.limit`) } : {},
			...context.used !== void 0 ? { used: readRequiredNumber(context.used, `${label}.context.used`) } : {}
		};
	}
	return sanitized;
}
