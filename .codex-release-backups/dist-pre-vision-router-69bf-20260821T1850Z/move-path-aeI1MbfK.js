...
import { i as guardedRename } from "./pinned-write-CKWgTqHs.js";
import { t as registerTempPathForExit } from "./temp-cleanup-Cr3s_L0p.js";
import { randomUUID } from "node:crypto";
import { constants } from "node:fs";
import path from "node:path";
import fs$1 from "node:fs/promises";
//#region ../../../../../../openclaw/node_modules/@openclaw/fs-safe/dist/move-path.js
...
async function movePathWithCopyFallback(options) {
	...
	try {
		...
		unregisterStaged.setIdentity(await fs$1.lstat(staged));
		await assertCopyDestinationOutsideSource(sourcePath, targetPath);
		await guardedRename({
			from: staged,
			to: targetPath
		...
		if (!stagedCommitted) {
			try {
				const stagedIdentity = await fs$1.lstat(staged);
