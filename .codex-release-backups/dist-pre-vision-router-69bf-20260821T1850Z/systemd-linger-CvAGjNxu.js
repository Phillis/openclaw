import { S as resolveSystemdUserServiceAccount, i as readSystemdUserLingerStatus, r as enableSystemdUserLinger, x as isSystemdUserServiceAvailable } from "./systemd-uvTpBsgP.js";
import { t as note } from "./note-C_xoKlB9.js";
//#region src/commands/systemd-linger.ts
async function readGatewayServiceLingerStatus(env) {
	const user = resolveSystemdUserServiceAccount(env);
	if (!user) return null;
	return await readSystemdUserLingerStatus({
		env,
		user
	});
}
