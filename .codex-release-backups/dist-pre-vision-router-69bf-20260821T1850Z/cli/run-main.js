...
import { i as normalizeEnv, n as isTruthyEnvValue } from "../env-uyT2Z2BT.js";
...
import { n as resolveManifestCommandAliasOwnerInRegistry, r as resolveManifestToolOwnerInRegistry } from "../manifest-command-aliases-DFXNq8y7.js";
import { t as assertSupportedRuntime } from "../runtime-guard-mcgA7MEH.js";
...
import { a as shouldSkipPluginCommandRegistration, r as shouldRegisterPrimaryCommandOnly, t as isReservedNonPluginCommandRoot } from "../command-registration-policy-advooG02.js";
import { n as resolveCliNetworkProxyPolicy, t as resolveCliCommandPathPolicy } from "../command-path-policy-CKibiM2Y.js";
...
import { t as formatCliCommandSuggestions } from "../command-suggestions-CfaK5Nz7.js";
...
const loadLoggingModule = async () => await import("../logging-BZAcYnCh.js");
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-6vMA061a.js");
const loadManifestCommandAliasesRuntimeModule = async () => await import("../manifest-command-aliases.runtime--UHkLUiy.js");
const loadProxyLifecycleModule = async () => await import("../proxy-lifecycle-BpVSwzWI.js");
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }, { ensureCliExecutionBootstrap }, { defaultRuntime }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-command-CsGtVozU.js"),
		import("../version-qnM0RpgZ.js"),
		import("../banner-DvlwVH8L.js"),
		import("../command-startup-policy-Y6HKZ0iQ.js"),
		import("../command-execution-startup-BBRxZs7e.js"),
	...
	const beforeRun = async (opts) => {
		...
		if (!await startupTrace.measure("gateway-run-pre-bootstrap", async () => {
			const { prepareGatewayRunBootstrap, recheckGatewayRunBootstrap, wasPreparedGatewayRunCoreStatePristine, wasPreparedGatewayRunStatePristine } = await import("../pre-bootstrap-DsEx-mIb.js");
		...
		await startupTrace.measure("gateway-run-bootstrap", async () => {
			...
			const { reloadTrustedGatewayRunEnvironment } = await import("../pre-bootstrap-DsEx-mIb.js");
...
async function closeCliResources() {
	const finalizers = [
		async () => {
			const { listRegisteredAgentHarnesses, disposeRegisteredAgentHarnesses } = await import("../registry-CDshjpWQ.js");
		...
		async () => {
			...
			if (hasManagedProviderLocalServices()) {
				const { stopManagedProviderLocalServices } = await import("../provider-local-service-BhtYKika.js");
		...
		async () => {
			...
			if (hasProviderTransportDispatcherPool()) {
				const { closeProviderTransportDispatcherPool } = await import("../provider-transport-dispatcher-pool-KyMpWWap.js");
		...
		async () => {
			...
			if (hasMemoryRuntime()) {
				const { closeActiveMemorySearchManagersCore } = await import("../memory-runtime-DsFlPwsp.js");
...
async function shouldStartLocalOnboarding(snapshot) {
	...
	const { readLocalOnboardingStateForConfig } = await import("../local-onboarding-state-MF0R7TfF.js");
...
async function resolveConfiguredTuiLaunchTarget(config, options) {
	...
	const { resolveAgentEffectiveModelPrimary, resolveDefaultAgentId } = await import("../agent-scope-DHmCBvSP.js");
...
async function resolveReachableGateway(config, options) {
	...
	const { probeGatewayConfiguredModel } = await import("../onboard-helpers-BTdKJ26T.js");
...
async function resolveGatewayProbePlan(config) {
	...
	if (normalizeOptionalString(config.gateway?.mode) === "remote" && remoteUrl) try {
		const { resolveGatewayClientBootstrap } = await import("../client-bootstrap-CS5Yplw9.js");
...
async function resolveLocalGatewayProbeTargets(config) {
	const [{ resolveGatewayPort }, { resolveControlUiLinks }, { resolveGatewayClientBootstrap }, { readActiveGatewayLockPort }] = await Promise.all([
		...
		import("../control-ui-links-DByEu-g2.js"),
		import("../client-bootstrap-CS5Yplw9.js"),
		import("../gateway-lock-egrmX4RG.js")
	]);
...
async function ensureCliEnvProxyDispatcher() {
	try {
		...
		const { ensureGlobalUndiciEnvProxyDispatcher } = await import("../undici-global-dispatcher-TmdPVHlW.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-DkNcew_6.js"), import("../coverage-BFQk1rJT.js")]));
...
async function runCliWithPreparedOutputMode(originalArgv, options) {
	...
	if (!isHelpOrVersionInvocation && !isDatabaseInvocation && !isAgentExecInvocation(normalizedInvocation.commandPath) && shouldLoadCliDotEnv(loadGlobalEnv)) await startupTrace.measure("dotenv", async () => {
		if (isRemoteAgentDispatchInvocation(normalizedArgv, normalizedInvocation.primary)) {
			...
			const { loadCliDotEnv } = await import("../dotenv-CbzVTcDw.js");
			loadCliDotEnv({
				loadGlobalEnv,
				quiet: true
			});
	...
	if (!isHelpOrVersionInvocation && isGatewayRunInvocation) await startupTrace.measure("gateway-run-select-environment", async () => {
		const [{ selectGatewayRunEnvironment }, { defaultRuntime }] = await Promise.all([import("../pre-bootstrap-DsEx-mIb.js"), import("../runtime-Dn74rUMz.js")]);
	...
	const bareSessionInvocation = !isHelpOrVersionInvocation && mayContainBareSessionUrl ? (await import("../session-ref-3eSJ0o1f.js")).parseBareSessionInvocation(normalizedArgv) : null;
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-CdPYG-9e.js").then((configIo) => normalizedInvocation.primary === "update" ? configIo.readSourceConfigBestEffort() : configIo.readBestEffortConfig({
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-vfpcJXvO.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-CAtyLb_m.js");
		...
		if (bareSessionInvocation) {
			...
			const { runTuiCliAction } = await import("../tui-cli-CeNQ_iRC.js");
		...
		if (bareRootLaunchTarget) {
			if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
				...
				const { runRemoteGatewayInferenceOnboarding } = await import("../onboard-remote-gateway-C9cmKSE8.js");
			...
			if (bareRootLaunchTarget.kind === "onboarding") {
				...
				const { setupWizardCommand } = await import("../onboard-CylBfB-X.js");
			...
			if (bareRootLaunchTarget.kind === "tui") {
				...
				const { runTui } = await import("../tui-CdpcnLro.js");
				await runTui({
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-CejZBhag.js"));
		...
		try {
			const [{ buildProgram }, { formatUncaughtError }, { formatCliFailureLines }, { runFatalErrorHooks }, { installUnhandledRejectionHandler, isBenignUncaughtExceptionError, isUncaughtExceptionHandled }, { restoreRuntimeTerminalState }] = await startupTrace.measure("core-imports", () => Promise.all([
				import("../program-DNbJueFw.js"),
				import("../infra/errors.js"),
				import("../failure-output-BdoII5gQ.js"),
				import("../fatal-error-hooks-BiXaGF_Q.js"),
				import("../unhandled-rejections-BZPyxEhW.js"),
				import("../runtime-Dn74rUMz.js")
			]));
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-F73anP78.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-DOwuyJTa.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const [{ registerPluginCliCommandsFromValidatedConfig }, { resolveCliStartupPolicy }] = await Promise.all([import("../cli-C5zioeg9.js"), import("../command-startup-policy-Y6HKZ0iQ.js")]);
					const startupPolicy = resolveCliStartupPolicy({
						argv: parseArgv,
