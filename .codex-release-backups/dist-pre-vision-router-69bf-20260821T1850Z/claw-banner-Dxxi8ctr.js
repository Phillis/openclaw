...
import { i as supportsDecorativeEmoji, t as decorativeEmoji } from "./decorative-emoji-DymOaaLu.js";
	...
	for (let row = 0; row < ROWS; row++) {
		...
		if (wordmarkRow) {
			...
			for (let col = 0; col < wordmarkRow.length; col++) {
				...
				out += ch === " " ? " " : (params.wordmarkTint?.(23 + col) ?? identityTint)(ch);
			}
		}
...
async function animateBanner(opts) {
	...
	try {
		...
		for (let pass = 0; pass < shimmerPasses; pass++) for (let x = MASCOT_WIDTH; x < 77; x += 4) {
