...
import { A as resolveExpiresAtMsFromEpochSeconds, s as asFiniteNumber, x as parseStrictFiniteNumber } from "./number-coercion-oCkfUEEq.js";
import { t as expectDefined } from "./expect-runtime--WgnKYXT.js";
import "./number-runtime-CoAPZzJY.js";
import "./string-coerce-runtime-D9ocX9lc.js";
import { ut as isJsonObject } from "./shared-client-CueuLl3e.js";
import { r as clampPercent, t as PROVIDER_LABELS } from "./provider-usage.shared-DxRYR38m.js";
import "./provider-usage-x5juOG3c.js";
...
function summarizeRateLimitSnapshot(snapshot, nowMs) {
	...
	const reachedType = normalizeOptionalString(snapshot.rateLimitReachedType) ?? normalizeOptionalString(snapshot.rate_limit_reached_type);
...
function readRateLimitWindow(snapshot, key) {
	...
	return {
		resetsAtMs: resolveExpiresAtMsFromEpochSeconds(asFiniteNumber(window.resetsAt) ?? asFiniteNumber(window.resets_at), { maxMs: 864e13 }) ?? 0,
...
function snapshotHasDisplayableData(snapshot) {
	if (normalizeOptionalString(snapshot.rateLimitReachedType) ?? normalizeOptionalString(snapshot.rate_limit_reached_type)) return true;
...
function readOptionalNumberField(record, ...keys) {
	const value = keys.map((key) => asFiniteNumber(record[key])).find((entry) => entry !== void 0);
...
function formatLimitLabel(snapshot) {
	const label = normalizeOptionalString(snapshot.limitName) ?? normalizeOptionalString(snapshot.limit_name) ?? normalizeOptionalString(snapshot.limitId) ?? normalizeOptionalString(snapshot.limit_id);
...
function snapshotHasLimitBlock(snapshot) {
	return Boolean(normalizeOptionalString(snapshot.rateLimitReachedType) ?? normalizeOptionalString(snapshot.rate_limit_reached_type) ?? readWindowEntries(snapshot).some((entry) => entry.window.usedPercent !== void 0 && entry.window.usedPercent >= 100));
}
function isCodexLimitSnapshot(snapshot) {
	const id = normalizeOptionalString(snapshot.limitId) ?? normalizeOptionalString(snapshot.limit_id);
...
function resolveCodexProviderUsagePlan(snapshot) {
	const plan = normalizeOptionalString(snapshot.planType) ?? normalizeOptionalString(snapshot.plan_type);
...
function formatCodexCreditSummary(credits) {
	if (!credits) return;
	if ((asBoolean(credits.hasCredits) ?? asBoolean(credits.has_credits)) === false) return;
	if (asBoolean(credits.unlimited)) return "Unlimited credits";
...
function formatWindowSignature(value) {
	if (!isJsonObject(value)) return "";
	return `${asFiniteNumber(value.usedPercent) ?? asFiniteNumber(value.used_percent) ?? ""}:${asFiniteNumber(value.resetsAt) ?? asFiniteNumber(value.resets_at) ?? ""}`;
}
