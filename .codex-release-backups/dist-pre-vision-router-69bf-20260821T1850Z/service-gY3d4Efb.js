...
import { c as validateCloudWorkerProfileSettings } from "./zod-schema-CpTL91PM.js";
...
import { h as onSessionIdentityMutation } from "./session-accessor.sqlite-lifecycle-BFaW8ajj.js";
...
import { a as WORKER_PROTOCOL_MAX_INFERENCE_PAYLOAD_BYTES, g as validateWorkerInferenceTerminalOutcome, h as validateWorkerInferenceTerminalFrame, p as validateWorkerInferenceEventFrame } from "./worker-inference-DaOiVsCq.js";
import { t as boundedJsonUtf8Bytes } from "./json-utf8-bytes-3IFmJZrr.js";
import "./device-provider-r5BsRUuF.js";
import { n as registerWorkerInferenceSessionDrain } from "./inference-control-internal-CQh2JfZC.js";
import { a as validateWorkerConnectionIdentity, c as createWorkerCredentialMaterial, n as admitWorkerConnection, o as verifyWorkerAdmissionHandshake, r as resolveLocalWorkerBuild, t as deriveEnvironmentIntent } from "./service-contract-D-KMCo4L.js";
import { t as boundedWorkerError } from "./worker-error-BxWIGLZI.js";
import { i as normalizeWorkerSshEndpoint, r as normalizeWorkerDesktopEndpoint, t as WorkerSessionAlreadyAttachedError } from "./store-DI9reGQg.js";
...
function createWorkerCredentialBroker(options) {
	...
	const ensurePendingCredential = (record, sessionId) => {
		...
		const credentialHasDurableTurn = credential?.deliveredAtMs !== null && credential?.ownerEpoch === record.ownerEpoch && credential.sessionId === sessionId && sessionId !== null && options.placementStore?.hasWorkerTurn({
			sessionId,
			environmentId: record.environmentId,
			ownerEpoch: record.ownerEpoch
		}) === true;
	...
	const attachSession = async (request) => {
		...
		return withLock(request.environmentId, async () => {
			...
			let currentBuild;
			try {
				currentBuild = resolveLocalWorkerBuild(current.bootstrapReceipt) ?? await options.prepareInstallation("bundle");
			} catch {
	...
	const acquireTurnCredential = (binding) => withLock(binding.environmentId, async () => {
	...
	const acknowledgeCredentialDelivery = (claim) => {
...
const TUNNEL_START_TIMEOUT_MS = 3 * 6e4;
function createWorkerEnvironmentAccess(options) {
	...
	const startTunnel = async (request) => {
		...
		await withLock(request.environmentId, async () => {
			...
			if (record.providerId === "device" && !record.sshEndpoint && record.bootstrapReceipt.installKind === "local") {
				const profileSettings = record.profileSnapshot.settings;
				const deviceId = isRecord(profileSettings) ? profileSettings.device : void 0;
				const sessionId = record.attachedSessionIds[0];
				if (!nodeTunnels || typeof deviceId !== "string" || !deviceId.trim() || !sessionId) throw serviceError("invalid_state", "Device worker tunnel runtime is unavailable");
			...
			if (!verifyWorkerAdmissionHandshake(record.bootstrapReceipt, currentBundle)) throw serviceError("invalid_state", "Worker must bootstrap the current build before continuing");
		...
		const timeoutError = serviceError("provider_failure", "Worker tunnel did not connect within 3 minutes; check worker SSH reachability and retry");
		try {
	...
	const observeDesktop = async (request) => {
		...
		await withLock(request.environmentId, async () => {
			...
			const record = store.get(request.environmentId);
			if (!record) throw serviceError("environment_not_found", `Unknown worker environment: ${request.environmentId}`);
			if (!inState(record, "ready", "idle", "attached") || record.destroyRequestedAtMs !== null || !record.leaseId || !record.sshEndpoint || !record.desktop) throw serviceError("invalid_state", "environment has no desktop; desktop is a warm-time capability of the profile");
			const provider = providerFor(record.providerId);
			ownerEpoch = record.ownerEpoch;
		...
		const { DESKTOP_OBSERVE_PATH, mintDesktopObserverToken } = await import("./observe-bridge-0xkj9jpR.js");
	...
	const stopTunnel = async (environmentId, ownerEpoch) => {
		await withLock(environmentId, async () => {
			await Promise.all([tunnels?.stop(environmentId, ownerEpoch), nodeTunnels?.stop(environmentId, ownerEpoch)]);
		});
	};
	return {
		get: (environmentId) => {
...
function requireProviderProvisionTimeoutMs(timeoutMs) {
	if (timeoutMs === void 0) return;
	if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 2147e6) throw new WorkerProviderError(`Worker provider provision timeout must be an integer from 1 through ${MAX_TIMER_TIMEOUT_MS}ms`);
	return timeoutMs;
}
function createWorkerProviderLifecycle(options) {
	...
	const failBootstrap = async (record, leaseId, provider, error) => {
		...
		try {
			await callProvider(record.environmentId, () => provider.destroy(lifecycleLease(record, leaseId)));
		} catch (cleanupError) {
			saveError(destroying, /* @__PURE__ */ new Error(`${detail}; provider teardown pending: ${boundedWorkerError(cleanupError)}`));
			throw serviceError("bootstrap_failure", `Worker bootstrap failed; teardown is pending: ${detail}`);
		}
	...
	const finishProvision = async (record, provider, preparedInstallation) => {
		...
		try {
			const profile = requireWorkerProfile(record.profileSnapshot.settings);
			const providerTimeoutMs = options.providerCallTimeoutMs === void 0 ? requireProviderProvisionTimeoutMs(provider.resolveProvisionTimeoutMs?.(profile)) : void 0;
			lease = requireWorkerLease(await callProvider(record.environmentId, () => provider.provision(profile, record.provisionOperationId), providerTimeoutMs));
		} catch (error) {
			const detail = boundedWorkerError(error);
	...
	const resumeProvision = async (record, provider = providerFor(record.providerId)) => {
		let installation;
		if (record.state === "requested" && record.destroyRequestedAtMs === null && provider.provisionBeforeInstallation !== true) try {
	...
	const reconcileRecord = async (initialRecord) => {
		...
		if (record.destroyRequestedAtMs === null && inState(record, "ready", "idle", "attached")) {
			...
			try {
				currentBundle = localBuild ? void 0 : await options.prepareInstallation("bundle");
		...
		if (!leaseId) {
			...
			if (provisioned?.leaseId && provisioned.destroyRequestedAtMs !== null) await finishDestroy(provisioned, provider).catch(() => void 0);
			return;
		}
		...
		if (record.sharedHost !== null && record.sharedHost !== inspectedSharedHost) await tunnels?.stop(record.environmentId);
	...
	const createWithProfile = async (profileId, idempotencyKey, inherited) => {
		let stopping = options.isStopping();
		if (stopping) throw serviceError("invalid_state", "Worker environment service is stopping");
		...
		return withLock(environmentId, async () => {
			...
			if (existing) {
				if (existing.profileId !== normalizedProfileId || inherited !== void 0 && (existing.providerId !== inherited.providerId || !isDeepStrictEqual(existing.profileSnapshot, inherited.profileSnapshot))) throw serviceError("invalid_profile", "Idempotency key belongs to another profile");
...
function createWorkerTurnRpc(options) {
	...
	const executeSessionTool = async (identity, toolName, request, signal) => {
		...
		let result;
		try {
			result = await options.executeSessionTool(toolName === "sessions_spawn" ? {
				identity,
				toolName,
				request,
				...signal ? { signal } : {}
	...
	return {
		admitWorker: async (admission) => {
			...
			let expectedBuild;
			try {
				expectedBuild = resolveLocalWorkerBuild(store.get(admission.environmentId)?.bootstrapReceipt) ?? await options.prepareInstallation("bundle");
			} catch {
				return {
					ok: false,
					reason: "environment-unavailable"
			...
			if (!options.placementStore || admitted.identity.sessionId === null && admitted.identity.runId === null) return expired ? {
				ok: false,
				reason: "credential-expired"
			} : admitted;
...
function createWorkerEnvironmentService(options) {
	...
	const now = options.now ?? Date.now;
	const tunnelLifecycle = options.tunnelManager || options.nodeTunnelManager ? { stop: async (environmentId, ownerEpoch) => {
		await Promise.all([options.tunnelManager?.stop(environmentId, ownerEpoch), options.nodeTunnelManager?.stop(environmentId, ownerEpoch)]);
	} } : void 0;
	...
	const reconcilePass = async () => {
		await runTasksWithConcurrency({
			tasks: store.listForReconcile().map((candidate) => () => reconcileEnvironment(candidate.environmentId).catch(() => warn(`Worker environment reconcile failed (${candidate.environmentId}, ${candidate.providerId})`))),
			limit: 8
		});
	...
	const service = {
		...
		create: async (profileId, idempotencyKey) => environmentAccess.project(await providerLifecycle.createWithProfile(profileId, idempotencyKey)),
		createFromProfileSnapshot: async (profile, idempotencyKey) => environmentAccess.project(await providerLifecycle.createWithProfile(profile.profileId, idempotencyKey, {
			providerId: profile.providerId,
			profileSnapshot: profile.profileSnapshot
		})),
		destroy: async (environmentId) => environmentAccess.project(await providerLifecycle.destroy(environmentId)),
