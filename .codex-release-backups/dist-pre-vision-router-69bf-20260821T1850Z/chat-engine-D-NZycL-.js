...
import { i as loadSystemAgentOverview } from "./overview-Cs2G3rQv.js";
import { a as resolveSystemAgentVerifiedInferenceRoute, i as resolveSystemAgentExpectedAgentHarnessRuntimeArtifact } from "./verified-inference-FlkjxyZP.js";
import { s as prepareSystemAgentRunAdmission } from "./admitted-run-context-Dl2hrF3z.js";
import { a as parseSystemAgentOperation, c as isSystemAgentSensitiveConfigValue, i as isPersistentSystemAgentOperation, l as redactSystemAgentConfigPath, n as describeSystemAgentPersistentOperation, s as isInvalidConfigSetOperation, t as executeSystemAgentOperation } from "./operations-XBlhHXtv.js";
...
import { t as approvalQuestion } from "./dialogue-Cyo44s8o.js";
import { n as resolveOperatorApprovalDecision, r as resolvePendingOperatorProposal, t as classifySystemAgentApprovalText } from "./operator-approval-29Q96zRE.js";
import { a as normalizeCliModel } from "./helpers-BDfTQdWm.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./embedded-agent-tool-results-DdlTNRZf.js"),
		import("./system-agent-tool-Dw3bkJyX.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-Dg3Ub67J.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-BaC-L2QF.js")).runEmbeddedAgent)({
				...shared,
				preparedRunAdmission,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
...
var ChatTurnRouter = class {
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.options.classifyApproval ?? (await import("./approval-intent-w_QAB7qE.js")).classifySystemAgentApprovalIntent)({
			message: text,
	...
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.options.planWithAssistant ?? (await import("./assistant-Clgy_vcl.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
...
var SystemAgentChatEngine = class {
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-Clgy_vcl.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
