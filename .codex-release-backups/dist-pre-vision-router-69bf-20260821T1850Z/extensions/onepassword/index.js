...
import { n as extractErrorCode } from "../../errors-CqPTYU6G.js";
...
import { t as resolveTrustedOnePasswordCli } from "../../onepassword-op-path-CPhyLinO.js";
...
function readOptionalOnePasswordString(record, key) {
	const value = record[key];
...
function readOnePasswordNumber(record, key, fallback, options) {
...
function parseOnePasswordConfig(value) {
	...
	return {
		...
		opTimeoutMs: readOnePasswordNumber(value, "opTimeoutMs", 15e3, {
			integer: true,
			allowZero: false
		}),
		items
...
var onepassword_default = definePluginEntry({
	...
	register(api) {
		...
		api.registerCli(async ({ program, config }) => {
			const { registerOnePasswordCommands } = await import("../../cli-3rjnuIGk.js");
			const { registerOnePasswordSecretRefCommands } = await import("../../secret-ref-cli-vLcwsdyS.js");
