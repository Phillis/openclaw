...
import { I as markHostPluginUsageDiagnosticEvent, f as isDiagnosticsEnabled, o as emitTrustedDiagnosticEvent } from "./diagnostic-events-Djn4AVRp.js";
...
async function runIsolatedAgentRuntimeCompletion(params) {
	...
	try {
		const operation = (async () => {
			const { runIsolatedCompletion } = await import("./isolated-completion-DucFb-3O.js");
...
async function resolveAgentId(params) {
	...
	const { resolveDefaultAgentId } = await import("./agent-scope-DHmCBvSP.js");
...
function createRuntimeLlm(options = {}) {
	...
	return { complete: async (params) => {
		...
		const [{ prepareSimpleCompletionModelForAgent, completeWithPreparedSimpleCompletionModel, resolveSimpleCompletionSelectionForAgent }, cfg] = await Promise.all([import("./simple-completion-runtime-DyZZ_-C7.js"), Promise.resolve(resolveRuntimeConfig(options))]);
