...
import { L as collectChannelSchemaMetadataCore, R as collectPluginSchemaMetadataCore } from "./io-BTBpQ7uO.js";
...
import { p as findWildcardHintMatch } from "./zod-schema-CpTL91PM.js";
import { n as ChannelsSchema } from "./zod-schema.channels-config-DkZP1G3R.js";
import { r as isReservedSystemAgentId } from "./agent-id-BYpRMvce.js";
import { i as sameDefaultInferenceRoute, n as projectInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-DFbS6qbC.js";
import { i as appendSystemAgentAuditEntry, r as SYSTEM_AGENT_AUDIT_STORE_LABEL } from "./audit-BL7eqPcK.js";
import { n as t } from "./i18n-BzsUVhtU.js";
import { n as redactConfigObject, t as REDACTED_SENTINEL } from "./redact-snapshot-C6BdvGLp.js";
import { a as parseConfigSetPath, o as parseConfigSetValue } from "./config-cli-path-DUkpyN_X.js";
import { i as isKernelOwnedChannelConfigKey, n as classifyConfigSchemaPathSegment, t as buildConfigSchemaCore } from "./schema-B2oqZMYf.js";
import { r as isOpenClawTrustedPluginInstallSpec } from "./install-provenance-BEswa1wG.js";
...
const loadOverviewModule$1 = async () => await import("./overview-Ce_eWyqC.js");
...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { scheduleSafeGatewayRestart } = await import("./restart-coordinator-Ds4CqvX_.js");
		return scheduleSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-CIPh93OW.js");
	if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
	const listPlugins = deps?.listChannelSetupPlugins ?? (await import("./setup-registry-xG__pnsQ.js")).listChannelSetupPlugins;
	const resolveEntries = deps?.resolveChannelSetupEntries ?? (await import("./discovery-DKVvltoZ.js")).resolveChannelSetupEntries;
	const isConfigured = deps?.isChannelConfigured ?? (await import("./channel-configured-shared-BSjxNWQ7.js")).isStaticallyChannelConfigured;
	const { shouldShowChannelInSetup } = await import("./discovery-DKVvltoZ.js");
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-D2PiLkuL.js");
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-D2PiLkuL.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-BO6RV4qB.js")).applySystemAgentSetup;
			const surface = ctx.deps?.setupSurface ?? "cli";
			const recovery = surface === "cli" ? await (await import("./setup-recovery-C6oOjT_5.js")).loadLocalSetupRecovery(operation.workspace) : void 0;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-BO6RV4qB.js");
...
async function isPluginBackingDefaultInferenceRoute(pluginId) {
	...
	const { resolveModelRuntimePolicy } = await import("./model-runtime-policy-CmDglk1g.js");
	...
	const { resolveOwningPluginIdsForProviderRef } = await import("./providers-BrsKMBdL.js");
...
const loadOverviewModule = async () => await import("./overview-Ce_eWyqC.js");
	...
	switch (operation.kind) {
		...
		case "plugin-list":
			await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
				const { runPluginsListCommand } = await import("./plugins-list-command-CL8WZnAn.js");
				await runPluginsListCommand({}, pluginRuntime);
			}))(runtime);
			return { applied: false };
		case "plugin-search":
			await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
				const { runPluginsSearchCommand } = await import("./plugins-search-command-BcMS66q1.js");
		...
		case "config-schema": {
			const { buildConfigSchemaCore, lookupConfigSchema } = await import("./schema-CsV65lV3.js");
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-LNT_fr4j.js");
		...
		case "create-agent":
			...
			return await applyPersistentOperation({
				...
				opts,
				run: async (ctx) => {
					const createAgentForOperation = ctx.deps?.createAgent ?? (await import("./agent-create-CCgIvhCe.js")).createAgent;
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-BIfXub6a.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-Sz8rrNf0.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-CdpcnLro.js")).runTui)({
