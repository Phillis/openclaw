...
import "./simple-completion-runtime-PPshd0wh.js";
...
function createCodexAppServerAgentHarness(options) {
	...
	return {
		...
		...sessionCatalogControl && sessionRuntime ? { sessionFork: {
			...
			fork: async (params) => {
				const { forkCodexUpstreamSession } = await import("./upstream-session-fork-CFvDDDDm.js");
		...
		authBinding: { fingerprint: async (params) => {
			const { fingerprintCodexAppServerAuthBinding } = await import("./auth-binding-DCgrR_DF.js");
		...
		fetchUsageSnapshot: async (ctx) => {
			const { fetchCodexAppServerUsageSnapshot } = await import("./usage-B5QErs_8.js");
		...
		loadMcpToolCatalog: async (params) => {
			const { loadCodexEffectiveMcpCatalog } = await import("./effective-mcp-catalog-OPknE_HW.js");
		...
		runIsolatedCompletionV2: async (params) => {
			...
			const { runCodexIsolatedCompletion } = await import("./isolated-completion-r01Ozojx.js");
		...
		finalizeSettledTurn: async (params) => {
			const { runCodexSettledTurnFinalization } = await import("./settled-turn-finalizer-mdYIWvnC.js");
			return runCodexSettledTurnFinalization(params, { pluginConfig: options?.resolvePluginConfig?.() ?? options?.pluginConfig });
		},
		runSideQuestion: async (params) => {
			const { runCodexAppServerSideQuestion } = await import("./side-question-CGUbOWew.js");
		...
		compact: async (params) => {
			const { maybeCompactCodexAppServerSession } = await import("./compact-BobmD1ix.js");
		...
		reset: async (params) => {
			if (params.sessionId) {
				const [{ reclaimCurrentCodexSessionGeneration, sessionBindingIdentity }, { retireCodexAppServerSessionGeneration }] = await Promise.all([import("./session-binding-DJYa_KUK.js"), import("./session-retirement-hU5DFDot.js")]);
