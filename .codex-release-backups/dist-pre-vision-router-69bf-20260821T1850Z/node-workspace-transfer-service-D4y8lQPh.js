...
import { T as MAX_WORKSPACE_MANIFEST_BYTES, _ as parseWorkerWorkspaceManifest, h as MAX_RECONCILIATION_TOTAL_BYTES, p as MAX_RECONCILIATION_ENTRIES, r as readWorkspaceFileSnapshotWithLimit, w as MAX_WORKSPACE_INVENTORY_TOTAL_BYTES, y as serializeWorkerWorkspaceManifest } from "./workspace-actual-manifest-oJ3NxEjy.js";
import { d as readActualWorkspaceManifest, o as assertWorkspaceMatchesManifest } from "./workspace-reconcile-B4y8jodH.js";
import { f as workerWorkspaceTransferPaths } from "./workspace-result-staging-BNDzN-sY.js";
import { i as runLocalCommandToFile, r as readWorkspaceTransferPaths, t as createGitTransferList } from "./workspace-sync-local-cHGb0AEv.js";
...
async function successfulGit(root, args) {
	...
	if (result.termination !== "exit" || result.code !== 0) throw new Error("Worker workspace Git inspection failed");
...
async function prepareNodeWorkspaceTransferSnapshot(params) {
	...
	let includePaths;
	if (gitAdmin) {
		if (await fs.realpath(await successfulGit(root, ["rev-parse", "--show-toplevel"])) !== root) throw new Error("Worker git workspace sync requires the managed worktree root");
		baseCommit = await successfulGit(root, [
			"rev-parse",
			"--verify",
			"HEAD"
		]);
		if (!/^[a-f0-9]{40}(?:[a-f0-9]{24})?$/u.test(baseCommit)) throw new Error("Worker workspace Git base is not a commit id");
		const transferable = await readWorkspaceTransferPaths(await createGitTransferList({
			gitRoot: root,
