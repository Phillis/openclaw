...
import { t as buildConfiguredAcpSessionKey } from "./persistent-bindings.types-Dc67mvsP.js";
...
async function ensureConfiguredAcpBindingSession(params) {
	...
	try {
		...
		if (resolution.kind === "ready" && sessionStructurallyMatchesConfiguredBinding({
			...
			if (params.spec.model && normalizeOptionalString(resolution.meta.runtimeOptions?.model) !== params.spec.model) await acpManager.updateSessionRuntimeOptions({
				cfg: params.cfg,
				sessionKey,
		...
		await acpManager.initializeSession({
			...
			runtimeOptions: params.spec.model ? { model: params.spec.model } : void 0,
			cwd: params.spec.cwd,
