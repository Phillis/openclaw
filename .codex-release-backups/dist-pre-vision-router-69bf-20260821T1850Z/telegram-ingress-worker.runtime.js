...
import { r as resolveTelegramTransport, s as isRetryableTelegramApiError, w as normalizeTelegramApiRoot, y as readTelegramRetryAfterMs } from "./fetch-DrlSGPoD.js";
import { n as resolveTelegramLongPollTimeoutSeconds, t as TELEGRAM_GET_UPDATES_REQUEST_TIMEOUT_MS } from "./request-timeouts-D1yM1dSn.js";
...
function postPollError(port, err, retryAfterMs) {
	const errorCode = readTelegramErrorCode(err);
	port.postMessage({
		type: "poll-error",
		message: formatErrorMessage(err),
		...errorCode === void 0 ? {} : { errorCode },
		...errorCode === 429 && retryAfterMs !== void 0 && Number.isFinite(retryAfterMs) && retryAfterMs > 0 ? { retryAfterMs } : {},
		finishedAt: Date.now()
	});
}
	...
	try {
		for (;;) {
			...
			try {
				...
				try {
					await sleepWithAbort(retryAfterMs ?? computeBackoff(TELEGRAM_RETRY_BACKOFF_POLICY, failures), stopController.signal, { ref: false });
				} catch (sleepErr) {
