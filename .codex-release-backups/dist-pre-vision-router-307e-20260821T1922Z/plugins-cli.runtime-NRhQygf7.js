...
import { p as shortenHomeInString } from "./utils-DEqefz4f.js";
...
import { a as resolveConfiguredRuntimePluginInstallCandidate, i as collectConfiguredRuntimePluginIds } from "./configured-runtime-plugin-installs-Q-6qNNB5.js";
//#region src/cli/plugins-cli.runtime.ts
function createModuleLoader(load) {
	let promise;
	return () => promise ??= load();
}
const loadPluginsConfigState = createModuleLoader(() => import("./config-state-BvkvVJDV.js"));
const loadPluginsStatus = createModuleLoader(() => import("./status-w7FCcBu7.js"));
const loadPluginSlotSelection = createModuleLoader(() => import("./slot-selection-RiStyG4_.js"));
const loadPluginsCommandHelpers = createModuleLoader(() => import("./plugins-command-helpers-BDS3Q0c1.js"));
const loadPluginsRegistryRefresh = createModuleLoader(() => import("./registry-refresh-D135GGOT.js"));
...
async function runPluginsEnableCommandUnlocked(idInput) {
	let id = idInput;
	assertConfigWriteAllowedInCurrentMode();
	const { enableExplicitlySelectedPluginInConfig } = await import("./enable-bjuiLODD.js");
...
async function runPluginsDisableCommandUnlocked(idInput) {
	...
	const { normalizePluginId } = await loadPluginsConfigState();
	const { buildPluginRegistrySnapshotReport } = await loadPluginsStatus();
	const { setPluginEnabledInConfig } = await import("./plugins-config-I53Fqex9.js");
	const { refreshPluginRegistryAfterConfigMutation } = await loadPluginsRegistryRefresh();
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-M4nBxLSU.js");
...
async function runPluginMarketplaceEntriesCommand(opts) {
	const catalog = await import("./official-external-plugin-catalog-nO_tXW2L.js");
	const cfg = getRuntimeConfig();
...
async function runPluginMarketplaceRefreshCommand(opts) {
	const { loadConfiguredHostedOfficialExternalPluginCatalogEntries } = await import("./official-external-plugin-catalog-nO_tXW2L.js");
	const cfg = getRuntimeConfig();
	...
	const { clearManagedPluginOfficialCatalogCache } = await import("./management-service-BX6kMSg1.js");
	clearManagedPluginOfficialCatalogCache();
	let gatewayRefreshed = true;
	if (result.source !== "bundled-fallback") {
		const { notifyGatewayPluginMetadataChanged } = await import("./plugins-update-gateway-signal-LSujTTbN.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-CkxQr8sW.js");
