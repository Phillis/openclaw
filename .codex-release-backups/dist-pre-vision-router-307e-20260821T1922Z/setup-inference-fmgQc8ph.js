...
import { i as createSystemAgentModelSelectionUpdater, r as createQuickstartNotePrompter, t as applySystemAgentModelSelection } from "./setup-apply-lTlhueue.js";
...
async function runSetupInferenceTest(params) {
	...
	try {
		...
		let result;
		if (plan.runner === "cli") result = await (deps.runCliAgent ?? (await import("./cli-runner-B6Jf0mVW.js")).runCliAgent)({
		...
		else result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-DQGrbvPv.js")).runEmbeddedAgent)({
			preparedRunAdmission,
			sessionId,
			sessionKey,
			sessionManager,
