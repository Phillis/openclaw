...
import { D as resolveExpiresAtMsFromDurationMs, M as resolveNonNegativeIntegerOption, g as isFutureDateTimestampMs, t as MAX_DATE_TIMESTAMP_MS, w as parseStrictPositiveInteger } from "./number-coercion-oCkfUEEq.js";
...
import { a as listAgentIds, d as resolveAgentWorkspaceDir, p as resolveDefaultAgentId, s as resolveAgentConfig } from "./agent-scope-config-BdXMWufB.js";
...
import { n as resolveWorkboardSqlitePath, t as createWorkboardSqliteStores } from "./sqlite-store-CEmiP_4i.js";
...
function normalizeAutomation(value, fallback = {}) {
	if (!isRecord(value)) return Object.keys(fallback).length ? fallback : void 0;
	const record = value;
...
var WorkboardChangeTracker = class {
	...
	track(store) {
		return {
			...
			...store.summarizeBoards ? { summarizeBoards: async () => await store.summarizeBoards() } : {}
		};
	}
	subscribe(listener) {
...
var WorkboardCoreStore = class {
	...
	async listBoards() {
		...
		const aggregates = this.store.summarizeBoards ? await this.store.summarizeBoards() : (await this.list()).map((card) => ({
		...
		for (const aggregate of aggregates) {
			...
			if (aggregate.archived) summary.archived += aggregate.total;
			else summary.active += aggregate.total;
...
const sharedSqliteStores = /* @__PURE__ */ new Map();
