...
import { i as withSuppressedNotes } from "./note-D7f3pYFE.js";
...
async function ensureConfigReady(params, recoveryDeps) {
	...
	const runStateMigrationPreflight = async () => {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-ChKU0XJ9.js")).runDoctorConfigPreflight({
			migrateState: true,
	...
	const { formatConfigIssueLines } = await import("./issue-format-XbK6cqWL.js");
	...
	if (shouldOfferRecovery && !isPluginPackagingFailure) {
		...
		if ((await offerInvalidConfigRecovery({
			...
			retry: async () => {
				configSnapshotPromise = null;
				const { runDoctorConfigPreflight } = await import("./doctor-config-preflight-ChKU0XJ9.js");
