...
import { o as safePathSegmentHashed } from "./install-safe-path-Blov4TZi.js";
...
async function cleanupRetainedLegacyNpmPackages(params) {
	...
	for (const packageDir of listManagedNpmPackageDirs(params.npmRoot)) {
		if (!hasRetainedManagedNpmInstallMarker(packageDir) || markerPreservesPackageFiles(resolveRetainedManagedNpmInstallMarkerPath(packageDir)) || params.activeInstallPaths.some((installPath) => isPathEqualOrInside(packageDir, installPath))) continue;
		try {
...
async function cleanupRetainedManagedNpmInstallGenerations(params = {}) {
	...
	for (const projectRoot of listManagedPluginNpmRootsSync(npmDir)) {
		...
		if (markerEntries.length === 0 || markerEntries.some((entry) => markerPreservesPackageFiles(path.join(markerDir, entry.name))) || !isPathEqualOrInside(projectsDir, projectRoot) || activeInstallPaths.some((installPath) => isPathEqualOrInside(projectRoot, installPath))) continue;
		try {
