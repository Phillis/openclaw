...
function createCodexAppServerAgentHarness(options) {
	...
	return {
		...
		...sessionCatalogControl && sessionRuntime ? { sessionFork: {
			...
			fork: async (params) => {
				const { forkCodexUpstreamSession } = await import("./upstream-session-fork-BwiOIhzA.js");
		...
		authBinding: { fingerprint: async (params) => {
			const { fingerprintCodexAppServerAuthBinding } = await import("./auth-binding-BIAcljeZ.js");
		...
		fetchUsageSnapshot: async (ctx) => {
			const { fetchCodexAppServerUsageSnapshot } = await import("./usage-DqHgqBF5.js");
		...
		loadMcpToolCatalog: async (params) => {
			const { loadCodexEffectiveMcpCatalog } = await import("./effective-mcp-catalog-COft_5Wm.js");
		...
		runIsolatedCompletionV2: async (params) => {
			...
			const { runCodexIsolatedCompletion } = await import("./isolated-completion-Cg1c5oDw.js");
		...
		finalizeSettledTurn: async (params) => {
			const { runCodexSettledTurnFinalization } = await import("./settled-turn-finalizer-BqvSl2PO.js");
			return runCodexSettledTurnFinalization(params, { pluginConfig: options?.resolvePluginConfig?.() ?? options?.pluginConfig });
		},
		runSideQuestion: async (params) => {
			const { runCodexAppServerSideQuestion } = await import("./side-question-xHh_cNQq.js");
		...
		compact: async (params) => {
			const { maybeCompactCodexAppServerSession } = await import("./compact-Cs4sGmeN.js");
		...
		reset: async (params) => {
			if (params.sessionId) {
				const [{ reclaimCurrentCodexSessionGeneration, sessionBindingIdentity }, { retireCodexAppServerSessionGeneration }] = await Promise.all([import("./session-binding-DXH85j2q.js"), import("./session-retirement-Dsm4bgzO.js")]);
