...
import { a as resolveSystemAgentVerifiedInferenceRoute, i as resolveSystemAgentExpectedAgentHarnessRuntimeArtifact } from "./verified-inference-D6Zm0v-p.js";
import { s as prepareSystemAgentRunAdmission } from "./admitted-run-context-BxSN0sUe.js";
import { a as parseSystemAgentOperation, c as isSystemAgentSensitiveConfigValue, i as isPersistentSystemAgentOperation, l as redactSystemAgentConfigPath, n as describeSystemAgentPersistentOperation, s as isInvalidConfigSetOperation, t as executeSystemAgentOperation } from "./operations-CnWfueHS.js";
...
import { t as approvalQuestion } from "./dialogue-HWwzAIyJ.js";
import { n as resolveOperatorApprovalDecision, r as resolvePendingOperatorProposal, t as classifySystemAgentApprovalText } from "./operator-approval-C1YkVBa4.js";
import { a as normalizeCliModel } from "./helpers-8rnQvDzb.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./system-agent-tool-V-X7tzD5.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-B6Jf0mVW.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-DQGrbvPv.js")).runEmbeddedAgent)({
				...shared,
				preparedRunAdmission,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
...
var ChatTurnRouter = class {
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.options.classifyApproval ?? (await import("./approval-intent-DJdTf7M6.js")).classifySystemAgentApprovalIntent)({
			message: text,
	...
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.options.planWithAssistant ?? (await import("./assistant-atYSEy3S.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
...
var SystemAgentChatEngine = class {
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-atYSEy3S.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
