...
import { r as defaultRuntime$1 } from "../../runtime-DtFIMC-W.js";
import { n as normalizeAgentId } from "../../agent-id-Db0rqw_J.js";
import { a as listAgentIds, d as resolveAgentWorkspaceDir, p as resolveDefaultAgentId } from "../../agent-scope-config-BdXMWufB.js";
...
async function buildPolicyCheckReport(options, runtime, ownerSurface) {
	...
	const evaluation = await evaluatePolicy({
		...
		cfg,
		cwd: options.cwd ?? resolveAgentWorkspaceDir(cfg, resolvePolicyCommandAgentId(cfg, options.agent, ownerSurface)),
...
function resolvePolicyCommandAgentId(cfg, rawAgentId, surface) {
	...
	if (requestedAgentId) {
		...
		if (!listAgentIds(cfg).includes(agentId)) throw new Error(`Unknown agent id "${requestedAgentId}". Run \`openclaw agents list\` to see configured agents.`);
		return agentId;
	}
...
async function policyCompareCandidatePath(options) {
	...
	return resolve(options.cwd ?? resolveAgentWorkspaceDir(snapshot.config, resolvePolicyCommandAgentId(snapshot.config, options.agent, "policy compare")), policyPath);
}
