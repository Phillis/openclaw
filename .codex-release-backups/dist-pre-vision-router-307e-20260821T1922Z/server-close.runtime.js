...
import { a as disposeAcpSessionManagerInstance, t as getAcpSessionManager } from "./manager-AL25oBaL.js";
...
import { r as drainActiveSessionsForShutdown } from "./session-reset-service-BCatUQJr.js";
...
async function disposeAllBundleLspRuntimesOnDemand() {
	const { disposeAllBundleLspRuntimes } = await import("./agent-bundle-lsp-runtime-CFENIjaD.js");
	await disposeAllBundleLspRuntimes();
}
async function drainRetainedEmbeddingProvidersOnDemand() {
	const { drainRetainedOpenAiEmbeddingProviders } = await import("./embeddings-http-B-_f6Vfd.js");
	...
	return async (opts) => {
		...
		try {
			...
			await shutdownStep("code-mode-runs", async () => {
				const { disposeAllCodeModeRuns } = await import("./code-mode-state-DUXYiS-C.js");
