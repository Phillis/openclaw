...
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-C6TaS4lM.js");
	...
	const finalizers = [
		...
		async () => {
			...
			if (getActiveMcpLoopbackRuntime()) {
				const { closeMcpLoopbackServer } = await import("../mcp-http-DHMIbHSf.js");
		...
		async () => {
			...
			if (hasMemoryRuntime()) {
				const { closeActiveMemorySearchManagersCore } = await import("../memory-runtime-_MrsttHR.js");
...
async function runCliWithPreparedOutputMode(originalArgv, options) {
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-BmRiwdP0.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-FkeBXe3f.js");
		...
		if (bareSessionInvocation) {
			...
			const { runTuiCliAction } = await import("../tui-cli-2eqcRvea.js");
		...
		if (bareRootLaunchTarget) {
			if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
				...
				const { runRemoteGatewayInferenceOnboarding } = await import("../onboard-remote-gateway-B4EQrNsP.js");
			...
			if (bareRootLaunchTarget.kind === "onboarding") {
				...
				const { setupWizardCommand } = await import("../onboard-UcAA6Mu9.js");
			...
			if (bareRootLaunchTarget.kind === "tui") {
				...
				const { runTui } = await import("../tui-DF0cp9dd.js");
				await runTui({
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-D2DceyHu.js"));
		...
		try {
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-zzJzM7Uo.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-Bzv8qC9n.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const [{ registerPluginCliCommandsFromValidatedConfig }, { resolveCliStartupPolicy }] = await Promise.all([import("../cli-I1AtrrAO.js"), import("../command-startup-policy-DuVCDd2T.js")]);
					const startupPolicy = resolveCliStartupPolicy({
						argv: parseArgv,
