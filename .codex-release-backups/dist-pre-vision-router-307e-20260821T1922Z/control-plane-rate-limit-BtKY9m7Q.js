...
function consumeControlPlaneWriteBudget(params) {
	...
	if (!bucket || nowMs - bucket.windowStartMs >= 6e4) {
		if (!controlPlaneBuckets.has(key) && controlPlaneBuckets.size >= CONTROL_PLANE_BUCKET_MAX_ENTRIES) pruneMapToMaxSize(controlPlaneBuckets, CONTROL_PLANE_BUCKET_MAX_ENTRIES - 1);
