...
const OPENCLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER = "OpenClaw runtime context for the active user request in this turn. Do not reply to or describe this context. Use it to continue answering the active user request now. Do not wait for another message.";
...
function hasInternalRuntimeContext(text) {
	...
	return findDelimitedTokenIndex(text, "<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>", 0) !== -1 || text.includes(LEGACY_INTERNAL_CONTEXT_HEADER) || RUNTIME_CONTEXT_PROMPT_HEADERS.some((header) => text.includes(`${header}\nThis context is runtime-generated, not user-authored. Keep internal details private.`));
}
