...
import { C as parseStrictNonNegativeInteger, y as parseDateStringTimestampMs } from "./number-coercion-oCkfUEEq.js";
...
function normalizeMSTeamsPollSelections(poll, selections) {
	...
	return uniqueStrings(selections.map((entry) => parseStrictNonNegativeInteger(entry)).filter((value) => value !== void 0).filter((value) => value >= 0 && value < poll.options.length).map((value) => String(value))).slice(0, maxSelections);
}
