...
import { a as parseSystemAgentOperation, c as isSystemAgentSensitiveConfigValue, i as isPersistentSystemAgentOperation, l as redactSystemAgentConfigPath, n as describeSystemAgentPersistentOperation, s as isInvalidConfigSetOperation, t as executeSystemAgentOperation } from "./operations-DMW4CIQ1.js";
...
import { t as approvalQuestion } from "./dialogue-DZe4RKV8.js";
import { n as resolveOperatorApprovalDecision, r as resolvePendingOperatorProposal, t as classifySystemAgentApprovalText } from "./operator-approval-CPEW2Hqv.js";
import { a as normalizeCliModel } from "./helpers-BwMbSm34.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./system-agent-tool-BziIOELE.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-ODsPxryM.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-DPCuWBmW.js")).runEmbeddedAgent)({
				...shared,
				preparedRunAdmission,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
...
var ChatTurnRouter = class {
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.options.classifyApproval ?? (await import("./approval-intent-BZK79X7L.js")).classifySystemAgentApprovalIntent)({
			message: text,
	...
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.options.planWithAssistant ?? (await import("./assistant-6vygnYZH.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
...
var SystemAgentChatEngine = class {
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-6vygnYZH.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
