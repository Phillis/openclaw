...
import { o as resolveSystemAgentVerifiedInferenceState } from "./verified-inference-ByFu4AcK.js";
import { t as executeSystemAgentOperation } from "./operations-DMW4CIQ1.js";
...
import { t as SystemAgentChatEngine } from "./chat-engine-ruLdFbtw.js";
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	await (opts.runChannelsAdd ?? (await import("./add-BL06OZwd.js")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
...
async function runSystemAgentTui(opts, runtime) {
	...
	for (;;) {
		...
		const runTui = boundOpts.runTui ?? (await import("./tui-Bc41uPh1.js")).runTui;
		try {
			await runTui({
				local: true,
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (verified) {
			...
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-DcxNkklg.js"), import("./model-thinking-default-B2XhNHdp.js")]);
