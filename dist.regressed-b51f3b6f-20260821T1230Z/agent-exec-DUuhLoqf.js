...
import { a as writeRuntimeJson, o as writeRuntimeStdout } from "./runtime-DtFIMC-W.js";
...
async function resolveExecBaseConfig(opts) {
	...
	const { createConfigIO, getRuntimeConfig } = await import("./io-BapGs-eZ.js");
...
async function agentExecCommand(positionalMessage, opts, runtime, deps = {}) {
	...
	try {
		...
		configIo = await import("./io-BapGs-eZ.js");
		...
		const pluginInstallContext = opts.isolated !== true && opts.authEnvOnly !== true ? await import("./install-root-context-DKxRswKj.js") : void 0;
		...
		const { resolveDefaultAgentDir } = await import("./agent-scope-config-BcqgNGFc.js");
		const storedAuthAgentDir = resolveDefaultAgentDir(baseConfig);
		restoreEnvironment = setAgentExecEnvironment({
			stateDir,
			cwd
		});
		runtimePaths = await import("./paths-C39JGP_F.js");
		...
		if (isExecutionIdentityCollectionEnabled(runConfig)) try {
			stopLocalAuditWriter = (await import("./agent-local-audit-B-erlo5S.js")).startAgentLocalAuditWriter({ stateDir });
		} catch {}
		const [{ withAuthProfileStoreAgentDir, withEnvOnlyAuthProfileStore }, { withHostExecInheritedEnvOmitted }, { listKnownProviderAuthEnvVarNames }, runAgent] = await Promise.all([
			import("./auth-profiles-CoWkpj1M.js"),
			import("./host-env-security-B23eCvGK.js"),
			import("./provider-env-vars-CctqK30a.js"),
			deps.runAgent ? Promise.resolve(deps.runAgent) : import("./agent-Bn1KCNXz.js").then((module) => module.agentCommand)
		]);
