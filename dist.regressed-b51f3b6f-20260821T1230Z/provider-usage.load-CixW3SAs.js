...
import { a as resolveProviderAuthEnvVarCandidates } from "./provider-env-vars-D88PwWxT.js";
import "./config-Dl8DJbzM.js";
import { t as dedupeProfileIds } from "./profile-list-BA-END0g.js";
...
import { F as resolveProviderUsageSnapshotWithPlugin, P as resolveProviderUsageAuthWithPlugin, f as listProviderUsagePluginDescriptors } from "./provider-runtime-DLDa-mVy.js";
...
import { x as resolveUsableCustomProviderApiKey } from "./model-auth-provider-config-Dg58X7Zu.js";
...
import { a as isOAuthOnlyUsageProvider, i as ignoredErrors, o as providerUsageLabel, s as raceUsageTimeout } from "./provider-usage.shared-DxRYR38m.js";
...
async function loadProviderUsageSummary(opts = {}) {
	...
	const tasks = auths.map((auth) => {
		const failureSnapshot = (error) => ({
			provider: auth.provider,
			displayName: displayNames.get(auth.provider) ?? providerUsageLabel(auth.provider) ?? auth.provider,
			windows: [],
			error
		});
		return raceUsageTimeout(fetchProviderUsageSnapshot({
			auth,
			config,
			env,
			agentDir: opts.agentDir,
