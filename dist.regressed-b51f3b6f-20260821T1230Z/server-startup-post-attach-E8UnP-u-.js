...
function scheduleContextCachePrewarm(params) {
	...
	const warm = async () => {
		if (stopped) return;
		const { prewarmContextWindowCacheAfterReady } = await import("./context-BIFynNZW.js");
...
async function prewarmGatewaySessionListData(cfg, agentId) {
	const [{ loadCombinedSessionStoreForGatewayCore }, { listSessionsFromStoreAsync }] = await Promise.all([import("./combined-store-gateway-fblIiZ4W.js"), import("./session-utils-list-B5qn6kSr.js")]);
...
function dashboardDataPrewarmItems(cfg, log) {
	...
	return [...agentIds.map((agentId) => ({
		...
		name: "plugins",
		load: async () => {
			const { listManagedPlugins } = await import("./management-service-jBGRJh6-.js");
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-Csuhv8k8.js"));
const loadMainSessionRestartRecoveryMarkingModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-marking-DoCYESYs.js"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-RjT9WtG0.js"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-BGyFP5MQ.js"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-CBg3wstz.js"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-D7erG4eT.js"));
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	runWithGatewayIndependentRootWorkAdmission(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-B7EicRRG.js");
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-DBxvtN9Z.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadPreparedModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				...
				import("./prepared-model-catalog-DcxNkklg.js"),
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-drI3RETO.js")).scheduleGatewayUpdateCheck(...args),
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	const startSidecars = () => params.minimalTestGateway ? Promise.resolve({
		...
		try {
			const { scheduleSubagentRegistrySweep } = await import("./subagent-registry-BeTV-3wU.js");
