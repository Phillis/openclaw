...
import { a as normalizeChannelId, n as getLoadedChannelPlugin, t as getChannelPlugin } from "./registry-B3yYjPW1.js";
...
import { n as resolveChannelSetupCliOptionMetadata } from "./cli-add-options-DmP2XAlV.js";
import { n as requireValidConfigFileSnapshot } from "./config-validation-DcltZ3vk.js";
import { t as normalizeExternalChannelSetupConfig } from "./config-compatibility-LiVJlf1w.js";
import { c as shouldUseWizard } from "./shared-C4IRKaDP.js";
...
const channelSetupPluginInstallLoader = createLazyImportLoader(() => import("./plugin-install-5x4P7GD7.js"));
const onboardChannelsLoader = createLazyImportLoader(() => import("./onboard-channels-Dlggg57y.js"));
...
async function resolveCatalogChannelEntry(raw, cfg) {
	...
	if (!trimmed) return;
	return (cfg ? await import("./trusted-catalog-BbfiVUQj.js").then(({ listTrustedChannelPluginCatalogEntries }) => listTrustedChannelPluginCatalogEntries({
		cfg,
		workspaceDir: resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg))
	})) : await import("./catalog-CeZsZzVw.js").then(({ listRawChannelPluginCatalogEntries }) => listRawChannelPluginCatalogEntries({ excludeWorkspace: true }))).find((entry) => {
...
async function channelsAddCommandImpl(opts, runtime, params) {
	...
	if (shouldUseWizard(params)) {
		...
		const { resolveInitialWizardChannel, runChannelsAddWizardFlow } = await import("./add-wizard-BFLcrTGH.js");
	...
	if (catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-DcIaJWX1.js");
