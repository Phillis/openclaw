...
import { t as compareOpenClawReleaseVersions } from "./npm-registry-spec-D3pNhy09.js";
...
import { a as channelToNpmTag, u as resolveDevUpstreamRef } from "./update-channels-Dv2OGOSa.js";
import { t as readPackageManagerSpec } from "./package-json-Dl4NSL9y.js";
...
async function checkGitUpdateStatus(params) {
	...
	const upstreamRes = trackingRevision ? await runCommandWithTimeout([
		"git",
		"-C",
		root,
		"rev-parse",
	...
	const commitAtSeconds = commitAtRes?.code === 0 ? Number.parseInt(commitAtRes.stdout.trim(), 10) : NaN;
	...
	const trackingUpstream = upstreamRes && upstreamRes.code === 0 ? upstreamRes.stdout.trim() || null : null;
	const receiptUpstream = !trackingUpstream && branch === "HEAD" && sha && params.upstreamFallback?.currentSha.trim().toLowerCase() === sha.toLowerCase() ? params.upstreamFallback.upstreamRef.trim() || null : null;
	const upstream = trackingUpstream ?? receiptUpstream;
	const upstreamSource = trackingUpstream ? "tracking" : receiptUpstream ? "receipt" : void 0;
	...
	const upstreamCommitRes = canCompareUpstream && upstream && sha ? await runCommandWithTimeout([
		"git",
		"-C",
		root,
		"rev-parse",
