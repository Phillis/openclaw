...
import { n as describeSystemAgentPersistentOperation } from "./operations-DMW4CIQ1.js";
...
import { t as getUpdateAvailable } from "./update-startup-DS5TMtEG.js";
import { n as getHealthCache } from "./health-state-DMGbWfAx.js";
import { n as SystemAgentWizardAnswerError, t as SystemAgentChatEngine } from "./chat-engine-ruLdFbtw.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = await createAdmittedWizardSession(() => new WizardSession(async (prompter, signal, runnerSession) => {
			await runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-JX-wK_YQ.js"), import("./setup.shared-BaXgZkEJ.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const { verifySystemAgentInferenceWithFallback } = await import("./inference-fallback-CLOwivJx.js");
