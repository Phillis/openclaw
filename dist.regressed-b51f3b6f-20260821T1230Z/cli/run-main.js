...
const loadCliRegistryLoaderModule = async () => await import("../cli-registry-loader-BRcqExUF.js");
	...
	const finalizers = [
		...
		async () => {
			...
			if (getActiveMcpLoopbackRuntime()) {
				const { closeMcpLoopbackServer } = await import("../mcp-http-DtyQzYQj.js");
		...
		async () => {
			...
			if (hasMemoryRuntime()) {
				const { closeActiveMemorySearchManagersCore } = await import("../memory-runtime-CkvWK5YC.js");
...
async function runCliWithPreparedOutputMode(originalArgv, options) {
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-Dv_Uw4Z1.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-Dplh6sY4.js");
		...
		if (bareSessionInvocation) {
			...
			const { runTuiCliAction } = await import("../tui-cli-CqRdfmlD.js");
		...
		if (bareRootLaunchTarget) {
			if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
				...
				const { runRemoteGatewayInferenceOnboarding } = await import("../onboard-remote-gateway-Df5dwOOr.js");
			...
			if (bareRootLaunchTarget.kind === "onboarding") {
				...
				const { setupWizardCommand } = await import("../onboard-CW7z_AYJ.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-BzqVcr6P.js"));
		...
		try {
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-Bf7Jw-bA.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-B-Zr41q2.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const [{ registerPluginCliCommandsFromValidatedConfig }, { resolveCliStartupPolicy }] = await Promise.all([import("../cli-B0X2lNu3.js"), import("../command-startup-policy-DuVCDd2T.js")]);
					const startupPolicy = resolveCliStartupPolicy({
						argv: parseArgv,
