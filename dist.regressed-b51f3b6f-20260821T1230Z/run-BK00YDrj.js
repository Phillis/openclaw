...
import { j as findOpenClawStateDatabaseSchemaMigrationRequiredError } from "./openclaw-state-db-DlCMR4eQ.js";
import { p as ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV } from "./config-env-vars-BITg3hPR.js";
import { r as CONFIG_AUDIT_STORE_LABEL } from "./io.audit-BpOif9wN.js";
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-BqflJpoA.js");
	return {
		enabled: true,
...
async function runGatewayCommandOnce(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-B1LKN_Nd.js");
	...
	const serverImportMeasurement = startupTrace.startMeasure("cli.server-import", () => import("./server-UhyFIRwD.js"));
	...
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-B1LKN_Nd.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-B00u4ltO.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-B1LKN_Nd.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			const { maybeExplainGatewayServiceStop } = await import("./shared-D9Dzva2d.js");
		...
		if (findOpenClawAgentDatabaseMediaMigrationRequiredError(err)) try {
			const { parkCurrentLaunchAgentForMaintenance } = await import("./launchd-C7IKi2Uv.js");
...
async function runGatewayCommand(opts, hooks = {}, recoveryDeps) {
	try {
		...
		const { offerInvalidConfigRecovery } = await import("./invalid-config-recovery-7gpqxjWF.js");
