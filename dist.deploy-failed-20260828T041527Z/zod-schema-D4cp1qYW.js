...
import { a as AgentSandboxSchema, c as MemorySearchSchema, d as AgentModelSchema, f as AgentToolModelSchema, i as AgentModelRuntimeEntrySchema, n as AgentEntrySchema, o as ElevatedAllowFromSchema, r as AgentModelPolicySchema, s as HeartbeatSchema, t as AgentContextLimitsSchema, u as ToolsSchema } from "./zod-schema.agent-runtime-WXtAE1HE.js";
import { l as isValidSecretRef } from "./ref-contract-BHWY70rN.js";
import { D as SecretsConfigSchema, I as TypingModeSchema, L as VisibleRepliesSchema, N as TtsConfigSchema, O as SsrFPolicyConfigSchema, V as createAllowDenyChannelRulesSchema, d as HexColorSchema, f as HumanDelaySchema, l as GroupChatSchema, m as InboundDebounceSchema, n as BlockStreamingCoalesceSchema, t as BlockStreamingChunkSchema, v as ModelsConfigSchema, w as SecretInputSchema, x as QueueSchema, y as NativeCommandsSettingSchema } from "./zod-schema.core-CTdpjCBO.js";
...
import { i as ApprovalsSchema, n as ChannelsSchema } from "./zod-schema.channels-config-D6JdoD4u.js";
...
import { r as NODE_WORKER_CAPACITY_MAX } from "./node-runner-inventory-C6KxqRM_.js";
...
const LoopGovernorAlertChannelSchema = strictObject({
	channel: string().min(1),
	to: string().min(1),
	accountId: string().optional(),
	threadId: union([string(), number()]).optional()
}).optional();
const LoopGovernorSchema = strictObject({
	agents: array(string().min(1)),
	maxTurnsPerHour: number().int().min(1),
	alertChannel: LoopGovernorAlertChannelSchema
}).superRefine((value, ctx) => {
	if (value.agents.length === 0) ctx.addIssue({
		code: ZodIssueCode.custom,
		path: ["agents"],
...
//#endregion
//#region src/config/cloud-worker-project-profiles.ts
...
function normalizeCloudRepo(originUrl) {
	...
	if (!source.host || segments.length < 2) return;
...
const CONTROL_UI_ENVIRONMENT_ATTRIBUTE = "data-openclaw-environment";
...
const GatewayOperatorRoleDefinitionSchema = strictObject({
	sessions: strictObject({ others: _enum([
		"none",
		"view",
		"suggest",
		"write"
...
const SessionSchema = object({
	...
	rotation: object({
		maxTurns: number().int().positive().optional(),
		maxAgeHours: number().positive().optional(),
		ceilingTokens: number().positive().optional(),
		progressFloorTokens: number().positive().optional()
	}).strict().optional(),
	sharing: object({
...
function projectTelemetryFieldMetadata(field) {
	return Object.fromEntries(Object.entries(TELEMETRY_FIELD_SCHEMAS).flatMap(([fieldPath, schema]) => {
		const value = configUiMetadata.get(schema)?.[field];
		return typeof value === "string" ? [[fieldPath, value]] : [];
	}));
}
...
//#endregion
export { schemaHasChildren as C, findWildcardHintMatch as S, parseGitUrl as _, CONTROL_UI_BASE_PATH_ATTRIBUTE as a, asSchemaObject as b, CONTROL_UI_ENVIRONMENT_ATTRIBUTE as c, DESKTOP_FIELD_HELP as d, DESKTOP_FIELD_LABELS as f, normalizeCloudRepo as g, validateCloudWorkerProfileSettings as h, NodeHostMcpServerNameSchema as i, CONTROL_UI_OWNER_BOOTSTRAP_PROFILE_HINT as l, CLOUD_WORKER_FIELD_LABELS as m, projectTelemetryFieldMetadata as n, CONTROL_UI_BOOTSTRAP_CONFIG_PATH as o, CLOUD_WORKER_FIELD_HELP as p, McpServerNameSchema as r, CONTROL_UI_BOOTSTRAP_PROFILE_FRAGMENT_PARAM as s, OpenClawSchema as t, CONTROL_UI_TERMINAL_ENABLED_ATTRIBUTE as u, parseNonNegativeByteSize as v, cloneSchema as x, parseByteSize as y };
