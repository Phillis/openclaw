...
import { b as require_v10, p as MessageFlags$1 } from "./v10-BDbFcnZN.js";
import { $ as createUserDmChannel, D as hasDiscordV2Components, O as serializePayload, Z as stripUndefinedFields, it as createChannelMessage, ut as getChannel } from "./discord-Bt2iGHi3.js";
import { c as resolveDiscordAccount } from "./accounts-DWE66f3w.js";
import { t as parseAndResolveDiscordTarget } from "./target-resolver-Hct3FMk9.js";
import { r as resolveDiscordReplyMessageId, t as chunkDiscordTextWithMode } from "./chunk-DyXeQ8jK.js";
import { t as isDiscordThreadChannelType } from "./channel-type-CADjTAMq.js";
import { i as fetchChannelPermissionsDiscord, l as createDiscordClient } from "./send.permissions-IU6U8J_d.js";
...
async function parseAndResolveChannelRecipient(raw, cfg, accountId) {
	const resolved = await parseAndResolveDiscordTarget(raw, {
		cfg: requireRuntimeConfig(cfg, "Discord recipient resolution"),
		accountId
	}, { defaultKind: "channel" });
	return {
		kind: resolved.kind,
...
async function buildDiscordSendError(err, ctx) {
	...
	try {
		...
		if (isDiscordThreadChannelType(probedChannelType)) required.push("SendMessagesInThreads");
	...
	if (isDiscordThreadChannelType(probedChannelType)) probedPermissions.push("SendMessagesInThreads");
...
async function sendDiscordText(params) {
	const { rest, channelId, text, request, reply, maxLinesPerMessage, components, embeds, allowedMentions, chunkMode, silent, suppressEmbeds, maxChars, onResult, onPlatformSendDispatch } = params;
	...
	const sendChunk = async (chunk, isFirst) => {
		...
		if (!chunk.trim() && !chunkComponents?.length && !chunkEmbeds?.length) throw new Error("Message must be non-empty for Discord sends");
		const chunkReplyTo = resolveDiscordReplyMessageId(reply, isFirst);
		const kind = chunkComponents?.length || chunkEmbeds?.length ? "card" : "text";
		...
		return {
			...
			}, "text", { safety: "nonce-protected-create" }),
			replyToId: chunkReplyTo,
			kind
		};
	};
	if (chunks.length === 1) {
		const { result, replyToId, kind } = await sendChunk(expectDefined(chunks.at(0), "single Discord text chunk"), true);
...
async function sendDiscordMedia(params) {
	const { rest, channelId, text, mediaUrl, filename, mediaAccess, mediaLocalRoots, mediaReadFile, maxBytes, reply, request, maxLinesPerMessage, components, embeds, allowedMentions, chunkMode, silent, suppressEmbeds, maxChars, onResult, onPlatformSendDispatch } = params;
	...
	let res;
	try {
		res = await request(async () => {
			await onPlatformSendDispatch?.();
			return createChannelMessage(rest, channelId, { body });
		}, "media", { safety: "nonce-protected-create" });
	} catch (err) {
...
//#endregion
export { resolveDiscordSendEmbeds as C, resolveDiscordSendComponents as S, parseAndResolveChannelRecipient as T, DiscordSendError as _, normalizeDiscordPollInput as a, createDiscordMessageNonce as b, normalizeStickerIds as c, resolveDiscordTargetChannelId as d, sendDiscordMedia as f, DISCORD_MAX_STICKER_BYTES as g, DISCORD_MAX_EVENT_COVER_BYTES as h, formatReactionEmoji as i, resolveChannelId as l, DISCORD_MAX_EMOJI_BYTES as m, buildDiscordTextChunks as n, normalizeEmojiName as o, sendDiscordText as p, buildReactionIdentifier as r, normalizeReactionEmoji as s, buildDiscordSendError as t, resolveDiscordChannel as u, SUPPRESS_NOTIFICATIONS_FLAG as v, resolveDiscordSuppressEmbeds as w, resolveDiscordMessageFlags as x, buildDiscordMessageRequest as y };
