...
import { u as readSessionTitleFieldsFromTranscript } from "./session-utils-list-D98WVYL8.js";
import { n as resolveUtilityModelRefForAgent } from "./utility-model-bDQ2cMC-.js";
import { n as generateConversationLabelWithFallback } from "./conversation-label-generator-DWj-FPO1.js";
import { a as isValidAttachmentBase64 } from "./chat-attachments-DPVCWrZk.js";
...
const WORKTREE_SESSION_TITLE_TIMEOUT_MS = 8e3;
const WORKTREE_SESSION_TITLE_ATTEMPT_TIMEOUT_MS = 4e3;
...
function prepareWorktreeSessionTitle(params) {
	...
	if (!source) return;
	const generated = withTimeout(generateDashboardSessionTitle({
		...params,
		entry: params.entry ?? void 0,
		timeoutMs: WORKTREE_SESSION_TITLE_ATTEMPT_TIMEOUT_MS
	}), WORKTREE_SESSION_TITLE_TIMEOUT_MS, "worktree title generation").catch((error) => {
	...
	return {
		source,
		generated,
		persist: async (agentId, entry, sessionKey, storePath) => {
			try {
...
async function maybeGenerateSessionTitle(params) {
	...
	return await getOrCreatePromise(sessionTitleRequests, requestKey, async () => {
		const displayName = await (params.titleGeneration ?? generateDashboardSessionTitle({
			cfg: params.cfg,
			agentId: params.agentId,
			entry: params.entry,
			userMessage: sourceText
...
//#endregion
export { maybeGenerateSessionTitle as a, maybeGenerateDashboardSessionTitle as i, hasExplicitSessionName as n, prepareWorktreeSessionTitle as o, isDashboardSessionTitleCandidate as r, buildDashboardSessionTitleSource as t };
