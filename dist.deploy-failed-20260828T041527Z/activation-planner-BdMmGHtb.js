...
import { s as createPluginIdScopeSet, u as normalizePluginIdScope } from "./current-plugin-metadata-snapshot-AW4B7-Km.js";
import { n as loadPluginManifestRegistryForPluginRegistry } from "./plugin-registry-contributions-JopjOY3b.js";
import { i as passesManifestOwnerBasePolicy, r as isBundledManifestOwner, t as hasExplicitManifestOwnerTrust } from "./manifest-owner-policy-BL1Kt38K.js";
...
function listCommandTriggerReasons(plugin, command) {
	return dedupeReasons([
		listHasNormalizedValue(plugin.activation?.onCommands, command, normalizeCommandId) ? "activation-command-hint" : null,
		listHasNormalizedValue(plugin.cliCommands?.map((descriptor) => descriptor.name), command, normalizeCommandId) ? "manifest-cli-command-owner" : null,
		listHasNormalizedValue((plugin.commandAliases ?? []).flatMap((alias) => alias.cliCommand ?? alias.name), command, normalizeCommandId) ? "manifest-command-alias" : null
	]);
}
