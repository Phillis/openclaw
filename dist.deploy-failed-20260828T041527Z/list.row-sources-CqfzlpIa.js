...
import { f as modelCatalogLogicalKey } from "./model-selection-shared-DbjoXfPH.js";
...
import { r as shouldSuppressBuiltInModelCore } from "./model-suppression-DsP285N1.js";
import { n as projectModelCatalogEntryForRoute, r as resolveConfiguredModelCatalogOverrides } from "./model-catalog-route-D1rTnqax.js";
...
const providerRuntimeModuleLoader = createLazyImportLoader(() => import("./provider-runtime-ynIloPGD.js"));
...
const modelCatalogModuleLoader = createLazyImportLoader(() => import("./prepared-model-catalog-CiP0KTjK.js"));
const scopedModelCatalogModuleLoader = createLazyImportLoader(() => import("./list.scoped-catalog-DcObmhGs.js"));
const modelResolverModuleLoader = createLazyImportLoader(() => import("./model-C8oBhdcb.js"));
...
function buildRow(params) {
	...
	return toModelRow({
		...
		authAvailabilityAuthoritative: params.allowAuthAvailabilityOverride === true || normalizeProviderIdForAuth(params.model.provider) === "openai" || params.authEvaluation.routeResolution !== null
	});
}
...
async function appendDiscoveredRows(params) {
	...
	const modelResolver = params.modelRegistry && params.resolveWithRegistry !== false ? (await modelResolverModuleLoader.load()).resolveModelWithRegistry : void 0;
...
async function appendConfiguredProviderRows(params) {
	...
	for (const [provider, providerConfig] of Object.entries(params.context.cfg.models?.providers ?? {})) for (const configuredModel of providerConfig.models ?? []) {
		...
		const key = modelKey(replaceMode ? params.context.canonicalizeProvider(provider) : provider, modelId);
...
async function appendConfiguredRows(params) {
	const resolveModelWithRegistry = params.modelRegistry ? (await modelResolverModuleLoader.load()).resolveModelWithRegistry : void 0;
