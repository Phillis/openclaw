...
import { j as resolveIntegerOption, x as parseStrictFiniteNumber } from "./number-coercion-CLj0HTDM.js";
...
import { r as extractFirstTextBlock } from "./chat-message-content-BibNiFIq.js";
...
import { i as startOAuthLoopbackCallbackServer } from "./oauth-Dyo6Xu7D.js";
import { l as readMcpOAuthStoreReadOnly, p as operatorMcpOAuthIdentity } from "./mcp-oauth-store--87F5Wew.js";
import { i as completeMcpOAuthAuthorization, o as countMcpOAuthPrincipals, s as readMcpOAuthCredentialsStatus, t as clearMcpOAuthCredentials, u as startMcpOAuthAuthorization } from "./mcp-oauth-inLDZQye.js";
import { t as createSessionMcpRuntime } from "./agent-bundle-mcp-runtime-DlEFrKcw.js";
import { t as buildBundleMcpToolsFromCatalog } from "./agent-bundle-mcp-materialize-Mr-8KHzr.js";
...
import { i as updateConfiguredMcpServerTools, n as unsetConfiguredMcpServer, r as updateConfiguredMcpServer, t as setConfiguredMcpServer } from "./mcp-config-mutation-BoLnQ2z9.js";
import { t as resolveGatewayAuthOptions } from "./gateway-secret-options-BLFucVsF.js";
...
var OpenClawChannelBridge = class {
	...
	async start() {
		...
		const [{ resolveGatewayClientBootstrap }, { GatewayClient: GatewayClientCtor }, { startGatewayClientWhenEventLoopReady }, { APPROVALS_SCOPE, READ_SCOPE, WRITE_SCOPE }, { GATEWAY_CLIENT_CAPS, GATEWAY_CLIENT_MODES, GATEWAY_CLIENT_NAMES }] = await Promise.all([
			...
			import("./client-CRP1bYOF.js"),
			import("./client-start-readiness-grqUg7wf.js"),
			import("./method-scopes-BS88o7W3.js"),
			import("./client-info-BDjmvs89.js")
		]);
...
async function createChannelMcpRuntime(opts = {}) {
	...
	return {
		...
		close: async () => {
			...
			if (errors.length > 1) throw new AggregateError(errors, "OpenClaw channel MCP shutdown failed");
		}
	};
}
//#endregion
...
const MCP_OAUTH_CALLBACK_TIMEOUT_MS = 300 * 1e3;
...
const MCP_CODEX_APPROVAL_ANNOTATION_HINT = "tools have no safety annotations; calls will require interactive approval";
		...
		if (resolved?.kind === "http") {
			if (server.auth === "oauth") {
				if (asNullableRecord(server.oauth)?.identity !== "per-requester") {
					...
					if (authStatus.state === "requires-authorization") issues.push(issue("warning", `OAuth credentials require additional authorization; run ${formatCliCommand(`openclaw mcp login ${name}`)}`));
...
function countConnectedMcpPrincipals(name, server) {
	const resolved = resolveMcpTransportConfig(name, server);
...
async function buildMcpStatusEntries(servers) {
	...
	return Promise.all(entries.map(async ([name, server]) => {
		...
		} else entry.connectedPrincipals = countConnectedMcpPrincipals(name, server);
		return entry;
	}));
}
...
function registerMcpCli(program) {
	...
	mcp.command("serve").description("Expose OpenClaw channels over MCP stdio").option("--url <url>", "Gateway WebSocket URL (defaults to gateway.remote.url when configured)").option("--token <token>", "Gateway token (if required)").option("--token-file <path>", "Read gateway token from file").option("--password <password>", "Gateway password (if required)").option("--password-file <path>", "Read gateway password from file").option("--claude-channel-mode <mode>", "Claude channel notification mode: auto, on, or off", "auto").option("-v, --verbose", "Verbose logging to stderr", false).action(async (opts) => {
		try {
			...
			defaultRuntime.error(`MCP server failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("openclaw gateway status --deep --require-rpc")} to inspect Gateway health.`);
			defaultRuntime.exit(1);
		}
	});
