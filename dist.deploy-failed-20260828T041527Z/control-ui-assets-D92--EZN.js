...
function inspectControlUiAssetHealth(indexPath) {
	if (!indexPath) return {
		kind: "missing-index",
		indexPath
	};
	let html;
	try {
		if (statSync$1(indexPath).size > 256 * 1024) return {
			kind: "incomplete",
			indexPath,
	...
	for (const tag of html.matchAll(/<(?:link|script)\b[^>]*>/giu)) {
		...
		if (!existsSync$1(path.join(path.dirname(indexPath), asset))) return {
			kind: "incomplete",
			indexPath,
			missingAsset: asset
		};
...
async function ensureControlUiAssetsBuilt(runtime = defaultRuntime, opts = {}) {
	...
	let missingStartupAsset = health.kind === "incomplete" ? health.missingAsset : void 0;
	...
	if (!opts.force && !opts.root) {
		...
		if (detectedRoot) {
			...
			missingStartupAsset = detectedHealth.kind === "incomplete" ? detectedHealth.missingAsset : void 0;
			if (detectedHealth.kind === "ready") return {
				ok: true,
				built: false
			};
		}
	...
	if (!opts.force && inspectControlUiAssetHealth(indexPath).kind === "ready") return {
		ok: true,
		built: false
	};
	...
	if (builtHealth.kind === "missing-index") return controlUiAssetsFailure(`Control UI build completed but ${indexPath} is still missing.`, true);
	if (builtHealth.kind === "incomplete") return controlUiAssetsFailure(`Control UI build completed but startup asset ${builtHealth.missingAsset} is missing.`, true);
	return {
		ok: true,
		built: true
	};
...
//#endregion
export { resolveControlUiAssetHealth as a, resolveControlUiRootSync as c, isPackageProvenControlUiRootSync as i, ensureControlUiAssetsBuilt as n, resolveControlUiDistIndexPathForRoot as o, isControlUiStartupAssetsReady as r, resolveControlUiRootOverrideSync as s, CONTROL_UI_ASSETS_BUILD_TIMEOUT_MS as t };
