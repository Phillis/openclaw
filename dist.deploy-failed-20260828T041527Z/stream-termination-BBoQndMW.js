...
function resolveTimeoutMs(model, fallbackMs) {
	return asPositiveFiniteNumber(model?.requestTimeoutMs) ?? fallbackMs;
}
...
function buildStalledErrorEvent(partial, model) {
	if (partial) return {
		type: "error",
		reason: "error",
		error: {
...
function buildUnterminatedErrorEvent(partial, model) {
	if (partial) return {
		type: "error",
		reason: "error",
		error: {
	...
	return {
		type: "error",
		reason: "error",
		error: synthesizeMinimalAssistantMessage("opencode-go stream ended without a terminal event", "error", model)
	};
}
function buildCaughtErrorEvent(partial, error, model) {
...
function synthesizeMinimalAssistantMessage(errorMessage, stopReason, model) {
	return {
		role: "assistant",
		content: [],
		api: model.api,
	...
	return (model, context, callOptions) => {
		...
		(async () => {
			try {
				...
				if (!settled) finishWith(buildCaughtErrorEvent(lastSeenPartial, error, model));
			} finally {
				cleanup();
			}
		})();
