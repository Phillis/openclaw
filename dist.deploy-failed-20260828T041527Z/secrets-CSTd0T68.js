...
import { n as hasAuthProfileConfigConflict, r as hasCurrentAuthProfileConfigConflict, t as applyAuthProfileConfigWithConflictCheck } from "./auth-config-0vBEthnE.js";
import { a as parseEnv, c as readText, u as sanitizeName } from "./helpers-S9wIX9Dg.js";
import { _ as readHermesSecretDetails, d as createHermesSecretItem, h as hermesItemSkipped, l as HERMES_REASON_SECRET_NO_LONGER_PRESENT, m as hermesItemError, n as HERMES_REASON_AUTH_PROFILE_EXISTS, p as hermesItemConflict, r as HERMES_REASON_AUTH_PROFILE_WRITE_FAILED, s as HERMES_REASON_MISSING_SECRET_METADATA } from "./items-DAZiegZe.js";
import { r as normalizeHermesProviderId } from "./model-BUZPOVA5.js";
import { n as collectHermesProviderSecretBindings } from "./config-providers-C6CYBMUl.js";
...
async function buildOpenCodeSecretCandidates(authPath) {
	...
	const candidates = [];
	if (normalizeOptionalString(opencode.key)) candidates.push({
	...
	if (normalizeOptionalString(opencodeGo.key)) candidates.push({
	...
	if (normalizeOptionalString(githubCopilot.refresh) && !githubCopilotEnterpriseUrl) candidates.push({
...
async function buildHermesPoolSecretCandidates(authPath, globalAuthPath) {
	...
	for (const sourceProvider of [...sourceProviders].toSorted()) {
		...
		for (const rawEntry of rawEntries) {
			...
			if (!sourceCredentialId || authType !== "api_key" || source !== "manual" || !normalizeOptionalString(rawEntry.access_token)) continue;
...
async function readSecretCandidateValue(details, source) {
	if (details.sourceKind === "opencode-auth-json") {
		...
		return normalizeOptionalString((isRecord(auth[sourceProvider]) ? auth[sourceProvider] : {})[secretField]);
	}
	if (details.sourceKind === "hermes-auth-json") {
		...
		return isRecord(entry) ? normalizeOptionalString(entry.access_token) : void 0;
	}
