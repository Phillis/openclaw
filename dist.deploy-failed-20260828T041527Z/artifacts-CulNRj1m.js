...
import { _ as validateArtifactsDownloadParams, v as validateArtifactsGetParams, y as validateArtifactsListParams } from "./src-4dv5TpeQ.js";
...
import { i as getTaskSessionLookupByIdForStatus } from "./task-status-access-BpeKxCiz.js";
import { p as visitSessionMessagesAsync } from "./session-transcript-readers-fCOIrclF.js";
...
import { g as resolveSessionSharingTarget, r as authorizeIncognitoSessionTarget, u as createSessionListEntryFilter } from "./session-sharing-DSLYm21V.js";
import { c as parseManagedOutgoingArtifactId, d as resolveManagedOutgoingMediaArtifactDownload, f as resolveManagedOutgoingMediaUrlDownload } from "./managed-image-attachments-BmbhAxU8.js";
import { t as assertValidParams } from "./validation-kYFXohur.js";
import { n as resolveSessionKeyForRun } from "./server-session-key-Dp2sc_RX.js";
...
function resolveAuthorizedArtifactSession(query, cfg, client) {
	...
	const roleVisibilityDenied = Boolean(cfg && hasOperatorBoundary(client, cfg) && target && createSessionListEntryFilter({
		client,
		cfg
	})?.(target.storeKey, target.entry) === false);
	if (!error && !roleVisibilityDenied) return resolved;
...
async function loadArtifacts(query, cfg, opts = {}, client = null) {
...
async function findArtifact(params, cfg, opts = {}, client = null) {
...
const artifactsHandlers = {
	"artifacts.list": async ({ params, respond, context, client }) => {
		...
		const loaded = await runArtifactSessionOperation(respond, () => loadArtifacts(admittedQuery, cfg, { includeDownloadData: false }, client));
		if (!loaded.ok) return;
	...
	"artifacts.get": async ({ params, respond, context, client }) => {
		...
		const admittedQuery = admitArtifactQuery(params, cfg, respond);
		if (!admittedQuery) return;
		const found = await runArtifactSessionOperation(respond, () => findArtifact(admittedQuery, cfg, { includeDownloadData: false }, client));
		if (!found.ok) return;
	...
	"artifacts.download": async ({ params, respond, context, client }) => {
		...
		if (admittedQuery.sessionKey && !admittedQuery.runId && !admittedQuery.taskId && parseManagedOutgoingArtifactId(params.artifactId)) {
			const resolvedResult = await runArtifactSessionOperation(respond, () => resolveAuthorizedArtifactSession(admittedQuery, cfg, client));
		...
		const found = await runArtifactSessionOperation(respond, () => findArtifact(admittedQuery, cfg, { downloadArtifactId: params.artifactId }, client));
		if (!found.ok) return;
		const { artifact } = found.value;
		if (!artifact) {
			respondArtifactNotFound(respond, params.artifactId);
