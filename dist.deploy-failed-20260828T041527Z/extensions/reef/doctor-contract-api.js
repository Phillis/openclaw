...
import { A as reefReplayStoreKey, B as parseReefSetupSession, C as REEF_REVIEWS_MAX_ENTRIES, G as REEF_AUDIT_HEAD_NAMESPACE, J as REEF_AUDIT_MIGRATION_NAMESPACE, K as REEF_AUDIT_MAX_ENTRIES, M as REEF_REGISTRATION_NAMESPACE, N as REEF_REGISTRATION_SESSION_KEY, Q as reefAuditEntryKey, S as REEF_REPLAY_TTL_MS, W as REEF_AUDIT_HEAD_KEY, X as REEF_AUDIT_STORE_MAX_ENTRIES, Y as REEF_AUDIT_NAMESPACE, Z as parseReefAuditHead, _ as REEF_KEYS_MIGRATION_KEY, b as REEF_REPLAY_MAX_ENTRIES, d as REEF_DELIVERED_MAX_ENTRIES, f as REEF_DELIVERED_NAMESPACE, g as REEF_KEYS_KEY, h as REEF_DURABLE_MIGRATION_NAMESPACE, it as parseVerdict, j as REEF_REGISTRATION_IDENTITY_KEY, k as parseReefKeys, m as REEF_DURABLE_MIGRATION_KEY, n as legacyReefFileExists, o as REEF_TRUST_STORE_MAX_ENTRIES, p as REEF_DELIVERED_TTL_MS, q as REEF_AUDIT_MIGRATION_KEY, r as resolveLegacyReefStateDir, s as REEF_TRUST_STORE_NAMESPACE, t as REEF_DURABLE_LEGACY_FILENAMES, u as resolveReefTrustStoreKey, v as REEF_KEYS_MIGRATION_NAMESPACE, vt as verifyChain, w as REEF_REVIEWS_NAMESPACE, x as REEF_REPLAY_NAMESPACE, y as REEF_KEYS_NAMESPACE, yt as verifyChainSegment, z as parseReefIdentityBinding } from "../../doctor-state-paths-Cb0p5p6T.js";
...
function parseLegacySignedReceipt(value) {
	...
	if (value.status !== "accepted" && value.status !== "rejected") throw new Error("invalid Reef replay receipt status");
	if (value.category !== void 0 && typeof value.category !== "string") throw new Error("invalid Reef replay receipt category");
	return {
		id,
		bodyHash,
		auditHead,
		status: value.status,
		signature,
		...value.category !== void 0 ? { category: value.category } : {}
	};
}
function parseLegacyReviewRequest(value) {
	const direction = value.direction;
	if (direction !== "inbound" && direction !== "outbound") throw new Error("invalid Reef review direction");
	return {
