...
function normalizeXiaomiTtsProviderConfig(rawConfig) {
	...
	return {
		...
		baseUrl: normalizeXiaomiTtsBaseUrl(trimToUndefined(raw?.baseUrl) ?? trimToUndefined(process.env.XIAOMI_BASE_URL)),
		model: trimToUndefined(raw?.model) ?? trimToUndefined(raw?.modelId) ?? trimToUndefined(process.env.XIAOMI_TTS_MODEL) ?? DEFAULT_XIAOMI_TTS_MODEL,
		voice: trimToUndefined(raw?.speakerVoice) ?? trimToUndefined(raw?.speakerVoiceId) ?? trimToUndefined(raw?.voice) ?? trimToUndefined(raw?.voiceId) ?? trimToUndefined(process.env.XIAOMI_TTS_VOICE) ?? DEFAULT_XIAOMI_TTS_VOICE,
...
function readXiaomiTtsProviderConfig(config) {
	...
	return {
		...
		voice: trimToUndefined(config.speakerVoice) ?? trimToUndefined(config.speakerVoiceId) ?? trimToUndefined(config.voice) ?? trimToUndefined(config.voiceId) ?? normalized.voice,
...
function readXiaomiTtsOverrides(overrides) {
	...
	return {
		...
		voice: trimToUndefined(overrides.speakerVoice) ?? trimToUndefined(overrides.speakerVoiceId) ?? trimToUndefined(overrides.voice) ?? trimToUndefined(overrides.voiceId),
...
function decodeXiaomiAudioData(body) {
	...
	const audioData = trimToUndefined(asOptionalRecord(asOptionalRecord(asOptionalRecord((Array.isArray(root?.choices) ? root.choices : [])[0])?.message)?.audio)?.data);
