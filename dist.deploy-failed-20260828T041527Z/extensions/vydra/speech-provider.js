import { DEFAULT_VYDRA_BASE_URL, DEFAULT_VYDRA_SPEECH_MODEL, DEFAULT_VYDRA_VOICE_ID, downloadVydraAsset, extractVydraResultUrls, normalizeVydraBaseUrl } from "./shared.js";
...
function normalizeVydraSpeechConfig(rawConfig) {
	const raw = asOptionalRecord(asOptionalRecord(rawConfig.providers)?.vydra) ?? asOptionalRecord(rawConfig.vydra);
	return {
		apiKey: normalizeResolvedSecretInputString({
			value: raw?.apiKey,
			path: "tts.providers.vydra.apiKey"
		}),
		baseUrl: normalizeVydraBaseUrl(normalizeOptionalString(raw?.baseUrl) ?? normalizeOptionalString(process.env.VYDRA_BASE_URL)),
		model: normalizeOptionalString(raw?.model) ?? normalizeOptionalString(process.env.VYDRA_TTS_MODEL) ?? "elevenlabs/tts",
		voiceId: normalizeOptionalString(raw?.voiceId) ?? normalizeOptionalString(process.env.VYDRA_TTS_VOICE_ID) ?? "21m00Tcm4TlvDq8ikWAM"
	};
}
