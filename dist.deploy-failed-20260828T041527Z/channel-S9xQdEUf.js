...
import { f as createChannelMessageAdapterFromOutbound, g as bindIngressLifecycleToReplyOptions, h as createChannelIngressError } from "./channel-outbound-DO-F9-0m.js";
...
import { c as createTopLevelChannelConfigAdapter, s as createScopedDmSecurityResolver } from "./channel-config-helpers-C7An4wuC.js";
...
import { t as createDirectDmPreCryptoGuardPolicy } from "./direct-dm-guard-policy-DIf2z0C5.js";
import { n as createChannelIngressMonitor } from "./ingress-monitor-5WsYdIbW.js";
import { i as runPassiveAccountLifecycle } from "./channel-lifecycle.core-CnejcREy.js";
...
import "./direct-dm-guard-policy-D2VB0RnO.js";
import { c as collectStatusIssuesFromLastError, d as createDefaultChannelRuntimeState, u as createComputedAccountStatusAdapter } from "./status-helpers-CopMHs_f.js";
import { r as buildTrafficStatusSummary, t as buildPassiveChannelStatusSummary } from "./extension-shared-BO-DUGkx.js";
...
import { a as createFixedWindowRateLimiter } from "./webhook-ingress-ByuWujwG.js";
import { r as secp256k1 } from "./sha2-DF7LOszp.js";
import { h as randomBytes, p as hexToBytes, t as base64 } from "./base-CN8S0a2r.js";
import { t as cbc } from "./aes-D4oOuwFi.js";
import { a as getPublicKey, n as SimplePool, r as finalizeEvent, s as verifyEvent } from "./esm-DvxIBjlD.js";
import { n as NostrProfileSchema, t as NostrConfigSchema } from "./config-schema-CgcIn0fz.js";
import { c as validatePrivateKey, u as DEFAULT_RELAYS } from "./setup-adapter-Bs06grZz.js";
import { a as resolveDefaultNostrAccountId, i as listNostrAccountIds, n as nostrSetupContract, o as resolveNostrAccount, r as nostrSetupWizard, s as normalizePubkey } from "./setup-surface-BTzyfAX_.js";
...
function inspectNostrIngressEvent(event) {
	if (!isRecord(event)) throw new NostrIngressPermanentError("invalid-event", "Nostr event must be an object.");
	return {
...
function deserializeNostrIngressEvent(rawEvent, claimedId) {
	...
	if (!isRecord(parsed)) throw new NostrIngressPermanentError("invalid-event", `Nostr ingress row ${claimedId} has an invalid event shape.`);
...
function createNostrIngress(options) {
	...
	const admitOnce = async (prepared) => {
		...
		try {
			...
			throw new Error(`Nostr durable admission failed: ${formatErrorMessage(error)}`, { cause: error });
		}
	};
	return {
		ready: async () => {
...
const startNostrGatewayAccount = async (ctx) => {
	...
	if (!channelRuntime?.inbound?.buildContext) throw new Error("Nostr requires its registered channel runtime context builder");
	...
	const resolveInboundAccess = async (senderPubkey, rawBody, contextBinding) => await resolveStableChannelMessageIngress({
		channelId: "nostr",
		accountId: account.accountId,
		...
		start: async () => {
			const bus = await startNostrBus({
				...
				onMessage: async (senderPubkey, text, reply, meta, lifecycle) => {
					...
					const { dispatchInboundDirectDm } = await import("./inbound-direct-dm-runtime-BqL-Pf6E.js");
					await dispatchInboundDirectDm({
						...
						resolveChannelIngress: async (contextBinding) => {
							...
							if (!exactAccess.senderAccess.allowed) throw new Error(`Nostr sender authorization changed before dispatch (${senderPubkey})`);
							return exactAccess;
						},
						cfg: ctx.cfg,
						channel: "nostr",
...
function inferNostrTargetChatType(target) {
	try {
