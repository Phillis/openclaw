...
import { i as listWorkspacePackageExportAliasEntries, n as buildPluginLoaderJitiOptions, o as resolvePluginLoaderModuleConfig, r as createPluginLoaderModuleCacheKey, t as buildPluginLoaderAliasMap } from "./sdk-alias-TKlN0lQ-.js";
...
function clearRequireCacheSubtree(resolvedPath, dependencyRoot, seen) {
	...
	if (cached) {
		for (const child of cached.children) if (isPathInside(dependencyRoot, child.id)) clearRequireCacheSubtree(child.id, dependencyRoot, seen);
	}
...
function clearPluginModuleLoaderLifecycleCache(params) {
	...
	for (const [modulePath, rootDir] of params.moduleRoots) {
		...
		clearNativeRequireJavaScriptModuleCache(modulePath, { dependencyRoot: path.basename(extensionsDir) === "extensions" && path.basename(distDir) === "dist" ? distDir : rootDir });
	}
...
//#endregion
export { getPluginModuleLoaderStats as a, isJavaScriptModulePath as c, getCachedPluginSourceModuleLoader as i, tryNativeRequireJavaScriptModule as l, createPluginModuleLoaderCache as n, installOpenClawInternalCorePackageNativeResolver as o, getCachedPluginModuleLoader as r, installOpenClawPluginSdkNativeResolver as s, clearPluginModuleLoaderLifecycleCache as t, toSafeImportPath as u };
