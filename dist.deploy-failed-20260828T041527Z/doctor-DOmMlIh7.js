...
import { n as readPersistedInstalledPluginIndex } from "./installed-plugin-index-store-7GzEHL03.js";
...
import { i as withDoctorSqliteMaintenanceLock, r as isDestructiveDoctorSessionSqliteMode } from "./doctor-sqlite-maintenance-lock-9ifiaibD.js";
...
async function doctorCommand(runtime, options) {
	if (options?.stateSqlite) {
		...
		const { runDoctorStateSqliteCompact } = await import("./doctor-state-sqlite-compact-CQfWLk9f.js");
	...
	await (await import("./doctor-health-BkIwZX0n.js")).runDoctorHealthFlow(runtime, options);
}
async function maybeCreateSessionSqliteGithubIssue(runtime, report, options) {
	...
	if (!approved && options.nonInteractive !== true && options.json !== true) {
		const { promptYesNo } = await import("./prompt-BgjBlZUk.js");
