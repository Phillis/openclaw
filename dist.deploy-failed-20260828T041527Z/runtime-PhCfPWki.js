...
import { t as assertMediaNotDataUrl } from "./sandbox-paths-C7Hkb46-.js";
import { _ as readToolStringParam, c as parseAvailableTags, d as readNonNegativeIntegerParam, h as readStringArrayParam, m as readReactionParams, p as readPositiveIntegerParam } from "./common-CI1GnPjt.js";
import { t as jsonResult } from "./tool-results-BCM3fdVS.js";
import { t as readBooleanParam } from "./boolean-param-AuSHeYDH.js";
import { a as withNormalizedTimestamp } from "./date-time-Ch20W-k8.js";
...
import { g as PermissionFlagsBits, i as ChannelType } from "./v10-BDbFcnZN.js";
import { d as resolveDiscordTargetChannelId } from "./send.shared-CkphMmyC.js";
import { o as mergeDiscordAccountConfig, s as resolveDefaultDiscordAccountId, t as createDiscordActionGate } from "./accounts-DWE66f3w.js";
import { r as sendDiscordComponentMessage } from "./send.components-BRDFts-N.js";
import { c as readDiscordComponentSpec, s as coerceDiscordComponentParam } from "./components-ff8c52vl.js";
import { a as normalizeDiscordSlug, c as resolveDiscordChannelConfigWithFallback, n as isDiscordGroupAllowedByPolicy, v as resolveGroupDmAllow } from "./allow-list-BsByEWnJ.js";
import { n as resolveDiscordChannelId } from "./target-parsing-CcbK_-tL.js";
import { r as sendStickerDiscord, t as sendMessageDiscord } from "./send.outbound-TwwYymg7.js";
import { i as resolveDiscordReplyReference, n as createReusableDiscordReplyReference } from "./chunk-DyXeQ8jK.js";
import { t as isDiscordThreadChannelType } from "./channel-type-CADjTAMq.js";
import { c as hasAnyGuildPermissionDiscord, d as createDiscordRuntimeAccountContext, i as fetchChannelPermissionsDiscord, n as canManageGuildRoleDiscord, s as hasAnyChannelPermissionDiscord, t as canManageGuildMemberRoleDiscord, y as getGateway } from "./send.permissions-IU6U8J_d.js";
import { n as getPresence } from "./presence-cache-z1N8MbJ3.js";
import { A as removeRoleDiscord, B as removeChannelPermissionDiscord, C as fetchGuildInfoDiscord, D as kickMemberDiscord, E as fetchVoiceStatusDiscord, F as uploadStickerDiscord, I as createChannelDiscord, L as deleteChannelDiscord, M as timeoutMemberDiscord, N as listGuildEmojisDiscord, O as listGuildChannelsDiscord, P as uploadEmojiDiscord, R as editChannelDiscord, S as fetchChannelInfoDiscord, T as fetchRoleInfoDiscord, V as setChannelPermissionDiscord, _ as searchMessagesDiscord, b as banMemberDiscord, c as DiscordThreadInitialMessageError, d as editMessageDiscord, f as fetchMessageDiscord, g as readMessagesDiscord, h as pinMessageDiscord, i as removeReactionDiscord, j as resolveEventCoverImage, k as listScheduledEventsDiscord, l as createThreadDiscord, m as listThreadsDiscord, n as reactMessageDiscord, o as sendVoiceMessageDiscord, p as listPinsDiscord, r as removeOwnReactionsDiscord, t as fetchReactionsDiscord, u as deleteMessageDiscord, v as unpinMessageDiscord, w as fetchMemberInfoDiscord, x as createScheduledEventDiscord, y as addRoleDiscord, z as moveChannelDiscord } from "./send-BSE1MTZY.js";
...
function readDiscordParentIdParam(params) {
	...
	return readToolStringParam(params, "parentId");
}
/**
...
function readDiscordChannelCreateParams(params) {
	...
	return {
		guildId: readToolStringParam(params, "guildId", { required: true }),
		...
		topic: readToolStringParam(params, "topic") ?? void 0,
...
function readDiscordChannelEditParams(params) {
	const parentId = readDiscordParentIdParam(params);
	return {
		channelId: readToolStringParam(params, "channelId", { required: true }),
		name: readToolStringParam(params, "name") ?? void 0,
		topic: readToolStringParam(params, "topic") ?? void 0,
		position: readNonNegativeIntegerParam(params, "position") ?? void 0,
...
function createDiscordMessagingActionContext(params) {
	const accountId = readToolStringParam(params.input, "accountId");
	...
	return {
		...
		accountId,
		resolveChannelId: () => discordMessagingActionRuntime.resolveDiscordChannelId(readToolStringParam(params.input, "channelId", { required: true })),
		...
		resolveReactionChannelId: async () => {
			const target = readToolStringParam(params.input, "channelId") ?? readToolStringParam(params.input, "to", { required: true });
...
async function resolveGuildIdForGuildAdminAction(params) {
	const guildId = readToolStringParam(params.values, "guildId");
	if (guildId) return guildId;
	const channelLikeId = readToolStringParam(params.values, "channelId") ?? readToolStringParam(params.values, "categoryId");
...
function readChannelScopedPermissionTargetId(action, values) {
	if (action === "eventCreate") return readToolStringParam(values, "channelId");
	if (action === "categoryEdit" || action === "categoryDelete") return readToolStringParam(values, "categoryId");
	return readToolStringParam(values, "channelId");
}
async function resolveGuildAdminActionPermissions(params) {
	if (params.action !== "channelEdit") return params.guard.permissions;
	const channelId = readToolStringParam(params.values, "channelId");
	...
	if (!isDiscordThreadChannelType("type" in channel ? channel.type : void 0)) return params.guard.permissions;
...
async function verifySenderGuildAdminPermission(params) {
	...
	const senderUserId = readToolStringParam(params.values, "senderUserId");
	...
	if (params.action === "roleAdd" || params.action === "roleRemove") {
		const targetUserId = readToolStringParam(params.values, "userId", { required: true });
		const roleId = readToolStringParam(params.values, "roleId", { required: true });
	...
	if (params.action === "channelPermissionSet" || params.action === "channelPermissionRemove") {
		const targetType = readToolStringParam(params.values, "targetType");
		if (targetType === "member") return;
		const targetId = readToolStringParam(params.values, "targetId", { required: true });
...
async function runRoleMutation(params) {
	const guildId = readToolStringParam(params.values, "guildId", { required: true });
	const userId = readToolStringParam(params.values, "userId", { required: true });
	const roleId = readToolStringParam(params.values, "roleId", { required: true });
	await params.mutate({
		guildId,
		userId,
...
function readChannelPermissionTarget(params) {
	return {
		channelId: readToolStringParam(params, "channelId", { required: true }),
		targetId: readToolStringParam(params, "targetId", { required: true })
	};
}
async function handleDiscordGuildAction(action, params, isActionEnabled, cfg, options) {
	const accountId = readToolStringParam(params, "accountId");
	...
	switch (action) {
		case "memberInfo": {
			...
			const guildId = readToolStringParam(params, "guildId", { required: true });
		...
		case "emojiList": {
			...
			const guildId = await resolveGuildIdForGuildAdminAction({
				values: params,
				accountId,
				cfg
			});
			if (!guildId) throw new Error("Discord emoji listing requires guildId or a server channel.");
			await assertGuildMetadataReadAllowed(guildId);
			const limit = Math.min(readPositiveIntegerParam(params, "limit") ?? 100, 100);
			const fetchEmojis = async () => (await discordGuildActionRuntime.listGuildEmojisDiscord(guildId, withOpts())).flatMap(({ name, id, animated }) => name && id ? [{
				name,
				identifier: `${name}:${id}`,
				...animated ? { animated } : {}
			}] : []).toSorted((left, right) => left.name.localeCompare(right.name) || left.identifier.localeCompare(right.identifier));
			const gateway = getGateway(accountId ?? resolveDefaultDiscordAccountId(cfg));
			return jsonResult({
				ok: true,
				emojis: (gateway ? await gateway.fetchGuildEmojis(guildId, fetchEmojis) : await fetchEmojis()).slice(0, limit)
			});
		}
		case "emojiUpload": {
			...
			const mediaUrl = readToolStringParam(params, "mediaUrl", { required: true });
		...
		case "stickerUpload": {
			...
			const guildId = readToolStringParam(params, "guildId", { required: true });
			const name = readToolStringParam(params, "name", { required: true });
			const description = readToolStringParam(params, "description", { required: true });
			const tags = readToolStringParam(params, "tags", { required: true });
			const mediaUrl = readToolStringParam(params, "mediaUrl", { required: true });
			return jsonResult({
				ok: true,
		...
		case "channelInfo": {
			...
			const channelId = readToolStringParam(params, "channelId", { required: true });
		...
		case "eventCreate": {
			...
			const guildId = readToolStringParam(params, "guildId", { required: true });
			const name = readToolStringParam(params, "name", { required: true });
			const startTime = readToolStringParam(params, "startTime", { required: true });
			const endTime = readToolStringParam(params, "endTime");
			const description = readToolStringParam(params, "description");
			const channelId = readToolStringParam(params, "channelId");
			const location = readToolStringParam(params, "location");
			const imageUrl = readToolStringParam(params, "image", { trim: false });
			const entityTypeRaw = readToolStringParam(params, "entityType");
		...
		case "categoryEdit": {
			if (!isActionEnabled("channels")) throw new Error("Discord channel management is disabled.");
			const categoryId = readToolStringParam(params, "categoryId", { required: true });
		...
		case "channelPermissionSet": {
			...
			const targetType = readToolStringParam(params, "targetType", { required: true }) === "member" ? 1 : 0;
			const allow = readToolStringParam(params, "allow");
			const deny = readToolStringParam(params, "deny");
...
async function handleDiscordMessageManagementAction(ctx) {
	switch (ctx.action) {
		...
		case "fetchMessage": {
			...
			const messageLink = readToolStringParam(ctx.params, "messageLink");
			let guildId = readToolStringParam(ctx.params, "guildId");
			let channelId = readToolStringParam(ctx.params, "channelId");
			let messageId = readToolStringParam(ctx.params, "messageId");
			if (messageLink) {
		...
		case "readMessages": {
			...
			const query = {
				...
				before: readToolStringParam(ctx.params, "before"),
				after: readToolStringParam(ctx.params, "after"),
				around: readToolStringParam(ctx.params, "around")
			};
			return jsonResult({
				ok: true,
				channelId,
		...
		case "editMessage": {
			...
			const channelId = ctx.resolveChannelId();
			const messageId = readToolStringParam(ctx.params, "messageId", { required: true });
			const content = readToolStringParam(ctx.params, "content", { required: true });
			await ctx.assertReadTargetAllowed({ channelId });
			return jsonResult({
				ok: true,
		...
		case "searchMessages": {
			...
			let guildId = readToolStringParam(ctx.params, "guildId");
			const content = readToolStringParam(ctx.params, "content") ?? readToolStringParam(ctx.params, "query");
			if (!content) throw new Error("Discord search requires content or query text.");
			const channelId = readToolStringParam(ctx.params, "channelId");
			...
			const authorId = readToolStringParam(ctx.params, "authorId");
...
function resolveActionReplyReference(ctx, replyToId) {
	const reply = ctx.options?.reply;
	return reply ? resolveDiscordReplyReference({
...
async function appendDiscordThreadRenameResult(ctx, params) {
	...
	try {
		if (!isDiscordThreadChannelType((await discordMessagingActionRuntime.fetchChannelInfoDiscord(channelId, ctx.withOpts())).type)) return {
			...params.payload,
...
async function handleDiscordMessageSendAction(ctx) {
	switch (ctx.action) {
		case "sticker": {
			...
			const to = readToolStringParam(ctx.params, "to", { required: true });
			const content = readToolStringParam(ctx.params, "content");
			...
			return jsonResult({
				ok: true,
				result: await discordMessagingActionRuntime.sendStickerDiscord(to, stickerIds, ctx.withOpts({
					content,
		...
		case "sendMessage": {
			...
			const mediaUrl = readToolStringParam(ctx.params, "mediaUrl", { trim: false }) ?? readToolStringParam(ctx.params, "path", { trim: false }) ?? readToolStringParam(ctx.params, "filePath", { trim: false });
			...
			const filename = readToolStringParam(ctx.params, "filename");
			const replyTo = readToolStringParam(ctx.params, "replyTo");
			const threadName = readToolStringParam(ctx.params, "threadName");
			const sessionKey = readToolStringParam(ctx.params, "__sessionKey");
			const agentId = readToolStringParam(ctx.params, "__agentId");
			if (componentSpec) {
				...
				const result = await discordMessagingActionRuntime.sendDiscordComponentMessage(to, payload, {
					...ctx.withOpts(),
					silent,
		...
		case "threadCreate": {
			...
			const channelId = ctx.resolveChannelId();
			const name = readToolStringParam(ctx.params, "name", { required: true });
			const messageId = readToolStringParam(ctx.params, "messageId");
			const content = readToolStringParam(ctx.params, "content");
			const payload = {
				name,
				messageId,
		...
		case "threadList": {
			if (!ctx.isActionEnabled("threads")) throw new Error("Discord threads are disabled.");
			const guildId = readToolStringParam(ctx.params, "guildId", { required: true });
			const channelId = readToolStringParam(ctx.params, "channelId");
			const includeArchived = readBooleanParam(ctx.params, "includeArchived");
			const before = readToolStringParam(ctx.params, "before");
			const limit = readPositiveIntegerParam(ctx.params, "limit");
		...
		case "threadReply": {
			...
			const channelId = ctx.resolveChannelId();
			const content = readToolStringParam(ctx.params, "content", { required: true });
			const mediaUrl = readToolStringParam(ctx.params, "mediaUrl");
			const replyTo = readToolStringParam(ctx.params, "replyTo");
			return jsonResult({
				ok: true,
...
function readDiscordModerationCommand(action, params) {
	...
	return {
		action,
		guildId: readToolStringParam(params, "guildId", { required: true }),
		userId: readToolStringParam(params, "userId", { required: true }),
		durationMinutes: readNonNegativeIntegerParam(params, "durationMinutes"),
		until: readToolStringParam(params, "until"),
...
async function handleDiscordModerationAction(action, params, isActionEnabled, cfg) {
	...
	const senderUserId = readToolStringParam(params, "senderUserId");
...
async function handleDiscordPresenceAction(action, params, isActionEnabled, cfg) {
	...
	const accountId = readToolStringParam(params, "accountId") ?? resolveDefaultDiscordAccountId(cfg);
	const gateway = getGateway(accountId);
	if (!gateway) throw new Error(`Discord gateway not available for account "${accountId}". The bot may not be connected.`);
	if (!gateway.isConnected) throw new Error(`Discord gateway is not connected for account "${accountId}".`);
	const statusRaw = readToolStringParam(params, "status") ?? "online";
	if (!VALID_STATUSES.has(statusRaw)) throw new Error(`Invalid status "${statusRaw}". Must be one of: ${[...VALID_STATUSES].join(", ")}`);
	const status = statusRaw;
	const activityTypeRaw = readToolStringParam(params, "activityType");
	const activityName = readToolStringParam(params, "activityName");
	const activities = [];
	if (activityTypeRaw || activityName) {
		...
		if (typeNum === 1) {
			const url = readToolStringParam(params, "activityUrl");
			if (url) activity.url = url;
		}
		const state = readToolStringParam(params, "activityState");
...
async function handleDiscordAction(params, cfg, options) {
	...
	if (action === "setPresence") return await handleDiscordPresenceAction(action, params, isActionEnabled, cfg);
	throw new Error(`Unknown action: ${action}`);
}
//#endregion
