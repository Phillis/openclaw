...
function renderExistingSkillsSection(existingSkills) {
	...
	return [
		"",
		"Writable skills:",
		...shown.map((skill) => truncateUtf16Safe(`- ${skill.name}${skill.description ? ` — ${skill.description}` : ""}${skill.userAuthored ? " (user-authored)" : ""}`, EXPERIENCE_REVIEW_MAX_SKILL_LINE_CHARS)),
...
function renderUsedSkillsSection(usedSkills) {
	...
	const preference = "Prefer improving a used Workshop-owned workspace skill when it governs the learning.";
...
//#endregion
export { countSkillModelIterations as n, selectCurrentSkillTurnMessages as r, buildSkillExperienceReviewPrompt as t };
