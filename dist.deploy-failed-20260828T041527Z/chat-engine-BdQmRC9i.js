...
import { i as loadSystemAgentOverview } from "./overview-CefSkUmi.js";
import { o as prepareSystemAgentRunAdmission } from "./admitted-run-context-KQIZywud.js";
import { a as parseSystemAgentOperation, c as isSystemAgentSensitiveConfigValue, i as isPersistentSystemAgentOperation, l as redactSystemAgentConfigPath, n as describeSystemAgentPersistentOperation, s as isInvalidConfigSetOperation, t as executeSystemAgentOperation } from "./operations-Djud2UfI.js";
...
import { a as resolveSystemAgentVerifiedInferenceRoute, i as resolveSystemAgentExpectedAgentHarnessRuntimeArtifact } from "./verified-inference-BWEX8g0I.js";
import { t as approvalQuestion } from "./dialogue-CLPEitzt.js";
import { n as resolveOperatorApprovalDecision, r as resolvePendingOperatorProposal, t as classifySystemAgentApprovalText } from "./operator-approval-CIH8v6vM.js";
import { r as extractAgentRunText } from "./agent-run-result-DFovjOVm.js";
import { a as normalizeCliModel } from "./helpers-BJECcEJB.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./system-agent-tool-C-a9w0lT.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-B_eCZ6FD.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-fNRs635m.js")).runEmbeddedAgent)({
				...shared,
				preparedRunAdmission,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
...
var ChatTurnRouter = class {
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.options.classifyApproval ?? (await import("./approval-intent-BNbaTVZy.js")).classifySystemAgentApprovalIntent)({
			message: text,
	...
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.options.planWithAssistant ?? (await import("./assistant-BpPW76pw.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
...
var SystemAgentChatEngine = class {
	...
	*/
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-BpPW76pw.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
