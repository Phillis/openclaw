...
import { D as resolveExpiresAtMsFromDurationMs, n as MAX_TIMER_TIMEOUT_MS, o as asDateTimestampMs } from "./number-coercion-CLj0HTDM.js";
...
import { Mn as executeSqliteQueryTakeFirstSync, Nn as getNodeSqliteKysely, Xt as resolveOpenClawStateSqlitePath, d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, jn as executeSqliteQuerySync } from "./openclaw-state-db-kmBThqu6.js";
import { r as withOpenClawStateDatabaseReadOnly } from "./openclaw-state-db-readonly-KXgHmJVs.js";
import { c as normalizeToolPolicyName } from "./tool-policy-shared-DmpG3HvD.js";
import "./tool-policy-B1rvCc4B.js";
import { I as getAgentToolResultMiddlewareMatcherScope, L as listAgentToolResultMiddlewares } from "./loader-BcKpDiEM.js";
import { i as isPidDefinitelyDead } from "./pid-alive-BcyyC-CC.js";
import { g as getGlobalHookRunnerRegistry, r as hasGlobalHooks, s as getToolHookMatcherScope, v as mergePluginToolMatcherScopes } from "./hook-runner-global-CDBq1X4a.js";
import { B as getTrustedToolPolicyMatcherScope, H as requestDeferredPluginToolApproval, V as cancelDeferredPluginToolApproval, b as runBeforeToolCallHook, ct as PluginApprovalResolutions, y as hasBeforeToolCallPolicy } from "./agent-tools.before-tool-call-D89j2U3t.js";
import { l as payloadTextResult } from "./common-CI1GnPjt.js";
...
const MCP_APPROVAL_UNAVAILABLE_REASON = "MCP tool approval timed out (no operator connected). Approve in the Control UI, or set mcp.servers.<id>.codex.defaultToolsApprovalMode:\"approve\" for trusted servers.";
...
async function runNativeHookRelayPermissionRequest(params) {
	...
	try {
		...
		if (decision === "timed-out" && request.toolName.startsWith("mcp__")) return params.adapter.renderPermissionDecisionResponse("deny", MCP_APPROVAL_UNAVAILABLE_REASON);
	} catch (error) {
...
async function prepareNativeHookMutableFileBinding(request) {
	...
	const prepared = await prepareSystemRunMutableFileBinding({
		command: {
			kind: "shell",
			text: readNonEmptyStringPreservingWhitespace(request.toolInput.command) ?? ""
		},
		cwd: request.cwd
	});
...
function nativeHookRelayPermissionAllowAlwaysKey(params) {
	...
	hash.update("\0");
	hash.update(params.binding ? permissionRequestBindingFingerprint(params.binding) : "no-file-binding");
	return hash.digest("hex");
}
