...
import { d as normalizeStringEntriesLower, v as uniqueStrings } from "./string-normalization-e_fvmxMf.js";
...
import { C as resolveOAuthDir, w as resolveStateDir } from "./paths-BBSTUjD5.js";
...
import { S as tryResolveDefaultAgentId, a as listAgentIds, h as resolveDefaultAgentDir } from "./agent-scope-config-CUBiGmG3.js";
...
import { a as isPrimarySessionTranscriptFileName, n as formatSessionArchiveTimestamp } from "./artifacts-FzMa6c2e.js";
import { a as resolveSessionFilePathOptions, i as resolveSessionFilePathCore, l as resolveSessionTranscriptsDirForAgent, o as resolveSessionStorePathCore } from "./paths-DVAvlIOc.js";
import { t as decodeMountInfoPath } from "./mountinfo-path-BCOIljp0.js";
import { a as listPluginDoctorSessionRouteStateOwners } from "./doctor-contract-registry-Dbcs8AwY.js";
...
import { o as listConfiguredChannelIdsForReadOnlyScope } from "./channel-presence-policy-oJ8soWzX.js";
import { a as resolveSharedAuthStorePath, i as resolveSharedAuthStoreOwnership } from "./path-resolve-Bjd7UUgA.js";
import { l as resolveCanonicalMainSessionKey, n as canonicalizeMainSessionAlias } from "./main-session-CPkeRwvL.js";
import { d as loadExactSessionEntryReadOnly, p as loadSessionEntryReadOnly } from "./session-accessor.sqlite-entry-Ik-U-wpI.js";
...
import { At as applySessionEntryReplacements, J as scanDoctorSessionEntriesStrict, W as iterateDoctorSessionKeyBatches, kt as applySessionEntryLifecycleMutation } from "./session-accessor-fcDZuc2H.js";
import { c as resolveSessionStoreTargets } from "./targets-CSCF74bk.js";
...
import { n as formatSubagentRecoveryWedgedReason, r as isSubagentRecoveryWedgedEntry, t as clearWedgedSubagentRecoveryAbort } from "./subagent-recovery-state-1AUqfKvI.js";
import { n as isHeartbeatOkResponse, r as isHeartbeatUserMessage } from "./heartbeat-filter-6HwCh_sJ.js";
...
import { n as resolveHeartbeatDeliveryTarget } from "./targets-E-6YuwPm.js";
import "./selection-uTriTeN_.js";
import { t as note } from "./note-YH_0kY-3.js";
import { i as updateLegacySessionStore, t as loadLegacySessionStore } from "./state-migrations.legacy-session-store-jLfPw61O.js";
import { i as isSharedAuthStoreOwner } from "./agent-delete-safety-Cz9lIXnV.js";
import { n as clearTuiLastSessionPointers } from "./tui-last-session-B8OUMZ4i.js";
...
function describeHeartbeatSessionTargetIssues(cfg) {
	...
	for (const { agentId, heartbeat: heartbeatConfig } of resolveHeartbeatAgents(cfg)) {
		...
		const fix = ownerTarget ? `  Fix: set commands.ownerAllowFrom or a channel allowFrom to a direct-message owner, set heartbeat.target="none", or choose an explicit heartbeat target.` : `  Fix: point heartbeat.session at a session the agent actually owns, set heartbeat.target="none" to suppress delivery, or remove the heartbeat.session field to fall back to the agent main session.`;
		warnings.push([
...
function detectMacCloudSyncedStateDir(stateDir, deps) {
	...
	const resolvedStatePath = resolvePathThroughExistingAncestor(stateDir, deps?.resolveRealPath ?? tryResolveRealPath, path) ?? path.resolve(stateDir);
...
function detectStateIntegrityHealthIssues(cfg, params) {
	...
	const storePath = agentId ? resolveSessionStorePathCore(cfg.session?.store, { agentId }) : void 0;
...
async function noteStateIntegrity(cfg, prompter, configPath, options) {
	...
	const inspectAgentSessionIntegrity = async (target, inspectLegacyStore) => {
		...
		const isSessionKeyOccupied = (sessionKey) => sqliteSessionKeys.has(sessionKey) || legacyOrder.has(sessionKey);
		...
		const recent = [];
		const addRecent = (sessionKey, entry, order) => {
			recent.push({
				entry,
				order,
				sessionKey
			...
			recent.sort((left, right) => {
				...
				return (typeof right.entry.updatedAt === "number" ? right.entry.updatedAt : 0) - leftUpdated || left.order - right.order;
			});
			if (recent.length > 5) recent.pop();
		};
		const inspectMergedEntry = (sessionKey, entry, order) => {
		...
		if (mergedEntryCount > 0) {
			const recentTranscriptCandidates = recent.map(({ entry, sessionKey }) => [sessionKey, entry]).filter(([key]) => !isSlashRoutingSessionKey(key));
			const missing = recentTranscriptCandidates.filter(([key, entry]) => {
				...
				return !existsFile(resolveSessionFilePathCore(sessionId, entry, sessionPathOpts));
			});
			...
			if (wedgedSubagentSessions.length > 0) {
				...
				const visibleWedgedReasons = uniqueStrings(wedgedSubagentSessions.map(({ reason }) => reason)).slice(0, 2);
		...
		if (sqliteEntryCount === 0 && inspectLegacyStore && existsDir(sessionsDir)) {
			...
			for (const [, entry] of legacyEntries) {
				...
				try {
					referencedTranscriptPaths.add(resolveComparableTranscriptPath(resolveSessionFilePathCore(entry.sessionId, entry, sessionPathOpts)));
				} catch {}
			}
