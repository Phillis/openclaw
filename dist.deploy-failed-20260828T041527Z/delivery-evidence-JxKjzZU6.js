...
import { a as hasVisibleAgentPayload, t as collectMediaUrlsFromRecord } from "./message-visibility-B7uGWSom.js";
...
function hasVisibleOutboundDeliveryEvidence(result) {
	return hasVisibleCommittedMessagingToolDeliveryEvidence(result) || result.didSendViaMessagingTool === true && !hasGranularMessagingToolDeliveryEvidence(result) || hasAcceptedSessionSpawnEvidence(result.acceptedSessionSpawns) || hasPositiveNumber(result.successfulCronAdds);
}
...
function hasCommittedNonMessagingOutboundDeliveryEvidence(result) {
	return hasAcceptedSessionSpawnEvidence(result.acceptedSessionSpawns) || hasPositiveNumber(result.successfulCronAdds);
}
