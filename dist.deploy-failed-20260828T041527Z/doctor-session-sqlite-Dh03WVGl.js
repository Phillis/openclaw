...
import { C as tryResolveLegacyCompatibilityAgentId, S as tryResolveDefaultAgentId } from "./agent-scope-config-CUBiGmG3.js";
...
import { Dn as getCanonicalSqliteNamedIndexContracts, xt as resolveSqliteDatabaseFilePaths } from "./openclaw-state-db-kmBThqu6.js";
...
import { a as resolveStoredSessionOwnerAgentId } from "./session-store-key-DRF7yKG5.js";
import { i as closeOpenClawAgentDatabaseByPath, p as isOpenClawAgentDatabaseOpen, r as clearOpenClawAgentDatabaseOpenFailure } from "./openclaw-agent-db-BEQsKM0c.js";
import { L as ensureOpenClawAgentDatabasePermissions, O as OPENCLAW_AGENT_SCHEMA_SQL, r as migrateOpenClawAgentDatabaseForMaintenance, t as assertOpenClawAgentDatabaseForMaintenance } from "./openclaw-agent-db-maintenance-_0tYy-zT.js";
...
import { i as resolveUnsuffixedSqliteTargetFromSessionStorePath } from "./session-sqlite-target-10dvR_dO.js";
import { a as resolveAllAgentSessionStoreCandidateTargetsSync, c as resolveSessionStoreTargets, i as resolveAgentSessionStoreTargetsSync, o as resolveAllAgentSessionStoreTargetsSync } from "./targets-CSCF74bk.js";
import { n as normalizeLegacySessionEntryDelivery } from "./state-migrations.legacy-session-store-jLfPw61O.js";
import { n as importSqliteSessionRowsBatch } from "./session-accessor.sqlite-import-BXRu2nl2.js";
import { n as assertDoctorSqliteMaintenancePathsNotAliased, r as isDestructiveDoctorSessionSqliteMode } from "./doctor-sqlite-maintenance-lock-9ifiaibD.js";
import { c as readOnlySqliteValidationSnapshot, d as resolveTargetSqlitePath, f as scanReadOnlySqliteActiveTranscriptFiles, i as readOnlySqliteDbStats, l as readSqliteEntryCount, n as createTranscriptEventReader, t as countTranscriptEventsForPath, u as readTranscriptFingerprint } from "./doctor-session-sqlite-readers-BpdoXrJ0.js";
import { t as compactDoctorSqliteFile } from "./doctor-sqlite-compact-DynA2X6G.js";
import { i as sumDoctorSessionSqliteTargets, n as createDoctorSessionSqliteTotals, r as isSessionSqliteMigrationWarning, t as createDoctorSessionSqliteTargetReport } from "./doctor-session-sqlite-types-4f8oCqeR.js";
...
function resolveDoctorSessionSqliteTargets(params) {
	...
	if (params.allAgents) {
		const targets = filterLegacySessionStoreTargets(resolveAllAgentSessionStoreTargetsSync(params.cfg, { env: params.env }), params.mode);
...
function resolveLegacyTranscriptPath(target, entry) {
	...
	if (relocatedPath && fs.existsSync(relocatedPath)) return relocatedPath;
	return relocatedPath ? defaultPath : void 0;
}
...
function validateImportedRecordBeforeArchive(record, report, snapshot) {
	...
	if (sqliteEvents < result.events) report.issues.push({
...
function archiveImportedTranscripts(target, records, report, activeRun) {
	...
	for (const record of records) {
		...
		if (!fs.existsSync(record.transcriptPath)) continue;
		try {
			plannedArchives.push({
				moves: planImportedTranscriptArtifactsToArchive(target, record.sessionKey, record.transcriptPath, reservedArchivePaths),
				record
			});
		} catch (err) {
			report.issues.push({
	...
	recordPlannedMigrationMoves(activeRun, migrationTarget, plannedArchives.flatMap((archive) => archive.moves));
	const completedMoves = [];
	for (const { moves, record } of plannedArchives) try {
		const archivePaths = [];
		for (const move of moves) {
