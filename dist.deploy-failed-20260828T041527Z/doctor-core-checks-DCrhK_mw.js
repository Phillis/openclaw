...
import { i as resolveKnownModelRefMigrationTarget, n as collectCodexRuntimeCompatibilityWarnings, o as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-CehDs9fI.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-yEEGuNZz.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-CLpPRmJu.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-DI7O7d6q.js");
...
const legacyStateCheck = {
	...
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-BmV28Q4w.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-3q1UZrLY.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-BesLGelU.js");
...
function createModelReferenceCheck() {
	return {
		...
		source: "doctor",
		async detect(ctx) {
			const { inspectConfiguredModelReferences } = await import("./model-reference-validation-I0vVQSWv.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	defaultEnabled: false,
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-DD8u5-Oc.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-DD8u5-Oc.js");
...
const telegramGeneralTopicConversationsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectTelegramGeneralTopicConversationRepairs } = await import("./doctor-telegram-general-topic-conversations-BhXlVsns.js");
	...
	async repair(ctx) {
		const { repairTelegramGeneralTopicConversations } = await import("./doctor-telegram-general-topic-conversations-BhXlVsns.js");
		const effect = {
			kind: "state",
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-Cm81ybSz.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-Cm81ybSz.js");
...
const gatewayPlatformNotesCheck = {
	...
	async detect(ctx) {
		...
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-CHP5TbF5.js");
...
function createConvertedWorkflowChecks(deps) {
	return [
		...
		...isExperimentalClawsEnabled() ? [{
			...
			source: "doctor",
			async detect(ctx) {
				const [{ collectClawStateHealthFindings }, { listConfiguredMcpServers }] = await Promise.all([import("./doctor-CaZYuxv3.js"), import("./mcp-config-DFvgAb2Z.js")]);
