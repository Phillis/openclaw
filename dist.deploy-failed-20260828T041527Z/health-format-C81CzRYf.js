...
const formatHealthChannelLines = (summary, opts = {}) => {
	...
	for (const channelId of channelOrder) {
		...
		const activeSummaries = (accountMode === "all" ? Object.values(accountSummaries) : filteredSummaries ?? (channelSummary.accounts ? Object.values(accountSummaries) : [])).filter((account) => account.enabled !== false && account.configured !== false && account.linked !== false && account.statusState !== "disabled" && account.statusState !== "unconfigured");
		const selectedSummary = activeSummaries.find((account) => account.healthState && account.healthState !== "healthy" || account.statusState && account.statusState !== "linked" && account.statusState !== "configured") ?? filteredSummaries?.[0] ?? channelSummary;
		const botUsernames = activeSummaries.map((account) => {
