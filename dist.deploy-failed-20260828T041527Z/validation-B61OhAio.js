...
function matchesJsonType(value, type) {
	switch (type) {
		...
		case "object": return isObjectBackedRecord(value) && !Array.isArray(value);
...
function coerceWithJsonSchema(value, schema) {
	...
	if (schemaTypes.includes("object") && isObjectBackedRecord(nextValue) && !Array.isArray(nextValue)) applySchemaObjectCoercion(nextValue, schema);
...
function validateToolArguments(tool, toolCall) {
	...
	if (isJsonSchemaObject(tool.parameters)) {
		...
		if (coerced !== args) if (isObjectBackedRecord(args) && isObjectBackedRecord(coerced)) {
