...
import { s as peekSessionMcpRuntime } from "./agent-bundle-mcp-manager-api-1m62vMRl.js";
import "./agent-bundle-mcp-runtime-DlEFrKcw.js";
import { c as resolveMcpAppSandboxPort, i as getMcpAppViewLease, s as buildMcpAppSandboxPath } from "./mcp-ui-resource-DAkuvpb4.js";
import { i as readJsonBodyOrError, s as sendJson, y as respondPlainText } from "./http-common-Dy8Dj7pv.js";
import { i as classifyMcpAppStandalonePath, n as MCP_APP_STANDALONE_VIEW_PATH, t as MCP_APP_STANDALONE_PATH } from "./gateway-http-route-contracts-ByqHS7gV.js";
import { i as requireMcpAppInteraction, n as executeMcpAppOperation, r as parseMcpAppOperation, s as withMcpAppActiveView } from "./mcp-app-operations-C0pyoRlp.js";
...
function runStandaloneMcpAppHost(config) {
	...
	const request = async (method, params) => {
		const response = await fetch(config.viewPath, {
			...
			cache: "no-store",
			credentials: "omit",
			signal: payload?.operationTimeoutMs != null ? AbortSignal.timeout(payload.operationTimeoutMs) : void 0
		});
	...
	browser.addEventListener("message", (event) => {
		...
		handler(message.params ?? {}).then((result) => respond(message.id, result)).catch((error) => reject(message.id, -32e3, errorMessage(error, "MCP App operation timed out; try again")));
	});
	...
	try {
		return await withMcpAppActiveView(active, "read", async () => {
			...
			sendJsonRepresentation(req, res, 200, {
				...
				serverResources,
				...view.requestTimeoutMs !== void 0 ? { operationTimeoutMs: addTimerTimeoutGraceMs(view.requestTimeoutMs) } : {}
			});
			return true;
		});
	} catch (error) {
