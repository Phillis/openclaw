...
async function runRemoteGatewayInferenceOnboarding(target, runtime = defaultRuntime, deps = {}) {
	const callGateway = deps.callGateway ?? (await import("./call-DPYKD0iw.js")).callGatewayCli;
	const runGuidedOnboarding = deps.runGuidedOnboarding ?? (await import("./onboard-guided-DGUHhgpY.js")).runGuidedOnboarding;
	...
	const activate = async (params) => {
		...
		for (;;) {
			...
			if (activation.gatewayRestartRequired === true && remainingBeforeAttemptMs <= 0) break;
			try {
				verification = await request({
					method: "openclaw.setup.verify",
					payload: {},
					timeoutMs: activation.gatewayRestartRequired === true ? Math.min(GATEWAY_SETUP_VERIFY_TIMEOUT_MS, remainingBeforeAttemptMs) : GATEWAY_SETUP_VERIFY_TIMEOUT_MS
				});
				const retryableResult = activation.gatewayRestartRequired === true && !verification.ok && verification.status === "unavailable";
				const remainingMs = restartDeadline - Date.now();
				if (!retryableResult || remainingMs <= 0) break;
				await setTimeout(Math.min(retryDelayMs, remainingMs));
				retryDelayMs = Math.min(retryDelayMs * 2, 2e3);
			} catch (error) {
				const retryable = activation.gatewayRestartRequired === true && (isGatewayTransportError(error) || isGatewayClientRequestError(error) && error.retryable);
				const remainingMs = restartDeadline - Date.now();
				if (!retryable || remainingMs <= 0) throw error;
				const requestedDelay = isGatewayClientRequestError(error) ? error.retryAfterMs ?? retryDelayMs : retryDelayMs;
				await setTimeout(Math.min(requestedDelay, remainingMs));
				retryDelayMs = Math.min(retryDelayMs * 2, 2e3);
			}
		}
		if (!verification) throw new Error("Gateway did not finish restarting before inference verification.");
	...
	await runGuidedOnboarding({}, runtime, {
		...
		runSystemAgentChat: async () => {
			...
			await (deps.runTui ?? (await import("./tui-Bs69Gc0u.js")).runTui)({
				config: boundConfig,
				deliver: false,
