...
import { t as MarkdownItCallable } from "./markdown-it-BB9GnmiM.js";
import { d as tokenizeHtmlTags, r as markdownToIR } from "./construct-fallbacks-Dvy1yFH8.js";
...
import { i as normalizeMatrixResolvableTarget, t as isMatrixQualifiedUserId } from "./target-ids-DEUqtLsd.js";
import { r as buildMatrixReactionContent } from "./reaction-common-CTyQTnvd.js";
import { i as isStrictDirectRoom } from "./direct-room-CQh3MRKp.js";
import { a as EventType, c as MsgType, l as RelationType, n as persistMatrixDirectRoomMapping, s as MSC4357_LIVE_KEY, t as inspectMatrixDirectRooms } from "./direct-management-DeaumWfF.js";
import { O as textDecode, S as UINT32_LE, a as UnsupportedFileTypeError, i as InternalParserError, n as CouldNotDetermineFileTypeError } from "./BasicParser-Ce0bn6Mo.js";
import { a as fromBuffer, t as APEv2Parser } from "./APEv2Parser-ChXlOA9U.js";
import { t as require_src } from "./src-D0deKpII.js";
import { n as TrackType, r as TrackTypeValueToKeyMap } from "./types-Dw42w_cH.js";
import { l as toRatio, n as decodeString } from "./Util-BgxYFDuK.js";
import { c as TimestampFormat, i as LyricsContentType } from "./ID3v2Token-GRdoXaR9.js";
import { r as hasID3v1Header } from "./ID3v1Parser-Cqgsck_t.js";
import { createHash } from "node:crypto";
import { fileTypeFromBuffer } from "file-type";
//#region extensions/matrix/src/matrix/send/client.ts
const loadMatrixSendClientRuntime = createLazyRuntimeModule(() => import("./client-bootstrap-C7EM6oLY.js"));
...
function getTextContent(text) {
	...
	return normalizeOptionalString(text["m.text"] ?? text["org.matrix.msc1767.text"] ?? text.body) ?? "";
}
...
const tableParser = new MarkdownItCallable({
	html: false,
	linkify: false,
	typographer: false
});
...
const spoilerParser = new MarkdownItCallable({
	html: false,
	linkify: true,
	typographer: false
});
...
function maskEscapedMentions(markdown) {
	...
	while (idx < markdown.length) {
		...
		if (codeFenceLength === 0 && markdown[idx] === "\\" && markdown[idx + 1] === "@" && !metadataRanges.some((range) => idx >= range.start && idx < range.end)) {
			marker ??= createMatrixPrivateMarkers(markdown, "Matrix mention formatting exhausted its private marker pool").open;
			masked += marker;
			idx += 2;
			continue;
		}
...
function restoreEscapedMentions(text, marker, inCode = false) {
	return marker ? text.replaceAll(marker, inCode ? "\\@" : "@") : text;
}
function restoreEscapedMentionsInBlockTokens(tokens, marker) {
	if (!marker) return;
	for (const token of tokens) if ((token.type === "fence" || token.type === "code_block") && token.content) token.content = restoreEscapedMentions(token.content, marker, true);
}
...
function mutateInlineTokensWithMentions(params) {
	const nextChildren = [];
	const restoreMention = (text) => restoreEscapedMentions(text, params.escapedMentionMarker);
	...
	for (const child of params.children) {
		...
		if (child.type !== "text" || !child.content) {
			for (const nested of child.children ?? []) nested.content = restoreMention(nested.content);
			nextChildren.push(child);
			continue;
		}
		...
		for (const match of matches) {
			if (match.start > cursor) nextChildren.push(createTextToken(child, restoreMention(child.content.slice(cursor, match.start))));
			cursor = match.end;
			if (match.kind === "room") {
				roomMentioned = true;
		...
		if (cursor < child.content.length) nextChildren.push(createTextToken(child, restoreMention(child.content.slice(cursor))));
	}
	return {
...
const mpegParserLoader = {
	...
	async load() {
		return (await import("./MpegParser-Cubo7ADi.js")).MpegParser;
	}
};
//#endregion
...
const apeParserLoader = {
	...
	async load() {
		return (await import("./APEv2Parser-Uo2zzPxC.js")).APEv2Parser;
	}
};
//#endregion
...
const asfParserLoader = {
	...
	async load() {
		return (await import("./AsfParser-Dn3xHEo9.js")).AsfParser;
	}
};
//#endregion
...
const dsdiffParserLoader = {
	...
	async load() {
		return (await import("./DsdiffParser-BCeNMDsy.js")).DsdiffParser;
	}
};
//#endregion
...
const aiffParserLoader = {
	...
	async load() {
		return (await import("./AiffParser-Dbp5L5nK.js")).AIFFParser;
	}
};
//#endregion
...
const dsfParserLoader = {
	...
	mimeTypes: ["audio/dsf"],
	async load() {
		return (await import("./DsfParser-DailWeb2.js")).DsfParser;
	}
};
//#endregion
...
const flacParserLoader = {
	...
	async load() {
		return (await import("./FlacParser-CMY5iw3E.js")).FlacParser;
	}
};
//#endregion
...
const matroskaParserLoader = {
	...
	async load() {
		return (await import("./MatroskaParser-EfTy_prf.js")).MatroskaParser;
	}
};
//#endregion
...
const mp4ParserLoader = {
	...
	async load() {
		return (await import("./MP4Parser-4gIxAyZJ.js")).MP4Parser;
	}
};
//#endregion
...
const musepackParserLoader = {
	...
	async load() {
		return (await import("./MusepackParser-Cb8jHmm9.js")).MusepackParser;
	}
};
//#endregion
...
const oggParserLoader = {
	...
	async load() {
		return (await import("./OggParser-Bt-AZXaq.js")).OggParser;
	}
};
//#endregion
...
const wavpackParserLoader = {
	...
	async load() {
		return (await import("./WavPackParser-BOdDTARM.js")).WavPackParser;
	}
};
//#endregion
...
const riffParserLoader = {
	...
	async load() {
		return (await import("./WaveParser-D1ZqwDlK.js")).WaveParser;
	}
};
//#endregion
