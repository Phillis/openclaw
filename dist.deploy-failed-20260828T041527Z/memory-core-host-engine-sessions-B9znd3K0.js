...
import { o as statRegularFile, r as readRegularFile } from "./regular-file-Dwz6p59y.js";
...
import { f as parseUsageCountedSessionIdFromFileName, g as materializeSessionArchiveForRead, r as isCompactionCheckpointTranscriptFileName, s as isSessionArchiveArtifactName, u as isUsageCountedSessionTranscriptFileName } from "./artifacts-FzMa6c2e.js";
...
import { Bt as tableExists, Nn as getNodeSqliteKysely, ft as stripInboundMetadata, jn as executeSqliteQuerySync } from "./openclaw-state-db-kmBThqu6.js";
...
import { l as listSessionTranscriptInstances } from "./session-accessor.sqlite-entry-Ik-U-wpI.js";
...
import { r as isHeartbeatUserMessage } from "./heartbeat-filter-6HwCh_sJ.js";
import "./fs-utils-DgC06wMX.js";
import { n as retryTransientMemoryRead, r as hashText } from "./read-retry-oGrTS9IG.js";
import { t as readTranscriptContentRevisionSync } from "./openclaw-runtime-CjBbrdBh.js";
import { h as resolveTranscriptSessionKeyBySessionId, i as listSessionEntries, m as resolveStorePath, o as loadTranscriptEventsSync, u as readTranscriptStatsSync } from "./session-store-runtime-ZfR7yV2q.js";
import { i as isExecCompletionEvent } from "./heartbeat-events-filter-Cumrha5E.js";
import "./query-expansion-_C4ftD-k.js";
import { i as isDreamingNarrativeSessionStoreKey, n as extractAgentIdFromSessionPath, r as extractAgentIdFromSessionsDir } from "./openclaw-runtime-session-D_Vopbw1.js";
...
function isDreamingNarrativeGeneratedRecord(record) {
	...
	if (!record || typeof record !== "object" || Array.isArray(record)) return false;
	const candidate = record;
	if (hasDreamingNarrativeIdentity(candidate.runId) || hasDreamingNarrativeIdentity(candidate.sessionKey)) return true;
	const message = candidate.type === "message" ? asOptionalRecord(candidate.message) : void 0;
	if (message) {
		const metadata = asOptionalRecord(message["__openclaw"]);
		if ((message.role === "assistant" || message.role === "toolResult") && hasDreamingNarrativeIdentity(metadata?.runId)) return true;
	}
	if (!candidate.data || typeof candidate.data !== "object" || Array.isArray(candidate.data)) return false;
	const nested = candidate.data;
	return hasDreamingNarrativeIdentity(nested.runId) || hasDreamingNarrativeIdentity(nested.sessionKey);
}
...
function projectSessionMetadata(agentId, row, participantIds = []) {
	return {
		agentId,
...
function loadMemorySessionMetadata(params) {
	const result = withOpenClawAgentDatabaseReadOnly(({ db }) => {
		let query = getNodeSqliteKysely(db).selectFrom("session_windows").select(SESSION_METADATA_COLUMNS).where("session_id", "=", params.sessionId);
		if (params.sessionKey) query = query.where("session_key", "=", params.sessionKey);
		const row = executeSqliteQuerySync(db, query).rows[0];
		return row ? projectSessionMetadata(params.agentId, row) : void 0;
	}, params);
	return result.found ? result.value : void 0;
}
function readSessionArchives(db, selectors) {
	if (selectors.length === 0 || !tableExists(db, "session_transcript_archives")) return [];
	return executeSqliteQuerySync(db, getNodeSqliteKysely(db).selectFrom("session_transcript_archives").select([
		"archive_name",
		"session_id",
		"session_key",
		"created_at"
	]).where((expression) => expression.or([expression("session_id", "in", selectors), expression("session_key", "in", selectors)])).orderBy("created_at").orderBy("session_id")).rows.map((archive) => ({
...
function loadArchivedSessions(params) {
	const sessionIds = [...new Set(params.sessionIds)];
	if (sessionIds.length === 0) return [];
	const result = withOpenClawAgentDatabaseReadOnly(({ db }) => readSessionArchives(db, sessionIds), params);
...
function resolveMemorySessionTargets(params) {
	...
	const result = withOpenClawAgentDatabaseReadOnly(({ db }) => {
		const sessionDb = getNodeSqliteKysely(db);
		const hasParticipants = tableExists(db, "session_participants");
		if (!hasParticipants && sessionIds.length === 0 && hookSources.length === 0) return [];
		let query = sessionDb.selectFrom("session_windows").select(SESSION_METADATA_COLUMNS).where((expression) => expression.or([
			...sessionIds.length > 0 ? [expression("session_windows.session_id", "in", sessionIds), expression("session_windows.session_key", "in", sessionIds)] : [],
			...hookSources.length > 0 ? [expression("session_windows.hook_external_content_source", "in", hookSources)] : [],
			...participants.length > 0 && hasParticipants ? [expression.exists(expression.selectFrom("session_participants").select("session_key").whereRef("session_participants.session_key", "=", "session_windows.session_key").where("actor_id", "in", participants))] : []
		]));
		if (since !== void 0) query = query.where("session_windows.created_at", ">=", since);
		const rows = executeSqliteQuerySync(db, query.orderBy("session_windows.created_at").orderBy("session_windows.session_id")).rows;
		const knownRows = since === void 0 || sessionIds.length === 0 ? rows : executeSqliteQuerySync(db, sessionDb.selectFrom("session_windows").select(["session_id", "session_key"]).where((expression) => expression.or([expression("session_id", "in", sessionIds), expression("session_key", "in", sessionIds)]))).rows;
		...
		if (rows.length > 0 && hasParticipants) {
			...
			const participantRows = executeSqliteQuerySync(db, sessionDb.selectFrom("session_participants").select(["session_key", "actor_id"]).where("session_key", "in", keys).orderBy("session_key").orderBy("actor_id")).rows;
			for (const participant of participantRows) {
				const ids = participantIds.get(participant.session_key) ?? [];
				if (!ids.includes(participant.actor_id)) ids.push(participant.actor_id);
				participantIds.set(participant.session_key, ids);
			}
		}
		const targets = new Map(rows.map((row) => [row.session_id, projectSessionMetadata(params.agentId, row, participantIds.get(row.session_key))]));
	...
	for (const sessionId of sessionIds) if (!resolvedSelectors.has(sessionId)) targets.push({
		agentId: params.agentId,
		sessionId,
...
//#endregion
export { listSessionFilesForAgent as a, normalizeSessionTranscriptPathForComparison as c, resolveSessionIdentityForTranscriptFile as d, sessionPathForFile as f, listSessionTranscriptCorpusEntriesForAgent as h, buildSessionEntry as i, parseCanonicalSessionSyncTargetFromPath as l, statSessionEntrySync as m, loadMemorySessionMetadata as n, loadDreamingNarrativeTranscriptPathSetForAgent as o, sessionPathForSessionIdentity as p, resolveMemorySessionTargets as r, loadSessionTranscriptClassificationForAgent as s, loadArchivedSessions as t, resolveSessionFileForSyncTarget as u };
