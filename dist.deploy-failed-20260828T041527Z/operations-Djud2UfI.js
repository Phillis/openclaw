...
import { J as collectChannelSchemaMetadataCore, Y as collectPluginSchemaMetadataCore } from "./io-ClLVsBMp.js";
...
import { S as findWildcardHintMatch } from "./zod-schema-D4cp1qYW.js";
...
import { i as appendSystemAgentAuditEntry, r as SYSTEM_AGENT_AUDIT_STORE_LABEL } from "./audit-BSOqBbQY.js";
import { s as parseConfigSetValue } from "./config-cli-path-D0uGAN56.js";
import { i as isKernelOwnedChannelConfigKey, n as classifyConfigSchemaPathSegment, t as buildConfigSchemaCore } from "./schema-Wla-hUMt.js";
import { i as sameDefaultInferenceRoute, n as projectInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-B_O-SIuc.js";
import { r as isOpenClawTrustedPluginInstallSpec } from "./install-provenance-D_0AXLg6.js";
...
const loadOverviewModule$1 = async () => await import("./overview-BApMUp9e.js");
...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { scheduleSafeGatewayRestart } = await import("./restart-coordinator-B9F2i5-O.js");
		return scheduleSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-DLL4izhV.js");
	if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
	const listPlugins = deps?.listChannelSetupPlugins ?? (await import("./setup-registry-BAI4CCYS.js")).listChannelSetupPlugins;
	const resolveEntries = deps?.resolveChannelSetupEntries ?? (await import("./discovery-obnslX9j.js")).resolveChannelSetupEntries;
	const isConfigured = deps?.isChannelConfigured ?? (await import("./channel-configured-shared-C8i0pDNh.js")).isStaticallyChannelConfigured;
	const { shouldShowChannelInSetup } = await import("./discovery-obnslX9j.js");
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-ChElhCE_.js");
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-ChElhCE_.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-BzxYMfcc.js")).applySystemAgentSetup;
			const surface = ctx.deps?.setupSurface ?? "cli";
			const recovery = surface === "cli" ? await (await import("./setup-recovery-DIAXiFHd.js")).loadLocalSetupRecovery(operation.workspace) : void 0;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-model-selection-okj-b_gG.js");
...
async function isPluginBackingDefaultInferenceRoute(pluginId) {
	...
	const { resolveOwningPluginIdsForProviderRef } = await import("./providers-DOAndBK-.js");
...
async function executePluginInstall(operation, runtime, opts) {
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			await ctx.commit(async () => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-szLMsKiz.js");
...
const loadOverviewModule = async () => await import("./overview-BApMUp9e.js");
	...
	switch (operation.kind) {
		...
		case "plugin-list":
			await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
				const { runPluginsListCommand } = await import("./plugins-list-command-8HA0CQyH.js");
		...
		case "config-schema": {
			const { buildConfigSchemaCore, lookupConfigSchema } = await import("./schema-Bz23wq_0.js");
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-DHbxCHQI.js");
		...
		case "create-agent":
			...
			return await applyPersistentOperation({
				...
				opts,
				run: async (ctx) => {
					const createAgentForOperation = ctx.deps?.createAgent ?? (await import("./agent-create-cyrEu2ez.js")).createAgent;
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-DWv4NHBn.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-BKOtn7Gu.js");
			await statusCommand({ timeoutMs: 1e4 }, runtime);
			return { applied: false };
		}
		case "health": {
			const { healthCommand } = await import("./health-CK080YwI.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-Bs69Gc0u.js")).runTui)({
