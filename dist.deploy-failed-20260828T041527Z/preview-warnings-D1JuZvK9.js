...
import { r as listAgentEntries, s as resolveAgentConfig, t as AgentSelectionRequiredError } from "./agent-scope-config-CUBiGmG3.js";
...
import { n as isToolAllowedByPolicies, r as isToolAllowedByPolicyName } from "./tool-policy-match-DfCekeWz.js";
...
import { t as resolveDoctorPrimaryModelRef } from "./primary-model-ref-CEIbLtXL.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-DFYFwu4G.js"));
...
async function resolveDoctorChannelPreviewConfig(params) {
	const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] = await Promise.all([import("./command-secret-gateway-XKE_5DRH.js"), import("./command-secret-targets-BwcUnigT.js")]);
...
async function collectDoctorPreviewNotes(params) {
	...
	warnings.push(...await collectActiveToolSchemaProjectionWarnings({
		cfg: params.cfg,
		env,
		...params.runWithPluginMetadataSnapshot ? { runWithPluginMetadataSnapshot: params.runWithPluginMetadataSnapshot } : {}
	}));
	const channelPluginRuntime = await import("./channel-plugin-blockers-9KU5G4um.js");
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-C08p_iWK.js");
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-DO9gWCS2.js");
	...
	const { collectCodexRouteWarnings } = await import("./codex-route-warnings-CupOMUPu.js");
	...
	if (hasPluginConfig) {
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-DWSvUanO.js");
	...
	if (hasSubagentAllowlistConfig(params.cfg)) {
		const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } = await import("./stale-subagent-allowlist-DSta9Qp_.js");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-zIaedw5C.js");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-CQmZj4Xv.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-BRiQMCss.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-COHtH6ql.js");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-CvxDeSfB.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-YvpiMgdt.js");
	...
	const { collectStaleConfiguredAuthOrderWarnings } = await import("./stale-auth-order-C9QI2muc.js");
