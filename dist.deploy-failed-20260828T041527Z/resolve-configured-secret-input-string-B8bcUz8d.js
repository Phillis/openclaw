...
import { n as describeSecretResolutionOperatorDiagnostic, r as describeSecretResolutionOperatorRecovery } from "./resolve-errors-C2XRFrol.js";
...
async function resolveConfiguredSecretInputString(params) {
	...
	try {
		...
		const operatorDiagnostic = style === "detailed" ? describeSecretResolutionOperatorDiagnostic(error) : void 0;
		const operatorRecovery = style === "detailed" ? describeSecretResolutionOperatorRecovery(error) : void 0;
		...
		return { unresolvedRefReason: operatorDetail ? `${unresolvedReason} ${operatorDetail}.` : unresolvedReason };
	}
}
async function resolveConfiguredSecretRefOnlyInputString(params) {
	if (!resolveConfigSecretRef({
		config: params.config,
		path: params.path,
		value: params.value,
		defaults: params.config.secrets?.defaults
