...
import { d as isMessagingToolSendAction, p as isPluginNativeMessagingTool } from "./tool-loop-detection-K29Fi2y0.js";
import { o as readEmbeddedMessageDeliveryFact } from "./embedded-agent-message-delivery-Dwtqwdl4.js";
import { r as isDeliveredMessagingToolResult, t as hasPluginMessagingDeliveryId } from "./embedded-agent-message-tool-source-reply-sK3rstcH.js";
...
function buildDeliveredMessagingFailureFallback(event, result) {
	...
	const delivered = deliveryFact ? deliveryFact.status === "settled" : isPluginNativeMessagingTool(event.toolName) && isDeliveredMessagingToolResult({
...
function createAgentToolResultMiddlewareRunner(ctx, handlers) {
	...
	const resolvedHandlersLoader = createLazyPromiseLoader(async () => {
		const { loadAgentToolResultMiddlewaresForRuntime } = await import("./agent-tool-result-middleware-loader-CJ5z2kZv.js");
