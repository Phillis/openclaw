...
import { u as resolveProfileUnusableUntilForDisplay } from "./usage-wmbnTa19.js";
import { r as resolvePersistedSessionRuntimeId } from "./session-runtime-compat-CuMcNwkW.js";
...
import { N as isCodexFastServiceTier, _ as CODEX_PLUGINS_MARKETPLACE_NAME, i as resolveCodexAppServerRuntimeOptions } from "./config-Cup3m5Mg.js";
import { Dt as isJsonObject, bt as resolveCodexAppServerFallbackApiKeyCacheKey, c as getLeasedSharedCodexAppServerClient, gt as resolveCodexAppServerAuthAccountCacheKey, h as releaseLeasedSharedCodexAppServerClient, rt as hasCodexAppServerLiveThread, tt as consumeCodexAppServerLiveThread, vt as resolveCodexAppServerAuthProfileIdForAgent } from "./shared-client-DsH0bBjk.js";
import { _ as sessionBindingIdentity, i as bindingStoreKey, m as reclaimCurrentCodexSessionGeneration, o as createCodexSessionGenerationSupersededError, r as assertCodexBindingMayBeReplaced, u as normalizeCodexAppServerBindingModelProvider } from "./session-binding-BlplpJ9G.js";
import { a as closeCodexStartupClientBestEffort } from "./attempt-client-cleanup-CBrsZNhS.js";
import { o as assertCodexThreadResumeResponse, r as assertCodexThreadAcceptsDirectInput } from "./protocol-validators-CpTKO3aJ.js";
import { t as listAllCodexAppServerModels } from "./models-C08KodZj.js";
import { o as defaultCodexAppInventoryCache, r as buildCodexPluginAppCacheKey } from "./plugin-app-cache-key-BEgBHcyj.js";
import { Ct as refreshCodexPluginRuntimeState, Tt as isOpenAiCuratedMarketplaceName } from "./thread-lifecycle-Ctc1w7N1.js";
import "./incognito-session-KJUXvrtm.js";
import { o as resolveCodexNativeExecutionBlock, s as resolveCodexNativeSandboxBlock } from "./request-D5ZqL_4v.js";
...
import { s as summarizeCodexAccountUsage } from "./rate-limits-DR7azd8a.js";
import { a as formatComputerUseStatus, c as formatSkills, i as formatCodexStatus, l as formatThreads, n as formatAccount, o as formatList, r as formatCodexDisplayText, s as formatModels, t as buildHelp } from "./command-formatters-BO9Vuy3O.js";
import { _ as hasCodexAdminScope, a as parseCodexPermissionsModeArg, c as setCodexConversationModel, d as stopCodexConversationTurn, g as canMutateCodexHost, h as CODEX_NATIVE_EXECUTION_AUTH_ERROR, i as parseCodexFastModeArg, l as setCodexConversationPermissions, m as CODEX_HOST_INSPECTION_AUTH_ERROR, n as parseCodexPluginMarketplaceId, o as readCodexConversationActiveTurn, p as CODEX_FULL_PERMISSIONS_AUTH_ERROR, r as formatPermissionsMode, s as setCodexConversationFastMode, t as discoverCodexMarketplacePlugins, u as steerCodexConversationTurn } from "./plugin-marketplace-discovery-D7YRUjFI.js";
import { i as readCodexConversationBindingData, n as createCodexConversationBindingData, o as resolveCodexDefaultWorkspaceDir, t as createCodexCliNodeConversationBindingData } from "./conversation-binding-data-DnmF-CZM.js";
import { n as resolveCodexBindingAppServerConnection } from "./binding-connection-D9kC-bwG.js";
import { i as formatCodexCliSessions } from "./node-cli-sessions-BW4aNinn.js";
import { i as safeCodexControlRequest, n as readCodexStatusProbes, r as requestOptions, t as codexControlRequest } from "./command-rpc-B_kLrckZ.js";
import { i as readCodexComputerUseStatus, r as installCodexComputerUse, t as defaultCodexPluginMetadataCache } from "./plugin-metadata-cache-QEUvWc1l.js";
		...
		run: async () => await deps.bindingStore.withLease(identity, async () => {
			...
			const commitResumedThread = async (value, client, { authProfileId, assertCurrent }) => {
				...
				try {
					...
					if (bindingBeforeCommit && !sameOwner) await releaseCodexAppServerBindingSubscription(bindingBeforeCommit);
					assertCurrent();
...
async function setConversationModel(deps, ctx, pluginConfig, args) {
	...
	if (!normalized) {
		const currentSession = ctx.sessionId && ctx.sessionKey ? getSessionEntry({
			storePath: resolveStorePath(ctx.config.session?.store, { agentId: target.agentId }),
			sessionKey: ctx.sessionKey,
			hydrateSkillPromptRefs: false,
			readConsistency: "latest"
		}) : void 0;
		const selectedModel = currentSession && currentSession.sessionId === ctx.sessionId ? currentSession.modelOverride ?? currentSession.model : void 0;
		const binding = await deps.bindingStore.read(target.identity);
...
async function startThreadAction(deps, ctx, pluginConfig, kind, args) {
	...
	if (kind === "compact") {
		...
		if (target.identity.kind === "conversation") {
			if (!isSameCodexAppServerThreadOwner(binding, ctx.sessionId ? await deps.bindingStore.read(sessionBindingIdentity({
				sessionId: ctx.sessionId,
				sessionKey: ctx.sessionKey,
				agentId: sessionTarget.agentId,
				config: ctx.config
			})) : void 0)) return "Codex compaction is unavailable because the conversation-bound thread differs from the current session binding. Resume that thread into the session first.";
		}
	...
	await deps.codexControlRequest(pluginConfig, CODEX_CONTROL_METHODS.review, {
		threadId: binding.threadId,
...
async function handleCodexPluginsSubcommand(ctx, rest, io, runtime) {
	...
	if (normalized === "available") {
		if (args.length > 0) return { text: "Usage: /codex plugins available" };
...
async function installCodexPlugin(requestedId, io, runtime) {
	...
	if (!requested) return { text: "Invalid plugin identifier. Use /codex plugins install <plugin>@<marketplace> with ASCII letters, digits, underscores, or hyphens." };
	let plugin;
	let workspaceDir;
	try {
		workspaceDir = await runtime.workspaceDir();
	...
	if (appsNeedingAuth.length > 0) {
		const apps = appsNeedingAuth.map((app) => formatCodexDisplayText(app.name)).slice(0, 5).join(", ");
		return { text: `${formatCodexDisplayText(requestedId)} was installed and authorized, but ${apps} still require connector authentication. Complete sign-in before using those apps.${refreshWarning} ${POLICY_REFRESH_HINT}` };
	}
	const status = alreadyInstalled ? "was already installed in Codex and is now authorized" : "was installed and authorized";
	return { text: `${formatCodexDisplayText(requestedId)} ${status}.${refreshWarning} ${POLICY_REFRESH_HINT}` };
}
...
function formatAvailablePlugins(plugins, warnings) {
	if (plugins.length === 0) return ["No Codex plugins were discovered for the current workspace.", ...warnings.map((warning) => `Warning: ${formatCodexDisplayText(warning)}`)].join("\n");
	return [
		...
		...plugins.slice(0, 30).map((plugin) => {
			...
			const description = plugin.description ? ` - ${formatCodexDisplayText(plugin.description)}` : "";
