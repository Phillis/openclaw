...
import { n as unscopedPackageName } from "./install-safe-path-DQTxRazZ.js";
import { c as resolveUserPath } from "./home-dir-BFvskzn8.js";
import "./utils-Bw16L5tB.js";
import { t as installPluginFromGitSpec } from "./git-install-BPOOTAxx.js";
import { o as resolvePackageExtensionEntries, r as loadPluginManifest } from "./manifest-DFeZvDdx.js";
import { o as resetPluginSlotsToDefaults } from "./slots-CQdAEuat.js";
...
import { a as isPrereleaseResolutionAllowed, i as isOpenClawOrgNpmSpec, o as isPrereleaseSemverVersion, r as isExactSemverVersion, s as parseRegistryNpmSpec } from "./npm-registry-spec-BdgyvSs0.js";
import { a as resolveDefaultPluginExtensionsDir, c as resolvePluginInstallDir } from "./install-paths-DllFtsSG.js";
import { n as satisfiesPluginApiRange, t as resolvePackagePluginApiRange } from "./package-compat-BQXdZhrB.js";
import { r as validatePackageExtensionEntriesForInstall } from "./package-entry-resolution-ZXIkJGCl.js";
import { a as resolveTrustedSourceLinkedOfficialNpmInstall, o as resolveTrustedSourceLinkedOfficialNpmSpec, r as resolveTrustedSourceLinkedOfficialClawHubInstall, t as hasOfficialNpmIdReplacement } from "./official-external-install-records-DOxgmTy-.js";
import { t as parseClawHubPluginSpec } from "./clawhub-spec-Er3Np6VI.js";
import { i as getOfficialExternalPluginCatalogEntry, v as resolveOfficialExternalPluginInstall } from "./official-external-plugin-catalog-C1KgYx9P.js";
...
import { n as createNpmMetadataEnv, o as resolveNpmSpecMetadata } from "./install-source-utils-D2m0UUwS.js";
...
import { a as resolveBundledPluginSources } from "./bundled-sources-8oSxLSsI.js";
import { a as diffDeclaredSurfaceWidening, c as resolveAcceptedSurfaceCurrent, f as resolvePluginInstallRecordIntegrity, n as capturePluginCapabilityConsentHandlerErrors, r as computeDeclaredSurfaceHash, t as buildPluginCapabilityConsentReview, u as resolvePluginArtifactDeclaredSurface } from "./capability-consent-WudatxYT.js";
import { B as isUnavailableNpmTarget, J as resolvePluginInstallTransactionSink, U as copyPluginInstallTransactionRequest, V as attachPluginInstallOwnerMigrations, Y as settlePluginInstallTransactions, q as resolvePluginInstallTransaction, z as PLUGIN_INSTALL_ERROR_CODE } from "./install-managed-npm-state-BKmVpI9X.js";
import { a as reconcileRegisteredOpenClawHostLinks, t as auditDeclaredOpenClawHostDependency } from "./plugin-peer-link-CNPdFqM4.js";
import { r as installPluginFromNpmSpec } from "./install-DJ6ueg-H.js";
import { n as isUnavailableClawHubTarget, t as CLAWHUB_INSTALL_ERROR_CODE } from "./clawhub-error-codes-Bqqw9uh0.js";
import { t as installPluginFromClawHub } from "./clawhub-wnJByH0c.js";
...
import { t as installPluginFromMarketplace } from "./marketplace-CYY05lla.js";
import { a as readInstalledPackageVersion, i as readInstalledPackageManifest, n as expectedIntegrityForUpdate, r as isPackageVersionDowngrade, t as comparePackageUpdateVersions } from "./package-update-utils-DjAfvLGS.js";
...
function buildPluginUpdateVersionOutcome(params) {
	...
	return {
		...
		message: unchanged ? `${params.pluginId} already at ${currentLabel}.${params.channelFallbackSuffix}` : `${verb} ${params.pluginId}: ${currentLabel} -> ${params.nextVersion ?? "unknown"}.${params.channelFallbackSuffix}`,
...
async function runPluginUpdateAttempt(params) {
	...
	if (!result.ok && params.record.source === "npm" && params.npmSpecs?.fallbackSpec && result.code !== PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_BLOCKED && result.code !== PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED) {
		...
		result = await installNpmSpec(installParams({
...
function preparePluginUpdateCapabilityConsent(params) {
	let previousDeclared;
	try {
		previousDeclared = resolvePluginArtifactDeclaredSurface(params.installPath, process.env, { config: params.config });
	} catch {}
	let acceptedSurface;
	let artifactReviewed = false;
	return {
		onBeforePluginArtifactCommit: async ({ stagedArtifactDir }) => {
			acceptedSurface = void 0;
			artifactReviewed = false;
			const artifactContext = {
				config: params.config,
				currentArtifactDir: params.installPath
			};
			const declared = resolvePluginArtifactDeclaredSurface(stagedArtifactDir, process.env, artifactContext);
			artifactReviewed = true;
			const { widened, hasWidening } = previousDeclared ? diffDeclaredSurfaceWidening(previousDeclared, declared) : {
				widened: void 0,
				hasWidening: false
			};
			const priorAcceptanceCurrent = previousDeclared !== void 0 && resolveAcceptedSurfaceCurrent(params.record, previousDeclared);
			const priorIntegrity = resolvePluginInstallRecordIntegrity(params.record);
			const enabled = !params.packagePluginIds?.length || params.packagePluginIds.some((ownedPluginId) => resolveEffectiveEnableState({
				id: ownedPluginId,
				origin: "global",
				config: normalizePluginsConfig(params.config.plugins),
				rootConfig: params.config
			}).enabled);
			if ((!previousDeclared || hasWidening || params.record.acceptedSurface !== void 0 && (!priorAcceptanceCurrent || !priorIntegrity)) && enabled) {
				...
				const packageName = typeof packageManifest?.name === "string" ? packageManifest.name : void 0;
				const packageVersion = typeof packageManifest?.version === "string" ? packageManifest.version : void 0;
				const manifest = {
					...loadedManifest.ok ? loadedManifest.manifest : {},
					name: (loadedManifest.ok ? loadedManifest.manifest.name : void 0) ?? packageName ?? params.pluginId,
					version: (loadedManifest.ok ? loadedManifest.manifest.version : void 0) ?? packageVersion
				};
				const { integrity: _previousIntegrity, npmIntegrity: _previousNpmIntegrity, clawpackSha256: _previousClawpackIntegrity, gitCommit: _previousGitCommit, acceptedSurface: _previousAcceptedSurface, acceptedSurfaceHash: _previousAcceptedSurfaceHash, acceptedSurfaceAt: _previousAcceptedSurfaceAt, acceptedSurfaceIntegrity: _previousAcceptedSurfaceIntegrity, ...previousRecordWithoutIntegrity } = params.record;
				...
				const finalDeclared = resolvePluginArtifactDeclaredSurface(stagedArtifactDir, process.env, artifactContext);
				if (acknowledgment?.reviewToken !== computeDeclaredSurfaceHash(finalDeclared)) throw new ManagedPluginLifecycleError(`Plugin "${params.pluginId}" requires capability consent; rerun with --accept-capabilities.`, { capabilityConsent: {
					pluginId: params.pluginId,
					reviewToken: computeDeclaredSurfaceHash(finalDeclared),
					...previousDeclared ? { widened: diffDeclaredSurfaceWidening(previousDeclared, finalDeclared).widened } : {}
				} });
				acceptedSurface = finalDeclared;
				return;
			}
			if (!hasWidening && priorAcceptanceCurrent && priorIntegrity) acceptedSurface = declared;
		},
		acceptInstallRecord: (record) => {
			if (!artifactReviewed) throw new Error(`Plugin "${params.pluginId}" update did not review the staged artifact capabilities.`);
			if (!acceptedSurface) return record;
...
function recordPluginUpdateFailure(params) {
	...
	if (params.disableOnFailure && !params.dryRun && !preserveInstalledPayload) {
		const message = `Disabled "${params.pluginId}" after plugin update failure; OpenClaw will continue without it. ` + params.message;
...
async function finalizePluginUpdateSummary(params) {
	...
	if (params.ranNpmInstaller) try {
		changed = await repairOpenClawPeerLinksForNpmInstalls({
			config: params.config,
			logger: params.logger
		}) || changed;
	} catch (error) {
...
async function reconcileUnchangedUpdate(params) {
	const newerExactPinnedDefaultLine = !params.hasSpecOverride && !params.hasOfficialNpmSpec ? await resolveNewerExactPinnedNpmDefaultLine({
		currentVersion: params.currentVersion,
		effectiveSpec: params.effectiveSpec,
	...
	return {
		...
		outcome: {
			...
			message: newerExactPinnedDefaultLine && params.effectiveSpec ? formatNewerExactPinnedNpmDefaultLineMessage({
				pluginId: params.pluginId,
				effectiveSpec: params.effectiveSpec,
				currentVersion: params.currentVersion,
				newer: newerExactPinnedDefaultLine
...
async function updateNpmInstalledPlugins(params) {
	...
	for (const pluginId of targets) {
		...
		if (replacementPluginId && replacementPluginId !== pluginId && Object.hasOwn(installs, replacementPluginId)) {
			outcomes.push({
				pluginId,
				status: "error",
		...
		const npmSpecOverride = params.specOverrides?.[pluginId] ?? (replacementPluginId ? trustedOfficialNpmSpec : void 0);
		...
		const preserveNpmRecordIntent = record.source === "npm" && npmSpecs?.installSpec !== npmSpecs?.recordSpec && updateChannel === "extended-stable";
		...
		if (resultSource === "npm") {
			...
			next = recordPluginInstall(usedOfficialNpmFallback ? withoutPluginInstallRecord(next, resolvedPluginId) : next, capabilityConsent.acceptInstallRecord({
				...
				spec: resolveNpmInstallRecordSpec({
					...
					pinResolvedRegistrySpec: params.syncOfficialPluginInstalls && trustedSourceLinkedOfficialInstall && !preserveNpmRecordIntent || usedOfficialNpmFallback && updateChannel !== "extended-stable"
				}),
...
async function syncPluginsForUpdateChannel(params) {
	...
	if (params.channel === "dev") for (const [pluginId, record] of Object.entries(installs)) {
		...
		for (const bridge of bridges) {
			...
			const bridgeNpmIntegrity = effectiveNpmSpec === npmSpec ? bridge.expectedIntegrity?.trim() : void 0;
		...
		for (const [pluginId, record] of Object.entries(installs)) {
			...
			loadHelpers.addPath(bundledInfo.localPath);
			if (record.source === "path" && userPathsEqual(record.sourcePath, bundledInfo.localPath, env) && userPathsEqual(record.installPath, bundledInfo.localPath, env)) continue;
			next = recordPluginInstall(next, {
				pluginId,
				source: "path",
				sourcePath: bundledInfo.localPath,
