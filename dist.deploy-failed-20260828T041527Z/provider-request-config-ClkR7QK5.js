...
import { o as assertSecretInputResolved } from "./types.secrets-Bre8L6Ts.js";
import { r as resolveProviderRequestCapabilities } from "./provider-attribution-CNkOWY2x.js";
...
function applyPreparedRuntimeAuthToModel(model, preparedAuth) {
	...
	return routeFacts ? attachModelProviderRequestRouteFacts(next, routeFacts.providerMetadataOwners) : next;
}
...
function resolveProviderRequestPolicyConfig(params) {
	...
	const capabilities = params.routeFacts?.capabilities ?? resolveProviderRequestCapabilities({
		...policyInput,
	...
	const protectedAttributionKeys = new Set(Object.keys(capabilities.attributionHeaders ?? {}).map((key) => normalizeLowercaseStringOrEmpty(key)));
	...
	return {
		...
		trustConfiguredBaseUrlOrigin: !privateNetworkAccess.explicitlyDenied && (capabilities.endpointClass === "custom" || capabilities.endpointClass === "local")
	};
}
...
const MODEL_PROVIDER_REQUEST_ROUTE_FACTS_SYMBOL = Symbol.for("openclaw.modelProviderRequestRouteFacts");
...
function attachModelProviderRequestRouteFacts(model, providerMetadataOwners) {
	if (!providerMetadataOwners || !model.provider) return model;
	const next = { ...model };
