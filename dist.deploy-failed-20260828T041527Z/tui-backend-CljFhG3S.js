...
import { i as loadSystemAgentOverview, r as formatSystemAgentStartupMessage } from "./overview-CefSkUmi.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-BnR7bfUE.js";
import { t as executeSystemAgentOperation } from "./operations-Djud2UfI.js";
...
import { o as resolveSystemAgentVerifiedInferenceState } from "./verified-inference-BWEX8g0I.js";
import { t as SystemAgentChatEngine } from "./chat-engine-BdQmRC9i.js";
import { randomUUID } from "node:crypto";
//#region src/system-agent/tui-backend.ts
async function loadHostedSetupForTui() {
	const [{ createClackPrompter }, hostedSetup] = await Promise.all([import("./clack-prompter-BzttpXIv.js"), import("./hosted-setup.runtime.js")]);
	return {
		createClackPrompter,
		hostedSetup
	};
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	await (opts.runChannelsAdd ?? (await import("./add-NeMTEpeA.js")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
...
async function runSystemAgentTui(opts, runtime) {
	...
	for (;;) {
		...
		const runTui = boundOpts.runTui ?? (await import("./tui-Bs69Gc0u.js")).runTui;
		try {
			await runTui({
				local: true,
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (verified) {
			...
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-CiP0KTjK.js"), import("./model-thinking-default-B9r48KTn.js")]);
