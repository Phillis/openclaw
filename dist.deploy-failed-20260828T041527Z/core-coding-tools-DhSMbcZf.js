...
import { r as openRootFileFollowingParents } from "./boundary-file-read-h_n3tTfV.js";
import { r as formatErrorMessage } from "./errors-Ccx0R-_Z.js";
import "./agent-scope-DigoIwHb.js";
import { s as resolveAgentConfig } from "./agent-scope-config-CUBiGmG3.js";
import { d as PROCESS_TOOL_DISPLAY_SUMMARY } from "./tool-catalog-DKzjKSZr.js";
import "./path-alias-guards-C2hsuc07.js";
import { n as assertSandboxPath } from "./sandbox-paths-C7Hkb46-.js";
import { n as resolvePathFromInput, r as toRelativeSandboxPath, t as preserveAtPrefixedRelativePath } from "./path-policy-DK2wTBdY.js";
import { l as rewrapToolWithBeforeToolCallHook, u as wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call-D89j2U3t.js";
import { G as hasOnlyCrlfLineEndings, H as withFileMutationQueue, K as normalizeToLF, U as withFileMutationQueues, W as decodeUtf8File, k as createReadTool, q as restoreLineEndings } from "./sessions-BLpYW515.js";
import { T as isToolWrappedWithBeforeToolCallHook, c as bindAgentToolActionDescriptor, u as copyAgentToolMetadata } from "./gateway-caller-context-DNtidJOJ.js";
import { c as resolveReadOnlyWorkspaceSkillMounts } from "./workspace-mounts-DBv2Eyoj.js";
import { t as buildSandboxFsMounts } from "./fs-paths-DNCksiBH.js";
...
import { t as applyToolAvailabilityDescriptions } from "./agent-tools.deferred-followup-BVU3cm9j.js";
import { a as createSandboxedReadTool, c as resolveAdaptiveReadMaxBytes, d as wrapToolWorkspaceRootGuard, f as wrapToolWorkspaceRootGuardWithOptions, i as createSandboxedEditTool, l as wrapReadToolWithSkillContent, m as withMemoryWriteProvenance, n as createHostWorkspaceWriteTool, o as createSandboxedWriteTool, r as createOpenClawReadTool, t as createHostWorkspaceEditTool } from "./agent-tools.read-B3XWmIod.js";
import { t as resolveExecCommandHighlighting } from "./exec-command-highlighting-BnagRpCO.js";
import { i as resolveMergedSafeBinProfileFixtures } from "./exec-safe-bin-runtime-policy-BVSOxLTx.js";
import { i as processSchema, n as execSchema, t as execCompletionSchema } from "./bash-tools.schemas-B7Xq1jzd.js";
...
function finalizeAgentTools(options) {
	...
	const finalized = applyToolAvailabilityDescriptions(withAbort, { agentId: options.agentId });
...
function resolvePatchFileOps(options) {
	...
	const rootPromise = root(containmentRoot);
	const toCanonicalMutationRelative = async (filePath, pathOptions) => {
		...
		return toRelativeSandboxPath(await fs$1.realpath(containmentRoot).catch(() => containmentRoot), canonicalAbsolute, pathOptions);
	};
	return withPatchMemoryWriteProvenance({
		observer: options.memoryWriteProvenance,
		operations: {
...
async function assertPatchParentPath(rawFilePath, options) {
...
async function resolvePatchPath(rawFilePath, options, aliasPolicy = PATH_ALIAS_POLICIES.strict) {
	if (options.sandbox) {
		const filePath = await preserveAtPrefixedRelativePath(rawFilePath, options.cwd, options.sandbox.bridge);
...
const bashToolsModuleLoader$1 = createLazyImportLoader(() => import("./bash-tools-CDPygDUk.js"));
...
const bashToolsModuleLoader = createLazyImportLoader(() => import("./bash-tools-CDPygDUk.js"));
...
function sandboxReadMounts(sandbox) {
	const mounts = buildSandboxFsMounts(sandbox).filter((mount) => mount.source !== "workspace").map((mount) => ({
...
function guardHostWorkspaceTool(tool, options) {
	return path.resolve(options.containmentRoot) === path.resolve(options.codingRoot) ? wrapToolWorkspaceRootGuard(tool, options.codingRoot) : wrapToolWorkspaceRootGuardWithOptions(tool, options.containmentRoot, { resolutionCwd: options.codingRoot });
}
...
function createCoreCodingTools(options) {
	...
	if (sandboxRoot && !sandboxFsBridge && (options.includeBaseCodingTools || options.includeShellTools)) throw new Error("Sandbox filesystem bridge is unavailable.");
	...
	if (options.includeBaseCodingTools) {
		...
		if (baseToolNames.has("read")) {
			...
			}) : createOpenClawReadTool((options.baseToolFactories?.createReadTool ?? createReadTool)(options.codingRoot, {
			...
			const guarded = options.workspaceOnly ? wrapToolWorkspaceRootGuardWithOptions(wrapped, sandboxRoot ?? options.containmentRoot, sandboxRoot ? {
