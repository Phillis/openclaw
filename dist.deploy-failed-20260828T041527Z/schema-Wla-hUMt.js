...
import { a as GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA, t as CHANNEL_IDS } from "./ids-Cgp0iV_A.js";
import { n as VERSION } from "./version-CkBmshxX.js";
import { t as isSensitiveConfigPath } from "./sensitive-paths-uMRji-4g.js";
import { C as schemaHasChildren, S as findWildcardHintMatch, b as asSchemaObject, d as DESKTOP_FIELD_HELP, f as DESKTOP_FIELD_LABELS, m as CLOUD_WORKER_FIELD_LABELS, n as projectTelemetryFieldMetadata, p as CLOUD_WORKER_FIELD_HELP, t as OpenClawSchema, x as cloneSchema } from "./zod-schema-D4cp1qYW.js";
...
const AGENT_FIELD_HELP = {
	...
	"ui.prefs.accent": "User-selected Control UI accent color in #RRGGBB format. Overrides ui.seamColor; clear it to restore the configured seam color or theme default.",
...
const AUTOMATION_FIELD_HELP = {
	...
	"session.rotation": "Bounded-channel controls that rotate long-lived channel-peer conversation sessions and cap shared-main context. When a trigger is configured, the current channel session is archived (rotation, lossless) and a fresh `:rK` session starts; `agent:<id>:main` is protected and instead applies the ceiling. Omit this block for today's unbounded session behavior.",
	"session.rotation.maxTurns": "Rotate a channel-peer session after this many turns (admission-bumped). The old session is archived by rotation and new turns route to `base:rK`; e.g. 120 keeps admission in the `fits` majority.",
	"session.rotation.maxAgeHours": "Rotate a channel-peer session once it is older than this many hours since the base session started, bounding total conversation lifetime across epochs.",
	"session.rotation.ceilingTokens": "Hard token ceiling for long-lived sessions that cannot rotate (e.g. `agent:<id>:main`). When the estimated context reaches this, admission forces a compaction cycle instead of riding the window; fail open if compaction is unavailable. Leave unset for engine-driven behavior only.",
	"session.rotation.progressFloorTokens": "Target context size below the ceiling that a forced ceiling compaction settles toward. Leave unset to use the provider/engine default floor.",
...
const CORE_FIELD_HELP = {
	...
	"update.checkOnStart": "Checks the OpenClaw update endpoint when the gateway starts, including read-only extended-stable hints (default: true). Set false to disable update checks and anonymous update pings.",
	...
	"gateway.roles.definitions.*": "One named operator role. Every definition must explicitly provide its session-sharing policy, allowed session and run agents, and operator-scope ceiling, and can require sandbox isolation for newly created sessions.",
	...
	"gateway.tailscale.preserveFunnel": "Deprecated migration guard for mode='serve'. If an external Funnel still targets the ordinary Gateway listener, OpenClaw leaves exposure unchanged and warns that only plugin-authenticated webhooks remain usable until password auth is configured and mode is migrated to 'funnel'.",
	...
	"agents.defaults.systemAgent": "Target settings for ambient OpenClaw system-agent and Custodian inference plus selected unscoped operator reads.",
...
const RUNTIME_FIELD_HELP = {
	...
	"tools.updatePlan": "Unified `progress_card` status tool for durable plans and narrative notes. Enabled by default; set false to opt out.",
	...
	"gateway.controlUi.github.token": "SecretRef-backed service credential for Control UI GitHub previews and project discovery. Prefer explicit configuration for clear service ownership. Omit it to retain the GH_TOKEN/GITHUB_TOKEN fallback from the shared Gateway process environment. An explicitly configured but unavailable credential fails closed. Agent tool identities are never used here.",
	...
	"gateway.controlUi.automaticallyFetchFavicons": "Fetch link favicons through the Gateway (default on). The Gateway requests only HTTPS /favicon.ico from public destinations, applies strict SSRF checks to every DNS result and redirect, and validates bounded image bytes. Set false to prevent all favicon route requests and destination fetches.",
	...
	"nodeHost.workerRuns": "Opt in to full OpenClaw worker session hosting from Gateway-managed bundles. Disabled by default.",
	...
	"nodeHost.workerRuns.containerImage": "Optional Node 22+ image for container-isolated workers (default: \"node:22-slim\"). Use a digest-pinned, private-registry, or preloaded image when needed; missing images are pulled on first use.",
