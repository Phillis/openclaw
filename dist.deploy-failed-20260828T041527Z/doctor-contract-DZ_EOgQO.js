...
import { c as stripRetiredChannelKeys, i as defineChannelAliasMigration, l as hasLegacyAccountStreamingAliases, n as defineKeyMoveMigration, o as normalizeChannelAccounts } from "./runtime-doctor-migrations-D-k1ye_X.js";
import { a as normalizeRealtimeVoiceActivationNamePrefix, n as isSupportedRealtimeVoiceActivationName } from "./activation-name-DlfylsUE.js";
import "./realtime-voice-activation-S48Yty8C.js";
...
const streamingAliasMigration = defineChannelAliasMigration({
	...
	normalizeAccountExtra: ({ account, pathPrefix, changes }) => {
		const accountVoice = asNullableRecord(account.voice);
		if (!accountVoice || !migrateLegacyTtsConfig(asNullableRecord(accountVoice.tts), `${pathPrefix}.voice.tts`, changes)) return {
			entry: account,
			changed: false
		};
		return {
...
function hasLegacyTtsProviderKeys(value) {
	const tts = asNullableRecord(value);
	if (!tts) return false;
...
function hasLegacyDiscordGuildChannelAgentId(value) {
	const guilds = asNullableRecord(asNullableRecord(value)?.guilds);
	if (!guilds) return false;
	return Object.values(guilds).some((guildValue) => {
		const channels = asNullableRecord(asNullableRecord(guildValue)?.channels);
		if (!channels) return false;
		return Object.values(channels).some((channel) => Object.hasOwn(asNullableRecord(channel) ?? {}, "agentId"));
	});
}
function hasUnsupportedRealtimeWakeNamesInVoice(value) {
	const wakeNames = asNullableRecord(asNullableRecord(value)?.realtime)?.wakeNames;
...
function getOrCreateTtsProviders(tts) {
	const providers = asNullableRecord(tts.providers) ?? {};
	tts.providers = providers;
	return providers;
}
function mergeLegacyTtsProviderConfig(tts, legacyKey, providerId) {
	const legacyValue = asNullableRecord(tts[legacyKey]);
	if (!legacyValue) return false;
	const providers = getOrCreateTtsProviders(tts);
	const existing = asNullableRecord(providers[providerId]) ?? {};
...
function normalizeUnsupportedRealtimeWakeNames(entry, pathPrefix, changes) {
	const voice = asNullableRecord(entry.voice);
	const realtime = asNullableRecord(voice?.realtime);
...
function isDiscordChannelAgentBinding(value, match) {
	const binding = asNullableRecord(value);
	const bindingMatch = asNullableRecord(binding?.match);
	const peer = asNullableRecord(bindingMatch?.peer);
...
function normalizeDiscordGuildChannelAgentIds(params) {
	...
	for (const [guildId, guildValue] of Object.entries(guilds)) {
		const guild = asNullableRecord(guildValue);
		const channels = asNullableRecord(guild?.channels);
		if (!guild || !channels) continue;
		let channelsChanged = false;
		const nextChannels = { ...channels };
		for (const [channelId, channelValue] of Object.entries(channels)) {
			const channel = asNullableRecord(channelValue);
...
const legacyConfigRules = [
	...
	},
	{
		...
		match: (value) => hasLegacyAccountStreamingAliases(value, (accountValue) => {
			return hasLegacyTtsProviderKeys(asNullableRecord(asNullableRecord(accountValue)?.voice)?.tts);
		})
	},
	{
...
function normalizeCompatibilityConfig({ cfg }) {
	...
	const rawEntry = asNullableRecord(tuningKnobs.config.channels?.discord);
	if (!rawEntry) return {
		config: cfg,
		changes: []
	};
	...
	const voice = asNullableRecord(updated.voice);
	if (voice && migrateLegacyTtsConfig(asNullableRecord(voice.tts), "channels.discord.voice.tts", changes)) {
		updated = {
			...updated,
			voice
		};
