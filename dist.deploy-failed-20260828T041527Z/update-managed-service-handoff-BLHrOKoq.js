...
import { m as resolveGatewayWindowsTaskName } from "./constants-ChqKLfPp.js";
import { n as resolvePreferredOpenClawTmpDir } from "./tmp-openclaw-dir-DnyL0lW9.js";
import { Mn as executeSqliteQueryTakeFirstSync, Nn as getNodeSqliteKysely, Xt as resolveOpenClawStateSqlitePath } from "./openclaw-state-db-kmBThqu6.js";
import { i as resolveNodeSqliteLocation, t as openNodeSqliteDatabase } from "./node-sqlite-_e3IvfT7.js";
import { i as resolveExecutableFromPathEnv } from "./executable-path-HS2Pej6k.js";
import { i as isPidDefinitelyDead, r as isPidAlive, t as getFileLockProcessStartTime } from "./pid-alive-BcyyC-CC.js";
import { d as resolveSystemdServiceName, n as findInstalledSystemdGatewayScope } from "./systemd-scope-Dt6qzIxA.js";
import { t as SUPERVISOR_HINT_ENV_VARS } from "./supervisor-markers-BXjiMLrU.js";
import { f as resolveLaunchAgentLabel } from "./launchd-system-4KAEGLKr.js";
import { o as resolveLaunchAgentPlistPath } from "./launchd-service-files-B8DZVvug.js";
import { t as forceKillChildProcessTree } from "./child-process-tree-DO3eGAOe.js";
import { h as resolveUpdateInstallRoot } from "./restart-sentinel-DedQJXFM.js";
import { r as CONTROL_PLANE_UPDATE_SENTINEL_META_ENV } from "./update-control-plane-sentinel-CjDvg0VZ.js";
import { n as applyDevUpdateTargetEnv } from "./update-dev-target-BGSFRuDr.js";
...
function resolveUpdateCliArgv(params) {
	...
	if (execPath && !/^(?:node|bun)(?:\.exe)?$/iu.test(path.basename(execPath))) return [execPath, ...updateArgs];
	return ["openclaw", ...updateArgs];
}
function formatManagedServiceUpdateCommand(params) {
	return resolveUpdateCliArgv(params ?? {}).toSpliced(3, 1).join(" ");
}
...
function waitForHandoffResponse(child, command) {
	return new Promise((resolve, reject) => {
		...
		const onInputClose = () => finish(/* @__PURE__ */ new Error("managed update handoff control input closed"));
		const onData = (chunk) => {
			buffered = `${buffered}${chunk.toString()}`.slice(-1024);
			const newline = buffered.indexOf("\n");
			if (newline >= 0) finish(buffered.slice(0, newline));
		};
		const timeout = setTimeout(() => {
			onOutputError(/* @__PURE__ */ new Error(`managed update handoff did not ${command ? "respond" : "signal readiness"} within 30 seconds`));
...
async function spawnManagedServiceUpdateHandoff(params, rootIdentity, owner) {
	...
	if (parentStartIdentity === null) throw new Error("managed update parent process start identity is unavailable");
	...
	if (params.supervisor === "systemd") {
		...
		spawnArgs.unshift("--user", "--scope", "--collect", `--unit=openclaw-update-${suffix}.scope`, spawnCommand);
		spawnCommand = systemdRun;
	}
	const stateDatabasePath = resolveOpenClawStateSqlitePath(serviceEnv);
	const parentExitTimeoutMs = Math.min(2147483647, Math.max(0, params.restartDelayMs ?? 0) + Math.max(0, params.restartDrainTimeoutMs) + PARENT_EXIT_SHUTDOWN_RESERVE_MS);
	const helperParams = {
		parentPid,
	...
	try {
		...
		if (`${readiness}\n` === HANDOFF_READY_MARKER) {
			...
			if (helper?.owner !== params.handoffId || !isPidAlive(helper.pid) || getFileLockProcessStartTime(helper.pid)?.toString() !== helper.startIdentity) {
				forceKillChildProcessTree(child);
...
async function startManagedServiceUpdateHandoff(params) {
	...
	if (params.supervisor === "systemd" && (await findInstalledSystemdGatewayScope(params.env ?? process.env))?.scope === "system") throw new Error("Managed update handoff requires a user-scope systemd unit; perform a manual system-service update.");
	...
	const owner = { handoffId: params.handoffId ?? randomUUID() };
...
function readManagedServiceUpdateHandoffLease(root, stale) {
	...
	try {
		...
		const lease = getNodeSqliteKysely(db);
		const row = executeSqliteQueryTakeFirstSync(db, lease.selectFrom("managed_update_handoffs").select(["owner", "payload_json"]).where("install_root", "=", root));
		if (!row) return null;
		const payload = JSON.parse(row.payload_json);
		if (!isRecord(payload) || Object.keys(payload).length !== 3 || payload.version !== 1 || typeof payload.pid !== "number" || !Number.isInteger(payload.pid) || payload.pid <= 0 || typeof payload.startIdentity !== "string" || !payload.startIdentity) return;
		const current = {
			owner: row.owner,
			pid: payload.pid,
		...
		if (current.owner !== stale.handoffId || current.pid !== stale.helper?.pid || current.startIdentity !== stale.helper.startIdentity || !isPidDefinitelyDead(current.pid) && (observedStart === null || String(observedStart) === current.startIdentity)) return current;
		return executeSqliteQueryTakeFirstSync(db, lease.deleteFrom("managed_update_handoffs").where("install_root", "=", root).where("owner", "=", current.owner).where("payload_json", "=", row.payload_json).returning("owner")) ? null : void 0;
	} catch {
		return;
	} finally {
		db?.close();
...
function sendManagedServiceUpdateHandoffCommand(identity, command) {
	const child = activeManagedServiceUpdateHandoffs.get(resolveUpdateInstallRoot(identity.installRoot))?.launcher;
	if (!child?.stdin || !child.stdout || child.stdin.destroyed) return Promise.resolve(null);
	return waitForHandoffResponse(child, command).catch(() => null);
}
async function requestManagedServiceUpdateHandoffPark(identity) {
	return claimManagedServiceUpdateHandoff(identity) && await sendManagedServiceUpdateHandoffCommand(identity, "park") === "parked" && claimManagedServiceUpdateHandoff(identity);
}
...
async function cancelManagedServiceUpdateHandoff(identity) {
	const root = resolveUpdateInstallRoot(identity.installRoot);
	const active = activeManagedServiceUpdateHandoffs.get(root);
	if (identity.kind !== "managed-update-handoff" || active?.handoffId !== identity.handoffId || active.cancelling) return false;
...
//#endregion
export { formatManagedServiceUpdateCommand as a, commitManagedServiceUpdateHandoff as i, cancelManagedServiceUpdateHandoff as n, requestManagedServiceUpdateHandoffPark as o, claimManagedServiceUpdateHandoff as r, startManagedServiceUpdateHandoff as s, buildManagedServiceHandoffUnavailableMessage as t };
