...
import { a as resolveSharedAuthStorePath, i as resolveSharedAuthStoreOwnership, o as resolveSharedMainAuthAgentDir } from "./path-resolve-Bjd7UUgA.js";
...
import { S as hasMatchingOAuthIdentity, a as loadPersistedAuthProfileStore, c as parseLegacyCredentialEntry, i as coercePersistedAuthProfileStore, o as loadPersistedSharedAuthProfileStore, r as coerceLegacyAuthStore, t as applyLegacyAuthStore, u as coerceAuthProfileState, x as areOAuthCredentialsEquivalent } from "./persisted-Bjx2XcL3.js";
import { t as loadJsonFileThroughSymlink } from "./json-file-DMm8gT_r.js";
import { C as listLegacyAuthProfileArchives, T as resolveLegacyOAuthPath, a as inspectPersistedAuthProfileStoreRaw, c as readPersistedAuthProfileStateRaw, h as runAuthProfileWriteTransaction, i as inspectPersistedAuthProfileStateRaw, m as resolveAuthProfileDatabasePath, o as inspectPersistedSharedAuthProfileStateRaw, s as inspectPersistedSharedAuthProfileStoreRaw, u as readPersistedSharedAuthProfileStateRaw } from "./sqlite-BsvahKHa.js";
import { i as clearAuthProfileMigrationDiagnostics } from "./legacy-source-diagnostic-SYdrQUmo.js";
import { E as isInheritedMainOAuthCredentialFromStores, v as saveAuthProfileStore } from "./store-C6iqqcJy.js";
import { n as clearRuntimeAuthProfileStoreSnapshots } from "./runtime-snapshots-a_60jBeK.js";
import { t as note } from "./note-YH_0kY-3.js";
import { i as resolveLegacyFlatAuthPath, n as resolveLegacyAuthProfilesPath, r as resolveLegacyAuthStatePath, t as listAuthProfileRepairCandidates } from "./doctor-auth-legacy-paths-CVFtjFfF.js";
import { a as recordLegacyMigrationRun, n as readLegacyMigrationReceipt, o as recordLegacyMigrationSource } from "./state-migrations.receipts-ImZIzoAI.js";
...
function projectSharedStoreCell(row) {
	return row ? { store_json: row.value_json } : void 0;
}
function projectSharedStateCell(row) {
	return row ? { state_json: row.value_json } : void 0;
}
function verifyAuthProfileMigrationTarget(receipt) {
	...
	try {
		...
		if (hasExpectedProfiles && receipt.expectedProfileSha256) {
			const row = targetStoreKey === "shared" ? projectSharedStoreCell(executeSqliteQueryTakeFirstSync(db, getNodeSqliteKysely(db).selectFrom("config_machine_state").select("value_json").where("state_key", "=", "authProfiles.store"))) : executeSqliteQueryTakeFirstSync(db, getNodeSqliteKysely(db).selectFrom("auth_profile_store").select("store_json").where("store_key", "=", "primary"));
		...
		if (receipt.expectedStateSha256) {
			const row = targetStoreKey === "shared" ? projectSharedStateCell(executeSqliteQueryTakeFirstSync(db, getNodeSqliteKysely(db).selectFrom("config_machine_state").select("value_json").where("state_key", "=", "authProfiles.state"))) : executeSqliteQueryTakeFirstSync(db, getNodeSqliteKysely(db).selectFrom("auth_profile_state").select("state_json").where("state_key", "=", "primary"));
	...
	for (const candidate of detected) {
		...
		try {
			...
			if (canonicalStore || configCanonicalStore || legacyStore || hasAuthProfileState(state)) {
				...
				try {
					assertAuthProfileMigrationSourcesUnchanged(candidate, sourceReceipts);
					verifiedStore = runAuthProfileWriteTransaction(transactionAgentDir, (database) => {
