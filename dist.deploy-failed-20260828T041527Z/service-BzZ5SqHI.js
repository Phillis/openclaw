...
import { h as validateCloudWorkerProfileSettings } from "./zod-schema-D4cp1qYW.js";
...
import { qg as WorkerMachineOptionsSchema } from "./src-4dv5TpeQ.js";
import { v as runWithGatewayIndependentRootWorkContinuation } from "./gateway-work-admission-CTDt7IQ1.js";
import { u as onSessionIdentityMutation } from "./session-history-eviction-DX5U9ZnW.js";
import "./session-accessor-fcDZuc2H.js";
import { a as WORKER_PROTOCOL_MAX_INFERENCE_PAYLOAD_BYTES, g as validateWorkerInferenceTerminalOutcome, h as validateWorkerInferenceTerminalFrame, p as validateWorkerInferenceEventFrame } from "./worker-inference-BzU_LUo9.js";
...
import { _ as sameWorkerSessionTurnClaim, v as serializeWorkerSessionTurnClaim } from "./placement-record-nLiaHmTd.js";
import { n as registerWorkerInferenceSessionDrain } from "./inference-control-internal-CQh2JfZC.js";
import { a as validateWorkerConnectionIdentity, c as createWorkerCredentialMaterial, l as hashWorkerCredential, n as StaleWorkerBuildError, o as verifyWorkerAdmissionHandshake, r as admitWorkerConnection, t as STALE_WORKER_BUILD_REASON } from "./admission-CzKwSq5g.js";
import { t as deriveEnvironmentIntent } from "./service-contract-DRAD5TG3.js";
import { i as normalizeWorkerSshEndpoint, r as normalizeWorkerDesktopEndpoint, t as WorkerSessionAlreadyAttachedError } from "./store-DO98-g2c.js";
	...
	const startTunnel = async (request) => {
		...
		await withLock(request.environmentId, async () => {
			...
			if (typeof nodeDeviceId === "string" && !record.sshEndpoint && record.bootstrapReceipt.installKind === "bundle") {
				...
				if (!nodeTunnels || !sessionId) throw serviceError("invalid_state", "Node worker tunnel runtime is unavailable");
...
function createWorkerNodeProvisioning(options) {
	return async (record, lease, provider, patch) => {
		const nodePatch = {
			...patch,
...
function createWorkerProviderLifecycle(options) {
	const { store, callBootstrap, callProvider, inState, move, saveError, serviceError, withLock } = options;
	...
	const finishProvision = async (record, provider, preparedInstallation) => {
		...
		try {
			...
			if (executionMode && !provider.supportedExecutionModes?.includes(executionMode)) throw new WorkerProviderError(`Worker provider ${provider.id} does not support ${executionMode} placement`);
			...
			if (provider.requiresNodeEnrollment === true && !prepareNodeEnrollment) throw new Error("Worker node enrollment runtime is unavailable");
			const provisionOptions = machineClass || executionMode || provider.requiresNodeEnrollment === true ? {
				...machineClass ? { machineClass } : {},
				...executionMode ? { executionMode } : {},
				...provider.requiresNodeEnrollment === true && prepareNodeEnrollment ? { beginNodeEnrollment: async () => await prepareNodeEnrollment(record) } : {}
			} : void 0;
			lease = requireWorkerLease(await callProvider(record.environmentId, () => provider.provision(profile, record.provisionOperationId, provisionOptions), providerTimeoutMs));
		} catch (error) {
		...
		if (record.destroyRequestedAtMs !== null) return move(record, "draining", patch);
		if (lease.node) return await finishNodeProvisioning(record, lease, provider, patch);
	...
	const finishDestroy = async (r, provider) => {
		...
		const stopped = await stopOwner(r, providerOwnsMachine ? "provider-destroying" : void 0);
	...
	const createWithProfile = async (profileId, idempotencyKey, createOptions = {}) => {
...
function createWorkerTurnRpc(options) {
	...
	const executeSessionTool = async (identity, toolName, request, signal) => {
		...
		const operation = toolName === "sessions_spawn" && "task" in request ? {
			toolName,
			request
...
function createWorkerEnvironmentService(options) {
	...
	const reconcileEnvironmentCore = async (environmentId) => {
		if (stopping) return;
	...
	const service = {
		...
		create: async (profileId, idempotencyKey, machineClass, executionMode) => {
		...
		createFromProfileSnapshot: async (profile, idempotencyKey, machineClass, executionMode) => {
