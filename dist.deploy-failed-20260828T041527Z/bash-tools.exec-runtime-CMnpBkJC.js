...
import { r as emitDiagnosticEventWithTrustedTraceContext } from "./diagnostic-events-BGzDm6gu.js";
...
import { n as createStreamingBinaryOutputSanitizer, o as getShellConfig } from "./shell-utils-DAgUwgg-.js";
import { a as renderExecUpdateText, n as appendExecTimeoutRetryGuidance, r as renderExecExitLabel } from "./bash-tools.exec-output-D1Z0C3oY.js";
import { C as chunkString, O as readEnvInt, b as tail, f as isProcessSessionIdTaken, g as markExited, n as addSession, r as appendOutput, v as recordNotifyOnExitRemoval, w as clampWithDefault } from "./bash-process-registry-08adq0zn.js";
...
import { P as DEFAULT_EXEC_APPROVAL_TIMEOUT_MS, j as resolveExecApprovalAllowedDecisions } from "./exec-approvals-PtbcLeQo.js";
import { i as scopedHeartbeatWakeOptionsForPolicy, t as resolveEventSessionKeyForPolicy } from "./event-session-routing-BivtcMJz.js";
import "./bash-tools.schemas-B7Xq1jzd.js";
import { a as removePathPrepend, n as findPathKey, r as mergePathPrepend } from "./path-prepend-Up5PqO7Z.js";
import { n as createSessionSlug } from "./session-slug-DUj8bbny.js";
...
function resolveExecTarget(params) {
	...
	if (sandboxRequired && !params.sandboxAvailable) throw new Error("This session requires a sandbox, but its sandbox runtime is unavailable.");
	if (sandboxRequired && params.elevatedRequested) throw new Error("Elevated execution is unavailable because this session requires a sandbox.");
	...
	if (sandboxRequired && (requestedTarget === "gateway" || requestedTarget === "node")) throw new Error(`exec host not allowed (requested ${renderExecTargetLabel(requestedTarget)}; this session requires a sandbox).`);
...
function maybeNotifyOnExit(session, status) {
	...
	const remove = enqueueSystemEventWithReceipt(eventText, eventSessionKey === "global" && session.agentId ? withSystemEventOwner(eventOptions, session.agentId) : eventOptions, { allowDuplicate: true });
	...
	if (!isSubagentSessionKey(sessionKey)) {
		...
		requestHeartbeat(sessionKey === "global" && session.agentId ? {
			...wakeOptions,
...
function formatExecFailureReason(params) {
	switch (params.failureKind) {
		...
		case "overall-timeout": {
			const retryGuidance = appendExecTimeoutRetryGuidance(typeof params.timeoutSec === "number" && params.timeoutSec > 0 ? `Command timed out after ${params.timeoutSec} seconds.` : "Command timed out.", params.failureKind);
...
async function runExecProcess({ startupSignal: initialStartupSignal, onUpdate: initialOnUpdate, beforeSpawn: initialBeforeSpawn, onSettledBeforeNotify: initialOnSettledBeforeNotify, ...opts }) {
